/**CFile****************************************************************
  ...
  Revision    [$Id: mvcUtils.c,v 1.16 2003/11/18 18:55:05 alanmi Exp $]
...
***********************************************************************/
bool Mvc_CoverIsCubeFree( Mvc_Cover_t * pCover )
{
    bool Result;
    // get the common cube
...
***********************************************************************/
bool Mvc_CoverCheckSuppContainment( Mvc_Cover_t * pCover1, Mvc_Cover_t * pCover2 )
{
    bool Result;
...
{
    Mvc_Cube_t * pCube;
    uint8 * pByte, * pByteStart, * pByteStop;
    int nBytes, nOnes;

    // get the number of unsigned chars in the cube's bit strings
    nBytes = pCover->nBits / (8 * sizeof(uint8)) + (int)(pCover->nBits % (8 * sizeof(uint8)) > 0);
...
int Mvc_CoverGetCubeSize( Mvc_Cube_t * pCube )
{
    ...
    nBytes = nBits / (8 * sizeof(uint8)) + (int)(nBits % (8 * sizeof(uint8)) > 0);
    // clean the counter of ones
    nOnes = 0;
    // set the starting and stopping positions
    pByteStart = (uint8 *)pCube->pData;
