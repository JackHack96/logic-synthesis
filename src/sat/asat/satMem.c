/**CFile****************************************************************
  ...
  Revision    [$Id: satMem.c,v 1.1 2005/07/08 01:01:27 alanmi Exp $]
...
***********************************************************************/
Sat_MmFixed_t * Sat_MmFixedStart( int nEntrySize, int nChunkSize, int nChunksAlloc )
{
    Sat_MmFixed_t * p;
...
***********************************************************************/
Sat_MmFlex_t * Sat_MmFlexStart( int nChunkSize, int nChunksAlloc )
{
    Sat_MmFlex_t * p;
...
Sat_MmStep_t * Sat_MmStepStart( int nSteps )
{
    ...
    for ( i = 0; i < p->nMems; i++ )
        p->pMems[i] = Sat_MmFixedStart( (8<<i), (16<<(p->nMems-i)), 100 );
    // set up the mapping of the required memory size into the corresponding manager
    p->nMapSize = (4<<p->nMems)+1;
    p->pMap = ALLOC( Sat_MmFixed_t *, p->nMapSize );
    p->pMap[0] = NULL;
    for ( k = 1; k <= 8; k++ )
        p->pMap[k] = p->pMems[0];
    for ( i = 1; i < p->nMems; i++ )
