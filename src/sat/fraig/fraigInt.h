...
#define FRAIG_SIM_ROUNDS     255   // the number of simulation round (each round = 32 bits)
// when changing this number, make sure that it does not exceed the number 
// of primes (FRAIG_MAX_PRIMES) used for hashing 
...
struct Fraig_ManStruct_t_
{
    ...
    int                   fUseImps;      // enables the use of implications
    ...
    Fraig_Sims_t *        pSimsReal;     // used for simulation patterns
    Fraig_Sims_t *        pSimsDiff;     // used for simulation patterns
    Fraig_Sims_t *        pSimsTemp;     // used for simulation patterns
    Fraig_Sims_t *        pVarSims[12];
...
struct Fraig_SimsStruct_t_
{
    unsigned              uHash;         // the hash value 
    unsigned              uTests[FRAIG_SIM_ROUNDS]; // the simulation info
};
...
// iterator through the dominators of the node
#define Fraig_NodeForEachDominator( pNode, pDom )\
    for ( pDom = pNode->pDoms; pDom; pDom = pDom->pNext )
...
/*=== fraigGc.c =============================================================*/
extern void                Fraig_TableReclaimNode( Fraig_Man_t * pMan, Fraig_Node_t * gNode );
extern void                Fraig_ManagerGarbageCollect( Fraig_Man_t * pMan );
extern void                Fraig_CheckCorrectRefs( Fraig_Man_t * p );
/*=== fraigImp.c =============================================================*/
extern void                Fraig_ManProveSomeImplications( Fraig_Man_t * pMan, Fraig_Node_t * pRoot );

/*=== fraigMem.c =============================================================*/
extern Fraig_MemFixed_t *  Fraig_MemFixedStart( int nEntrySize, int nChunkSize, int nChunksAlloc );
...
extern Fraig_Node_t *      Fraig_CofactorTwo( Fraig_Man_t * p, Fraig_Node_t * pFunc, Fraig_Node_t * pVar1, Fraig_Node_t * pVar2 );
extern Fraig_Node_t *      Fraig_Cofactor( Fraig_Man_t * p, Fraig_Node_t * pFunc, Fraig_Node_t * pVar );
extern Fraig_Node_t *      Fraig_Transfer( Fraig_Man_t * pManS, Fraig_Man_t * pManD, Fraig_Node_t * pNode, int * pVarMap );

/*=== fraigPrint.c ===============================================================*/
extern void                Fraig_PrinNodetList( Fraig_Man_t * pMan, Msat_IntVec_t * vNodes );
extern void                Fraig_PrintNodesSupps( Fraig_Man_t * p, Fraig_Node_t * pOld, Fraig_Node_t * pNew );
extern void                Fraig_PrintChoiceNodeLevels( Fraig_Man_t * p );
extern void                Fraig_PrintLevelStats( Fraig_Man_t * pMan );
extern void                Fraig_PrintChoicingStats( Fraig_Man_t * p );
extern void                Fraig_PrintNode( Fraig_Man_t * p, Fraig_Node_t * pNode );
...
/*=== fraigResub.c ===============================================================*/
extern int                 Fraig_NodeResubstitute( Fraig_Man_t * pMan, Fraig_Node_t * pNode, Fraig_Node_t ** ppNode1, Fraig_Node_t ** ppNode2 );
