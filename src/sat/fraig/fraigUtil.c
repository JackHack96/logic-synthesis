...
void Fraig_PrintNode( Fraig_Man_t * p, Fraig_Node_t * pNode )
{
    ...
    for ( i = 0; i < vNodes->nSize; i++ )
    {
        ...
        if ( Fraig_NodeIsVar(pTemp) )
        {
            ...
            Fraig_PrintBinary( stdout, (unsigned *)&pTemp->pSimsR->uTests, 20 );
            printf( "   " );
            Fraig_PrintBinary( stdout, (unsigned *)&pTemp->pSimsD->uTests, 20 );
            printf( "  %d\n", pTemp->fInv );
            continue;
        }
...
***********************************************************************/
int Fraig_ManUpdateLevel_rec( Fraig_Man_t * pMan, Fraig_Node_t * pNode )
{
    Fraig_Node_t * pTemp;
    ...
    Level1 = Fraig_ManUpdateLevel_rec( pMan, Fraig_Regular(pNode->p1) );
    Level2 = Fraig_ManUpdateLevel_rec( pMan, Fraig_Regular(pNode->p2) );
    pNode->Level = 1 + FRAIG_MAX( Level1, Level2 );
    if ( pNode->pNextE )
    {
        LevelE = Fraig_ManUpdateLevel_rec( pMan, pNode->pNextE );
...
void Fraig_ManReportChoices( Fraig_Man_t * pMan )
{
    ...
    pMan->nTravIds++;
    for ( i = 0; i < pMan->vOutputs->nSize; i++ )
        Fraig_ManUpdateLevel_rec( pMan, Fraig_Regular(pMan->vOutputs->pArray[i]) );
...
int Fraig_NodeSimsContained( Fraig_Man_t * pMan, Fraig_Node_t * pNode1, Fraig_Node_t * pNode2 )
{
    ...
    pUnsigned1 = pNode1->pSimsR->uTests;
    pUnsigned2 = pNode2->pSimsR->uTests;
    for ( i = 0; i < FRAIG_SIM_ROUNDS; i++ )
        if ( pUnsigned1[i] & ~pUnsigned2[i] )
            return 0;

    // compare systematic siminfo
    pUnsigned1 = pNode1->pSimsD->uTests;
    pUnsigned2 = pNode2->pSimsD->uTests;
...
int Fraig_ManPrintRefs( Fraig_Man_t * pMan )
{
    ...
    for ( i = 0; i < pMan->vNodes->nSize; i++ )
    {
        ...
        if ( pNode->nOnes == 0 || pNode->nOnes == FRAIG_SIM_ROUNDS * 32 )
            continue;
