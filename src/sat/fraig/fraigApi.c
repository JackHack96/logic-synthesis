...
int               Fraig_ManReadRefCount( Fraig_Man_t * p )                    { return p->fRefCount;  }
int               Fraig_ManReadChoicing( Fraig_Man_t * p )                    { return p->fChoicing;  }
int               Fraig_ManReadVerbose( Fraig_Man_t * p )                     { return p->fVerbose;   }
int               Fraig_ManReadSimRounds( Fraig_Man_t * p )                   { return FRAIG_SIM_ROUNDS; }
...
void            Fraig_ManSetRefCount( Fraig_Man_t * p, int fRefCount )      { p->fRefCount = fRefCount; }
...
unsigned *      Fraig_NodeReadSimInfo( Fraig_Node_t * p )                 { return p->pSimsR->uTests;  }
