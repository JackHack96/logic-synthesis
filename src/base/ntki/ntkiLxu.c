/**CFile****************************************************************
  ...
  Revision    [$Id: ntkiLxu.c,v 1.2 2004/01/30 02:04:23 satrajit Exp $]
...
static void Ntk_NetworkLxCollectInfo( Ntk_Network_t * pNet, Lxu_Data_t * p );
static void Ntk_NetworkLxSetupLiterals( Ntk_Network_t * pNet, Ntk_Node_t * pNode, Mvc_Cover_t * pCover );
static void Ntk_NetworkLxReconstruct( Ntk_Network_t * pNet, Lxu_Data_t * p );
static void Ntk_NetworkLxReconstructNode( Ntk_Node_t * pNodeNew, Ntk_Node_t ** ppNodes, Lxu_Data_t * p, int iValue );
...
/**Function*************************************************************
  ...
  Synopsis    [Performs layout_fast_extract on binary or MV network.]
...
***********************************************************************/
bool Ntk_NetworkLayoutFastExtract( Ntk_Network_t * pNet, Lxu_Data_t * p )
{
    ...
    Ntk_NetworkLxCollectInfo( pNet, p );

    // call the fast extract procedure
    // returns the number of divisor extracted
    if ( Lxu_FastExtract(p) )
    {
        // update the network
        Ntk_NetworkLxReconstruct( pNet, p );
...
***********************************************************************/
void Ntk_NetworkLxCollectInfo( Ntk_Network_t * pNet, Lxu_Data_t * p )
{
    Ntk_Node_t * pNode;
    Mvc_Cover_t ** ppIsets;
...
//    p->nNodesExt  is already assigned
	...
	p->pValue2Type  = ALLOC( int , p->nValuesAlloc + 1 ); // the node of each value	
    ...
    Ntk_NetworkForEachNode( pNet, pNode )
    {
        ...
        for ( i = 0; i < pNode->nValues; i++ )
        {
            ...
            if ( ppIsets[i] )
            {
                ...
                Ntk_NetworkLxSetupLiterals( pNet, pNode, ppIsets[i] );
            }
        }
        // increment
        iNode++;
...
***********************************************************************/
void Ntk_NetworkLxFreeInfo( Lxu_Data_t * p )
{
    FREE( p->pNode2Value );
    FREE( p->pValue2Node );
    FREE( p->ppCovers );
...
***********************************************************************/
void Ntk_NetworkLxSetupLiterals( Ntk_Network_t * pNet, Ntk_Node_t * pNode, Mvc_Cover_t * pCover )
{
    Ntk_Node_t * pFanin;
    Ntk_Pin_t * pPin;
    int iValue, i;
...
***********************************************************************/
void Ntk_NetworkLxReconstruct( Ntk_Network_t * pNet, Lxu_Data_t * p )
{
    Ntk_Node_t ** ppNodes;
    ...
    Ntk_NetworkForEachNode( pNet, pNode )
    {
        ...
        Ntk_NetworkLxReconstructNode( pNodeNew, ppNodes, p, iValue );
        // update the node
...
***********************************************************************/
void Ntk_NetworkLxReconstructNode( Ntk_Node_t * pNodeNew, Ntk_Node_t ** ppNodes, Lxu_Data_t * p, int iValue )
{
    Cvr_Cover_t * pCvr;
    Vm_VarMap_t * pVm;
    Mvc_Cover_t ** ppIsets;
