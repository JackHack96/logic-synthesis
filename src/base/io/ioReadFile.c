/**CFile****************************************************************
  ...
  Revision    [$Id: ioReadFile.c,v 1.21 2004/10/19 23:54:54 satrajit Exp $]
...
static void   Io_ReadFileLineClean( char * pLineBeg, char * pLineEnd );
static void   Io_ReadFileLineAdd( Io_Read_t * p, char * pLine );
...
***********************************************************************/
int Io_ReadFileMarkLinesAndDots( Io_Read_t * p )
{
    char * pCur;
    ...
    // add the first line
    Io_ReadFileLineAdd( p, p->pFileContents );
    // add other lines
    for ( pCur = p->pFileContents; *pCur; pCur++ )
        if ( *pCur == '\n' && *(pCur + 1) != '\0' ) // the next line exists
            Io_ReadFileLineAdd( p, pCur + 1 );
    ...
    for ( i = 0; i < p->nLines - 1; i++ )
        Io_ReadFileLineClean( p->pLines[i], p->pLines[i+1] - 1 );
    ...
    if ( p->nDotNodes )
        p->pIoNodes = ALLOC( Io_Node_t, p->nDotNodes );      
    if ( p->nDotMvs )
        p->pDotMvs = ALLOC( int, p->nDotMvs );      
    if ( p->nDotLatches )
        p->pIoLatches = ALLOC( Io_Latch_t, p->nDotLatches ); 
    if ( p->nDotInputArr )
        p->pDotInputArr = ALLOC( int, p->nDotInputArr ); 
    return 0;
}
...
***********************************************************************/
char * Io_ReadFileFileContents( char * FileName, int * pFileSize )
{
    FILE * pFile;
    int nFileSize;
    char * pBuffer;
...
***********************************************************************/
void Io_ReadFileLineClean( char * pLineBeg, char * pLineEnd )
{
    char * pTemp;
...
***********************************************************************/
void Io_ReadFileLineAdd( Io_Read_t * p, char * pLine )
{
    ...
    if ( *pLine == '.' )
    {
        ...
        else if ( pLine[1] == 'i' && pLine[2] == 'n' && pLine[3] == 'p' && pLine[4] == 'u' && pLine[5] == 't' && pLine[6] == 's' ) 
            p->nDotInputs++;
        else if ( pLine[1] == 'i' && pLine[2] == 'n' && pLine[3] == 'p' && pLine[4] == 'u' && pLine[5] == 't' && pLine[6] == '_' && pLine[7] == 'a' ) 
            p->nDotInputArr++;
        else if ( pLine[1] == 'o' && pLine[2] == 'u' && pLine[3] == 't' && pLine[4] == 'p' && pLine[5] == 'u' && pLine[6] == 't' && pLine[7] == 's' ) 
            p->nDotOutputs++;
    }
    // save this line
...
***********************************************************************/
char * Io_ReadFileGetSimilar( char * pFileNameWrong, char * pS1, char * pS2, char * pS3, char * pS4, char * pS5 )
{
    FILE * pFile;
    char * pFileNameOther;
    char * pFileGen;
