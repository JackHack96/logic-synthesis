/**CFile****************************************************************
  ...
  Revision    [$Id: aioReadLines.c,v 1.3 2004/04/08 04:48:24 alanmi Exp $]
...
static int         Aio_ReadLinesDotMv( Aio_Read_t * p, int Line );
static Aio_Var_t * Aio_ReadLinesDotMvCopyVar( Aio_Var_t * pVar );
static char *      Aio_ReadLinesDotMvPivot( char * pStr );
static int         Aio_ReadLinesDotLatch( Aio_Read_t * p, Aio_Latch_t * pIoLatch );
static int         Aio_ReadLinesDotNode( Aio_Read_t * p, Aio_Node_t * pIoNode );
static int         Aio_ReadLinesDotRes( Aio_Read_t * p, int Line );
static char *      Aio_ReadLinesLastEntry( char * pStr );
static void        Aio_ReadLinesRemoveExtenders( Aio_Read_t * p, int Line );
static int         Aio_ReadLinesDetectFileType( Aio_Read_t * p, Aio_Node_t * pIoNode );
...
/**Function*************************************************************
  ...
  through the lines and assigns the Aio_Node_t structure for each would-be node.]
...
***********************************************************************/
int Aio_ReadLines( Aio_Read_t * p )
{
    char * pDot;
    ...
    for ( i = p->fParsingExdcNet? p->LineExdc + 1: 0; i < p->nDots; i++ )
    {
        ...
        if ( pDot[1] == 'n' && pDot[2] == 'a' && pDot[3] == 'm' && pDot[4] == 'e' && pDot[5] == 's' )
            p->pAioNodes[ p->nDotNodes++ ].LineNames = p->pDots[i];
        else if ( pDot[1] == 't' && pDot[2] == 'a' && pDot[3] == 'b' && pDot[4] == 'l' && pDot[5] == 'e' )
        {
            ...
            p->pAioLatches[ p->nDotLatches++ ].LineLatch = p->pDots[i];
        ...
        {
            if ( Aio_ReadLinesDotRes( p, p->pDots[i] ) )
                return 1;
        }
        else
        {
    ...
    for ( n = 0; n < p->nDotInputs; n++ )
        Aio_ReadLinesRemoveExtenders( p, p->pDotInputs[n] );
    for ( n = 0; n < p->nDotOutputs; n++ )
        Aio_ReadLinesRemoveExtenders( p, p->pDotOutputs[n] );
    for ( n = 0; n < p->nDotNodes; n++ )
        Aio_ReadLinesRemoveExtenders( p, p->pAioNodes[n].LineNames );

    // preparse the .mv statements
    for ( n = 0; n < p->nDotMvs; n++ )
        if ( Aio_ReadLinesDotMv( p, p->pDotMvs[n] ) )
            return 1;
    ...
    {
        p->LineCur = 0;
        sprintf( p->sError, "The input file should contain one latch." );
        Aio_ReadPrintErrorMessage( p );
        return 1;
    }
    if ( p->nDotNodes != 2 )
    {
        p->LineCur = 0;
        sprintf( p->sError, "The input file should contain two nodes (output and next state)." );
        Aio_ReadPrintErrorMessage( p );
        return 1;
    }
    ...
    for ( n = 0; n < p->nDotLatches; n++ )
        if ( Aio_ReadLinesDotLatch( p, p->pAioLatches + n ) )
            return 1;

    // preparse the nodes
    for ( n = 0; n < p->nDotNodes; n++ )
        if ( Aio_ReadLinesDotNode( p, p->pAioNodes + n ) )
            return 1;

    // preparse the latch reset tables
    for ( n = 0; n < p->nDotLatches; n++ )
        if ( p->pAioLatches[n].AioNode.LineNames != -1 )
            if ( Aio_ReadLinesDotNode( p, &(p->pAioLatches[n].AioNode) ) )
                return 1;
...
***********************************************************************/
int Aio_ReadLinesDotMv( Aio_Read_t * p, int Line )
{
    Aio_Var_t * pVar;
    ...
    pPivot = Aio_ReadLinesDotMvPivot( pLine );
...
//        int * pnValuesCur;
        ...
        for ( i = 0; pTemp = strtok( NULL, ", \t" ); i++ )
        {
            ...
            // store the number of values if it is larger than 2
            if ( st_find_or_add( p->tName2Values, pTemp, (char***)&pnValuesCur ) )
            {
                if ( nValues != *pnValuesCur )
                {
                    sprintf( p->sError, "Node \"%s\" has with different number of values in different places (%d and %d).", pTemp, nValues, *pnValuesCur );
                    Aio_ReadPrintErrorMessage( p );
                    return 1;
                }
            }
...
***********************************************************************/
Aio_Var_t * Aio_ReadLinesDotMvCopyVar( Aio_Var_t * pVar )
{
    Aio_Var_t * pCopy;
    int i;
    pCopy = ALLOC( Aio_Var_t, 1 );
...
***********************************************************************/
char * Aio_ReadLinesDotMvPivot( char * pStr )
{
    char * pComma, * pTemp;
    // find the last comma
    pComma = NULL;
...
***********************************************************************/
int Aio_ReadLinesDotLatch( Aio_Read_t * p, Aio_Latch_t * pIoLatch )
{
    char * pTemp;
    ...
    pIoLatch->AioNode.LineNames = -1;
    if ( p->tLatch2ResLine )
        if ( !st_lookup( p->tLatch2ResLine, pIoLatch->pOutputName, (char**)&(pIoLatch->AioNode.LineNames) ) )
        {
            p->LineCur = pIoLatch->LineLatch;
...
***********************************************************************/
int Aio_ReadLinesDotRes( Aio_Read_t * p, int Line )
{
    char * pTemp;
    // get the latch output name (the last entry on .res line)
    pTemp = Aio_ReadLinesLastEntry( p->pLines[Line] );
    if ( pTemp[0] == '-' && pTemp[1] == '>' )
        pTemp += 2;
...
***********************************************************************/
int Aio_ReadLinesDotNode( Aio_Read_t * p, Aio_Node_t * pIoNode )
{
    ...
    pIoNode->pOutput = Aio_ReadLinesLastEntry( p->pLines[ pIoNode->LineNames ] );
    ...
    if ( pLine[0] == '.' && pLine[1] == 'd' && pLine[2] == 'e' && pLine[3] == 'f' )
    {
        ...
        else 
        {
            ...
            if ( pIoNode->Default < 0 || pIoNode->Default > 1000000 )
            {
                p->LineCur = LineNext;
    ...
    if ( p->Type == IO_FILE_NONE )
        p->Type = Aio_ReadLinesDetectFileType( p, pIoNode );

    // get the number of values
    if ( p->Type == IO_FILE_BLIF )
        pIoNode->nValues = 2;
    else if ( p->Type == IO_FILE_BLIF_MVS )
        pIoNode->nValues = strlen( Aio_ReadLinesLastEntry( p->pLines[ pIoNode->LineTable ] ) );
...
***********************************************************************/
char * Aio_ReadLinesLastEntry( char * pStr )
{
    char * pTemp, * pEnd;
...
***********************************************************************/
void Aio_ReadLinesRemoveExtenders( Aio_Read_t * p, int Line )
{
    char * pLine, * pCur;
    while ( 1 )
    {
...
***********************************************************************/
int Aio_ReadLinesAddNumValues( Aio_Read_t * p, char * pNodeName, int nValues )
{
    int * pnValuesCur;
    assert( nValues >= 0 );
    if ( p->tName2Values == NULL )
...
***********************************************************************/
int Aio_ReadLinesDetectFileType( Aio_Read_t * p, Aio_Node_t * pIoNode )
{
    ...
    if ( strlen( Aio_ReadLinesLastEntry( p->pLines[ pIoNode->LineTable ] ) ) > 1 )
