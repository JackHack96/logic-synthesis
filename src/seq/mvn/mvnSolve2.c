/**CFile****************************************************************
  ...
  Revision    [$Id: mvnSolve2.c,v 1.2 2004/04/08 04:48:26 alanmi Exp $]
...
static Au_Auto_t * Mvn_ComputeAutomaton( Mvn_Solve_t * p, int nStateLimit, int fUseLongNames, int fMoore );
static void Mvn_SolveDeriveStateNames( Mvn_Solve_t * p, Au_Auto_t * pAut );
...
***********************************************************************/
int Mvn_NetworkSolve2( Ntk_Network_t * pNetF, Ntk_Network_t * pNetS,  
...
if ( p->fVerbose )
{
    ...
    {
/*
        if ( fMoore && fProgressive )
        {
            if ( p->fVerbose )
                printf( "Skipping the computation of progressive because the solution is Moore.\n" );
            fProgressive = 0;
        }
*/
        if ( fProgressive )
...
clk = clock();
    ...
    p->pbPartsT = ALLOC( DdNode *, p->nPartsT );
    i = 0;
...
***********************************************************************/
Au_Auto_t * Mvn_ComputeAutomaton( Mvn_Solve_t * p, int nStateLimit, int fUseLongNames, int fMoore )
{
    Au_Auto_t * pAut;
    Vm_VarMap_t * pVm;
    Vmx_VarMap_t * pVmxIO;
    ...
    DdNode * bFormP, * bFormP2, * bFormCuv, * bImage;
    int * pEntry, nDigits, nBitsIO, nBitsO, iBit, * pBits, s, i, k;
    ...
    pAut = Au_AutoAlloc();
    pAut->pName        = util_strsav( "csf" );
    pAut->pMan         = p->pNetF->pMan;
    pAut->nInputs      = nBitsIO;
//    pAut->nOutputs     = nBitsO;
    pAut->nOutputs     = 0;
    pAut->nStates      = 0;
    ...
    assert( iBit == pAut->nInputs );

    // create the map to be used for the computation of SOP covers
//    pVmxIO = Vmx_VarMapCreateBinary( Fnc_ManagerReadManVm(pAut->pMan), Fnc_ManagerReadManVmx(pAut->pMan), pAut->nInputs, 0 );
    // reverse the ordering of the binary bits
    pBits       = Vmx_VarMapReadBits( p->pVmx );
    pBitsFirst  = Vmx_VarMapReadBitsFirst( p->pVmx ); 
    pBitsOrder  = Vmx_VarMapReadBitsOrder( p->pVmx );
    pBitsOrderR = ALLOC( int, pAut->nInputs );
    iBit = 0;
    for ( i = 0; i < p->nVarsU; i++ )
        for ( k = 0; k < pBits[i]; k++ )
    ...
    pVmxIO = Vmx_VarMapLookup( Fnc_ManagerReadManVmx(pAut->pMan), pVm, pAut->nInputs, pBitsOrderR );
    FREE( pBitsOrderR );
    ...
    //  P(u,v,ns) = Exist i,cs [ U(i,v,csF,u) & TF(i,v,csF,nsF) & TS(i,csS,nsS) & S(cs) ]
...
//    if ( !p->fVerbose )
    ...
    for ( s = 0; s < pAut->nStates; s++ )
    {
        ...
        {
clk = clock();
            bImage = Extra_bddImageCompute( p->ppTreeOs[i], pState->bSubset );   
clkIm += clock() - clk;
            if ( bImage == NULL )
            {
                Au_AutoFree( pAut );   pAut = NULL;
...
clk = clock();
        bFormP = Extra_bddImageCompute( p->pTreeT, pState->bSubset ); 
clkIm += clock() - clk;
        if ( bFormP == NULL )
        {
            Au_AutoFree( pAut );   pAut = NULL;
...
//    pAut->pStates[1]->fAcc = 0;
    ...
    for ( s = 0; s < pAut->nStates; s++ )
    {
        ...
        Cudd_RecursiveDeref( dd, pAut->pStates[s]->bSubset );
...
/*
    ...
    {
        Au_Auto_t * pAutTemp;
        pAut = Au_AutoFilterMarked( pAutTemp = pAut );
        Au_AutoFree( pAutTemp );
    }
*/
...
clk = clock();
    Au_AutoComplete( pAut, pAut->nInputs, 1, 0 );
...
***********************************************************************/
void Mvn_SolveDeriveStateNames( Mvn_Solve_t * p, Au_Auto_t * pAut )
{
    Au_State_t * pS;
    DdManager * dd = p->dd;
    Vmx_VarMap_t * pVmxExt;
    ...
    for ( s = 0; s < pAut->nStates; s++ )
    {
        ...
        while ( bFunc != b0 )
        {
            ...
            if ( fFirst )
                ...
                strcat( pS->pName, "_" );
            pNamePrev = pS->pName;
            pS->pName = ALLOC( char, strlen(pNamePrev) + strlen(pNameCur) + 3 );
            strcpy( pS->pName, pNamePrev );
            strcat( pS->pName, pNameCur );
            FREE( pNamePrev );
