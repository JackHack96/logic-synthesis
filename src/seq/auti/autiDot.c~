...
void Auti_AutoWriteDot( Aut_Auto_t * pAut, char * pFileName, int nStatesMax, int fShowCond )
{
    ...
    int nStatesIncomp, nStatesNonDet, i;
...
//        for ( s = 0; s < pAut->nStates; s++ )
		...
		/* begin UNIVR */
		i=0;
		for (i; i<pAut->nInit; i++){
		    if (strcmp(pState->pName,pAut->pInit[i]->pName) == 0){
		        fprintf( pFile, "%s",  ", shape = octagon");}}
		 // Old bugged version: fprintf( pFile, "%s", (s == 0? ", shape = octagon": "") );
		/* end UNIVR */ 
