...
/* begin UNIVR */
static int Auti_CommandAutoSbsSolve   ( Mv_Frame_t * pMvsis, int argc, char ** argv );
/* end UNIVR */
...
void Auti_Init( Mv_Frame_t * pMvsis )
{
	...
	/* begin UNIVR */
	Cmd_CommandAdd( pMvsis, "Automata manipulation", "sbssolve",     Auti_CommandAutoSbsSolve,     0 ); 
	/* end UNIVR */
...
/**Function*************************************************************
  ...
  Synopsis    [solve an equation over FSM]

  Description [this function use the step by step procedure on .aut files]
...
***********************************************************************/
int Auti_CommandAutoSbsSolve( Mv_Frame_t * pMvsis, int argc, char **argv )
{
   FILE * pOut, * pErr, * pFile;
    Aut_Auto_t * pAutS, * pAutSD, * pAutF, * pAutP, * pAutPD , * pAutM;
    char * pAutNameF, * pAutNameS;
	char * pAutNameSComp, * pAutPName, * pAutPNameTemp , * pAutPSuppName, * pAutPCompName, * pAutPCompNameTemp, * pAutFsmName;
	char * pTemp;
    char * pStringU, * pStringV, * pInputString;
    char * FileNameOut;
    int c;
    int fVerbose, fUseLongNames;
    int fProgressive, nProgressive, nParts;
    int fWriteAut;
    ...
    fUseLongNames = 0;
    util_getopt_reset();
    while ( ( c = util_getopt( argc, argv, "lpvha" ) ) != EOF )
    {
        switch ( c )
        {
			case 'p':
...
pAutF = Aio_AutoRead( pMvsis, pAutNameF, NULL );

//    pAutF = Io_ReadAutwork( pMvsis, pAutNameF );
    if ( pAutF == NULL )
    {
        fprintf( pErr, "Cannot read in network from file \"%s\".\n", pAutNameF );
        goto usage;
    }
pAutS = Aio_AutoRead( pMvsis, pAutNameS, NULL );

   // pAutS = Io_ReadAutwork( pMvsis, pAutNameS );
    if ( pAutS == NULL )
    {
        fprintf( pErr, "Cannot read in network from file \"%s\".\n", pAutNameS );
        goto usage;
 ...
 if ( fVerbose )
    {
		...
		Auti_AutoPrintStats( pOut, pAutF, Aut_AutoReadVarNum(pAutF), fVerbose );
		printf( "\n" );
		Auti_AutoPrintStats( pOut, pAutS, Aut_AutoReadVarNum(pAutS), fVerbose );
        printf( "\n" );
    ...
    // solve the problem
	// complementing S
if ( Auti_AutoCheckIsNd( stdout, pAutS, Aut_AutoReadVarNum(pAutS), 0 ) )
    {
        fprintf( pOut, "The automaton in \"%s\" is non-deterministic. Determinization will be performed.\n", pAutNameS );
        pAutSD = Auti_AutoDeterminizePart( pAutS, 0, 0 );
        if ( pAutSD != pAutS )
            Aut_AutoFree( pAutS );
        pAutS = pAutSD;
    }
    if ( Auti_AutoComplete( pAutS, Aut_AutoReadVarNum(pAutS), 0 ) )
    {
        fprintf( pOut, "The automaton in \"%s\" is completed before complementing.\n", pAutNameS );
    }

    Auti_AutoComplement( pAutS );

	pAutNameSComp = ALLOC(char, 100);
	if (fWriteAut)
    sprintf(pAutNameSComp,"%s%s",strtok(pAutNameS,"."),"_comp.aut");
	else
    sprintf(pAutNameSComp,"%s%s",strtok(pAutNameS,"."),"_comptemp.aut");

	Aio_AutoWrite( pAutS, pAutNameSComp, 0, 0 );

// calculating the product
 if ( !Auti_AutoProductCommonSupp( pAutF, pAutS, pAutNameF, pAutNameSComp, &pAutF, &pAutS ) )
    {
        printf( "Constructing the product has failed.\n" );
        return 0;
    }
 pAutP = Auti_AutoProduct( pAutF, pAutS, fUseLongNames );
	...
	pAutPNameTemp = ALLOC(char, 100);
	pAutPName = ALLOC(char, 100);
	snprintf(pAutPNameTemp, (strlen(pAutNameS)-strlen(strrchr(pAutNameS,'/') + 2)), "%s", pAutNameS);
	sprintf(pAutPName,"%s%s",pAutPNameTemp,"product.aut");
	FREE(pAutPNameTemp);

	if (fWriteAut)
    Aio_AutoWrite( pAutP, pAutPName, 0, 0 );
	else
	remove(pAutNameSComp);
...
// supporting the product
		pInputString = ALLOC(char, 100);
		sprintf(pInputString,"%s%s%s",pStringU,",",pStringV);
    if ( !Auti_AutoSupport( pAutP, pInputString ) ){
        fprintf( pErr, "Support computation has failed.\n" );
	return 0;
		}
	pAutPSuppName = ALLOC(char, 100);
		sprintf(pAutPSuppName,"%s%s",strtok(pAutPName,"."),"_supp.aut");

	if (fWriteAut){
	        Aio_AutoWrite( pAutP, pAutPSuppName, 0, 0 );
	}
	FREE(pAutPName);
    FREE(pInputString);
	// complementing the product
if ( Auti_AutoCheckIsNd( stdout, pAutP, Aut_AutoReadVarNum(pAutP), 0 ) )
    {
        fprintf( pOut, "The automaton in \"%s\" is non-deterministic. Determinization will be performed.\n", pAutPSuppName );
        pAutPD = Auti_AutoDeterminizePart( pAutP, 0, 0 );
        if ( pAutPD != pAutP )
            Aut_AutoFree( pAutP );
        pAutP = pAutPD;
    }
    if ( Auti_AutoComplete( pAutP, Aut_AutoReadVarNum(pAutP), 0 ) )
    {
        fprintf( pOut, "The automaton in \"%s\" is completed before complementing.\n", pAutPSuppName );
    }

	Auti_AutoComplement( pAutP );
	pAutPCompName = ALLOC(char, 100);
	pAutPCompNameTemp = ALLOC(char, 100);
	snprintf(pAutPCompNameTemp, (strlen(pAutPSuppName)-strlen(strrchr(pAutPSuppName,'/') + 2)), "%s", pAutPSuppName);
	sprintf(pAutPCompName,"%s%s",pAutPCompNameTemp,"x.aut");

	if (fWriteAut){
	    Aio_AutoWrite( pAutP, pAutPCompName, 0, 0 );
}
	FREE(pAutPCompNameTemp);
	FREE(pAutPSuppName);
// making the result progressive
if ( fProgressive )
    {
		int clk;
		...
		clk = clock();
        nStateBefore = Aut_AutoReadStateNum(pAutP);
		nProgressive = 0;
		if (strcmp(pStringU,"@")){ /* pStringU != "@"*/
	  	pTemp  = strtok( pStringU, "," );
		for ( nParts = 0; pTemp; nParts++ ){
			nProgressive++;
	        pTemp = strtok( NULL, "," );
    	}
	}
		Auti_AutoProgressive( pAutP, nProgressive, 0, 1 );
        nStatesAfter = Aut_AutoReadStateNum(pAutP);
		PRT( "Progressive", clock() - clk );
        if ( fVerbose )
            printf( "The resulting automaton is made progressive (%d states -> %d states).\n", 
                    nStateBefore, nStatesAfter );
	if (fWriteAut){					
	pAutFsmName = ALLOC(char, 100);
    sprintf(pAutFsmName,"%s%s",strtok(pAutPCompName,"."),"fsm.aut");
	FREE(pAutPCompName);
	Aio_AutoWrite( pAutP, pAutFsmName, 0, 0 );
	FREE(pAutFsmName);
}
}
// minimizing the result
    pAutM = Auti_AutoStateMinimize( pAutP );
    if ( pAutM != pAutP )
        Aut_AutoFree( pAutP );
    if ( pAutM == NULL )
    {
    ...
    Aio_AutoWrite( pAutM, FileNameOut, 0, 0 );
    Aut_AutoFree( pAutM );


//    if ( Mvn_AutworkSolve( pAutF, pAutS, pStringU, pStringV, FileNameOut, nStatesLimit, 
...
usage:
	fprintf( pErr, "\n" );
	fprintf( pErr, "Usage:   sbssolve [-s num] [-pmlvh] <aut_F> <aut_S> <U_vars> <V_vars> <file_out>\n");
	fprintf( pErr, "         solves language equation F * X = S when F and S are represented\n" );
	fprintf( pErr, "         as automata in .aut format\n" );
	...
	fprintf( pErr, " -a         : toggles writing the intermediate results on files [default = %s]\n", (fWriteAut? "yes": "no") );
	fprintf( pErr, " -h         : print the help message\n" );
	fprintf( pErr, " <aut_F>    : the file for the automaton F\n" );
	fprintf( pErr, " <aut_S>    : the file for the automaton S\n" );
...
int Auti_CommandAutoSupport( Mv_Frame_t * pMvsis, int argc, char **argv )
{
    ...
    if (pAut->nStates ==1 )
    {     
    printf("Warning: the automaton has only 1 state. At the moment BALM can't handle it, try to define an automaton with two equivalent states.\n", pAut->nStates);
    return 0;
    }
    /*   end UNIVR    */
...
usage:
	...
	fprintf( pErr, "                 If a variable has more than two values you must declare the number of values between parentheses.\n" );
	fprintf( pErr, " 		 e.g. support u,v(4) <file_in> <file_out>\n" );
