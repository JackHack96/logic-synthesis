/**CFile****************************************************************
  ...
  Revision    [$Id: mntkBdd.c,v 1.1 2005/02/28 05:34:28 alanmi Exp $]
...
static DdNode * Mntk_NodeBddsBuild( DdManager * dd, Mntk_Node_t * pNode );
static DdNode * Mntk_NodeBddsBuildCube( Mvc_Cover_t * pMvc, Mvc_Cube_t * pCube, DdManager * dd, DdNode * pbFuncs[], int nFuncs );
...
***********************************************************************/
int Mntk_NodeBddsPrepare( Mntk_Node_t * pNode )
{
    ...
    for ( i = 0; i < vWinTotal->nSize; i++ )
    {
        if ( vWinTotal->pArray[i]->pData[0] )
            continue;
        vWinTotal->pArray[i]->pData[0] = (char *)Mntk_NodeBddsBuild( dd, vWinTotal->pArray[i] );  
        Cudd_Ref( (DdNode *)vWinTotal->pArray[i]->pData[0] );
    }
    return 1;
}
...
***********************************************************************/
DdNode * Mntk_NodeBddsBuild( DdManager * dd, Mntk_Node_t * pNode )
{
    Mntk_Node_t * pFanin;
    DdNode * pbFuncs[10];
    ...
    for ( i = 0; i < (int)pNode->nLeaves; i++ )
    {
        ...
        pbFuncs[i] = Cudd_NotCond( (DdNode *)pFanin->pData[0], Mntk_IsComplement(pNode->ppLeaves[i]) );
    }
    // get the cover
    ...
    {
        bCube = Mntk_NodeBddsBuildCube( pMvc, pCube, dd, pbFuncs, pNode->nLeaves );  Cudd_Ref( bCube );
...
***********************************************************************/
DdNode * Mntk_NodeBddsBuildCube( Mvc_Cover_t * pMvc, Mvc_Cube_t * pCube, DdManager * dd, DdNode * pbFuncs[], int nFuncs )
{
...
***********************************************************************/
int Mntk_NodeBddsCleanup( Mntk_Node_t * pNode )
{
    Mntk_NodeVec_t * vWinTotal = pNode->p->vWinTotal;
    int i;
    for ( i = 0; i < vWinTotal->nSize; i++ )
...
***********************************************************************/
int Mntk_NodeBddsCheckResub( Mntk_Node_t * pNode, Mntk_Node_t ** ppCands, int nCands )
{
    DdNode * bImg0, * bImg1;
    DdNode * pbFuncs[10];
    DdNode * bCareSet;
