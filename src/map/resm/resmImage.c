/**CFile****************************************************************
  ...
  Revision    [$Id: resmImage.c,v 1.1 2005/01/23 06:59:52 alanmi Exp $]
...
static int Resm_NodeAddClauses( Sat_Solver_t * pSat, Resm_Node_t * pNode );

static void Resm_ResynthesisCone( Resm_Man_t * p, Resm_Node_t * pNodes[], int nNodes );
static void Resm_ResynthesisCone_rec( Resm_Node_t * pNode );
...
***********************************************************************/
int Resm_ResynthesisImage( Resm_Node_t * pPivot, Resm_Node_t * ppCands[], int nCands )
{
    Resm_Man_t * p = pPivot->p;
    Sat_Solver_t * pSat = pPivot->p->pSatImg;
//    Resm_NodeVec_t * vWinTotal = pPivot->p->vWinTotalCone;
//    Resm_NodeVec_t * vWinNodes = pPivot->p->vWinNodesCone;
    Resm_NodeVec_t * vWinTotal = pPivot->p->vWinTotal;
    Resm_NodeVec_t * vWinNodes = pPivot->p->vWinNodes;
    Resm_Node_t * pNodes[10];
...
//    int nBackTrackLimit = -1;
    ...
    if ( fDelimitCones )
        Resm_ResynthesisCone( p, pNodes, nNodes );
    ////////////////////////////////////////////
    ...
    for ( i = 0; i < vWinNodes->nSize; i++ )
        if ( !Resm_NodeAddClauses( pSat, vWinNodes->pArray[i] ) )
        {
            assert( 0 );
            return 0;
        }
...
***********************************************************************/
int Resm_NodeAddClauses( Sat_Solver_t * pSat, Resm_Node_t * pNode )
{
    ...
    {
        ppNodes[i] = Resm_Regular(pNode->ppLeaves[i]);
        fComps[i]  = Resm_IsComplement(pNode->ppLeaves[i]);
        Vars[i]    = Resm_Regular(pNode->ppLeaves[i])->Num2;
        assert( Vars[i] < nVars );
    }
...
***********************************************************************/
void Resm_ResynthesisCone( Resm_Man_t * p, Resm_Node_t * pNodes[], int nNodes )
{
    int i;
    ...
    for ( i = 0; i < nNodes; i++ )
        Resm_ResynthesisCone_rec( pNodes[i] );
    // select only unmarked
    Resm_NodeVecClear( p->vWinTotalCone );
    for ( i = 0; i < p->vWinTotal->nSize; i++ )
        if ( p->vWinTotal->pArray[i]->fUsed == 0 )
            Resm_NodeVecPush( p->vWinTotalCone, p->vWinTotal->pArray[i] );
    // select only unmarked
    Resm_NodeVecClear( p->vWinNodesCone );
    for ( i = 0; i < p->vWinNodes->nSize; i++ )
        if ( p->vWinNodes->pArray[i]->fUsed == 0 )
            Resm_NodeVecPush( p->vWinNodesCone, p->vWinNodes->pArray[i] );
...
/*
    ...
    {
        int fPrint = 0;
        Resm_Node_t * pNode = p->vWinNodesCone->pArray[i];
        int k, m;
        for ( k = 0; k < pNode->nLeaves; k++ ) 
        for ( m = k+1; m < pNode->nLeaves; m++ ) 
            if ( Resm_Regular(pNode->ppLeaves[k]) == Resm_Regular(pNode->ppLeaves[m]) )
            {
                printf( "Resynthesizing node %d. Node %d. Fanin %d is duplicated.\n", 
                    pNodes[0]->Num, pNode->Num, Resm_Regular(pNode->ppLeaves[m])->Num );
                fPrint = 1;
            }
if ( fPrint )
printf( "\n" );
...
***********************************************************************/
void Resm_ResynthesisCone_rec( Resm_Node_t * pNode )
{
    int i;
    ...
    for ( i = 0; i < pNode->nLeaves; i++ )
        Resm_ResynthesisCone_rec( Resm_Regular(pNode->ppLeaves[i]) );
}
