/**CFile****************************************************************
  ...
  Revision    [$Id: resmFilSim.c,v 1.1 2005/01/23 06:59:51 alanmi Exp $]
...
static void Resm_TestOneWord( unsigned * puSimRoot, unsigned ** ppuSimNodes, int iWord, int nNodes, int fPhase, unsigned * puTruth );

static void Resm_SimulateNode( Resm_Node_t * pNode );
static unsigned Resm_SimulateWord( Mvc_Cover_t * pMvc, unsigned * pSimInfo, int nInputs );
...
***********************************************************************/
int Resm_TestNodes( Resm_Man_t * p, Resm_Node_t * pRoot, Resm_Node_t * pNodes[], int nNodes )
{
...
//    p->uRes0All[3] = p->uRes1All[3] = 0;
    ...
    {
        Resm_TestOneWord( puSimRoot, ppuSimNodes, i, nNodes, 0, uRes0 );
        Resm_TestOneWord( puSimRoot, ppuSimNodes, i, nNodes, 1, uRes1 );
...
***********************************************************************/
void Resm_TestOneWord( unsigned * puSimRoot, unsigned ** ppuSimNodes, int iWord, int nNodes, int fPhase, unsigned * puTruth )
{
...
***********************************************************************/
void Resm_SimulateWindow( Resm_Man_t * p )
{
    Resm_Node_t * pNode;
    int i, v;
    // mark all the nodes
    ...
    for ( i = 0; i < p->vWinTotal->nSize; i++ )
    {
        ...
        if ( pNode->fMark0 == 0 )
        {
            for ( v = 0; v < p->nSimRounds; v++ )
                pNode->pSims[v] = RESM_RANDOM_UNSIGNED;
        }
        else
        {
            Resm_SimulateNode( pNode );
            pNode->fMark0 = 0;
        }
    }
    // mark all the nodes
...
***********************************************************************/
void Resm_SimulateNode( Resm_Node_t * pNode )
{
    Resm_Node_t * pFanin;
    ...
    for ( i = 0; i < (int)pNode->nLeaves; i++ )
    {
        ...
        fCompls[i] = Resm_IsComplement(pNode->ppLeaves[i]);
    ...
    for ( i = 0; i < pNode->p->nSimRounds; i++ )
    {
        ...
        pNode->pSims[i] = Resm_SimulateWord( pMvc, pSimInfo, pNode->nLeaves );
    }
}
...
***********************************************************************/
unsigned Resm_SimulateWord( Mvc_Cover_t * pMvc, unsigned * pSimInfo, int nInputs )
{
    Mvc_Cube_t * pCube;
