/**CFile****************************************************************
  ...
  Revision    [$Id: mioUtils.c,v 1.8 2005/02/28 05:34:28 alanmi Exp $]
...
static void Mio_WriteGate( FILE * pFile, Mio_Gate_t * pGate );
...
void Mio_GateDelete( Mio_Gate_t * pGate )
{
    ...
    FREE( pGate->pForm );
    if ( pGate->pMvc )
        Mvc_CoverFree( pGate->pMvc );
    if ( pGate->pMvcC )
        Mvc_CoverFree( pGate->pMvcC );
    if ( pGate->bFunc )
...
***********************************************************************/
void Mio_WriteLibrary( FILE * pFile, Mio_Library_t * pLib )
{
    Mio_Gate_t * pGate;

    fprintf( pFile, "# The genlib library \"%s\".\n", pLib->pName );
    Mio_LibraryForEachGate( pLib, pGate )
        Mio_WriteGate( pFile, pGate );
}
...
***********************************************************************/
void Mio_WriteGate( FILE * pFile, Mio_Gate_t * pGate )
{
    Mio_Pin_t * pPin;

    fprintf( pFile, "GATE " );
    fprintf( pFile, "%6s ",       pGate->pName );
    fprintf( pFile, "%0.2f ",     pGate->dArea );
    fprintf( pFile, "O=%20s ",    pGate->pForm );
    fprintf( pFile, "   " );
...
void Mio_WritePin( FILE * pFile, Mio_Pin_t * pPin )
{
    ...
    fprintf( pFile, "    PIN " );
    fprintf( pFile, "%4s ",      pPin->pName );
    fprintf( pFile, "%8s ",      pPhaseNames[pPin->Phase] );
    fprintf( pFile, "%4d ",      (int)pPin->dLoadInput );
    fprintf( pFile, "%4d ",      (int)pPin->dLoadMax );
    fprintf( pFile, "%0.2f ",    pPin->dDelayBlockRise );
    fprintf( pFile, "%0.2f ",    pPin->dDelayFanoutRise );
    fprintf( pFile, "%0.2f ",    pPin->dDelayBlockFall );
    fprintf( pFile, "%0.2f ",    pPin->dDelayFanoutFall );
    fprintf( pFile, "\n" );
}
...
void Mio_DeriveTruthTable2( Mio_Gate_t * pGate, unsigned uTruthsIn[][2], int nTruths, int nInputs, unsigned uTruthRes[] )
{
    ...
    if ( nInputs < 6 )
    {
        ...
        Mvc_CoverForEachCube( pGate->pMvc, pCube )
        {
            uSignCube[0] = MIO_FULL;
            Mvc_CubeForEachVarValue( pGate->pMvc, pCube, Var, Value )
            {
                assert( Value );
                if ( Value == 1 )
