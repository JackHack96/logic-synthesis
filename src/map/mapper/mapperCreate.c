/**CFile****************************************************************
  ...
  Revision    [$Id: mapperCreate.c,v 1.16 2005/07/08 01:01:24 alanmi Exp $]
...
***********************************************************************/
char *          Map_NodeReadData0( Map_Node_t * p )                   { return p->pData0;    }
char *          Map_NodeReadData1( Map_Node_t * p )                   { return p->pData1;    }
...
Map_Cut_t *     Map_NodeReadCut0( Map_Node_t * p )                    { return p->pCutBest[0];   }
Map_Cut_t *     Map_NodeReadCut1( Map_Node_t * p )                    { return p->pCutBest[1];   }
Map_Node_t *    Map_NodeReadOne( Map_Node_t * p )                     { return p->p1;        }
Map_Node_t *    Map_NodeReadTwo( Map_Node_t * p )                     { return p->p2;        }
void            Map_NodeSetData0( Map_Node_t * p, char * pData )      { p->pData0  = pData;  }
void            Map_NodeSetData1( Map_Node_t * p, char * pData )      { p->pData1  = pData;  }
...
Map_Man_t * Map_ManCreate( int nInputs, int nOutputs, int fVerbose )
{
    ...
    p->nVarsMax  = s_pSuperLib->nVarsMax;
    p->fVerbose  = fVerbose;
...
//    printf( "Node = %d bytes. Cut = %d bytes. Super = %d bytes.\n", sizeof(Map_Node_t), sizeof(Map_Cut_t), sizeof(Map_Super_t) ); 
    p->mmNodes    = Extra_MmFixedStart( sizeof(Map_Node_t), 10000, 100 );
    p->mmCuts     = Extra_MmFixedStart( sizeof(Map_Cut_t), 10000, 100 );
