/**CFile****************************************************************
  ...
  Revision    [$Id: mapperFraig.c,v 1.7 2005/07/08 01:01:24 alanmi Exp $]
...
***********************************************************************/
Map_Man_t * Map_ManDupFraig( Fraig_Man_t * pMan )
{
    Map_Man_t * pManMap;
    Fraig_Node_t ** ppNodes, ** ppInputs, ** ppOutputs;
    Fraig_Node_t * pNode1, * pNode2, * pNodeE, * pNodeR;
    Map_Node_t ** pInputsMap, ** pOutputsMap;
    Map_Node_t * pNodeMap, * pNodeMap1, * pNodeMap2, * pNodeMapE, * pNodeMapR;
    ...
    pManMap  = Map_ManCreate( nInputs, nOutputs, Fraig_ManReadVerbose(pMan) );
    if ( pManMap == NULL )
        return NULL;

    // read the parameters
    pInputsMap  = Map_ManReadInputs( pManMap );
    pOutputsMap = Map_ManReadOutputs( pManMap );
    ...
    for ( i = 1; i < nNodes; i++ )
    {
        ...
        pNodeMap1 = (Map_Node_t *)Fraig_NodeReadData0( Fraig_Regular(pNode1) );
        pNodeMap2 = (Map_Node_t *)Fraig_NodeReadData0( Fraig_Regular(pNode2) );
        pNodeMap  = Map_NodeAnd( pManMap, 
            Map_NotCond(pNodeMap1, Fraig_IsComplement(pNode1)), 
            Map_NotCond(pNodeMap2, Fraig_IsComplement(pNode2)) ); 
    ...
    for ( i = 1; i < nNodes; i++ )
    {
        ...
        pNodeMap = (Map_Node_t *)Fraig_NodeReadData0( ppNodes[i] );
        pNodeMapE = (Map_Node_t *)Fraig_NodeReadData0( pNodeE );
        pNodeMapR = (Map_Node_t *)Fraig_NodeReadData0( pNodeR );
        // set the equivalent and the representative nodes
        Map_NodeSetNextE( pNodeMap, pNodeMapE );
        Map_NodeSetRepr( pNodeMapE, pNodeMapR );
        // set the relative phase of pNodeE compared to pNodeR
//        Map_NodeSetPhase( pNodeMapE, Fraig_NodeComparePhase(pNodeE, pNodeR) );
    ...
    for ( i = 0; i < nOutputs; i++ )
    {
        if ( Fraig_Regular(ppOutputs[i]) == Fraig_ManReadConst1(pMan) )
            pNodeMap = Map_ManReadConst1(pManMap);
        else
            pNodeMap = (Map_Node_t *)Fraig_NodeReadData0( Fraig_Regular(ppOutputs[i]) );
        pOutputsMap[i] = Map_NotCond(pNodeMap, Fraig_IsComplement(ppOutputs[i]));
    }
    assert( Map_ManCheckConsistency( pManMap ) );
    ...
    for ( i = 0; i < pManMap->nInputs; i++ )
    {
        ...
        if ( fCompl )
            ...
            for ( k = 0; k < pManMap->nSimRounds - 1; k++ )
                pManMap->pSimInfo[i][k] = pSimInfo[k];
        pManMap->pSimInfo[i][pManMap->nSimRounds - 1] = MAP_RANDOM_UNSIGNED;
    }
    ...
    Map_ManSetChoiceNodeNum( pManMap, nChoiceNodes );
    Map_ManSetChoiceNum( pManMap, nChoices );
    return pManMap;
}
...
***********************************************************************/
Map_Man_t * Map_ManBalanceFraig( Fraig_Man_t * pMan, int * pInputArrivals )
{
    Map_Man_t * pManMap;
    Fraig_Man_t * pManNew;
    pManNew = Fraig_ManBalance( pMan, 0, pInputArrivals );
    pManMap = Map_ManDupFraig( pManNew );
