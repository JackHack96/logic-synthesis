/**CFile****************************************************************
  ...
  Revision    [$Id: mapperTree.c,v 1.10 2005/07/08 01:01:25 alanmi Exp $]
...
int Map_LibraryReadTree( Map_SuperLib_t * pLib, char * pFileName, char * pExcludeFile )
{
    ...
    /* pFile = fopen( pFileName, "r" ); // use file_open */
    ...
    if ( pExcludeFile )
    {
        ...
        if ( (num = Mio_LibraryReadExclude( pMvsis, pExcludeFile, tExcludeGate )) == -1 )
        {
...
int Map_LibraryReadFileTree( Map_SuperLib_t * pLib, FILE * pFile, char *pFileName )
{
    ...
    /* pFileGen = fopen( pLibFile, "r" );  // using file_open */
    ...
    pLib->pGenlib = Mio_LibraryRead( Mv_FrameGetGlobalFrame(), pLibFile, 0 );
