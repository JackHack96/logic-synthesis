/**CFile****************************************************************
  ...
  Revision    [$Id: extra.h,v 1.18 2005/04/10 23:27:03 alanmi Exp $]
...
#define Hash_IsComplement(p)  (((int)((long) (p) & 02)) > 0)
...
#define PRS(dd,f)       printf("Support of %s = ", #f ); Extra_PrintSupport( (dd),(f) )
...
/* gets the numeric value of one minterm (nVars < 32) */
...
extern DdNode *    Extra_bddBitsToCube2( DdManager * dd, int Code, int CodeWidth, DdNode ** pbVars );
/* computes the positive polarty cube composed of the vars in the array, and vice versa */
extern DdNode *    Extra_bddComputeCube( DdManager * dd, DdNode ** bXVars, int nVars );
extern DdNode **   Extra_bddComputeVarArray( DdManager * dd, DdNode * bCube, int * pnVars );
...
typedef struct Extra_OverlapCache_t_ Extra_OverlapCache_t;

extern Extra_OverlapCache_t * Extra_OverlapCacheStart( DdManager * dd, int nSize );
extern void             Extra_OverlapCacheClean( Extra_OverlapCache_t * p );
extern void             Extra_OverlapCacheStop( Extra_OverlapCache_t * p );
extern int              Extra_bddOverlap( Extra_OverlapCache_t * p, DdNode * bFunc1, DdNode * bFunc2 );
extern int              Extra_OverlapCacheReadSuccess( Extra_OverlapCache_t * p );
extern int              Extra_OverlapCacheReadFailure( Extra_OverlapCache_t * p );
...
/*=== extraDdNodePath_old.c =================================================================*/

/* the number of DD nodes pointed to under the cut */
extern int         Extra_NodesUnderCut( DdManager * dd, DdNode * Func, int CutLevel );
/* the number of DD nodes above the cut */
extern int         Extra_NodesAboveCut( DdManager * dd, DdNode * Func, int CutLevel );
/* for each node in the BDD, computes the sum of paths leading to it from the root */
extern st_table *  Extra_bddNodePaths( DdManager * dd, DdNode * bFunc );
/* for each node in the ADD, computes the sum of paths leading to it from the root */
extern st_table *  Extra_addNodePaths( DdManager * dd, DdNode * bFunc );
...
/* returns the array of nodes sorted by level from top to bottom */
extern DdNode **   Extra_NodesByLevel( DdManager * dd, DdNode * Func, int * pnNodes );
/* returns the node in the ADD, which has the max number of paths from the top node */
extern DdNode *    Extra_NodeWithMaxPaths( DdManager * dd, DdNode * Func );
...
typedef struct Extra_BitGraph_t_  Extra_BitGraph_t;

extern Extra_BitGraph_t * Extra_BitGraphAlloc( int nVerts );
extern void               Extra_BitGraphInvert( Extra_BitGraph_t * pGraph );
extern void               Extra_BitGraphFree( Extra_BitGraph_t * pGraph );
extern void               Extra_BitGraphEdgeAdd( Extra_BitGraph_t * pGraph, int i, int j );
extern Extra_BitGraph_t * Extra_BitGraphRandom( int nVerts, double Density );
extern void               Extra_BitGraphPrint( Extra_BitGraph_t * pGraph );
extern int                Extra_BitGraphColor( Extra_BitGraph_t * pGraph, int * pColors, int fVerbose );
extern int                Extra_BitGraphColorVerify( Extra_BitGraph_t * pGraph, int * pColors );
...
extern void        Extra_ProgressBarUpdate( ProgressBar * p, int nItemsCur, char * pString );
...
typedef struct Extra_Queue_t_ Extra_Queue_t;
extern Extra_Queue_t *  Extra_QueueStart( int nSize );
extern void             Extra_QueueStop( Extra_Queue_t * p );
extern int              Extra_QueueRead( Extra_Queue_t * p, char ** ppEntry );
extern void             Extra_QueueWrite( Extra_Queue_t * p, char * pEntry );
...
/* returns the reduced offset of the given cube */
extern DdNode *    Extra_zddReducedOffset( DdManager * dd, DdNode * zCover, DdNode * zCube );
extern DdNode *     extraZddReducedOffset( DdManager * dd, DdNode * zCover, DdNode * zCube );
