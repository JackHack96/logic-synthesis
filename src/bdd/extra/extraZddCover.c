/**CFile****************************************************************
  ...
  Revision    [$Id: extraZddCover.c,v 1.2 2004/01/06 21:02:54 alanmi Exp $]
...
/**Function********************************************************************
  ...
  Synopsis    [Returns the reduced offset of the give cube.]
  ...
  DdNode * zCube)     /* the cube */
{
    ...
    do {
        dd->reordered = 0;
        res = extraZddReducedOffset(dd, zCover, zCube);
    } while (dd->reordered == 1);
    return(res);

} /* end of Extra_zddReducedOffset */
  ...
  DdNode * zCube)     /* the cube */
{
    ...
    // make sure that the cube is okay 
    assert( zCube != z0 );

    // if the cover is empty, everything is blocked
    if ( zCover == z0 )
        return z1;
    // if the cover is tautology, no blocking exists
    if ( Extra_zddEmptyBelongs( dd, zCover ) )
        return z0;
    // if the cover is not trivial, but the cube is tautology, it is blocked
    if ( zCube == z1 )
        return z1;

    // check cache 
    zRes = cuddCacheLookup2Zdd(dd, extraZddReducedOffset, zCover, zCube);
    if ( zRes )
        return zRes;
    else
    {
        DdNode * zCover0, * zCover1, * zCover2;
        DdNode * zUnion0, * zUnion1;
        ...
        // if a variable belongs only to the cube, cover cannot block it
        if ( LevelCover > LevelCube )
            zRes = extraZddReducedOffset( dd, zCover, cuddT(zCube) );
        // if a variable belongs only to the cover, cofactor the cover
        else if ( LevelCover < LevelCube )
        {
            // decompose the cover 
            extraDecomposeCover(dd, zCover, &zCover0, &zCover1, &zCover2);

            // compute the union (C0,C2)
            zUnion0 = cuddZddUnion( dd, zCover0, zCover2 );
            if ( zUnion0 == NULL )
                return NULL;
            cuddRef( zUnion0 );

            // compute the union (C1,C2)
            zUnion1 = cuddZddUnion( dd, zCover1, zCover2 );
            if ( zUnion1 == NULL )
            {
                Cudd_RecursiveDerefZdd( dd, zUnion0 );
                return NULL;
            }
            cuddRef( zUnion1 );

            // get the results
            zRes0 = extraZddReducedOffset( dd, zUnion0, zCube );
            if ( zRes0 == NULL )
            {
                Cudd_RecursiveDerefZdd( dd, zUnion0 );
                Cudd_RecursiveDerefZdd( dd, zUnion1 );
                return NULL;
            }
            cuddRef( zRes0 );
            Cudd_RecursiveDerefZdd( dd, zUnion0 );
            ...
            // get the results
            zRes1 = extraZddReducedOffset( dd, zUnion1, zCube );
            if ( zRes1 == NULL )
            {
                Cudd_RecursiveDerefZdd( dd, zRes0 );
                Cudd_RecursiveDerefZdd( dd, zUnion1 );
            ...
            // get the result
            zRes = extraZddMinUnion( dd, zRes0, zRes1 );
            if ( zRes == NULL )
            {
                Cudd_RecursiveDerefZdd( dd, zRes0 );
                Cudd_RecursiveDerefZdd( dd, zRes1 );
        ...
        else // if ( LevelCover == LevelCube ) 
        {
            ...
            // get the results
            zRes0 = extraZddReducedOffset( dd, zUnion0, cuddT(zCube) );
            if ( zRes0 == NULL )
            {
                Cudd_RecursiveDerefZdd( dd, zUnion0 );
                Cudd_RecursiveDerefZdd( dd, zUnion1 );
            ...
            // get the results
            zRes1 = extraZddReducedOffset( dd, zUnion1, cuddT(zCube) );
            if ( zRes1 == NULL )
            {
                Cudd_RecursiveDerefZdd( dd, zRes0 );
                Cudd_RecursiveDerefZdd( dd, zUnion1 );
            ...
            // get the index of the positive ZDD variable
            TopZddVar = (zCover->index >> 1) << 1;

            // --------------- compose the result ------------------ 
            if ( TopZddVar == zCube->index ) // cube has positive literal
            {
                zRes0 = extraZddNotSupSet( dd, zTemp = zRes0, zRes1 );
                if ( zRes0 == NULL )
                {
                    Cudd_RecursiveDerefZdd( dd, zTemp );
                    Cudd_RecursiveDerefZdd( dd, zRes1 );
                ...
                zRes = cuddZddGetNode(dd, TopZddVar, zRes0, zRes1 );
            }
            else
            {
                zRes1 = extraZddNotSupSet( dd, zTemp = zRes1, zRes0 );
                if ( zRes1 == NULL )
                {
                    Cudd_RecursiveDerefZdd( dd, zTemp );
                    Cudd_RecursiveDerefZdd( dd, zRes0 );
                ...
                zRes = cuddZddGetNode(dd, TopZddVar, zRes1, zRes0 );
            }
        ...
        // insert the result into cache 
        cuddCacheInsert2( dd, extraZddReducedOffset, zCover, zCube, zRes );
        return zRes;
    }
} /* end of extraZddReducedOffset */
