/**CFile****************************************************************
  ...
  Revision    [$Id: extraBddMisc.c,v 1.5 2004/01/06 21:02:54 alanmi Exp $]
...
/**Function********************************************************************
  ...
  returns the minterm with the smallest bits-to-integer value. Another difference
  is that if the variable set (bSupp) does not contain some variables in the 
  support of the function (bFunc), the resulting minterm also does not contain them. 
  In this case, the resulting minterm may be not the cube in the on-set of function.
  However, there exist such an assignment of the variables in the function that
  are not in tbe minterm, that when added to the minterm, it will become the cube
  of the function.]
...
DdNode * Extra_bddGetOneMinterm( DdManager * dd, DdNode * bFunc, DdNode * bSupp )
{
    ...
    LevelF = cuddI( dd, bFuncR->index );
    LevelS = cuddI( dd, bSupp->index );
    if ( LevelF < LevelS ) // the top var belongs to bFunc, not to bSupp
    {
        // cofactor bFunc
        if ( Cudd_IsComplement(bFunc) )
        {
        ...
        // if the negative branch is not zero, use it
        if ( bFunc0 != b0 )
            return Extra_bddGetOneMinterm( dd, bFunc0, bSupp );
        else
            return Extra_bddGetOneMinterm( dd, bFunc1, bSupp );
    }

    if ( LevelF == LevelS ) // the top var belongs to both
    {
        // cofactor bFunc
        if ( Cudd_IsComplement(bFunc) )
        {
        ...
        if ( bFunc0 != b0 )
        {
            bResP = Extra_bddGetOneMinterm( dd, bFunc0, cuddT(bSupp) );         Cudd_Ref( bResP );
            bRes  = Cudd_bddAnd( dd, bResP, Cudd_Not(dd->vars[bSupp->index]) ); Cudd_Ref( bRes  );
        }
        else
        {
            bResP = Extra_bddGetOneMinterm( dd, bFunc1, cuddT(bSupp) );         Cudd_Ref( bResP );
            bRes  = Cudd_bddAnd( dd, bResP,  dd->vars[bSupp->index] );          Cudd_Ref( bRes  );
        }
    }
    else // the top var belongs to bSupp, not to bFunc
    {
        // add this var in the negative polarity
        bResP = Extra_bddGetOneMinterm( dd, bFunc, cuddT(bSupp) );              Cudd_Ref( bResP );
        bRes  = Cudd_bddAnd( dd, bResP, Cudd_Not(dd->vars[bSupp->index]) );     Cudd_Ref( bRes  );
    }
    Cudd_RecursiveDeref( dd, bResP );
    Cudd_Deref( bRes );
    return bRes;
}
...
/**Function*************************************************************
  ...
  Synopsis    [Returns the integer number of the minterm.]
...
***********************************************************************/
int Extra_bddMintermNum( DdManager * dd, DdNode * bMint )
{
    DdNode * bMintR, * bMint0, * bMint1;
    int Value;
    ...
    bMintR = Cudd_Regular(bMint);
    if ( Cudd_IsComplement(bMint) )
    {
        bMint0 = Cudd_Not( cuddE(bMintR) );
        bMint1 = Cudd_Not( cuddT(bMintR) );
    }
    else
    {
        bMint0 = cuddE(bMintR);
    ...
    {
        assert( bMint1 == b0 );
        Value = Extra_bddMintermNum( dd, bMint0 );
        return (Value << 1);
    }
    else
    {
        assert( bMint1 != b0 );
        Value = Extra_bddMintermNum( dd, bMint1 );
        return (Value << 1) | 1;
    }
}
...
/**Function********************************************************************
  ...
  Description [Returns a bdd composed of elementary bdds found in array BddVars[] such 
  that the bdd vars encode the number Value of bit length CodeWidth (most significant bit 
  is encoded with the *last* bdd variable). If the variables BddVars are not specified,
  takes the first CodeWidth variables of the manager]
...
******************************************************************************/
DdNode * Extra_bddBitsToCube2( DdManager * dd, int Code, int CodeWidth, DdNode ** pbVars )
{
    int z;
    DdNode * bTemp, * bVar, * bVarBdd, * bResult;
...
/**Function********************************************************************
  ...
  Synopsis    [Computes the array of BDD variables from the positive polarty cube.]
...
******************************************************************************/
DdNode ** Extra_bddComputeVarArray( DdManager * dd, DdNode * bCube, int * pnVars )
{
    DdNode ** pbRes;
    int nVars;

    pbRes = ALLOC( DdNode *, dd->size );
    for ( nVars = 0; bCube != b1; bCube = cuddT(bCube), nVars++ )
        pbRes[nVars] = dd->vars[bCube->index];
