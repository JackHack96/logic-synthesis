/**CFile****************************************************************
  ...
  Revision    [$Id: fmnImageMv.c,v 1.2 2003/11/18 18:55:12 alanmi Exp $]
...
typedef struct dcmnImagePartInfoStruct    Fm_ImagePartInfo_t;
struct dcmnImagePartInfoStruct
{
...
static DdNode *   dcmnImageMvCompute( DdManager * dd, DdNode * bCare, DdNode ** pbFuncs, int nFuncs, 
                    Vmx_VarMap_t * pVmxGlo, Vmx_VarMap_t * pVmxLoc, DdNode ** pbVars, int nBddSize );
static DdNode *   dcmnImageMv_rec( DdManager * dd, array_t * pParts, DdNode * bCubeIn, Vmx_VarMap_t * pVmxGlo, int nBddSize );
static DdNode *   dcmnImageMvValue_rec( DdManager * dd, array_t * pParts, int iPart, DdNode * bCubeIn, Vmx_VarMap_t * pVmxGlo, int nBddSize );
static DdNode *   dcmnImageMvSmallSize( DdManager * dd, array_t * pParts, DdNode * bCubeIn, int nBddSize );
static array_t *  dcmnImageMvDisjointSupport( DdManager * dd, array_t * pParts );

static Fm_ImagePartInfo_t * dcmnImageMvPartAllocate( DdNode * bSupp, DdNode ** pbFuncs, DdNode ** pbCodes, int nValues );
static Fm_ImagePartInfo_t * dcmnImageMvPartCreate( DdManager * dd, DdNode * bCare, DdNode * pbFuncs[], DdNode * pbCodes[], int nValues, Vmx_VarMap_t * pVmxGlo, DdNode ** pbImagePart, bool fUseAnd );

static void       dcmnImageMvPartFree( DdManager * dd, Fm_ImagePartInfo_t * p );
static int        dcmnImageMvPartCompareBddSize( Fm_ImagePartInfo_t ** ppPart1, Fm_ImagePartInfo_t ** ppPart2 );
static void       dcmnImageMvPartDeref( DdManager * dd, DdNode ** pbFuncs, int nFuncs );
static void       dcmnImageMvPartConstrain( DdManager * dd, DdNode * bCare, DdNode ** pbFuncs, int nValues, DdNode * pbConstrs[], bool fUseAnd );
static DdNode *   dcmnImageMvPartSupport( DdManager * dd, DdNode ** pbFuncs, int nFuncs, Vmx_VarMap_t * pVmxGlo );
static DdNode *   dcmnImageMvPartConvolve( DdManager * dd, DdNode ** pbFuncs, DdNode ** pbCodes, int nFuncs );
static DdNode *   dcmnImageMvPartImage( DdManager * dd, DdNode * bCubeIn, Fm_ImagePartInfo_t * p );
static void       dcmnImageMvPartArrayFree( DdManager * dd, array_t * pParts );
...
***********************************************************************/
DdNode * Dcmn_ImageMvCompute( DdManager * dd, DdNode * bCare, DdNode ** pbFuncs, int nFuncs, 
    Vmx_VarMap_t * pVmxGlo, Vmx_VarMap_t * pVmxLoc, DdNode ** pbVars, int nBddSize )
{
    Vm_VarMap_t * pVm;
    ...
    for ( i = 0; i < nValuesOut; i++ )
    {
        pbImages[i] = dcmnImageMvCompute( dd, pbCofs[i], pbFuncs, nFuncs, pVmxGlo, pVmxLoc, pbVars, nBddSize );  
        if ( pbImages[i] == NULL )
        {
            dcmnImageMvPartDeref( dd, pbImages, i - 1 );
            dcmnImageMvPartDeref( dd, pbCofs, nValuesOut );
            dcmnImageMvPartDeref( dd, pbCodesLocal, nValuesOut );
            return NULL;
        }
        Cudd_Ref( pbImages[i] );
    }
    bImage = dcmnImageMvPartConvolve( dd, pbImages, pbCodesLocal, nValuesOut ); Cudd_Ref( bImage );
    dcmnImageMvPartDeref( dd, pbCofs, nValuesOut );
    dcmnImageMvPartDeref( dd, pbImages, nValuesOut );
    dcmnImageMvPartDeref( dd, pbCodesLocal, nValuesOut );
    Cudd_Deref( bImage );
    return bImage;
}
...
***********************************************************************/
DdNode * dcmnImageMvCompute( DdManager * dd, DdNode * bCare, DdNode ** pbFuncs, int nFuncs, 
    Vmx_VarMap_t * pVmxGlo, Vmx_VarMap_t * pVmxLoc, DdNode ** pbVars, int nBddSize )
{
	...
	for ( i = 0; i < nVarsIn; i++ )
	{
        ...
        pNew = dcmnImageMvPartCreate( dd, bCare, pbFuncs + iValue, 
    ...
    if ( array_n(pParts) > 0 )
    {
        array_sort( pParts, dcmnImageMvPartCompareBddSize );
	    ...
	    bImageProper = dcmnImageMv_rec( dd, pParts, bCubeIn, pVmxGlo, nBddSize );  
	    if ( bImageProper == NULL )
	    {
		    Cudd_RecursiveDeref( dd, bImage );
	        Cudd_RecursiveDeref( dd, bCubeIn );
		    dcmnImageMvPartArrayFree( dd, pParts );
		    return NULL;
	    }
...
***********************************************************************/
DdNode * dcmnImageMv_rec( DdManager * dd, array_t * pParts, DdNode * bCubeIn, Vmx_VarMap_t * pVmxGlo, int nBddSize )
{
	DdNode * bImage;
    DdNode * pbImages[32];
	...
	if ( array_n(pParts) == 1 )
    {
        ...
        return dcmnImageMvPartImage( dd, bCubeIn, pStart );
    }

	// checking for disjoint supports
	pDisjs = dcmnImageMvDisjointSupport( dd, pParts );
	if ( pDisjs != NULL ) // there are disjoint support groups
	{
		...
		for ( i = 0; i < array_n(pDisjs); i++ )
		{
			...
			bImageNew = dcmnImageMv_rec( dd, pArray, bCubeIn, pVmxGlo, nBddSize );   
			if ( bImageNew == NULL )
			{
				Cudd_RecursiveDeref( dd, bImage );
	...
	if ( bImage = dcmnImageMvSmallSize( dd, pParts, bCubeIn, nBddSize ) )
		return bImage;
    ...
    {
        pbImages[i] = dcmnImageMvValue_rec( dd, pParts, i, bCubeIn, pVmxGlo, nBddSize );  
        if ( pbImages[i] == NULL )
        {
            dcmnImageMvPartDeref( dd, pbImages, i - 1 );
            return NULL;
    ...
    bImage = dcmnImageMvPartConvolve( dd, pbImages, pStart->pbCodes, pStart->nValues );
    Cudd_Ref( bImage );
    dcmnImageMvPartDeref( dd, pbImages, pStart->nValues );
    Cudd_Deref( bImage );
    return bImage;
}
...
***********************************************************************/
DdNode * dcmnImageMvValue_rec( DdManager * dd, array_t * pParts, int iPart, DdNode * bCubeIn, Vmx_VarMap_t * pVmxGlo, int nBddSize )
{
	...
	for ( i = 1; i < array_n(pParts); i++ )
	{
        ...
        // create the partition corresponding to the i-th MV variable
        pNew = dcmnImageMvPartCreate( dd, bCare, pCur->pbFuncs, 
    ...
    {
        array_sort( pPartsNew, dcmnImageMvPartCompareBddSize );
	    // call the recursive image computation
	    bImageProper = dcmnImageMv_rec( dd, pPartsNew, bCubeIn, pVmxGlo, nBddSize );  
	    if ( bImageProper == NULL )
	    {
		    Cudd_RecursiveDeref( dd, bImage );
		    dcmnImageMvPartArrayFree( dd, pPartsNew );
		    return NULL;
	    }
	    Cudd_Ref( bImageProper );
...
***********************************************************************/
array_t * dcmnImageMvDisjointSupport( DdManager * dd, array_t * pParts )
{
...
***********************************************************************/
DdNode * dcmnImageMvSmallSize( DdManager * dd, array_t * pParts, DdNode * bCubeIn, int nBddSize )
{
...
//		bComp = Cudd_bddXnor( dd, pCur->bPart, pCur->bVar );	Cudd_Ref( bComp );
        bComp = dcmnImageMvPartConvolve( dd, pCur->pbFuncs, pCur->pbCodes, pCur->nValues );  Cudd_Ref( bComp );
...
***********************************************************************/
Fm_ImagePartInfo_t * dcmnImageMvPartAllocate( DdNode * bSupp, DdNode ** pbFuncs, DdNode ** pbCodes, int nValues )
{
    Fm_ImagePartInfo_t * p;
    int i;
...
***********************************************************************/
Fm_ImagePartInfo_t * dcmnImageMvPartCreate( DdManager * dd, DdNode * bCare, DdNode * pbFuncs[], 
...
//	bConstr = Cudd_bddConstrain( dd, pbFuncs[i], bCareSet ); Cudd_Ref( bConstr );
    dcmnImageMvPartConstrain( dd, bCare, pbFuncs, nValues, pbConstrs, fUseAnd );
    // compute the support of the partition
    bSupp = dcmnImageMvPartSupport( dd, pbConstrs, nValues, pVmxGlo ); Cudd_Ref( bSupp );
...
//		bImage = Cudd_bddAnd( dd, bTemp = bImage, Cudd_NotCond(pbVars[i], (bConstr == b0)) );  Cudd_Ref( bImage );
        *pbImagePart = dcmnImageMvPartConvolve( dd, pbConstrs, pbCodes, nValues );  Cudd_Ref( *pbImagePart );
        pPart = NULL;
    }
	else
	{
		pPart = dcmnImageMvPartAllocate( bSupp, pbConstrs, pbCodes, nValues );
        *pbImagePart = NULL;
	}
    // deref the support
    Cudd_RecursiveDeref( dd, bSupp );
    // dereference the computed constrain
    dcmnImageMvPartDeref( dd, pbConstrs, nValues );
    return pPart;
}
...
***********************************************************************/
void dcmnImageMvPartFree( DdManager * dd, Fm_ImagePartInfo_t * p )
{
    int i;
...
***********************************************************************/
int dcmnImageMvPartCompareBddSize( Fm_ImagePartInfo_t ** ppPart1, Fm_ImagePartInfo_t ** ppPart2 )
{
...
***********************************************************************/
void dcmnImageMvPartDeref( DdManager * dd, DdNode ** pbFuncs, int nFuncs )
{
    int i;
    for ( i = 0; i < nFuncs; i++ )
...
***********************************************************************/
void dcmnImageMvPartConstrain( DdManager * dd, DdNode * bCare,
...
***********************************************************************/
DdNode * dcmnImageMvPartSupport( DdManager * dd, DdNode ** pbFuncs, int nFuncs, Vmx_VarMap_t * pVmxGlo )
{
...
***********************************************************************/
DdNode * dcmnImageMvPartConvolve( DdManager * dd, DdNode ** pbFuncs, DdNode ** pbCodes, int nFuncs )
{
...
***********************************************************************/
DdNode * dcmnImageMvPartImage( DdManager * dd, DdNode * bCubeIn, Fm_ImagePartInfo_t * p )
{
    DdNode * bRes, * bPart;
    bPart = dcmnImageMvPartConvolve( dd, p->pbFuncs, p->pbCodes, p->nValues );  Cudd_Ref( bPart );
...
***********************************************************************/
void dcmnImageMvPartArrayFree( DdManager * dd, array_t * pParts )
{
	int i;
	Fm_ImagePartInfo_t * pCur;
	// free the partitions
...
******************************************************************************/
void dcmnImageMvTimeoutSet( int timeout )
{
	s_Timeout = timeout;
}
