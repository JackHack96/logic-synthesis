...
i++) {
inputs[i].
name = mk_var_name();                        /* default */
inputs[i].
boolean_flag = FALSE;                        /* undefined */
inputs[i].
enumtype_ptr = 0;                        /* undefined */
inputs[i].
entries = ALLOC(struct Entry, 1);        /* undefined */
}
} else if (!
strcmp(targv[0],
".o")) {
...
inputs[i].
name = util_strsav(targv[i + 1]);    /* assign new name */
}
} else if (!
strcmp(targv[0],
".ob")) {
...
enumtypes[k].
codes = ALLOC(struct Code, nc);
for (
i = 0;
i<nc;
i++) {
enumtypes[k].codes[i].
assigned = FALSE;                /* undefined */
enumtypes[k].codes[i].
bit_vector =
int_to_binary(i, nb);
enumtypes[k].codes[i].
symbol_ptr = 0;                /* undefined */
}

enumtypes[k].
symbols = ALLOC(struct Symbol, ns);
for (
i = 0;
i<ns;
i++) {
enumtypes[k].symbols[i].
token = util_strsav(targv[i + 4]);
enumtypes[k].symbols[i].
code_ptr = i;
/*
 * assign codes
...
(void)
fprintf(stderr,
"warning: ignored questionable input ...\n");
(void)
fprintf(stderr,
"----> %s", line);
}
...
(void)
fprintf(stderr,
"jedi: unable to find input truth table\n");
exit(1);
}
...
inputs[ni-1].
boolean_flag = FALSE;                /* presentState input */
} else if (!
strcmp(targv[0],
".o")) {
if (oflag) {
...
outputs[0].
boolean_flag = FALSE;                /* nextState output */
} else if (!
strcmp(targv[0],
".ilb")) {
if (!iflag) {
...
enumtypes[0].
symbols = ALLOC(struct Symbol, 1);        /* undefined */
enumtypes[0].
links = ALLOC(struct Link *, 1);        /* undefined */
...
inputs[l].entries[k].
token = util_strsav(targv[1]);
inputs[l].entries[k].
enumtype_ptr = 0;

if (!
check_dont_care(inputs[l]
.entries[k].token)) {
found = FALSE;
for (
j = 0;
...
enumtypes[0].ns++;
enumtypes[0].
symbols = REALLOC(struct Symbol,
                  enumtypes[0].symbols, enumtypes[0].ns);
m       = enumtypes[0].ns - 1;

inputs[l].entries[k].
symbol_ptr = m;
enumtypes[0].symbols[m].
token =
...
outputs[0].entries[k].
token = util_strsav(targv[2]);
outputs[0].entries[k].
enumtype_ptr = 0;

if (!
check_dont_care(outputs[0]
.entries[k].token)) {
found = FALSE;
for (
j = 0;
...
(void)
fprintf(stderr,
"warning: ignored incorrect input ...\n");
(void)
fprintf(stderr,
"----> %s", line);
}
 ...
 */
enumtypes[0].
links = ALLOC(struct Link *, enumtypes[0].ns);
for (
i = 0;
i<enumtypes[0].
ns;
