...
FILE *fout;
{
  ...
  while (fgets(line, BUFSIZE, fin) != NULL) {
    ...
    if (targc == 0) {
      ...
      if (iflag) {
        (void)fprintf(stderr, "warning: .i has been declared more than once\n");
        continue;
      }
      ...
      if (oflag) {
        (void)fprintf(stderr, "warning: .o has been declared more than once\n");
        continue;
      }
      ...
      if (!iflag) {
        (void)fprintf(stderr, "fatal error: .i has not been defined\n");
        exit(1);
      }
      ...
      if (!oflag) {
        (void)fprintf(stderr, "fatal error: .o has not been defined\n");
        exit(1);
      }
      ...
      for (i = 0; i < nc; i++) {
        ...
        enumtypes[k].codes[i].bit_vector = int_to_binary(i, nb);
      ...
      for (i = 0; i < ns; i++) {
        enumtypes[k].links[i] = ALLOC(struct Link, ns);
        for (j = 0; j < ns; j++) {
       ...
       */
      for (i = 0; i < ni; i++) {
        inputs[i].entries = REALLOC(struct Entry, inputs[i].entries, np);
        inputs[i].entries[k].token = util_strsav(targv[i]);
        /*
         * if not boolean type
         */
        if (!inputs[i].boolean_flag) {
          l = inputs[i].entries[k].enumtype_ptr = inputs[i].enumtype_ptr;
       ...
       */
      for (i = 0; i < no; i++) {
        outputs[i].entries = REALLOC(struct Entry, outputs[i].entries, np);
        outputs[i].entries[k].token = util_strsav(targv[i + ni]);
        /*
         * if not boolean type
         */
        if (!outputs[i].boolean_flag) {
          l = outputs[i].entries[k].enumtype_ptr = outputs[i].enumtype_ptr;
          for (j = 0; j < enumtypes[l].ns; j++) {
...
FILE *fout;
{
  ...
  while (fgets(line, BUFSIZE, fin) != NULL) {
    ...
    if (targc == 0) {
      ...
      if (sflag) {
        (void)fprintf(stderr, "warning: .s has been declared more than once\n");
        continue;
      }
      ...
      if (bitsFlag) {
        if (code_length < t) {
          (void)fprintf(stderr, "warning: code length %d is not long enough: ",
                        code_length);
      ...
      for (i = 0; i < nc; i++) {
        ...
        enumtypes[0].codes[i].bit_vector = int_to_binary(i, nb);
      ...
      if (!iflag || !oflag || !sflag) {
        (void)fprintf(
            stderr, "fatal error: either .i, .o, or .s has not been defined\n");
        exit(1);
      }
       ...
       * parse presentState
       */
      inputs[l].entries = REALLOC(struct Entry, inputs[l].entries, np);
      ...
      if (!check_dont_care(inputs[l].entries[k].token)) {
        ...
        if (!found) {
          enumtypes[0].ns++;
          enumtypes[0].symbols =
              REALLOC(struct Symbol, enumtypes[0].symbols, enumtypes[0].ns);
          m = enumtypes[0].ns - 1;
       ...
       * parse nextState
       */
      outputs[0].entries = REALLOC(struct Entry, outputs[0].entries, np);
  ...
  for (i = 0; i < enumtypes[0].ns; i++) {
    enumtypes[0].links[i] = ALLOC(struct Link, enumtypes[0].ns);
  }
