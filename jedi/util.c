...
int distance(code1, code2, width) char *code1, *code2;
int width;
{
  int dist;
  int pdist;
...
char *int_to_binary(num, width) int num, width;
{
  char *buffer;
  int i;
  int j;
...
int binary_to_int(binary, width) char *binary;
int width;
{
  int i, total;
  int mask;
...
parse_line(line) register char *line;
{
...
FILE *my_fopen(fname, mode) char *fname;
char *mode;
{
  FILE *fp;

  if ((fp = fopen(fname, mode)) == NULL) {
    (void)fprintf(stderr, "error: couldn't open file %s under mode (%s)\n",
                  fname, mode);
    exit(1);
  }
