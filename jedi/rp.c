...
void combinatorial_optimize(cost_function, generate_function, accept_state,
                            reject_state, fin, fout, ferr, beginning_states,
                            ending_states, starting_temperature,
                            maximum_temperature, problem_size,
                            verbose) int (*cost_function)();
...
should_accept(temp, cost_function) double temp;
...
double update_temp(temp, start_temp) double temp;
double start_temp;
{
...
inner_loop_criterion(num_states_per_stage,
                     num_states_generated) int num_states_per_stage,
...
stopping_criterion(c1, c2, c3, c4, stop_temp, temp) double c1, c2, c3, c4;
...
double find_starting_temp(starting_temperature,
                          maximum_temperature) double starting_temperature,
    maximum_temperature;
{
  double delta_c;
  double start_temp;
...
double determine_cost(cost_function) int (*cost_function)();
{
  int ret_val;
  double cost = 0.0;
...
generate_new_state(range, generate_function) int range;
...
double random_generator(left, right) int left, right;
{
...
int int_random_generator(x, y) int x, y;
{
  int i;
