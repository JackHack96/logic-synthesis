...
cluster_embed(type) struct Enumtype type;
{
  int j;
  int ns, nc;
  int *sa;
...
int select_state(type, sa) struct Enumtype type;
int *sa;
{
  int s, i, j;
  int ns, we;
...
int select_code(type, sa, s) struct Enumtype type;
int *sa, s;
{
  int c;
  int best, force;
