...
int         SendTokenBack; // used to send the token back to the input stream in case of
// a missing keywork
...
int kwrd_cmp(char *name, char *keywrd) {
    ...
    if ((len = (int) strlen(name)) != (strlen(keywrd)))
        return 0;
    if (!len)
        return 0;
    /* if length 0 exit */
...
void check_args(int argc, char **argv) {
    ...
    LOWERCASE = 1;
    INIT      = '3';
    while ((c = (char) getopt(argc, argv, "s:S:d:D:c:C:i:I:l:L:hHvVuUnN$")) > 0) {
        switch (toupper(c)) {
            ...
            case 'L':
                if (kwrd_cmp(optarg, "IN")) {
                    ...
                    if (kwrd_cmp(optarg, "OUT")) {
                        ...
                        (void) fprintf(stderr,
                                       "\tUnknow direction for a port of type linkage\n");
                        help = 1;
                    }
                }
                break;
    ...
    if (help) {
        (void) fprintf(stderr,
                       "\tUsage: vst2blif [options] <library> [infile [outfile]]\n");
        (void) fprintf(
                stderr, "\t\t if outfile is not given will be used stdout, if also\n");
        (void) fprintf(stderr,
        ...
        (void) fprintf(
                stderr,
                "\t-c <name>\t .clock <name>  will be added to the blif file\n");
        (void) fprintf(
                stderr,
                "\t-i <value>\t default value for latches, must be between 0 and 3\n");
        (void) fprintf(stderr,
                       "\t-l <in/out>\t sets the direction for linkage ports\n");
                            ...
                            if
                                ...
                                if
                                    ...
                                    if (num >= (MAXTOKENLEN - 1)) {
                                        sentback = c;
                                        (void) fprintf(stderr,
                                                       "*Parse warning* Line %u: token too long !\n",
                                                       line);
                                    } else {
                                        if (LOWERCASE)
                                            *t = tolower(c);
...
void addSIG(struct SIGstruct **SIGptr, char *name, char dir, int start,
    ...
    (*SIGptr)->next  = NULL;
#ifdef DEBUG
    (void)fprintf(stderr, "\n\t\tAdded SIGNAL <%s>, dir = %c, start =%d, end =%d",
                  (*SIGptr)->name, (*SIGptr)->dir, (*SIGptr)->start,
                  (*SIGptr)->end);
#endif
}
...
void vst_error(char *name, char *next) {
    ...
    (void) fprintf(
            stderr,
            "*print_error* Line %u : skipping text until the keyword %s is reached\n",
            line, next);
    SendTokenBack = 1;
    do {
        get_next_token(w);
...
struct LibCell *new_lib_cell(char *name, struct BITstruct *ports, int latch) {
    ...
    if ((tmp->formals = (struct Ports *) calloc(1, num * sizeof(struct Ports))) ==
        NULL) {
        print_error("Allocation print_error or not enought memory !");
    }
    ...
    tmp->npins = num;
    num--;
    for (bptr = ports->next, pptr = tmp->formals, j = 0;
         (j < num) && (bptr != NULL); pptr++, j++, bptr = bptr->next) {
        if (j > num)
            print_error("(new_lib_cell) error ...");
#ifdef DEBUG
...
struct LibCell *scan_library(char *LibName) {
    ...
    if ((first.next)->next == NULL) {
        fprintf(stderr, "Library file %s does *NOT* contains gates !", LibName);
    ...
    if (Bports != NULL) {
        ...
        if (t == 'S') {
            ...
            for (j = 1, Lpptr = Lptr->formals; j < num; j++, Lpptr++, Pptr++) {
                ...
                if (Fptr == NULL)
                    (void) print_error(
                            "Mismatch between COMPONENT declaration and library one");
            }
            if (t == 'L') {
                Fptr = Fports->next;
                while (Fptr != NULL) {
                ...
                if (Fptr == NULL)
                    (void) print_error(
                            "Mismatch between COMPONENT declaration and libray one");
            }
            releaseBIT(Fports->next);
            tmp->io      = Bports->next;
            Bports->next = NULL;
                    ...
                    if (kwrd_cmp(w, "OUT")) {
                        ...
                        if (kwrd_cmp(w, "INOUT")) {
                            ...
                            if (kwrd_cmp(w, "LINKAGE")) {
                                ...
                                (void) fprintf(
                                        stderr,
                                        "* print_error * Line %u : unknown direction of a port\n",
                                        line);
...
#ifdef DEBUG
                                (void)fprintf(stderr, "\nadded vector of name : %s[%d..%d]",
                                              TMPptr->name, start, end);
#endif
                            } else {
                                for (j = start; j <= end; j++) {
                                    (void) sprintf(tmp, "%s[%d]", TMPptr->name, j);
...
void get_entity(struct Cell **Entity) {
    ...
    if (kwrd_cmp(w, "GENERIC")) {
        get_next_token(w);
        if (*w != '(')
            warning("expected '(' after GENERIC keyword");
        num = 1;
        do {
            get_next_token(w);
            if (*w == '(')
...
void get_signal(struct SIGstruct **Internals) {
    ...
    while (SIGptr->next != NULL) {
        ...
        if (vect) {
#ifdef DEBUG
            (void)fprintf(stderr, "\nadded vector of name : %s[%d..%d]", name, start,
                          end);
#endif
...
#ifdef DEBUG
    (void)fprintf(stderr,
                  "\n=========================\nParsing instance %s of component "
                  "%s\n==========================\n",
                  name, cell->name);
#endif
...
#ifdef DEBUG
                    (void)fprintf(stderr,
                                  "(%d)\tAptr->name=<%s>\tCptr->name=<%s>\tIname=<%s>\n",
                                  j, Aptr->name, Cptr->name, Iname);
#endif
                    if (kwrd_cmp(Cptr->name, Iname))
                        break;
                }
                if (ACTptr->start < 0) {
                    /*
...
#endif
            /*
                        for(iF=FORMterm.start, iA=ACTterm.start;
               iF!=FORMterm.end+incF; iF+=incF, iA+=incA) {
                        ...
                        if (iA!=ACTterm.end+incA) {
                            warning("Actual vector's dimension differs to formal's
               one");
                        }
            */
        }
    ...
    while (Iptr != NULL) {
        switch ((Iptr->what)->type) {
            case 'S':(void) fprintf(Out, ".subckt %s ", (Iptr->what)->name);
                ...
                for (j = 0, Aptr++, Fptr++; j < (Iptr->what)->npins - 1;
                     j++, Aptr++, Fptr++) {
                    (void) fprintf(Out, "%s=%s ", Fptr->name, Aptr->name);
                }
                break;
            case 'L':(void) fprintf(Out, ".latch %s ", (Iptr->what)->name);
                print_ordered_signals(Iptr->actuals,
                                      what_lib_gate((Iptr->what)->name, LIBRARY));
