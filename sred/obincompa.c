...

{
  ...
  for (i = 0; i < np; i++) {
    for (j = 0; j < i; j++) {
      if (strcmp(itable[i].input, itable[j].input) == 0) {
        if (strbar(itable[i].output) != 1 && strbar(itable[j].output) != 1 &&
            boolcmp(itable[i].output, itable[j].output) != 0) {
  ...
  while (newpair == 1) {
    ...
    for (i = 0; i < np; i++) {
      for (j = 0; j < i; j++) {
        if (strcmp(itable[i].input, itable[j].input) == 0) {
          if (is_in_set(GETSET(incograph, itable[i].nlab - 1),
                        itable[j].nlab - 1)) {
            if (!is_in_set(GETSET(incograph, itable[i].plab - 1),
                           itable[j].plab - 1)) {
              set_insert(GETSET(incograph, itable[i].plab - 1),
                         itable[j].plab - 1);
              set_insert(GETSET(incograph, itable[j].plab - 1),
                         itable[i].plab - 1);
              newpair = 1;
            }
          }
