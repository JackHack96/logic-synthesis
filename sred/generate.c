 ...
 *
 * $Source: /users/pchong/CVS/sis/sred/generate.c,v $
 * $Author: pchong $
 * $Revision: 1.1.1.1 $
...
pointer = 0;
for (i=0; i<chain; i++) pointer += pCHAINS->weight[i];
pointer += genclass-1;
...
for (i=0; i<np;  i++) /* loop on the symbolic product terms */
{
  ...
  {
    if (is_in_set (GETSET(pCHAINS->implied, pointer), itable[i].plab-1) &&
         itable[i].nlab != 0)
          set_insert (class, itable[i].nlab-1);
  }
} 
