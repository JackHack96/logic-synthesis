 ...
 *
 * $Source: /users/pchong/CVS/sis/sred/clinclus.c,v $
 * $Author: pchong $
 * $Revision: 1.1.1.1 $
...
cl_inclus(genclass,subclass,index)
int genclass,subclass,index;

{
  ...
  { 
    ...
    {
      if ( is_in_set (classi, i) ) inclusa = 1; else inclusa = 0;
    }
    if ( ! is_in_set (classj, i) && is_in_set (classi, i) ) strict++;
    i++;
  }
  
  if ( inclusa == 1  && strict > 0 ) value = 1; else value = 0;
/*printf("cl_inclus  = %d\n", value);*/
  set_free (classi);
  set_free (classj);
  return(value);
