 ...
 *
 * $Source: /users/pchong/CVS/sis/sred/newnstate.c,v $
 * $Author: pchong $
 * $Revision: 1.1.1.1 $
  ...
  /* finds the next-states class ( i.e. the next state in the reduced 
    ...
    for(i=0; i<np; i++)
    {
      if ( strcmp(itable[i].input,itable[ingresso].input) == 0 )
      {
        if ( is_in_set (GETSET(copertura1, pclass-1), itable[i].plab-1))
        {
          if ( itable[i].nlab != 0 ) set_insert (nclass, itable[i].nlab-1);
        }
      }
    }
    ...
    if ( count == 0 ) value = -1; else value = i + 1;
