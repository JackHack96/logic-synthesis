...
int debug_level; /* how deep in the recursion to provide info */
{
  ...
  nelem = 0;
  sm_foreach_row(A, prow) { nelem += prow->length; }
  sparsity = (double)nelem / (double)(A->nrows * A->ncols);

  /* Determine an upper bound on the solution */
  bound = 1;
  sm_foreach_col(A, pcol) { bound += WEIGHT(weight, pcol->col_num); }
...
stats_t *stats;
{
  ...
  if (debug) {
    ...
    (void)printf(" %3dx%3d sel=%3d bnd=%3d lb=%3d %12s ", A->nrows, A->ncols,
                 select->cost + stats->gimpel, bound + stats->gimpel,
                 lb_new + stats->gimpel,
                 util_print_time(util_cpu_time() - stats->start_time));
  }
  ...
  if (lb_new >= bound) {
    ...
    stats->component++;
    best1 = sm_mincov(L, select1, weight, 0, bound - select->cost, depth + 1,
                      stats);
    stats->component--;
...
solution_t *indep;
{
  ...
  if (indep != NIL(solution_t)) {
    ...
    sm_foreach_col(A, pcol) { (void)sm_row_insert(indep_cols, pcol->col_num); }
  }
