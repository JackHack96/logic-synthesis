...
sm_row *sm_mat_bin_minimum_cover(sm_matrix *M, int *weights, int heuristic,
                                 int debug, int ubound, int option,
                                 int (*record_fun)()) {
    is_unate = 0;
    return mat_minimum_cover(M, weights, heuristic, debug, ubound, option,
                             record_fun);
}

sm_row *sm_mat_minimum_cover(sm_matrix *M, int *weights, int heuristic,
                             int debug, int ubound, int option,
                             int (*record_fun)()) {
    is_unate = 1;
    return mat_minimum_cover(M, weights, heuristic, debug, ubound, option,
...
static sm_row *mat_minimum_cover(sm_matrix *M, int *w, int heuristic,
                                 int l_debug, int ubound, int l_option,
                                 int (*record_fun)()) {
    ...
    if (g_debug > 0) {
        ...
        nelem = 0;
        sm_foreach_row(M, prow) { nelem += prow->length; }
    ...
    if (g_option & 2048) {
        ...
        unate =
                set_clear(set_new(M->last_col->col_num + 1), M->last_col->col_num + 1);
        do {
            added = 0;
            temp  = sm_alloc();
            /* find unate columns */
            sm_foreach_col(M, pcol) {
                if (pcol->col_num % 2) {
                    if (!pcol->prev_col || pcol->prev_col->col_num != pcol->col_num - 1) {
                        set_insert(unate, pcol->col_num);
                        set_insert(unate, (pcol->col_num - 1));
                    }
                } else {
                    if (!pcol->next_col || pcol->next_col->col_num != pcol->col_num + 1) {
                        set_insert(unate, pcol->col_num);
                        set_insert(unate, (pcol->col_num + 1));
                    }
                }
            ...
            for (i = 0; M->nrows && i <= M->last_row->row_num; i++) {
                ...
                toadd = 1;
                sm_foreach_row_element(prow, elem) {
                    if (!(elem->col_num % 2) && !is_in_set(unate, elem->col_num)) {
                        toadd = 0;
                        break;
                    }
                }
    ...
    if (ubound > 0) {
        ...
        sm_foreach_col(A, pcol) { best->cost += WEIGHT(weights, pcol->col_num); }
    }
    best->cost++;
    ...
    if (!g_record_fun) {
        ...
        if (g_debug > 0) {
            (void) fprintf(misout,
                           "calls %d leaves %d r1 %d r2 %d r3 %d r4 %d r5 %d\n",
        ...
        cost = 0;
        sm_foreach_row_element(result, elem) { cost += WEIGHT(w, elem->col_num); }
        if (cost != best->cost) {
...
static sm_matrix *reorder_rows(sm_matrix *M) {
...
void sm_bin_mincov(int row_num) {
    sm_row     *curr_row;
        ...
        if (g_record_fun) {
            ...
            if (g_debug > 0) {
                (void) fprintf(misout, "  new 'best' solution %d; time %s; calls %d\n",
                               best->cost, util_print_time(util_cpu_time() - start_time),
                               call_count);
                (void) fflush(misout);
#ifdef DEBUG
                if (g_debug > 5) {
    ...
    for (col = 0; !ended && col < curr_row->length; col++) {
        ...
        for (uplevel_elem = elem->prev_row; uplevel_elem != NIL(sm_element);
            ...
            if (expanded[uplevel_elem->row_num]) {
                if ((EXPANDED & (sm_get(int, uplevel_elem)))) {
                    if (!(MARKED & (sm_get(int, expanded[uplevel_elem->row_num])))) {
                        break;
                    }
                } else {
                    break;
        ...
        if (uplevel_elem != NIL(sm_element)) {
             ...
             */
            if (!(EXPANDED & (sm_get(int, uplevel_elem)))) {
                sm_put(uplevel_elem, ((sm_get(
                        int, uplevel_elem)) | MARKED));
#ifdef DEBUG
                if (g_debug > 9) {
    ...
    sm_foreach_row_element(curr_row, elem) { sm_put(elem, 0); }
 ...
 */
static int compare(value_t *value1, value_t *value2) {
    return value2->value - value1->value;
}

static int compare_rev(value_t *value1, value_t *value2) {
    return value1->value - value2->value;
}

/* debugging stuff */
static char *literal_name(int literal) {
    static char buf[20];
...
void print_lev(int row_num, char *p1, char *p2, char *p3, char *p4) {
    int i;
...
void ps(pset set) { (void) fprintf(misout, ps1(set)); }

void pm(sm_matrix *mat) { sm_print(misout, mat); }

void wm(sm_matrix *mat) { sm_write(misout, mat); }
