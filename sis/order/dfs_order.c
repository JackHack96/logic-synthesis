 ...
 */
array_t *order_dfs(array_t *roots, st_table *leaves, int fixed_root_order) {
    int          order_count;
    node_t       *node;
    st_generator *gen;
    ...
    (void) extract_transitive_fanin(internal_roots, leaves, tfi_inputs_first,
                                    tfi_outputs_first);
    ...
    (void) order_nodes_rec(internal_roots, info_table, visited, leaves, order_list,
                           &order_count);
 ...
 * ordering technique.
 */
array_t *bdd_tfi_inputs_first(array_t *roots, st_table *leaves) {
    int      i;
    node_t   *node;
    ...
    for (i = 0; i < array_n(roots); i++) {
        ...
        (void) st_insert(tfi, (char *) node,
                         NIL(char)); /* tfi table just used for quick searching */
    }
 ...
 */
static void
extract_transitive_fanin(array_t *roots, st_table *leaves, array_t *tfi_inputs_first, array_t *tfi_outputs_first) {
    int          i;
    node_t       *node;
    st_generator *gen;
    st_table     *tfi     = st_init_table(st_ptrcmp, st_ptrhash);
    ...
    st_foreach_item(leaves, gen, (char **) &node, NIL(
            char *)) {
 ...
 */
static void visit_inputs_first(node_t *node, st_table *leaves, st_table *tfi, array_t *tfi_inputs_first) {
    int    i;
    node_t *fanin;
     ...
     */
    if (st_lookup(tfi, (char *) node, NIL(char *)))
        return;
...
static void visit_outputs_first(node_t *node, st_table *visited, st_table *tfi, array_t *tfi_outputs_first) {
    lsGen  gen;
    node_t *fanout;
...
static void propagate_level_to_fanins(node_t *node, st_table *info_table) {
    int      i;
    info_t   *info;
    int      level;
    node_t   *factor_node;
    ...
    assert(st_lookup(info_table, (char *) node, (char **) &info));
    level = info->level;
    (void) propagate_level_thru_factored_form(factor_node, level, factor_table,
                                              info_table);
    for (i = 0; i < array_n(factor_array); i++) {
        factor_node = array_fetch(node_t *, factor_array, i);
...
static void propagate_level_thru_factored_form(node_t *node, int level, st_table *factor_table, st_table *info_table) {
    int    i;
    node_t *fanin;
    info_t *info;
    ...
    foreach_fanin(node, i, fanin) {
        if (st_lookup(factor_table, (char *) fanin, NIL(char *))) {
            (void) propagate_level_thru_factored_form(fanin, level + 1, factor_table,
                                                      info_table);
            continue;
        }
...
static int propagate_max_leaf_from_fanins(node_t *node, st_table *info_table) {
    int    i;
    info_t *info;
    node_t *fanin;
    int    max_leaf = 0;
...
static void
order_nodes_rec(array_t *roots, st_table *info_table, st_table *visited, st_table *leaves, array_t *node_list,
                int *order_count) {
    int     *slot;
    ...
    for (i = 0; i < array_n(info_array); i++) {
        ...
        if (st_find(leaves, (char *) node, (char ***) &slot)) {
            *slot = (*order_count)++;
            ...
            (void) order_nodes_rec(fanin_array, info_table, visited, leaves, node_list,
                                   order_count);
...
static int max_leaf_cmp(char **key1, char **key2) {
