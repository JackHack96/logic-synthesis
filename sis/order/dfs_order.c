 ...
 * ordering technique.
 */
array_t *bdd_tfi_inputs_first(roots, leaves) array_t *roots;
st_table *leaves;
{
  int i;
  node_t *node;
 ...
 */
static void extract_transitive_fanin(roots, leaves, tfi_inputs_first,
                                     tfi_outputs_first) array_t *roots;
st_table *leaves;
 ...
 */
static void visit_inputs_first(node, leaves, tfi,
                               tfi_inputs_first) node_t *node;
st_table *leaves;
st_table *tfi;
array_t *tfi_inputs_first;
{
...
static void visit_outputs_first(node, visited, tfi,
                                tfi_outputs_first) node_t *node;
st_table *visited;
st_table *tfi;
array_t *tfi_outputs_first;
{
...
static void propagate_level_to_fanins(node, info_table) node_t *node;
st_table *info_table;
{
  int i;
  info_t *info;
...
static void propagate_level_thru_factored_form(node, level, factor_table,
                                               info_table) node_t *node;
int level;
...
static int propagate_max_leaf_from_fanins(node, info_table) node_t *node;
st_table *info_table;
{
  int i;
  info_t *info;
...
static void order_nodes_rec(roots, info_table, visited, leaves, node_list,
                            order_count) array_t *roots;
st_table *info_table;
st_table *visited;
st_table *leaves;
array_t *node_list;
...
static int max_leaf_cmp(key1, key2) char **key1, **key2;
{
