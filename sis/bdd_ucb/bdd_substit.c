        ...
        array_t *new_array;    /* of bdd_t */
{
    ...
    new_vars = array_alloc(
    int, (unsigned int) array_n(old_vars));
    for (i = 0; i < array_n(old_vars); i++) {
        old_var = array_fetch(
        int, old_vars, i);
        st_lookup_int(map, (refany) old_var, &new_var);
        (void) array_insert(
        int, new_vars, i, new_var);
    }
        ...
        array_t *new_vars;
{
    ...
    int, old_vars, array_n(old_vars) - 1);
    if (fId > last_varId) {
        /*
    ...
    if (fId > top_varId) {
        ...
        if (fId < top_varId) {
            ...
            new_varId  = array_fetch(
            int, new_vars, old_index);
        }
