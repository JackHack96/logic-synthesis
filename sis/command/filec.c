 ...
 */
char *fgets_filec(buf, size, stream, prompt)
	char *buf;int size;FILE *stream;char *prompt; {
	...
	while ((n_read = read(sno, buf, size)) > 0) {
		...
		if (file == NIL(char)) {
			...
			*file++ = '\0';
			modname = 1;
			path = (*last_word == '~') ?
					util_tilde_expand(last_word) : last_word;
		}
		len = strlen(file);
		dir = opendir(path);
			...
			while ((dp = readdir(dir)) != NIL(struct dirent)) {
				if (strncmp(file, dp->d_name, len) == 0) {
					if (last_char == ESC) {
						...
						array_insert_last(char *, names,
								util_strsav(dp->d_name));
					}
				}
			}
			(void) closedir(dir);
...
static int cmp(s1, s2)
	char **s1, **s2; {
	return (strcmp(*s1, *s2));
}

static int match(newmatch, lastmatch, actual)
	char *newmatch, *lastmatch, *actual; {
	int i = 0;
 ...
 */
char * hist_subst(line, changed)
	char *line;int *changed; {
	...
	*changed = 0;
	...
	if (*line == SUBST) {
		...
		*new++ = '\0'; /* makes change in contents of line */
		...
		if (start == NIL(char)) {
			*--new = SUBST;
			bad_modify: (void) fprintf(stderr, "Modifier failed\n");
			return (NIL(char));
		}
		while (last != start) {
			*b++ = *last++;
...
static char * bad_event(n)
	int n; {
	(void) fprintf(stderr, "Event %d not found\n", n);
	return (NIL(char));
}

static char * do_subst(dest, new)
	char *dest, *new; {
	while (*dest = *new++) {
		dest++;
	}
