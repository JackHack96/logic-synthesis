...
/* ARGSUSED */
static int sigterm(sig, code, scp) {
  (void)signal(SIGINT, SIG_IGN); /* ignore further ctl-c */
  longjmp(env, 1);
}
...
com_execute(network, command) network_t **network;
char *command;
{
  int status, argc;
  int loop;
...
static int com_dispatch(network, argc, argv) network_t **network;
int argc;
char **argv;
{
  int status;
 ...
 */
static int apply_alias(network, argcp, argvp, loop) network_t **network;
int *argcp;
char ***argvp;
int *loop;
{
...
static char *split_line(command, argc, argv) char *command;
int *argc;
char ***argv;
{
...
static int check_shell_escape(p, status) char *p;
int *status;
{
  char *value;
  while (isspace(*p)) {
