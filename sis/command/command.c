...
#else
/* ARGSUSED */
static RETSIGTYPE sigterm( sig, code, scp) {
	(void) signal(SIGINT, SIG_IGN); /* ignore further ctl-c */
	longjmp(env, 1);
}
#endif

int com_execute(network, command)
	network_t **network;char *command; {
	int status, argc;
	int loop;
	char *commandp, **argv;
...
static int com_dispatch(network, argc, argv)
	network_t **network;int argc;char **argv; {
	int status;
	char *value;
 ...
 */
static int apply_alias(network, argcp, argvp, loop)
	network_t **network;int *argcp;char ***argvp;int *loop; {
	...
	for (; *loop < 20; (*loop)++) {
		...
		if (stopit != 0
				|| avl_lookup(alias_table, argv[0], (char **) &alias) == 0) {
			return 0;
		}
...
static char * split_line(command, argc, argv)
...
static int check_shell_escape(p, status)
	char *p;int *status; {
	char *value;
	while (isspace(*p)) {
		p++;
