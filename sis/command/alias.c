...
/* ARGSUSED */
int com_alias(network_t **network, int argc, char **argv) {
    int           i;
    char          *key, *value;
    alias_descr_t *alias;
    avl_generator *gen;
    ...
    if (argc == 1) {
        avl_foreach_item(alias_table, gen, AVL_FORWARD, &key, &value) {
            print_alias(value);
        }
        return 0;

    } else if (argc == 2) {
        if (avl_lookup(alias_table, argv[1], &value)) {
            print_alias(value);
        }
        return 0;
    }
    ...
    key = argv[1];
    if (avl_delete(alias_table, &key, &value)) {
...
/* ARGSUSED */
int com_unalias(network_t **network, int argc, char **argv) {
    int  i;
    char *key, *value;

    if (argc < 2) {
        (void) fprintf(miserr, "usage: unalias name1 name2 ...\n");
        return 1;
    }
...
void com_free_argv(int argc, char **argv) {
    int i;
...
void com_alias_free(char *value) {
