...
int com_source(network, argc, argv)
	...
	char *prompt_string, *real_filename, line[1024], *command;
	delay_model_t model;
	FILE *fp;
	...
	usage: (void) fprintf(miserr, "source [-psx] filename\n");
...
/* ARGSUSED */
static int end_loop(network, lp_lits, lp_time, model)
	network_t **network;int lp_lits, lp_time;delay_model_t model; {
