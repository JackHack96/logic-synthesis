...
while ((
c = util_getopt(argc, argv, "ipsxltm:")
) != EOF) {
switch (c) {
...
FREE(real_filename);
return !
silent;    /* error return if not silent */
}
...
#ifdef SIS
if (prompt_string == NIL(char)) prompt_string = "sis> ";
#else
if (prompt_string ==
...
if (
fgets_filec(line, MAX_STR, fp, prompt_string) == NULL) {
if (interactive) {
if (quit_count++ < 5) {
(void)
fprintf(miserr,
...
((lp_count <= 0) || !
end_loop(network, lp_lits, lp_time, model
)));
...
(void)
fprintf(miserr,
"\t-p supply prompt before reading each line\n");
(void)
fprintf(miserr,
"\t-s silently ignore nonexistant file\n");
(void)
fprintf(miserr,
"\t-x echo each line as it is executed\n");
(void)
fprintf(miserr,
"\t-l loop while literal count decreases\n");
(void)
fprintf(miserr,
"\t-t loop white the arrival time decreases\n");
(void)
fprintf(miserr,
