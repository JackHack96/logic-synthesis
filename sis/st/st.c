...
#define do_hash(key, table)                                                    \
  ((table->hash == st_ptrhash)                                                 \
       ? ST_PTRHASH((key), (table)->num_bins)                                  \
       : (table->hash == st_numhash)                                           \
             ? ST_NUMHASH((key), (table)->num_bins)                            \
...
st_table *st_init_table(compare, hash) int (*compare)();
int (*hash)();
{
...
void st_free_table(table) st_table *table;
{
...
st_lookup(table, key, value) st_table *table;
register char *key;
char **value;
{
  int hash_val;
...
st_lookup_int(table, key, value) st_table *table;
register char *key;
int *value;
{
  int hash_val;
...
st_add_direct(table, key, value) st_table *table;
char *key;
char *value;
{
  int hash_val;
...
st_find_or_add(table, key, slot) st_table *table;
char *key;
char ***slot;
{
  int hash_val;
...
st_find(table, key, slot) st_table *table;
char *key;
char ***slot;
{
  int hash_val;
...
static rehash(table) register st_table *table;
{
...
st_table *st_copy(old_table) st_table *old_table;
{
  st_table *new_table;
...
st_delete(table, keyp, value) register st_table *table;
register char **keyp;
char **value;
{
  int hash_val;
...
st_delete_int(table, keyp, value) register st_table *table;
register int *keyp;
char **value;
{
  int hash_val;
...
int st_foreach(table, func, arg) st_table *table;
enum st_retval (*func)();
char *arg;
{
  st_table_entry *ptr, **last;
...
st_strhash(string, modulus) register char *string;
int modulus;
{
  register int val = 0;
  register int c;
...
st_numhash(x, size) char *x;
int size;
{ return ST_NUMHASH(x, size); }

st_ptrhash(x, size) char *x;
int size;
{ return ST_PTRHASH(x, size); }

st_numcmp(x, y) char *x;
char *y;
{ return ST_NUMCMP(x, y); }

st_ptrcmp(x, y) char *x;
char *y;
{ return ST_NUMCMP(x, y); }

st_generator *st_init_gen(table) st_table *table;
{
  st_generator *gen;
...
int st_gen(gen, key_p, value_p) st_generator *gen;
char **key_p;
char **value_p;
{
  register int i;
...
int st_gen_int(gen, key_p, value_p) st_generator *gen;
char **key_p;
int *value_p;
{
  register int i;
...
void st_free_gen(gen) st_generator *gen;
{ FREE(gen); }
