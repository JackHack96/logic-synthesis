...
st_table *st_init_table_with_params(int (*compare)(), int (*hash)(), int size,
                                    int density, double grow_factor,
                                    int reorder_flag) {
    int      i;
    st_table *new;
...
static rehash(register st_table *table) {
...
int st_foreach(st_table *table, enum st_retval (*func)(), char *arg) {
    ...
    for (i = 0; i < table->num_bins; i++) {
        ...
        while (ptr != NIL(st_table_entry)) {
            ...
            switch (retval) {
                case ST_CONTINUE:last = &ptr->next;
                    ptr = *last;
                    break;
                case ST_STOP:return 0;
                case ST_DELETE:*last = ptr->next;
...
int st_numcmp(char *x, char *y) { return ST_NUMCMP(x, y); }

int st_ptrcmp(char *x, char *y) { return ST_NUMCMP(x, y); }
...
void st_free_gen(st_generator *gen) { FREE(gen); }
