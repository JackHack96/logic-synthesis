...
void g_free(g, f_free_g, f_free_v, f_free_e) graph_t *g;
void (*f_free_g)();
void (*f_free_v)();
void (*f_free_e)();
{
...
graph_t *g_dup(g, f_copy_g, f_copy_v, f_copy_e) graph_t *g;
gGeneric (*f_copy_g)();
gGeneric (*f_copy_v)();
gGeneric (*f_copy_e)();
{
...
array_t *g_graph_sort(g, cmp) graph_t *g;
int (*cmp)();
{
  int i;
  lsGen gen;
  ...
  foreach_vertex(g, gen, v) { array_insert(vertex_t *, v_array, i++, v); }
  array_sort(v_array, cmp);
  return (v_array);
}

lsList g_get_edges(g) graph_t *g;
{
  if (g == NIL(graph_t)) {
...
lsList g_get_in_edges(v) vertex_t *v;
{
  if (v == NIL(vertex_t)) {
...
lsList g_get_out_edges(v) vertex_t *v;
{
  if (v == NIL(vertex_t)) {
...
edge_t *g_add_edge(v1, v2) vertex_t *v1, *v2;
{
  edge_t_int *edge;
  lsHandle handle;
  graph_t *g;
...
static void g_del_from_list(list, item) lsList list;
lsGeneric item;
{
  lsGen gen;
...
void g_delete_edge(e, f_free_e) edge_t *e;
void (*f_free_e)();
{
  lsGeneric junk;
...
graph_t *g_edge_graph(e) edge_t *e;
{
  if (e == NIL(edge_t)) {
...
vertex_t *g_e_source(e) edge_t *e;
{
  if (e == NIL(edge_t)) {
...
lsList g_get_vertices(g) graph_t *g;
{
  if (g == NIL(graph_t)) {
...
vertex_t *g_add_vertex(g) graph_t *g;
{
  lsHandle handle;
  vertex_t_int *vert;
...
void g_delete_vertex(v, f_free_v, f_free_e) vertex_t *v;
void (*f_free_v)();
void (*f_free_e)();
{
  edge_t *e;
...
graph_t *g_vertex_graph(v) vertex_t *v;
{
  if (v == NIL(vertex_t)) {
