...
static int check_rect_value(sm_matrix *A, array_t *row_cost, array_t *col_cost, rect_t *rect) {
...
static sm_row *find_max_row_intersection(sm_matrix *A, array_t *row_cost, rect_t *rect, int *col_values) {
...
rect_t *greedy_row(sm_matrix *A, array_t *row_cost, array_t *col_cost, sm_row *seed) {
    sm_element *p;
    sm_row     *save, *prow;
    ...
    sm_foreach_row_element(seed, p) {
        col_values[p->col_num] = VALUE(p) - array_fetch(int, col_cost, p->col_num);
    }
    ...
    while (prow != 0 && rect->cols->length > 1) {
        ...
        sm_foreach_row_element(prow, p) { col_values[p->col_num] += VALUE(p); }
    ...
    if (greedy_row_debug) {
        (void) printf("greedy_row: best rectangle is %d by %d value=%d\n",
                      best_rect->rows->length, best_rect->cols->length,
                      best_rect->value);
    }
