...
/* ARGSUSED */
static int best_subkernel_sop(co_rect, rect, state_p) sm_matrix *co_rect;
...
char *state_p;
{
  ...
  sm_foreach_col_element(rect->rows, p) {
    cks += array_fetch(int, state->row_cost, p->row_num) - 1;
  }
  ...
  sm_foreach_row_element(rect->cols, p) {
    ks += array_fetch(int, state->col_cost, p->col_num);
  }
...
/* ARGSUSED */
static int best_subkernel_factor(co_rect, rect, state_p) sm_matrix *co_rect;
register rect_t *rect;
char *state_p;
{
  register sm_element *p, *p1, *p2;
  ...
  sm_foreach_col_element(rect->rows, p1) {
    sm_foreach_row_element(rect->cols, p2) {
      ...
      (void)st_insert(fanout, key, NIL(char));
    }
  }
...
static rect_t *best_subkernel(kernel_cube_matrix, row_cost, col_cost,
                              func) sm_matrix *kernel_cube_matrix;
...
rect_t *choose_subkernel(A, row_cost, col_cost, option) sm_matrix *A;
array_t *row_cost, *col_cost;
int option;
{
  rect_t *rect = 0;
  ...
  case 0:
    ...
    break;
  case 1:
    rect = best_subkernel(A, row_cost, col_cost, best_subkernel_sop);
    break;
  case 2:
    rect = best_subkernel(A, row_cost, col_cost, best_subkernel_factor);
    break;
  default:
    fail("bad option in choose_subkernel");
    break;
  }
  return rect;
}
