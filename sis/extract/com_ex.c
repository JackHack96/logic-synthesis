...
static int com_cube_extract(network_t **network, int argc, char **argv) {
    lsGen     gen;
    ...
    while ((c = util_getopt(argc, argv, "bcdft:v:")) != EOF) {
        switch (c) {
            case 'b':use_best_subcube = 1;
                break;
            case 'c':use_complement = 1;
                break;
            case 'd':cube_extract_debug = 1;
                break;
            case 'f':use_best_subcube = 2;
                break;
            case 't':thres = atoi(util_optarg);
                break;
            case 'v':cube_extract_debug = atoi(util_optarg);
                break;
            default:goto usage;
        }
    }
    ...
    foreach_node(*network, gen, node) {
        switch (node_function(node)) {
            ...
            case NODE_AND:
            case NODE_OR:
            case NODE_COMPLEX:ex_node_to_sm(node, cube_literal_matrix);
                break;
            default:;
        }
    }
    ...
    usage:
    (void) fprintf(siserr, "usage: gcx [-bcdf] [-t thresh]\n");
...
static int com_kernel_extract(network_t **network, int argc, char **argv) {
    sm_matrix *func;
    node_t    *node;
    ...
    while ((c = util_getopt(argc, argv, "1abcdfot:v:")) != EOF) {
        switch (c) {
            case '1':one_pass = 1;
                break;
            case 'a':use_all_kernels = 1;
                break;
            case 'b':use_best_subkernel = 1;
                break;
            case 'c':use_complement = 1;
                break;
            case 'd':kernel_extract_debug = 1;
                break;
            case 'f':use_best_subkernel = 2;
                break;
            case 'o':use_overlap = 1;
                need_duplicate   = 1;
                break;
            case 't':thres = atoi(util_optarg);
                break;
            case 'v':kernel_extract_debug = atoi(util_optarg);
                break;
            default:goto usage;
        }
    }
    ...
    do {
        ...
        foreach_node(*network, gen, node) {
            switch (node_function(node)) {
                ...
                case NODE_AND:
                case NODE_OR:
                case NODE_COMPLEX:func = sm_alloc();
    ...
    usage:
    (void) fprintf(siserr, "usage: gkx [-1abcdfo] [-t thresh]\n");
...
/* ARGSUSED */
static int print_kernel(node_t *kernel, node_t *co_kernel, char *state) {
    (void) fprintf(sisout, "(");
...
static int com_print_kernel(network_t **network, int argc, char **argv) {
    node_t  *node;
    array_t *node_vec;
...
static int com_gdiv(network_t **network, int argc, char **argv) {
    node_t  *node, *divisor;
    array_t *node_vec;
    int     i;
...
end_extract() { sm_cleanup(); }

static int com_fast_extract(network_t **network, int argc, char **argv) {
    sm_matrix *cube_literal_matrix;
    ...
    while ((c = util_getopt(argc, argv, "b:f:s:lo:z")) != EOF) {
        switch (c) {
            case 'o':ONE_PASS = 1;
                break;
            case 'l':PRESERVE_LEVEL = 1;
                break;
            case 'b':ONE_PASS = FX_DELETE = 1;
    ...
    do {
        ...
        foreach_node(*network, gen, node) {
            switch (node_function(node)) {
                ...
                case NODE_AND:
                case NODE_OR:
                case NODE_COMPLEX:(void) array_insert_last(node_t *, node_set, node);
    ...
    (void) fprintf(siserr, "     -b\t\t upper bound for the number of divisors, "
                           "default is 50000 \n");
    (void) fprintf(siserr, "     -l\t\t preserves the level of each node, used "
                           "for timing optimization\n");
    (void) fprintf(siserr, "     -z\t\t extract zero-weight factors also\n");
    return 1;
}

static st_table *find_required_time(network_t *network, int max_level) {
...
/* After extraction remove all the new nodes which cause the level some output
 * go beyond the max_level orginally computed for the network. We have already
 * assigned an allowed level to each node in the network. The new level can not
 * be greater than the allowed level.
 */
static void remove_unwanted_nodes(network_t *network, st_table *level_table, st_table *required_table) {
    array_t *nodevec;
    node_t  *np, *fanin;
    int     tmp;
    char    *dummy;
