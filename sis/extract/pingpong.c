...
static int row_value(sm_row *prow, array_t *row_cost) {
    register sm_element *p;
    register int        value;

    value = -array_fetch(int, row_cost, prow->row_num);
    sm_foreach_row_element(prow, p) { value = value + VALUE(p); }
    return value;
}

static sm_row *choose_row_seed(sm_matrix *A, array_t *row_cost) {
...
static sm_row *choose_next_row_seed(sm_matrix *A, array_t *row_cost, rect_t *rect) {
    register int        value, max_value;
    register sm_element *p;
    sm_row              *prow, *best_row;
...
static int col_value(sm_col *pcol, array_t *col_cost) {
    register sm_element *p;
    register int        value;

    value = -array_fetch(int, col_cost, pcol->col_num);
    sm_foreach_col_element(pcol, p) { value = value + VALUE(p); }
    return value;
}

static sm_col *choose_col_seed(sm_matrix *A, array_t *col_cost) {
    register int value, max_value;
...
static sm_col *choose_next_col_seed(sm_matrix *A, array_t *col_cost, rect_t *rect) {
    register int        value, max_value;
    register sm_element *p;
    sm_col              *pcol, *best_col;
...
static int update_best(rect_t *rect1, rect_t *rect2, rect_t **best_rect) {
    int got_new_best;
...
static rect_t *ping_pong_improve(sm_matrix *A, array_t *row_cost, array_t *col_cost, rect_t *best_rect) {
    rect_t *rect1, *rect2;
    sm_row *best_row;
    sm_col *best_col;
    int    got_new_best;
...
static rect_t *ping_pong_onepass(sm_matrix *A, array_t *row_cost, array_t *col_cost) {
    sm_row *row_seed;
    sm_col *col_seed;
    rect_t *rect1, *rect2;

    row_seed = choose_row_seed(A, row_cost);
    if (row_seed == NIL(sm_row))
        return rect_alloc();
...
rect_t *ping_pong(sm_matrix *A, array_t *row_cost, array_t *col_cost) {
    rect_t *best_rect;
