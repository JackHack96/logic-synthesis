 ...
 *
 * $Source: /users/pchong/CVS/sis/sis/extract/fast_extract.c,v $
 * $Author: pchong $
 * $Revision: 1.1.1.1 $
...
/* Divide each fanout */
static int
fx_divide_each_fanout(fanout, newnode, complement)
array_t *fanout;
node_t *newnode;
int complement;
{
 ...
 */
static ddivisor_t *
choose_best_divisor(B,ddivisor_set, wdmax)
sm_matrix *B;
lsList ddivisor_set;
int *wdmax;
{
 ...
 */
int
fx_compute_divisor_weight(func, divisor)
sm_matrix *func;
ddivisor_t *divisor;
{
    int weight, w;
 ...
 */
static void
sd_reexpress(B,rect,dd112_on,index,newcolumn)
sm_matrix *B;
rect_t *rect;
int dd112_on;
int index;
 ...
 */
static array_t *
dd_reexpress(B,divisor,node_index)
sm_matrix *B;
ddivisor_t *divisor;
int *node_index;
{
    ...
    if (DTYPE(divisor) == D112) *node_index = new_index;
 ...
 */
static void
dd_modify(B,div,node_index, newcubes,ddivisor_set)
sm_matrix *B;
ddivisor_t *div;
int node_index;
array_t *newcubes;
            ...
            for (j = i+1; j < array_n(newcubes); j++) {
                ...
                if (sm_cube1->sis_id == sm_cube2->sis_id) {
                    ...
                    if (check_set) {
                        ...
                        if (found == 0) (void) ddivisor_cell_free(dd_cell);
                        (void) ddivisor_free(divisor);
                    } else {
                        lsRemoveItem(divisor->sthandle,&data);
                        (void) ddivisor_free(divisor);
 ...
 */
static void
sd_mod_old_divisor(divisor,ddivisor_set)
ddivisor_t *divisor;
ddset_t *ddivisor_set;
{
...
static void
sd_mod_new_divisor(divisor,ddivisor_set)
ddivisor_t *divisor;
ddset_t *ddivisor_set;
{
    ddivisor_cell_t *cell;
 ...
 */
static void
sd_mod_INBETWEEN(dd_cell, ddivisor_set, colnum)
ddivisor_cell_t *dd_cell;
ddset_t *ddivisor_set;
int colnum;
{
 ...
 */
static void
sd_mod_INDIVISOR(dd_cell, ddivisor_set)
ddivisor_cell_t *dd_cell;
ddset_t *ddivisor_set;
{
...
int
sd_check_affected(dd_cell, ddivisor_set, newcolumn)
ddivisor_cell_t *dd_cell;
ddset_t *ddivisor_set;
int newcolumn;
{
 ...
 */
static void
sd_modify(B,rect, ddivisor_set, newcolumn)
sm_matrix *B;
rect_t *rect;
ddset_t *ddivisor_set;
int newcolumn;
