...
void ex_setup_globals(network_t *network, int dup_fcts) {
    int    index;
    node_t *node, *node1;
    lsGen  gen;
...
void ex_setup_globals_single(node_t *node) {
    int    i;
    node_t *fanin;
...
void ex_free_globals(int dup_fcts) {
    int    i;
    node_t *node1;
...
char *ex_map_index_to_name(int i) { return node_name(nodeindex_nodeof(global_node_index, i)); }
...
void ex_node_to_sm(register node_t *node, register sm_matrix *M) {
    ...
    foreach_set(node->F, last, p) {
        ...
        foreach_fanin(node, i, fanin) {
            ...
            if (pelement != NIL(sm_element)) {
                ...
                value_cell->sis_index = nodeindex_indexof(global_node_index, node);
...
sm_matrix *ex_network_to_sm(network_t *network) {
    node_t    *node;
    sm_matrix *M;
    lsGen     gen;
...
node_t *ex_sm_to_node(sm_matrix *func) {
    int         i, nin;
...
static int ex_divide_each_fanout(array_t *fanout, array_t *cubes, node_t *newnode, int debug) {
...
int ex_divide_function_into_network(sm_matrix *func, array_t *fanout, array_t *cubes, int debug) {
    node_t *node;
    int    index, count;
