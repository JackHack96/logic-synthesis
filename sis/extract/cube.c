...
int
sparse_cube_extract(A, value_threshold, use_best_subcube)
        sm_matrix *A;
        int value_threshold;
        int use_best_subcube;
{
    ...
    row_cost = array_alloc(
    int, 0);
    sm_foreach_row(A, prow)
    {
        array_insert(
        int, row_cost, prow->row_num, 1);
    }

    /* cost for each column is 1 */
    col_cost = array_alloc(
    int, 0);
    sm_foreach_col(A, pcol)
    {
        array_insert(
        int, col_cost, pcol->col_num, 1);
    }
        ...
        rect_t *rect;
{
    ...
    new_index = ex_divide_function_into_network(func, fanout,
                                                NIL(array_t), cube_extract_debug);
    array_free(fanout);
    sm_free(func);
...
/* ARGSUSED */
static int
best_subcube_helper(co_rect, rect, state)
        sm_matrix *co_rect;
        rect_t *rect;
        char *state;
{
        ...
        sm_matrix *A;
{
    ...
    gen_all_rectangles(A, best_subcube_helper, NIL(
    char));
    return best_rect;
}
        ...
        char *state;
{
    ...
    sm_foreach_col_element(rect->rows, p1)
    {
        ...
        (void) st_insert(func_table, key, NIL(
        char));
    }
...
static rect_t *
best_subcube_factored(A)
        sm_matrix *A;
{
    cube_literal_matrix = A;
    best_rect           = rect_alloc();
    gen_all_rectangles(A, best_subcube_factored_helper, NIL(
    char));
    return best_rect;
}
