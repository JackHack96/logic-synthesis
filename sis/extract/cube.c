...
int sparse_cube_extract(sm_matrix *A, int value_threshold, int use_best_subcube) {
    sm_row  *prow;
    sm_col  *pcol;
    rect_t  *rect;
...
static void update_cube_literal_matrix(sm_matrix *A, array_t *row_cost, array_t *col_cost, rect_t *rect) {
    sm_element   *p, *p1, *p2;
    sm_matrix    *func;
    array_t      *fanout;
...
static int bound_rect_size_simple(sm_matrix *co_rect, rect_t *rect) {
    sm_row *prow;
    sm_col *pcol;
    int    bound;
...
/* ARGSUSED */
static int best_subcube_helper(sm_matrix *co_rect, rect_t *rect, char *state) {
    int bound;
...
static rect_t *best_subcube(sm_matrix *A) {
...
/* ARGSUSED */
static int best_subcube_factored_helper(sm_matrix *co_rect, rect_t *rect, char *state) {
    sm_element   *p1;
    sm_row       *prow;
...
static rect_t *best_subcube_factored(sm_matrix *A) {
...
static rect_t *choose_subcube(sm_matrix *A, array_t *row_cost, array_t *col_cost, int option) {
    rect_t *rect;
    ...
    switch (option) {
        ...
        case 1:rect = best_subcube(A);
            break;

        case 2:rect = best_subcube_factored(A);
            break;

        default: fail("bad option in choose_subcube");
            break;
    }
...
static void clear_rect(sm_matrix *A, rect_t *rect) {
