...
void heap_entry_free(entry) heap_entry_t *entry;
{ FREE(entry); }
...
void heap_free(heap, delFunc) register heap_t *heap;
void (*delFunc)();
{
  int i;
...
heap_entry_t *findmax_heap(heap) register heap_t *heap;
{
  if (heap->heapnum == 0) {
...
void swap_entry(s1, s2) heap_entry_t *s1, *s2;
{
  heap_entry_t temp;
...
void resize_heap(heap) register heap_t *heap;
{
  heap->heapsize *= 2;
...
void insert_heap(heap, entry) register heap_t *heap;
register heap_entry_t *entry;
{
  int current, parent;
 ...
 * return NILL.
 */
heap_entry_t *deletemax_heap(heap) register heap_t *heap;
{
  int current = 1;
  int child = 2;
  heap_entry_t *entry;
