...
static ddset_t *fx_node_2c_kernels(sm_matrix *node_func, int index) {
    lsGen      gen;
    ddset_t    *ddset;
    ddivisor_t *divisor;
...
static node_t *fx_ddivisor_to_node(ddivisor_t *divisor) {
    sm_matrix *func;
    node_t    *node;
...
node_t *fx_2c_kernel_best(node_t *node, int (*function)(), char *state) {
    sm_matrix  *node_func;
    ddset_t    *ddset;
    ddivisor_t *divisor;
    ...
    switch (node_function(node)) {
        ...
        case NODE_1:
        case NODE_INV:
        case NODE_BUF:return NIL(node_t);
        case NODE_AND:
        case NODE_OR:
        case NODE_COMPLEX:node_func = sm_alloc();
