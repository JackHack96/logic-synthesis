...
static ddset_t *fx_node_2c_kernels(node_func, index) sm_matrix *node_func;
int index;
{
  lsGen gen;
  ddset_t *ddset;
...
static node_t *fx_ddivisor_to_node(divisor) ddivisor_t *divisor;
{
  sm_matrix *func;
  node_t *node;
...
node_t *fx_2c_kernel_best(node, function, state) node_t *node;
int (*function)();
char *state;
{
  sm_matrix *node_func;
