...
void
kernel_extract(A, sis_index, use_all_kernels)
        sm_matrix *A;
        int sis_index;
        int use_all_kernels;
{
    ...
    if (A->nrows == 0) return;        /* ignore empty functions */
    ...
    sm_foreach_row(A, prow)
    {
        value_cell_free(function_cubes[prow->row_num]);
    }
    FREE(function_cubes);
}
...
int
sparse_kernel_extract(value_threshold, use_best_subkernel)
        int value_threshold, use_best_subkernel;
{
    array_t    *fanout;
    sm_row     *kernel_cube;
    sm_matrix  *func;
    ...
    if (kernel_extract_debug) {
        if (kernel_extract_debug == 99) dump_kernel_table();
        ...
        (void) fprintf(sisout,
                       "kernel-cube matrix is %d by %d (%d nonzero, %4.3f%% sparse)\n",
                       kernel_cube_matrix->nrows, kernel_cube_matrix->ncols,
                       nonzero, sparsity);
    }
    ...
    do {
        rect = choose_subkernel(kernel_cube_matrix,
                                global_row_cost, global_col_cost, use_best_subkernel);
        ...
        (void) ex_divide_function_into_network(func, fanout,
                                               NIL(array_t), kernel_extract_debug);
#else
        ...
        if (kernel_extract_debug) {
            (void) fprintf(sisout,
                           "subkernel rectangle is %d by %d with value %d\n",
        ...
        rect_t *rect;
{
    ...
    sm_foreach_col_element(rect->rows, p1)
    {
        sm_foreach_row_element(rect->cols, p2)
        {
            ...
            (void) st_insert(value_table, p->user_word, NIL(
            char));
...
int
overlapping_kernel_extract(value_threshold, use_best_subkernel)
        int value_threshold, use_best_subkernel;
{
