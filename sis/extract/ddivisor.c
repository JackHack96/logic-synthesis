...
void ddivisor_free(ddivisor) register ddivisor_t *ddivisor;
{
...
void ddivisor_cell_free(ddivisor_cell) ddivisor_cell_t *ddivisor_cell;
{ FREE(ddivisor_cell); }
...
static void table_cleanup(table) sm_matrix *table;
{
  register sm_row *row;
  register sm_element *p;

  sm_foreach_row(table, row){
      sm_foreach_row_element(row, p){lsDestroy((lsList)p->user_word, 0);
}
}
(void)sm_free(table);
}
...
static void ddset_cleanup(ddivisor_set) ddset_t *ddivisor_set;
{
  register sm_element *p;
  register sm_row *row;
  ...
  sm_foreach_row(ddivisor_set->Dother_set, row){sm_foreach_row_element(row, p){
...
void ddset_free(ddivisor_set) ddset_t *ddivisor_set;
{
  lsGen gen1, gen2;
  ddivisor_t *divisor;
...
void sm_cube_free(sm_cube) register sm_cube_t *sm_cube;
{
...
/* Do operation {c1} \ {c2} */
void clear_row_element(c1, c2) register sm_row *c1, *c2;
{
  register sm_element *p;

  sm_foreach_row_element(c2, p) { (void)sm_row_remove(c1, p->col_num); }
}

/* Generate double-cube divisor and store info. in corresponding dd_cell. */

ddivisor_t *gen_ddivisor(cube1, cube2, dd_cell) sm_row *cube1, *cube2;
ddivisor_cell_t *dd_cell;
{
  sm_row *base;
...
static int variable_count(divisor) ddivisor_t *divisor;
{
  int count = 4;
  int i, j, t1[2], t2[2];
...
int decide_dtype(div) register ddivisor_t *div;
{
  sm_row *temp;
...
static int check_Dother(div1, div2) ddivisor_t *div1, *div2;
{
...
static int check_D222(div1, div2, phase) ddivisor_t *div1, *div2;
int *phase;
{
...
static int check_D223(div1, div2, phase) ddivisor_t *div1, *div2;
int *phase;
{
 ...
 */
int check_exist(div1, div2, phase) ddivisor_t *div1, *div2;
int *phase;
{
...
static lsList lookup_ddset(divisor, table, key1, key2) ddivisor_t *divisor;
sm_matrix *table;
int key1, key2;
{
...
static lsList lookup_ddset_other(divisor, table) ddivisor_t *divisor;
sm_matrix *table;
{
  ...
  if ((p1 = sm_find(table, key1, key2)) == NIL(sm_element)) {
    ...
    p3 =
        sm_insert((sm_matrix *)p2->user_word, divisor->cube1->last_col->col_num,
...
void hash_ddset_table(divisor, check_set) ddivisor_t *divisor;
lsList check_set;
{
  lsHandle handle;
...
lsList choose_check_set(divisor, ddivisor_set) register ddivisor_t *divisor;
...
ddivisor_t *check_append(div, ddivisor_set, dd_cell) register ddivisor_t *div;
ddset_t *ddivisor_set;
ddivisor_cell_t *dd_cell;
{
  int phase = 0;
 ...
 */
int extract_ddivisor(B, index, row, ddivisor_set) sm_matrix *B;
int index;
int row;
ddset_t *ddivisor_set;
{
