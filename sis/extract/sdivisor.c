...
static void sdivisor_free(sdivisor) char *sdivisor;
{
...
void sdset_free(sdset) sdset_t *sdset;
{
  void sdivisor_free();
...
static void col_cell_free(col_cell) col_cell_t *col_cell;
{ FREE(col_cell); }

/* compare function which will sort array elements in increasing order */

static int compare(obj1, obj2) lsGeneric obj1, obj2;
{
...
static void find_unconsidered_cols(B, sdset) sm_matrix *B;
sdset_t *sdset;
{
...
static array_t *cols_extract(columns, k) lsList columns;
int k;
{
  int flag = 1;
  array_t *set;
 ...
 * the sdset->heap.
 */
static void coin1_compute(B, set, sdset) sm_matrix *B;
array_t *set;
sdset_t *sdset;
{
  int i, j;
 ...
 */
static void coin2_compute(B, set, sdset) sm_matrix *B;
array_t *set;
sdset_t *sdset;
{
  int i;
  ...
  if ((array_n(set) == 0) || (lsLength(sdset->col_set) == 0))
    return;
...
static void sdset_append(sdset, T) sdset_t *sdset;
array_t *T;
{
  col_cell_t *col_cell;
  lsHandle handle;
 ...
 */
static sdivisor_t *find_sdivisor(B, sdset, wdmax) sm_matrix *B;
sdset_t *sdset;
int wdmax;
{
  lsHandle handle;
...
sdivisor_t *extract_sdivisor(B, sdset, wdmax) sm_matrix *B;
sdset_t *sdset;
int wdmax;
{
  if (B->nrows == 0)
