...
static void sdivisor_free(char *sdivisor) {
...
static void col_cell_free(col_cell_t *col_cell) { FREE(col_cell); }

/* compare function which will sort array elements in increasing order */

static int compare(lsGeneric obj1, lsGeneric obj2) {
...
static void find_unconsidered_cols(sm_matrix *B, sdset_t *sdset) {
...
static array_t *cols_extract(lsList columns, int k) {
    int        flag = 1;
    array_t    *set;
    col_cell_t *col_cell;
    lsHandle   handle;
 ...
 * the sdset->heap.
 */
static void coin1_compute(sm_matrix *B, array_t *set, sdset_t *sdset) {
    int          i, j;
    sdivisor_t   *sd;
 ...
 */
static void coin2_compute(sm_matrix *B, array_t *set, sdset_t *sdset) {
    int          i;
    sdivisor_t   *sd;
    sm_col       *col, *col1, *col2;
    col_cell_t   *col_cell1, *col_cell2;
...
static void sdset_append(sdset_t *sdset, array_t *T) {
    col_cell_t *col_cell;
    lsHandle   handle;
    int        i;
 ...
 */
static sdivisor_t *find_sdivisor(sm_matrix *B, sdset_t *sdset, int wdmax) {
    lsHandle     handle;
    sm_col       *col;
