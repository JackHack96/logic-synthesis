...
static int check_rect_value(sm_matrix *A, array_t *col_cost, array_t *row_cost, rect_t *rect) {
...
static sm_col *find_max_col_intersection(sm_matrix *A, array_t *col_cost, rect_t *rect, int *row_values) {
    ...
    sm_foreach_col_element(seed, p) {
        ...
        sm_foreach_row_element(prow, p1) {
            col_values[p1->col_num] = -array_fetch(int, col_cost, p1->col_num);
        }
    }
...
rect_t *greedy_col(sm_matrix *A, array_t *col_cost, array_t *row_cost, sm_col *seed) {
    sm_element *p;
    sm_col     *save, *pcol;
    ...
    sm_foreach_col_element(seed, p) {
        row_values[p->row_num] = VALUE(p) - array_fetch(int, row_cost, p->row_num);
    }

    /* Setup the initial rectangle value */
    total_col_cost = array_fetch(int, col_cost, seed->col_num);
    ...
    while (pcol != 0 && rect->rows->length > 1) {
        ...
        total_col_cost += array_fetch(int, col_cost, pcol->col_num);
        ...
        sm_foreach_col_element(pcol, p) { row_values[p->row_num] += VALUE(p); }
    ...
    if (greedy_col_debug) {
        (void) printf("greedy_col: best rectangle is %d by %d value=%d\n",
                      best_rect->cols->length, best_rect->rows->length,
                      best_rect->value);
    }
