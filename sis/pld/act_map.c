...
network_t *act_map_network(network, init_param, cost_table)
...
static iterative_improvement(network, cost_table,
...
COST_STRUCT *act_evaluate_map_cost(node, init_param, network, delay_values,
                                   cost_table) node_t *node;
act_init_param_t *init_param;
network_t *network;
...
COST_STRUCT *make_tree_and_map(node, act_of_node) node_t *node;
...
ACT_VERTEX_PTR
my_create_act(node, mode, cost_table, network, delay_values) node_t *node;
float mode;
st_table *cost_table;
network_t *network;
array_t *delay_values;
...
static ACT_VERTEX_PTR my_create_act_general(node) node_t *node;
{
  array_t *nodevec;
  ACT_VERTEX_PTR act;
...
static int all_fanins_positive(node) node_t *node;
{
...
void act_init_multiple_fo_array(act_of_node) ACT_VERTEX_PTR act_of_node;
{
...
****************************************************************************/
void put_node_names_in_act(vertex, list) ACT_VERTEX_PTR vertex;
array_t *list;
{
  node_t *node;
...
/***********************************************************************
   ...
   to the cost
***********************************************************************/
int map_act(vertex) ACT_VERTEX_PTR vertex;
...
/**************************************************************************
   ...
   return 0
***************************************************************************/
static int OR_pattern(root) ACT_VERTEX_PTR root;
{
...
/************************************************************************
 ...
 at the present node.
*************************************************************************/
static int minimum_cost_index(cost, num_entries) int cost[];
int num_entries;
{
...
static improve_network(network, cost_table, init_param) network_t *network;
st_table *cost_table;
act_init_param_t *init_param;
...
*********************************************************************************/
static char *formulate_IP(nodevec, M, g, num_ones, clust_num,
                          iteration_num) array_t *nodevec;
int **M;
int g[];
int num_ones[];
int clust_num;
...
****************************************************************************/
static void read_LINDO_file(filename, variable, iteration_gain) char *filename;
int *variable;
int *iteration_gain;
...
return.
*************************************************************************************/
static void partial_scan_lindoFile(lindoFile) FILE *lindoFile;
{
  char word[BUFSIZE];
...
*************************************************************************/
int is_anyfo_PO(node) node_t *node;
{
  lsGen genfo;
  node_t *fanout;
  node_function_t node_fun;
...
int print_network(network, cost_table, iteration_num) network_t *network;
st_table *cost_table;
int iteration_num;
{
  lsGen gen;
...
static decomp_big_nodes(network, cost_table, gain,
                        init_param) network_t *network;
st_table *cost_table;
int *gain;
act_init_param_t *init_param;
...
int cost_of_node(node, cost_table) node_t *node;
st_table *cost_table;
{
  char *dummy;
...
array_t *OR_literal_order(node) node_t *node;
...
array_t *single_cube_order(node) node_t *node;
...
******************************************************************************************/
static act_quick_phase(network, cost_table, init_param) network_t *network;
st_table *cost_table;
act_init_param_t *init_param;
{
...
void free_cost_struct(cost_node) COST_STRUCT *cost_node;
{
  if (cost_node->cost != 0) {
    if (ACT_DEBUG)
...
/*ARGSUSED*/
void free_cost_table(table) st_table *table;
{
  char *arg;
...
/*ARGSUSED */
static enum st_retval free_table_entry(key, value, arg) char *key, *value, *arg;
{
...
/*ARGSUSED*/
void free_cost_table_without_freeing_key(table) st_table *table;
{
  char *arg;
...
/*ARGSUSED */
enum st_retval free_table_entry_without_freeing_key(key, value, arg) char *key,
    *value, *arg;
{
  (void)free_cost_struct((COST_STRUCT *)value);
  return ST_DELETE;
...
*************************************************************************************/
node_t *act_mux_node(control, left, right) node_t *control, *left, *right;
{
...
/********************************************************************************
  ...
  Actel paper.
*********************************************************************************/
static node_t *basic_block_node(A0, A1, SA, B0, B1, SB, S0, S1,
                                cost_table) node_t *A0,
    *A1, *SA, *B0, *B1, *SB, *S0, *S1;
st_table *cost_table;
{
...
static void print_output_node_with_string(node, name) node_t *node;
char *name;

{
  ...
  switch (node_fun) {
  case NODE_0:
    (void)fprintf(BDNET_FILE, "GND");
    break;
  case NODE_1:
    (void)fprintf(BDNET_FILE, "Vdd");
    break;
  default:
    node_print_rhs(BDNET_FILE, node);
  }
  (void)fprintf(BDNET_FILE, "\";\n");
}

static void print_node_with_string(node, name, cost_table) node_t *node;
char *name;
st_table *cost_table;
...
*********************************************************************************/
network_t *act_break_network(network, table) network_t *network;
st_table *table;
{
  array_t *nodevec;
  int size;
...
----------------------------------------------------------------------------*/
act_break_node(node, network, table) node_t *node;
network_t *network;
st_table *table;
{
...
void set_mark_act(PRESENT_ACT) ACT_VERTEX_PTR PRESENT_ACT;
{
...
***************************************************************************************/
static node_t *act_get_function(network, node, vertex,
                                table) network_t *network;
node_t *node;
...
---------------------------------------------------------------------*/
node_t *get_node_literal_of_vertex(vertex, network) ACT_VERTEX_PTR vertex;
network_t *network;
{
  node_t *node;
...
************************************************************************************/
void free_node_if_possible(node, vertex) node_t *node;
ACT_VERTEX_PTR vertex;
{
...
void act_act_free(node) node_t *node;
{
  void local_actFree();
...
-------------------------------------------------------------*/
static void
    free_nodes_in_act_and_change_multiple_fo(vertex) ACT_VERTEX_PTR vertex;
{
  if (vertex->mark == MARK_COMPLEMENT_VALUE)
    return;
  vertex->mark = MARK_COMPLEMENT_VALUE;
...
static void init_bdnet(network, table) network_t *network;
st_table *table;
...
------------------------------------------------------------------*/
static char *act_get_node_fn_type(node, cost_table) node_t *node;
st_table *cost_table;
{
  node_t *fanin;
  char *dummy;
...
network_t *act_network_remap(network, cost_table) network_t *network;
...
**********************************************************************************************/
int act_node_remap(network, node, cost_table, FLAG_DECOMP,
                   HEURISTIC_NUM_ORIG) network_t *network;
node_t *node;
st_table *cost_table; /* original */
...
COST_STRUCT *make_tree_and_map_delay(node, act_of_node, network, delay_values,
                                     cost_table, mode) node_t *node;
ACT_VERTEX_PTR act_of_node;
network_t *network;
array_t *delay_values;
st_table *cost_table;
...
--------------------------------------------------------------------*/
static int map_act_delay(vertex, network, delay_values, cost_table,
                         mode) ACT_VERTEX_PTR vertex;
network_t *network;
array_t *delay_values;
st_table *cost_table;
float mode;
...
static int minimum_costdelay_index(cost, delay, num_entries, mode) int cost[];
double delay[];
int num_entries;
float mode;
{
...
/*-------------------------------------------------------------------
  ...
  that corresponds to it. Gets the node by name.
---------------------------------------------------------------------*/
static node_t *get_node_of_vertex(vertex, network) ACT_VERTEX_PTR vertex;
network_t *network;
{
  node_t *node;
...
array_t *act_get_corr_fanin_nodes(node, order_list1) node_t *node;
array_t *order_list1;
{
  network_t *network;
  int i;
