        ...
        st_table *cost_table;
{
    ...
    if ((node_num_literal(node) == node_num_cube(node)) ||
        ...
        if (init_param->mode == AREA) cost_node = make_tree_and_map(node, act_of_node);
            ...
            cost_node = make_tree_and_map_delay(node, act_of_node, network, delay_values,
    ...
    switch (init_param->HEURISTIC_NUM) {
        ...
        case 4:make_act(node);
            ...
            if (init_param->mode == AREA) cost_node2 = make_tree_and_map(node, act2_of_node);
                ...
                cost_node2 = make_tree_and_map_delay(node, act2_of_node, network, delay_values,
                                                     cost_table, init_param->mode);
            act_act_free(node);
            ...
            if (init_param->mode == AREA) cost_node1 = make_tree_and_map(node, act1_of_node);
                ...
                cost_node1 = make_tree_and_map_delay(node, act1_of_node, network, delay_values,
                                                     cost_table, init_param->mode);
            act_act_free(node);
            if (init_param->mode == AREA) {
...
decomp_big_nodes(network, cost_table,
&decomp_gain, init_param);
...
(void)
print_network(network, cost_table, iteration_num
);
...
if (iteration_gain >= (init_param->
GAIN_FACTOR *cost_network
)) {
nogain = FALSE;
}
continue;
...
node = array_fetch(node_t * , nodevec, i);
(void)
st_insert(node2num_table, node_long_name(node),
(char *)i);
}
...
if (
node_num_fanin(node)
> init_param->FANIN_COLLAPSE) continue;
if (
is_anyfo_PO(node)
) {
continue;
...
assert (st_lookup(cost_table, name1,
...
/*-------------------------------------------------------------------------------*/
assert (st_lookup(cost_table, node_long_name(fanout),
...
assert (st_lookup(node2num_table, node_long_name(fanout),
...
j++) {
(void)
free_cost_struct( focost[clust_num_add1][j]);
...
(void)
sprintf (outfile,
"/usr/tmp/actel.lindo.out%d.XXXXXX", iteration_num);
sfd = mkstemp(outfile);
if (sfd != -1) {
close(sfd);
(void)
sprintf(buf,
"lindo < %s > %s", infile, outfile);
...
/*-------------------------------------------*/
read_LINDO_file(outfile, variable,
&collapse_gain);
}
...
assert(!
st_insert(fo_table, node_long_name(fanout),
(char *) num_fo));
}
foreach_fanout(node, genfo, fanout
) {
...
assert (st_delete(cost_table,

&name1, &dummy));
(void)
free_cost_struct((COST_STRUCT
*) dummy);
FREE(name1);
...
/*-------------------------------------------------------*/
assert (st_lookup_int(fo_table, node_long_name(fanout),

&num_fo));
focost_ind[j][num_fo] = 0;
assert(!
st_insert(cost_table, name,
(char *) focost[j][num_fo]));
} /* foreach_fanout */
...
/*----------------------------------------*/
(*gain) +=
act_node_remap(network, node, cost_table, FLAG_DECOMP,
              init_param
->HEURISTIC_NUM);
...
/* checking */
/*----------*/
assert (st_lookup(cost_table, node_long_name(node),
...
cost_struc = act_evaluate_map_cost(node, init_param, NIL(network_t),
                                   NIL(array_t), NIL(st_table));
array_insert_last(COST_STRUCT
*, cost_array, cost_struc);
...
assert (st_lookup(table, node_long_name(node),
...
assert(!
st_insert(table, node_new_name,
(char *) cost_node));
/* free the storage associated with the act (bdd) */
/*------------------------------------------------------*/
set_mark_act(cost_node
->act);
free_nodes_in_act_and_change_multiple_fo(cost_node
->act);
...
j = 0;
j <
array_n(vertex_name_array);
j++) {
vertex_name_struct = array_fetch(VERTEX_NODE * , vertex_name_array, j);
vertex_name_struct->vertex->
name = node_long_name(vertex_name_struct->node);
    ...
    switch (vertex->pattern_num) {
	   ...
	   original vertex->low (i.e. v_low)    	     	 */
        ...
        case 7: vertex->low->mark  = MARK_COMPLEMENT_VALUE;
            ...
            return node2;
        default:(void) printf(" act_get_function(): unknown pattern_num %d found\n", vertex->pattern_num);
            exit(1);
    } /* switch */
    return NIL(node_t);
}
        ...
        st_table *cost_table;
{
    ...
    switch (node_fun) {
        ...
        default:
            if (node->type != INTERNAL) return NIL(
            char); /* may not be needed */
            /* change made on Aug. 18, 1991 when pointed out by Andre' */
            /*---------------------------------------------------------*/
            if (!st_lookup(cost_table, node_long_name(node), &dummy)) return NIL(
            char);
            cost_node = (COST_STRUCT *) dummy;
            if (cost_node->cost == 0) {
        ...
        int FLAG_DECOMP, HEURISTIC_NUM_ORIG;
{
    ...
    for (i     = 0; i < num_nodes2; i++) {
        ...
        node1_dup->name = NIL(
        char);  /* possibility of leak? */
        node1_dup->short_name = NIL(
        char);
   ...
   vertex->mapped = 1 also */
    ...
    if (vertex->high->value == 4) {
        if (vertex->high->multiple_fo != 0) {
            ...
            if (vertex->high->high->multiple_fo != 0) vhighhigh_cost = 0;
                ...
                vhighhigh_cost = map_act_delay(vertex->high->high, network, delay_values,
                                               cost_table, mode);
    ...
    if (vertex->low->value == 4) {
        ...
        max_input_arrival     = my_max(
                my_max(act_get_arrival_time(node, cost_table), act_get_arrival_time(nlow, cost_table)),
    ...
    if (vertex->high->value == 4) {
        ...
        max_input_arrival     = my_max(
                my_max(act_get_arrival_time(node, cost_table), act_get_arrival_time(nhigh, cost_table)),
                max_input_int_arrival);
    ...
    if ((OR_pattern(vertex))
        ...
        max_input_int_arrival = my_max(
                my_max(vertex->low->low->low->arrival_time, vertex->low->low->high->arrival_time),
                vhigh_delay);
        max_input_arrival     = my_max(
                my_max(act_get_arrival_time(node, cost_table), act_get_arrival_time(nlow, cost_table)),
