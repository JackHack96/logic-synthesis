 ...
 *
 * $Source: /users/pchong/CVS/sis/sis/pld/act_bdd.c,v $
 * $Author: pchong $
 * $Revision: 1.1.1.1 $
...
act_t *my_act_and(p, vertex, name) sm_element *p;
act_t *vertex;
char *name;
{
  int is_first = 0;
  ...
  if (is_first) {
    ...
    case 0:
      ...
      break;
    case 1:
      p_vertex->high = h_vertex;
      p_vertex->low = l_vertex;
      break;
    case 2:
      (void)fprintf(sisout, "error in my_and_act,\n");
      break;
    }
  } else {
    switch ((int)p->user_word) {
    case 0:
      ...
      break;
    case 1:
      p_vertex->high = vertex;
      p_vertex->low = l_vertex;
      p_vertex->index_size = vertex->index_size + 1;
      break;
    case 2:
      (void)fprintf(sisout, "error in my_and act, p->u_w == 2\n");
      break;
    }
  }
  return p_vertex;
...
/* Or 2 ACT's*/
act_t *my_or_act_F(array_b, cover, array) array_t *array_b;
array_t *array;
sm_row *cover;
{
  int i;
...
static void join(parent_vertex, vertex, down_vertex) act_t *vertex,
    *down_vertex, *parent_vertex;
{
...
act_t *my_and_act(act_l, act_r, b_col, name) act_t *act_l, *act_r;
int b_col;
char *name;
{
  act_t *act;
...
trace_act(vertex) act_t *vertex;
{
  if ((vertex->low != NIL(act_t)) && (vertex->high != NIL(act_t))) {
    (void)fprintf(sisout, "current variable - %s\n", vertex->name);
  }

  if (vertex->low != NIL(act_t)) {
    trace_act(vertex->low);
    if (vertex->high != NIL(act_t)) {
      trace_act(vertex->high);
    }
  }
