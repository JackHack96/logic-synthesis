...
/*-----------------------------------------------------------------------------------
  As obvious from the name, it does partial collapse on the network when it finds
  that lindo is not in the path. Assigns a score to each nodewhich is a measure of
        ...
        act_init_param_t *init_param;
{
    ...
    act_partial_collapse_assign_score_network(network, score_table, cost_table, init_param);
    ...
    while (1) {
        ...
        if (ACT_DEBUG) (void) printf("node from score_table = %s\n", node_long_name(node));
        collapsed = act_partial_collapse_node(network, node, cost_table, init_param,
                                              &gain, score_table);
        if (collapsed) {
            if (ACT_DEBUG) (void) printf("collapsed node %s\n", node_long_name(node));
            total_gain += gain;
        }
    }
}
...
int
act_partial_collapse_node(network, node, cost_table, init_param, pgain, score_table)
        network_t *network;
        node_t *node;
        st_table *cost_table;
        act_init_param_t *init_param;
        ...
        st_table *score_table;
{
    ...
    for (i       = 0; i < num_fanout; i++) {
        ...
        */
        ...
        cost_struct =
                act_evaluate_map_cost(dup_fanout, init_param, NIL(network_t),
    ...
    for (i = 0; i < num_fanout; i++) {
        ...
        /*----------------------------------*/
        (void) act_partial_collapse_assign_score_node(fanout, score_table, cost_table, init_param);
...
int
act_partial_collapse_assign_score_node(node, score_table, cost_table, init_param)
        node_t *node;
        st_table *score_table;
        st_table *cost_table;
        act_init_param_t *init_param;
...
static
node_t *
act_partial_collapse_find_max_score(network, score_table, pscore)
        network_t *network;
        st_table *score_table;
        int *pscore;
{
...
/*--------------------------------------------------------------------------------
  Assigns a score to each internal node. If the cost of the node is > 3, it is 0.
        ...
        act_init_param_t *init_param;
{
    ...
    for (i    = 0; i < num_nodes; i++) {
        ...
        score = act_partial_collapse_assign_score_node(node, score_table, cost_table,
                                                       init_param);
