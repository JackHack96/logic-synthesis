...
void merge_node(network, MAX_FANIN, MAX_COMMON_FANIN, MAX_UNION_FANIN, filename,
...
sm_row *sm_shortest_row(A) sm_matrix *A;
{
...
static sm_row *
    xln_merge_find_neighbor_of_row1_with_minimum_neighbors(row1,
                                                           coeff) sm_row *row1;
sm_matrix *coeff;
{
  sm_row *row, *row2;
...
xln_merge_update_neighbor_info(coeff, row1, row2) sm_matrix *coeff;
sm_row *row1, *row2;
{
...
---------------------------------------------------------------------------------------*/
st_table *xln_collapse_nodes_after_merge(network, match1_array, match2_array,
                                         support, verbose) network_t *network;
array_t *match1_array, *match2_array;
int support, verbose;
{
  st_table *table;
...
int xln_merge_are_fanouts_in_table(table, node) st_table *table;
node_t *node;
{
  lsGen gen;
  node_t *fanout;
