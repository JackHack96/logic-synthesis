 ...
 *
 * $Source: /users/pchong/CVS/sis/sis/pld/xln_merge.c,v $
 * $Author: pchong $
 * $Revision: 1.2 $
...
merge_node(network_t *network, int MAX_FANIN, int MAX_COMMON_FANIN, int MAX_UNION_FANIN, char *filename, int verbose,
           int use_lindo, array_t *match1_array, array_t *match2_array) {
    ...
    for (i = 0, col = 0; i < array_n(cand_node_array); i++) {
        ...
        for (j = i + 1; j < array_n(cand_node_array); j++) {
            ...
            if (num_intsec <= MAX_COMMON_FANIN && num_union <= MAX_UNION_FANIN) {
    ...
    /* either lindo not available or user did not choose lindo option - use a
       heuristic to solve maximum cardinality matching problem */
    /*------------------------------------------------------------------------------------------*/
    if (!use_lindo) {
        xln_merge_nodes_without_lindo(coeff, cand_node_array, match1_array,
                                      match2_array);
        ...
        if (sfd != -1) {
            ...
            /* Print the results */
            /*-------------------*/
            num_match = get_Lindo_result(cand_node_array, coeff, match1_array,
                                         match2_array, lindo_out, outputFile);
        }
        FREE(lindo_in);
        FREE(buf);
        FREE(lindo_out);
    ...
    if (verbose)
        (void) printf("Total number of matchings = %d\n\n", num_match);

    (void) fprintf(outputFile, "Merging two CLB's into one CLB\n");
    if (verbose)
        (void) printf("Merging two CLB's into one CLB\n");
    for (i = 0; i < array_n(match1_array); i++) {
        ...
        (void) fprintf(outputFile, "Merge node %s and %s\n", node_long_name(node1),
                       node_long_name(node2));
        if (verbose)
            (void) printf("Merge node %s and %s\n", node_long_name(node1),
                          node_long_name(node2));
    }
...
/*----------------------------------------------------------------------------------------------------
  An alternate to lindo option. Uses greedy merging. A node with minimum
mergeable nodes is picked first. It is combined with a neighbor node that has
minimum number of neighbors. The pairs of matches are stored in corresponding
entries of match1_array and match2_array.
-----------------------------------------------------------------------------------------------------*/
void xln_merge_nodes_without_lindo(sm_matrix *coeff, array_t *cand_node_array, array_t *match1_array,
                                   array_t *match2_array) {
    node_t *n1, *n2;
    sm_row *row1, *row2;
...
sm_row *sm_shortest_row(sm_matrix *A) {
...
static sm_row *xln_merge_find_neighbor_of_row1_with_minimum_neighbors(sm_row *row1, sm_matrix *coeff) {
    sm_row     *row, *row2;
    sm_element *p;
    int        min_neighbr;
...
void xln_merge_update_neighbor_info(sm_matrix *coeff, sm_row *row1, sm_row *row2) {
...
/*-------------------------------------------------------------------------------------
  There may be cases where after merge, a node can be collapsed into its fanouts
without destroying the feasibility of the network. Returns the rest of the nodes
of the network that are not present in match1_array or match2_array.
---------------------------------------------------------------------------------------*/
st_table *xln_collapse_nodes_after_merge(network_t *network, array_t *match1_array, array_t *match2_array, int support,
                                         int verbose) {
    st_table     *table;
    array_t      *nodevec; /* stores the nodes that are collapsed, so that they can be
                       deleted later */
...
int xln_merge_are_fanouts_in_table(st_table *table, node_t *node) {
    lsGen  gen;
    node_t *fanout;
    ...
    foreach_fanout(node, gen, fanout) {
        if (!st_is_member(table, (char *) fanout))
            return 0;
    }
    return 1;
}
