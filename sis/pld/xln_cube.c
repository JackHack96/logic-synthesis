...
case ONE:
array_insert(node_t
*, one_array, count1, fanin);
count1++;
count++;
break;
case ZERO:
array_insert(node_t
*, zero_array, count0, fanin);
count++;
count0++;
break;
case TWO:
...
(void)
node_print_rhs(sisout, cube_node
);
(void) printf("(fanins = %d) ",
node_num_fanin(cube_node)
);
}
(void) printf("\n\n");
}
