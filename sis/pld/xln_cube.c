 ...
 *
 * $Source: /users/pchong/CVS/sis/sis/pld/xln_cube.c,v $
 * $Author: pchong $
 * $Revision: 1.1.1.1 $
...
------------------------------------------------------------------------*/
void xln_network_ao_map(network_t *network, int size) {
    array_t *nodevec;
    int     num, i;
    node_t  *node;
...
/*------------------------------------------------------------------------
  ...
  also.
-------------------------------------------------------------------------*/
void xln_node_ao_map(node_t *node, int size) {
...
-------------------------------------------------------------------------*/
void xln_extract_big_ands_from_cube(network_t *network, node_t *node, node_cube_t cube, int size) {
    ...
    /*---------------------------------------------------------*/
    if (pld_num_fanin_cube(cube, node) <= size)
        return 0;
    ...
    foreach_fanin(node, j, fanin) {
        ...
        switch (literal) {
            case ONE:array_insert(node_t *, one_array, count1, fanin);
                count1++;
                count++;
                break;
            case ZERO:array_insert(node_t *, zero_array, count0, fanin);
                count++;
                count0++;
                break;
            case TWO:break;
...
/*------------------------------------------------------------------------
  ...
  ------------------------------------------------------------------------*/
node_t *xln_make_big_and(array_t *one_array, array_t *zero_array, int num_one, int num_zero) {
    int    i;
...
-----------------------------------------------------------------------*/
void xln_ao_replace_node_with_tlus(network_t *network, node_t *node, int size) {
...
/*-----------------------------------------------------------------------
  ...
  2) After all cubes in pcubevec have been thus processed, the tlu's
     ...
     different fanin).
-------------------------------------------------------------------------*/
int xln_make_tlus(network_t *network, node_t *node, array_t **pcubevec, array_t **ptluvec, int size) {
...
int xln_ao_compare(node_t **pn1, node_t **pn2) {
    int num1, num2;
...
-----------------------------------------------------------------------*/
int xln_is_cube_absorbed(node_t *cube_node, array_t *tluvec, int size) {
