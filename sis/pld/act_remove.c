...
int p_actRemove(network_t **network, int argc, char **argv)
{
  ...
  for (i = 0; i < input_size; i++) {
    ...
    p_actDestroy(&(ACT_SET(current_node)->act_num[locality]), locality);
...
/* ARGSUSED */
void p_actDestroy(act_set_entry_ptr, locality) ACT_ENTRY_PTR *act_set_entry_ptr;
int locality;
{
  ACT_ENTRY_PTR act_set_entry;

  if (*act_set_entry_ptr == NULL)
    return;
...
void p_dagDestroy(dag) ACT_VERTEX_PTR dag;
{
