...
ite_vertex *ite_for_cube_F(sm_matrix *F, node_t *node) {
...
ite_vertex *my_ite_and(sm_element *p, ite_vertex *vertex, char *name, node_t *fanin) {
    ...
    if (vertex == NIL(ite_vertex)) {
        switch ((int) p->user_word) {
            case 0:p_vertex = my_shannon_ite(vertex_IF, ite_0, ite_1);
                break;
            case 1:p_vertex = my_shannon_ite(vertex_IF, ite_1, ite_0);
                break;
            case 2:(void) fprintf(misout, "error in my_ite_and,\n");
                exit(1);
        }
    } else {
        switch ((int) p->user_word) {
            case 0:p_vertex = my_shannon_ite(vertex_IF, ite_0, vertex);
                break;
            case 1:p_vertex = my_shannon_ite(vertex_IF, vertex, ite_0);
                break;
            case 2:(void) fprintf(misout, "error in my_and act, p->u_w == 2\n");
                exit(1);
        }
    }
    return p_vertex;
...
ite_vertex *my_or_ite_F(array_t *array_b, sm_row *cover, array_t *array, network_t *network) {
    int        i;
    ite_vertex *vertex;
    sm_element *p;
    ite_vertex *ite;
    ...
    sm_foreach_row_element(cover, p) {
        name =
                sm_get_col(array_fetch(sm_matrix * , array, i), p->col_num)->user_word;
...
static void my_ite_OR(ite_vertex *up_vertex, ite_vertex *down_vertex) {
    ...
    st_foreach_item(table, stgen, &key, (char **) &vertex) {
        if (vertex->value != 3)
            continue;
        if (vertex->IF->value == 0)
            vertex->IF   = down_vertex;
        if (vertex->THEN->value == 0)
            vertex->THEN = down_vertex;
        if (vertex->ELSE->value == 0)
            vertex->ELSE = down_vertex;
    }
    st_free_table(table);
}
...
-------------------------------------------------------------------------------*/
ite_vertex *ite_literal(int b_col, char *name, node_t *fanin) {
    ite_vertex *ite;
...
ite_vertex *my_shannon_ite(ite_vertex *ite_IF, ite_vertex *ite_THEN, ite_vertex *ite_ELSE) {
    ite_vertex *ite;

    ite = ite_alloc();
    ite->value = 3;
    ite->index_size =
            ite_ELSE->index_size + ite_THEN->index_size + ite_IF->index_size;
    ite->IF   = ite_IF;
    ite->ELSE = ite_ELSE;
    ite->THEN = ite_THEN;
    return ite;
...
static int compare(char *obj1, char *obj2) {
    ...
    if (ite1->index_size == ite2->index_size)
        value = 0;
    if (ite1->index_size > ite2->index_size)
        value = 1;
    if (ite1->index_size < ite2->index_size)
...
-------------------------------------------------------------*/
node_t *act_make_node_from_row(sm_row *row, sm_matrix *F, network_t *network) {
    node_t     *n, *n1, *f, *fanin;
    int        phase;
    sm_col     *col;
    sm_element *p;
...
--------------------------------------------------------*/
st_table *act_make_name_to_element_table(sm_row *row, sm_matrix *F) {
    st_table   *table;
    char       *name;
    sm_element *p;
    sm_col     *col;
...
ite_vertex *ite_OR_itevec(array_t *itevec) {
    int        i;
...
ite_vertex *ite_check_for_single_literal_cubes(sm_matrix *F, network_t *network) {
...
-----------------------------------------------------------------*/
node_t *act_make_node_from_matrix(sm_matrix *F, network_t *network) {
    node_t *n, *n1, *f;
    sm_row *row;
