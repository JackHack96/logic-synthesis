...
ite_vertex *ite_for_cube_F(F, node) sm_matrix *F;
node_t *node;
{
...
ite_vertex *my_ite_and(p, vertex, name, fanin) sm_element *p;
ite_vertex *vertex;
char *name;
node_t *fanin;
{
...
ite_vertex *my_or_ite_F(array_b, cover, array, network) array_t *array_b;
array_t *array;
sm_row *cover;
network_t *network;
{
...
static void my_ite_OR(up_vertex, down_vertex) ite_vertex *up_vertex,
    *down_vertex;
{
...
-------------------------------------------------------------------------------*/
ite_vertex *ite_literal(b_col, name, fanin) int b_col;
char *name;
node_t *fanin;
{
  ite_vertex *ite;
...
ite_vertex *my_shannon_ite(ite_IF, ite_THEN, ite_ELSE) ite_vertex *ite_IF,
    *ite_THEN, *ite_ELSE;
{
  ite_vertex *ite;
...
-------------------------------------------------------------*/
node_t *act_make_node_from_row(row, F, network) sm_row *row;
sm_matrix *F;
network_t *network;
{
  node_t *n, *n1, *f, *fanin;
...
--------------------------------------------------------*/
st_table *act_make_name_to_element_table(row, F) sm_row *row;
sm_matrix *F;
{
  st_table *table;
  char *name;
...
ite_vertex *ite_OR_itevec(itevec) array_t *itevec;
{
  int i;
...
ite_vertex *ite_check_for_single_literal_cubes(F, network) sm_matrix *F;
network_t *network;
{
...
-----------------------------------------------------------------*/
node_t *act_make_node_from_matrix(F, network) sm_matrix *F;
network_t *network;
{
  node_t *n, *n1, *f;
  sm_row *row;
