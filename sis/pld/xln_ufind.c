...
unionop(node1, node2) struct tree_node *node1;
...
make_son(node1, node2) struct tree_node *node1, *node2;
{
  node1->parent = node2;
...
struct tree_node *find_tree(node) struct tree_node *node;
{
...
int estimate_clb_no(network, size) network_t *network;
int size;
{
  int upper_bound;
...
and_or_map(network, size) network_t *network;
int size;
{
...
int estimate_net_no(network) network_t *network;
{
  node_t *node;
  int value = 0, i;
  array_t *order;
