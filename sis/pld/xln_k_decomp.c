...
void karp_decomp_network(network, SUPPORT, one_node,
                         user_node) network_t *network;
int SUPPORT;
int one_node;
node_t *user_node;
{
...
------------------------------------------------------------------------------------------*/
static void karp_decomp_node_internal(node, intersection, lambda_indices,
                                      NUM_NODES, SUPPORT) node_t *node;
int **intersection;
int *lambda_indices;
...
/**************************************************************
  ...
  decomposition.
**************************************************************/
static array_t *karp_decomp_internal(node, intersection, lambda_indices,
                                     NUM_NODES, SUPPORT) node_t *node;
int **intersection;
int *lambda_indices;
int NUM_NODES, SUPPORT;
...
static void form_incompatibility_graph(node, node_inv, intersection,
                                       incompatible, lambda_table,
                                       lambda_indices, NUM_NODES,
                                       SUPPORT) node_t *node;
node_t *node_inv;
...
static void form_u_intersection(node, cube, node_inv, intersection,
                                incompatible, lambda_table, lambda_indices,
                                NUM_NODES, SUPPORT) node_t *node;
node_cube_t cube;
node_t *node_inv;
int **intersection;
...
--------------------------------------------------------------------*/
static void get_lambda_cube(cube, lambda_cube, lambda_indices,
                            SUPPORT) node_cube_t cube;
struct my_cube *lambda_cube;
int *lambda_indices;
int SUPPORT;
{
...
static int val_literal(literal) node_literal_t literal;
{
  int value;
...
static int **init_incompatible(NUM_NODES) int NUM_NODES;
{
  int i;
  int j;
  int **incompatible;
...
static void end_incompatible(incompatible, NUM_NODES) int **incompatible;
int NUM_NODES;
{
  int i;
...
/*---------------------------------------------------------------------
  ...
  incompatible.
----------------------------------------------------------------------*/
static void make_incompatible(a, b, incompatible, NUM_NODES,
                              SUPPORT) struct my_cube a,
    b;
int **incompatible;
int NUM_NODES, SUPPORT;
{
...
----------------------------------------------------------------------------*/
static void generate_all_minterms(min_arr, num_min, cube,
                                  support) int min_arr[];
int *num_min;
struct my_cube cube;
int support;
...
static void generate_minterm_with_index(min_arr, num_min, cube, support, value,
                                        i) int min_arr[];
int *num_min;
struct my_cube cube;
int support;
...
-------------------------------------------------------------------*/
static void tree_init(treenode, NUM_NODES) tree_node **treenode;
int NUM_NODES;
{
...
/*---------------------------------------------------------------------
  ...
  parent tree.
-----------------------------------------------------------------------*/
static void form_compatibility_classes(treenode, incompatible,
                                       NUM_NODES) tree_node **treenode;
int **incompatible;
int NUM_NODES;
{
...
-----------------------------------------------------------------*/
static void mark_roots_of_trees(treenode, root_table, NUM_NODES,
                                CLASS_NUM) tree_node **treenode;
st_table *root_table;
...
static void get_combination(node, comb_num, lambda_indices,
                            SUPPORT) node_t *node;
int comb_num;
int *lambda_indices;
int SUPPORT;
...
static void mark_all_inputs(node, lambda_table, lambda_indices,
                            SUPPORT) node_t *node;
st_table *lambda_table;
...
----------------------------------------------------------------*/
static int is_fanin_in_lambda(node, fanin, lambda_indices,
                              SUPPORT) node_t *fanin;
node_t *node;
int *lambda_indices, SUPPORT;
{
...
/*---------------------------------------------------------------
  ...
  lambda_set.
-----------------------------------------------------------------*/
static int marked(fanin, lambda_table) node_t *fanin;
st_table *lambda_table;
{
  char *dummy;
  int found;
...
static int find_alphas_and_G(node_to_decomp, lambda_table, treenode,
                             lambda_indices, NUM_NODES, SUPPORT, CLASS_NUM,
                             NUM_ALPHAS) node_t *node_to_decomp;
st_table *lambda_table;
tree_node **treenode;
int *lambda_indices;
...
/*-----------------------------------------------------------------------------
  ...
  lambda_space).
------------------------------------------------------------------------------*/
static node_t *product_node_for_minterm(minterm, node_to_decomp, lambda_indices,
                                        SUPPORT) int minterm;
...
--------------------------------------------------------------------------*/
static node_t *find_G(node_to_decomp, lambda_table, lambda_indices, treenode,
                      NUM_NODES, SUPPORT, NUM_ALPHAS) node_t *node_to_decomp;
st_table *lambda_table;
int *lambda_indices;
tree_node **treenode;
...
/*ARGSUSED*/
static void free_tree(treenode, root_table, lambda_table,
                      NUM_NODES) tree_node **treenode;
...
/*ARGSUSED*/
static enum st_retval k_decomp_free_table_entry(key, value, arg) char *key,
    *value, *arg;
{ return ST_DELETE; }
...
--------------------------------------------------------------------*/
static void replace_node_decomposed(network, node_to_replace,
                                    NUM_ALPHAS) network_t *network;
...
static void alloc_my_cube(cube, support) struct my_cube *cube;
int support;
{ cube->variable = ALLOC(int, support); }

static void free_my_cube(cube) struct my_cube *cube;
...
static void end_intersection(intersection) int **intersection;
{
  int i;
  for (i = 0; i < 3; i++) {
    FREE(intersection[i]);
...
--------------------------------------------------------------------------*/
network_t *xln_k_decomp_node_with_network(user_node, lambda_indices,
                                          SUPPORT) node_t *user_node;
int *lambda_indices, SUPPORT;
{
...
----------------------------------------------------------------------*/
array_t *xln_k_decomp_node_with_array(node, lambda_indices, SUPPORT,
                                      bound_alphas) node_t *node;
...
array_t *xln_checking_lambda_indices_create(node, lambda_indices,
                                            SUPPORT) node_t *node;
int *lambda_indices;
int SUPPORT;
{
...
xln_checking_lambda_indices(node, lambda_indices, check_vec) node_t *node;
int *lambda_indices;
array_t *check_vec;
{
  int i;
...
/*--------------------------------------------------------------------
  ...
  lambda_indices.
---------------------------------------------------------------------*/
int xln_modify_lambda_indices(node, node_simpl, lambda_indices,
                              SUPPORT) node_t *node,
    *node_simpl;
int *lambda_indices, SUPPORT;
{
...
xln_print_incompatible_matrix(incompatible, NUM_NODES) int **incompatible;
int NUM_NODES;
{
  int i, j;
