...
void karp_decomp_network(network_t *network, int SUPPORT, int one_node,
                         node_t *user_node) {
    ...
    for (i         = 0; i < num_nodes; i++) {
        ...
        if (node->type != INTERNAL)
            continue;
        karp_decomp_node_internal(node, intersection, lambda_indices, NUM_NODES,
                                  SUPPORT);
    }
...
/*----------------------------------------------------------------------------------------
  For the node, generates a lambda set, and then calls the karp_decomp_internal
routine. If the node cannot be decomposed, calls the split routine. Recursively
decomposes till feasible node generated.
------------------------------------------------------------------------------------------*/
static void karp_decomp_node_internal(node_t *node, int **intersection,
                                      int *lambda_indices, int NUM_NODES,
                                      int SUPPORT) {
    int     j, num;
    node_t  *new_node;
    array_t *newvec;
    ...
    while (1) {
        ...
        newvec = karp_decomp_internal(node, intersection, lambda_indices, NUM_NODES,
                                      SUPPORT);
...
/**************************************************************
  ...
  decomposition.
**************************************************************/
static array_t *karp_decomp_internal(node_t *node, int **intersection,
                                     int *lambda_indices, int NUM_NODES,
                                     int SUPPORT) {
    ...
    find_alphas_and_G(node, lambda_table, treenode, lambda_indices, NUM_NODES,
                      SUPPORT, CLASS_NUM, NUM_ALPHAS);
    nodevec = array_alloc(node_t *, 0);
...
static void form_incompatibility_graph(node_t *node, node_t *node_inv,
                                       int **intersection, int **incompatible,
                                       st_table *lambda_table,
                                       int *lambda_indices, int NUM_NODES,
                                       int SUPPORT) {
    int         node_numcubes;
...
static void form_u_intersection(node_t *node, node_cube_t cube,
                                node_t *node_inv, int **intersection,
                                int **incompatible, st_table *lambda_table,
                                int *lambda_indices, int NUM_NODES,
                                int SUPPORT) {
    int            NULL_INT;
    ...
    for (i = node_inv_numcubes - 1; i >= 0; i--) {
        ...
        foreach_fanin(node, j, fanin) {
            if (marked(fanin, lambda_table))
                continue; /* ignore lambda fanins */
        ...
        make_incompatible(lambda_cube, lambda_cube_inv, incompatible, NUM_NODES,
                          SUPPORT);
...
--------------------------------------------------------------------*/
static void get_lambda_cube(node_cube_t cube, struct my_cube *lambda_cube,
                            int *lambda_indices, int SUPPORT) {
    int            j;
    node_literal_t literal;
...
static int val_literal(node_literal_t literal) {
    int value;
...
static int **init_incompatible(int NUM_NODES) {
    int i;
    int j;
    int **incompatible;
...
static void end_incompatible(int **incompatible, int NUM_NODES) {
    int i;
...
/*---------------------------------------------------------------------
  ...
  incompatible.
----------------------------------------------------------------------*/
static void make_incompatible(struct my_cube a, struct my_cube b,
                              int **incompatible, int NUM_NODES, int SUPPORT) {
...
----------------------------------------------------------------------------*/
static void generate_all_minterms(int min_arr[], int *num_min,
                                  struct my_cube cube, int support) {
    generate_minterm_with_index(min_arr, num_min, cube, support, 0, 0);
}

static void generate_minterm_with_index(int min_arr[], int *num_min,
                                        struct my_cube cube, int support,
                                        int value, int i) {
    if (i == support - 1) {
        switch (cube.variable[i]) {
            case 0:min_arr[*num_min] = 2 * value;
                *num_min = *num_min + 1;
                return;
            case 1:

                min_arr[*num_min] = 2 * value + 1;
                *num_min = *num_min + 1;
                return;
            case 2:

                min_arr[*num_min] = 2 * value;
                *num_min = *num_min + 1;
                min_arr[*num_min] = 2 * value + 1;
                *num_min = *num_min + 1;
                return;
    ...
    switch (cube.variable[i]) {
        case 0:
            generate_minterm_with_index(min_arr, num_min, cube, support, 2 * value,
                                        i + 1);
            return;
        case 1:
            generate_minterm_with_index(min_arr, num_min, cube, support, 2 * value + 1,
                                        i + 1);
            return;
        case 2:
            generate_minterm_with_index(min_arr, num_min, cube, support, 2 * value,
...
-------------------------------------------------------------------*/
static void tree_init(tree_node **treenode, int NUM_NODES) {
...
/*---------------------------------------------------------------------
  ...
  parent tree.
-----------------------------------------------------------------------*/
static void form_compatibility_classes(tree_node **treenode, int **incompatible,
                                       int NUM_NODES) {
...
-----------------------------------------------------------------*/
static void mark_roots_of_trees(tree_node **treenode, st_table *root_table,
                                int NUM_NODES, int *CLASS_NUM) {
...
static void get_combination(node_t *node, int comb_num, int *lambda_indices,
                            int SUPPORT) {
    int num_fanin;
...
static void mark_all_inputs(node_t *node, st_table *lambda_table,
                            int *lambda_indices, int SUPPORT) {
...
----------------------------------------------------------------*/
static int is_fanin_in_lambda(node_t *node, node_t *fanin, int *lambda_indices,
                              int SUPPORT) {
...
/*---------------------------------------------------------------
  ...
  lambda_set.
-----------------------------------------------------------------*/
static int marked(node_t *fanin, st_table *lambda_table) {
    char *dummy;
    int  found;
    ...
    if (!found) {
        (void) fprintf(sisout,
                       " Fatal error: marked(fanin) : error in finding node\n",
                       node_name(fanin));
        exit(1);
    }
    return (int) dummy;
...
static int find_alphas_and_G(node_t *node_to_decomp, st_table *lambda_table,
                             tree_node **treenode, int *lambda_indices,
                             int NUM_NODES, int SUPPORT, int CLASS_NUM,
                             int NUM_ALPHAS) {
    ...
    G_node = find_G(node_to_decomp, lambda_table, lambda_indices, treenode,
                    NUM_NODES, SUPPORT, NUM_ALPHAS);
...
/*-----------------------------------------------------------------------------
  ...
  lambda_space).
------------------------------------------------------------------------------*/
static node_t *product_node_for_minterm(int minterm, node_t *node_to_decomp,
                                        int *lambda_indices, int SUPPORT) {
    ...
    if (length != SUPPORT) {
        (void) fprintf(
                sisout,
                "Fatal error: product_node_for_minterm(): length not SUPPORT\n");
        exit(1);
    }
    for (i = 0; i < length; i++) {
...
--------------------------------------------------------------------------*/
static node_t *find_G(node_t *node_to_decomp, st_table *lambda_table,
                      int *lambda_indices, tree_node **treenode, int NUM_NODES,
                      int SUPPORT, int NUM_ALPHAS) {
    ...
    for (i = node_num_cube(node_to_decomp) - 1; i >= 0; i--) {
        ...
        for (k = 0; k < num_min; k++) {
            ...
            minterm    = min_arr[k];
            xl_binary1(treenode[minterm]->class_num, NUM_ALPHAS, encoded_char);
...
/*ARGSUSED*/
static void free_tree(tree_node **treenode, st_table *root_table,
                      st_table *lambda_table, int NUM_NODES) {
...
/*ARGSUSED*/
static enum st_retval k_decomp_free_table_entry(char *key, char *value,
                                                char *arg) {
    return ST_DELETE;
}
...
--------------------------------------------------------------------*/
static void replace_node_decomposed(network_t *network, node_t *node_to_replace,
                                    int NUM_ALPHAS) {
...
static void alloc_my_cube(struct my_cube *cube, int support) {
    cube->variable = ALLOC(int, support);
}

static void free_my_cube(struct my_cube *cube) { FREE(cube->variable); }
...
static void end_intersection(int **intersection) {
    int i;
    for (i = 0; i < 3; i++) {
        FREE(intersection[i]);
    }
...
--------------------------------------------------------------------------*/
network_t *xln_k_decomp_node_with_network(node_t *user_node,
                                          int *lambda_indices, int SUPPORT) {
    ...
    /* Added July 1 '93 */
    new_support =
            xln_modify_lambda_indices(user_node, node_simpl, lambda_indices, SUPPORT);
    if (new_support <= 1) {
    ...
    end_intersection(intersection);
    (void) network_sweep(
            network); /* node simplify may be simplifying some nodes */
    return network;
}
...
----------------------------------------------------------------------*/
array_t *xln_k_decomp_node_with_array(node_t *node, int *lambda_indices,
                                      int SUPPORT, int bound_alphas) {
...
array_t *xln_checking_lambda_indices_create(node_t *node, int *lambda_indices,
                                            int SUPPORT) {
    array_t *check_vec;
    int     i;
    node_t  *fanin;
...
void xln_checking_lambda_indices(node_t *node, int *lambda_indices,
                                 array_t *check_vec) {
    int    i;
    node_t *fanin;
...
/*--------------------------------------------------------------------
  ...
  lambda_indices.
---------------------------------------------------------------------*/
int xln_modify_lambda_indices(node_t *node, node_t *node_simpl,
                              int *lambda_indices, int SUPPORT) {
    int    i, j;
    node_t *fanin;
...
void xln_print_incompatible_matrix(int **incompatible, int NUM_NODES) {
    int i, j;
