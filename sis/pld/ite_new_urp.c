...
void act_ite_intermediate_new_make_ite(node_t *node, act_init_param_t *init_param) {
    ...
    switch (node_fun) {
        case NODE_0:ite       = ite_alloc();
            ite->value = 0;
            ACT_ITE_ite(node) = ite;
            break;
        case NODE_1:ite = ite_alloc();
            ite->value = 1;
            ACT_ITE_ite(node) = ite;
            break;
        default:ite_end_table = st_init_table(st_ptrcmp, st_ptrhash);
            ite_0             = ite_alloc();
            ite_0->value = 0;
 ...
 * in a table so they can be easily accessed
   Note: node must be in the network (the only use of network is to get the
 fanin nodes
   from the fanin names: so the restriction can be removed in future, if
 needed)*/

void act_ite_new_make_ite(node_t *node, act_init_param_t *init_param) {
    ite_vertex          *ite_lit, *ite_if, *ite_else, *ite_else_if, *ite_else_then,
                        *ite_else_else, *ite_a, *ite_b, *ite_1_sav, *ite_0_sav, *ite_unate;
    node_function_t     node_fun, node_fun_if;
    node_t              *node_if, *node_else_then, *node_else_else, *fanin_a, *fanin_b,
                        *variable, *fanin;
    ...
    switch (node_fun) {
        case NODE_PI:;
        case NODE_PO:return;
        case NODE_0:ACT_ITE_ite(node) = ite_0;
            return;
        case NODE_1:ACT_ITE_ite(node) = ite_1;
            return;
    ...
    switch (init_param->VAR_SELECTION_LIT) {
        case 0:variable = node_most_binate_variable(node);
            break;
        case (-1):
    ...
    assert(variable);
    node_algebraic_cofactor(node, variable, &node_else_then, &node_else_else,
                            &node_if);
    ...
    switch (node_fun_if) {
        case NODE_0:ACT_ITE_ite(node) = ite_else;
            break;
        case NODE_BUF:fanin = node_get_fanin(node_if, 0);
            ite_if          = ite_new_literal(fanin);
            ACT_ITE_ite(node) = my_shannon_ite(ite_if, ite_1, ite_else);
            break;
        case NODE_INV:fanin = node_get_fanin(node_if, 0);
            ite_if          = ite_new_literal(fanin);
...
/*-----------------------------------------------------------
  ...
  returned.
-------------------------------------------------------------*/
node_t *node_most_binate_variable(node_t *node) {

    int    *lit_count_array, j, j2, j2p1, num_cube, max_count, diff_count, index,
           j_count, j_diff_count;
    node_t *variable;
...
/*---------------------------------------------------------------------
   ...
   of node.
----------------------------------------------------------------------*/
ite_vertex *ite_convert_match_to_ite(node_t *node, ACT_MATCH *match) {

    node_function_t node_fun, node_fun_S1, node_fun_S0;
    ite_vertex      *ite, *ite_IF, *ite_THEN, *ite_ELSE, *ite_lit, *ite_IF_1,
                    *ite_IF_2;
    node_t          *fanin;

    node_fun = node_function(node);
    switch (node_fun) {
        case NODE_0:return ite_0;
        case NODE_1:return ite_1;
        case NODE_BUF:fanin = node_get_fanin(node, 0);
            ite_lit         = ite_new_literal(fanin);
            ite             = my_shannon_ite(ite_lit, ite_1, ite_0);
            return ite;
        case NODE_INV:fanin = node_get_fanin(node, 0);
            ite_lit         = ite_new_literal(fanin);
...
ite_vertex *ite_for_muxnode(node_t *control, node_t *zero, node_t *one) {
    ...
    switch (node_fun) {
        case NODE_0:ite = ite_0;
            break;
        case NODE_1:ite = ite_1;
            break;
        case NODE_BUF:fanin = node_get_fanin(mux_node, 0);
            ite             = ite_buffer(fanin);
            break;
        case NODE_INV:fanin = node_get_fanin(mux_node, 0);
            ite             = ite_inv(fanin);
            break;
        default:ite_IF = ite_get_vertex(control);
...
ite_vertex *ite_buffer(node_t *node) {
...
ite_vertex *ite_inv(node_t *node) {
...
ite_vertex *ite_get_vertex(node_t *node) {
    node_function_t node_fun;
    ite_vertex      *ite;
    node_t          *fanin;
...
-----------------------------------------------------------------------*/
ite_vertex *ite_create_ite_for_orthogonal_cubes(node_t *node) {
...
-----------------------------------------------------------------------*/
int ite_find_mux_remaining(int p, int q, int count) {
    int q_3;
...
void ite_interleave_muxfree_and_full(array_t *muxfree_ite_vec, array_t *full_ite_vec, int m, int f) {
    ...
    /*------------------------------------------------------*/
    if ((m == array_n(muxfree_ite_vec)) && (f >= (array_n(full_ite_vec) - 1)))
        return;
    ...
    if (f == array_n(full_ite_vec)) {
        ...
        if (diff == 1) {
            ...
            if (diff == 2) {
                ...
                if (ite_is_inv(ite_a)) {
                    ite = ite_new_OR_with_inv(ite_a,
                                              ite_b); /* saving a mux (an inverter) */
                } else {
                    if (ite_is_inv(ite_b)) {
    ...
    /* combination of muxfree and full ites to be combined - at least one of each
     * present*/
    /*-----------------------------------------------------------------------------------*/
...
ite_vertex *ite_new_OR(ite_vertex *ite_a, ite_vertex *ite_b) {
    ite_vertex *ite;
...
----------------------------------------------------------------------*/
ite_vertex *ite_new_OR_with_inv(ite_vertex *ite_a, ite_vertex *ite_b) {
    ite_vertex *temp, *ite;
...
int ite_is_inv(ite_vertex *ite) {
    if (ite->value == 2 && ite->phase == 0)
        return 1; /* this should happen only in a canonical ite */
    if (ite->value == 3 && ite->IF->value == 2 && ite->IF->phase == 1 &&
        ite->THEN->value == 0 && ite->ELSE->value == 1)
        return 1;
    return 0;
}
...
-------------------------------------------------------*/
int node_num_binate_variables(node_t *node) {
    int    i, num_binate = 0;
    node_t *fanin;

    for (i = 0; i < node_num_fanin(node); i++) {
        fanin = node_get_fanin(node, i);
        if (node_input_phase(node, fanin) == BINATE)
            num_binate++;
    }
    return num_binate;
}
...
int ite_assign_var_weight(node_t *node, node_t *fanin, int j, int *lit_count_array) {
    ...
    node_algebraic_cofactor(node, fanin, &node_else_then, &node_else_else,
                            &node_if);
...
int node_compute_binateness(node_t *node) {
...
/* no changes needed */
static sm_matrix *build_F(node_t *node) {
    int            i, j;
    node_cube_t    cube;
        ...
        foreach_fanin(node, j, fanin) {
            ...
            switch (literal) {
                ...
                case TWO:sm_insert(F, i, j)->user_word = (char *) 2;
                    break;
                default:(void) fprintf(misout, "error in F construction \n");
                    break;
            }
        }
    }
...
ite_vertex *ite_new_ite_for_unate_cover(node_t *node, act_init_param_t *init_param) {
    ...
    node_t        *variable, *variable_lit, *sub_node, *sub_node_1, *node_cube,
                  *temp_node;
    ...
    if (is_neg_phase) {
        ...
        for (i     = 0, j = 0; i < F->ncols; i++) {
            switch (phase[i]) {
                case 0:weights[j] = M->ncols;
                    j++;
                    break;
                case 1:weights[j] = pos_weight;
                    j++;
                    break;
                case 2:break;
            }
    ...
    sm_foreach_row_element(cover, p) { p->user_word = (char *) phase[p->col_num]; }
    sm_free_space(M);

    /* constructs nodes that go with selected fanins */
    /*-----------------------------------------------*/

    sm_foreach_row(F, row) { row->user_word = (char *) 0; }
