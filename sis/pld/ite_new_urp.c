...
ACT_ITE_ite(node) =
my_shannon_ite(ite_lit, ite_1, ite_0
);
return;
}
...
assert                 (variable);
node_algebraic_cofactor(node, variable,
&node_else_then, &node_else_else, &node_if);
...
ACT_ITE_ite(node) =
my_shannon_ite(ite_if, ite_1, ite_else
);
break;
case NODE_INV:
fanin  = node_get_fanin(node_if, 0);
ite_if = ite_new_literal(fanin);
ACT_ITE_ite(node) =
my_shannon_ite(ite_if, ite_else, ite_1
);
break;
default:
...
if (num_cube_if == 1 &&
node_num_fanin(node_if)
== 2) {
fanin_a = node_get_fanin(node_if, 0);
if (
node_input_phase(node_if, fanin_a
) == NEG_UNATE) {
fanin_b = node_get_fanin(node_if, 1);
if (
node_input_phase(node_if, fanin_b
) == NEG_UNATE) {
...
/*------------------------------------------------------*/
if ((m ==
array_n(muxfree_ite_vec)
) && (f >= (
array_n(full_ite_vec)
- 1))) return;
