...
array_t *p_alap_order_nodes(node_vec) array_t *node_vec;
{
...
enum st_retval p_addPI(key, value, arg) char *key, *value, *arg;
{
  node_t *node;
  int level;
  array_t *pi_list;
...
array_t *pld_shuffle(list) array_t *list;
...
array_t *pld_order_nodes(node_vec, pi_only) array_t *node_vec;
int pi_only;
{
  node_t *dummy_node, *t_node, *tt_node, *root, *node;
  int i;
  ...
  p_recOrder(root, max_table, fanin_table, root_table, order_list, pi_only);
  node_free(dummy_node);
...
void p_rootAdd(node, root_table) node_t *node;
st_table *root_table;
{
  char *dummy;
  ...
  if (!st_lookup(root_table, (char *)node, &dummy)) {
    ...
    foreach_fanin(node, i, fanin) { p_rootAdd(fanin, root_table); }
  }
  return;
}

void p_levelAdd(node, level_table, root_table) node_t *node;
st_table *level_table, *root_table;
{
  ...
  foreach_fanin(node, i, fanin) {
    ...
    if (st_lookup(level_table, (char *)fanin, &dummy)) {
      ...
      (void)st_insert(level_table, (char *)fanin, (char *)(current_level + 1));
    }
  }
...
int p_maxAdd(node, max_table, level_table, root_table) node_t *node;
...
void p_recOrder(node, max_table, fanin_table, root_table, order_list,
                pi_only) node_t *node;
  ...
  for (i = 0; i < array_n(fanin_record_list); i++) {
    ...
    if (!st_lookup(fanin_table, (char *)(fanin_record->node), &dummy)) {
      p_recOrder(fanin_record->node, max_table, fanin_table, root_table,
                 order_list, pi_only);
      if (!pi_only) {
        if (st_lookup(iv_table, (char *)fanin_record->node, &dummy)) {
          array_insert_last(node_t *, order_list, (node_t *)dummy);
        }
      }
...
/*ARGSUSED*/
enum st_retval p_cleanRTable(key, value, arg) char *key, *value, *arg;
{
  st_table *root_table;
  node_t *node;
...
void p_recFree(node, root_table) node_t *node;
st_table *root_table;
{
  char *dummy;
  node_t *fanin;
...
int p_fanin_record_sort(key1, key2) char **key1, **key2;
{
...
/*ARGSUSED*/
enum st_retval p_cleanFRTable(key, value, arg) char *key, *value, *arg;
{
  fanin_record_t *fanin_record;
