...
total_cost +=
network_num_internal(network_node);
assert(!
st_insert(cost_table, node_long_name(node),
(char *) network_node));
}
array_free(nodevec);
if (XLN_DEBUG)
...
(void)
st_delete(table_affected_nodes,
&name, &dummy);
if (node->type != INTERNAL) continue;
if (
is_anyfo_PO(node)
...
(void)
st_insert(table_affected_nodes, node_long_name(fanout),
(char *) fanout);
foreach_fanin(fanout, k, fanin
) {
if (fanin == node) continue;
(void)
st_insert(table_affected_nodes, node_long_name(fanin),
(char *) fanin);
}
...
st_foreach_item(table_affected_nodes, stgen,
&key, (char **) &node) {
if (node->type != INTERNAL) continue;
array_insert_last(node_t
*, nodevec, node);
...
/*----------------------------------------------------------------*/
xln_collapse_remap(node,
&network_node, init_param);
...
st_foreach_item(fo_table, gen,
&key, (char **) &network_node) {
network_free(network_node);
}
st_free_table(fo_table);
}
...
init_param->
flag_decomp_good = flag_decomp_save;
}
