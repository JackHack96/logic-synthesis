...
void xln_partial_collapse(network_t *network, xln_init_param_t *init_param) {
    ...
    while (trivial_collapsed) {
        trivial_collapsed = xln_do_trivial_collapse_network(
                network, init_param->support, init_param->xln_move_struct.MOVE_FANINS,
    ...
    while (num) {
        ...
        for (i               = 0; i < num; i++) {
            ...
            if (cost_node > init_param->COST_LIMIT)
                continue;
...
int xln_cost_of_node(node_t *node, st_table *cost_table) {
    network_t *network_node;

    assert(st_lookup(cost_table, node_long_name(node), (char **) &network_node));
    if (network_node == NIL(network_t))
        return 0;
...
assigned any values.
--------------------------------------------------------------------------------------------*/
int xln_partial_collapse_node(network_t *network, node_t *node, st_table *cost_table, xln_init_param_t *init_param,
                              int *pgain) {
    int          gain, old_cost;
...
void xln_free_fo_table_without_freeing_key(st_table *fo_table) {
    st_generator *gen;
    char         *key;
    network_t    *network_node;
...
void xln_collapse_remap(node_t *node, network_t **pnetwork, xln_init_param_t *init_param) {
    network_t *network2;
    int       flag_decomp_save;

    /* if this flag is GOOD, then all the mapping options have been tried */
    /*--------------------------------------------------------------------*/
    if (init_param->good_or_fast == GOOD)
        return;
...
--------------------------------------------------------------------*/
void xln_collapse_check_area(network_t **pnetwork, xln_init_param_t *init_param, int roth_karp_flag) {
    network_t *network1;

    network1 = xln_check_network_for_collapsing_area(*pnetwork, init_param,
                                                     roth_karp_flag);
...
evaluates them. Returns the network with fewer nodes. Returns NIL if collapsing
  could not be done. If roth_karp flag is 0, roth_karp decomposition is not
done.
-----------------------------------------------------------------------------------*/
network_t *xln_check_network_for_collapsing_area(network_t *network, xln_init_param_t *init_param, int roth_karp_flag) {
