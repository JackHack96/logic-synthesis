...
if (cost_struct1) {
ite_my_free(cost_struct1
->ite);
my_free_act(cost_struct1
->act);
act_free_match(cost_struct1
->match);
FREE(cost_struct1);
}
...
if (cost_struct2 !=
NIL (ACT_ITE_COST_STRUCT)
) {
if (cost_struct2->match) {
act_free_match(cost_struct2
->match);
...
cost_struct->ite->
node = node;
act_ite_remap_put_node_names_in_ite(cost_struct
->ite, table1, network1);
assert(cost_struct
->act ==
NIL (ACT_VERTEX)
