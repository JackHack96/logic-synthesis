...
---------------------------------------------------------*/
void act_ite_cost_struct_replace(node_t *n1, node_t *n2, int map_alg) {
...
void ite_my_free(ite_vertex *ite) {
    st_table     *table;
    char         *key;
    st_generator *stgen;
    ite_vertex   *vertex;

    if (ite == NIL(ite_vertex))
        return;
...
void ite_my_free_vertex(ite_vertex *vertex) {
    if (vertex->fo != NULL) {
...
/*----------------------------------------------------------------------
  ...
  internal nodes.
-----------------------------------------------------------------------*/
void act_free_ite_network(network_t *network) {
    lsGen  gen;
    node_t *node;

    foreach_node(network, gen, node) { act_free_ite_node(node); }
}

void act_free_ite_node(node_t *node) {
    /*
...
void act_ite_remap_update_ite_fields(st_table *table1, network_t *network1, node_t *node) {
...
void act_ite_remap_put_node_names_in_ite(ite_vertex *ite, st_table *table1, network_t *network1) {
    st_table     *table;
    char         *key;
    st_generator *stgen;
    ite_vertex   *vertex;
...
void my_traverse_ite(ite_vertex *vertex, st_table *table) {
    if (vertex->value != 3)
        return;
...
------------------------------------------------------------------*/
st_table *ite_my_traverse_ite(ite_vertex *ite) {
    st_table *table;
...
/*-------------------------------------------------------------------
  ...
  vertex->fanin.
---------------------------------------------------------------------*/
node_t *ite_get_node_literal_of_vertex(ite_vertex *vertex) {
    node_t *node;
...
-------------------------------------------------------------*/
void ite_free_nodes_in_multiple_fo_ite(ite_vertex_ptr vertex) {
...
/*-----------------------------------------------------------
  ...
  multiple fanout vertices.
-------------------------------------------------------------*/
void act_free_nodes_in_multiple_fo_act(ACT_VERTEX_PTR vertex) {
    if (vertex->mark == MARK_COMPLEMENT_VALUE)
        return;
    vertex->mark = MARK_COMPLEMENT_VALUE;
    if (vertex->value <= 1)
