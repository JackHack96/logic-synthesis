...
---------------------------------------------------------*/
act_ite_cost_struct_replace(n1, n2, map_alg) node_t *n1, *n2;
int map_alg;
{
...
ite_my_free(ite) ite_vertex *ite;
{
  st_table *table;
  char *key;
  st_generator *stgen;
...
ite_my_free_vertex(vertex) ite_vertex *vertex;
{
  if (vertex->fo != NULL) {
...
/*----------------------------------------------------------------------
  ...
  internal nodes.
-----------------------------------------------------------------------*/
act_free_ite_network(network) network_t *network;
{
  lsGen gen;
  node_t *node;
...
act_free_ite_node(node) node_t *node;
{
  /*
...
act_ite_remap_update_ite_fields(table1, network1, node) st_table *table1;
network_t *network1;
node_t *node;
{
...
act_ite_remap_put_node_names_in_ite(ite, table1, network1) ite_vertex *ite;
st_table *table1;
network_t *network1;
{
  st_table *table;
...
my_traverse_ite(vertex, table) ite_vertex *vertex;
st_table *table;
{
  if (vertex->value != 3)
    return;
...
------------------------------------------------------------------*/
st_table *ite_my_traverse_ite(ite) ite_vertex *ite;
{
  st_table *table;
...
/*-------------------------------------------------------------------
  ...
  vertex->fanin.
---------------------------------------------------------------------*/
node_t *ite_get_node_literal_of_vertex(vertex) ite_vertex *vertex;
{
  node_t *node;
...
-------------------------------------------------------------*/
ite_free_nodes_in_multiple_fo_ite(vertex) ite_vertex_ptr vertex;
{
...
/*-----------------------------------------------------------
  ...
  multiple fanout vertices.
-------------------------------------------------------------*/
act_free_nodes_in_multiple_fo_act(vertex) ACT_VERTEX_PTR vertex;
{
  if (vertex->mark == MARK_COMPLEMENT_VALUE)
    return;
  vertex->mark = MARK_COMPLEMENT_VALUE;
