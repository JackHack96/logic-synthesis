 ...
 *
 * $Source: /users/pchong/CVS/sis/sis/pld/com_pld.c,v $
 * $Author: pchong $
 * $Revision: 1.1.1.1 $
 ...
 */
void traverse(ACT_VERTEX_PTR v, void (*manipulate)()) {
    v->mark = (v->mark == 1) ? 0 : 1;
    (*manipulate)(v);
    if ((v->index) != v->index_size) {
        /* not a terminal vertex   */
        if ((v->mark) != v->low->mark)
            traverse(v->low, manipulate);
        if ((v->mark) != v->high->mark)
            traverse(v->high, manipulate);
    }
}
...
/* command line interpreter for the ULM_merge command
                -Nishizaki*/
int com_ULM_merge(network_t **network, int argc, char **argv) {
    ...
    while ((c = util_getopt(argc, argv, "f:c:n:u:o:Flv")) != EOF) {
        switch (c) {
            ...
            case 'c':MAX_COMMON_FANIN = atoi(util_optarg);
                if (MAX_COMMON_FANIN < 0)
                    goto usage;
                break;
            case 'u':MAX_UNION_FANIN = atoi(util_optarg);
                if (MAX_UNION_FANIN < 0)
                    goto usage;
                break;
    ...
    merge_node(*network, MAX_FANIN, MAX_COMMON_FANIN, MAX_UNION_FANIN, filename,
               verbose, use_lindo, match1_array, match2_array);
    if (final_collapse_after_merge) {
        if (verbose)
            (void) printf("checking for collapses after merge...\n");
        st_free_table(xln_collapse_nodes_after_merge(
                *network, match1_array, match2_array, support, verbose));
    }
    ...
    usage:
    (void) fprintf(siserr,
                   "usage: xl_merge [-f MAX_FANIN] [-c MAX_COMMON_FANIN] [-u "
                   "MAX_UNION_FANIN] [-n support] [-o filename] [-vlF]\n");
    (void) fprintf(siserr, "    -f \t\tMAX_FANIN is the limit on the fanin of a "
                           "mergeable node(default = 4)\n");
    (void) fprintf(siserr, "    -c \t\tMAX_COMMON_FANIN is the limit on the "
                           "common fanins of two mergeable nodes(default = 4)\n");
    (void) fprintf(siserr, "    -u \t\tMAX_UNION_FANIN is the limit on the union "
                           "of the fanins of two mergeable nodes(default = 5)\n");
    (void) fprintf(siserr,
                   "    -n \t\tsupport\tfanin limit of nodes. (default = 5)\n");
    (void) fprintf(siserr, "    -o \t\tfilename is the file in which information "
                           "about the nodes merged is printed. Must specify.\n");
    (void) fprintf(siserr, "    -l \t\tdo not use lindo, use a heuristic to solve "
                           "max. card. matching.\n");
    (void) fprintf(siserr, "    -F \t\tdo not do further collapsing after "
                           "matching pairs of nodes (default: do it).\n");
    (void) fprintf(siserr, "    -v \t\tverbosity option.\n");
    return 1;
}

/*command line interpreter for the ULM_merge command */
int com_ULM_cover(network_t **network, int argc, char **argv) {
    ...
    usage:
    (void) fprintf(siserr, "usage: xl_cover [-n number] [-h heuristic] [-e "
                           "cover_node_limit_exact] [-u cover_node_limit_heur]\n");
    (void) fprintf(siserr, "-n \t\tlimit on the fanin\n");
    (void) fprintf(siserr,
                   "-h 0,1,2,3 \tuses heuristics for binate covering problem\n");
    (void) fprintf(siserr,
                   "   -  0(exact), 1 (Luciano's heuristic), 2 (greedy - for "
                   "large examples),\n\t 3(default)(automatically switches "
                   "heuristic number depending on e and u values)\n");
    (void) fprintf(siserr, "-e \t\tif h = 3, applies exact cover if number of "
                           "nodes < number (Default 30)\n");
    (void) fprintf(siserr, "-u \t\tif h = 3, applies heur 2 only if number of "
                           "nodes < number (Default 200)\n");
    return 1;
}
...
int com__node_value(network_t **network, int argc, char **argv) {
    lsGen  gen;
    node_t *node;
    int    c, count, value;
    ...
    while ((c = util_getopt(argc, argv, "v:")) != EOF) {
        switch (c) {
            ...
            default:(void) fprintf(sisout, "usage: _xl_node_value -v \n");
                (void) fprintf(sisout,
                               "-v should be followed by the fanin limit for the node\n");
                return 1;
        }
    }
    if (value < 1)
    ...
    foreach_node(*network, gen, node) {
        if (node_num_fanin(node) >= value) {
            count++;
            (void) fprintf(sisout, "%s:%d  ", node_name(node), node_num_fanin(node));
        }
    }
 ...
 * whole network.*/
int com_partition(network_t **network, int argc, char **argv) {
    int support               = 5;
    int c;
    int trivial_collapse_only =
                0; /* does just a trivial collapse without creating infeasibilities*/
    int MOVE_FANINS           = 0;
    int MAX_FANINS            = 15;
...
int com__count_nets(network_t **network) {
    int net_number;
...
int com__estimate_clb(network_t **network, int argc, char **argv) {
    int upper_bound, value;
    if (argc != 2) {
        (void) fprintf(sisout, "Error in commamnd\n ");
        (void) fprintf(sisout, "usage: _xl_clb n , n = maximum # of inputs          "
                               "                                   to a CLB \n");
        return 1;
    }
    value       = atoi(argv[argc - 1]);
    upper_bound = estimate_clb_no(*network, value);
    (void) fprintf(sisout, "estimate is %d\n", upper_bound);
    return 0;
}

/* karp decomposer*/
int com_karp_decomp(network_t **network, int argc, char **argv) {
    int              c;
    ...
    util_getopt_reset();
    while ((c = util_getopt(argc, argv, "n:f:p:v:der")) != EOF) {
        switch (c) {
            case 'n':support = atoi(util_optarg);
                if (support < 2) {
                ...
                break;
            case 'p':node = network_find_node(*network, util_optarg);
                if (node == NIL(node_t)) {
                    (void) printf(
                            "Error: xl_k_decomp: node %s not found, try altname of the node\n",
                            util_optarg);
                    return 1;
                }
                ...
                just_one_node = 1;
                break;
            case 'f':MAX_FANINS_K_DECOMP = atoi(util_optarg);
                break;
            case 'v':XLN_DEBUG = atoi(util_optarg);
                break;
            case 'r':RECURSIVE = 1;
            ...
            default:
                (void) fprintf(sisout,
                               "usage: xl_k_decomp [-n support] [-p nodename] [-v "
                               "verbosity_level] [-f MAX_FANINS_K_DECOMP] [-der]\n");
                ...
                (void) fprintf(sisout, "-f\tMAX_FANINS_K_DECOMP\t use exhaustive k_decomp "
                                       "if number of fanins\n");
                (void) fprintf(
                        sisout,
    ...
    init_param->MAX_FANINS_K_DECOMP = MAX_FANINS_K_DECOMP;
    init_param->RECURSIVE           = RECURSIVE;
    init_param->DESPERATE           = DESPERATE;
    if (EXHAUSTIVE) {
...
int com_and_or_map(network_t **network, int argc, char **argv) {
    int size;

    if (argc != 2) {
        (void) fprintf(sisout, "Error in command\n");
...
int com_divide_network(network_t **network, int argc, char **argv) {
    int size = 5, c;
    util_getopt_reset();
    XLN_DEBUG = 0;

    while ((c = util_getopt(argc, argv, "n:d")) != EOF) {
        switch (c) {
            case 'n':size = atoi(util_optarg);
                break;
            case 'd':XLN_DEBUG = 1;
                break;
            default:(void) fprintf(sisout, "usage: xl_split -n -d \n");
                (void) fprintf(sisout,
    ...
    if (size == 0) {
        (void) fprintf(sisout, "Error, split parameter must be greater than zero\n");
...
static void print_partition_usage() {
    (void) fprintf(siserr, "usage: xl_partition [-n support] [-m (MOVE_FANINS)] "
                           "[-M MAX_FANINS] [-t]\n");
    (void) fprintf(siserr, "n - #of inputs to each partition\n");
    (void) fprintf(siserr,
                   "m - moves fanins of the node if collapsing is not feasible\n");
    (void) fprintf(siserr, "M - if m option used, moves fanins only if number of "
                           "fanins at most the number (Default:15)\n");
    (void) fprintf(siserr, "t - collapse a node into all fanouts, if all of them "
                           "remain feasible, delete node\n");
}

int com_act_main(network_t **network, int argc, char **argv) {
    st_table *cost_table;
    int      c;
    ...
    int   HEURISTIC_NUM;   /* chooses the heuristic for building the
                          ...
                          cofactor tree), = 3 a combination of the two, = 4
                          contructs both   subject graphs for each node, pick the
                          one with lower cost.*/
    ...
    /*---------------*/
    util_getopt_reset();
    while ((c = util_getopt(argc, argv, "M:h:n:f:d:g:r:m:i:oqDlsv")) != EOF) {
        switch (c) {
            case 'M':MAXOPTIMAL = atoi(util_optarg);
                break;
            case 'n':NUM_ITER = atoi(util_optarg);
                if (NUM_ITER < 0) {
                    (void) printf("num_iteration(%d) should be > 0\n", NUM_ITER);
                    return 1;
                }
                break;
            case 'h':HEURISTIC_NUM = atoi(util_optarg);
                ...
                break;
            case 'f':FANIN_COLLAPSE = atoi(util_optarg);
                if (FANIN_COLLAPSE < 0) {
                ...
                break;
            case 'd':DECOMP_FANIN = atoi(util_optarg);
                if (DECOMP_FANIN < 1) {
                    (void) printf("decomp_fanin(%d) should be > 0\n", DECOMP_FANIN);
                    return 1;
                }
                break;
            case 'g':GAIN_FACTOR = atof(util_optarg);
                ...
                break;
            case 'r':BREAK = 1;
                (void) strcpy(filename, util_optarg);
            ...
            case 'm':mode = atof(util_optarg);
                ...
                break;
            case 'i':fileflag = 1;
                (void) strcpy(init_param->delayfile, util_optarg);
                break;
            case 'q':QUICK_PHASE = 1;
                break;
            case 'o':act_is_or_used = 0;
                break;
            case 'l':LAST_GASP = 1;
                break;
            case 'D':DISJOINT_DECOMP = 1;
                break;
            case 's':ACT_STATISTICS = 1;
                break;
            case 'v':ACT_DEBUG = 1;
    ...
    init_param->HEURISTIC_NUM   = HEURISTIC_NUM;
    init_param->NUM_ITER        = NUM_ITER;
    init_param->FANIN_COLLAPSE  = FANIN_COLLAPSE;
    init_param->GAIN_FACTOR     = GAIN_FACTOR;
    init_param->DECOMP_FANIN    = DECOMP_FANIN;
    init_param->DISJOINT_DECOMP = DISJOINT_DECOMP;
    init_param->QUICK_PHASE     = QUICK_PHASE;
    init_param->LAST_GASP       = LAST_GASP;
    init_param->BREAK           = BREAK;
    init_param->mode            = mode;

    if ((fileflag == 0) && (mode != 0.0))
        goto usage;
    ...
    *network = act_map_network(*network, init_param, cost_table);
    ...
    usage:
    (void) printf(" usage: act_map [-h heuristic_num] [-m mode] [-n "
                  "num_iteration] [-f collapse_fanin] [-M MAXOPTIMAL]\n");
    (void) printf("	 \t\t[-g gain_factor] [-d decomp_fanin] [-r filename] "
                  "[-i delayfile] [-qolDsv]\n");
    (void) printf(
            "     -h\theuristic_num\tFor making subject-graphs (default = 2).\n");
    (void) printf("     \t\t\t\t1=>ROBDD, 2 =>BDD (cofactor-tree)\n");
    (void) printf("     \t\t\t\t3=>program decides, 4 => both 1 and 2\n");
    (void) printf("     -m\tmode\tChoose any real number between AREA (0.0) and "
                  "DELAY (1.0) (default = 0.0).\n");
    (void) printf("     -n\tnum_iteration\tNumber of iterations (default = 0).\n");
    (void) printf("     -f\tcollapse_fanin\tUpper bound on fanin for collapse "
                  "(default = 3).\n");
    (void) printf("     -M\tMAXOPTIMAL\tall possible orderings tried for ROBDD if "
                  "number of fanins of the node <= MAXOPTIMAL (default = 6).\n");
    (void) printf("     -d\tdecomp_fanin\tLower bound on fanin for decomposition "
                  "(default = 4).\n");
    (void) printf("     -g\tgain_factor\tCriterion for going to next iteration "
                  "(default = 0.01).\n");
    (void) printf("     -r\tfilename\tFinal restructuring to be done and "
                  "bdnet-like file.\n");
    (void) printf("     \t\t\t       filename to be created. \n");
    (void) printf("     -i\tfilename\tdelayfile is the name of the file with "
                  "info. about delay of\n");
    (void) printf("     \t\t\t       a basic block for different fanouts. if mode "
                  "!= 0.0, must specify. \n");
...
int com_xln_ao(network_t **network, int argc, char **argv) {
    int support, c;
    ...
    util_getopt_reset();
    while ((c = util_getopt(argc, argv, "n:v:")) != EOF) {
        switch (c) {
            case 'n':support = atoi(util_optarg);
                if (support < 2) {
            ...
            case 'v':XLN_DEBUG = atoi(util_optarg);
                break;
            default:(void) fprintf(sisout, "usage: xl_ao [-n number] [-v verbosity level]\n");
...
int com_xln_improve(network_t **network, int argc, char **argv) {
    ...
    util_getopt_reset();
    while ((c = util_getopt(argc, argv, "M:n:c:f:g:l:v:Aabdmr")) != EOF) {
        switch (c) {
            case 'n':support = atoi(util_optarg);
                if (support < 2) {
            ...
            case 'M':MAX_FANINS = atoi(util_optarg);
                ...
                break;
            case 'a':good_or_fast = FAST;
                break;
            case 'A':absorb = 0;
                break;
            case 'c':cover_node_limit = atoi(util_optarg);
                break;
            case 'l':lit_bound = atoi(util_optarg);
                break;
            case 'v':XLN_DEBUG = atoi(util_optarg);
                break;
            case 'b':XLN_BEST = 1;
                break;
            case 'g':flag_decomp_good = atoi(util_optarg);
                break;
            case 'd':DESPERATE = 0;
                break;
            case 'r':RECURSIVE = 1;
            ...
            default:
                ...
                (void) fprintf(sisout, "\t [-d(DESP)][-v verbosity level] [-A(don't move "
                                       "fanins after decomp -g)]\n");
                (void) fprintf(sisout, "\t [-a(don't try all decompositions)] [-m "
                                       "(MOVE_FANINS)][-M MAX_FANINS]\n");
                (void) fprintf(sisout, "-n\tnumber\tfanin limit of nodes.\n");
                (void) fprintf(
                        sisout,
                        "-c\tcover_node_limit exact cover of node till these many nodes.\n");
                (void) fprintf(
                        sisout, "-l\tnum_literals > lit_bound => good decompose the node.\n");
                (void) fprintf(
                        siserr,
                        "-m\tmoves fanins of the node if collapsing is not feasible\n");
                (void) fprintf(siserr, "-M\tif m option used, moves fanins only if number "
                                       "of fanins at most the number (Default:15)\n");
                (void) fprintf(sisout, "-v\tverbosity level.\n");
                return 1;
        }
    }
    ...
    init_param = ALLOC(xln_init_param_t, 1);
    init_param->support                     = support;
    init_param->cover_node_limit            = cover_node_limit;
    init_param->lit_bound                   = lit_bound;
    init_param->flag_decomp_good            = flag_decomp_good;
    init_param->good_or_fast                = good_or_fast;
    init_param->absorb                      = absorb;
    init_param->DESPERATE                   = DESPERATE;
    init_param->RECURSIVE                   = RECURSIVE;
    init_param->MAX_FANINS_K_DECOMP         = MAX_FANINS_K_DECOMP;
    init_param->xln_move_struct.MOVE_FANINS = MOVE_FANINS;
    init_param->xln_move_struct.MAX_FANINS  = MAX_FANINS;
    xln_improve_network(*network, init_param);
    FREE(init_param);
    return 0;
}

int com_xln_reduce_levels(network_t **network, int argc, char **argv) {
    int              support, heur, c;
    int              MAX_FANINS, MOVE_FANINS, bound_alphas;
    int              collapse_input_limit;
    int              traversal_method; /* 1 then topological traversal, else levels sorted wrt
                           width */
    xln_init_param_t *init_param;
    ...
    util_getopt_reset();
    while ((c  = util_getopt(argc, argv, "c:n:v:h:M:A:mt")) != EOF) {
        switch (c) {
            case 'n':support = atoi(util_optarg);
                if (support < 2) {
            ...
            case 'v':XLN_DEBUG = atoi(util_optarg);
                break;
            case 'h':heur = atoi(util_optarg);
                break;
            case 'c':collapse_input_limit = atoi(util_optarg);
                break;
            case 'M':MAX_FANINS = atoi(util_optarg);
                break;
            case 'A':bound_alphas = atoi(util_optarg);
                break;
            case 'm':MOVE_FANINS = 0;
                break;
            case 't':traversal_method = 0;
                break;
            default:(void) fprintf(sisout, "usage:xl_rl [-n number] [-v verbosity level]\n");
                (void) fprintf(sisout,
                               "\t[-m (DONT_MOVE_FANINS)] [-M MAX_FANINS] [-h heur]\n");
                (void) fprintf(sisout,
                               "\t[-t (trav_method-levels)] [-c collapse_input_limit]\n");
                (void) fprintf(sisout, "-n\tnumber\tfanin limit of nodes.\n");
                (void) fprintf(sisout, "-h\theur\t1=>collapse into any fanout, 2=> "
                                       "collapse into critical fanouts\n");
                (void) fprintf(sisout, "-c\tnumber\tconsider collapsing of network if "
                                       "PI's less (def = 10).\n");
                (void) fprintf(
                        siserr,
                        "-m\tmoves fanins of the node if collapsing is not feasible\n");
    ...
    init_param = ALLOC(xln_init_param_t, 1);
    init_param->support                      = support;
    init_param->heuristic                    = heur; /* overuse of heuristic */
    init_param->collapse_input_limit         = collapse_input_limit;
    init_param->traversal_method             = traversal_method;
    init_param->xln_move_struct.MOVE_FANINS  = MOVE_FANINS;
    init_param->xln_move_struct.MAX_FANINS   = MAX_FANINS;
    init_param->xln_move_struct.bound_alphas = bound_alphas;
...
int com_xln_partial_collapse(network_t **network, int argc, char **argv) {
    int              size, c;
    ...
    util_getopt_reset();
    while ((c = util_getopt(argc, argv, "M:n:C:v:c:g:l:bamA")) != EOF) {
        switch (c) {
            case 'n':size = atoi(util_optarg);
                if (size < 2) {
                    (void) printf(
                            "Error: xl_part_coll: violation*** 2 <=number: exiting \n");
                    return 1;
                }
                break;
            case 'm':MOVE_FANINS = 1;
            ...
            case 'g':flag_decomp_good = atoi(util_optarg);
                break;
            case 'a':good_or_fast = 1;
                break;
            case 'A':absorb = 1;
                break;
            case 'C':COST_LIMIT = atoi(util_optarg);
                break;
            case 'c':cover_node_limit = atoi(util_optarg);
                break;
            case 'l':lit_bound = atoi(util_optarg);
            ...
            default:
                ...
                (void) fprintf(
                        sisout, " [-g (flag_decomp_good)] [-a (apply all_decomp_schemes)]\n");
                (void) fprintf(sisout, " [-A (move fanins after decomp -g)] [-m "
                                       "(MOVE_FANINS)] [-M (MAX_FANINS)]\n");
                (void) fprintf(sisout, "-n\tnumber\tfanin limit of nodes.\n");
                (void) fprintf(
                        sisout,
    ...
    init_param = ALLOC(xln_init_param_t, 1);
    init_param->support = size;
    init_param->COST_LIMIT =
            COST_LIMIT; /* node has higher cost => do not collapse */
    init_param->cover_node_limit            = cover_node_limit;
    init_param->lit_bound                   = lit_bound;
    init_param->good_or_fast                = good_or_fast;
    init_param->absorb                      = absorb;
    init_param->flag_decomp_good            = flag_decomp_good;
    init_param->RECURSIVE                   = 0;
    init_param->DESPERATE                   = 1;
    init_param->MAX_FANINS_K_DECOMP         = 7;
    init_param->xln_move_struct.MOVE_FANINS = MOVE_FANINS;
    init_param->xln_move_struct.MAX_FANINS  = MAX_FANINS;
...
int com_xln_decomp_for_merging_network(network_t **network, int argc, char **argv) {
    int              c;
    xln_init_param_t *init_param;
    ...
    usage:
    (void) fprintf(siserr, "usage: xl_decomp_two [-f MAX_FANIN] [-c "
                           "MAX_COMMON_FANIN] [-u MAX_UNION_FANIN]\n");
    (void) fprintf(siserr,
                   "\t\t\t[-n support] [-h heuristic] [-l lower_common_bound]\n");
    (void) fprintf(siserr, "\t\t\t[-L cube_support_lower_bound] [-v]\n");
    (void) fprintf(siserr, " -f\t\tMAX_FANIN is the limit on the fanin of a "
                           "mergeable node(default = 4)\n");
    (void) fprintf(siserr, " -c\t\tMAX_COMMON_FANIN is the limit on the common "
                           "fanins of two mergeable nodes(default = 4)\n");
    (void) fprintf(siserr, " -u\t\tMAX_UNION_FANIN is the limit on the union of "
                           "the fanins of two mergeable nodes(default = 5)\n");
    (void) fprintf(sisout, "-n\tnumber\tfanin limit of nodes. (default = 5)\n");
    (void) fprintf(
            sisout, "-h\theuristic\t 1 = ALL_CUBES, 2 = PAIR_NODES. (default = 1)\n");
...
int com_xln_absorb_nodes_in_fanins(network_t **network, int argc, char **argv) {
    int c;
    int support;
    int method;
    int MAX_FANINS;
    ...
    if (argc - util_optind != 0)
        goto usage;
    if (method) {
        xln_network_reduce_infeasibility_by_moving_fanins(*network, support,
                                                          MAX_FANINS);
    } else {
    ...
    usage:
    (void) fprintf(siserr,
                   "xln_absorb \t[-n support] [-m method] [-f MAX_FANINS] [-v]\n");
    (void) fprintf(sisout, "-n\tsupport\tfanin limit of nodes. (default = 5)\n");
    (void) fprintf(sisout, "-f\tMAX_FANINS\t don't consider nodes above these "
                           "fanins(default = 15)\n");
    (void) fprintf(sisout,
                   "-m\tmethod\t0 => absorb, 1 => move fanins (default = 1)\n");
    (void) fprintf(siserr, "-v \tverbosity option.\n");
    return 1;
}

int com_xln_collapse_check_area(network_t **network, int argc, char **argv) {
    int              c;
    int              support;
    int              collapse_input_limit;
    int              roth_karp_flag;
    ...
    usage:
    (void) fprintf(siserr, "xl_coll_ck \t[-n support] [-c input_collapse_lim] [-k "
                           "(don't roth)] [-v]\n");
    (void) fprintf(sisout, "-n\tsupport\tfanin limit of nodes. (default = 5)\n");
    (void) fprintf(sisout,
                   "-c\tcollapse\t use cofactor and roth-karp(default = 9)\n");
    (void) fprintf(siserr, "-v \tverbosity option.\n");
    return 1;
}
