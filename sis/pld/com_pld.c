      ...
      while(( c = util_getopt(argc, argv, "n:f:p:v:der")) != EOF){
		...
		case 'p':
	        ...
	        return 1;
 			...
 			(void)printf("Error: xl_k_decomp: node %s PI\n", util_optarg);
                    ...
                    if (node->type == PRIMARY_OUTPUT) node = node_get_fanin(node, 0);
		    just_one_node = 1;
		    break;
		case 'f':
		...
		case 'd':
		    DESPERATE = 0;
                    break;
		default:
	 	    (void) fprintf(sisout, "usage: xl_k_decomp [-n support] [-p nodename] [-v verbosity_level] [-f MAX_FANINS_K_DECOMP] [-der]\n");
...
char **argv;
{
  ...
  if ((fileflag == 0) && (mode != 0.0)) goto usage;
  ...
  while(( c = util_getopt(argc, argv, "M:n:c:f:g:l:v:Aabdmr")) != EOF){
	  ...
	  case 'M':
        ...
        case 'f':
          ...
          break;
        default:
          (void) fprintf(sisout, "usage: xl_imp [-n number] [-c cover_node_limit] [-l lit_bound]\n");
  ...
  char **argv;
{
  ...
  while(( c = util_getopt(argc, argv, "M:n:C:v:c:g:l:bamA")) != EOF){
	  ...
	  case 'M':
        ...
        case 'l':
          ...
          break;
        default:
          (void) fprintf(sisout, "usage: xl_part_coll [-n number] [-C cost_limit] [-b]");
          (void) fprintf(sisout, " [-c cover_node_limit] [-l lit_bound] [-v verbosity level]\n");
  ...
  init_param->COST_LIMIT = COST_LIMIT; /* node has higher cost => do not collapse */
  init_param->cover_node_limit = cover_node_limit;
  init_param->lit_bound = lit_bound;
  init_param->good_or_fast = good_or_fast;
  init_param->absorb = absorb;
