 ...
 */
void traverse(v, manipulate) ACT_VERTEX_PTR v;
void (*manipulate)();
...
/* command line interpreter for the ULM_merge command
                -Nishizaki*/
int com_ULM_merge(network, argc, argv) network_t **network;
int argc;
char **argv;
{
  ...
  while ((c = util_getopt(argc, argv, "f:c:n:u:o:Flv")) != EOF) {
    ...
    case 'n':
      ...
      break;
    case 'c':
      MAX_COMMON_FANIN = atoi(util_optarg);
      if (MAX_COMMON_FANIN < 0)
        goto usage;
      break;
    case 'u':
      MAX_UNION_FANIN = atoi(util_optarg);
...
/*command line interpreter for the ULM_merge command */
int com_ULM_cover(network, argc, argv) network_t **network;
int argc;
char **argv;
{
  ...
  while ((c = util_getopt(argc, argv, "n:h:e:u:d")) != EOF) {
    ...
    case 'n':
      ...
      break;
    case 'e':
      cover_node_limit_exact = atoi(util_optarg);
      break;
    case 'u':
      cover_node_limit_heur_upper = atoi(util_optarg);
      break;
    case 'h':
      bincov_heuristics = atoi(util_optarg);
      break;
    case 'd':
      XLN_DEBUG = 1;
      break;
...
int com__node_value(network, argc, argv) network_t **network;
int argc;
char **argv;
{
  lsGen gen;
  ...
  while ((c = util_getopt(argc, argv, "v:")) != EOF) {
    switch (c) {
    case 'v':
      value = atoi(util_optarg);
      break;
    default:
      (void)fprintf(sisout, "usage: _xl_node_value -v \n");
      (void)fprintf(sisout,
 ...
 * whole network.*/
int com_partition(network, argc, argv) network_t **network;
int argc;
char **argv;
{
  int support = 5;
...
int com__count_nets(network) network_t **network;
{
  int net_number;
...
com__estimate_clb(network, argc, argv) network_t **network;
int argc;
char **argv;
{
...
/* karp decomposer*/
com_karp_decomp(network, argc, argv) network_t **network;
int argc;
char **argv;
{
  int c;
...
com_and_or_map(network, argc, argv) network_t **network;
int argc;
char **argv;
{
  int size;
...
com_divide_network(network, argc, argv) network_t **network;
int argc;
char **argv;
{
  int size = 5, c;
...
com_act_main(network, argc, argv) network_t **network;
int argc;
char **argv;
{
  st_table *cost_table;
...
com_xln_ao(network, argc, argv) network_t **network;
int argc;
char **argv;
{
  int support, c;
  ...
  while ((c = util_getopt(argc, argv, "n:v:")) != EOF) {
    ...
    case 'v':
      ...
      break;
    default:
      (void)fprintf(sisout, "usage: xl_ao [-n number] [-v verbosity level]\n");
...
com_xln_improve(network, argc, argv) network_t **network;
int argc;
char **argv;
{
...
com_xln_reduce_levels(network, argc, argv) network_t **network;
int argc;
char **argv;
{
  int support, heur, c;
  ...
  while ((c = util_getopt(argc, argv, "c:n:v:h:M:A:mt")) != EOF) {
    ...
    case 't':
      ...
      break;
    default:
      (void)fprintf(sisout, "usage:xl_rl [-n number] [-v verbosity level]\n");
      (void)fprintf(sisout,
...
com_xln_partial_collapse(network, argc, argv) network_t **network;
int argc;
char **argv;
{
  int size, c;
...
int com_xln_decomp_for_merging_network(network, argc, argv) network_t **network;
int argc;
char **argv;
{
  int c;
  ...
  while ((c = util_getopt(argc, argv, "f:c:n:u:h:l:L:v")) != EOF) {
    switch (c) {
    case 'f':
      init_param->MAX_FANIN = atoi(util_optarg);
      if (init_param->MAX_FANIN < 0)
        goto usage;
      break;
    case 'c':
      init_param->MAX_COMMON_FANIN = atoi(util_optarg);
      if (init_param->MAX_COMMON_FANIN < 0)
        goto usage;
      break;
    case 'u':
      init_param->MAX_UNION_FANIN = atoi(util_optarg);
      if (init_param->MAX_UNION_FANIN < 0)
        goto usage;
      break;
    case 'n':
      init_param->support = atoi(util_optarg);
      if (init_param->support < 0)
        goto usage;
      break;
    case 'h':
      init_param->heuristic = atoi(util_optarg);
      if ((init_param->heuristic < 1) || (init_param->heuristic > 2))
        goto usage;
      break;
    case 'l':
      init_param->common_lower_bound = atoi(util_optarg);
      break;
    case 'L':
      init_param->cube_support_lower_bound = atoi(util_optarg);
      break;
    case 'v':
      XLN_DEBUG = 1;
      break;
...
int com_xln_absorb_nodes_in_fanins(network, argc, argv) network_t **network;
int argc;
char **argv;
{
  int c;
...
int com_xln_collapse_check_area(network, argc, argv) network_t **network;
int argc;
char **argv;
{
  int c;
  ...
  while ((c = util_getopt(argc, argv, "c:n:kv")) != EOF) {
    ...
    case 'v':
      ...
      break;
    case 'k':
      roth_karp_flag = 0; /* do not do roth-karp */
      break;
    default:
      goto usage;
    }
