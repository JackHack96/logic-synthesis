...
---------------------------------------------------------------------------*/
act_ite_map_node_with_iter_imp(node, init_param) node_t *node;
act_init_param_t *init_param;
{
  node_function_t node_fun;
  network_t *network1;
  /* network_t *new_network; */ /* commented because using new bdd package - not
                                   using */
  ...
  if (node_fun == NODE_0 || node_fun == NODE_1 || node_fun == NODE_BUF) {
    ...
    */
    ...
    act_ite_map_network_with_iter(network1,
                                  init_param); /* with at most one iteration */
     ...
     */
    /*---------------------------------------------------------------------------*/
    if (ACT_ITE_SLOT(node)->cost > 2 && (node_fun != NODE_AND) &&
        (node_fun != NODE_OR) && save_map_method == MAP_WITH_ITER) {
    ...
    /*
    if (ACT_ITE_SLOT(node)->cost > 3 && act_bdd_new_sav && (node_fun !=
    NODE_AND) && (node_fun != NODE_OR)) { new_network =
    act_map_using_new_bdd(network1, init_param);
...
int network_num_nonzero_cost_nodes(network) network_t *network;
{
  int count;
  node_t *node;
  node_function_t node_fun;
