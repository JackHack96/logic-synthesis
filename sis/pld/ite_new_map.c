...
); /* with at most one iteration */
act_free_ite_network(network1); /* frees the ite, act and match */
ACT_ITE_SLOT(node)->
cost = network_num_nonzero_cost_nodes(network1);
...
*/
init_param->
FANIN_COLLAPSE = save_fanin_collapse;
init_param->
MAP_METHOD = save_map_method;
init_param->
NUM_ITER = save_iter;
init_param->
LAST_GASP = save_last_gasp;
init_param->
BREAK = save_break;
/* ACT_BDD_NEW = act_bdd_new_sav; */

}
return ACT_ITE_SLOT(node)->
cost;
}
