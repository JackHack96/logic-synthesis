...
----------------------------------------------------------------------------*/
xln_reduce_levels(net, init_param
)
network_t        **net;
...
/*-------------------------------------------------------------*/
if (changed_network) {
xln_network_move_fanins_for_delay(network, init_param,
0);
}
}
/* try a cofactor based approach - new network is good if number of levels less
   or levels same, but nodes less                                            */
/*---------------------------------------------------------------------------*/
...
(void)
delay_latest_output(network,
&max_level);
(void)
delay_latest_output(network1,
&max_level1);
...
-----------------------------------------------------------------------------*/
int
xln_one_pass_by_levels(network, threshold, init_param)
        network_t *network;
        double threshold;
        xln_init_param_t *init_param;
{
...
int
xln_one_pass_topol(network, threshold, init_param)
        network_t *network;
        double threshold;
        xln_init_param_t *init_param;
{
        ...
        xln_init_param_t *init_param;
{
    ...
    foreach_fanout(node, gen, fanout)
    {
        ...
        /* if (!xln_is_node_critical(fanout, threshold)) continue; */ /*commented  July 30 */
            ...
            if (init_param->heuristic == 2) {
                ...
                if ((num_comp_fanin <= size) ||
                    ((num_comp_fanin == size + 1) && (size != 2) &&
                     (flag1    = xln_is_just_one_fanin_critical(fanout, comp_fanin,
                                                                size, &fanin)))
                        ) {
                    array_insert_last(node_t * , fanoutvec, fanout);
                    if (flag1) {
    ...
    for (j = 0; j < array_n(fanoutvec); j++) {
        ...
        if (XLN_DEBUG)
            (void) printf("--collapsing %s into %s\n",
                          node_long_name(node), node_long_name(fanout));
        flag = 1;
    }
    ...
    if (init_param->heuristic == 2) return flag;
    ...
    if (init_param->xln_move_struct.MOVE_FANINS) {
        if (xln_node_move_fanins_for_delay(node, init_param->support,
...
-----------------------------------------------------------------------------*/
xln_is_just_one_fanin_critical(fanout, comp_fanin, size, fanin
)
node_t  *fanout;
array_t *comp_fanin;
int     size;
...
*
fanin              = node;
if (num_level_fanout_minus_2 > 1) return 0;
} else {
if (level_node == level_fanout_minus_1) return 0;
}
}
return 1;
}
/*------------------------------------------------------
  ...
  node is critical.
-------------------------------------------------------*/
xln_is_node_critical(node, threshold
)
node_t *node;
double threshold;
{
...
assert(slack
.rise == slack.fall);
if (slack.rise <= threshold) return 1;
return 0;
}
...
if (
node_get_fanin_index(n1, fanin
) == (-1)) num_composite_fanin++;
}
if ((is_n2_fanin_of_n1 >= 0) || (is_n1_fanin_of_n2 >= 0)) num_composite_fanin--;
return
num_composite_fanin;
}

array_t *
xln_composite_fanin(n1, n2)
        node_t *n1, *n2;
{
    array_t *faninvec;
    node_t  *fanin;
    ...
    {
        if (fanin != n2) array_insert_last(node_t * , faninvec, fanin);
    }
    foreach_fanin(n2, i, fanin)
    {
...
assert (st_lookup(table,(char *)

fanout, (char **) &crit_fanin));
node_algebraic_cofactor(fanout, crit_fanin,
&pos, &neg, &rem);
p           = node_or(pos, rem);
q           = node_or(neg, rem);
...
-----------------------------------------------------------------------------*/
int
xln_try_collapsing_node(node, fanoutvec, init_param)
        node_t *node;
        array_t *fanoutvec;
        xln_init_param_t *init_param;
{
    ...
    for (i  = 0; i < array_n(fanoutvec); i++) {
        ...
        if (diff <= 0) {
            ...
            if (init_param->xln_move_struct.MOVE_FANINS) {
                if (diff ==
                    xln_node_move_fanins_for_delay(fanout, init_param->support,
                                                   init_param->xln_move_struct.MAX_FANINS,
                                                   diff,
                                                   init_param->xln_move_struct.bound_alphas)
                        ) {
...
node = array_fetch(node_t * , nodevec, i);
if (
xln_cofactor_decomp_node(node, support, AREA)) {
changed = 1;
}
}
array_free(nodevec);
...
node = array_fetch(node_t * , levelvec, j);
if (
xln_cofactor_decomp_node(node, support, DELAY)) {
changed = 1;
}
}
array_free(levelvec);
...
------------------------------------------------------------------------*/
array_t *
xln_array_of_critical_nodes_at_levels(network, threshold)
        network_t *network;
        double threshold;
{
    double       max_level;
...
xln_level_width_compare_function(obj1, obj2
)
array_t **obj1, **obj2;
{
return (
