 ...
 *
 * $Source: /users/pchong/CVS/sis/sis/pld/xln_new_part.c,v $
 * $Author: pchong $
 * $Revision: 1.1.1.1 $
...
imp_part_network(network, size, MOVE_FANINS, MAX_FANINS) network_t *network;
int size;
int MOVE_FANINS;
int MAX_FANINS;
{
  ...
  while (changed) {
    changed = 0;
    if (xln_do_trivial_collapse_network(network, size, MOVE_FANINS, MAX_FANINS))
      changed = 1;
    ...
    foreach_node(network, gen, node) {
      if ((node_function(node) == NODE_PI) || (node_function(node) == NODE_PO))
        continue;
      st_insert(table, (char *)node, (char *)l);
      l++;
      foreach_fanin(node, i, fi) {
        if (node_function(fi) == NODE_PI)
          continue;
        if (composite_fanin(node, fi) <= size) {
          sol = ALLOC(sol_t, 1);
          sol->in = fi;
          sol->out = node;
          sol->value = 0;
          foreach_fanout(fi, gen1, fo) { sol->value += cost_fanin(fo, fi); }
          (void)array_insert_last(sol_t *, values, sol);
        }
      }
    }
    ...
    for (i = 0; i < array_n(cover); i++) {
      sol = array_fetch(sol_t *, cover, i);
      fi = sol->in;
      node = sol->out;
      if (composite_fanin(node, fi) <= size) {
        if (node_collapse(node, fi)) {
          if (XLN_DEBUG > 1) {
            (void)fprintf(sisout, "collapsed %s into %s\n", node_long_name(fi),
                          node_long_name(node));
          }
        } else {
...
array_t *array_disjoint(a, s, t) array_t *a;
int s;
st_table *t;
{
  int i, *n_out;
...
int xln_comp(o1, o2) sol_t **o1, **o2;
{
  sol_t *sol1, *sol2;
  sol1 = *o1;
  sol2 = *o2;
...
-------------------------------------------------------------------------------*/
int xln_do_trivial_collapse_network(network, size, MOVE_FANINS,
                                    MAX_FANINS) network_t *network;
int size;
...
int MAX_FANINS;
{
  ...
  changed = 1;
  while (changed) {
    num_collapsed_iter = xln_do_trivial_collapse_network_one_iter(
        network, size, MOVE_FANINS, MAX_FANINS);
...
int xln_do_trivial_collapse_network_one_iter(network, size, MOVE_FANINS,
                                             MAX_FANINS) network_t *network;
int size;
int MOVE_FANINS; /* if want to move fanins */
int MAX_FANINS;
...
-------------------------------------------------------------------------------*/
int xln_do_trivial_collapse_node(node, size, MOVE_FANINS,
                                 MAX_FANINS) node_t *node;
int size;
int MOVE_FANINS, MAX_FANINS;
{
  ...
  if (is_collapsed <= 0)
    return 0; /* either not INTERNAL, or some fo PO, or not
                 collapsible */
...
---------------------------------------------------------------------------*/
int xln_do_trivial_collapse_node_without_moving(node, size) node_t *node;
int size;
{
  int j;
  array_t *fo_array;
...
static int cost_fanin(po, fanin) node_t *po, *fanin;
{
  node_t *fi;
  int j, cost;
  ...
  foreach_fanin(po, j, fi) {
    if (node_get_fanin_index(fanin, fi) >= 0)
      cost--;
  }
  return (cost);
}

static int composite_fanin(node1, node2) node_t *node1, *node2;
{
  node_t *fi;
  int inp, j;
  ...
  foreach_fanin(node2, j, fi) {
    if (node_get_fanin_index(node1, fi) < 0)
      inp++;
  }
  return (inp);
}
