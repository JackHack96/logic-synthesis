...
/* ARGSUSED */
void p_actDup(node_t *old,
              node_t new) { /*	ACTs are NOT copied on duplication	*/
}

void p_transitiveFree(node_t *node) {
    lsGen  gen;
    node_t *fanout;
...
/* instead of node */
/*ARGSUSED */
void my_free_act(ACT_VERTEX_PTR vertex) {
    st_table *table;
    char     *arg;
...
/* ARGSUSED */
enum st_retval free_table_entry_improper(char *key, char *value, char *arg) {
...
void delete_act(act_t *vertex, st_table *table) {
    char *dummy;
