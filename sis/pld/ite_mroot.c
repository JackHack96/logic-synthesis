...
void act_ite_create_mroot_ite_network(network_t *network, act_init_param_t *init_param)
{
  ...
  for (i = 0; i < array_n(nodevec); i++) {
    ...
    act_ite_intermediate_new_make_ite(
        node, init_param); /* to skirt the problem of ite_1, ite_0 */
...
void act_ite_mroot_modify_fields_node(node_t *node)
{
  ...
  if (ite_is_buffer(ite)) {
    ...
    table_free =
        st_init_table(st_ptrcmp, st_ptrhash); /* for storing the potential
...
void act_ite_create_mroot_ite_node(node_t *node, act_init_param_t *init_param)
{
  ...
  if (node->type == PRIMARY_INPUT || node->type == PRIMARY_OUTPUT)
    return;
  /*
  if (factor_num_literal(node) > FAC_TO_SOP_RATIO_BOUND *
  node_num_literal(node)) { act_ite_new_make_ite(node, init_param); } else {
...

{
  ...
  foreach_primary_output(network, gen, po) {
    ...
    if (po_fanin->type != PRIMARY_INPUT) {
      array_insert_last(ite_vertex_ptr, multiple_fo_array,
                        ACT_ITE_ite(po_fanin));
    }
  }
