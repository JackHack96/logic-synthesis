...
); /* to skirt the problem of ite_1, ite_0 */
...
/*--------------------------------------*/
st_foreach_item(table_free, stgen,
&key, &value) {
if (
st_lookup(table, key,
&value)) continue;
...
if (fanin->type != PRIMARY_INPUT) {
ite->
IF = ACT_ITE_ite(fanin);
(void)
st_insert(table_free,
(char *) ite_if, (char *) ite_if);
} else {
(void)
st_insert(table,
(char *) ite_if, (char *) ite_if);
...
if (
ite_is_buffer(ite_if)
) {
act_ite_mroot_modify_part(ite, ite_if,
'I', table, table_free);
} else {
...
ite_then = ite->THEN;
if (
ite_is_buffer(ite_then)
) {
act_ite_mroot_modify_part(ite, ite_then,
'T', table, table_free);
} else {
...
ite_else = ite->ELSE;
if (
ite_is_buffer(ite_else)
) {
act_ite_mroot_modify_part(ite, ite_else,
'E', table, table_free);
} else {
...
(void)
st_insert(table_free,
(char *) child->THEN, (char *) child->THEN);
(void)
st_insert(table_free,
(char *) child->ELSE, (char *) child->ELSE);
(void)
st_insert(table_free,
(char *) child->IF, (char *) child->IF);
(void)
st_insert(table_free,
...
case 'T':
parent->
THEN = ACT_ITE_ite(fanin);
break;
case 'E':
parent->
ELSE = ACT_ITE_ite(fanin);
break;
}
...
(char *) ite, (char *) ite);
ite->
multiple_fo = 1;  /* to fool the mapper */
ite->
pattern_num = -1;
ite->
cost = 0;
...
ite = array_fetch(ite_vertex * , multiple_fo_array, i);
if (ite->value == 3) {
act_ite_mroot_initialize_area_ite(ite
->IF, multiple_fo_array, table);
act_ite_mroot_initialize_area_ite(ite
->THEN, multiple_fo_array, table);
act_ite_mroot_initialize_area_ite(ite
->ELSE, multiple_fo_array, table);
}
}
st_free_table(table);
}
...
if (
st_lookup(table,
(char *) vertex, (char **) &value)) {
if (vertex->multiple_fo == 0) {
array_insert_last(ite_vertex
*, multiple_fo_array, vertex);
}
...
act_ite_mroot_initialize_area_ite(vertex
->IF, multiple_fo_array, table);
act_ite_mroot_initialize_area_ite(vertex
->THEN, multiple_fo_array, table);
act_ite_mroot_initialize_area_ite(vertex
...
assert(!
st_insert(table,
(char *) vertex->IF, (char *) vertex->IF));
ite_mroot_traverse_ite(vertex
->IF, table);
}
if (!
st_is_member(table,
...
assert(!
st_insert(table,
(char *) vertex->THEN, (char *) vertex->THEN));
ite_mroot_traverse_ite(vertex
->THEN, table);
}
if (!
...
assert(!
st_insert(table,
(char *) vertex->ELSE, (char *) vertex->ELSE));
ite_mroot_traverse_ite(vertex
->ELSE, table);
}
}
