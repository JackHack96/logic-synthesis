...
void p_act_node_free(node) node_t *node;
{ p_actDestroy(&ACT_SET(node)->GLOBAL_ACT, 0); }

ACT_VERTEX_PTR
p_rootCopy(u) ACT_VERTEX_PTR u;
{
...
ACT_VERTEX_PTR
p_rootComplement(u) ACT_VERTEX_PTR u;
{
...
ACT_VERTEX_PTR
p_dagComplement(u) ACT_VERTEX_PTR u;
{
  char *dummy;
  ACT_VERTEX_PTR v;
...
ACT_VERTEX_PTR
p_act_construct(node, order_list, locality) node_t *node;
array_t *order_list;
int locality;
{
  array_t *node_vec;
...
int p_act_size(act) ACT_VERTEX_PTR act;
{
  switch (act->id) {
  case 0:
    return act->value;
