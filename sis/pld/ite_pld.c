...
void ite_assgn_num(ite_vertex *ite, int *n_ptr, int *addr_max_stored) {
...
void ite_print(ite_vertex *ite, int *addr_max_stored) {
    int found, i;
    if ((ite->print_mark) == 1)
        return;
...
void ite_print_dag(ite_vertex *ite) {
...
void ite_clear_dag(ite_vertex *ite) {
    if (ite->mark == 0)
        return;
    ite->mark       = 0;
    ite->print_mark = 0;
...
void ite_print_out(ite_vertex *ite) {
    int node_num, max_stored;
