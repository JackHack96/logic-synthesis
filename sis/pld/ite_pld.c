...
void ite_assgn_num(ite, n_ptr, addr_max_stored) ite_vertex *ite;
int *n_ptr;
int *addr_max_stored;
{
...
void ite_print(ite, addr_max_stored) int *addr_max_stored;
ite_vertex *ite;
{
  int found, i;
  if ((ite->print_mark) == 1)
  ...
  if (0 <= ite->value && ite->value <= 2) {
    ...
    printf("[%d]=[%d, %d, %d], cost = %d, pattern_num = %d\n", ite->mark,
           ite->IF->mark, ite->THEN->mark, ite->ELSE->mark, ite->cost,
           ite->pattern_num);
    (ite->print_mark) = 1;
...
void ite_print_dag(ite) ite_vertex *ite;
{
...
void ite_clear_dag(ite) ite_vertex *ite;
{
  if (ite->mark == 0)
    return;
  ite->mark = 0;
...
void ite_print_out(ite) ite_vertex *ite;
{
  int node_num, max_stored;
