...
ACT_VERTEX_PTR
actReduce(fn_graph) ACT_VERTEX_PTR fn_graph;
  ...
  for (i = num_var; i >= 0; i--) {
    ...
    for (j = 0; j < q_size; j++) {
      ...
      if ((key.low == oldkey.low) && (key.high == oldkey.high)) {
        ...
        if (i != num_var) {
          u->low = array_fetch(ACT_VERTEX_PTR, subgraph, u->low->id);
          u->high = array_fetch(ACT_VERTEX_PTR, subgraph, u->high->id);
        }
        oldkey.low = key.low;
...
void p_addLists(v) ACT_VERTEX_PTR v;
{
  int index;
...
int p_compare2key(q1, q2) Q_NODE_PTR *q1, *q2;
