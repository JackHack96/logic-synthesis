...
static act_t *urp_F(sm_matrix *F) {
    int        b_col, ex;
...
static void split_F(sm_matrix *F, int b_col, sm_matrix *Fl, sm_matrix *Fr) {
    sm_matrix  *Fhi, *Flo;
    sm_row     *F_row;
    sm_col     *pcol;
    sm_element *p;
...
void my_sm_copy_row(sm_row *F_row, sm_matrix *F, int col, sm_matrix *Fp) {
    int j;
    int row;
    ...
    for (j = 0; j < F_row->length; j++) {
        if (j != col) {
            sm_insert(F, row, j)->user_word = sm_row_find(F_row, j)->user_word;
        } else if (j == col) {
