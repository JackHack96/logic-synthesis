        ...
        for (i = 0; i < array_n(flow_array); i++) {
            if (array_fetch(int, flow_array, i) == 1) {
                node = graph2network_node(pld_nishiza_network,
                                          mf_get_node(pld_nishiza_graph, array_fetch(
                char *, from_array, i)));
...

{
    ...
    weight = ALLOC(
    int, 2 * array_n(match_sink_array));
...
/*---------------------------------------*/
for (
prow = matrix->first_row;
prow != 0;
prow = prow->next_row
) {
if (
sm_find(matrix, prow
->row_num, best_col->col_num) == 0) continue;
array_insert(sm_row
*, row_array,
array_n(row_array), prow
);
if (XLN_DEBUG)
...
if (DELETE_FLAG)
sm_delcol(matrix, next_best_col
->col_num);
...

{
    ...
    */ /* commented because want to try right now my heuristic */
    ...
    /*----------------------------------------------------------------------------*/
    row_solution_basic = sm_minimum_cover(matrix_basic, NIL(
    int), bincov_heuristic, XLN_DEBUG);
...
if (col->col_num %2 == 1) {
sm_row_insert(row_solution, col
->col_num);
}
}
return;
...
if (!best_col) return;
sm_row_insert(row_solution, best_col
->col_num);
...
sm_delcol(matrix, best_col
->col_num);
if (odd_col && (odd_col->col_num == best_col->col_num + 1)) {
sm_delcol(matrix, odd_col
->col_num);
}
sm_mat_bin_minimum_cover_nonbasic(matrix, row_solution, table
);
