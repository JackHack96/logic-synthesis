...
int com_act_ite_map(network_t **network, int argc, char **argv) {
    int              c;
    ...
    util_getopt_reset();
    while ((c = util_getopt(argc, argv,
                            "A:C:F:G:M:V:b:d:f:h:l:m:n:v:DLNUacorsuw")) != EOF) {
        switch (c) {
            case 'A':ACT_ITE_ALPHA = atoi(util_optarg);
                break;
            case 'C':init_param->COST_LIMIT = atoi(util_optarg);
                break;
            case 'D':init_param->DECOMP_METHOD = USE_FACTOR;
                break;
            case 'F':init_param->COLLAPSE_FANINS_OF_FANOUT = atoi(util_optarg);
                break;
            case 'G':ACT_ITE_GAMMA = atoi(util_optarg);
                break;
            case 'L':init_param->LAST_GASP = 1;
                break;
            case 'M':MAXOPTIMAL = atoi(util_optarg);
                break;
            case 'N':init_param->COLLAPSE_METHOD = NEW;
                break;
            case 'U':init_param->COLLAPSE_UPDATE = EXPENSIVE;
                break;
            case 'V':init_param->VAR_SELECTION_LIT = atoi(util_optarg);
                break;
            case 'a':init_param->ALTERNATE_REP = 1;
                break;
            case 'b':init_param->ITE_FANIN_LIMIT_FOR_BDD = atoi(util_optarg);
                break;
            case 'c':init_param->map_alg = 0;
                break;
            case 'd':init_param->DECOMP_FANIN = atoi(util_optarg);
                break;
            case 'f':init_param->FANIN_COLLAPSE = atoi(util_optarg);
                break;
            case 'h':init_param->HEURISTIC_NUM = atoi(util_optarg);
                if (init_param->HEURISTIC_NUM != 0) {
                    (void) printf("only -h 0 supported\n");
                    return 1;
                }
                break;
            case 'l':init_param->lit_bound = atoi(util_optarg);
                break;
            case 'm':init_param->MAP_METHOD = atoi(util_optarg);
                break;
            case 'n':init_param->NUM_ITER = atoi(util_optarg);
                break;
            case 'o':act_is_or_used = 0;
                break;
            case 'r':init_param->BREAK = 1;
                break;
            case 's':ACT_ITE_STATISTICS = 1;
                break;
            case 'u':USE_FAC_WHEN_UNATE = 0;
                break;
            case 'v':ACT_ITE_DEBUG = atoi(util_optarg);
                break;
            case 'w':UNATE_SELECT = 1;
                break;
            default:FREE(init_param);
                (void) fprintf(sisout,
                               "usage: ite_map [-o (or_gate_not_used)] [-h HEURISTIC_NUM] "
                               "[-d DECOMP_FANIN] [-c (complete_matchin)] \n");
                (void) fprintf(
                        sisout,
                        "\t[-f FANIN_COLLAPSE] [F COLLAPSE_FANINS_OF_FANOUT] [-n NUM_ITER] "
                        "[-s (STATISTICS)] [-L (last_gasp)] [-N (collapse_method NEW)]\n");
                (void) fprintf(sisout,
                               "\t[-a (use bdd as alternate representation)] [-U (make "
                               "collapse_update expensive)] [-v verbosity level]\n");
                (void) fprintf(
                        sisout, "[-m MAP_METHOD 0:old,1:new,2:with_iter3:with_just_decomp] "
                                "[-V new var selection method - lit count] HEURISTIC_NUM\t0 "
                                "=> ite, 1 => canonical ite (default 0)\n");
                (void) fprintf(sisout, "\t[-D (DECOMP_METHOD: use factored form)] [-w "
                                       "(unate_select = cover)]\n");
                return 1;
        }
    }
...
int com_act_ite_create_and_map_mroot_network(network_t **network, int argc, char **argv) {
    int              c;
    act_init_param_t *init_param;
    ...
    util_getopt_reset();
    while ((c = util_getopt(argc, argv,
                            "A:C:F:G:M:V:b:d:f:h:l:m:n:v:DLNUacorsu")) != EOF) {
        switch (c) {
            case 'A':ACT_ITE_ALPHA = atoi(util_optarg);
                break;
            case 'C':init_param->COST_LIMIT = atoi(util_optarg);
            ...
            default:FREE(init_param);
                (void) fprintf(
                        sisout,
                        "usage: ite_map_mroot [-o (or_gate_not_used)] [-h HEURISTIC_NUM] [-d "
                        "DECOMP_FANIN] [-c (complete_matchin)] \n");
                (void) fprintf(
                        sisout,
                        "\t[-f FANIN_COLLAPSE] [F COLLAPSE_FANINS_OF_FANOUT] [-n NUM_ITER] "
                        "[-s (STATISTICS)] [-L (last_gasp)] [-N (collapse_method NEW)]\n");
...
int com_act_ite_mux_network(network_t **network, int argc, char **argv) {
    ...
    util_getopt_reset();
    while ((c = util_getopt(argc, argv,
                            "A:C:F:G:M:R:V:b:d:f:h:l:m:n:v:DLNUacorsu")) != EOF) {
        switch (c) {
            case 'A':ACT_ITE_ALPHA = atoi(util_optarg);
                break;
            case 'C':init_param->COST_LIMIT = atoi(util_optarg);
            ...
            case 'N':init_param->COLLAPSE_METHOD = NEW;
                break;
            case 'R':FAC_TO_SOP_RATIO = atof(util_optarg);
                break;
            case 'U':init_param->COLLAPSE_UPDATE = EXPENSIVE;
                break;
            case 'V':init_param->VAR_SELECTION_LIT = atoi(util_optarg);
            ...
            default:FREE(init_param);
                (void) fprintf(sisout,
                               "usage: ite_mux [-o (or_gate_not_used)] [-h HEURISTIC_NUM] "
                               "[-d DECOMP_FANIN] [-c (complete_matchin)] \n");
                (void) fprintf(
                        sisout,
                        "\t[-f FANIN_COLLAPSE] [F COLLAPSE_FANINS_OF_FANOUT] [-n NUM_ITER] "
                ...
                (void) fprintf(
                        sisout,
                        "\t[-D (DECOMP_METHOD: use factored form)] [-R fac_to_sop_ratio]\n");
                return 1;
        }
    }
    /* some default values */
...
int com_act_bool_map_network(network_t **network, int argc, char **argv) {
    int c;
    int map_alg;
    int print_flag;
    ...
    while ((c     = util_getopt(argc, argv, "cop")) != EOF) {
        switch (c) {
            ...
            case 'p':print_flag = 0;
                break;
            default:
                (void) fprintf(sisout, "usage: act_bool [-c (complete_matching)] [-p (do "
                                       "not print matches)]\n");
                return 1;
        }
    }
...
void act_ite_alloc(node_t *node) {
...
void act_ite_free(node)node_t *node;
{
    ACT_ITE_COST_STRUCT *cost_node;
