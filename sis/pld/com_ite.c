...
init_param->
COLLAPSE_UPDATE = INEXPENSIVE;
init_param->
COLLAPSE_METHOD = OLD;
init_param->
DECOMP_METHOD = USE_GOOD_DECOMP;
init_param->
ALTERNATE_REP = 0; /* do not use it */
init_param->
MAP_METHOD = NEW;
init_param->
...
while ((
c = util_getopt(argc, argv, "A:C:F:G:M:V:b:d:f:h:l:m:n:v:DLNUacorsuw")
) != EOF){
switch (c){
case 'A':
...
case 'D':
init_param->
DECOMP_METHOD = USE_FACTOR;
break;
case 'F':
init_param->
COLLAPSE_FANINS_OF_FANOUT = atoi(util_optarg);
break;
case 'G':
...
case 'U':
init_param->
COLLAPSE_UPDATE = EXPENSIVE;
break;
case 'V':
init_param->
VAR_SELECTION_LIT = atoi(util_optarg);
break;
case 'a':
init_param->
ALTERNATE_REP = 1;
...
case 'b':
init_param->
ITE_FANIN_LIMIT_FOR_BDD = atoi(util_optarg);
break;
case 'c':
init_param->
map_alg = 0;
...
(void)
fprintf(sisout,
"usage: ite_map [-o (or_gate_not_used)] [-h HEURISTIC_NUM] [-d DECOMP_FANIN] [-c (complete_matchin)] \n");
(void)
fprintf(sisout,
"\t[-f FANIN_COLLAPSE] [F COLLAPSE_FANINS_OF_FANOUT] [-n NUM_ITER] [-s (STATISTICS)] [-L (last_gasp)] [-N (collapse_method NEW)]\n");
(void)
fprintf(sisout,
"\t[-a (use bdd as alternate representation)] [-U (make collapse_update expensive)] [-v verbosity level]\n");
(void)
fprintf(sisout,
"[-m MAP_METHOD 0:old,1:new,2:with_iter3:with_just_decomp] [-V new var selection method - lit count] HEURISTIC_NUM\t0 => ite, 1 => canonical ite (default 0)\n");
(void)
fprintf(sisout,
"\t[-D (DECOMP_METHOD: use factored form)] [-w (unate_select = cover)]\n");
return 1;
}
}
/* some default values */
...
while ((
c = util_getopt(argc, argv, "A:C:F:G:M:V:b:d:f:h:l:m:n:v:DLNUacorsu")
) != EOF){
switch (c){
case 'A':
ACT_ITE_ALPHA = atoi(util_optarg);
...
(void)
fprintf(sisout,
"usage: ite_map_mroot [-o (or_gate_not_used)] [-h HEURISTIC_NUM] [-d DECOMP_FANIN] [-c (complete_matchin)] \n");
(void)
fprintf(sisout,
"\t[-f FANIN_COLLAPSE] [F COLLAPSE_FANINS_OF_FANOUT] [-n NUM_ITER] [-s (STATISTICS)] [-L (last_gasp)] [-N (collapse_method NEW)]\n");
(void)
...
(void)
fprintf(sisout,
"\t[-D (DECOMP_METHOD: use factored form)] \n");
return 1;
}
}
/* some default values */
...
while ((
c = util_getopt(argc, argv, "A:C:F:G:M:R:V:b:d:f:h:l:m:n:v:DLNUacorsu")
) != EOF){
switch (c){
case 'A':
ACT_ITE_ALPHA = atoi(util_optarg);
...
(void)
fprintf(sisout,
"usage: ite_mux [-o (or_gate_not_used)] [-h HEURISTIC_NUM] [-d DECOMP_FANIN] [-c (complete_matchin)] \n");
(void)
fprintf(sisout,
"\t[-f FANIN_COLLAPSE] [F COLLAPSE_FANINS_OF_FANOUT] [-n NUM_ITER] [-s (STATISTICS)] [-L (last_gasp)] [-N (collapse_method NEW)]\n");
(void)
        ...
        char **argv;
{
    ...
    while ((c     = util_getopt(argc, argv, "cop")) != EOF) {
        switch (c) {
            ...
            case 'p':print_flag = 0;
                break;
            default:(void) fprintf(sisout, "usage: act_bool [-c (complete_matching)] [-p (do not print matches)]\n");
                return 1;
        }
    }
