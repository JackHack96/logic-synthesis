...
----------------------------------------------------------------------------*/
void xln_selective_good_decomp(network_t *network, int lit_limit, float alpha) {
    ...
    num           = array_n(nodevec);
    lit_sop_total = 0;
    foreach_node(network, gen, node) { lit_sop_total += node_num_literal(node); }
    if (lit_sop_total < lit_limit)
        return; /* small network */
    for (i = 0; i < num; i++) {
        node        = array_fetch(node_t *, nodevec, i);
        num_lit_sop = node_num_literal(node);
        num_lit_fac = factor_num_literal(node);
        if (num_lit_fac < num_lit_sop * alpha)
            decomp_good_node(network, node);
    }
    array_free(nodevec);
}

void xln_improve_network(network_t *network, xln_init_param_t *init_param) {
    array_t *nodevec;
    int     i, num;
    node_t  *node;
...
/*---------------------------------------------------------
  ...
  the new network.
----------------------------------------------------------*/
void xln_improve_node(node_t *node, xln_init_param_t *init_param) {
    node_t    *dup_node;
    network_t *network1;
...
-----------------------------------------------------------------------------*/
network_t *xln_best_script(node_t *node, xln_init_param_t *init_param) {
    ...
    if (init_param->flag_decomp_good == 1) {
        ...
        if (init_param->absorb) {
            xln_network_reduce_infeasibility_by_moving_fanins(
                    network1, init_param->support, 15);
        }
    }
    xln_network_ao_map(network1, init_param->support);
    if (XLN_DEBUG > 2)
        (void) printf("\tafter ao map %d nodes, ", network_num_internal(network1));
    ...
    if (init_param->flag_decomp_good == 2) {
        ...
        if (network_num_internal(network2) == 1) {
            ...
            if (init_param->absorb) {
                xln_network_reduce_infeasibility_by_moving_fanins(
                        network2, init_param->support, 15);
            }
    ...
    if ((init_param->support > 2) && (init_param->support <= 5)) {
        ...
        if (network_num_internal(network1) > upper_bound) {
            network_free(network1);
            network1 =
                    xln_cofactor_decomp(node, init_param->support, AREA); /* for AREA */
            if (XLN_DEBUG > 2)
    ...
    if (init_param->good_or_fast == FAST)
        return network1;
...
-------------------------------------------------------------*/
void xln_cover_or_partition(network_t *network, xln_init_param_t *init_param) {
...
void xln_network_print(network_t *network) {
    int     i, num;
    node_t  *node;
    array_t *nodevec;
...
-------------------------------------------------------------------------*/
void xln_try_other_mapping_options(node_t *node, network_t **pnetwork, xln_init_param_t *init_param) {
    ...
    if ((num1 <= 2) || (node_num_fanin(node) == num_lit))
        return;
    ...
    if (num_lit > init_param->lit_bound) {
        ...
        if (XLN_DEBUG > 2)
            (void) printf("\tdone good decomp, num nodes = %d\n",
                          network_num_internal(network4));
    }
    split_network(network4, init_param->support);
    if (XLN_DEBUG > 2)
        (void) printf("\tafter splitting %d nodes, ",
                      network_num_internal(network4));
    ...
    if (XLN_DEBUG > 2)
        (void) printf("\tafter disj decomp %d nodes, ", num3);
    ...
    if (XLN_DEBUG > 2)
        (void) printf("\tafter tech decomp %d nodes, ",
                      network_num_internal(network2));
    xln_cover_or_partition(network2, init_param);
...
------------------------------------------------------------------*/
node_t *xln_select_fanin_for_cofactor_area(node_t *node) {
    int    num_cube;
    int    *lit_count_arr;
    int    j, jx2;
    node_t *fanin;
    ...
    foreach_fanin(node, j, fanin) {
        jx2 = 2 * j;
        if ((lit_count_arr[jx2] == num_cube) ||
            lit_count_arr[jx2 + 1] == num_cube) {
            FREE(lit_count_arr);
            return fanin;
        }
    }
...
------------------------------------------------------------------*/
node_t *xln_select_fanin_for_cofactor_delay(node_t *node) {
