 ...
 *
 * $Source: /users/pchong/CVS/sis/sis/pld/act_delay.c,v $
 * $Author: pchong $
 * $Revision: 1.1.1.1 $
...
array_t *act_order_for_delay(node, cost_table) node_t *node;
st_table *cost_table;
{
  ...
  /*---------------------------------------------*/
  if (ACT_DEBUG)
    (void)printf("printing delay order_list for node %s\n",
                 node_long_name(node));
...
int arrival_compare_fn(c1, c2) COST_STRUCT **c1, **c2; /*  check this */
{
  double arr1, arr2;
