 ...
 *
 * $Source: /users/pchong/CVS/sis/sis/pld/xln_aodecomp.c,v $
 * $Author: pchong $
 * $Revision: 1.1.1.1 $
...
int pld_decomp_and_or(network_t *network, node_t *f, int size) {
    node_t *g, *t, *clit, *c;
    int    i;
    if (XLN_DEBUG) {
        node_print(sisout, f);
...
static void decomp_and(network_t *network, node_t *f, int and_limit) {
    ...
    if ((node_function(f) == NODE_0) || (node_function(f) == NODE_1)) {
        (void) fprintf(
                sisout,
                "redundancy in logic, advise u to \n minimise logic and proceed\n");
...
static void decomp_or(network_t *network, node_t *f, int or_limit) {
    ...
    for (i = array_n(leaves) - 1; i >= 0; i--) {
        ...
        switch (node_input_phase(f, node_get_fanin(f, i))) {
            ...
            case NEG_UNATE:node_replace(leaf, node_literal(fanin, 0));
                break;
            case BINATE:(void) fprintf(sisout, "BINATE \n");
...
void balanced_tree(int n, int m, node_t *(*node_func)(), node_t **root, array_t **nodes, array_t **leaves) {
    node_t  *np, *rt;
    array_t *ns, *lv;
    int     d, s;
    bool    first;
