...
void act_bool_map_network(network_t *network, int map_alg, int act_is_or_used,
                          int print_flag) {
    array_t         *nodevec;
    ...
    for (i  = 0; i < array_n(nodevec); i++) {
        ...
        if (match) {
            ...
            if ((node->type != PRIMARY_INPUT) && (node->type != PRIMARY_OUTPUT))
                if (print_flag)
                    (void) printf("no match found for node %s\n", io_name(node, 0));
        }
    }
    array_free(nodevec);
...
ACT_MATCH *act_is_act_function(node_t *f, int map_alg, act_is_or_used) {
    ...
    if (node_function(f) == NODE_0)
        return act_alloc_match(node_constant(0), node_constant(0), node_constant(0),
                               node_constant(0), node_constant(0), node_constant(0),
                               node_constant(0), node_constant(0));
    if (node_function(f) == NODE_1)
        return act_alloc_match(node_constant(1), node_constant(1), node_constant(1),
                               node_constant(1), node_constant(1), node_constant(1),
    ...
    */
    ...
    foreach_fanin(f, i, fanin) {
        ...
        node_free(fanin_lit);
        simplify_node(f_fanin_pos, SIM_METHOD_SNOCOMP, SIM_DCTYPE_NONE,
    ...
    /*-------------------------------------------------------*/
    foreach_fanin(f, i, fanin) {
        if (!st_lookup(table, (char *) fanin, (char **) &cofc))
            continue;
    ...
    for (i = 0; i < num_fanin; i++) {
        ...
        if (!st_lookup(table, (char *) faninA, (char **) &cofcA))
            continue;
        if (cofcA->pos_mux != 1)
            continue;
        cof_A = cofcA->pos;
        ...
        for (j = i + 1; j < num_fanin; j++) {
            ...
            if (!st_lookup(table, (char *) faninB, (char **) &cofcB))
                continue;
            cof_B = cofcB->pos;
            ...
            if (my_node_equal(cof_A, cof_B)) {
                ...
                if (act_is_mux_function(cof_Ac_Bc, &A0, &A1, &SA)) {
                    ...
                    /*------------------*/
                    match =
                            act_alloc_match(A0, A1, SA, node_dup(cofcA->B0),
                                            node_dup(cofcA->B1), node_dup(cofcA->SB),
                                            node_literal(faninA, 1), node_literal(faninB, 1));
                    act_bool_free_table(table);
                    return match;
                }
            }
    ...
    for (i = 0; i < num_fanin; i++) {
        ...
        for (j = i + 1; j < num_fanin; j++) {
            ...
            if (act_is_mux_function(cof_Ac_Bc, &A0, &A1, &SA)) {
                ...
                if (act_find_H(f, G, fanin_A_neg, fanin_B_neg, i, j, cof_Ac_B, cof_A_Bc,
                               &B0, &B1, &SB)) {
                    match =
                            act_alloc_match(A0, A1, SA, B0, B1, SB, fanin_A_pos, fanin_B_pos);
            ...
            node_free_list(cof_Ac_Bc, fanin_B_pos, fanin_B_neg, NIL(node_t),
                           NIL(node_t));
        }
        node_free_list(fanin_A_pos, fanin_A_neg, NIL(node_t), NIL(node_t),
                       NIL(node_t));
    }
    act_bool_free_table(table);
    return NIL(ACT_MATCH);
...
---------------------------------------------------------------------*/
void act_bool_set_cof_neg(COFC_STRUCT *cofc) {
...
void act_bool_free_table(st_table *table) {
    st_generator *stgen;
    node_t       *fanin;
    COFC_STRUCT  *cofc;
...
void act_bool_free_cofc(COFC_STRUCT *cofc) {
    node_free(cofc->pos);
    node_free(cofc->B0);
    node_free(cofc->B1);
    node_free(cofc->SB);
...
ACT_MATCH *act_alloc_match(node_t *A0, node_t *A1, node_t *SA, node_t *B0,
...
/*--------------------------------------------------------------------------------
  Returns 1 if f can be implemented as a mux. If so, it returns the
corresponding select, zero, one fanins. Else returns 0. NOTE: Assumes that the
node has been simplified.
----------------------------------------------------------------------------------*/
int act_is_mux_function(node_t *f, node_t **zero, node_t **one,
                        node_t **select) {
    ...
    switch (num_fanin) {
        case 0:
            ...
            *select = node_constant(1);
            return 1;
        case 1:fanin = node_get_fanin(f, 0);
            if (node_fn == NODE_BUF) {
                *zero   = node_constant(0);
                *one    = node_constant(1);
        ...
        case 3:
            ...
            if ((pld_num_fanin_cube(cube0, f) != 2) ||
                (pld_num_fanin_cube(cube1, f) != 2))
                return 0;
...
void act_map_mux(node_t **zero, node_t **one, node_t **select, node_t *z, node_t *o, node_t *s) {
    *zero   = node_literal(z, 1);
    *one    = node_literal(o, 1);
    *select = node_literal(s, 1);
}

void act_print_match(ACT_MATCH *match) {
...
void act_print_match_field(node_t *node, char *pin) {
    (void) printf("%s = ", pin);
    node_print_rhs(stdout, node);
    (void) printf("\n");
}

void act_free_match(ACT_MATCH *match) {
    if (match == NIL(ACT_MATCH))
        return;
...
----------------------------------------------------------------------*/
static node_t *act_form_G(node_t *A, node_t *Ac_B, node_t *a, node_t *b, node_t *a_c) {
    node_t *t1, *t2, *t3, *t4;
...
int act_find_H(node_t *f, node_t *G, node_t *Ac, node_t *Bc, int index_A, int index_B, node_t *Ac_B, node_t *A_Bc,
               node_t **pB0, node_t **pB1, node_t **pSB) {
    node_t *ac_bc, *one, *C;
    ...
    /* check H = 1 */
    /*-------------*/
    if (act_find_g_and_match(f, G, one, one, ac_bc, pB0, pB1, pSB)) {
        node_free(ac_bc);
        node_free(one);
        return 1;
    }
    foreach_fanin(f, i, C) {
        ...
        if (act_find_g_and_match(f, G, C_lit_pos, one, ac_bc, pB0, pB1, pSB)) {
        ...
        if (act_find_g_and_match(f, G, C_lit_neg, one, ac_bc, pB0, pB1, pSB)) {
...
---------------------------------------------------------------*/
int act_find_g_and_match(node_t *f, node_t *G, node_t *C, node_t *D, node_t *ac_bc, node_t **pB0, node_t **pB1,
                         node_t **pSB) {
...
void node_free_list(node_t *n1, node_t *n2, node_t *n3, node_t *n4, node_t *n5) {
    if (n1 != NIL(node_t))
        node_free(n1);
    if (n2 != NIL(node_t))
        node_free(n2);
...
/*-------------------------------------------------------------------------------
  ...
  returns 0.
--------------------------------------------------------------------------------*/
int act_is_two_input_H(node_t *node) {
    node_t *a, *b;

    if ((node_num_fanin(node) != 2) || (node_num_cube(node) != 1))
        return 0;
    a = node_get_fanin(node, 0);
    b = node_get_fanin(node, 1);
    if ((node_input_phase(node, a) == NEG_UNATE) &&
        (node_input_phase(node, b) == NEG_UNATE))
        return 0;
    return 1;
}
