...
act_bool_map_network(network, map_alg, act_is_or_used,
                     print_flag) network_t *network;
...
----------------------------------------------------------------------------*/
ACT_MATCH *act_is_act_function(f, map_alg, act_is_or_used) node_t *f;
...
---------------------------------------------------------------------*/
act_bool_set_cof_neg(cofc) COFC_STRUCT *cofc;
{
...
act_bool_free_table(table) st_table *table;
{
  st_generator *stgen;
  node_t *fanin;
  COFC_STRUCT *cofc;
...
act_bool_free_cofc(cofc) COFC_STRUCT *cofc;
{
  node_free(cofc->pos);
  node_free(cofc->B0);
  node_free(cofc->B1);
...
ACT_MATCH *act_alloc_match(A0, A1, SA, B0, B1, SB, S0, S1) node_t *A0, *A1, *SA,
    *B0, *B1, *SB, *S0, *S1;
{
  ACT_MATCH *match;
...
----------------------------------------------------------------------------------*/
int act_is_mux_function(f, zero, one, select) node_t *f, **zero, **one,
    **select;
{
...
act_map_mux(zero, one, select, z, o, s) node_t **zero, **one, **select, *z, *o,
    *s;
{
...
act_print_match(match) ACT_MATCH *match;
{
...
act_print_match_field(node, pin) node_t *node;
char *pin;
{
  (void)printf("%s = ", pin);
...
act_free_match(match) ACT_MATCH *match;
{
...
----------------------------------------------------------------------*/
static node_t *act_form_G(A, Ac_B, a, b, a_c) node_t *A, *Ac_B, *a, *b, *a_c;
{
  node_t *t1, *t2, *t3, *t4;
...
int act_find_H(f, G, Ac, Bc, index_A, index_B, Ac_B, A_Bc, pB0, pB1,
               pSB) node_t *f,
    *G, *Ac, *Bc, *Ac_B, *A_Bc;
int index_A, index_B;
node_t **pB0, **pB1, **pSB;
...
---------------------------------------------------------------*/
int act_find_g_and_match(f, G, C, D, ac_bc, pB0, pB1, pSB) node_t *f, *G, *C,
    *D, *ac_bc, **pB0, **pB1, **pSB;
{
...
node_free_list(n1, n2, n3, n4, n5) node_t *n1, *n2, *n3, *n4, *n5;
{
  if (n1 != NIL(node_t))
    node_free(n1);
  if (n2 != NIL(node_t))
...
/*-------------------------------------------------------------------------------
  ...
  returns 0.
--------------------------------------------------------------------------------*/
int act_is_two_input_H(node) node_t *node;
{
  node_t *a, *b;
