...
------------------------------------------------------------------------*/
pld_replace_node_by_network(node, network1) node_t *node;
network_t *network1;
{
...
------------------------------------------------------------------*/
node_t *pld_remap_get_node(node1, table1) node_t *node1;
st_table *table1;
{
...
--------------------------------------------------------------------*/
pld_remap_init_corr(table1, network, network1) st_table *table1;
...
----------------------------------------------------------------------------*/
array_t *pld_nodes_from_cubes(node) node_t *node;
{
  array_t *nodevec;
  int i;
  node_cube_t cube;
...
------------------------------------------------------------------*/
node_t *pld_make_node_from_cube(node, cube) node_t *node;
node_cube_t cube;
{
...
------------------------------------------------------------------*/
array_t *pld_cubes_of_node(node) node_t *node;
{
  array_t *cubevec;
  int i;
  node_cube_t cube;
...
/*-------------------------------------------------------------------------
  ...
  array common.
--------------------------------------------------------------------------*/
array_t *pld_get_non_common_fanins(node, common) node_t *node;
array_t *common;
{
  array_t *nc;
  node_t *fanin;
...
int pld_is_node_in_array(node, vec) node_t *node;
array_t *vec;
{
  int i;
  node_t *n;
...
int pld_num_fanin_cube(cube, node) node_cube_t cube;
node_t *node;
{
  int i;
  int num_fanin;
...
/*--------------------------------------------------------------
  ...
  n2. Else 0.
---------------------------------------------------------------*/
int pld_is_fanin_subset(n1, n2) node_t *n1, *n2;
{
  int j;
  node_t *fanin;
...
array_t *pld_get_array_of_fanins(node) node_t *node;
{
  int i;
  node_t *fanin;
  array_t *faninvec;
...
pld_replace_node_with_array(node, nodevec) node_t *node;
array_t *nodevec;
{
  node_t *n;
  int i;
...
pld_simplify_network_without_dc(network) network_t *network;
{
  lsGen gen;
  node_t *node;
...
int xln_is_network_feasible(network, support) network_t *network;
int support;
{
  lsGen gen;
  node_t *node;
...
int my_node_equal(node1, node2) node_t *node1, *node2;
{
...
st_table *pld_insert_intermediate_nodes_in_table(network) network_t *network;
{
  st_table *table;
  node_t *node;
  lsGen gen;
...
------------------------------------------*/
pld_delete_array_nodes_from_table(table, array) st_table *table;
array_t *array;
{
  int i;
  node_t *node;
...
sm_delete_rows_covered_by_col(matrix, col) sm_matrix *matrix;
sm_col *col;
{
  array_t *row_array;
...
sm_delete_rows_in_array(matrix, row_array) sm_matrix *matrix;
array_t *row_array;
{
  int i;
  sm_row *row;
...
sm_delete_cols_covered_by_row(matrix, row) sm_matrix *matrix;
sm_row *row;
{
  array_t *col_array;
...
sm_delete_cols_in_array(matrix, col_array) sm_matrix *matrix;
array_t *col_array;
{
  int i;
  sm_col *col;
...
array_t *sm_get_rows_covered_by_col(matrix, col) sm_matrix *matrix;
sm_col *col;
{
  array_t *row_array;
  sm_element *p;
...
array_t *sm_get_cols_covered_by_row(matrix, row) sm_matrix *matrix;
sm_row *row;
{
  array_t *col_array;
  sm_element *p;
