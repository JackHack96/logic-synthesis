 ...
 *
 * $Source: /users/pchong/CVS/sis/sis/pld/pld_util.c,v $
 * $Author: pchong $
 * $Revision: 1.1.1.1 $
...
------------------------------------------------------------------------*/
void pld_replace_node_by_network(node_t *node, network_t *network1) {
...
------------------------------------------------------------------*/
node_t *pld_remap_get_node(node_t *node1, st_table *table1) {
        ...
        foreach_fanin(node1, j, fanin1) {
            ...
            switch (literal1) {
                case ONE: /* +ve phase */
                    ...
                    cube = cube2;
                    break;
                case ZERO:fanin_lit = node_literal(fanin, 0);
                    cube2           = node_and(cube, fanin_lit);
                    node_free(fanin_lit);
                    node_free(cube);
                    cube = cube2;
                    break;
                case TWO:break;
                default:(void) printf("error in the cube\n");
                    exit(1);
            }
        }
...
--------------------------------------------------------------------*/
void pld_remap_init_corr(st_table *table1, network_t *network, network_t *network1) {
    lsGen  gen;
    node_t *pi1, *node;
...
----------------------------------------------------------------------------*/
array_t *pld_nodes_from_cubes(node_t *node) {
    array_t     *nodevec;
    int         i;
    node_cube_t cube;
    node_t      *node_cube;
...
------------------------------------------------------------------*/
node_t *pld_make_node_from_cube(node_t *node, node_cube_t cube) {
    ...
    foreach_fanin(node, j, fanin) {
        ...
        switch (literal) {
            case ONE:n1 = node_literal(fanin, 1);
                n2      = node_and(n, n1);
                node_free(n);
                node_free(n1);
                n = n2;
                break;
            case ZERO:n1 = node_literal(fanin, 0);
                n2       = node_and(n, n1);
                node_free(n);
                node_free(n1);
                n = n2;
...
------------------------------------------------------------------*/
array_t *pld_cubes_of_node(node_t *node) {
    array_t     *cubevec;
    int         i;
    node_cube_t cube;
...
/*-------------------------------------------------------------------------
  ...
  array common.
--------------------------------------------------------------------------*/
array_t *pld_get_non_common_fanins(node_t *node, array_t *common) {
    array_t *nc;
    node_t  *fanin;
    int     i;
    ...
    foreach_fanin(node, i, fanin) {
        if (pld_is_node_in_array(fanin, common))
            continue;
...
int pld_is_node_in_array(node_t *node, array_t *vec) {
    int    i;
    node_t *n;
...
int pld_num_fanin_cube(node_cube_t cube, node_t *node) {
    int            i;
    int            num_fanin;
    int            count;
    node_literal_t literal;
...
/*--------------------------------------------------------------
  ...
  n2. Else 0.
---------------------------------------------------------------*/
int pld_is_fanin_subset(node_t *n1, node_t *n2) {
    int    j;
    node_t *fanin;

    foreach_fanin(n1, j, fanin) {
        if (node_get_fanin_index(n2, fanin) >= 0)
            continue;
        return 0;
    }
    return 1;
...
array_t *pld_get_array_of_fanins(node_t *node) {
    int     i;
    node_t  *fanin;
    array_t *faninvec;
...
void pld_replace_node_with_array(node_t *node, array_t *nodevec) {
    node_t *n;
    int    i;
...
void pld_simplify_network_without_dc(network_t *network) {
    lsGen  gen;
    node_t *node;
...
int xln_is_network_feasible(network_t *network, int support) {
    lsGen  gen;
    node_t *node;
...
int my_node_equal(node_t *node1, node_t *node2) {
    ...
    if ((fn1 == NODE_0) && (fn2 == NODE_0))
        return 1;
    if ((fn1 == NODE_1) && (fn2 == NODE_1))
        return 1;
    return node_equal(node1, node2);
}

st_table *pld_insert_intermediate_nodes_in_table(network_t *network) {
    st_table *table;
    node_t   *node;
    lsGen    gen;
...
------------------------------------------*/
void pld_delete_array_nodes_from_table(st_table *table, array_t *array) {
    int    i;
    node_t *node;
    char   *dummy;
...
void sm_delete_rows_covered_by_col(sm_matrix *matrix, sm_col *col) {
    array_t *row_array;
...
void sm_delete_rows_in_array(sm_matrix *matrix, array_t *row_array) {
    int    i;
    sm_row *row;
...
void sm_delete_cols_covered_by_row(sm_matrix *matrix, sm_row *row) {
    array_t *col_array;
...
void sm_delete_cols_in_array(sm_matrix *matrix, array_t *col_array) {
    int    i;
    sm_col *col;
...
array_t *sm_get_rows_covered_by_col(sm_matrix *matrix, sm_col *col) {
    array_t    *row_array;
    sm_element *p;
    sm_row     *row;
...
array_t *sm_get_cols_covered_by_row(sm_matrix *matrix, sm_row *row) {
    array_t    *col_array;
    sm_element *p;
    sm_col     *col;
