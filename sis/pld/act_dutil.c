...
void act_delay_trace_forward(network, cost_table,
                             delay_values) network_t *network;
st_table *cost_table;
array_t *delay_values;
{
  double delaynum, total_delay, fanin_delay, max_fanin_delay,
  ...
  for (i = 0; i < nsize; i++) {
    ...
    if (num_fanout == 0)
      fail("Error: act_delay_trace(): node with 0 fanout found");
...
/*--------------------------------------------------------------------------------
   ...
   of some output.
----------------------------------------------------------------------------------*/
void act_delay_trace_backward(network, cost_table,
                              delay_values) network_t *network;
st_table *cost_table;
array_t *delay_values;
{
  double delaynum, max_arrival_time;
  double max_output_required_time, required_time, min_node_required_time,
  ...
  node_function_t node_fun;
  COST_STRUCT *cost_node;
  st_table *po_table; /* to keep track of all the po's that have their required
                         times set. */
  int flag_all_set, flag_some_not_set; /* indicate if the required times of po's
                                          were set in the first pass */
  lsGen gen;
  ...
  foreach_primary_output(network, gen, po) {
    ...
    if (required_time == DELAY_NOT_SET)
      flag_some_not_set = 1;
    else {
      flag_all_set = 1;
  ...
  if (flag_all_set == 0) {
    ...
    foreach_primary_output(network, gen, po) {
      ...
      assert(!st_insert(po_table, (char *)node_long_name(po),
                        (char *)node_long_name(po)));
    }
  } else if (flag_some_not_set == 0)
    ; /* all po's required times have been set */
  else {
  ...
  for (i = 0; i < nsize; i++) {
    ...
    if (ACT_DEBUG)
      (void)printf("---- arrival time of node %s = %f\n", node_long_name(node),
                   cost_node->required_time);
  }
  array_free(nodevec);
}

double act_get_arrival_time(node, cost_table) node_t *node;
st_table *cost_table;
{
  COST_STRUCT *cost_node;
...
double act_get_required_time(node, cost_table) node_t *node;
st_table *cost_table;
{
  COST_STRUCT *cost_node;
...
******************************************************************************/
COST_STRUCT *act_set_arrival_time(node, cost_table, arrival) node_t *node;
st_table *cost_table;
double arrival;
{
  COST_STRUCT *cost_node;
  ...
  if (st_lookup(cost_table, node_long_name(node), (char **)&cost_node)) {
    ...
    if (ACT_DEBUG)
      (void)printf(
          "act_set_arrival_time():node %s found in the cost_table...\n",
          node_long_name(node));
    if (ACT_DEBUG)
      (void)printf("setting the arrival time of %s to %f..\n",
  ...
  if (ACT_DEBUG)
    (void)printf(
        "act_set_arrival_time():node %s not found in the cost_table...\n",
        node_long_name(node));
  if (ACT_DEBUG)
    (void)printf("setting the arrival time of %s to %f..\n",
                 node_long_name(node), arrival);
...
/*****************************************************************************
  ...
  Allocates storage for a node if it is not found in the cost_table.
******************************************************************************/
COST_STRUCT *act_set_required_time(node, cost_table, required) node_t *node;
st_table *cost_table;
double required;
{
  COST_STRUCT *cost_node;
  ...
  if (st_lookup(cost_table, node_long_name(node), (char **)&cost_node)) {
    ...
    if (ACT_DEBUG)
      (void)printf(
          "act_set_required_time():node %s found in the cost_table...\n",
          node_long_name(node));
    if (ACT_DEBUG)
      (void)printf("setting the required time of %s to %f..\n",
  ...
  if (ACT_DEBUG)
    (void)printf(
        "act_set_required_time():node %s not found in the cost_table...\n",
        node_long_name(node));
  if (ACT_DEBUG)
    (void)printf("setting the required time of %s to %f..\n",
                 node_long_name(node), required);
...
double act_get_bddfanout_delay(vertex, delay_values,
                               cost_table) ACT_VERTEX_PTR vertex;
array_t *delay_values;
st_table *cost_table;
{
  int num_fanout;
...
--------------------------------------------------------------*/
double act_get_node_delay_correction(node, delay_values, assumed_numfo,
                                     actual_numfo) node_t *node;
array_t *delay_values;
int assumed_numfo, actual_numfo;
{
  ...
  if ((actual_numfo == 0) || (assumed_numfo == 0))
    return ((double)0.0);
...
------------------------------------------------------------------*/
void act_set_pi_arrival_time_network(network, cost_table) network_t *network;
st_table *cost_table;
{
  node_t *pi;
  lsGen gen;
...
/*-----------------------------------------------------------------
  ...
  entry cost_node.
------------------------------------------------------------------*/
COST_STRUCT *act_set_pi_arrival_time_node(pi, cost_table) node_t *pi;
st_table *cost_table;
{
  double arrival_time;

  arrival_time = delay_get_parameter(pi, DELAY_ARRIVAL_RISE);
  if (arrival_time == DELAY_NOT_SET)
    arrival_time = (double)0.0;
  return act_set_arrival_time(pi, cost_table, arrival_time);
}

void act_invalidate_cost_and_arrival_time(cost_node) COST_STRUCT *cost_node;
{ cost_node->cost_and_arrival_time = (double)(-1.0); }

double act_cost_delay(cost_node, mode) COST_STRUCT *cost_node;
float mode;
{
  if (cost_node->cost_and_arrival_time < 0.0)
    return (double)(((double)(1.00 - mode)) * (double)cost_node->cost +
                    ((double)mode) * cost_node->arrival_time);
  else
    return cost_node->cost_and_arrival_time;
}
...
------------------------------------------------------------------------*/
double act_delay_for_fanout(delay_values, num_fanout) array_t *delay_values;
int num_fanout;
{
  ...
  if (num_fanout > (delay_size - 1)) {
    ...
    delaynum = delaynum1 +
               (delaynum1 - delaynum2) * (double)(num_fanout - delay_size1);
  } else {
...
void act_set_slack_network(network, cost_table) network_t *network;
st_table *cost_table;
{
  lsGen gen;
  node_t *node;
...
--------------------------------------------------------------------------*/
COST_STRUCT *act_set_slack_node(node, cost_table) node_t *node;
st_table *cost_table;
{
  COST_STRUCT *cost_node;
...
/*-------------------------------------------------------------------------
   ...
   returns the slack.
--------------------------------------------------------------------------*/
double act_get_slack_node(node, cost_table) node_t *node;
st_table *cost_table;
{
  COST_STRUCT *cost_node;
...
--------------------------------------------------------------------------*/
void act_find_critical_nodes(network, cost_table,
                             threshold_slack) network_t *network;
st_table *cost_table;
...
array_t *
    act_compute_area_delay_weight_network_for_collapse(network, cost_table,
                                                       mode) network_t *network;
st_table *cost_table;
float mode;
{
  lsGen gen;
  ...
  foreach_node(network, gen, node) {
    if (node->type != INTERNAL)
      continue; /* important probably */
    assert(st_lookup(cost_table, node_long_name(node), (char **)&cost_node));
    cost_node->area_weight =
        act_compute_area_weight_node_for_collapse(node, cost_node);
    c_node_pair_array = act_compute_area_delay_weight_node_for_collapse(
        node, cost_node, cost_table, mode);
    if ((c_node_pair_array == NIL(array_t)) ||
        (array_n(c_node_pair_array) == 0))
      ;
    else
      array_append(c_pair_array, c_node_pair_array);
...
-----------------------------------------------------------------------------*/
array_t *act_compute_area_delay_weight_node_for_collapse(node, cost_node,
                                                         cost_table,
                                                         mode) node_t *node;
COST_STRUCT *cost_node;
st_table *cost_table;
float mode;
{
  ...
  double small_arrival_at_input_of_fanout, largest_arrival_at_input_of_node,
      diff;
  COLLAPSIBLE_PAIR *c_pair;

  if (cost_node->is_critical == NO)
    return NIL(array_t);
  if (node->type != INTERNAL)
    return NIL(array_t);
  ...
  foreach_fanout(node, gen, fanout) {
    if (fanout->type == PRIMARY_OUTPUT)
      continue;
    assert(
        st_lookup(cost_table, node_long_name(fanout), (char **)&cost_fanout));
    if (cost_fanout->is_critical == NO)
      continue;
    /* what happens to the next routine if fanout has just 1 input? */
    /*--------------------------------------------------------------*/
    small_arrival_at_input_of_fanout =
        act_smallest_fanin_arrival_time_at_fanout_except_node(fanout, node,
                                                              cost_table);
    if (small_arrival_at_input_of_fanout >= (double)MAXINT)
      continue; /* is this OK */
    largest_arrival_at_input_of_node =
    ...
    /* assign a collapsible_pair to (node, fanout) pair, assign storage for
     * their names */
    /*----------------------------------------------------------------------------------*/
    ...
    c_pair->weight = ((double)(cost_node->area_weight * (1.00 - mode))) +
                     diff * (mode); /* is this OK ? */
...
-----------------------------------------------------------------*/
double act_compute_area_weight_node_for_collapse(node, cost_node) node_t *node;
COST_STRUCT *cost_node;
{
  int num_fanout;
...
double act_smallest_fanin_arrival_time_at_fanout_except_node(
    fanout, node, cost_table) node_t *fanout,
    *node;
st_table *cost_table;
{
  double act_min;
  ...
  foreach_fanin(fanout, i, fanin) {
    ...
    if (act_min > cost_fanin->arrival_time)
      act_min = cost_fanin->arrival_time;
  }
  return act_min;
}

double act_largest_fanin_arrival_time_at_node(node, cost_table) node_t *node;
st_table *cost_table;
{
  node_t *fanin;
  double act_max;
  ...
  foreach_fanin(node, i, fanin) {
    assert(st_lookup(cost_table, node_long_name(fanin), (char **)&cost_fanin));
    if (act_max < cost_fanin->arrival_time)
      act_max = cost_fanin->arrival_time;
  }
  return act_max;
}
...
--------------------------------------------------------------------*/
int c_pair_compare_function(cp1, cp2) COLLAPSIBLE_PAIR **cp1, **cp2;
{
...
------------------------------------------------------------*/
double act_get_max_arrival_time(network, cost_table) network_t *network;
st_table *cost_table;
{
  lsGen gen;
  node_t *po;
  ...
  foreach_primary_output(network, gen, po) {
    ...
    if (max_arrival_time < arrival_time)
...
------------------------------------------------------------------------*/
st_table *act_assign_topol_indices_network(network) network_t *network;
{
  array_t *nodevec;
  int size, i;
  node_t *node;
...
-----------------------------------------------------------------------*/
array_t *act_topol_sort_fanins(node, topol_table, network) node_t *node;
st_table *topol_table;
network_t *network;
{
  int i, size;
  ...
  foreach_fanin(node, i, fanin) {
    assert(
        st_lookup(topol_table, node_long_name(fanin), (char **)&topol_fanin));
...
--------------------------------------------------------------------*/
int act_topol_compare_function(ts1, ts2) TOPOL_STRUCT **ts1, **ts2;
{
...
void act_delete_topol_table_entries(topol_table) st_table *topol_table;
{ st_foreach(topol_table, act_delete_topol_table_entry, NIL(char)); }

enum st_retval act_delete_topol_table_entry(key, value, arg) char *key, *value,
    *arg;
{
...
free_topol_struct(topol_node) TOPOL_STRUCT *topol_node;
{
