...
void act_delay_trace_forward(network_t *network, st_table *cost_table,
...
/*--------------------------------------------------------------------------------
   ...
   of some output.
----------------------------------------------------------------------------------*/
void act_delay_trace_backward(network_t *network, st_table *cost_table,
                              array_t *delay_values) {
...
double act_get_arrival_time(node_t *node, st_table *cost_table) {
    COST_STRUCT *cost_node;
...
double act_get_required_time(node_t *node, st_table *cost_table) {
    COST_STRUCT *cost_node;
...
******************************************************************************/
COST_STRUCT *act_set_arrival_time(node_t *node, st_table *cost_table,
                                  double arrival) {
    COST_STRUCT *cost_node;
    char        *name;
...
/*****************************************************************************
  ...
  Allocates storage for a node if it is not found in the cost_table.
******************************************************************************/
COST_STRUCT *act_set_required_time(node_t *node, st_table *cost_table,
                                   double required) {
    COST_STRUCT *cost_node;
    char        *name;
...
double act_get_bddfanout_delay(ACT_VERTEX_PTR vertex, array_t *delay_values,
                               st_table *cost_table) {
    int    num_fanout;
    double delaynum;
...
--------------------------------------------------------------*/
double act_get_node_delay_correction(node_t *node, array_t *delay_values,
                                     int assumed_numfo, int actual_numfo) {
...
------------------------------------------------------------------*/
void act_set_pi_arrival_time_network(network_t *network, st_table *cost_table) {
    node_t *pi;
    lsGen  gen;
...
/*-----------------------------------------------------------------
  ...
  entry cost_node.
------------------------------------------------------------------*/
COST_STRUCT *act_set_pi_arrival_time_node(node_t *pi, st_table *cost_table) {
    double arrival_time;
...
void act_invalidate_cost_and_arrival_time(COST_STRUCT *cost_node) {
    cost_node->cost_and_arrival_time = (double) (-1.0);
}
...
------------------------------------------------------------------------*/
double act_delay_for_fanout(array_t *delay_values, int num_fanout) {
...
void act_set_slack_network(network_t *network, st_table *cost_table) {
    lsGen  gen;
    node_t *node;
...
--------------------------------------------------------------------------*/
COST_STRUCT *act_set_slack_node(node_t *node, st_table *cost_table) {
    COST_STRUCT *cost_node;
...
/*-------------------------------------------------------------------------
   ...
   returns the slack.
--------------------------------------------------------------------------*/
double act_get_slack_node(node_t *node, st_table *cost_table) {
    COST_STRUCT *cost_node;
...
--------------------------------------------------------------------------*/
void act_find_critical_nodes(network_t *network, st_table *cost_table,
...
array_t *act_compute_area_delay_weight_network_for_collapse(
        network_t *network, st_table *cost_table, float mode) {
    lsGen       gen;
    node_t      *node;
    COST_STRUCT *cost_node;
...
-----------------------------------------------------------------------------*/
array_t *act_compute_area_delay_weight_node_for_collapse(node_t *node,
                                                         COST_STRUCT *cost_node,
                                                         st_table *cost_table,
                                                         float mode) {
    node_t           *fanout;
...
-----------------------------------------------------------------*/
double act_compute_area_weight_node_for_collapse(node_t *node,
                                                 COST_STRUCT *cost_node) {
    int num_fanout;
...
double act_smallest_fanin_arrival_time_at_fanout_except_node(
        node_t *fanout, node_t *node, st_table *cost_table) {
    double      act_min;
    node_t      *fanin;
    int         i;
...
double act_largest_fanin_arrival_time_at_node(node_t *node,
                                              st_table *cost_table) {
    node_t      *fanin;
    double      act_max;
    int         i;
...
--------------------------------------------------------------------*/
int c_pair_compare_function(COLLAPSIBLE_PAIR **cp1, COLLAPSIBLE_PAIR **cp2) {
...
------------------------------------------------------------*/
double act_get_max_arrival_time(network_t *network, st_table *cost_table) {
    lsGen  gen;
    node_t *po;
...
------------------------------------------------------------------------*/
st_table *act_assign_topol_indices_network(network_t *network) {
    array_t      *nodevec;
    int          size, i;
    node_t       *node;
...
-----------------------------------------------------------------------*/
array_t *act_topol_sort_fanins(node_t *node, st_table *topol_table,
                               network_t *network) {
    int          i, size;
    node_t       *fanin;
...
--------------------------------------------------------------------*/
int act_topol_compare_function(TOPOL_STRUCT **ts1, TOPOL_STRUCT **ts2) {
...
void act_delete_topol_table_entries(st_table *topol_table) {
    st_foreach(topol_table, act_delete_topol_table_entry, NIL(char));
}

enum st_retval act_delete_topol_table_entry(char *key, char *value, char *arg) {
    free_topol_struct((TOPOL_STRUCT *) value);
    return ST_DELETE;
}

void free_topol_struct(TOPOL_STRUCT *topol_node) {
