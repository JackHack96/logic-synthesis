 ...
 *
 * $Source: /users/pchong/CVS/sis/sis/pld/act_util.c,v $
 * $Author: pchong $
 * $Revision: 1.1.1.1 $
...
---------------------------------------------------------------------------*/
int is_fanin_of(node_t *node1, node_t *node2) {
    int    i, result;
    node_t *fanin;
...
COST_STRUCT *
act_allocate_cost_node(node_t *node, int cost, double arrival_time, double required_time, double slack, int is_critical,
                       double area_weight, double cost_and_arrival_time, ACT_VERTEX_PTR act) {
...
void act_final_check_network(network_t *network, st_table *cost_table) {
    lsGen    gen;
    node_t   *node;
    st_table *table;
...
void act_final_check_node(node_t *node, st_table *cost_table, st_table *table) {
    COST_STRUCT *cost_node;
    ...
    if (ACT_DEBUG)
        (void) printf("traversing node %s, cost = %d, pattern_num = %d\n",
                      node_long_name(node), cost_node->cost,
                      cost_node->act->pattern_num);
    my_traverse_act(cost_node->act);
    act_check(cost_node->act);
    delete_act(cost_node->act, table);
    /* act_traverse_not_initialize_act(cost_node->act);
       my_traverse_act(cost_node->act); */
}

/* ARGSUSED */
enum st_retval act_free_check_table(char *key, char *value, char *arg) { return ST_DELETE; }

ACT_VERTEX_PTR act_allocate_act_vertex(int mark_value) {
    ACT_VERTEX_PTR act;
...
void act_check(ACT_VERTEX_PTR vertex) {
    st_table *table;
    enum st_retval act_free_check_table();
...
-------------------------------------------------------------*/
void act_check_vertex(ACT_VERTEX_PTR vertex, st_table *table) {
    ...
    /* if (ACT_DEBUG)
       ...
       vertex->value); */
    assert(vertex->value == 0 || vertex->value == 1 || vertex->value == 4);
    if ((vertex->value != 4) && (vertex->low != NIL(ACT_VERTEX))) fail(" vertex low not nil");
    if ((vertex->value != 4) && (vertex->high != NIL(ACT_VERTEX))) fail(" vertex high not nil");
    if ((vertex->value == 4) && (vertex->low == NIL(ACT_VERTEX))) fail(" vertex low nil");
    if ((vertex->value == 4) && (vertex->high == NIL(ACT_VERTEX))) fail(" vertex high nil");
    if (vertex->value == 4) {
        act_check_vertex(vertex->low, table);
        act_check_vertex(vertex->high, table);
    }
}
...
/*-----------------------------------------------------
  ...
  vertex_name_array.
------------------------------------------------------*/
void act_change_vertex_child(ACT_VERTEX_PTR vertex, ACT_VERTEX_PTR child, int flag, node_t *node) {
    node_t         *fanin;
    ...
    if (child->low->value == 0 && child->high->value == 1) {
        ...
        newchild = act_allocate_act_vertex(MARK_COMPLEMENT_VALUE);
        newchild->value = 4;
        newchild->name  = child->name; /* right? */
        if (flag == LOW)
            vertex->low = newchild;
        else
            vertex->high = newchild;
    ...
    array_insert_last(VERTEX_NODE *, vertex_name_array, vertex_name_struct);
}

VERTEX_NODE *act_allocate_vertex_node_struct(ACT_VERTEX_PTR vertex, node_t *node) {
...
-----------------------------------------------------------*/
void act_add_terminal_vertices(ACT_VERTEX_PTR vertex) {
...
/*--------------------------------------------------------------------------------
  Puts the correct node names in the act vertices after partial collapsing of
the node. Also updates fields of the cost_struct. Note that cost_struct
corresponds to the node of the network.
----------------------------------------------------------------------------------*/
void act_partial_collapse_update_act_fields(network_t *network, node_t *dup_fanout, COST_STRUCT *cost_struct) {
    node_t *fanout;
...
/*------------------------------------------------------------------------
  ...
  network.
-------------------------------------------------------------------------*/
void act_partial_collapse_put_node_names_in_act(ACT_VERTEX_PTR vertex, network_t *network) {
    node_t *node;
    ...
    if (vertex->mark != vertex->low->mark)
        act_partial_collapse_put_node_names_in_act(vertex->low, network);
    if (vertex->mark != vertex->high->mark)
        act_partial_collapse_put_node_names_in_act(vertex->high, network);
}

/*--------------------------------------------------------------------------------
  Puts the correct node names in the act vertices after the remapping of the
node. Also updates fields of the cost_struct. Note that cost_struct corresponds
to the node of the network.
----------------------------------------------------------------------------------*/
void act_remap_update_act_fields(st_table *table1, network_t *network1, node_t *node, COST_STRUCT *cost_struct) {
...
/*------------------------------------------------------------------------
  ...
  vertex.
-------------------------------------------------------------------------*/
void act_remap_put_node_names_in_act(ACT_VERTEX_PTR vertex, st_table *table1, network_t *network1) {
    node_t *node1, *node;
    ...
    if (vertex->mark != vertex->low->mark)
        act_remap_put_node_names_in_act(vertex->low, table1, network1);
    if (vertex->mark != vertex->high->mark)
        act_remap_put_node_names_in_act(vertex->high, table1, network1);
}
...
void act_put_nodes(node_t *node, node_t *n, array_t *order_list) {
    int    num_fanin;
    node_t *fanin;
    int    i;
...
/**********************************************************************
   ...
   multiple_fo_array.
************************************************************************/
void act_initialize_act_area(ACT_VERTEX_PTR vertex, array_t *multiple_fo_array) {
...
void act_initialize_act_delay(ACT_VERTEX_PTR vertex) {
    /* to save repetition, mark the vertex as visited */
    /*------------------------------------------------*/
    if (vertex->mark == 0)
        vertex->mark = 1;
    ...
    vertex->cost        = 0;
    vertex->arrival_time =
            (double) 0.0; /* so that multiple_fo vertices are taken care of */
    vertex->mapped                  = 0;
    vertex->multiple_fo             = 0;
    vertex->multiple_fo_for_mapping = 0;
    ...
    if (vertex->mark != vertex->low->mark)
        act_initialize_act_delay(vertex->low);
    else {
        vertex->low->multiple_fo += 1;
        vertex->low->multiple_fo_for_mapping += 1;
    }
    ...
    if (vertex->mark != vertex->high->mark)
        act_initialize_act_delay(vertex->high);
    else {
        vertex->high->multiple_fo += 1;
        vertex->high->multiple_fo_for_mapping += 1;
    }
...
void make_multiple_fo_array_delay(ACT_VERTEX_PTR vertex, array_t *multiple_fo_array) {
    ...
    if (vertex->mark != vertex->low->mark)
        make_multiple_fo_array_delay(vertex->low, multiple_fo_array);
    if (vertex->mark != vertex->high->mark)
        make_multiple_fo_array_delay(vertex->high, multiple_fo_array);
}
...
******************************************************************************/
void my_array_insert_unique(ACT_VERTEX_PTR vertex, array_t *multiple_fo_array) {
    int            i, nsize;
    ACT_VERTEX_PTR v;
...
/******************************************************************************************
  Just prints out the BDD (OBDD) starting at the vertex. It first visits the low
child and then the right child. Marks all the nodes that are visited so that in
future, it does not traverse an already traversed (marked) node.
******************************************************************************************/
void my_traverse_act(ACT_VERTEX_PTR vertex) {
...
static act_traverse_not_initialize_act(ACT_VERTEX_PTR
vertex)
{
/* to save repetition, mark the vertex as visited */
/*------------------------------------------------*/
...
*****************************************************************************************/
void print_vertex(ACT_VERTEX_PTR vertex) {

    /* terminal vertex */
    /*-----------------*/
    if (vertex->value != 4) {
        (void) printf(
                "value = %d, id = %d, index = %d, multiple_fo_for_mapping = %d\n",
                vertex->value, vertex->id, vertex->index,
                vertex->multiple_fo_for_mapping);
    ...
    if (vertex->name != NIL(char))
        (void) printf("name = %s, index = %d, num_fanouts = %d, "
                      "multiple_fo_for_mapping = %d\n",
                      vertex->name, vertex->index, vertex->multiple_fo + 1,
                      vertex->multiple_fo_for_mapping);
    else
        (void) printf(
                "id = %d, index = %d, num_fanouts = %d, multiple_fo_for_mapping = %d\n",
