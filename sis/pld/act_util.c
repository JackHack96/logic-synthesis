...
st_table *cost_table, *table;
{
  ...
  if (ACT_DEBUG) (void) printf("traversing node %s, cost = %d, pattern_num = %d\n",
			            node_long_name(node), cost_node->cost, cost_node->act->pattern_num);
...
st_table *table;
{
  if (st_is_member(table, (char *) vertex)) return;
...
node_t *node;
{
  ...
  /*------------------------------------------------------------*/
  if (child->value == 0) {
	 if (node != NIL (node_t)) assert(node_num_fanin(node) == 0);
	 ...
	 newchild->value = 0;
	 if (flag == LOW) vertex->low = newchild;
	 else vertex->high = newchild;
	 return;
  }
  if (child->value == 1) {
	 if (node != NIL (node_t)) assert(node_num_fanin(node) == 0);
  ...
  array_t *multiple_fo_array;
{
 ...
 /*------------------------------------------------*/
  ...
  if (vertex->multiple_fo != 0) my_array_insert_unique(vertex, multiple_fo_array);
  if (vertex->mark != vertex->low->mark) make_multiple_fo_array_delay(vertex->low, multiple_fo_array);
  if (vertex->mark != vertex->high->mark) make_multiple_fo_array_delay(vertex->high, multiple_fo_array);
}
   ...
   ACT_VERTEX_PTR vertex;
{
 ...
 /*------------------------------------------------*/
  ...
  if (vertex->mark != vertex->low->mark)  act_traverse_not_initialize_act(vertex->low);
  else {
      /* important to check that it is a multiple_fo vertex, else you may be
	 adding  a vertex too many times in the multiple_fo array.*/
      /*--------------------------------------------------------------------*/
  ...
  if (vertex->mark != vertex->high->mark)  act_traverse_not_initialize_act(vertex->high);
  else {
      vertex->high->multiple_fo += 1;
      vertex->high->multiple_fo_for_mapping += 1;
