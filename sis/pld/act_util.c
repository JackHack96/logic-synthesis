...
---------------------------------------------------------------------------*/
int is_fanin_of(node1, node2) node_t *node1, *node2;
{
  int i, result;
  node_t *fanin;
...
COST_STRUCT *act_allocate_cost_node(node, cost, arrival_time, required_time,
                                    slack, is_critical, area_weight,
                                    cost_and_arrival_time, act) node_t *node;
...
act_final_check_network(network, cost_table) network_t *network;
st_table *cost_table;
{
  lsGen gen;
  node_t *node;
...
act_final_check_node(node, cost_table, table) node_t *node;
...
/* ARGSUSED */
enum st_retval act_free_check_table(key, value, arg) char *key, *value, *arg;
{ return ST_DELETE; }

ACT_VERTEX_PTR
act_allocate_act_vertex(mark_value) int mark_value;
{
  ACT_VERTEX_PTR act;
...
act_check(vertex) ACT_VERTEX_PTR vertex;
...
-------------------------------------------------------------*/
act_check_vertex(vertex, table) ACT_VERTEX_PTR vertex;
st_table *table;
{
  ...
  if ((vertex->value != 4) && (vertex->low != NIL(ACT_VERTEX)))
    fail(" vertex low not nil");
  if ((vertex->value != 4) && (vertex->high != NIL(ACT_VERTEX)))
...
/*-----------------------------------------------------
  ...
  vertex_name_array.
------------------------------------------------------*/
act_change_vertex_child(vertex, child, flag, node) ACT_VERTEX_PTR vertex, child;
int flag;
node_t *node;
{
  node_t *fanin;
...
VERTEX_NODE *act_allocate_vertex_node_struct(vertex,
                                             node) ACT_VERTEX_PTR vertex;
node_t *node;
{
...
-----------------------------------------------------------*/
act_add_terminal_vertices(vertex) ACT_VERTEX_PTR vertex;
{
...
----------------------------------------------------------------------------------*/
act_partial_collapse_update_act_fields(network, dup_fanout,
                                       cost_struct) network_t *network;
node_t *dup_fanout;
...
----------------------------------------------------------------------------------*/
act_remap_update_act_fields(table1, network1, node,
                            cost_struct) st_table *table1;
network_t *network1;
node_t *node;
COST_STRUCT *cost_struct;
...
/*------------------------------------------------------------------------
  ...
  vertex.
-------------------------------------------------------------------------*/
act_remap_put_node_names_in_act(vertex, table1, network1) ACT_VERTEX_PTR vertex;
st_table *table1;
network_t *network1;
{
  node_t *node1, *node;
...
void act_put_nodes(node, n, order_list) node_t *node;
node_t *n;
array_t *order_list;
{
  int num_fanin;
...
/**********************************************************************
   ...
   multiple_fo_array.
************************************************************************/
void act_initialize_act_area(vertex, multiple_fo_array) ACT_VERTEX_PTR vertex;
...
void act_initialize_act_delay(vertex) ACT_VERTEX_PTR vertex;
{
  /* to save repetition, mark the vertex as visited */
  /*------------------------------------------------*/
  if (vertex->mark == 0)
...
make_multiple_fo_array_delay(vertex, multiple_fo_array) ACT_VERTEX_PTR vertex;
array_t *multiple_fo_array;
{
...
******************************************************************************/
my_array_insert_unique(vertex, multiple_fo_array) ACT_VERTEX_PTR vertex;
array_t *multiple_fo_array;
{
  int i, nsize;
  ACT_VERTEX_PTR v;
...
******************************************************************************************/
void my_traverse_act(vertex) ACT_VERTEX_PTR vertex;
{
...
static act_traverse_not_initialize_act(vertex) ACT_VERTEX_PTR vertex;
{
  /* to save repetition, mark the vertex as visited */
  /*------------------------------------------------*/
  if (vertex->mark == 0)
  ...
  if (vertex->mark != vertex->low->mark)
    act_traverse_not_initialize_act(vertex->low);
  else {
    /* important to check that it is a multiple_fo vertex, else you may be
       adding  a vertex too many times in the multiple_fo array.*/
    /*--------------------------------------------------------------------*/
  ...
  if (vertex->mark != vertex->high->mark)
    act_traverse_not_initialize_act(vertex->high);
  else {
    vertex->high->multiple_fo += 1;
    vertex->high->multiple_fo_for_mapping += 1;
  }
...
*****************************************************************************************/
void print_vertex(vertex) ACT_VERTEX_PTR vertex;
{
