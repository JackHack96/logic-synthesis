...
static ite_vertex *urp_F(sm_matrix *F, node_t *node) {
    ...
    sm_matrix  *F_IF, *F_ELSE_THEN, *F_ELSE_ELSE, *F_new_IF,
               *act_sm_complement_special_case();
    sm_element *p;
    char       *b_name;
    ...
    if (b_col != -1) {
        ...
        if (!ex) {
            ...
            /* check if F_IF is of the form a' or a' b'. If so, put a + b (a) as the
               IF child, and swap the THEN and ELSE children (done slightly later) */
            /*-------------------------------------------------------------------------------*/
            if (special_case = act_mux_inputs_special_case(F_IF, &a_col_special,
                                                           &b_col_special)) {
                F_new_IF = act_sm_complement_special_case(F_IF, special_case,
                                                          a_col_special, b_col_special);
                sm_free_space(F_IF);
                F_IF = F_new_IF;
            }
...
static int good_bin_var(sm_matrix *A, int *ex) {
    int        *o, *z, i, var;
    sm_col     *pcol;
    sm_element *p;
    int        unate;
    ...
    sm_foreach_col(A, pcol) {
        sm_foreach_col_element(pcol, p) {
            switch ((int) p->user_word) {
                ...
                case 0:z[i]++;
                    break;
                default:(void) fprintf(misout, "Err in sm->user_word\n");
                    break;
            }
        }
        i++;
...
static int find_var(int *o, int *z, int size) {
...
void ite_split_F(sm_matrix *F, int b_col, sm_matrix **F_THENELSE, sm_matrix **F_THEN, sm_matrix **F_ELSE) {
    ...
    sm_foreach_col_element(pcol, p) {
        ...
        switch ((int) p->user_word) {
            case 0:my_sm_copy_row(F_row, Felse, b_col, F);
                break;
            case 1:my_sm_copy_row(F_row, Fthen, b_col, F);
                break;
            case 2:my_sm_copy_row(F_row, Fthenelse, b_col, F);
                break;
            default:(void) fprintf(misout, "error in duplicating row\n");
                break;
        }
    }
...
static void update_F(sm_matrix *F, int b_col) {
    sm_col     *p_col;
    sm_element *p;

    p_col = sm_get_col(F, b_col);
    sm_foreach_col_element(p_col, p) { p->user_word = (char *) 2; }
}
...
--------------------------------------------------------------------------------*/
int act_mux_inputs_special_case(sm_matrix *F, int *a_col, int *b_col) {
    ...
    sm_foreach_row_element(F->first_row, p) {
        switch ((int) p->user_word) {
            ...
            case 1:return 0;
            case 2:break;
            default:(void) fprintf(misout, "error in row\n");
                break;
        }
    }
    return count_neg;
...
------------------------------------------------------------------------------*/
sm_matrix *act_sm_complement_special_case(sm_matrix *F, int special_case, int a_col, int b_col) {
    sm_matrix  *not_F;
    sm_row     *r;
    sm_element *not_p;
    int        row, j;
