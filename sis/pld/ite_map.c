...
void act_ite_preprocess(network_t *network, act_init_param_t *init_param) {
    array_t *nodevec;
    int     i;
    node_t  *node;
    ...
    for (i  = 0; i < array_n(nodevec); i++) {
        ...
        if (node->type == PRIMARY_INPUT || node->type == PRIMARY_OUTPUT)
            continue;
        if (node_num_literal(node) > init_param->lit_bound)
            decomp_quick_node(network, node);
    }
    array_free(nodevec);
}
...
------------------------------------------------------------------*/
void act_ite_map_network(network_t *network, act_init_param_t *init_param) {
    array_t *nodevec;
    int     i;
    node_t  *node;
    int     total_cost = 0;
...
int act_ite_map_node(node_t *node, act_init_param_t *init_param) {
    ...
    /* note: not handling constants separately, because either remove them by
       sweep
    ...
    if (match) {
        ...
        if (node_fun == NODE_0 || node_fun == NODE_1 || node_fun == NODE_BUF) {
            cost_node->cost =
                    0; /* added Nov. 18, 92 - earlier wrong cost assignment */
        } else {
            cost_node->cost = 1;
        }
    ...
    switch (init_param->HEURISTIC_NUM) {
        ...
        case 3:
            ...
            if (node_num_fanin(node) <= init_param->ITE_FANIN_LIMIT_FOR_BDD) {
                act_of_node = my_create_act(node, init_param->mode, NIL(st_table),
                                            node->network, NIL(array_t));
            ...
            break;
        default: fail("heuristic number out of range");
            break;
    }
    return cost_node->cost;
}
...
--------------------------------------------------------------------*/
int act_ite_make_tree_and_map(node_t *node) {
...
int act_map_ite(ite_vertex_ptr vertex) {
    ...
    if ((vertex->ELSE->value == 3) && (vertex->ELSE->multiple_fo == 0) &&
        ...
        if (vertex->ELSE->ELSE->multiple_fo == 0) {
            ...
            if (vertex->ELSE->ELSE->value == 3) {
                ...
                temp_elseelse_cost =
                        velseelseif_cost + velseelsethen_cost + velseelseelse_cost;
            }
        }
    }
    ...
    if (ACT_ITE_DEBUG > 3)
        (void) printf(
                "vertex id = %d, index = %d, cost = %d, name = %s, pattern_num = %d\n",
                vertex->id, vertex->index, vertex->cost, vertex->name,
                vertex->pattern_num);
    return vertex->cost;
}
...
-------------------------------------------------------------------*/
int act_vertex_ite_complemented(ite_vertex_ptr vertex) { return ((vertex->value == 2) && (vertex->phase == 0)); }
...
/*----------------------------------------------------------------------
   ...
   multiple_fo_array.
------------------------------------------------------------------------*/
static void act_initialize_ite_area(ite_vertex_ptr vertex, array_t *multiple_fo_array) {
...
--------------------------------------------------------------------*/
node_t **act_ite_order_fanins(node_t *node) {
    node_t **order_list, *fanin;
    int    i;

    order_list = ALLOC(node_t *, node_num_fanin(node));
    foreach_fanin(node, i, fanin) { order_list[i] = fanin; }
    return order_list;
}
...
void ite_free_cost_struct(ACT_ITE_COST_STRUCT *cost_struct) {
    if (cost_struct == NIL(ACT_ITE_COST_STRUCT))
        return;
    FREE(cost_struct);
}
...
------------------------------------------------------------------------*/
int act_bdd_make_tree_and_map(node_t *node, ACT_VERTEX_PTR act_of_node) {
    int            num_acts;
    int            i;
    int            num_mux_struct;
