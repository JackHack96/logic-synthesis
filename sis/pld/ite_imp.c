...
/*
  ...
  Jan 5, 93: decomp_big_nodes accepts a decomposition even if the cost remains
  the same. however, after experimenting, it turns out that this move is not
  good. Even gives worse results. So was commented out on Jan. 6 '93.

  Jan 11 '93: In act_node_remap(), if DECOMP_FLAG is 1, number of iterations is
  made 1. Some other fields are changed as well. Also, the network after
  decomposition is a "broken" one, ie, in terms of ACT1 blocks (for almost all
  practical cases).

  Jan 13 '93: Added DECOMP_METHOD field in act_init_param: USE_GOOD_DECOMP and
  USE_FACTOR are two methods now. In principle, they are the same;
  implementations differ. USE_GOOD_DECOMP constructs a decomposed network and
  maps each node separately, whereas USE_FACTOR contructs an ite for the
  factored form of the node.
*/

/*------------------------------------------------------------------------------------
 Improve the network using repeated partial collapse and decomp till NUM_ITER
are completed, or no further improvement is obtained. Then quick-phase is
entered to determine which nodes should be realized in negative phase.
--------------------------------------------------------------------------------------*/

void act_ite_iterative_improvement(network_t *network, act_init_param_t *init_param) {
...
void ite_improve_network(network_t *network, act_init_param_t *init_param) {
    int nogain;
    int iteration_num;
    int iteration_gain;
    int collapse_gain;
...
static void ite_decomp_big_nodes(network_t *network, int *gain, act_init_param_t *init_param) {
    array_t         *nodevec;
    int             i;
    node_t          *node;
    ...
    *gain = 0;
    ...
    for (i      = 0; i < array_n(nodevec); i++) {
        ...
        if (num_fanin < init_param->DECOMP_FANIN)
            continue;
        if (ACT_ITE_SLOT(node)->cost <= 2)
            continue;

        /* if constructed optimally, do not decompose - not implemented yet */
        /*------------------------------------------------------------------*/
        num_cube = node_num_cube(node);
        if ((num_cube == 1) || (node_num_literal(node) == num_cube))
            continue;

        switch (init_param->DECOMP_METHOD) {
            case USE_GOOD_DECOMP:(*gain) += act_ite_node_remap(network, node, FLAG_DECOMP, init_param);
                break;
            case USE_FACTOR:(*gain) += act_ite_map_factored_form(node, init_param);
                break;
            default:(void) printf("unexpected DECOMP_METHOD\n");
                exit(1);
        }
    }
    if (ACT_ITE_DEBUG)
...
network_t *act_ite_network_remap(network_t *network, act_init_param_t *init_param) {
    array_t *nodevec;
    node_t  *node;
    int     i;
    int     gain;
...
--------------------------------------------------------------------------*/
void act_last_gasp(network_t *network, act_init_param_t *init_param) {
    array_t *nodevec;
    int     i;
    node_t  *node;
    int     DECOMP_FLAG = 0;
...
is 0 if it is called from act_last_gasp.
------------------------------------------------------------------------------------*/
int act_ite_node_remap(network_t *network, node_t *node, int FLAG_DECOMP, act_init_param_t *init_init_param) {
    ...
    int              cost_network1;
    st_table         *table1;
    int              num_cubes, gain, alternate_gain, num_nodes2, i, improved,
                     HEURISTIC_NUM_ORIG;
    ...
    if (FLAG_DECOMP == 0 && init_init_param->ALTERNATE_REP) {
        ...
        if (improved) {
            ...
            if (ACT_ITE_SLOT(node)->cost <= 2)
                return alternate_gain; /* can't improve any more */
            cost_node_original = ACT_ITE_SLOT(node)->cost;
        }
    }
    ...
    if (FLAG_DECOMP == 0)
        decomp_tech_network(network1, 1000, 1000); /* changed from 2, 2 */
    ...
    /* this makes sure that the nodes of network1 do not lose their ite, match or
     * network */
    /*------------------------------------------------------------------------------------*/
    init_param->BREAK = 0;
    act_ite_map_network_with_iter(network1, init_param);
    ...
    for (i     = 0; i < num_nodes2; i++) {
        ...
        if (init_init_param->MAP_METHOD == MAP_WITH_ITER ||
            init_init_param->MAP_METHOD == MAP_WITH_JUST_DECOMP) {
...
void act_ite_network_update_pi(node_t *node1_dup, node_t *node1, st_table *table1, network_t *network1) {
    ...
    for (i = 0; i < array_n(pivec); i++) {
        ...
        assert(st_lookup(table1, (char *) pi1, (char **) &pi1_dup));
        network_change_node_name(network_node1, pi,
                                 util_strsav(node_long_name(pi1_dup)));
    }
...
int ite_print_network(network_t *network, int iteration_num) {
    lsGen           gen;
    node_t          *node;
    int             TOTAL_COST;
    node_function_t node_fun;
...
/*-----------------------------------------------------------------------------------
  If using bdd now, use ite and vice versa. If cost improves, accept the
alternate representation.
------------------------------------------------------------------------------------*/
int act_ite_use_alternate_rep(node_t *node, act_init_param_t *init_param) {
    ...
    if (init_param->HEURISTIC_NUM <= 1) {
        act_of_node =
                my_create_act(node, init_param->mode, NIL(st_table), node->network,
