...
(void)
ite_print_network(network, iteration_num
);
}
}
}
...
node = array_fetch(node_t * , nodevec, i);
gain +=
act_ite_node_remap(network, node, DECOMP_FLAG, init_param
);
}
array_free(nodevec);
...
assert (strcmp(node_long_name(pi), node_long_name(pi1_dup))
...
ACT_ITE_SLOT(node1_dup)->
network = network_node1;
ACT_ITE_SLOT(node1_dup)->
cost = network_num_nonzero_cost_nodes(network_node1);
ACT_ITE_SLOT(node1)->
