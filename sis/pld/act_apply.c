...
ACT_VERTEX_PTR
apply(v1, v2, op) ACT_VERTEX_PTR v1, v2;
int op;
...
ACT_VERTEX_PTR
applyStep(v1, v2, op) ACT_VERTEX_PTR v1, v2;
int op;
  ...
  if (st_lookup(app_table, (char *)key, &dummy)) {
    ...
    if (v1->value == NO_VALUE || v2->value == NO_VALUE) {
      ...
      case (OR + P00):
        ...
        break;
      case (XOR):
        u->value = (v1->value && !(v2->value)) || (v2->value && !(v1->value));
        break;
      case (XNOR):
        u->value = (v1->value && v2->value) || (!(v2->value) && !(v1->value));
        break;
...
int two_cmp(key1, key2) char *key1, *key2;
{
...
int two_hash(key, modulus) char *key;
int modulus;
{
  TWO_KEY_PTR nkey;
  int bucket;
...
/*ARGSUSED*/
enum st_retval applyCleanUp(key, value, arg) char *key;
char *value;
char *arg;
