...
ACT_VERTEX_PTR applyStep(ACT_VERTEX_PTR v1, ACT_VERTEX_PTR v2, int op) {
    char           *dummy;
    ...
    if (st_lookup(app_table, (char *) key, &dummy)) {
        ...
        if (v1->value == NO_VALUE || v2->value == NO_VALUE) {
            ...
            u->index_size = v1->index_size;
            switch (op) {
                case (AND + P11):u->value = v1->value && v2->value;
                    break;
                ...
                case (OR + P00):u->value = !(v1->value) || !(v2->value);
                    break;
                case (XOR):u->value = (v1->value && !(v2->value)) || (v2->value && !(v1->value));
                    break;
                case (XNOR):u->value = (v1->value && v2->value) || (!(v2->value) && !(v1->value));
                    break;
                default:break;
            }
        }
...
int two_cmp(char *key1, char *key2) {
...
int two_hash(char *key, int modulus) {
    TWO_KEY_PTR nkey;
    int         bucket;
...
/*ARGSUSED*/
enum st_retval applyCleanUp(char *key, char *value, char *arg) {
    TWO_KEY_PTR nkey;
    nkey = (TWO_KEY_PTR) key;
    FREE(nkey);
    return (ST_CONTINUE);
