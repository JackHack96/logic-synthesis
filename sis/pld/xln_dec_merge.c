...
node2 = node2;
affinity_struct->
common = xln_node_find_common_inputs(node1, node2);
array_insert_last(AFFINITY_STRUCT
*, affinity_vec, affinity_struct);
}
}
...
assert(!
st_insert(table1,
(char *) node1, (char *) node1));
assert(!
st_insert(table2,
(char *) node2, (char *) node2));
array_insert_last(AFFINITY_STRUCT
*, matched_vec, affinity_struct);
}
...
xln_decomp_for_merge_pair_of_nodes(affinity_struct
->node1,
...
------------------------------------------------------------------------------*/
xln_decomp_for_merge_pair_of_nodes(n1, n2, init_param
)
node_t           *n1, *n2;
xln_init_param_t *init_param;
{
...
array_insert_last(node_t
*, nodevec, n1);
array_insert_last(node_t
*, nodevec, n2);

cube_node_vec = xln_fill_tables(cubenode2node_table, node2cubenode_table, nodevec,
                                init_param);
xln_decomp_for_merge_cube_nodes(cube_node_vec, cubenode2node_table,
                                               node2cubenode_table, init_param
);
...
array_t          *cube_node_vec;
st_table         *cubenode2node_table, *node2cubenode_table;
xln_init_param_t *init_param;
{
array_t         *affinity_vec;
int             i, j;
...
/*---------------------------------------------------------------*/
assert (st_lookup(cubenode2node_table,(char *)
...
if ((n1 == n2) &&
(
xln_num_composite_fanin(cube_node1, cube_node2
) <= init_param->support)
) continue;
...
assert (network_check(n1

->network));
assert(!
st_insert(table,
(char *) cube_node1, (char *) cube_node1));
assert(!
st_insert(table,
(char *) cube_node2, (char *) cube_node2));
...
array_t *
xln_node_find_common_inputs(node1, node2)
        node_t *node1, *node2;
{
...
node_t           *n1, *n2;
AFFINITY_STRUCT  *affinity_struct;
st_table         *table, *node2cubenode_table;
xln_init_param_t *init_param;
{
...
for (
j = 0;
(j < init_param->MAX_COMMON_FANIN && common_ptr <
array_n(common)
);
j++, common_ptr++) {
...
/*-------------------------------------------------------------*/
(void)
node_substitute(n1, sub_node1,
0);
(void)
node_substitute(n2, sub_node2,
0);
...
---------------------------------------------------------------------------*/
node_t *
xln_extract_supercube_from_cube(node, subset_common)
        node_t *node;
        array_t *subset_common;
{
...
---------------------------------------------------------------------------*/
xln_add_more_literals(psub_node, node, fanin_bound_more, nc, nc_ptr
)
node_t  **psub_node, *node;
int     fanin_bound_more;
array_t *nc;
int     nc_ptr;
{
int           final, i;
...
-----------------------------------------------------------*/
int
pld_affinity_compare_function(p1, p2)
        AFFINITY_STRUCT **p1, **p2;
{
    int num1, num2;
...
-----------------------------------------------------------------*/
array_t *
xln_infeasible_nodes(network, support)
        network_t *network;
        int support;
{
    array_t *nodevec;
    ...
    foreach_node(network, gen, node)
    {
        ...
        if (node_num_fanin(node) <= support) continue;
...
------------------------------------------------------------------------*/
array_t *
xln_fill_tables(cubenode2node_table, node2cubenode_table, nodevec, init_param)
        st_table *cubenode2node_table, *node2cubenode_table;
        array_t *nodevec;
        xln_init_param_t *init_param;
{
...
---------------------------------------------------------------------------*/
xln_add_more_cubes(psub_node, cube_node, n,
                              node2cubenode_table, table
)
node_t   **psub_node, *cube_node, *n;
st_table *node2cubenode_table, *table;
{
...
if (nin_sub_node < nin_cube_node) return;

assert(nin_sub_node
== nin_cube_node);

assert (st_lookup(node2cubenode_table,(char *)

n, (char **) &cubenode_vec));
for (
i = 0;
i <
array_n(cubenode_vec);
i++) {
...
st_foreach_item(node2cubenode_table, stgen,
&key, (char **) &cubevec) {
array_free(cubevec);
}
