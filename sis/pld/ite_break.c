...
void act_ite_break_network(network_t *network, act_init_param_t *init_param) {
    array_t *nodevec;
    int     i;
...
----------------------------------------------------------------------------*/
int act_ite_break_node(node_t *node, act_init_param_t *init_param) {
    ...
    if (init_param->MAP_METHOD == MAP_WITH_ITER ||
        init_param->MAP_METHOD == MAP_WITH_JUST_DECOMP) {
...
***************************************************************************************/
node_t *act_ite_get_function(network_t *network, node_t *node, ite_vertex_ptr vertex) {

    node_t *node1, *node2, *ELSE_THEN, *ELSE_ELSE, *ELSE_IF, *ELSE_ELSE_IF,
           *ELSE_ELSE_THEN, *ELSE_ELSE_ELSE;
    node_t *IF, *THEN, *ELSE, *IF_THEN, *IF_ELSE, *IF_IF, *THEN_THEN, *THEN_ELSE,
           *THEN_IF;
    ...
    switch (vertex->pattern_num) {
        ...
        case 0:IF = act_ite_get_function(network, node, vertex->IF);
        ...
        case 1:vertex->THEN->mark = MARK_COMPLEMENT_VALUE;
        ...
        case 2:vertex->ELSE->mark = MARK_COMPLEMENT_VALUE;
        ...
        case 3:vertex->THEN->mark = MARK_COMPLEMENT_VALUE;
            vertex->ELSE->mark    = MARK_COMPLEMENT_VALUE;
        ...
        case 4:vertex->IF->mark = MARK_COMPLEMENT_VALUE;
        ...
        case 5:vertex->IF->mark = MARK_COMPLEMENT_VALUE;
            vertex->THEN->mark  = MARK_COMPLEMENT_VALUE;
        ...
        case 6:vertex->IF->mark = MARK_COMPLEMENT_VALUE;
            vertex->ELSE->mark  = MARK_COMPLEMENT_VALUE;
        ...
        case 7:vertex->IF->mark = MARK_COMPLEMENT_VALUE;
            vertex->THEN->mark  = MARK_COMPLEMENT_VALUE;
            vertex->ELSE->mark  = MARK_COMPLEMENT_VALUE;
        ...
        case 8:vertex->ELSE->mark = MARK_COMPLEMENT_VALUE;
        ...
        case 9:vertex->ELSE->mark    = MARK_COMPLEMENT_VALUE;
            ...
            ELSE_ELSE_THEN =
                    act_ite_get_function(network, node, vertex->ELSE->ELSE->THEN);
            ELSE_ELSE_ELSE =
                    act_ite_get_function(network, node, vertex->ELSE->ELSE->ELSE);
...
void ite_set_MARK_VALUE_ite(ite_vertex *ite) {
...
---------------------------------------------------------------*/
void ite_reset_mark_ite(ite_vertex *ite) {
    st_table     *table;
    char         *key;
    st_generator *stgen;
    ite_vertex   *vertex;
    ...
    st_foreach_item(table, stgen, &key, (char **) &vertex) { vertex->mark = 0; }
    st_free_table(table);
}
...
************************************************************************************/
void ite_free_node_if_possible(node_t *node, ite_vertex *vertex) {
...
int act_new_act_break_node(node_t *node) {
    network_t           *network;
...
---------------------------------------------------------------*/
void ite_check_ite(ite_vertex *ite, node_t *node) {
    st_table     *table;
    char         *key;
    st_generator *stgen;
    ite_vertex   *vertex;
...
all these nodes in the network. Original node is changed.
***************************************************************************************/
node_t *act_new_act_get_function(network_t *network, node_t *node, ACT_VERTEX_PTR vertex) {
    ...
    switch (vertex->pattern_num) {
        ...
        case 4:vertex->low->low->mark = MARK_COMPLEMENT_VALUE;
            ...
            ELSE_ELSE_ELSE =
                    act_new_act_get_function(network, node, vertex->low->low->low);
            ELSE_ELSE_THEN =
                    act_new_act_get_function(network, node, vertex->low->low->high);
        ...
        case 5:;
        case 6:vertex->high->mark   = MARK_COMPLEMENT_VALUE;
            vertex->high->low->mark = MARK_COMPLEMENT_VALUE;

            THEN_ELSE_ELSE =
                    act_new_act_get_function(network, node, vertex->high->low->low);
            THEN_ELSE_THEN = THEN_THEN =
                    act_new_act_get_function(network, node, vertex->high->low->high);
