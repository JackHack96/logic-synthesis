...
void p_actCreate4Set(node_vec, node_list, locality, order_style, mode, network,
                     delay_values, cost_table) array_t *node_vec,
    *node_list;
...
ACT_VERTEX_PTR
p_actCreateStep(current_node, level, node_list, locality) node_t *current_node;
int level, locality;
array_t *node_list;
...
ACT_VERTEX_PTR
p_optimalDag(current_node, node_list, mode, network, delay_values,
             cost_table) node_t *current_node;
array_t *node_list;
float mode;
network_t *network;
array_t *delay_values;
...
void p_permute(current_node, list, n, best_cost_ptr, best_cost_and_delay_ptr,
               pbest_list, mode, network, delay_values,
               cost_table) node_t *current_node;
...
void p_applyCreate(node, order_list) node_t *node;
array_t *order_list;
{
...
/*ARGSUSED*/
enum st_retval p_freeTree(key, value, arg) char *key, *value, *arg;
{
  tree_t *tree_entry;
  node_t *node;
...
ACT_VERTEX_PTR
p_treeNodeDag(node, tree_table, order_list) node_t *node;
st_table *tree_table;
array_t *order_list;
{
  node_t *fanin_node;
...
ACT_VERTEX_PTR
p_terminalDag(value, index_size) int value, index_size;
{
  ACT_VERTEX_PTR u;
...
act_print_array(list) array_t *list;
{
  int i;
  node_t *n;
