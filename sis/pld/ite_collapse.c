        ...
        st_table *score_table;
{
    ...
    for (i = 0; i < num_fanout; i++) {
        ...
        ACT_DEF_ITE_SLOT(dup_fanout) = NIL(
        char);
        ...
        st_table *score_table;
{
    ...
    num_fanout   = array_n(real_fanout_array);
    fanout_array = array_alloc(node_t * , 0);
    good_array   = array_alloc(
    int, 0);
    bad_array = array_alloc(
    int, 0);
...
cost_struct->ite->
node = fanout;
act_ite_partial_collapse_put_node_names_in_ite(cost_struct
->ite, network);
assert(cost_struct
->act ==
NIL (ACT_VERTEX)
...
if (vertex->fanin !=
network_find_node(network, node_long_name(vertex
->fanin))) {
(void) printf("need partial collapse put node names\n");
}
assert(vertex
->
fanin = network_find_node(network, node_long_name(vertex->fanin))
);
vertex->
name = node_long_name(vertex->fanin);
}
st_free_table(table);
}
