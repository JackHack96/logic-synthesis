...
/*-----------------------------------------------------------------------------------
  ...
  the gain that might be obtained by collapsing the node into each of the
fanouts. This score is just based on the number of fanins, fanouts, cost of node
etc. Updates the score after each successful partial collapse. Tries to collapse
  till all nodes get score 0, or have been tried for  collapse, but
unsuccessfully.
------------------------------------------------------------------------------------*/
int act_ite_partial_collapse(network_t *network, act_init_param_t *init_param) {
    st_table *score_table;
    int      collapsed, gain, total_gain, score;
    node_t   *node, *act_ite_partial_collapse_find_max_score();

    if (!init_param->FANIN_COLLAPSE)
        return 0;

    score_table = st_init_table(strcmp, st_strhash);
    act_ite_partial_collapse_assign_score_network(network, score_table,
                                                  init_param);

    total_gain = 0;
    while (1) {
        node =
                act_ite_partial_collapse_find_max_score(network, score_table, &score);
        if (score == 0) {
...
int act_ite_partial_collapse_node(network_t *network, node_t *node,
                                  act_init_param_t *init_param, int *pgain,
                                  st_table *score_table) {
    int                 gain;
    char                *name, *value;
    ...
    foreach_fanout(node, genfo, fanout) {
        ...
        if (num_composite <= init_param->COLLAPSE_FANINS_OF_FANOUT)
            continue;
    ...
    gain      = cost_node->cost;
    foreach_fanout(node, genfo, fanout) { gain += ACT_ITE_SLOT(fanout)->cost; }
    ...
    for (i       = 0; i < num_fanout; i++) {
        ...
        simplify_node(dup_fanout, SIM_METHOD_SNOCOMP, SIM_DCTYPE_NONE,
        ...
        switch (init_param->MAP_METHOD) {
            case MAP_WITH_ITER:;
            case MAP_WITH_JUST_DECOMP:act_ite_map_node_with_iter_imp(dup_fanout, init_param);
                break;
            case NEW:(void) act_ite_new_map_node(dup_fanout, init_param);
                break;
            case OLD:(void) act_ite_map_node(dup_fanout, init_param);
                break;
            default:(void) printf("mapping method %d not known\n", init_param->MAP_METHOD);
                exit(1);
        }
        ...
        if (init_param->MAP_METHOD == NEW || init_param->MAP_METHOD == OLD) {
            act_ite_partial_collapse_update_ite_fields(network, dup_fanout,
                                                       cost_struct);
        }
        gain -= cost_struct->cost;
        if (gain <= 0)
            break; /* now you may not have to go through all the fanouts before
    ...
    if (init_param->COLLAPSE_UPDATE == EXPENSIVE) {
        ...
        st_foreach_item(update_nodetable, stgen, (char **) &fanin, &value) {
            act_ite_partial_collapse_assign_score_node(fanin, score_table,
                                                       init_param);
        }
...
/*---------------------------------------------------------------------------------------
  Is potentially more powerful than act_ite_partial_collapse_node(), in that,
even if the total collapse is not beneficial, it may happen that collapsing node
into some fanouts (those in good_array) may be benficial. In that case, this is
done. If the overall gain is not positive (because of fanouts whose cost
increases as a result of this collapse - ie in bad_array), node will not be
deleted from network.
-----------------------------------------------------------------------------------------*/

int act_ite_partial_collapse_node_new(network_t *network, node_t *node,
                                      act_init_param_t *init_param, int *pgain,
                                      st_table *score_table) {
    ...
    if (gain_bad <= 0) {
        ...
        for (j = 0; j < array_n(bad_array); j++) {
            ...
            act_free_ite_node(dup_fanout);
            node_free(
                    dup_fanout); /* act_free_ite() is called from within node_free() */
        }
    } else {
        for (j = 0; j < array_n(bad_array); j++) {
            index      = array_fetch(int, bad_array, j);
            ...
            /* updating the score of the fanout */
            /*----------------------------------*/
            act_ite_partial_collapse_assign_score_node(fanout, score_table,
                                                       init_param);
            (void) node_replace(fanout, dup_fanout);
        }
    }
    ...
    if (gain_bad > 0) {
        ...
        /*---------------------------------------------------------*/
        if (ACT_ITE_DEBUG > 1)
            (void) printf("collapsed node %s into all the fanouts\n",
                          node_long_name(node));
        act_free_ite_node(node);
        network_delete_node(network, node);
    }
    ...
    *pgain = gain_good + (gain_bad * (gain_bad > 0));
    return (
            *pgain); /* not returning 1, *pgain > 0 iff some collapsing took place */
}

int act_ite_partial_collapse_assign_score_node(node_t *node,
                                               st_table *score_table,
...
-------------------------------------------------------------*/
static node_t *act_ite_partial_collapse_find_max_score(network_t *network,
                                                       st_table *score_table,
                                                       int *pscore) {
    lsGen  gen;
...
---------------------------------------------------------------------------------*/
int act_ite_partial_collapse_assign_score_network(
        network_t *network, st_table *score_table, act_init_param_t *init_param) {
    array_t *nodevec;
...
void act_ite_partial_collapse_update_ite_fields(
        network_t *network, node_t *dup_fanout, ACT_ITE_COST_STRUCT *cost_struct) {
    node_t *fanout;

    fanout = network_find_node(network, node_long_name(dup_fanout));
    cost_struct->node = fanout;
    if (cost_struct->match)
        return;
    if (cost_struct->ite) {
...
void act_ite_partial_collapse_put_node_names_in_ite(ite_vertex *ite, network_t *network) {
    st_table     *table;
    char         *key;
    st_generator *stgen;
    ite_vertex   *vertex;
    ...
    st_foreach_item(table, stgen, &key, (char **) &vertex) {
        if (vertex->value != 2)
            continue;
        if (vertex->fanin !=
            network_find_node(network, node_long_name(vertex->fanin))) {
