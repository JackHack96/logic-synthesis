...
equivalence classes is 2, you can do it!!! g has to be a single fanout node, not
a PI.
...
/*--------------------------------------------------------------------------------
  ...
  first.
---------------------------------------------------------------------------------*/
void xln_network_move_fanins_for_delay(network_t *network, xln_init_param_t *init_param, int init_delay_trace) {
    array_t *nodevec;
    int     i;
    node_t  *node;
    int     support;
    ...
    if (init_param->xln_move_struct.MOVE_FANINS == 0)
        return;
    ...
    for (i = 0; i < array_n(nodevec); i++) {
        ...
        if (!xln_is_node_critical(node, (double) 0))
            continue;
        /* move as much as possible - takes care that a non-critical fanin does not
           become over-critical */
        /*----------------------------------------------------------------------------*/
        (void) xln_node_move_fanins_for_delay(node, support, MAX_FANINS, MAXINT,
                                              bound_alphas);
    }
    array_free(nodevec);
}

int xln_node_move_fanins_for_delay(node_t *node, int support, int MAX_FANINS, int diff, int bound_alphas) {
    int     infeasibility;
    int     improvement;
    int     i, num_fanin;
    node_t  *f;
    ...
    for (i      = 0; i < array_n(faninvec); i++) {
        ...
        if (xln_is_node_critical(f, (double) 0))
            continue;
