...
static int com_print_gcd(network_t **network, int argc, char **argv) {
    int     i;
    array_t *node_vec;
    node_t  *node;
    ...
    usage:
    (void) fprintf(miserr, "_gcd n1 n2 ...\n");

    (void) fprintf(miserr, "   where none of n1, n2,..., nn are primary inputs\n");
    return 1;
}

static int com_print_factor(network_t **network, int argc, char **argv) {
    int     i, j;
    array_t *node_vec;
    array_t *factor_vec;
    ...
    for (i = 0; i < array_n(node_vec); i++) {
        ...
        (void) fprintf(misout, "Prime factorization for node %s\n", node_name(node));
    ...
    usage:
    (void) fprintf(miserr, "_prime_factor n1 n2 ...\n");
