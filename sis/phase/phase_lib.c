...
phase_lib_setup(network) network_t *network;
{
  lib_class_t *class;
  node_t *node;
  lsGen gen;
...
void phase_invertible_set(row_data) row_data_t *row_data;
{
...
void phase_node_invert(np) node_t *np;
{
  lib_gate_t *gate, *dual_gate;
  lib_class_t *dual_class;
...
static lib_gate_t *min_area_gate(class) lib_class_t *class;
{
  lib_gate_t *gate;
  lsGen gen;
  char *dummy;
...
double phase_value(node_phase) node_phase_t *node_phase;
{
  row_data_t *row_data;
  lib_gate_t *gate, *dual_gate;
  lib_class_t *dual_class;
...
double cost_comp(net_phase) net_phase_t *net_phase;
{
  sm_row *row;
  row_data_t *rd;
  double count;
...
void phase_record(network, net_phase) network_t *network;
