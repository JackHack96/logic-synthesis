...
static void inv_save_setup(net_phase) net_phase_t *net_phase;
{
...
static int inv_save_comp(net_phase, row) net_phase_t *net_phase;
sm_row *row;
{
  register sm_col *col;
...
void phase_invert(net_phase, node_phase) net_phase_t *net_phase;
node_phase_t *node_phase;
{
...
static void phase_do_invert(net_phase, row) net_phase_t *net_phase;
sm_row *row;
{
  register sm_col *col;
...
node_phase_t *phase_get_best(net_phase) net_phase_t *net_phase;
{
...
double network_cost(net_phase) net_phase_t *net_phase;
{ return net_phase->cost; }

void phase_unmark_all(net_phase) net_phase_t *net_phase;
{
...
void phase_mark(node_phase) node_phase_t *node_phase;
{
  row_data_t *rd;
...
net_phase_t *phase_dup(net_phase) net_phase_t *net_phase;
{
...
void phase_replace(old, new) net_phase_t *old, *new;
{
  sm_matrix *matrix;
  array_t *rows;
...
static row_data_t *row_data_dup(row_data) row_data_t *row_data;
{
  row_data_t *new;
...
static element_data_t *
    element_data_dup(element_data) element_data_t *element_data;
{
  element_data_t *new;
...
void phase_free(net_phase) net_phase_t *net_phase;
{
  int i;
  node_phase_t *node_phase;
  row_data_t *rd;
...
void phase_print(net_phase) net_phase_t *net_phase;
{
  sm_row *row, *row1;
  row_data_t *rd;
  sm_element *element;
...
static void check_phase(net_phase) net_phase_t *net_phase;
{
  sm_row *row;
  row_data_t *rd;
  int save;
 ...
 */
static void phase_dec(net_phase, row) net_phase_t *net_phase;
sm_row *row;
{
  sm_row *fanout;
  sm_element *element;
 ...
 */
static void phase_inc(net_phase, row) net_phase_t *net_phase;
sm_row *row;
{
  sm_row *fanout;
  sm_element *element;
...
static bool fanout_to_po(f) node_t *f;
{
  node_t *np;
  lsGen gen;
...
void phase_random_assign(net_phase) net_phase_t *net_phase;
{
  node_phase_t *node_phase;
  row_data_t *rd;
  int i;
...
net_phase_t *phase_setup(network) network_t *network;
{
  node_t *np;
...
phase_node_setup(np, table, net_phase) node_t *np;
nodeindex_t *table;
net_phase_t *net_phase;
{
  node_function_t func;
...
phase_fanout_setup(rowi, np, table, net_phase, phase) int rowi;
node_t *np;
nodeindex_t *table;
net_phase_t *net_phase;
int phase;
...
static void phase_count_comp(np, table, net_phase) node_t *np;
nodeindex_t *table;
net_phase_t *net_phase;
{
  sm_row *row;
