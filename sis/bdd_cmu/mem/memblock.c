...
#else
    mem_copy(dest, src, size) pointer dest;
pointer src;
SIZE_T size;
#endif
{
...
#else
    mem_fatal(message) char *message;
#endif
{
...
#else
    block_size_index(size) SIZE_T size;
#endif
{
  if (size < 1)
    return (-1);
...
#else
    remove_from_free_list(b) block b;
#endif
{
  int i;
...
#endif
{
  ...
  buddy_offset = (SIZE_T)(((INT_PTR)b - (INT_PTR)b->seg->base_address) ^
                          ((SIZE_T)1 << b->size_index));
...
#else
    trim_to_size(b, size_index) block b;
int size_index;
#endif
{
  block bb;
...
#else
    merge_and_free(b) block b;
#endif
{
  block bb;

  for (bb = buddy(b); bb && !bb->used && bb->size_index == b->size_index;
       bb = buddy(b)) {
...
#else
    mem_get_block(size) SIZE_T size;
#endif
{
  int i;
  int size_index;
  ...
  if (i > MAX_SIZE_INDEX) {
    ...
    /* length. */
    alloc_size += ((curr_seg->limit + alloc_size - 1) & ~(alloc_size - 1)) -
                  curr_seg->limit;
    if ((sbrk_ret = (pointer)SBRK(0)) !=
            (pointer)((INT_PTR)curr_seg->base_address + curr_seg->limit) ||
      ...
      /* created. */
      alloc_size +=
          ((SIZE_T)1 << alloc_size_index) + ROUNDUP(sizeof(struct segment_));
      new_seg = 1;
    } else
      new_seg = 0;
    ...
    if (new_seg) {
      ...
      curr_seg->base_address =
          (pointer)((INT_PTR)curr_seg + ROUNDUP(sizeof(struct segment_)));
      curr_seg->limit = 0;
...
#else
    mem_resize_block(p, new_size) pointer p;
SIZE_T new_size;
#endif
{
  int new_size_index;
  ...
  for (bb = buddy(b); bb && (INT_PTR)b < (INT_PTR)bb && !bb->used &&
                      bb->size_index == b->size_index;
       bb = buddy(b)) {
    remove_from_free_list(bb);
    if (++(b->size_index) == new_size_index)
