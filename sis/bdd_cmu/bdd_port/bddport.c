 ...
 */
static bdd_t *bdd_construct_bdd_t(struct bdd_manager_ *mgr, struct bdd_ *fn) {
    bdd_t *result;
...
*/
void bdd_end(struct bdd_manager_ *mgr) {
...
void bdd_set_mgr_init_dflts(bdd_mgr_init *mgr_init) {
    cmu_bdd_warning("bdd_set_mgr_init_dflts: translated to no-op in CMU package");
    return;
}

struct bdd_manager_ *bdd_start(int nvariables) {
    ...
    /*
     * Calls to UCB bdd_get_variable are translated into cmu_bdd_var_with_id
     * calls.  However, cmu_bdd_var_with_id assumes that single variable BDDs have
     * already been created for all the variables that we wish to access.  Thus,
     * following, we explicitly create n variables.  We do not care about the
     * return value of cmu_bdd_new_var_last; in the CMU package, the single
     * variable BDDs are NEVER garbage collected.
     */
...
struct bdd_manager_ *bdd_start_with_params(int nvariables, bdd_mgr_init *mgr_init) {
    cmu_bdd_warning("bdd_start_with_params: translated to bdd_start(nvariables) "
                    "in CMU package");
...
bdd_t *bdd_create_variable(struct bdd_manager_ *mgr) { return bdd_construct_bdd_t(mgr, cmu_bdd_new_var_last(mgr)); }

bdd_t *bdd_create_variable_after(struct bdd_manager_ *mgr, bdd_variableId after_id) {
    ...
    /* No need to free after_var, since single variable BDDs are never garbage
     * collected */

    return result;
}

bdd_t *bdd_get_variable(struct bdd_manager_ *mgr, bdd_variableId variable_ID) {
    struct bdd_ *fn;
...
bdd_t *bdd_dup(bdd_t *f) { return bdd_construct_bdd_t(f->mgr, cmu_bdd_identity(f->mgr, f->node)); }
...
void bdd_free(bdd_t *f) {
    ...
    /*
     * In case the user tries to free this bdd_t again, set the free field to
     * TRUE, and NIL out the other fields.  Then free the bdd_t structure itself.
     */
    f->free = TRUE;
...
bdd_t *bdd_and(bdd_t *f, bdd_t *g, boolean f_phase, boolean g_phase) {
    ...
    mgr    = f->mgr;
    temp1  = ((f_phase == TRUE) ? cmu_bdd_identity(mgr, f->node)
...
bdd_t *bdd_and_smooth(bdd_t *f, bdd_t *g, array_t *smoothing_vars) {
    ...
    (void) cmu_bdd_assoc(
            mgr, -1); /* set the temp association as the current association */
...
bdd_t *bdd_cofactor(bdd_t *f, bdd_t *g) {
    return bdd_construct_bdd_t(f->mgr,
                               cmu_bdd_cofactor(f->mgr, f->node, g->node));
}

bdd_t *bdd_compose(bdd_t *f, bdd_t *v, bdd_t *g) {
    return bdd_construct_bdd_t(
            f->mgr, cmu_bdd_compose(f->mgr, f->node, v->node, g->node));
}
...
bdd_t *bdd_else(bdd_t *f) { return bdd_construct_bdd_t(f->mgr, cmu_bdd_else(f->mgr, f->node)); }
...
bdd_t *bdd_ite(bdd_t *i, bdd_t *t, bdd_t *e, boolean i_phase, boolean t_phase, boolean e_phase) {
    ...
    mgr    = i->mgr;
    temp1  = ((i_phase == TRUE) ? cmu_bdd_identity(mgr, i->node)
                                : cmu_bdd_not(mgr, i->node));
    temp2  = ((t_phase == TRUE) ? cmu_bdd_identity(mgr, t->node)
                                : cmu_bdd_not(mgr, t->node));
    temp3  = ((e_phase == TRUE) ? cmu_bdd_identity(mgr, e->node)
                                : cmu_bdd_not(mgr, e->node));
...
bdd_t *bdd_minimize_with_params(bdd_t *f, bdd_t *c, bdd_min_match_type_t match_type, boolean compl, boolean no_new_vars,
                                boolean return_min) { return bdd_minimize(f, c); }

bdd_t *bdd_not(bdd_t *f) { return bdd_construct_bdd_t(f->mgr, cmu_bdd_not(f->mgr, f->node)); }

bdd_t *bdd_one(struct bdd_manager_ *mgr) { return bdd_construct_bdd_t(mgr, cmu_bdd_one(mgr)); }
...
bdd_t *bdd_substitute(bdd_t *f, array_t *old_array, array_t *new_array) {
    ...
    if (num_old_vars != num_new_vars) {
        cmu_bdd_fatal(
                "bdd_substitute: mismatch of number of new and old variables");
    }
    ...
    assoc[2 * num_old_vars + 1] =
            (struct bdd_ *) 0; /* not sure if we need this second 0 */
...
bdd_t *bdd_then(bdd_t *f) { return bdd_construct_bdd_t(f->mgr, cmu_bdd_then(f->mgr, f->node)); }

bdd_t *bdd_top_var(bdd_t *f) { return bdd_construct_bdd_t(f->mgr, cmu_bdd_if(f->mgr, f->node)); }

bdd_t *bdd_xnor(bdd_t *f, bdd_t *g) { return bdd_construct_bdd_t(f->mgr, cmu_bdd_xnor(f->mgr, f->node, g->node)); }

bdd_t *bdd_xor(bdd_t *f, bdd_t *g) { return bdd_construct_bdd_t(f->mgr, cmu_bdd_xor(f->mgr, f->node, g->node)); }

bdd_t *bdd_zero(struct bdd_manager_ *mgr) { return bdd_construct_bdd_t(mgr, cmu_bdd_zero(mgr)); }

/*
Queries about BDD Formulas ----------------------------------------------------
*/

boolean bdd_equal(bdd_t *f, bdd_t *g) { return (f->node == g->node); }
...
boolean bdd_is_tautology(bdd_t *f, boolean phase) {
    return ((phase == TRUE) ? (f->node == cmu_bdd_one(f->mgr))
                            : (f->node == cmu_bdd_zero(f->mgr)));
}
...
boolean bdd_leq(bdd_t *f, bdd_t *g, boolean f_phase, boolean g_phase) {
    ...
    temp2        = ((g_phase == TRUE) ? cmu_bdd_identity(mgr, g->node)
                                      : cmu_bdd_not(mgr, g->node));
    implies_fn   = cmu_bdd_implies(mgr, temp1,
                                   temp2); /* returns a minterm of temp1*!temp2 */
...
double bdd_count_onset(bdd_t *f, array_t *var_array) {
    ...
    fraction = cmu_bdd_satisfying_fraction(
            f->mgr, f->node); /* cannot give support vars */
    return (fraction * pow((double) 2, (double) num_vars));
}

struct bdd_manager_ *bdd_get_manager(bdd_t *f) {
    return (f->mgr);
}
...
void bdd_get_stats(struct bdd_manager_ *mgr, bdd_stats *stats) {
    stats->gc.runtime =
            (long) mgr; /* for use in bdd_print_stats; this is a hack */
    /* SIS simplify package looks at these fields */
    stats->nodes.total = stats->nodes.used =
            (unsigned int) cmu_bdd_total_size(mgr);
    return;
}
...
var_set_t *bdd_get_support(bdd_t *f) {
    ...
    support =
            (struct bdd_ **) mem_get_block((num_vars + 1) * sizeof(struct bdd_ *));
    (void) cmu_bdd_support(mgr, f->node, support);

    for (i = 0; i < num_vars;
         ++i) { /* can never have more than num_var non-zero entries in array */
        var = support[i];
        if (var == (struct bdd_ *) 0) {
            break; /* have reach end of null-terminated array */
        }
        id = (int) (cmu_bdd_if_id(mgr, var) - 1); /* a variable is never garbage
                                                collected, so no need to free */
...
unsigned int bdd_num_vars(struct bdd_manager_ *mgr) { return (cmu_bdd_vars(mgr)); }

void bdd_print(bdd_t *f) {
    cmu_bdd_print_bdd(f->mgr, f->node, bdd_naming_fn_none,
                      bdd_terminal_id_fn_none, (pointer) 0, stdout);
}

void bdd_print_stats(bdd_stats stats, FILE *file) {
    struct bdd_manager_ *mgr;
...
int bdd_size(bdd_t *f) { return ((int) cmu_bdd_size(f->mgr, f->node, 1)); }

bdd_variableId bdd_top_var_id(bdd_t *f) { return ((bdd_variableId) (cmu_bdd_if_id(f->mgr, f->node) - 1)); }

/*
Miscellaneous -----------------------------------------------------------------
*/

bdd_external_hooks *bdd_get_external_hooks(struct bdd_manager_ *mgr) { return ((bdd_external_hooks *) mgr->hooks); }

void bdd_register_daemon(struct bdd_manager_ *mgr, void (*daemon)()) {
    cmu_bdd_warning("bdd_register_daemon: translated to no-op in CMU package");
}

void bdd_set_gc_mode(struct bdd_manager_ *mgr, boolean no_gc) {
    cmu_bdd_warning("bdd_set_gc_mode: translated to no-op in CMU package");
}

void bdd_dynamic_reordering(struct bdd_manager_ *mgr, bdd_reorder_type_t algorithm_type) {
    switch (algorithm_type) {
        case BDD_REORDER_SIFT:cmu_bdd_dynamic_reordering(mgr, cmu_bdd_reorder_sift);
            break;
        case BDD_REORDER_WINDOW:cmu_bdd_dynamic_reordering(mgr, cmu_bdd_reorder_stable_window3);
            break;
        case BDD_REORDER_NONE:cmu_bdd_dynamic_reordering(mgr, cmu_bdd_reorder_none);
            break;
        default:cmu_bdd_fatal("bdd_dynamic_reordering: unknown algorithm type");
    }
}

void bdd_reorder(struct bdd_manager_ *mgr) { cmu_bdd_reorder(mgr); }

bdd_variableId bdd_get_id_from_level(struct bdd_manager_ *mgr, long level) {
    struct bdd_ *fn;
...
long bdd_top_var_level(struct bdd_manager_ *mgr, bdd_t *fn) { return cmu_bdd_if_index(mgr, fn->node); }
