 ...
 */
static bdd_t *bdd_construct_bdd_t(mgr, fn) struct bdd_manager_ *mgr;
struct bdd_ *fn;
{
  bdd_t *result;
...
*/
void bdd_end(mgr) struct bdd_manager_ *mgr;
{
...
void bdd_set_mgr_init_dflts(mgr_init) bdd_mgr_init *mgr_init;
{
  cmu_bdd_warning("bdd_set_mgr_init_dflts: translated to no-op in CMU package");
  return;
}

struct bdd_manager_ *bdd_start(nvariables) int nvariables;
{
...
struct bdd_manager_ *bdd_start_with_params(nvariables, mgr_init) int nvariables;
...
bdd_t *bdd_create_variable(mgr) struct bdd_manager_ *mgr;
{ return bdd_construct_bdd_t(mgr, cmu_bdd_new_var_last(mgr)); }

bdd_t *bdd_create_variable_after(mgr, after_id) struct bdd_manager_ *mgr;
...
bdd_t *bdd_get_variable(mgr, variable_ID) struct bdd_manager_ *mgr;
...
bdd_t *bdd_and(f, g, f_phase, g_phase) bdd_t *f;
bdd_t *g;
boolean f_phase;
boolean g_phase;
{
...
bdd_t *bdd_and_smooth(f, g, smoothing_vars) bdd_t *f;
bdd_t *g;
...
bdd_t *bdd_between(f_min, f_max) bdd_t *f_min;
bdd_t *f_max;
{
  bdd_t *temp, *ret;
...
bdd_t *bdd_cofactor(f, g) bdd_t *f;
bdd_t *g;
{
...
bdd_t *bdd_compose(f, v, g) bdd_t *f;
bdd_t *v;
bdd_t *g;
{
...
bdd_t *bdd_consensus(f, quantifying_vars) bdd_t *f;
...
bdd_t *bdd_cproject(f, quantifying_vars) bdd_t *f;
...
bdd_t *bdd_else(f) bdd_t *f;
{ return bdd_construct_bdd_t(f->mgr, cmu_bdd_else(f->mgr, f->node)); }

bdd_t *bdd_ite(i, t, e, i_phase, t_phase, e_phase) bdd_t *i;
bdd_t *t;
bdd_t *e;
boolean i_phase;
boolean t_phase;
...
bdd_t *bdd_minimize(f, c) bdd_t *f;
bdd_t *c;
{
  return bdd_construct_bdd_t(f->mgr, cmu_bdd_reduce(f->mgr, f->node, c->node));
}

bdd_t *bdd_minimize_with_params(f, c, match_type, compl, no_new_vars,
                                return_min) bdd_t *f;
bdd_t *c;
...
bdd_t *bdd_not(f) bdd_t *f;
{ return bdd_construct_bdd_t(f->mgr, cmu_bdd_not(f->mgr, f->node)); }

bdd_t *bdd_one(mgr) struct bdd_manager_ *mgr;
{ return bdd_construct_bdd_t(mgr, cmu_bdd_one(mgr)); }

bdd_t *bdd_or(f, g, f_phase, g_phase) bdd_t *f;
bdd_t *g;
boolean f_phase;
boolean g_phase;
{
...
bdd_t *bdd_smooth(f, smoothing_vars) bdd_t *f;
array_t *smoothing_vars; /* of bdd_t *'s */
{
  int num_vars, i;
  bdd_t *fn, *result;
...
bdd_t *bdd_substitute(f, old_array, new_array) bdd_t *f;
...
boolean bdd_equal(f, g) bdd_t *f;
bdd_t *g;
{ return (f->node == g->node); }

bdd_t *bdd_intersects(f, g) bdd_t *f;
bdd_t *g;
{
  return bdd_construct_bdd_t(f->mgr,
                             cmu_bdd_intersects(f->mgr, f->node, g->node));
}

boolean bdd_is_tautology(f, phase) bdd_t *f;
boolean phase;
{
...
boolean bdd_leq(f, g, f_phase, g_phase) bdd_t *f;
bdd_t *g;
boolean f_phase;
boolean g_phase;
{
...
double bdd_count_onset(f, var_array) bdd_t *f;
...
struct bdd_manager_ *bdd_get_manager(f) bdd_t *f;
{
  return (f->mgr);
}

bdd_node *bdd_get_node(f, is_complemented) bdd_t *f;
...
void bdd_get_stats(mgr, stats) struct bdd_manager_ *mgr;
...
var_set_t *bdd_get_support(f) bdd_t *f;
{
...
array_t *bdd_get_varids(var_array) array_t *var_array;
{
  int i;
  bdd_t *var;
  array_t *result;
...
unsigned int bdd_num_vars(mgr) struct bdd_manager_ *mgr;
{ return (cmu_bdd_vars(mgr)); }

void bdd_print(f) bdd_t *f;
{
  cmu_bdd_print_bdd(f->mgr, f->node, bdd_naming_fn_none,
                    bdd_terminal_id_fn_none, (pointer)0, stdout);
}

void bdd_print_stats(stats, file) bdd_stats stats;
FILE *file;
{
  struct bdd_manager_ *mgr;
...
int bdd_size(f) bdd_t *f;
{ return ((int)cmu_bdd_size(f->mgr, f->node, 1)); }

bdd_variableId bdd_top_var_id(f) bdd_t *f;
...
bdd_external_hooks *bdd_get_external_hooks(mgr) struct bdd_manager_ *mgr;
{ return ((bdd_external_hooks *)mgr->hooks); }

void bdd_register_daemon(mgr, daemon) struct bdd_manager_ *mgr;
void (*daemon)();
{ cmu_bdd_warning("bdd_register_daemon: translated to no-op in CMU package"); }

void bdd_set_gc_mode(mgr, no_gc) struct bdd_manager_ *mgr;
boolean no_gc;
{ cmu_bdd_warning("bdd_set_gc_mode: translated to no-op in CMU package"); }

void bdd_dynamic_reordering(mgr, algorithm_type) struct bdd_manager_ *mgr;
...
void bdd_reorder(mgr) struct bdd_manager_ *mgr;
{ cmu_bdd_reorder(mgr); }

bdd_variableId bdd_get_id_from_level(mgr, level) struct bdd_manager_ *mgr;
long level;
{
  struct bdd_ *fn;
...
long bdd_top_var_level(mgr, fn) struct bdd_manager_ *mgr;
bdd_t *fn;
