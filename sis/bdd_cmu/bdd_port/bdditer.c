 ...
 *    continue the enumeration; and bdd_next_cube, which returns 1 if another
 * cube was found, and 0 otherwise. A cube is represented as an array of
 * bdd_literal (which are integers in {0, 1, 2}), where 0 represents negated
 * literal, 1 for literal, and 2 for don't care.  Returns a disjoint cover.  A
 * third routine is there to clean up.
 */
 ...
 */
bdd_gen *bdd_first_cube(fn, cube) bdd_t *fn;
array_t **cube; /* of bdd_literal */
{
   ...
   * the longest possible path from root to constant 1 is the number of
  ...
  if (bdd_is_tautology(fn, 0)) {
     ...
     *    if fn is just the constant 1, push_cube_stack will properly handle
     * this. Get a new pointer to fn->node beforehand: this increments the
     * reference count of fn->node; this is necessary, because when fn->node is
     * popped from the stack at the very end, it's ref count is decremented.
     */
 ...
 */
boolean bdd_next_cube(gen, cube) bdd_gen *gen;
array_t **cube; /* of bdd_literal */
{
 ...
 *    Return the generator.
 */
bdd_gen *bdd_first_node(fn, node) bdd_t *fn;
bdd_node **node; /* return */
{
  ...
  /*
   * Get the first node.  Get a new pointer to fn->node beforehand: this
   * increments the reference count of fn->node; this is necessary, because when
   * fn->node is popped from the stack at the very end, it's ref count is
   * decremented.
   */
  f = cmu_bdd_identity(manager, fn->node);
 ...
 */
boolean bdd_next_node(gen, node) bdd_gen *gen;
bdd_node **node; /* return */
{
  pop_node_stack(gen);
  if (gen->status == bdd_EMPTY) {
 ...
 */
int bdd_gen_free(gen) bdd_gen *gen;
{
  long num_vars;
  int i;
  ...
  /*
   * Free the data associated with this generator.  If there are any nodes
   * remaining on the stack, we must free them, to get their ref counts back to
 ...
 *        gen->gen.cubes.cube reflects the choices made to reach node at top of
 * the stack. For a node generator, gen->gen.nodes.visited reflects the nodes
 * already visited in the BDD dag.
 */
 ...
 *
 */
static void push_cube_stack(f, gen) bdd_node *f;
bdd_gen *gen;
{
  ...
  /*
   * Get the then and else branches of f. Note that cmu_bdd_then and
...
static void pop_cube_stack(gen) bdd_gen *gen;
{
  ...
  if (gen->stack.sp == 0) {
     ...
     * permanently pop the top node, and bdd_free it, since there are no more
     ...
     *  indexindex    id      (CMU has id 0 for constant, thus really start
     * numbering at 1; UCB starts numbering at 0)
     */
    ...
    for (i = topf_level + 1; i < array_n(gen->gen.cubes.cube); i++) {
      ...
      /*
       * No need to free var_bdd, since single variable BDDs are never garbage
       * collected. Note that level_i_id is just (mgr->indexindexes[i] - 1);
       * however, wanted to avoid using CMU internals.
       */
 ...
 *    The BDD is traversed using depth-first search, with the ELSE branch
 * searched before the THEN branch, and a node returned only after its children
 * have been returned.  Note that the returned bdd_node pointer has the
 * complement bit zeroed out.
 *
 *    Caution: If you are creating new BDD's while iterating through the
 ...
 *    Return nothing, just do it.
 */
static void push_node_stack(f, gen) bdd_node *f;
bdd_gen *gen;
{
  bdd_node *f0, *f1;
  ...
  if (f == cmu_bdd_one(mgr) || f == cmu_bdd_zero(mgr)) {
    /*
     * If f is the constant node and it has not been visited yet, then put it in
     * the visited table and set the gen->node pointer.  There is no need to put
     * it in the stack because the constant node does not have any branches, and
     * there is no need to free f because constant nodes have a saturated
     * reference count.
     */
    ...
    /*
     * f has not been marked as visited.  We don't know yet if any of its
     * branches remain to be explored.  First get its branches.  Note that
     * cmu_bdd_then and cmu_bdd_else automatically take care of inverted
     * pointers.
     */
    f0 = cmu_bdd_else(mgr, f);
    f1 = cmu_bdd_then(mgr, f);
    ...
    if (!st_lookup(gen->gen.nodes.visited, (refany)reg_f0, NIL(refany))) {
      /*
       * The 0 child has not been visited, so explore the 0 branch.  First push
       * f on the stack.  Bdd_free f1 since we will not need to access this
       * exact pointer any more.
       */
      gen->stack.stack[gen->stack.sp++] = f;
      push_node_stack(f0, gen);
      cmu_bdd_free(mgr, f1);
    } else if (!st_lookup(gen->gen.nodes.visited, (refany)reg_f1,
                          NIL(refany))) {
      /*
      ...
      /*
       * Both the 0 and 1 children have been visited. Thus we are done exploring
       * from f. Mark f as visited (put it in the visited table), and set the
       * gen->node pointer. We will no longer need to refer to f0 and f1, so
       * bdd_free them.  f will be bdd_freed when the visited table is freed.
       */
      st_insert(gen->gen.nodes.visited, (refany)reg_f, NIL(any));
      gen->node = reg_f;
      cmu_bdd_free(mgr, f0);
...
static void pop_node_stack(gen) bdd_gen *gen;
{
  bdd_node *branch_f;
 ...
 */
static void bdd_get_branches(f, f_T, f_E) bdd_node *f;
bdd_node **f_T; /* return */
bdd_node **f_E; /* return */
{
  BDD_SETUP(f);
