...
#else
    cmu_bdd_size_step(f, count_fn) bdd f;
int (*count_fn)();
#endif
{
  long result;
  ...
  result = (*count_fn)(f);
  if (!BDD_IS_CONST(f))
    result += cmu_bdd_size_step(BDD_THEN(f), count_fn) +
              cmu_bdd_size_step(BDD_ELSE(f), count_fn);
  BDD_MARK(f) = 0;
  return (result);
}
...
#else
    cmu_bdd_size(bddm, f, negout) cmu_bdd_manager bddm;
bdd f;
int negout;
#endif
{
...
#else
    cmu_bdd_size_multiple(bddm, fs, negout) cmu_bdd_manager bddm;
bdd *fs;
int negout;
#endif
{
...
static void
#if defined(__STDC__)
cmu_bdd_profile_step(cmu_bdd_manager bddm, bdd f, long *level_counts,
                     int (*count_fn)(bdd))
#else
    cmu_bdd_profile_step(bddm, f, level_counts, count_fn) cmu_bdd_manager bddm;
bdd f;
long *level_counts;
int (*count_fn)();
#endif
...
#else
    cmu_bdd_profile(bddm, f, level_counts, negout) cmu_bdd_manager bddm;
bdd f;
long *level_counts;
int negout;
#endif
...
#else
    cmu_bdd_profile_multiple(bddm, fs, level_counts,
                             negout) cmu_bdd_manager bddm;
bdd *fs;
long *level_counts;
int negout;
#endif
...
#else
    bdd_highest_ref_step(bddm, f, h) cmu_bdd_manager bddm;
bdd f;
hash_table h;
#endif
{
...
#else
    bdd_dominated_step(bddm, f, func_counts, h) cmu_bdd_manager bddm;
bdd f;
long *func_counts;
hash_table h;
#endif
...
#else
    cmu_bdd_function_profile(bddm, f, func_counts) cmu_bdd_manager bddm;
bdd f;
long *func_counts;
#endif
{
...
#else
    cmu_bdd_function_profile_multiple(bddm, fs,
                                      func_counts) cmu_bdd_manager bddm;
bdd *fs;
long *func_counts;
#endif
{
