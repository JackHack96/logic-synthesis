...
transform_env)
#else
    mtbdd_transform_closure(bddm, canonical_fn, transform_fn,
                            transform_env) cmu_bdd_manager bddm;
int (*canonical_fn)();
void (*transform_fn)();
...
#else
    mtcmu_bdd_one_data(bddm, value1, value2) cmu_bdd_manager bddm;
INT_PTR value1;
INT_PTR value2;
#endif
{
...
free_terminal_env)
#else
    cmu_mtbdd_free_terminal_closure(bddm, free_terminal_fn,
                                    free_terminal_env) cmu_bdd_manager bddm;
...
#else
    cmu_mtbdd_get_terminal(bddm, value1, value2) cmu_bdd_manager bddm;
INT_PTR value1;
INT_PTR value2;
#endif
{
...
#else
    cmu_mtbdd_terminal_value(bddm, f, value1, value2) cmu_bdd_manager bddm;
bdd f;
INT_PTR *value1;
INT_PTR *value2;
#endif
...
#else
    mtcmu_bdd_ite_step(bddm, f, g, h) cmu_bdd_manager bddm;
bdd f;
bdd g;
bdd h;
#endif
...
#else
    mtcmu_bdd_ite(bddm, f, g, h) cmu_bdd_manager bddm;
bdd f;
bdd g;
bdd h;
#endif
...
/* mtcmu_bdd_substitute(bddm, f) does the analog of cmu_bdd_substitute for
 * MTBDDs. */
...
#else
    mtcmu_bdd_substitute(bddm, f) cmu_bdd_manager bddm;
bdd f;
#endif
{
  long op;
  ...
  if (bdd_check_arguments(1, f)) {
    ...
    RETURN_BDD(cmu_bdd_substitute_step(bddm, f, op, mtcmu_bdd_ite_step,
                                       bddm->curr_assoc));
  }
  return ((bdd)0);
}
...
#else
    cmu_mtbdd_equal_step(bddm, f, g) cmu_bdd_manager bddm;
bdd f;
bdd g;
#endif
{
...
#else
    cmu_mtbdd_equal(bddm, f, g) cmu_bdd_manager bddm;
bdd f;
bdd g;
#endif
{
