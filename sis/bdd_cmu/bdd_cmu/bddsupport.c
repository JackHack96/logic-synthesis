...
static int
#if defined(__STDC__)
cmu_bdd_depends_on_step(cmu_bdd_manager bddm, bdd f, bdd_index_type var_index,
                        int mark)
#else
    cmu_bdd_depends_on_step(bddm, f, var_index, mark) cmu_bdd_manager bddm;
bdd f;
...
#else
    cmu_bdd_depends_on(bddm, f, var) cmu_bdd_manager bddm;
bdd f;
bdd var;
#endif
{
  if (bdd_check_arguments(2, f, var)) {
    ...
    if (cmu_bdd_type_aux(bddm, var) != BDD_TYPE_POSVAR) {
      cmu_bdd_warning(
          "cmu_bdd_depends_on: second argument is not a positive variable");
      if (BDD_IS_CONST(var))
        return (1);
    }
...
#else
    bdd_unmark_nodes(bddm, f) cmu_bdd_manager bddm;
bdd f;
#endif
{
  bdd temp;
...
#else
    cmu_bdd_support_step(bddm, f, support) cmu_bdd_manager bddm;
bdd f;
bdd *support;
#endif
{
...
#else
    cmu_bdd_support(bddm, f, support) cmu_bdd_manager bddm;
bdd f;
bdd *support;
#endif
{
