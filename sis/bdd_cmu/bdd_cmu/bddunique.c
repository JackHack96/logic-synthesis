...
#else
    bdd_rehash_var_table(table, grow) var_table table;
int grow;
#endif
{
  long i;
...
#else
    bdd_mark(bddm) cmu_bdd_manager bddm;
#endif
{
  long i, j;
  var_table table;
...
#else
    bdd_sweep_var_table(bddm, i, maybe_rehash) cmu_bdd_manager bddm;
long i;
int maybe_rehash;
#endif
{
...
#else
    bdd_sweep(bddm) cmu_bdd_manager bddm;
#endif
{
  long i;
...
#else
    cmu_bdd_gc(bddm) cmu_bdd_manager bddm;
#endif
{
  bdd_mark(bddm);
...
#else
    bdd_set_gc_limit(bddm) cmu_bdd_manager bddm;
#endif
{
...
#else
    bdd_clear_temps(bddm) cmu_bdd_manager bddm;
#endif
{
  long i, j;
  var_table table;
...
#else
    bdd_cleanup(bddm, code) cmu_bdd_manager bddm;
int code;
#endif
{
...
#else
    bdd_find_aux(bddm, indexindex, d1, d2) cmu_bdd_manager bddm;
...
#else
    bdd_check(bddm) cmu_bdd_manager bddm;
#endif
{
  long nodes;
  ...
  if (bddm->unique_table.entries > bddm->unique_table.gc_limit) {
    ...
    if (3 * nodes > 2 * bddm->unique_table.gc_limit && bddm->allow_reordering &&
        bddm->reorder_fn) {
      cmu_bdd_reorder_aux(bddm);
      if (4 * bddm->unique_table.entries > 3 * nodes &&
          3 * nodes > 4 * bddm->nodes_at_start)
  ...
  if (3 * bddm->op_cache.size < 2 * bddm->op_cache.entries &&
      32 * bddm->op_cache.size <
          bddm->op_cache.cache_ratio * bddm->unique_table.entries)
...
#else
    bdd_find(bddm, indexindex, f, g) cmu_bdd_manager bddm;
bdd_indexindex_type indexindex;
bdd f;
bdd g;
#endif
{
  ...
  if (f == g) {
    ...
    if (BDD_IS_OUTPOS(f))
      ...
      temp = BDD_NOT(bdd_find_aux(bddm, indexindex, (INT_PTR)BDD_NOT(f),
                                  (INT_PTR)BDD_NOT(g)));
    {
      BDD_SETUP(temp);
...
#else
    bdd_find_terminal(bddm, value1, value2) cmu_bdd_manager bddm;
INT_PTR value1;
INT_PTR value2;
#endif
{
  ...
  if ((*bddm->canonical_fn)(bddm, value1, value2, bddm->transform_env)) {
    (*bddm->transform_fn)(bddm, value1, value2, &value1, &value2,
                          bddm->transform_env);
...
#else
    cmu_bdd_clear_refs(bddm) cmu_bdd_manager bddm;
#endif
{
  long i, j;
  var_table table;
...
#else
    bdd_new_var_table(bddm) cmu_bdd_manager bddm;
#endif
{
  long i;
  var_table table;
...
#else
    cmu_bdd_init_unique(bddm) cmu_bdd_manager bddm;
#endif
{
  bddm->unique_table.tables = (var_table *)mem_get_block(
...
#else
    cmu_bdd_free_unique(bddm) cmu_bdd_manager bddm;
#endif
{
  long i, j;
  var_table table;
