...
if ((
result = (*terminal_fn)(bddm, &f, &g, env)
))
{
BDD_SETUP(result);
...
if (
bdd_lookup_in_cache2(bddm, op, f, g, &result))
return (result);
{
BDD_SETUP(f);
BDD_SETUP(g);
...
bdd
#if defined(__STDC__)
        bdd_apply1_step(cmu_bdd_manager bddm, bdd (*terminal_fn)(cmu_bdd_manager, bdd *, pointer),
...
if (
bdd_lookup_in_cache1(bddm, op, f, &result))
return (result);
{
BDD_SETUP(f);
