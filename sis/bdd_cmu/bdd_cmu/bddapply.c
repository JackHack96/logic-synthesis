...
bdd_apply2_step(cmu_bdd_manager bddm,
                ...
                long op, bdd f, bdd g, pointer env)
#else
    bdd_apply2_step(bddm, terminal_fn, op, f, g, env) cmu_bdd_manager bddm;
bdd (*terminal_fn)();
long op;
bdd f;
bdd g;
...
bdd
#if defined(__STDC__)
        bdd_apply2(cmu_bdd_manager bddm, bdd (*terminal_fn)(cmu_bdd_manager, bdd *, bdd *, pointer), bdd

f,
bdd g, pointer
env)
#else
    bdd_apply2(bddm, terminal_fn, f, g, env) cmu_bdd_manager bddm;
bdd (*terminal_fn)();
bdd f;
bdd g;
pointer env;
...
static bdd
#if defined(__STDC__)
bdd_apply1_step(cmu_bdd_manager bddm,
                bdd (*terminal_fn)(cmu_bdd_manager, bdd *, pointer),

                long op, bdd f, pointer env)
#else
    bdd_apply1_step(bddm, terminal_fn, op, f, env) cmu_bdd_manager bddm;
bdd (*terminal_fn)();
long op;
bdd f;
pointer env;
...
bdd
#if defined(__STDC__)
        bdd_apply1(cmu_bdd_manager bddm, bdd (*terminal_fn)(cmu_bdd_manager, bdd *, pointer), bdd

f,
pointer env
)
#else
    bdd_apply1(bddm, terminal_fn, f, g, env) cmu_bdd_manager bddm;
bdd (*terminal_fn)();
bdd f;
pointer env;
#endif
