...
static bdd
#if defined(__STDC__)
bdd_restrict_step(cmu_bdd_manager bddm, bdd f, bdd_indexindex_type g_indexindex,
                  bdd h, long op)
#else
    bdd_restrict_step(bddm, f, g_indexindex, h, op) cmu_bdd_manager bddm;
bdd f;
...
static bdd
#if defined(__STDC__)
cmu_bdd_compose_step(cmu_bdd_manager bddm, bdd f,
                     bdd_indexindex_type g_indexindex, bdd h, long op)
#else
    cmu_bdd_compose_step(bddm, f, g_indexindex, h, op) cmu_bdd_manager bddm;
bdd f;
bdd_indexindex_type g_indexindex;
bdd h;
long op;
...
#else
    cmu_bdd_compose_temp(bddm, f, g, h) cmu_bdd_manager bddm;
bdd f, g, h;
#endif
{
  BDD_SETUP(g);
  return (cmu_bdd_compose_step(bddm, f, BDD_INDEXINDEX(g), h,
                               OP_COMP + BDD_INDEXINDEX(g)));
}
...
#else
    cmu_bdd_compose(bddm, f, g, h) cmu_bdd_manager bddm;
bdd f, g, h;
#endif
{
  if (bdd_check_arguments(3, f, g, h)) {
    ...
    if (cmu_bdd_type_aux(bddm, g) != BDD_TYPE_POSVAR) {
      cmu_bdd_warning(
          "cmu_bdd_compose: second argument is not a positive variable");
      BDD_INCREFS(f);
      return (f);
    }
    FIREWALL(bddm);
    RETURN_BDD(cmu_bdd_compose_step(bddm, f, BDD_INDEXINDEX(g), h,
                                    OP_COMP + BDD_INDEXINDEX(g)));
  }
  return ((bdd)0);
}
...
bdd
#if defined(__STDC__)
cmu_bdd_substitute_step(cmu_bdd_manager bddm, bdd f, long op, bdd (*ite_fn)(cmu_bdd_manager, bdd, bdd, bdd),
                        var_assoc subst)
#else
    cmu_bdd_substitute_step(bddm, f, op, ite_fn, subst) cmu_bdd_manager bddm;
bdd f;
long op;
bdd (*ite_fn)();
var_assoc subst;
...
#else
    cmu_bdd_substitute(bddm, f) cmu_bdd_manager bddm;
bdd f;
#endif
{
  long op;
  ...
  if (bdd_check_arguments(1, f)) {
    ...
    RETURN_BDD(cmu_bdd_substitute_step(bddm, f, op, cmu_bdd_ite_step,
                                       bddm->curr_assoc));
  }
  return ((bdd)0);
}
