...
#else
    bdd_mark_shared_nodes(bddm, f) cmu_bdd_manager bddm;
bdd f;
#endif
{
  BDD_SETUP(f);
...
#else
    bdd_number_shared_nodes(bddm, f, h, next) cmu_bdd_manager bddm;
bdd f;
hash_table h;
long *next;
#endif
{
  BDD_SETUP(f);
  if (BDD_IS_CONST(f) || ((1 << cmu_bdd_type_aux(bddm, f)) &
                          ((1 << BDD_TYPE_POSVAR) | (1 << BDD_TYPE_NEGVAR))))
    return;
  if (BDD_MARK(f) == 0)
    return;
  if (BDD_MARK(f) == 2) {
...
char *
#if defined(__STDC__)
bdd_terminal_id(cmu_bdd_manager bddm, bdd f,
                char *(*terminal_id_fn)(cmu_bdd_manager, INT_PTR, INT_PTR,
                                        pointer),
                pointer

                    env)
#else
    bdd_terminal_id(bddm, f, terminal_id_fn, env) cmu_bdd_manager bddm;
bdd f;
char *(*terminal_id_fn)();
pointer env;
#endif
...
char *
#if defined(__STDC__)
bdd_var_name(cmu_bdd_manager bddm, bdd v,
             char *(*var_naming_fn)(cmu_bdd_manager, bdd, pointer),
             pointer

                 env)
#else
    bdd_var_name(bddm, v, var_naming_fn, env) cmu_bdd_manager bddm;
bdd v;
char *(*var_naming_fn)();
pointer env;
#endif
...
#else
    cmu_mtbdd_terminal_value_aux(bddm, f, value1, value2) cmu_bdd_manager bddm;
bdd f;
INT_PTR *value1;
INT_PTR *value2;
#endif
{
  ...
  if (BDD_IS_OUTPOS(f)) {
    ...
    *value2 = BDD_DATA1(f);
    ...
    (*bddm->transform_fn)(bddm, BDD_DATA0(f), BDD_DATA1(f), value1, value2,
                          bddm->transform_env);
}
