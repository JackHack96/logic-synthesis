...
#endif
{
    ...
    if (BDD_MARK(f) == 0)
        return;
    if (BDD_MARK(f) == 2) {
        bdd_insert_in_hash_table(h, f, (pointer)
        next);
        ++*next;
    }
    BDD_MARK(f) = 0;
...
char *
#if defined(__STDC__)
             bdd_terminal_id(cmu_bdd_manager bddm, bdd f, char *(*terminal_id_fn)(cmu_bdd_manager, INT_PTR, INT_PTR,
                             pointer), pointer
...
cmu_mtbdd_terminal_value_aux(bddm, f,
&v1, &v2);
if (terminal_id_fn)
...
if (f == BDD_ZERO(bddm))
return ("0");
sprintf(default_terminal_id,
"terminal %ld %ld", (long)v1, (long)v2);
...
char *
#if defined(__STDC__)
             bdd_var_name(cmu_bdd_manager bddm, bdd v, char *(*var_naming_fn)(cmu_bdd_manager, bdd, pointer), pointer
...
{
BDD_SETUP(v);
sprintf(default_var_name,
"var.%d", BDD_INDEX(bddm, v));
