...
#else
    cmu_bdd_satisfy_step(bddm, f) cmu_bdd_manager bddm;
bdd f;
#endif
{
  bdd temp;
...
#else
    cmu_bdd_satisfy(bddm, f) cmu_bdd_manager bddm;
bdd f;
#endif
{
...
static bdd
#if defined(__STDC__)
cmu_bdd_satisfy_support_step(cmu_bdd_manager bddm, bdd f,
                             bdd_indexindex_type *support)
#else
    cmu_bdd_satisfy_support_step(bddm, f, support) cmu_bdd_manager bddm;
bdd f;
bdd_indexindex_type *support;
#endif
{
...
#else
    cmu_bdd_satisfy_support(bddm, f) cmu_bdd_manager bddm;
bdd f;
#endif
{
  ...
  if (bdd_check_arguments(1, f)) {
    ...
    support =
        (bdd_indexindex_type *)mem_get_block((bddm->vars + 1) * sizeof(bdd));
    FIREWALL1(bddm, if (retcode == BDD_ABORTED || retcode == BDD_OVERFLOWED) {
    ...
    *p = 0;
    qsort((pointer)support, (unsigned)(p - support),
          sizeof(bdd_indexindex_type), index_cmp);
    while (p != support) {
      --p;
...
#else
    cmu_bdd_satisfying_fraction_step(bddm, f) cmu_bdd_manager bddm;
bdd f;
#endif
{
  long cache_result[2];
  ...
  if (bdd_lookup_in_cache1d(bddm, OP_SATFRAC, f, &cache_result[0],
                            &cache_result[1])) {
...
#else
    cmu_bdd_satisfying_fraction(bddm, f) cmu_bdd_manager bddm;
bdd f;
#endif
{
  if (bdd_check_arguments(1, f))
