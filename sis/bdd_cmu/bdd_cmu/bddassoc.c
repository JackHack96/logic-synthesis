...
#else
    cmu_bdd_assoc_eq(bddm, p, q) cmu_bdd_manager bddm;
bdd *p;
bdd *q;
#endif
{
...
#else
    check_assoc(bddm, assoc_info, pairs) cmu_bdd_manager bddm;
bdd *assoc_info;
int pairs;
#endif
{
  ...
  if (pairs)
    while (assoc_info[0] && assoc_info[1]) {
      if (cmu_bdd_type_aux(bddm, assoc_info[0]) != BDD_TYPE_POSVAR) {
        cmu_bdd_warning(
            "check_assoc: first element in pair is not a positive variable");
        return (0);
      }
      assoc_info += 2;
    }
...
#else
    cmu_bdd_new_assoc(bddm, assoc_info, pairs) cmu_bdd_manager bddm;
bdd *assoc_info;
int pairs;
#endif
{
  ...
  for (q = &bddm->assocs, p = *q, i = 0; p && p->id == i;
       q = &p->next, p = *q, ++i)
    ;
...
#else
    bdd_flush_id_entries(bddm, p, closure) cmu_bdd_manager bddm;
cache_entry p;
pointer closure;
#endif
{
...
#else
    cmu_bdd_free_assoc(bddm, assoc_id) cmu_bdd_manager bddm;
int assoc_id;
#endif
{
  bdd_indexindex_type i;
  ...
  cmu_bdd_warning(
      "cmu_bdd_free_assoc: no variable association with specified ID");
}
...
#else
    cmu_bdd_augment_temp_assoc(bddm, assoc_info, pairs) cmu_bdd_manager bddm;
bdd *assoc_info;
int pairs;
#endif
{
...
#else
    cmu_bdd_temp_assoc(bddm, assoc_info, pairs) cmu_bdd_manager bddm;
bdd *assoc_info;
int pairs;
#endif
{
...
#else
    cmu_bdd_assoc(bddm, assoc_id) cmu_bdd_manager bddm;
int assoc_id;
#endif
{
  int old_assoc;
