...
static void
#if defined(__STDC__)
bdd_exchange_aux(cmu_bdd_manager bddm, bdd f,
                 bdd_indexindex_type next_indexindex)
#else
    bdd_exchange_aux(bddm, f, next_indexindex) cmu_bdd_manager bddm;
bdd f;
bdd_indexindex_type next_indexindex;
#endif
{
  ...
  f2 = BDD_ELSE(f);
  {
    ...
    if (f12 == f22)
      ...
      temp2 =
          BDD_NOT(bdd_find_aux(bddm, BDD_INDEXINDEX(f), (INT_PTR)BDD_NOT(f12),
                               (INT_PTR)BDD_NOT(f22)));
...
static void
#if defined(__STDC__)
fixup_assoc(cmu_bdd_manager bddm, long indexindex1, long indexindex2,
            var_assoc va)
#else
    fixup_assoc(bddm, indexindex1, indexindex2, va) cmu_bdd_manager bddm;
long indexindex1;
long indexindex2;
var_assoc va;
#endif
{
  ...
  if (va->assoc[indexindex1] && va->last == bddm->indexes[indexindex1])
    va->last++;
  else if (!va->assoc[indexindex1] && va->assoc[indexindex2] &&
           va->last == bddm->indexes[indexindex2])
    va->last--;
}
...
#else
    sweep_var_table(bddm, i) cmu_bdd_manager bddm;
long i;
#endif
{
  long j;
...
#else
    bdd_exchange(bddm, i) cmu_bdd_manager bddm;
long i;
#endif
{
  bdd_indexindex_type next_indexindex;
    ...
    for (p = &table->table[j], f = *p; f; f = *p) {
      ...
      if (BDD_REFS(f) || BDD_TEMP_REFS(f)) {
        ...
        f2 = (bdd)BDD_DATA1(f);
        {
          ...
          BDD_SETUP(f2);
          if (BDD_INDEXINDEX(f1) != next_indexindex &&
              BDD_INDEXINDEX(f2) != next_indexindex)
            p = &f->next;
          else {
            *p = f->next;
            f->next = g;
...
#else
    cmu_bdd_var_block_reorderable(bddm, b, reorderable) cmu_bdd_manager bddm;
block b;
int reorderable;
#endif
{
...
#else
    bdd_exchange_var_blocks(bddm, parent, bi) cmu_bdd_manager bddm;
block parent;
long bi;
#endif
{
  ...
  /* fashion. */
  for (i = 0;
       i <= b1->last_index - b1->first_index + b2->last_index - b2->first_index;
       ++i) {
...
#else
    cmu_bdd_reorder_window2(bddm, b, i) cmu_bdd_manager bddm;
block b;
long i;
#endif
{
...
#else
    cmu_bdd_reorder_window3(bddm, b, i) cmu_bdd_manager bddm;
block b;
long i;
#endif
{
...
#else
    cmu_bdd_reorder_stable_window3_aux(bddm, b, levels) cmu_bdd_manager bddm;
block b;
char *levels;
#endif
{
...
#else
    cmu_bdd_reorder_stable_window3(bddm) cmu_bdd_manager bddm;
#endif
{
  char *levels;
...
static void
#if defined(__STDC__)
bdd_sift_block(cmu_bdd_manager bddm, block b, long start_pos,
               double max_size_factor)
#else
    bdd_sift_block(bddm, b, start_pos, max_size_factor) cmu_bdd_manager bddm;
block b;
long start_pos;
double max_size_factor;
#endif
...
static void
#if defined(__STDC__)
cmu_bdd_reorder_sift_aux(cmu_bdd_manager bddm, block b, block *to_sift,
                         double max_size_factor)
#else
    cmu_bdd_reorder_sift_aux(bddm, b, to_sift,
                             max_size_factor) cmu_bdd_manager bddm;
block b;
block *to_sift;
double max_size_factor;
#endif
{
  ...
  if (b->reorderable) {
    ...
    while (i) {
      ...
      widest = 0;
      for (j = 0; j <= i; ++j) {
        for (w = 0, k = to_sift[j]->first_index; k <= to_sift[j]->last_index;
             ++k)
...
#else
    cmu_bdd_reorder_sift_aux1(bddm, max_size_factor) cmu_bdd_manager bddm;
double max_size_factor;
#endif
{
  block *to_sift;
...
#else
    cmu_bdd_reorder_sift(bddm) cmu_bdd_manager bddm;
#endif
{
...
#else
    cmu_bdd_reorder_hybrid(bddm) cmu_bdd_manager bddm;
#endif
{
  long nodes;
  double max_size_factor;
  ...
  *(double *)bddm->reorder_data =
      1.0 + (2.0 * (nodes - bddm->unique_table.entries)) / nodes;
}
...
#else
    cmu_bdd_dynamic_reordering(bddm, reorder_fn) cmu_bdd_manager bddm;
void (*reorder_fn)();
#endif
{
...
#else
    bdd_add_internal_references(bddm) cmu_bdd_manager bddm;
#endif
{
  long i, j;
  var_table table;
        ...
        if (BDD_REFS(g) || BDD_TEMP_REFS(g)) {
          ...
          *f = g->next;
          if (i == bddm->vars && bddm->unique_table.free_terminal_fn)
            (*bddm->unique_table.free_terminal_fn)(
                bddm, BDD_DATA0(g), BDD_DATA1(g),
...
#else
    bdd_nuke_internal_references(bddm) cmu_bdd_manager bddm;
#endif
{
  long i, j;
  var_table table;
...
#else
    cmu_bdd_reorder_aux(bddm) cmu_bdd_manager bddm;
#endif
{
  if (bddm->reorder_fn) {
    bdd_flush_all(bddm);
...
#else
    cmu_bdd_reorder(bddm) cmu_bdd_manager bddm;
#endif
{
  cmu_bdd_gc(bddm);
