...
#else
    terminal_id_fn(bddm, v1, v2, junk) cmu_bdd_manager bddm;
INT_PTR v1;
INT_PTR v2;
pointer junk;
#endif
...
#endif
{
  cmu_bdd_print_bdd(bddm, f, bdd_naming_fn_none, terminal_id_fn, (pointer)0,
                    stderr);
}

#if defined(__STDC__)

static void error(char *op, bdd result, bdd expected, ...) {
  int i;
  va_list ap;
  bdd f;
...
#else
static void error(va_alist) va_dcl {
  int i;
  va_list ap;
  char *op;
  bdd result;
...
#else
    cofactor(table, var, value) tt table;
int var;
int value;
#endif
{
...
#else
    test_ite(f1, table1, f2, table2, f3, table3) bdd f1;
tt table1;
bdd f2;
tt table2;
bdd f3;
...
#else
    test_and(f1, table1, f2, table2) bdd f1;
tt table1;
bdd f2;
tt table2;
#endif
...
#else
    test_or(f1, table1, f2, table2) bdd f1;
tt table1;
bdd f2;
tt table2;
#endif
...
#else
    test_xor(f1, table1, f2, table2) bdd f1;
tt table1;
bdd f2;
tt table2;
#endif
...
#else
    test_compose(f1, table1, f2, table2) bdd f1;
tt table1;
bdd f2;
tt table2;
#endif
...
#endif
{
  ...
  expected = encoding_to_bdd(resulttable);
  if (result != expected)
    error("quantification", result, expected, f, vars[var1], vars[var2],
          (bdd)0);
  cmu_bdd_free(bddm, result);
  cmu_bdd_free(bddm, expected);
}
...
#else
    test_rel_prod(f1, table1, f2, table2) bdd f1;
tt table1;
bdd f2;
tt table2;
#endif
{
  ...
  expected = encoding_to_bdd(resulttable);
  if (result != expected)
    error("relational product", result, expected, f1, f2, vars[var1],
          vars[var2], (bdd)0);
  cmu_bdd_free(bddm, result);
  cmu_bdd_free(bddm, expected);
}
...
#else
    test_subst(f1, table1, f2, table2, f3, table3) bdd f1;
tt table1;
bdd f2;
tt table2;
bdd f3;
...
#endif
{
  ...
  expected = encoding_to_bdd(resulttable);
  if (result != expected)
    error("substitute", result, expected, f1, vars[var1], f2, vars[var2], f3,
          (bdd)0);
  cmu_bdd_free(bddm, result);
  cmu_bdd_free(bddm, expected);
}
...
#else
    test_inter_impl(f1, table1, f2, table2) bdd f1;
tt table1;
bdd f2;
tt table2;
#endif
...
#endif
{
  ...
  if (temp2 != cmu_bdd_zero(bddm))
    error("intersection of satisfy result with negated argument", temp2,
          cmu_bdd_zero(bddm), f, (bdd)0);
  cmu_bdd_free(bddm, temp1);
  cmu_bdd_free(bddm, temp2);
  var1 = ((unsigned long)random()) % TT_VARS;
  do
  ...
  if (temp3 != cmu_bdd_zero(bddm))
    error("intersection of satisfy support result with negated argument", temp3,
          cmu_bdd_zero(bddm), result, (bdd)0);
  cmu_bdd_free(bddm, result);
  cmu_bdd_free(bddm, temp1);
  cmu_bdd_free(bddm, temp2);
  ...
  if (cmu_bdd_satisfying_fraction(bddm, temp1) +
          cmu_bdd_satisfying_fraction(bddm, temp2) !=
...
#else
    test_gen_cof(f1, table1, f2, table2) bdd f1;
tt table1;
bdd f2;
tt table2;
#endif
{
  ...
  if (temp3 != cmu_bdd_one(bddm))
    error("d.c. comparison of generalized cofactor", temp3, cmu_bdd_one(bddm),
          f1, f2, (bdd)0);
  cmu_bdd_free(bddm, result);
  cmu_bdd_free(bddm, temp1);
  cmu_bdd_free(bddm, temp2);
...
#else
    test_reduce(f1, table1, f2, table2) bdd f1;
tt table1;
bdd f2;
tt table2;
#endif
{
  ...
  temp3 = cmu_bdd_or(bddm, temp1, temp2);
  if (temp3 != cmu_bdd_one(bddm))
    error("d.c. comparison of reduce", temp3, cmu_bdd_one(bddm), f1, f2,
          (bdd)0);
  cmu_bdd_free(bddm, result);
  cmu_bdd_free(bddm, temp1);
  cmu_bdd_free(bddm, temp2);
...
#else
    apply_and(bddm, f, g, env) cmu_bdd_manager bddm;
bdd *f;
bdd *g;
pointer env;
#endif
...
#else
    test_apply(f1, table1, f2, table2) bdd f1;
tt table1;
bdd f2;
tt table2;
#endif
...
#else
    test_size(f1, table1, f2, table2) bdd f1;
tt table1;
bdd f2;
tt table2;
#endif
...
#else
    canonical_fn(bddm, v1, v2, env) cmu_bdd_manager bddm;
INT_PTR v1;
INT_PTR v2;
pointer env;
#endif
...
static void
#if defined(__STDC__)
transform_fn(cmu_bdd_manager bddm, INT_PTR v1, INT_PTR v2, INT_PTR *r1,
             INT_PTR *r2, pointer env)
#else
    transform_fn(bddm, v1, v2, r1, r2, env) cmu_bdd_manager bddm;
INT_PTR v1;
INT_PTR v2;
INT_PTR *r1;
INT_PTR *r2;
...
#else
    mtbdd_mult_step(bddm, f, g) cmu_bdd_manager bddm;
bdd f;
bdd g;
#endif
{
...
#else
    mtbdd_mult(bddm, f, g) cmu_bdd_manager bddm;
bdd f;
bdd g;
#endif
{
...
#else
    mtbdd_add_step(bddm, f, g) cmu_bdd_manager bddm;
bdd f;
bdd g;
#endif
{
...
#else
    mtbdd_add(bddm, f, g) cmu_bdd_manager bddm;
bdd f;
bdd g;
#endif
{
...
#endif
{
  ...
  expected = encoding_to_bdd(resulttable);
  if (result != expected)
    error("swap variables", result, expected, f, vars[var1], vars[var2],
          (bdd)0);
  cmu_bdd_free(bddm, result);
  cmu_bdd_free(bddm, expected);
}
...
#else
    random_tests(iterations) int iterations;
#endif
{
  int i;
