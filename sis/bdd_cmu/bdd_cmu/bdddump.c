...
#else
    write(bddm, n, bytes, fp) cmu_bdd_manager bddm;
unsigned long n;
int bytes;
FILE *fp;
#endif
...
static void
#if defined(__STDC__)
cmu_bdd_dump_bdd_step(cmu_bdd_manager bddm, bdd f, FILE *fp, hash_table h,
                      bdd_index_type *normalized_indexes, int index_size,
                      int node_number_size)
#else
    cmu_bdd_dump_bdd_step(bddm, f, fp, h, normalized_indexes, index_size,
                          node_number_size) cmu_bdd_manager bddm;
bdd f;
FILE *fp;
hash_table h;
...
#endif
{
  ...
  case BDD_TYPE_POSVAR:
    ...
    write(bddm, (unsigned long)normalized_indexes[BDD_INDEX(bddm, f)],
          index_size, fp);
    break;
  ...
  case BDD_TYPE_NONTERMINAL:
    ...
    if (number && *number < 0) {
      ...
      write(bddm, (unsigned long)normalized_indexes[BDD_INDEX(bddm, f)],
            index_size, fp);
      cmu_bdd_dump_bdd_step(bddm, BDD_THEN(f), fp, h, normalized_indexes,
                            index_size, node_number_size);
      cmu_bdd_dump_bdd_step(bddm, BDD_ELSE(f), fp, h, normalized_indexes,
                            index_size, node_number_size);
    }
    break;
  default:
    cmu_bdd_fatal(
        "cmu_bdd_dump_bdd_step: unknown type returned by cmu_bdd_type");
  }
}
...
#else
    cmu_bdd_dump_bdd(bddm, f, vars, fp) cmu_bdd_manager bddm;
bdd f;
bdd *vars;
FILE *fp;
#endif
  ...
  if (bdd_check_arguments(1, f)) {
    for (i = 0; vars[i]; ++i)
      if (cmu_bdd_type(bddm, vars[i]) != BDD_TYPE_POSVAR) {
        cmu_bdd_warning(
            "cmu_bdd_dump_bdd: support is not all positive variables");
        return (0);
      }
    normalized_indexes = (bdd_index_type *)mem_get_block(
        (SIZE_T)(bddm->vars * sizeof(bdd_index_type)));
    ...
    FIREWALL1(bddm, if (retcode == BDD_IOERROR) {
      cmu_bdd_free_hash_table(h);
      mem_free_block((pointer)normalized_indexes);
      return (0);
    } else cmu_bdd_fatal("cmu_bdd_dump_bdd: got unexpected retcode"););
...
static bdd
#if defined(__STDC__)
cmu_bdd_undump_bdd_step(cmu_bdd_manager bddm, bdd *vars, FILE *fp,
                        bdd_index_type number_vars, bdd *shared,
                        long number_shared, long *shared_so_far, int index_size,
                        int node_number_size, int *error)
#else
    cmu_bdd_undump_bdd_step(bddm, vars, fp, number_vars, shared, number_shared,
                            shared_so_far, index_size, node_number_size,
                            error) cmu_bdd_manager bddm;
bdd *vars;
FILE *fp;
bdd_index_type number_vars;
bdd *shared;
...
#endif
{
  ...
  if (i == index_mask[index_size - 1]) {
    ...
    case NODELABEL_ENCODING:
      ...
      ++*shared_so_far;
      v = cmu_bdd_undump_bdd_step(bddm, vars, fp, number_vars, shared,
                                  number_shared, shared_so_far, index_size,
                                  node_number_size, error);
  ...
  temp1 = cmu_bdd_undump_bdd_step(bddm, vars, fp, number_vars, shared,
                                  number_shared, shared_so_far, index_size,
                                  node_number_size, error);
  temp2 = cmu_bdd_undump_bdd_step(bddm, vars, fp, number_vars, shared,
                                  number_shared, shared_so_far, index_size,
                                  node_number_size, error);
  if (*error) {
...
#else
    cmu_bdd_undump_bdd(bddm, vars, fp, error) cmu_bdd_manager bddm;
bdd *vars;
FILE *fp;
int *error;
#endif
{
  ...
  *error = 0;
  for (i = 0; vars[i]; ++i)
    if (cmu_bdd_type(bddm, vars[i]) != BDD_TYPE_POSVAR) {
      cmu_bdd_warning(
          "cmu_bdd_undump_bdd: support is not all positive variables");
      return ((bdd)0);
    }
  ...
  for (i = 0; i < number_shared; ++i)
    shared[i] = 0;
  shared_so_far = 0;
  result = cmu_bdd_undump_bdd_step(bddm, vars, fp, number_vars, shared,
                                   number_shared, &shared_so_far, index_size,
                                   node_number_size, error);
  for (i = 0; i < number_shared; ++i)
    if ((v = shared[i]))
