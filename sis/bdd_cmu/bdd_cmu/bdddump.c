...
#endif
{
    ...
    switch (cmu_bdd_type_aux(bddm, f)) {
        case BDD_TYPE_ZERO:write(bddm, FALSE_ENCODING, index_size + 1, fp);
            break;
        case BDD_TYPE_ONE:write(bddm, TRUE_ENCODING, index_size + 1, fp);
            break;
        case BDD_TYPE_CONSTANT:write(bddm, CONSTANT_ENCODING, index_size + 1, fp);
            write(bddm, (unsigned long) BDD_DATA(f)[0], sizeof(long), fp);
            write(bddm, (unsigned long) BDD_DATA(f)[1], sizeof(long), fp);
            break;
        case BDD_TYPE_POSVAR:write(bddm, POSVAR_ENCODING, index_size + 1, fp);
            write(bddm, (unsigned long) normalized_indexes[BDD_INDEX(bddm, f)], index_size, fp);
            break;
        case BDD_TYPE_NEGVAR:write(bddm, NEGVAR_ENCODING, index_size + 1, fp);
            write(bddm, (unsigned long) normalized_indexes[BDD_INDEX(bddm, f)], index_size, fp);
            break;
        case BDD_TYPE_NONTERMINAL:
            ...
            break;
        default:cmu_bdd_fatal("cmu_bdd_dump_bdd_step: unknown type returned by cmu_bdd_type");
    }
}
...
#endif
{
    ...
    if (i == index_mask[index_size - 1]) {
        ...
        if (*error)
            return ((bdd) 0);
        switch (encoding) {
            case TRUE_ENCODING: return (BDD_ONE(bddm));
            case FALSE_ENCODING: return (BDD_ZERO(bddm));
            case CONSTANT_ENCODING: value1 = read(error, sizeof(long), fp);
            ...
            case NEGVAR_ENCODING: i = read(error, index_size, fp);
            ...
            case NEGNODE_ENCODING: node_number = read(error, node_number_size, fp);
                ...
                if (encoding == POSNODE_ENCODING)
                    ...
                    return (BDD_NOT(v));
            case NODELABEL_ENCODING: node_number = *shared_so_far;
                ++*shared_so_far;
                ...
                v = cmu_bdd_identity(bddm, v);
                return (v);
            default: *error = BDD_UNDUMP_FORMAT;
                return ((bdd) 0);
        }
    }
    if (i >= number_vars) {
