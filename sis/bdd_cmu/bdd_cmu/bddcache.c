...
#define HASH3(d1, d2, d3)                                                      \
  ((INT_PTR)(d1) + (((INT_PTR)(d2)) << 1) + (((INT_PTR)(d3)) << 2))

static void
#if defined(__STDC__)
bdd_purge_entry(cmu_bdd_manager bddm, cache_entry *bin)
#else
    bdd_purge_entry(bddm, bin) cmu_bdd_manager bddm;
cache_entry *bin;
#endif
{
#if defined(__STDC__)
...
#else
    bdd_purge_lru(bddm, bin) cmu_bdd_manager bddm;
cache_entry *bin;
#endif
{
  if (bin[1])
...
#else
    bdd_get_entry(bddm, tag, bin) cmu_bdd_manager bddm;
int tag;
cache_entry *bin;
#endif
{
...
#else
    bdd_rehash1(bddm, p) cmu_bdd_manager bddm;
cache_entry p;
#endif
{
...
#else
    bdd_rehash2(bddm, p) cmu_bdd_manager bddm;
cache_entry p;
#endif
{
...
#else
    bdd_rehash3(bddm, p) cmu_bdd_manager bddm;
cache_entry p;
#endif
{
...
#else
    bdd_rehash_cache(bddm, grow) cmu_bdd_manager bddm;
int grow;
#endif
{
  long i;
  ...
  newtable = (cache_bin *)mem_get_block(
      (SIZE_T)(bddm->op_cache.size * sizeof(struct cache_bin_)));
...
#else
    bdd_insert_in_cache31(bddm, tag, d1, d2, d3, result) cmu_bdd_manager bddm;
int tag;
INT_PTR d1;
INT_PTR d2;
INT_PTR d3;
...
#else
    bdd_lookup_in_cache31(bddm, tag, d1, d2, d3, result) cmu_bdd_manager bddm;
int tag;
INT_PTR d1;
INT_PTR d2;
INT_PTR d3;
...
#endif
{
  ...
  if ((p = bin[0])) {
    q = CACHE_POINTER(p);
    if (q->slot[0] != d1 || q->slot[1] != d2 || q->slot[2] != d3 ||
        TAG(p) != tag)
      if ((p = bin[1])) {
        q = CACHE_POINTER(p);
        if (q->slot[0] != d1 || q->slot[1] != d2 || q->slot[2] != d3 ||
...
#else
    bdd_insert_in_cache22(bddm, tag, d1, d2, result1,
                          result2) cmu_bdd_manager bddm;
int tag;
INT_PTR d1;
INT_PTR d2;
INT_PTR result1;
...
#else
    bdd_lookup_in_cache22(bddm, tag, d1, d2, result1,
                          result2) cmu_bdd_manager bddm;
int tag;
INT_PTR d1;
INT_PTR d2;
...
#else
    bdd_insert_in_cache13(bddm, tag, d1, result1, result2,
                          result3) cmu_bdd_manager bddm;
int tag;
INT_PTR d1;
INT_PTR result1;
INT_PTR result2;
...
#else
    bdd_lookup_in_cache13(bddm, tag, d1, result1, result2,
                          result3) cmu_bdd_manager bddm;
int tag;
INT_PTR d1;
INT_PTR *result1;
...
#else
    cmu_bdd_ite_gc_fn(bddm, p) cmu_bdd_manager bddm;
cache_entry p;
#endif
{
  int i;
...
#else
    bdd_two_gc_fn(bddm, p) cmu_bdd_manager bddm;
cache_entry p;
#endif
{
  int i;
...
#else
    bdd_two_data_gc_fn(bddm, p) cmu_bdd_manager bddm;
cache_entry p;
#endif
{
  int i;
...
#else
    cmu_bdd_one_data_gc_fn(bddm, p) cmu_bdd_manager bddm;
cache_entry p;
#endif
{
  bdd f;
...
#else
    bdd_purge_cache(bddm) cmu_bdd_manager bddm;
#endif
{
  long i;
  int j;
...
void
#if defined(__STDC__)
        bdd_flush_cache(cmu_bdd_manager bddm, int (*flush_fn)(cmu_bdd_manager, cache_entry, pointer), pointer

closure)
#else
    bdd_flush_cache(bddm, flush_fn, closure) cmu_bdd_manager bddm;
int (*flush_fn)();
pointer closure;
#endif
{
...
#else
    bdd_flush_all(bddm) cmu_bdd_manager bddm;
#endif
{
  long i;
  int j;
...
#else
    bdd_cache_functions(bddm, args, gc_fn, purge_fn, return_fn,
                        flush_fn) cmu_bdd_manager bddm;
int args;
int (*gc_fn)();
void (*purge_fn)();
void (*return_fn)();
...
#else
    bdd_flush_tag(bddm, p, tag) cmu_bdd_manager bddm;
cache_entry p;
pointer tag;
#endif
{
...
#else
    cmu_bdd_free_cache_tag(bddm, tag) cmu_bdd_manager bddm;
int tag;
#endif
{
  if (tag < CACHE_TYPE_USER1 || tag >= CACHE_TYPE_USER1 + USER_ENTRY_TYPES ||
...
#else
    bdd_two_flush_fn(bddm, p, closure) cmu_bdd_manager bddm;
cache_entry p;
pointer closure;
#endif
{
...
#else
    cmu_bdd_init_cache(bddm) cmu_bdd_manager bddm;
#endif
{
  long i;
  int j;
  ...
  bddm->op_cache.size = TABLE_SIZE(bddm->op_cache.size_index);
  bddm->op_cache.table = (cache_bin *)mem_get_block(
      (SIZE_T)(bddm->op_cache.size * sizeof(cache_bin)));
  for (i = 0; i < bddm->op_cache.size; ++i)
    for (j = 0; j < 2; ++j)
...
#else
    cmu_bdd_free_cache(bddm) cmu_bdd_manager bddm;
#endif
{
  bdd_flush_all(bddm);
  mem_free_block((pointer)bddm->op_cache.table);
