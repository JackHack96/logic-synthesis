...
for (
i = CACHE_TYPE_USER1;
i < CACHE_TYPE_USER1+USER_ENTRY_TYPES; ++i)
if (!bddm->op_cache.rehash_fn[i])
break;
if (i == CACHE_TYPE_USER1+USER_ENTRY_TYPES)
return (-1);
bddm->op_cache.rehash_fn[i]=
rehash_fn;
bddm->op_cache.gc_fn[i]=
gc_fn;
bddm->op_cache.purge_fn[i]=
purge_fn;
bddm->op_cache.return_fn[i]=
return_fn;
bddm->op_cache.flush_fn[i]=
flush_fn;
return (i);
}
...
#endif
{
    ...
    bdd_flush_cache(bddm, bdd_flush_tag, (pointer)
    tag);
