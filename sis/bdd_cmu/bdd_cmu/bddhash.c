...
#else
    bdd_rehash_hash_table(h) hash_table h;
#endif
{
  long i;
  long hash;
...
#else
    bdd_insert_in_hash_table(h, f, data) hash_table h;
bdd f;
pointer data;
#endif
{
  ...
  p = (hash_rec)BDD_NEW_REC(h->bddm,
                            ALIGN(sizeof(struct hash_rec_)) + h->item_size);
  p->key = f;
  mem_copy((pointer)(ALIGN(sizeof(struct hash_rec_)) + (INT_PTR)p), data,
           (SIZE_T)h->item_size);
  hash = HASH(f);
  BDD_REDUCE(hash, h->size);
  p->next = h->table[hash];
...
#else
    bdd_lookup_in_hash_table(h, f) hash_table h;
bdd f;
#endif
{
  long hash;
...
#else
    bdd_new_hash_table(bddm, item_size) cmu_bdd_manager bddm;
int item_size;
#endif
{
  long i;
...
#else
    cmu_bdd_free_hash_table(h) hash_table h;
#endif
{
  long i;
  hash_rec p, q;
