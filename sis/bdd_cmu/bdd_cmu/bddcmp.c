...
#else
    bdd_fraction_compare(bddm, f, g) cmu_bdd_manager bddm;
bdd f;
bdd g;
#endif
{
...
static int
#if defined(__STDC__)
cmu_bdd_compare_step(cmu_bdd_manager bddm, bdd f, bdd g,
                     bdd_indexindex_type v_indexindex, long op)
#else
    cmu_bdd_compare_step(bddm, f, g, v_indexindex, op) cmu_bdd_manager bddm;
bdd f;
bdd g;
...
#else
    cmu_bdd_compare_temp(bddm, f, g, v) cmu_bdd_manager bddm;
bdd f;
bdd g;
bdd v;
#endif
{
  BDD_SETUP(v);
  return (cmu_bdd_compare_step(bddm, f, g, BDD_INDEXINDEX(v),
                               OP_CMPTO + BDD_INDEXINDEX(v)));
}
...
#else
    cmu_bdd_compare(bddm, f, g, v) cmu_bdd_manager bddm;
bdd f;
bdd g;
bdd v;
#endif
{
  if (bdd_check_arguments(3, f, g, v)) {
    ...
    if (cmu_bdd_type_aux(bddm, v) != BDD_TYPE_POSVAR) {
      cmu_bdd_warning(
          "cmu_bdd_compare: third argument is not a positive variable");
      return (0);
    }
    return (cmu_bdd_compare_step(bddm, f, g, BDD_INDEXINDEX(v),
                                 OP_CMPTO + BDD_INDEXINDEX(v)));
