...
#else
    cmu_bdd_one(bddm) cmu_bdd_manager bddm;
#endif
{
  return (BDD_ONE(bddm));
}
...
#else
    cmu_bdd_zero(bddm) cmu_bdd_manager bddm;
#endif
{
...
#else
    bdd_find_block(b, index) block b;
long index;
#endif
{
  long i, j, k;
...
#else
    bdd_block_delta(b, delta) block b;
long delta;
#endif
{
  long i;
...
#else
    shift_block(bddm, b, index) cmu_bdd_manager bddm;
block b;
long index;
#endif
{
  ...
  if (i == b->num_children || b->children[i]->first_index == index) {
    b->children = (block *)mem_resize_block(
        (pointer)b->children, (SIZE_T)(sizeof(block) * (b->num_children + 1)));
...
#else
    bdd_new_var(bddm, index) cmu_bdd_manager bddm;
...
#endif
{
  ...
  if (bddm->vars == bddm->maxvars) {
    ...
    bddm->maxvars = temp;
    bddm->variables = (bdd *)mem_resize_block(
        (pointer)bddm->variables, (SIZE_T)((bddm->maxvars + 1) * sizeof(bdd)));
    bddm->indexes = (bdd_index_type *)mem_resize_block(
        (pointer)bddm->indexes,
        (SIZE_T)((bddm->maxvars + 1) * sizeof(bdd_index_type)));
    bddm->indexindexes = (bdd_indexindex_type *)mem_resize_block(
        (pointer)bddm->indexindexes,
        (SIZE_T)(bddm->maxvars * sizeof(bdd_indexindex_type)));
    bddm->unique_table.tables = (var_table *)mem_resize_block(
        (pointer)bddm->unique_table.tables,
    ...
    for (p = bddm->assocs; p; p = p->next) {
      p->va.assoc = (bdd *)mem_resize_block(
          (pointer)p->va.assoc, (SIZE_T)((bddm->maxvars + 1) * sizeof(bdd)));
      for (i = oldmax; i < bddm->maxvars; ++i)
        p->va.assoc[i + 1] = 0;
    }
    bddm->temp_assoc.assoc =
        (bdd *)mem_resize_block((pointer)bddm->temp_assoc.assoc,
  ...
  /* Create the variable. */
  var = bdd_find_aux(bddm, (bdd_indexindex_type)bddm->vars,
                     (INT_PTR)BDD_ONE(bddm), (INT_PTR)BDD_ZERO(bddm));
...
#else
    cmu_bdd_new_var_first(bddm) cmu_bdd_manager bddm;
#endif
{
...
#else
    cmu_bdd_new_var_last(bddm) cmu_bdd_manager bddm;
#endif
{
...
#else
    cmu_bdd_new_var_before(bddm, var) cmu_bdd_manager bddm;
bdd var;
#endif
{
  if (bdd_check_arguments(1, var)) {
    ...
    if (cmu_bdd_type_aux(bddm, var) != BDD_TYPE_POSVAR) {
      cmu_bdd_warning(
          "cmu_bdd_new_var_before: argument is not a positive variable");
      if (BDD_IS_CONST(var))
...
#else
    cmu_bdd_new_var_after(bddm, var) cmu_bdd_manager bddm;
bdd var;
#endif
{
  if (bdd_check_arguments(1, var)) {
    ...
    if (cmu_bdd_type_aux(bddm, var) != BDD_TYPE_POSVAR) {
      cmu_bdd_warning(
          "cmu_bdd_new_var_after: argument is not a positive variable");
      if (BDD_IS_CONST(var))
        return (cmu_bdd_new_var_last(bddm));
    }
...
#else
    cmu_bdd_var_with_index(bddm, index) cmu_bdd_manager bddm;
long index;
#endif
{
...
#else
    cmu_bdd_var_with_id(bddm, indexindex) cmu_bdd_manager bddm;
long indexindex;
#endif
{
...
#else
    cmu_bdd_and_step(bddm, f, g) cmu_bdd_manager bddm;
bdd f;
bdd g;
#endif
{
  ...
  if (BDD_OUT_OF_ORDER(f, g))
    BDD_SWAP(f, g);
  if (bdd_lookup_in_cache31(bddm, CACHE_TYPE_ITE, (INT_PTR)f, (INT_PTR)g,
                            (INT_PTR)BDD_ZERO(bddm), (INT_PTR *)&result))
    return (result);
  ...
  bdd_insert_in_cache31(bddm, CACHE_TYPE_ITE, (INT_PTR)f, (INT_PTR)g,
                        (INT_PTR)BDD_ZERO(bddm), (INT_PTR)result);
  return (result);
}
...
#else
    cmu_bdd_xnor_step(bddm, f, g) cmu_bdd_manager bddm;
bdd f;
bdd g;
#endif
{
  ...
  if (bdd_lookup_in_cache31(bddm, CACHE_TYPE_ITE, (INT_PTR)f, (INT_PTR)g,
                            (INT_PTR)BDD_NOT(g), (INT_PTR *)&result))
...
#else
    cmu_bdd_ite_step(bddm, f, g, h) cmu_bdd_manager bddm;
bdd f;
bdd g;
bdd h;
#endif
...
#else
    cmu_bdd_ite(bddm, f, g, h) cmu_bdd_manager bddm;
bdd f;
bdd g;
bdd h;
#endif
...
#else
    cmu_bdd_and(bddm, f, g) cmu_bdd_manager bddm;
bdd f;
bdd g;
#endif
{
...
#else
    cmu_bdd_nand(bddm, f, g) cmu_bdd_manager bddm;
bdd f;
bdd g;
#endif
{
...
#else
    cmu_bdd_or(bddm, f, g) cmu_bdd_manager bddm;
bdd f;
bdd g;
#endif
{
...
#else
    cmu_bdd_nor(bddm, f, g) cmu_bdd_manager bddm;
bdd f;
bdd g;
#endif
{
...
#else
    cmu_bdd_xor(bddm, f, g) cmu_bdd_manager bddm;
bdd f;
bdd g;
#endif
{
...
#else
    cmu_bdd_xnor(bddm, f, g) cmu_bdd_manager bddm;
bdd f;
bdd g;
#endif
{
...
#else
    cmu_bdd_identity(bddm, f) cmu_bdd_manager bddm;
bdd f;
#endif
{
...
#else
    cmu_bdd_not(bddm, f) cmu_bdd_manager bddm;
bdd f;
#endif
{
  if (bdd_check_arguments(1, f)) {
...
#else
    cmu_bdd_if(bddm, f) cmu_bdd_manager bddm;
bdd f;
#endif
{
  if (bdd_check_arguments(1, f)) {
...
#else
    cmu_bdd_if_index(bddm, f) cmu_bdd_manager bddm;
bdd f;
#endif
{
  if (bdd_check_arguments(1, f)) {
...
#else
    cmu_bdd_if_id(bddm, f) cmu_bdd_manager bddm;
bdd f;
#endif
{
  if (bdd_check_arguments(1, f)) {
...
#else
    cmu_bdd_then(bddm, f) cmu_bdd_manager bddm;
bdd f;
#endif
{
  if (bdd_check_arguments(1, f)) {
...
#else
    cmu_bdd_else(bddm, f) cmu_bdd_manager bddm;
bdd f;
#endif
{
  if (bdd_check_arguments(1, f)) {
...
#else
    cmu_bdd_intersects_step(bddm, f, g) cmu_bdd_manager bddm;
bdd f;
bdd g;
#endif
{
  ...
  if (BDD_OUT_OF_ORDER(f, g))
    BDD_SWAP(f, g);
  if (bdd_lookup_in_cache31(bddm, CACHE_TYPE_ITE, (INT_PTR)f, (INT_PTR)g,
                            (INT_PTR)BDD_ZERO(bddm), (INT_PTR *)&temp))
    return (temp);
  BDD_TOP_VAR2(top_indexindex, bddm, f, g);
  BDD_COFACTOR(top_indexindex, f, f1, f2);
  BDD_COFACTOR(top_indexindex, g, g1, g2);
  ...
  temp = bdd_find(bddm, top_indexindex, BDD_ZERO(bddm),
                  cmu_bdd_intersects_step(bddm, f2, g2));
  if (temp == BDD_ZERO(bddm))
    bdd_insert_in_cache31(bddm, CACHE_TYPE_ITE, (INT_PTR)f, (INT_PTR)g,
                          (INT_PTR)BDD_ZERO(bddm), (INT_PTR)temp);
  return (temp);
...
#else
    cmu_bdd_intersects(bddm, f, g) cmu_bdd_manager bddm;
bdd f;
bdd g;
#endif
{
...
#else
    cmu_bdd_implies(bddm, f, g) cmu_bdd_manager bddm;
bdd f;
bdd g;
#endif
{
...
#else
    cmu_bdd_type_aux(bddm, f) cmu_bdd_manager bddm;
bdd f;
#endif
{
  BDD_SETUP(f);
...
#else
    cmu_bdd_type(bddm, f) cmu_bdd_manager bddm;
bdd f;
#endif
{
  if (bdd_check_arguments(1, f))
...
#else
    cmu_bdd_unfree(bddm, f) cmu_bdd_manager bddm;
bdd f;
#endif
{
  if (f) {
...
#else
    cmu_bdd_free(bddm, f) cmu_bdd_manager bddm;
bdd f;
#endif
{
  if (f) {
...
#else
    cmu_bdd_vars(bddm) cmu_bdd_manager bddm;
#endif
{
  return (bddm->vars);
}
...
#else
    cmu_bdd_total_size(bddm) cmu_bdd_manager bddm;
#endif
{
...
#else
    cmu_bdd_cache_ratio(bddm, new_ratio) cmu_bdd_manager bddm;
int new_ratio;
#endif
{
  int old_ratio;
...
#else
    cmu_bdd_node_limit(bddm, new_limit) cmu_bdd_manager bddm;
long new_limit;
#endif
{
  long old_limit;
...
#else
    cmu_bdd_overflow(bddm) cmu_bdd_manager bddm;
#endif
{
  int result;
...
overflow_env)
#else
    cmu_bdd_overflow_closure(bddm, overflow_fn,
                             overflow_env) cmu_bdd_manager bddm;
void (*overflow_fn)();
...
abort_env)
#else
    cmu_bdd_abort_closure(bddm, abort_fn, abort_env) cmu_bdd_manager bddm;
void (*abort_fn)();
pointer abort_env;
#endif
{
...
#else
    cmu_bdd_stats(bddm, fp) cmu_bdd_manager bddm;
FILE *fp;
#endif
{
  long i;
  ...
  mem += bddm->maxvars * (sizeof(bdd_index_type) + sizeof(bdd_indexindex_type) +
                          sizeof(bdd) + sizeof(var_table));
...
static int
#if defined(__STDC__)
bdd_default_canonical_fn(cmu_bdd_manager bddm, INT_PTR value1, INT_PTR value2,
                         pointer junk)
#else
    bdd_default_canonical_fn(bddm, value1, value2, junk) cmu_bdd_manager bddm;
INT_PTR value1;
INT_PTR value2;
pointer junk;
#endif
...
static void
#if defined(__STDC__)
bdd_default_transform_fn(cmu_bdd_manager bddm, INT_PTR value1, INT_PTR value2,
                         INT_PTR *result1, INT_PTR *result2, pointer junk)
#else
    bdd_default_transform_fn(bddm, value1, value2, result1, result2,
                             junk) cmu_bdd_manager bddm;
INT_PTR value1;
INT_PTR value2;
INT_PTR *result1;
INT_PTR *result2;
...
#endif
{
  ...
  bddm->check = 1;
  bddm->variables =
      (bdd *)mem_get_block((SIZE_T)((bddm->maxvars + 1) * sizeof(bdd)));
  bddm->indexes = (bdd_index_type *)mem_get_block(
      (SIZE_T)((bddm->maxvars + 1) * sizeof(bdd_index_type)));
  bddm->indexindexes = (bdd_indexindex_type *)mem_get_block(
      (SIZE_T)(bddm->maxvars * sizeof(bdd_indexindex_type)));
...
#else
    cmu_bdd_quit(bddm) cmu_bdd_manager bddm;
#endif
{
  int i;
  assoc_list p, q;
