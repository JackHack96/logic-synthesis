...
#endif
{
  ...
  if (b1->num_children) {
    ...
    if (i == j)
      ...
      if (start->first_index != b2->first_index ||
          end->last_index != b2->last_index)
        cmu_bdd_fatal("add_block: illegal block overlap");
      b2->num_children = j - i + 1;
      b2->children =
          (block *)mem_get_block((SIZE_T)(sizeof(block) * b2->num_children));
      ...
      b1->children = (block *)mem_resize_block(
          (pointer)b1->children, (SIZE_T)(sizeof(block) * b1->num_children));
    }
  } else {
    /* b1 and b2 are blocks representing just single variables. */
    b1->num_children = 1;
    b1->children =
        (block *)mem_get_block((SIZE_T)(sizeof(block) * b1->num_children));
    b1->children[0] = b2;
    b2->num_children = 0;
    b2->children = 0;
  }
...
#else
    cmu_bdd_new_var_block(bddm, v, n) cmu_bdd_manager bddm;
bdd v;
long n;
#endif
{
  ...
  if (bdd_check_arguments(1, v)) {
    ...
    if (cmu_bdd_type_aux(bddm, v) != BDD_TYPE_POSVAR) {
      cmu_bdd_warning(
          "cmu_bdd_new_var_block: second argument is not a positive variable");
      if (BDD_IS_CONST(v))
        return ((block)0);
    }
    ...
    if (b->last_index >= bddm->vars) {
      cmu_bdd_warning(
          "cmu_bdd_new_var_block: range covers non-existent variables");
