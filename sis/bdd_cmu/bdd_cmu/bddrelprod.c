...
static bdd
#if defined(__STDC__)
cmu_bdd_rel_prod_step(cmu_bdd_manager bddm, bdd f, bdd g, long op,
                      var_assoc vars)
#else
    cmu_bdd_rel_prod_step(bddm, f, g, op, vars) cmu_bdd_manager bddm;
bdd f;
bdd g;
long op;
var_assoc vars;
#endif
{
  ...
  if ((long)BDD_INDEX(bddm, f) > vars->last &&
      (long)BDD_INDEX(bddm, g) > vars->last)
...
#else
    cmu_bdd_rel_prod(bddm, f, g) cmu_bdd_manager bddm;
bdd f;
bdd g;
#endif
{
