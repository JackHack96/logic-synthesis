...
static bdd
#if defined(__STDC__)
cmu_bdd_swap_vars_aux_step(cmu_bdd_manager bddm, bdd f, bdd g, bdd h,
                           bdd_indexindex_type h_indexindex, long op)
#else
    cmu_bdd_swap_vars_aux_step(bddm, f, g, h, h_indexindex,
                               op) cmu_bdd_manager bddm;
bdd f;
bdd g;
bdd h;
...
#endif
{
  ...
  if (f_index >= bddm->indexes[h_indexindex] &&
      g_index >= bddm->indexes[h_indexindex]) {
    BDD_TEMP_INCREFS(f);
    BDD_TEMP_INCREFS(g);
...
static bdd
#if defined(__STDC__)
cmu_bdd_swap_vars_step(cmu_bdd_manager bddm, bdd f,
                       bdd_indexindex_type g_indexindex, bdd h, long op)
#else
    cmu_bdd_swap_vars_step(bddm, f, g_indexindex, h, op) cmu_bdd_manager bddm;
bdd f;
...
#endif
{
  ...
  if (f_index > bddm->indexes[g_indexindex]) {
    ...
    BDD_SETUP(h);
    temp1 = cmu_bdd_swap_vars_aux_step(bddm, BDD_THEN(f), BDD_ELSE(f), h,
                                       BDD_INDEXINDEX(h),
                                       OP_SWAPAUX + 2 * BDD_INDEXINDEX(h) + 1);
    temp2 = cmu_bdd_swap_vars_aux_step(bddm, BDD_THEN(f), BDD_ELSE(f), h,
                                       BDD_INDEXINDEX(h),
                                       OP_SWAPAUX + 2 * BDD_INDEXINDEX(h));
    result = bdd_find(bddm, g_indexindex, temp1, temp2);
  }
...
#else
    cmu_bdd_swap_vars_temp(bddm, f, g, h) cmu_bdd_manager bddm;
bdd f;
bdd g;
bdd h;
#endif
{
  ...
  if (g_index > h_index)
    return (cmu_bdd_swap_vars_step(bddm, f, BDD_INDEXINDEX(h), g,
                                   OP_SWAP + BDD_INDEXINDEX(h)));
  else
    return (cmu_bdd_swap_vars_step(bddm, f, BDD_INDEXINDEX(g), h,
                                   OP_SWAP + BDD_INDEXINDEX(g)));
}
...
#else
    cmu_bdd_swap_vars(bddm, f, g, h) cmu_bdd_manager bddm;
bdd f;
bdd g;
bdd h;
#endif
{
  ...
  if (bdd_check_arguments(3, f, g, h)) {
    ...
    if (cmu_bdd_type_aux(bddm, g) != BDD_TYPE_POSVAR ||
        cmu_bdd_type_aux(bddm, h) != BDD_TYPE_POSVAR) {
      cmu_bdd_warning("cmu_bdd_swap_vars: second and third arguments are not "
                      "both positive variables");
      BDD_INCREFS(f);
      return (f);
    }
    FIREWALL(bddm);
    ...
    if (g_index > h_index)
      RETURN_BDD(cmu_bdd_swap_vars_step(bddm, f, BDD_INDEXINDEX(h), g,
                                        OP_SWAP + BDD_INDEXINDEX(h)));
    else
      RETURN_BDD(cmu_bdd_swap_vars_step(bddm, f, BDD_INDEXINDEX(g), h,
                                        OP_SWAP + BDD_INDEXINDEX(g)));
  }
  return ((bdd)0);
}
