...
#endif
{
    ...
    if (f_index > bddm->indexes[g_indexindex]) {
        ...
        BDD_SETUP(h);
        temp1  = cmu_bdd_swap_vars_aux_step(bddm, BDD_THEN(f), BDD_ELSE(f), h, BDD_INDEXINDEX(h),
                                            OP_SWAPAUX + 2 * BDD_INDEXINDEX(h) + 1);
        temp2  = cmu_bdd_swap_vars_aux_step(bddm, BDD_THEN(f), BDD_ELSE(f), h, BDD_INDEXINDEX(h),
