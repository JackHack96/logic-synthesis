...
static void
#if defined(__STDC__)
bdd_print_top_var(cmu_bdd_manager bddm, bdd f,
                  ...
                  FILE *fp)
#else
    bdd_print_top_var(bddm, f, var_naming_fn, env, fp) cmu_bdd_manager bddm;
bdd f;
char *(*var_naming_fn)();
pointer env;
FILE *fp;
...
static void
#if defined(__STDC__)
cmu_bdd_print_bdd_step(cmu_bdd_manager bddm, bdd f,
                       char *(*var_naming_fn)(cmu_bdd_manager, bdd, pointer),

                       char *(*terminal_id_fn)(cmu_bdd_manager, INT_PTR,
                                               INT_PTR, pointer),
                       pointer

                           env,
                       FILE *fp, hash_table h, int indentation)
#else
    cmu_bdd_print_bdd_step(bddm, f, var_naming_fn, terminal_id_fn, env, fp, h,
                           indentation) cmu_bdd_manager bddm;
bdd f;
char *(*var_naming_fn)();
char *(*terminal_id_fn)();
pointer env;
...
#endif
{
  ...
  case BDD_TYPE_NONTERMINAL:
    ...
    if (number && *number < 0) {
      ...
      fputs("if ", fp);
      bdd_print_top_var(bddm, f, var_naming_fn, env, fp);
      cmu_bdd_print_bdd_step(bddm, BDD_THEN(f), var_naming_fn, terminal_id_fn,
                             env, fp, h, indentation + 2);
      chars(' ', indentation, fp);
      fputs("else if !", fp);
      bdd_print_top_var(bddm, f, var_naming_fn, env, fp);
      cmu_bdd_print_bdd_step(bddm, BDD_ELSE(f), var_naming_fn, terminal_id_fn,
                             env, fp, h, indentation + 2);
      chars(' ', indentation, fp);
      fputs("endif ", fp);
      bdd_print_top_var(bddm, f, var_naming_fn, env, fp);
  ...
  default:
    cmu_bdd_fatal(
        "cmu_bdd_print_bdd_step: unknown type returned by cmu_bdd_type");
  }
}
...
#else
    cmu_bdd_print_bdd(bddm, f, var_naming_fn, terminal_id_fn, env,
