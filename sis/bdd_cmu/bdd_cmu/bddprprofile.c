...
*fp)
#else
    cmu_bdd_print_profile_aux(bddm, level_counts, var_naming_fn, env,
                              line_length, fp) cmu_bdd_manager bddm;
long *level_counts;
char *(*var_naming_fn)();
pointer env;
int line_length;
...
#endif
{
  ...
  for (i = 0; i < n; ++i)
    if (level_counts[i]) {
      ...
      l = strlen(bdd_var_name(bddm, bddm->variables[bddm->indexindexes[i]],
                              var_naming_fn, env)) +
          strlen(profile_width);
      if (l > max_prefix_len)
        max_prefix_len = l;
  ...
  for (i = 0; i < n; ++i)
    if (level_counts[i]) {
      name = bdd_var_name(bddm, bddm->variables[bddm->indexindexes[i]],
                          var_naming_fn, env);
      fputs(name, fp);
      fputc(':', fp);
      sprintf(profile_width, "%d", level_counts[i]);
      chars(' ',
            (int)(max_prefix_len - strlen(name) - strlen(profile_width) + 1),
            fp);
...
*fp)
#else
    cmu_bdd_print_profile(bddm, f, var_naming_fn, env, line_length,
                          fp) cmu_bdd_manager bddm;
bdd f;
char *(*var_naming_fn)();
pointer env;
int line_length;
...
#endif
{
  ...
  if (bdd_check_arguments(1, f)) {
    level_counts =
        (long *)mem_get_block((SIZE_T)((bddm->vars + 1) * sizeof(long)));
...
/* cmu_bdd_print_profile_multiple is like cmu_bdd_print_profile except it
 * displays */
...
*fp)
#else
    cmu_bdd_print_profile_multiple(bddm, fs, var_naming_fn, env, line_length,
                                   fp) cmu_bdd_manager bddm;
bdd *fs;
char *(*var_naming_fn)();
pointer env;
...
#endif
{
  ...
  cmu_bdd_print_profile_aux(bddm, level_counts, var_naming_fn, env, line_length,
                            fp);
  mem_free_block((pointer)level_counts);
}

/* cmu_bdd_print_function_profile is like cmu_bdd_print_profile except it
 * displays */
...
*fp)
#else
    cmu_bdd_print_function_profile(bddm, f, var_naming_fn, env, line_length,
                                   fp) cmu_bdd_manager bddm;
bdd f;
char *(*var_naming_fn)();
pointer env;
...
/* cmu_bdd_print_function_profile_multiple is like
 * cmu_bdd_print_function_profile */
...
*fp)
#else
    cmu_bdd_print_function_profile_multiple(bddm, fs, var_naming_fn, env,
                                            line_length,
                                            fp) cmu_bdd_manager bddm;
bdd *fs;
char *(*var_naming_fn)();
