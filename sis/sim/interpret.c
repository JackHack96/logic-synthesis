...
void simulate_node(node_t *node) {
    int    i, value;
...
array_t *simulate_network(network_t *network, array_t *node_vec, array_t *in_value) {
    int     i, value;
    node_t  *node, *pi, *po;
    array_t *po_value;
    lsGen   gen;
...
array_t *simulate_stg(graph_t *stg, array_t *in_value, vertex_t **state) {
...
int sim_verify(network_t *network0, network_t *network1, int n_patterns) {
    int       i, j, k;
...
static void report_error(int *pi_values, int **po_values, network_t **networks, int faulty_po) {
    int i;
    int diff;
    ...
    (void) fprintf(misout, "\n");
    (void) fprintf(misout, "Note that the inputs and outputs are matched up by "
                           "order, not by name\n");
}

static void print_pattern(int n, int *pattern, int mask) {
    int i;
    for (i = 0; i < n; i++)
...
static void gen_random_pattern(int n, int *pattern, int seed) {
    int i;
    srandom(seed);
    for (i = 0; i < n; i++) {
...
static void simulate_network_32(network_t *network, int *pi_values, int *po_values) {
    int      i;
    lsGen    gen;
    node_t   *pi, *po;
...
static int simulate_network_32_rec(network_t *network, st_table *table, node_t *node) {
    int  i;
    char *value;
    int  result;
...
static int simulate_node_32(node_t *node, int **inputs) {
    register int i, j;
    node_t       *fanin;
    node_cube_t  cube;
