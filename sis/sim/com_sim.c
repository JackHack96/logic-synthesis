...
static void dump_outputs(FILE *fp, network_t *n, array_t *vec) {
    int     i;
    latch_t *l;
    lsGen   gen;
    node_t  *po;
...
static int com_simulate(network_t **network, int argc, char **argv) {
    int      i, j;
    int      c;
    int      do_logic = 1;
    int      do_stg   = 1;
    ...
    if (do_logic) {
        if (network_num_pi(*network) != 0) {
            if (argc - util_optind !=
                (network_num_pi(*network) - network_num_latch(*network))) {
                (void) fprintf(miserr,
                               "simulate network: network has %d inputs; %d values were "
                               "supplied.\n",
                               network_num_pi(*network) - network_num_latch(*network),
                               argc - 1);
                goto usage;
            ...
            foreach_primary_input(*network, gen, pi) {
                if ((l = latch_from_node(pi)) != NIL(latch_t)) {
                    ...
                    if ((value != 0) && (value != 1)) {
                        (void) fprintf(
                                siserr,
                                "Some latch has the value 'don't care' or 'undefined'.\n");
                        (void) fprintf(
                                siserr, "Use set_state to put the network in a valid state.\n");
                        array_free(in_value);
                        lsFinish(gen);
                        return 1;
                    }
            ...
            foreach_primary_output(*network, gen, po) {
                if ((l = latch_from_node(po)) != NIL(latch_t)) {
                    ...
                    if (control == NIL(node_t)) {
                        ...
                        (void) st_lookup_int(control_table, (char *) control, &control_value);
                        if (control_value) {
                            latch_set_current_value(l, value);
                        }
                    }
    ...
    if (do_stg) {
        if ((stg = network_stg(*network)) != NIL(graph_t)) {
            if (argc - util_optind != stg_get_num_inputs(stg)) {
                (void) fprintf(
                        miserr,
                        "simulate stg: stg has %d inputs; %d values were supplied.\n",
            ...
            for (i   = util_optind; i < stg_get_num_inputs(stg) + util_optind; i++) {
                if (strcmp(argv[i], "0") == 0) {
                    ...
                    (void) fprintf(miserr,
                                   "simulate stg: bad value '%s' -- should be 0 or 1\n",
                                   argv[i]);
                    array_free(in_value);
                    goto usage;
                }
            ...
            (void) fprintf(misout, "\nNext state: %s (%s)\n\n",
                           stg_get_state_name(state), stg_get_state_encoding(state));
            array_free(in_value);
            array_free(out_char);
        }
    }
...
static int com_set_state(network_t **network, int argc, char **argv) {
    int      do_stg   = 1;
    int      do_logic = 1;
    char     *state_name;
    char     *temp;
    ...
    if (do_logic) {
        if (network_num_latch(*network) != 0) {
            if (strlen(state_name) != network_num_latch(*network)) {
                (void) fprintf(
                        miserr,
                        "set_state: network had %d latches; %d values were supplied.\n",
            ...
            for (i = 0; i < strlen(state_name); i++) {
                if ((state_name[i] != '0') && (state_name[i] != '1')) {
                    (void) fprintf(miserr,
                                   "set_state: bad value '%c' -- should be 0 or 1\n", c);
                    goto usage;
                }
            }
            temp   = state_name;
    ...
    if (do_stg) {
        if ((stg = network_stg(*network)) != NIL(graph_t)) {
            if (encoding) {
                if ((v = stg_get_state_by_encoding(stg, state_name)) != NIL(vertex_t)) {
                    stg_set_current(stg, v);
                } else {
                    (void) fprintf(
                            miserr,
                            "set_state: state with encoding name %s not found in stg\n",
                            state_name);
                    goto usage;
                }
            } else {
                if ((v = stg_get_state_by_name(stg, state_name)) != NIL(vertex_t)) {
                    stg_set_current(stg, v);
                } else {
                    (void) fprintf(
                            miserr,
                            "set_state: state with symbolic name %s not found in stg\n",
                            state_name);
                    goto usage;
                }
            }
...
/*ARGSUSED*/
static int com_print_state(network_t **network, int argc, char **argv) {
    lsGen    gen;
    latch_t  *l;
    graph_t  *stg;
    vertex_t *v;
...
static int dump_vector(FILE *fp, array_t *vec)
{
  int i;
...
static int com_simulate(network_t **network, int argc, char **argv)
{
  ...
  for (i = 1; i < argc; i++) {
    if (strcmp(argv[i], "0") == 0) {
      ...
      (void)fprintf(miserr, "simulate: bad value '%s' -- should be 0 or 1\n",
                    argv[i]);
      array_free(in_value);
      goto usage;
    }
...
static int com_sim_verify(network_t **network, int argc, char **argv) {
    int       c;
    int       n_patterns = 1024;
    network_t *network1;
    char      buffer[1024];
    ...
    while ((c = util_getopt(argc, argv, "n:")) != EOF) {
        switch (c) {
            case 'n':n_patterns = atoi(util_optarg);
                if (n_patterns < 1)
                    goto usage;
                if (n_patterns % 32 != 0)
