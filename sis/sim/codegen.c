...
int sim_verify_codegen(network_t *network1, network_t *network2, int num, char *root) {
    FILE   *fp, *fpread;
    ...
    foreach_primary_input(network1, gen1, node1) {
        ...
        if (j == network_num_pi(network2)) {
            (void) fprintf(miserr, "No match for input '%s' in network2\n",
                           node_name(node1));
            return 0;
        }
        i++;
    ...
    foreach_primary_output(network1, gen1, node1) {
        ...
        if (j == network_num_po(network2)) {
            (void) fprintf(miserr, "No match for output '%s' in network2\n",
                           node_name(node1));
            return 0;
        }
        i++;
...
static void sim_gen_code(FILE *fp, network_t *network, char *func_name) {
                    ...
                    foreach_fanin(node, k, fanin) {
                        ...
                        switch (literal) {
                            case ONE:
                                if (!first_literal)
                                    (void) fprintf(fp, "&");
                                (void) fprintf(fp, "a[%d]", nodeindex_indexof(table, fanin));
                                first_literal = 0;
                                break;
                            case ZERO:
                                if (!first_literal)
