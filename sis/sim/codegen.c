...
if (
network_num_pi(network1)
!=
network_num_pi(network2)
) {
(void)
fprintf(miserr,
"Number of inputs do not agree\n");
return 0;
}

if (
network_num_po(network1)
!=
network_num_po(network2)
) {
(void)
fprintf(miserr,
"Number of outputs do not agree\n");
return 0;
}
...
sim_gen_code(fp, network1,
"func1");
sim_gen_code(fp, network2,
"func2");
...
fpread = com_open_file("driver.c", "r", NIL(
char *), /* silent */ 0);
            ...
            if (node_function(node) == NODE_0) {
                ...
                for (j = node_num_cube(node) - 1; j >= 0; j--) {
                    ...
                    foreach_fanin(node, k, fanin)
                    {
                        ...
                        switch (literal) {
                            case ONE: if (!first_literal) (void) fprintf(fp, "&");
                                (void) fprintf(fp, "a[%d]",
                                               nodeindex_indexof(table, fanin));
                                first_literal = 0;
                                break;
                            case ZERO: if (!first_literal) (void) fprintf(fp, "&");
                                (void) fprintf(fp, "~a[%d]",
                                               nodeindex_indexof(table, fanin));
                                first_literal = 0;
                                break;
