...
#define COMPARE(key, nodekey, compare)                                         \
  ((compare == fast_avl_numcmp) ? (int)key - (int)nodekey                      \
                                : (*compare)(key, nodekey))
...
fast_avl_tree *fast_avl_init_tree(compar) int (*compar)();
{
  fast_avl_tree *tree;
...
fast_avl_lookup(tree, key, value_p) fast_avl_tree *tree;
char *key;
char **value_p;
{
  fast_avl_node *node;
...
fast_avl_insert(tree, key, value) fast_avl_tree *tree;
char *key;
char *value;
{
...
static void fast_avl_record_gen_forward(node, gen) fast_avl_node *node;
...
static void fast_avl_record_gen_backward(node, gen) fast_avl_node *node;
fast_avl_generator *gen;
{
  if (node != NIL(fast_avl_node)) {
...
static fast_avl_nodelist *fast_avl_nodelist_alloc(count) int count;
{
...
fast_avl_generator *fast_avl_init_gen(tree, dir) fast_avl_tree *tree;
int dir;
{
  int count;
  fast_avl_generator *gen;
...
fast_avl_gen(gen, key_p, value_p) fast_avl_generator *gen;
char **key_p;
char **value_p;
{
  fast_avl_node *node;
...
void fast_avl_free_gen(gen) fast_avl_generator *gen;
{
  fast_avl_nodelist_free(gen->nodelist);
  fast_avl_generator_free(gen);
}

static void do_rebalance(stack_nodep, stack_n) fast_avl_node ***stack_nodep;
int stack_n;
{
  fast_avl_node **node_p, *node;
  int hl, hr;
...
static rotate_left(node_p) fast_avl_node **node_p;
{
...
static rotate_right(node_p) fast_avl_node **node_p;
{
...
static void fast_avl_walk_forward(node, func) fast_avl_node *node;
void (*func)();
{
  if (node != NIL(fast_avl_node)) {
...
static void fast_avl_walk_backward(node, func) fast_avl_node *node;
void (*func)();
{
  if (node != NIL(fast_avl_node)) {
...
void fast_avl_foreach(tree, func, direction) fast_avl_tree *tree;
void (*func)();
int direction;
{
...
static void free_entry(node, key_free, value_free) fast_avl_node *node;
void (*key_free)();
void (*value_free)();
{
  if (node != NIL(fast_avl_node)) {
...
void fast_avl_free_tree(tree, key_free, value_free) fast_avl_tree *tree;
void (*key_free)();
void (*value_free)();
{
...
static fast_avl_node *new_node(key, value) char *key;
char *value;
{
  fast_avl_node *new;
...
int fast_avl_numcmp(x, y) char *x, *y;
{ return (int)x - (int)y; }

int fast_avl_check_tree(tree) fast_avl_tree *tree;
{
  int error = 0;
...
static int do_check_tree(node, compar, error) fast_avl_node *node;
int (*compar)();
int *error;
{
...
void fast_avl_cleanup() {
  ...
  for (e = fast_avl_nodelist_freelist; e != NIL(fast_avl_nodelist); e = enext) {
    enext = e->next;
    FREE(e->arr);
    FREE(e);
  }
