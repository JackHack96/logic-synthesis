...
static void add_sum(sat_prob, literals, nlits, out_var) sat_t *sat_prob;
int *literals;
int nlits;
int out_var;
{
  ...
  switch (nlits) {
  case 0:
    sat_add_1clause(sat_prob, sat_neg(out_var));
    break;
  case 1:
    sat_add_2clause(sat_prob, literals[0], sat_neg(out_var));
    break;
  default:
...
/* gates of fanin > 1 */
static void atpg_sop_clauses(sat_prob, node, output_var) sat_t *sat_prob;
node_t *node;
int output_var;
{
  int lit;
  ...
  for (i = n_cubes; i--;) {
    ...
    if (n_lit == 1) {
      ...
      lit = (n_cubes == 1) ? output_var : sat_new_variable(sat_prob);
...
void atpg_node_clause(node, sat_prob) node_t *node;
sat_t *sat_prob;
{
  int in_var, out_var;
...
static void fault_path_outputs(node, fault_tfo, cone_heads) node_t *node;
array_t *fault_tfo;
array_t *cone_heads;
{
  atpg_clause_t *node_info;
...
static void tfanin_cone(node, sat_prob, pi_list) node_t *node;
sat_t *sat_prob;
array_t *pi_list;
{
  atpg_clause_t *node_info;
...
static void exdc_tfanin_cone(node, sat_prob, network, pi_list) node_t *node;
sat_t *sat_prob;
network_t *network;
array_t *pi_list;
{
...
static void atpg_gen_fault_clauses(sat_prob, fault_shadow) sat_t *sat_prob;
...
static void atpg_gen_fault_site(sat_prob, fault) sat_t *sat_prob;
fault_t *fault;
{
...
static void atpg_gen_active_clauses(sat_prob, fault_shadow) sat_t *sat_prob;
array_t *fault_shadow;
{
  int i, n;
  atpg_clause_t *node_info;
  ...
  for (i = 0, n = array_n(fault_shadow); i < n; i++) {
    ...
    sat_add_3clause(sat_prob, sat_neg(node_info->active_id), node_info->true_id,
                    node_info->fault_id);
    ...
    if (node_function(node) != NODE_PO) {
      ...
      if (node_num_fanout(node) == 1) {
        ...
        foreach_fanout(node, gen, fanout) {
          sat_add_literal(sat_prob, ATPG_CLAUSE_GET(fanout)->active_id);
        }
      }
    }
    if (i == (n - 1)) {
...
int atpg_network_fault_clauses(info, exdc_info, seq_info,
                               fault) atpg_ss_info_t *info;
...
fault_t *fault;
{
  ...
  for (i = 0; i < array_n(cone_heads); i++) {
    tfanin_cone(array_fetch(node_t *, cone_heads, i), info->atpg_sat, pi_list);
  }
