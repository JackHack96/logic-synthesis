...
static int *c_sum_literals = NIL(
int);
static int *c_cube_literals = NIL(
int);
        ...
        int out_var;
{
    ...
    switch (nlits) {
        ...
        case 1: sat_add_2clause(sat_prob, literals[0], sat_neg(out_var));
            break;
        default: sat_begin_clause(sat_prob);
            for (i = 0; i < nlits; i++) {
        ...
        int output_var;
{
    ...
    if (n_cubes > c_max_cubes) {
        c_max_cubes    = n_cubes;
        c_sum_literals = REALLOC(
        int, c_sum_literals, c_max_cubes);
    }
    i         = node_num_fanin(node);
    if (i > c_max_fanin) {
        c_max_fanin     = i;
        c_cube_literals = REALLOC(
        int, c_cube_literals, c_max_fanin);
    }
        ...
        sat_t *sat_prob;
{
    ...
    switch (node_function(node)) {
        case NODE_0: sat_add_1clause(sat_prob, sat_neg(ATPG_GET_VARIABLE(node)));
            break;
        case NODE_1: sat_add_1clause(sat_prob, ATPG_GET_VARIABLE(node));
            break;
        case NODE_PI: break;
        case NODE_PO:
        case NODE_BUF: in_var = ATPG_GET_VARIABLE(node_get_fanin(node, 0));
            ...
            break;
        case NODE_INV: in_var = ATPG_GET_VARIABLE(node_get_fanin(node, 0));
            out_var           = ATPG_GET_VARIABLE(node);
        ...
        case NODE_AND:
        case NODE_OR:
        case NODE_COMPLEX: atpg_sop_clauses(sat_prob, node, ATPG_GET_VARIABLE(node));
            break;
        default: fail("illegal node function");
    }
}
