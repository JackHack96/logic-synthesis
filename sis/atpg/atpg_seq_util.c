        ...
        st_table *table;
{
    ...
    for (i = 0; i < array_n(vars); i++) {
        varid = array_fetch(
        int, ids, i);
        st_insert(table, (char *) varid, NIL(
        char));
    }
    array_free(ids);
}
        ...
        array_t *faulty_state;
{
    ...
    foreach_latch(product_network, gen, latch)
    {
        if (i < state_length) {
            ...
            value = array_fetch(
            int, faulty_state, i - state_length);
        }
        i++;
        switch (value) {
            case 0:
            case 1: n1 = latch_get_output(latch);
                ...
                break;
            case 2: break;
            default: fail("latch values were not in set {0,1,2}");
                break;
        }
    }
