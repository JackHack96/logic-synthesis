...
void free_bdds_in_array(bdd_array) array_t *bdd_array;
{
  int i;
  bdd_t *fn;
...
network_t *convert_bdd_to_network(seq_info, fn) seq_info_t *seq_info;
bdd_t *fn;
{
 ...
 */
int convert_bdd_to_int(state_fn) bdd_t *state_fn;
{
  int state_int;
...
int convert_product_bdd_to_key(info, seq_info, state_fn) atpg_info_t *info;
seq_info_t *seq_info;
bdd_t *state_fn;
{
  bdd_gen *gen;
  ...
  array_t *latch_to_product_pi_ordering =
      seq_info->latch_to_product_pi_ordering;
...
st_table *get_pi_to_var_table(manager, info) bdd_manager *manager;
output_info_t *info;
{
  int index;
  node_t *pi;
...
bdd_t *seq_get_one_minterm(manager, fn, vars) bdd_manager *manager;
bdd_t *fn;
st_table *vars;
{
  /*    bdd_gen *gen;
...
bdd_t *find_good_constraint(current_set, total_set,
...
void use_cofactored_set(current_set, total_set,
                        cofactor_fn) bdd_t **current_set;
bdd_t **total_set;
BddFn cofactor_fn;
{
...
void bdd_add_varids_to_table(vars, table) array_t *vars;
st_table *table;
{
  int i;
  int varid;
...
bdd_t *convert_states_to_product_bdd(seq_info, good_state,
                                     faulty_state) seq_info_t *seq_info;
array_t *good_state;
array_t *faulty_state;
{
  ...
  init_state = ntbdd_node_to_local_bdd(init_node, data->manager,
                                       options->output_info->pi_ordering);
  network_delete_node(product_network, init_node);
  return init_state;
}

bdd_t *convert_state_to_bdd(seq_info, state) seq_info_t *seq_info;
array_t *state;
{
...
int derive_prop_key(seq_info) seq_info_t *seq_info;
{
...
int derive_inverted_prop_key(seq_info) seq_info_t *seq_info;
{
  array_t *good_state = seq_info->good_state;
  array_t *faulty_state = seq_info->faulty_state;
  int state_length, i, gval, fval, good_state_int, faulty_state_int, key;
