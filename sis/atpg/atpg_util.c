        ...
        int npi;
{
    ...
    if ((n_old_vectors + n_new_vectors) > array_n(just_sequence)) {
        for (i = n_old_vectors + n_new_vectors - array_n(just_sequence); i--;) {
            ...
            unsigned, npi);
            array_insert_last(
            unsigned *, just_sequence, just_vector);
        }
    }
    for (i        = n_new_vectors; i--;) {
        just_vector = array_fetch(
        unsigned *, just_sequence, i);
        tmp_vector = array_fetch(
        unsigned *, tmp_sequence, i);
        for (j = npi; j--;) {
    ...
    for (i = n_new_vectors; i--;) {
        just_vector = array_fetch(
        unsigned *, just_sequence, i + n_old_vectors);
        tmp_vector = array_fetch(
        unsigned *, tmp_sequence, i);
        for (j = npi; j--;) {
        ...
        int cnt;
{
    ...
    unsigned *, n_just_vectors + n_prop_vectors);
    ...
    for (i = n_prop_vectors; i--;) {
        ...
        array_insert(
        unsigned *, vectors, i + n_just_vectors, vector);
    }
        ...
        int npi;
{
    ...
    for (i = n_vectors; i--;) {
        ...
        array_insert_last(
        unsigned *, word_vectors, new_vector);
    }
}
