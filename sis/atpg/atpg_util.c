...
static int order_cmp(obj1, obj2) char *obj1;
char *obj2;
{
  node_index_t *n1, *n2;
...
int *fanin_dfs_sort(node, fanin_ptr) node_t *node;
...
node_t **sat_fanout_dfs_sort(node) node_t *node;
{
  ...
  qsort((void *)fanout_ptr, node_num_fanout(node), sizeof(node_t *), order_cmp);
  return fanout_ptr;
}

void concat_lists(l1, l2) lsList l1;
lsList l2;
{
  lsGeneric data;
...
void create_just_sequence(seq_info, n_new_vectors, old_vectors,
                          npi) seq_info_t *seq_info;
int n_new_vectors;
array_t *old_vectors;
int npi;
{
...
void atpg_derive_excitation_vector(info, n_pi_vars,
                                   vector) atpg_ss_info_t *info;
int n_pi_vars;
unsigned *vector;
{
  int i, v, ind;
...
void atpg_print_fault(fp, fault) FILE *fp;
fault_t *fault;
{
...
void atpg_print_vectors(fp, vectors, n_inputs) FILE *fp;
array_t *vectors;
int n_inputs;
{
  int i, j, seq_length;
...
void atpg_print_some_vectors(fp, vectors, n_inputs, seq_length) FILE *fp;
array_t *vectors;
int n_inputs;
int seq_length;
{
...
void atpg_print_bdd(bdd_fn, message) bdd_t *bdd_fn;
char *message;
{
  fprintf(sisout, "%s: \n", message);
  bdd_print(bdd_fn);
...
void atpg_print_results(info, seq_info) atpg_info_t *info;
seq_info_t *seq_info;
{
...
static void print_time_info(time_info) time_info_t *time_info;
{
  long total_time = time_info->total_time;

  fprintf(sisout, "setup time:\t\t\t%.2f\t%.2f%%\n",
          (time_info->setup) / 1000.0, 100.0 * time_info->setup / total_time);
...
sequence_t *derive_test_sequence(ss_info, seq_info, n_just_vectors,
                                 n_prop_vectors, npi,
...
int cnt;
{
  ...
  if (array_n(word_vectors) < (n_just_vectors + n_prop_vectors)) {
    lengthen_word_vectors(
        ss_info, n_just_vectors + n_prop_vectors - array_n(word_vectors),
        ss_info->n_real_pi);
  }
...
void reset_word_vectors(info) atpg_ss_info_t *info;
{
...
void lengthen_word_vectors(info, n_vectors, npi) atpg_ss_info_t *info;
int n_vectors;
int npi;
{
  array_t *word_vectors = info->word_vectors;
...
void fillin_word_vectors(info, seq_length, npi) atpg_ss_info_t *info;
int seq_length;
int npi;
{
  int h, i, j;
