...
void seq_setup(seq_info, info) seq_info_t *seq_info;
atpg_info_t *info;
{
  range_data_t *data;
...
void seq_product_setup(info, seq_info, network) atpg_info_t *info;
seq_info_t *seq_info;
network_t *network;
{
  ...
  extract_product_network_info(network_copy, seq_info->product_network,
                               options);
  ...
  if (info->atpg_opt->use_internal_states) {
    ...
    foreach_latch(seq_info->product_network, gen, l) {
      ...
      if (i < info->n_latch) {
        array_insert(int, seq_info->latch_to_product_pi_ordering, i++,
                     position);
      }
    }
  }
...
static void set_options(options) verif_options_t *options;
{
  options->verbose = 0;
...
void copy_orig_bdds(seq_info) seq_info_t *seq_info;
{
  int i;
...
void record_reset_state(seq_info, info) seq_info_t *seq_info;
atpg_info_t *info;
{
...
void construct_product_start_states(seq_info, n_latch) seq_info_t *seq_info;
int n_latch;
{
  ...
  seq_info->product_start_states =
      bdd_zero(seq_info->product_range_data->manager);
...
static void
    add_minterms_to_product_start_states(seq_info, state_cube,
                                         n_to_check) seq_info_t *seq_info;
array_t *state_cube;
int n_to_check;
{
  int i, val;
  ...
  new_product_start_states =
      bdd_or(seq_info->product_start_states, minterm_bdd, 1, 1);
  bdd_free(minterm_bdd);
  bdd_free(seq_info->product_start_states);
  seq_info->product_start_states = new_product_start_states;
}

bool calculate_reachable_states(seq_info) seq_info_t *seq_info;
{
  int i;
...
bdd_t *seq_derive_excitation_states(info, seq_info,
                                    n_pi_vars) atpg_ss_info_t *info;
seq_info_t *seq_info;
int n_pi_vars;
{
  ...
  excite_states = ntbdd_node_to_local_bdd(excite_node, data->manager,
                                          options->output_info->pi_ordering);
  network_delete_node(network_copy, excite_node);
  st_free_table(table);
  return excite_states;
}

int seq_reuse_just_sequence(n_inputs, n_latch, seq_info,
                            already_justified) int n_inputs;
int n_latch;
seq_info_t *seq_info;
bdd_t *already_justified;
{
  ...
  minterm = seq_get_one_minterm(range_data->manager, already_justified,
                                seq_info->var_table);
...
int seq_state_justify(info, seq_info, excite_states) atpg_info_t *info;
seq_info_t *seq_info;
bdd_t *excite_states;
{
  int i, stg_depth;
  ...
  for (i = 1; i < stg_depth; i++) {
    reached_excite_states =
        bdd_and(excite_states, array_fetch(bdd_t *, reached_sets, i), 1, 1);
...
int internal_states_seq_state_justify(info, seq_info,
                                      excite_states) atpg_info_t *info;
seq_info_t *seq_info;
bdd_t *excite_states;
{
  int i;
...
static void reverse_generate_Sj(info, seq_info, depth,
                                reached_excite_states) atpg_info_t *info;
seq_info_t *seq_info;
int depth;
bdd_t *reached_excite_states;
{
  ...
  minterm = seq_get_one_minterm(range_data->manager, reached_excite_states,
                                seq_info->var_table);
  state_justified = bdd_smooth(minterm, seq_info->real_pi_bdds);
  new_justified_states =
      bdd_or(seq_info->justified_states, state_justified, 1, 1);
  ...
  for (i = depth; i--;) {
    preimage =
        product_compute_reverse_image(state_justified, range_data, options);
    bdd_free(state_justified);
    support_reached =
        bdd_and(preimage, array_fetch(bdd_t *, reached_sets, i), 1, 1);
    bdd_free(preimage);
    minterm = seq_get_one_minterm(range_data->manager, support_reached,
                                  seq_info->var_table);
...
static void convert_trace_to_sequence(info, seq_info, table, sequence,
                                      input_vars, seq_length,
                                      key) atpg_info_t *info;
seq_info_t *seq_info;
st_table *table;
array_t *sequence;
array_t *input_vars;
...
int seq_reuse_prop_sequence(npi, seq_info, key) int npi;
seq_info_t *seq_info;
int key;
{
  ...
  if (old_prop_sequence == NIL(array_t))
    return 0;
...
int seq_fault_free_propagate(info, seq_info) atpg_info_t *info;
seq_info_t *seq_info;
{
  bdd_t *init_state;
...
int traverse_product_machine(info, seq_info, init_state,
                             prop) atpg_info_t *info;
seq_info_t *seq_info;
bdd_t *init_state;
bool prop;
{
  ...
  bdd_t *current_set, *total_set, *new_current_set, *output_fn,
      *incorrect_support, *incorrect_input, *incorrect_state, *tmp, *minterm;
  ...
  for (;;) {
    ...
    if (bdd_is_tautology(total_set, 1))
      return 0;
    new_current_set =
        product_compute_next_states(current_set, range_data, options);
  ...
  if (prop) {
    ...
    convert_trace_to_sequence(
        info, seq_info, seq_info->prop_sequence_table, seq_info->prop_sequence,
        seq_info->product_input_vars, seq_length, init_state_key);
  } else {
...
static int seq_check_output(current_set, data, output_index) bdd_t *current_set;
range_data_t *data;
int *output_index;
{
  int i;
...
*/
int good_faulty_PMT(fault, info, seq_info) fault_t *fault;
atpg_info_t *info;
seq_info_t *seq_info;
{
  range_data_t *range_data = seq_info->product_range_data;
  ...
  for (i = 0; i < array_n(output_info->xnor_nodes); i++) {
    ...
    output =
        ntbdd_node_to_bdd(node, range_data->manager, output_info->pi_ordering);
  ...
  if (info->atpg_opt->use_internal_states) {
    n_prop_vectors = traverse_product_machine(
        info, seq_info, seq_info->product_start_states, FALSE);
  } else {
...
static void insert_fault(network, fault, saved_nodes) network_t *network;
fault_t *fault;
saved_nodes_t *saved_nodes;
{
  int n_fanout, i;
...
static void remove_fault(network, saved_nodes) network_t *network;
saved_nodes_t *saved_nodes;
{
  int i;
