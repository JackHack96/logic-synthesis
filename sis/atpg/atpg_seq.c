        ...
        atpg_info_t *info;
{
    ...
    seq_info->good_state   = array_alloc(
    int, info->n_latch);
    seq_info->faulty_state = array_alloc(
    int, info->n_latch);
    ...
    seq_info->latch_to_pi_ordering = array_alloc(
    int, info->n_latch);
    ...
    foreach_latch(seq_info->network_copy, gen, l)
    {
        ...
        array_insert(
        int, seq_info->latch_to_pi_ordering, i++, position);
    }
        ...
        network_t *network;
{
    ...
    if (info->atpg_opt->use_internal_states) {
        seq_info->latch_to_product_pi_ordering = array_alloc(
        int, info->n_latch);
        ...
        foreach_latch(seq_info->product_network, gen, l)
        {
            ...
            if (i < info->n_latch) {
                array_insert(
                int, seq_info->latch_to_product_pi_ordering, i++, position);
            }
        }
    }
}
        ...
        atpg_info_t *info;
{
    ...
    array_t * just_sequence = array_alloc(
    unsigned *, 0);
    int    key;
        ...
        int n_latch;
{
    ...
    {
        for (i = 0; i < n_latch; i++) {
            position_in_pi_ordering = array_fetch(
            int, latch_to_pi_ordering, i);
        ...
        bdd_t *already_justified;
{
    ...
    for (i = seq_length; i--;) {
        ...
        old_vector = array_fetch(
        unsigned *, old_just_sequence, i);
        for (j = n_inputs; j--;) {
        ...
        int key;
{
    ...
    if (save_sequence_for_reuse) {
        sequence_for_reuse = array_alloc(
        unsigned *, seq_length);
    }
    ...
    for (i                  = 0; i < seq_length; i++) {
        ...
        if (save_sequence_for_reuse) {
            array_insert(
            unsigned *, sequence_for_reuse, i, vector_for_reuse);
        }
    }
    if (save_sequence_for_reuse) {
        ...
        int key;
{
    ...
    for (i     = seq_length; i--;) {
        vector = array_fetch(
        unsigned *, prop_sequence, i);
        old_vector = array_fetch(
        unsigned *, old_prop_sequence, i);
        for (j = npi; j--;) {
            vector[j] = old_vector[j];
        }
    }
        ...
        seq_info_t *seq_info;
{
    ...
    if (n_prop_vectors == 0) {
        ...
        st_insert(seq_info->prop_sequence_table, (char *) key, NIL(
        char));
    }
    bdd_free(init_state);
        ...
        saved_nodes_t *saved_nodes;
{
    ...
    switch (saved_nodes->which_case) {
        case 1: node_replace(saved_nodes->faulty_node, saved_nodes->orig_faulty_node);
            break;
        case 2: node_replace(saved_nodes->faulty_fanin, saved_nodes->orig_faulty_fanin);
            break;
        case 3:
        ...
        case 4:
            ...
            break;
        default: fprintf(sisout, "ERROR--saved_nodes->which case was not in [1,4]\n");
            break;
    }
}
