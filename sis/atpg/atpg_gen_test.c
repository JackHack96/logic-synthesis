        ...
        int cnt;
{
    ...
    switch (sat_value) {
        case SAT_ABSURD: fault->status = REDUNDANT;
            fault->redund_type         = CONTROL;
            info->statistics->sat_red += 1;
            break;
        case SAT_GAVEUP: fault->status = ABORTED;
            break;
        case SAT_SOLVED:
            if (info->seq) {
                ...
                test_sequence->vectors = array_alloc(
                unsigned *, 1);
                vector = ALLOC(
                unsigned, info->n_real_pi);
                atpg_derive_excitation_vector(ss_info, n_pi_vars, vector);
                array_insert(
                unsigned *, test_sequence->vectors, 0, vector);
            }
            if (fault->is_covered) {
                fault->status = TESTED;
            }
            break;
        default: fail("bad result returned by SAT");
            break;
    }
        ...
        int cnt;
{
    ...
    if (!(bdd_is_tautology(already_justified, 0))) {
        ...
        excitation_vector = array_fetch(
        unsigned *, seq_info->just_sequence,
                n_just_vectors);
            ...
            if (!guaranteed_test) {
                ...
                if (atpg_verify_test(ss_info, fault, test_sequence)) {
                    ...
                    for (i = array_n(test_sequence->vectors); i--;) {
                        vector = array_fetch(
                        unsigned *, test_sequence->vectors, i);
                        FREE(vector);
                    }
        ...
        int cnt;
{
    ...
    if (n_old_vectors && fault->is_covered) {
        ...
        assert(st_delete(info->sequence_table, (char **) &sequence_start, NIL(
        char *)));
