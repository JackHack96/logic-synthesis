...
/* This function generates a test sequence using a 3-step process of
 * combinational testing, fault-free justification, and fault-free propagation.
 * The returned test sequence is ALLOCed only if a test is found.
 */
sequence_t *generate_test(fault, info, ss_info, seq_info, exdc_info,
                          cnt) fault_t *fault;
atpg_info_t *info;
  ...
  case SAT_SOLVED:
    if (info->seq) {
      if (atpg_opt->use_internal_states) {
        test_sequence = internal_states_justify_and_ff_propagate(
            fault, info, ss_info, seq_info, n_pi_vars, cnt);
      } else {
        test_sequence = justify_and_ff_propagate(fault, info, ss_info, seq_info,
                                                 n_pi_vars, cnt);
      }
    } else {
...
/* The returned test sequence is ALLOCed only if a test is found.
 */
static sequence_t *justify_and_ff_propagate(fault, info, ss_info, seq_info,
                                            n_pi_vars, cnt) fault_t *fault;
atpg_info_t *info;
atpg_ss_info_t *ss_info;
seq_info_t *seq_info;
int n_pi_vars;
int cnt;
{
  ...
  already_justified = bdd_and(excite_states, seq_info->justified_states, 1, 1);
  if (!(bdd_is_tautology(already_justified, 0))) {
    ...
    excitation_vector =
        array_fetch(unsigned *, seq_info->just_sequence, n_just_vectors);
  ...
  if (fault->is_covered) {
    test_sequence = derive_test_sequence(
        ss_info, seq_info, actual_n_just_vectors, 0, info->n_real_pi, cnt);
    return test_sequence;
    ...
    if (!n_prop_vectors && atpg_opt->deterministic_prop &&
      ...
      if (st_lookup(seq_info->prop_sequence_table, (char *)key, NIL(char *))) {
        ...
        n_prop_vectors =
            seq_reuse_prop_sequence(info->n_real_pi, seq_info, key);
      } else {
        inverted_key = derive_inverted_prop_key(seq_info);
        if (st_lookup(seq_info->prop_sequence_table, (char *)inverted_key,
                      NIL(char *))) {
          stats->n_prop_reused += 1;
          n_prop_vectors =
              seq_reuse_prop_sequence(info->n_real_pi, seq_info, inverted_key);
        } else {
    ...
    if (n_prop_vectors) {
      test_sequence =
          derive_test_sequence(ss_info, seq_info, actual_n_just_vectors,
                               n_prop_vectors, info->n_real_pi, cnt);
      if (!guaranteed_test) {
        last_time = util_cpu_time();
...
/* The returned test sequence is ALLOCed only if a test is found.
 */
static sequence_t *internal_states_justify_and_ff_propagate(fault, info,
                                                            ss_info, seq_info,
                                                            n_pi_vars,
                                                            cnt) fault_t *fault;
atpg_info_t *info;
...
int cnt;
{
  ...
  excitation_vector =
      array_fetch(unsigned *, seq_info->just_sequence, n_just_vectors);
  atpg_derive_excitation_vector(ss_info, n_pi_vars, excitation_vector);
  if (bdd_equal(seq_info->start_state_used,
                seq_info->range_data->init_state_fn)) {
    n_old_vectors = 0;
  } else {
    bdd_key = convert_bdd_to_int(seq_info->start_state_used);
    assert(st_lookup(seq_info->state_sequence_table, (char *)bdd_key,
                     (char **)&sequence_list));
    ...
    create_just_sequence(seq_info, n_just_vectors + 1, old_vectors,
                         info->n_real_pi);
  }
  actual_n_just_vectors = get_min_just_sequence(ss_info, fault, seq_info);
  ...
  if (n_old_vectors && fault->is_covered) {
    assert(st_delete_int(seq_info->state_sequence_table, &bdd_key,
                         (char **)&sequence_list));
    (void)lsDelBegin(sequence_list, (lsGeneric *)&sequence_start);
    assert(
        st_delete(info->sequence_table, (char **)&sequence_start, NIL(char *)));
    if (lsLength(sequence_list) == 0) {
      new_start_states =
          bdd_and(seq_info->start_states, seq_info->start_state_used, 1, 0);
      bdd_free(seq_info->start_states);
      seq_info->start_states = new_start_states;
      lsDestroy(sequence_list, 0);
    } else {
      st_insert(seq_info->state_sequence_table, (char *)bdd_key,
                (char *)sequence_list);
    }
...
/* The returned test sequence is ALLOCed only if a test is found.
 */
sequence_t *generate_test_using_verification(fault, info, ss_info, seq_info,
                                             cnt) fault_t *fault;
atpg_info_t *info;
atpg_ss_info_t *ss_info;
seq_info_t *seq_info;
  ...
  if (n_test_vectors) {
    ...
    if (info->atpg_opt->use_internal_states) {
      if (!bdd_equal(start_state_used,
        ...
        assert(st_lookup(seq_info->state_sequence_table, (char *)key,
                         (char **)&sequence_list));
        assert(lsLength(sequence_list) > 0);
        (void)lsFirstItem(sequence_list, (lsGeneric *)&sequence_start, 0);
        old_vectors = sequence_start->vectors;
    ...
    if (tested) {
      ...
      fault->status = TESTED;
      if (internal_state_used) {
        assert(st_delete_int(seq_info->state_sequence_table, &key,
                             (char **)&sequence_list));
        (void)lsDelBegin(sequence_list, (lsGeneric *)&sequence_start);
        assert(st_delete(info->sequence_table, (char **)&sequence_start,
                         NIL(char *)));
        if (lsLength(sequence_list) == 0) {
          new_start_states =
              bdd_and(seq_info->product_start_states, start_state_used, 1, 0);
