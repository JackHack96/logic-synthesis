...
static int set_short_tests_options(info, argc, argv) atpg_info_t *info;
int argc;
char **argv;
{
...
int com_short_tests(network, argc, argv) network_t **network;
int argc;
char **argv;
{
  ...
  if (network_num_latch(*network) == 0) {
    fprintf(sisout,
            "Use the atpg command for combinational circuits.\nThe short_tests "
            "techniques are useful only for sequential circuits.\n");
    return 0;
  }
  ...
  if (atpg_opt->rtg) {
    ...
    info->tested_faults =
        atpg_random_cover(info, ss_info, NIL(atpg_ss_info_t), TRUE, &n_tests);
  ...
  if (atpg_opt->PMT_only) {
    ...
    while (lsFirstItem(info->faults, (lsGeneric *)&fault, &handle) == LS_OK) {
      ...
      (void)atpg_network_fault_clauses(ss_info, NIL(atpg_ss_info_t), seq_info,
                                       fault);
      time = util_cpu_time();
    ...
    while (lsFirstItem(info->faults, (lsGeneric *)&fault, &handle) == LS_OK) {
      ...
      test_sequence =
          generate_test(fault, info, ss_info, seq_info, NIL(atpg_ss_info_t), 0);
      ...
      case TESTED:
        ...
        if (atpg_opt->verbosity > 2) {
          atpg_print_vectors(sisout, test_sequence->vectors, info->n_real_pi);
        }
        if (ATPG_DEBUG) {
        ...
        if (atpg_opt->fault_simulate) {
          last_time = util_cpu_time();
          covered = seq_single_sequence_simulate(ss_info, test_sequence,
                                                 info->faults);
          concat_lists(covered,
                       seq_single_sequence_simulate(ss_info, test_sequence,
  ...
  if (atpg_opt->build_product_machines &&
    ...
    while (lsFirstItem(info->untested_faults, (lsGeneric *)&fault, &handle) ==
      ...
      last_time = util_cpu_time();
      test_sequence =
          generate_test_using_verification(fault, info, ss_info, seq_info, 0);
      time = util_cpu_time();
    ...
    while (lsFirstItem(info->final_untested_faults, (lsGeneric *)&fault,
                       &handle) == LS_OK) {
      if (atpg_opt->verbosity > 0) {
        atpg_print_fault(sisout, fault);
      }
...
static void update_start_states(info, seq_info, test_sequence,
                                ss_info) atpg_info_t *info;
seq_info_t *seq_info;
sequence_t *test_sequence;
atpg_ss_info_t *ss_info;
{
...
static void update_product_start_states(info, seq_info, test_sequence,
                                        ss_info) atpg_info_t *info;
seq_info_t *seq_info;
sequence_t *test_sequence;
atpg_ss_info_t *ss_info;
