        ...
        char **argv;
{
    ...
    while ((c = util_getopt(argc, argv, "cd:DfFhn:qrRptT:v:y:z:")) != EOF) {
        switch (c) {
            case 'c': atpg_opt->force_comb = TRUE;
                break;
            case 'd': atpg_opt->rtg_depth = atoi(util_optarg);
                break;
            case 'D': atpg_opt->deterministic_prop = FALSE;
                break;
            case 'f': atpg_opt->fault_simulate = FALSE;
                break;
            case 'F': atpg_opt->reverse_fault_sim = FALSE;
                break;
            case 'h': atpg_opt->fast_sat = TRUE;
                break;
            case 'n': atpg_opt->n_sim_sequences = atoi(util_optarg);
                if (atpg_opt->n_sim_sequences > WORD_LENGTH) {
                    return 0;
                }
                break;
            case 'q': atpg_opt->quick_redund     = TRUE;
                atpg_opt->build_product_machines = FALSE;
                break;
            case 'r': atpg_opt->rtg = FALSE;
                break;
            case 'R': atpg_opt->random_prop = FALSE;
                break;
            case 'p': atpg_opt->build_product_machines = FALSE;
                break;
            case 't': atpg_opt->tech_decomp = TRUE;
                break;
            case 'T': atpg_opt->timeout = atoi(util_optarg);
                if (atpg_opt->timeout < 0 || atpg_opt->timeout > 3600 * 24 * 365) {
                    return 0;
                }
                break;
            case 'v': atpg_opt->verbosity = atoi(util_optarg);
                break;
            case 'y': atpg_opt->prop_rtg_depth = atoi(util_optarg);
                break;
            case 'z': atpg_opt->n_random_prop_iter = atoi(util_optarg);
                break;
            default: return 0;
        }
    }
        ...
        atpg_info_t *atpg_info;
{
    ...
    ss_info->word_vectors    = array_alloc(
    unsigned *, 0);
    ss_info->prop_word_vectors = array_alloc(
    unsigned *, 0);
    for (i = atpg_info->atpg_opt->prop_rtg_depth; i--;) {
        vector = ALLOC(
        unsigned, ss_info->n_real_pi);
        array_insert_last(
        unsigned *, ss_info->prop_word_vectors, vector);
    }
    ss_info->real_po_values = ALLOC(
    unsigned, ss_info->n_real_po);
    ss_info->true_value = ALLOC(
    unsigned, ss_info->n_real_po);
        ...
        int stg_depth;
{
    ...
    seq_info->just_sequence = array_alloc(
    unsigned *, stg_depth + 1);
    seq_info->prop_sequence = array_alloc(
    unsigned *, stg_depth + 1);
    for (i = stg_depth + 1; i--;) {
        vector = ALLOC(
        unsigned, info->n_real_pi);
        array_insert(
        unsigned *, seq_info->just_sequence, i, vector);
        vector = ALLOC(
        unsigned, info->n_real_pi);
        array_insert(
        unsigned *, seq_info->prop_sequence, i, vector);
    }
        ...
        atpg_ss_info_t *ss_info;
{
    ...
    ss_info->pi_uid = ALLOC(
    int, ss_info->n_real_pi + ss_info->n_latch);
    ss_info->po_uid = ALLOC(
    int, ss_info->n_real_po + ss_info->n_latch);

    /* create the reset state array used in fault simulation */
    ss_info->reset_state = ALLOC(
    unsigned, ss_info->n_latch);
    ...
    ss_info->true_state = ALLOC(
    unsigned, ss_info->n_latch);
    ss_info->faulty_state = ALLOC(
    unsigned, ss_info->n_latch);
    ss_info->changed_node_indices = ALLOC(
    int, ss_info->n_latch + 2);
    ss_info->all_true_value = ALLOC(
    unsigned, ss_info->n_po);
    ss_info->all_po_values = ALLOC(
    unsigned, ss_info->n_po);
}
        ...
        atpg_info_t *info;
{
    ...
    foreach_primary_input(info->network, gen, orig_pi)
    {
        ...
        if (exdc_pi == NIL(node_t)) {
            new_pi = node_alloc();
            new_pi->name = ALLOC(
            char, strlen(node_long_name(orig_pi)) + 1);
    ...
    foreach_primary_output(info->network, gen, orig_po)
    {
        ...
        if (exdc_po == NIL(node_t)) {
            ...
            new_po->name = ALLOC(
            char, strlen(node_long_name(orig_po)) + 1);
        ...
        atpg_info_t *info;
{
    ...
    st_foreach_item(sequence_table, sgen, (char **) &sequence, NIL(
    char *)) {
        ...
        atpg_info_t *info;
{
    ...
    st_foreach_item(info->redund_table, sgen, (char **) &info_p, NIL(
    char *)) {
        FREE(info_p);
    }
        ...
        node_t **fanout_ptr;
{
    ...
    sm->or_input_masks  = ALLOC(
    unsigned, sm->n_inputs);
    sm->and_input_masks = ALLOC(
    unsigned, sm->n_inputs);
    for (i = sm->n_inputs; i--;) {
    ...
    switch (sm->type) {
        case NODE_0: sm->value = ALL_ZERO;
            sm->eval           = atpg_eval_sim_pi;
            return;
        case NODE_1: sm->value = ALL_ONE;
            sm->eval           = atpg_eval_sim_pi;
            return;
        case NODE_PI: sm->eval = atpg_eval_sim_pi;
            return;
        case NODE_PO:
            sm->fanin_values = ALLOC(
            unsigned *, sm->n_inputs);
            for (i = sm->n_inputs; i--;) {
        ...
        atpg_sim_node_t *sim_node;
{
    ...
    for (c = sim_node->n_cubes; c--;) {
        ...
        for (l = node_num_fanin(node); l--;) {
            ...
            switch (literal) {
                case ZERO: sim_node->function[c][l] = 0;
                    break;
                case ONE: sim_node->function[c][l] = 1;
                    break;
                case TWO: sim_node->function[c][l] = 2;
                    break;
        ...
        atpg_sim_node_t *sim_node;
{
    ...
    switch (sim_node->type) {
        ...
        case NODE_0:
        case NODE_1: break;
        case NODE_PO: FREE(sim_node->fanin_values);
            break;
        case NODE_BUF:
        case NODE_INV:
        case NODE_AND:
        case NODE_OR:
        case NODE_COMPLEX: FREE(sim_node->fanin_values);
            for (i = 3; i--;) {
