...
sat_new() {
    ...
    sat->one_clauses = array_alloc(
    int, ASIZE);
    sat->nclause   = 0;
    sat->lit_index = 0;

    /* stacks */
    sat->stk_var = array_alloc(
    int, ASIZE);
    sat->stk_inc = array_alloc(
    int, ASIZE);
    sat->stk_cla = array_alloc(
    int, ASIZE);
        ...
        bool add_impl;
{
    ...
    while (bot < array_n(sat->stk_var)) {
        ...
        fast_avl_foreach_item(table, sgen, AVL_FORWARD, (char **) &imp_lit, NIL(
                char *)) {
        ...
        int verbosity;
{
    ...
    sm_foreach_row(sat->matrix, r)
    {
        ...
        if (r->length == 1) {
            INV_UNBOUND(r);
            array_insert_last(
            int, sat->one_clauses, r->first_col->col_num);
        } else if (r->length == 0) {
            return SAT_ABSURD;
        }
    }
    for (i = 0; i < array_n(sat->one_clauses); i++) {
        c = sm_get_col(sat->matrix, array_fetch(
        int, sat->one_clauses, i));
