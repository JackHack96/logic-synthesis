...
static void print_usage() {
  ...
  fprintf(sisout,
          "-q\tquick redundancy removal--remove only SNE redundancies\n");
...
int st_fpcmp(obj1, obj2) char *obj1;
char *obj2;
{
...
int st_fphash(obj, modulus) char *obj;
int modulus;
{
  unsigned hash;
  fault_pattern_t *fp;
  ...
  hash =
      (unsigned)(fp->node) + (unsigned)(fp->fanin) + (unsigned)(fp->value + 1);
  return hash % modulus;
}

int com_redundancy_removal(network, argc, argv) network_t **network;
int argc;
char **argv;
{
  long begin_time;
  ...
  do {
    ...
    if (n_redundant == 0) {
      ...
      if (atpg_opt->quick_redund) {
        ...
        tested_faults =
            atpg_random_cover(info, ss_info, exdc_ss_info, TRUE, NIL(int));
      }
    ...
    if (atpg_opt->quick_redund) {
      redundancy_found = quick_atpg_application(
          info, ss_info, seq_info, exdc_ss_info, fp_table, aborted_table,
          prev_fault_table, &output_fns_changed);
    } else {
      redundancy_found = atpg_application(
          info, ss_info, seq_info, exdc_ss_info, fp_table, aborted_table,
          prev_fault_table, &output_fns_changed);
    }
    ...
    if (network_num_latch(*network) != info->n_latch) {
      ...
      if (atpg_opt->quick_redund) {
        ...
        st_foreach_item(info->sequence_table, sgen, (char **)&sequence,
                        NIL(char *)) {
          st_delete(info->sequence_table, (char **)&sequence, NIL(char *));
...
static bool atpg_application(info, ss_info, seq_info, exdc_info, fp_table,
                             aborted_table, prev_fault_table,
                             output_fns_changed) atpg_info_t *info;
...
bool *output_fns_changed;
{
  ...
  while (lsFirstItem(info->faults, (lsGeneric *)&fault, &handle) == LS_OK) {
    ...
    case TESTED: /* tested fault */
      ...
      if (cnt == atpg_opt->n_sim_sequences) {
        covered = atpg_seq_single_fault_simulate(
            ss_info, exdc_info, ss_info->word_vectors, info->faults,
            NIL(sequence_t *), 0);
        concat_lists(covered, atpg_seq_single_fault_simulate(
                                  ss_info, exdc_info, ss_info->word_vectors,
                                  untested_faults, NIL(sequence_t *), 0));
        extract_test_sequences(info, ss_info, covered, ss_info->word_vectors,
  ...
  atpg_simulate_old_sequences(info, ss_info, exdc_info, fp_table,
                              untested_faults);
  if (atpg_opt->verbosity > 0) {
  ...
  if (info->seq && atpg_opt->build_product_machines) {
    while (lsFirstItem(untested_faults, (lsGeneric *)&fault, &handle) ==
           LS_OK) {
      if (atpg_opt->verbosity > 0) {
        atpg_print_fault(sisout, fault);
      }
      ...
      case TESTED: /* tested fault */
        ...
        if (cnt == atpg_opt->n_sim_sequences) {
          covered = atpg_seq_single_fault_simulate(
              ss_info, exdc_info, ss_info->word_vectors, untested_faults,
              NIL(sequence_t *), 0);
          extract_test_sequences(info, ss_info, covered, ss_info->word_vectors,
                                 NIL(int), info->n_real_pi);
          foreach_fault(covered, gen, tf) {
...
static bool quick_atpg_application(info, ss_info, seq_info, exdc_info, fp_table,
                                   aborted_table, prev_fault_table,
                                   output_fns_changed) atpg_info_t *info;
atpg_ss_info_t *ss_info;
seq_info_t *seq_info;
...
bool *output_fns_changed;
{
  ...
  */
  while (lsFirstItem(info->faults, (lsGeneric *)&fault, &handle) == LS_OK) {
    ...
    sat_value =
        sat_solve(ss_info->atpg_sat, atpg_opt->fast_sat, atpg_opt->verbosity);
    switch (sat_value) {
    case SAT_ABSURD:
      if (atpg_opt->verbosity > 0) {
        fprintf(sisout, "Redundant\n");
    ...
    case SAT_SOLVED: /* tested fault */
      ...
      if (cnt == atpg_opt->n_sim_sequences) {
        covered = atpg_comb_single_fault_simulate(
            ss_info, exdc_info, ss_info->word_vectors, info->faults);
        concat_lists(covered, atpg_comb_single_fault_simulate(
                                  ss_info, exdc_info, ss_info->word_vectors,
                                  untested_faults));
        extract_test_sequences(info, ss_info, covered, ss_info->word_vectors,
                               NIL(int), info->n_pi);
 ...
 * FALSE otherwise
 */
static bool remove_redundancy(info, ss_info, fault,
                              output_fns_changed) atpg_info_t *info;
atpg_ss_info_t *ss_info;
fault_t *fault;
bool *output_fns_changed;
{
  ...
  if (fault->fanin == NIL(node_t)) {
    ...
    i = 0;
    foreach_fanout(fault->node, gen, fanout) { fanouts[i++] = fanout; }
...
static void save_fault_pattern(fault_pattern_table, prev_fault_table,
                               f) st_table *fault_pattern_table;
st_table *prev_fault_table;
fault_t *f;
{
  sequence_t **slot;
