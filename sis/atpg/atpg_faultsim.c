 ...
 */
static void atpg_network_simulate(info, real_pi_values, state,
                                  real_po_values) atpg_ss_info_t *info;
...
lsList atpg_seq_single_fault_simulate(ss_info, exdc_info, sequences, fault_list,
                                      original_sequences,
                                      cnt) atpg_ss_info_t *ss_info;
    ...
    while (lsNext(gen1, (lsGeneric *)&fault, &handle1) == LS_OK) {
      ...
      if (prev_fault != NIL(fault_t)) {
        ...
        if (node_function(prev_fault->node) == NODE_PI) {
          st_lookup_int(ss_info->pi_po_table, (char *)prev_fault->node,
                        &pi_index);
        ...
        for (j = ss_info->n_latch; j--;) {
          if (ss_info->sim_nodes[ss_info->pi_uid[j]].value !=
            ...
            ss_info->sim_nodes[ss_info->pi_uid[j]].value =
                fault->current_state[j];
            ss_info->changed_node_indices[n_changed_nodes] = ss_info->pi_uid[j];
            n_changed_nodes += 1;
          }
        }
      }
...
lsList atpg_random_cover(info, ss_info, exdc_ss_info, save_sequences,
                         n_tests_ptr) atpg_info_t *info;
...
int *n_tests_ptr;
{
  ...
  do {
    ...
    local_covered = atpg_seq_single_fault_simulate(
        ss_info, exdc_ss_info, sequences, info->faults, NIL(sequence_t *), 0);
    concat_lists(local_covered,
                 atpg_seq_single_fault_simulate(
                     ss_info, exdc_ss_info, sequences, info->untested_faults,
                     NIL(sequence_t *), 0));
...
int random_propagate(fault, info, ss_info, seq_info) fault_t *fault;
atpg_info_t *info;
atpg_ss_info_t *ss_info;
seq_info_t *seq_info;
{
  ...
  for (h = info->atpg_opt->n_random_prop_iter; h--;) {
    ...
    for (i = 0; i < seq_length; i++) {
      ...
      for (j = ss_info->n_latch; j--;) {
        if (ss_info->sim_nodes[ss_info->pi_uid[j]].value != faulty_state[j]) {
          ss_info->sim_nodes[ss_info->pi_uid[j]].value = faulty_state[j];
          ss_info->changed_node_indices[n_changed_nodes] = ss_info->pi_uid[j];
          n_changed_nodes += 1;
        }
      }
...
void fault_simulate(info, ss_info, exdc_info, cnt) atpg_info_t *info;
...
int cnt;
{
  ...
  covered_faults = atpg_seq_single_fault_simulate(
      ss_info, exdc_info, sequences, info->faults, ss_info->alloc_sequences,
      cnt);
  concat_lists(covered_faults,
               atpg_seq_single_fault_simulate(ss_info, exdc_info, sequences,
                                              info->untested_faults,
...
void atpg_sf_set_sim_masks(info, fault) atpg_ss_info_t *info;
fault_t *fault;
{
...
void atpg_sf_reset_sim_masks(info, fault) atpg_ss_info_t *info;
fault_t *fault;
{
  atpg_sim_node_t *node;
...
static void get_tfo_of_array(info, index, array_size) atpg_ss_info_t *info;
int *index;
int array_size;
{
  int id;
...
static void atpg_sim_tfo_array(info, array_size) atpg_ss_info_t *info;
int array_size;
{
  int f, index;
...
static void atpg_sim_get_po(info, real_po_values,
                            next_state) atpg_ss_info_t *info;
unsigned *real_po_values;
unsigned *next_state;
{
  int i, n_latch;
...
static bool sf_record_covered_faults(fault, true_value, po_values, exdc_value,
                                     npo, vector_number, original_sequences,
                                     cnt) fault_t *fault;
unsigned *true_value;
unsigned *po_values;
unsigned *exdc_value;
...
static void ss_record_covered_faults(po_values, faults_ptr,
                                     npo) unsigned *po_values;
fault_t **faults_ptr;
int npo;
{
  fault_t *f;
...
static void record_different_states(seq_info, state,
                                    n_latch) seq_info_t *seq_info;
unsigned *state;
int n_latch;
{
...
void extract_test_sequences(info, ss_info, faults, sequences, n_tests_ptr,
                            npi) atpg_info_t *info;
atpg_ss_info_t *ss_info;
lsList faults;
array_t *sequences;
int *n_tests_ptr;
int npi;
{
  ...
  foreach_fault(faults, gen, f) { ss_info->used[f->sequence_index] = TRUE; }
  for (i = WORD_LENGTH; i--;) {
    if (ss_info->used[i]) {
...
void atpg_set_sim_masks(info) atpg_ss_info_t *info;
{
  int i;
  fault_t *fault;
  atpg_sim_node_t *node;
...
void atpg_reset_sim_masks(info) atpg_ss_info_t *info;
{
  int i, id;
  fault_t *fault;
  atpg_sim_node_t *node;
...
int get_min_just_sequence(ss_info, fault, seq_info) atpg_ss_info_t *ss_info;
fault_t *fault;
seq_info_t *seq_info;
{
...
void simulate_entire_sequence(ss_info, fault, seq_info,
                              vectors) atpg_ss_info_t *ss_info;
fault_t *fault;
seq_info_t *seq_info;
array_t *vectors;
{
...
bool atpg_verify_test(ss_info, fault, test_sequence) atpg_ss_info_t *ss_info;
fault_t *fault;
...
lsList seq_single_sequence_simulate(ss_info, test_sequence,
                                    fault_list) atpg_ss_info_t *ss_info;
sequence_t *test_sequence;
lsList fault_list;
{
...
void fault_simulate_to_get_final_state(ss_info, test_sequence,
                                       seq_info) atpg_ss_info_t *ss_info;
sequence_t *test_sequence;
seq_info_t *seq_info;
{
  array_t *test_vectors = test_sequence->vectors;
...
static int seq_length_cmp(obj1, obj2) char *obj1;
char *obj2;
{
  sequence_t **s1, **s2;
...
void atpg_simulate_old_sequences(info, ss_info, exdc_info, fp_table,
...
lsList untested_faults;
{
  ...
  for (i = 0; i < n_seq; i += WORD_LENGTH) {
    ...
    local_covered = atpg_seq_single_fault_simulate(
        ss_info, exdc_info, sequences, info->faults, NIL(sequence_t *), 0);
    concat_lists(local_covered, atpg_seq_single_fault_simulate(
                                    ss_info, exdc_info, sequences,
...
static void atpg_fault_sequence_sim(atpg_info, ss_info) atpg_info_t *atpg_info;
atpg_ss_info_t *ss_info;
{
...
void extract_sequences(info, sequences_ptr, from, to, seq_length,
                       npi) atpg_ss_info_t *info;
...
static void fp_record_covered_faults(info, true_outputs,
...
static int seq_index_cmp(obj1, obj2) char *obj1;
char *obj2;
{
  sequence_t **s1, **s2;
...
void reverse_fault_simulate(info, ss_info) atpg_info_t *info;
atpg_ss_info_t *ss_info;
{
  long start_time;
  ...
  while (count-- &&
         lsFirstItem(info->faults, (lsGeneric *)&fault, &handle) == LS_OK) {
  ...
  for (i = 0; i < n_seq; i++) {
    covered =
        seq_single_sequence_simulate(ss_info, sequences_ptr[i], info->faults);
