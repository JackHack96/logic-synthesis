...
static int visit_row(A, prow, rows_visited, cols_visited) sm_matrix *A;
sm_row *prow;
int *rows_visited;
int *cols_visited;
{
...
static int visit_col(A, pcol, rows_visited, cols_visited) sm_matrix *A;
sm_col *pcol;
int *rows_visited;
int *cols_visited;
{
...
static sm_row *copy_row(A, prow) register sm_matrix *A;
...
static void block_copy(A, L, R) register sm_matrix *A;
...
int dec_block_partition(A, L, R) sm_matrix *A;
sm_matrix **L, **R;
{
  ...
  sm_foreach_row(A, prow) { prow->flag = 0; }
  sm_foreach_col(A, pcol) { pcol->flag = 0; }
