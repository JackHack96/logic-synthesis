...
static sm_row *copy_row(register sm_matrix *A, register sm_row *prow) {
    register sm_element *p;
    register sm_element *element;

    sm_foreach_row_element(prow, p) {
        element = sm_insert(A, p->row_num, p->col_num);
        sm_put(element, sm_get(
                char *, p));
    }
    return sm_get_row(A, prow->row_num);
}

static void block_copy(register sm_matrix *A, register sm_matrix **L, register sm_matrix **R) {
    ...
    sm_foreach_col((*L), pcol) {
        sm_put(pcol, sm_get(
                char *, sm_get_col(A, pcol->col_num)));
    }
...
int dec_block_partition(sm_matrix *A, sm_matrix **L, sm_matrix **R) {
