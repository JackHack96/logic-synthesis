...
sm_matrix *dec_node_to_sm(node_t *f) {
    sm_matrix      *M;
    sm_element     *element;
    sm_col         *column;
    node_cube_t    cube;
    ...
    for (i = 0; i < node_num_cube(f); i++) {
        ...
        foreach_fanin(f, j, fanin) {
            ...
            switch (literal) {
                case ONE:element = sm_insert(M, i, j);
                    sm_put(element, 1);
                    column = sm_get_col(M, j);
                    sm_put(column, fanin);
                    break;
                case ZERO:element = sm_insert(M, i, j);
                    sm_put(element, 0);
                    column = sm_get_col(M, j);
                    sm_put(column, fanin);
                    break;
...
node_t *dec_sm_to_node(sm_matrix *M) {
    ...
    sm_foreach_row(M, row) {
        ...
        sm_foreach_row_element(row, element) {
            ...
            switch (sm_get(int, element)) {
                case 1:lit = node_literal(fanin, 1);
                    break;
                case 0:lit = node_literal(fanin, 0);
                    break;
    ...
    for (i = 0; i < M->nrows; i++) {
        for (j = 0; j < M->ncols; j++) {
            ...
            if (element == NIL(sm_element)) {
                ...
                switch (sm_get(int, element)) {
                    case 0:(void) fprintf(misout, "  0");
                        break;
                    case 1:(void) fprintf(misout, "  1");
                        break;
                    default: fail("bad sm_element_data");
                        /* NOTREACHED */
                }
