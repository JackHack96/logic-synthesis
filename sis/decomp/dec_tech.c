...
void decomp_tech_network(network_t *network, int and_limit, int or_limit) {
    lsGen   gen;
    node_t  *node, *f;
    array_t *fa;
    int     i;
    ...
    for (i = array_n(fa) - 1; i >= 0; i--) {
        ...
        if (f->type == INTERNAL) {
            switch (node_function(f)) {
                case NODE_AND:decomp_and(network, f, and_limit);
                    break;
                case NODE_OR:decomp_or(network, f, or_limit);
                    break;
                default:;
            }
        }
...
static void decomp_and_or(network_t *network, node_t *f) {
    node_t *g, *t, *clit, *c;
    int    i;
...
static void decomp_and(network_t *network, node_t *f, int and_limit) {
    ...
    for (i = array_n(leaves) - 1; i >= 0; i--) {
        ...
        switch (node_input_phase(f, node_get_fanin(f, i))) {
            case POS_UNATE:node_replace(leaf, node_literal(fanin, 1));
                break;
            case NEG_UNATE:node_replace(leaf, node_literal(fanin, 0));
                break;
            default: fail("Error: wrong phase of a input in decomp_and");
        }
    }
...
static void decomp_or(network_t *network, node_t *f, int or_limit) {
    array_t *nodes, *leaves;
    node_t  *root, *leaf, *fanin, *node;
    int     i;
    ...
    for (i = array_n(leaves) - 1; i >= 0; i--) {
        ...
        switch (node_input_phase(f, node_get_fanin(f, i))) {
            ...
            case NEG_UNATE:node_replace(leaf, node_literal(fanin, 0));
                break;
            default: fail("Error: wrong phase of a input in decomp_or");
        }
    }
...
static void balanced_tree(int n, int m, node_t *(*node_func)(), node_t **root, array_t **nodes, array_t **leaves) {
    node_t  *np, *rt;
    array_t *ns, *lv;
    int     d, s;
    bool    first;
