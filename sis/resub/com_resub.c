...
int com_resub(network, argc, argv) network_t **network;
int argc;
char **argv;
{
  ...
  for (i = 0; i < array_n(nodevec); i++) {
    switch (method) {
    case 1:
      resub_bool_node(array_fetch(node_t *, nodevec, i));
      break;
    case 0:
      (void)resub_alge_node(array_fetch(node_t *, nodevec, i), use_complement);
      break;
    }
  }
  array_free(nodevec);
...
init_resub() { (void)com_add_command("resub", com_resub, 1); }
