 ...
 * "dist"
 */
void speed_absorb(node_t *f, speed_global_t *speed_param) {
...
void speed_absorb_array(node_t *f, speed_global_t *speed_param, array_t *temp_array, st_table *cache) {
    ...
    foreach_fanin(f, i, no) { array_insert_last(node_t *, del_array, no); }

    if (speed_param->debug)
        (void) fprintf(sisout, "Collapsing nodes \n");
    more_to_come = TRUE;
    while (more_to_come) {
        more_to_come = FALSE;
        for (i       = 1; i < array_n(temp_array); i++) {
            ...
            if (speed_param->debug && node_num_fanin(temp) > 2) {
                (void) fprintf(sisout,
                               "WARN: Collapsing at node %s : %s with mo re than 2 ip\n",
