...
static int speed_delay_parameters_dup(node_t *from, node_t *to, speed_global_t *speed_param, int delay_flag) {
    delay_time_t time;
    double       temp;
...
network_t *speed_network_create_from_node(node_t *f, speed_global_t *speed_param, int delay_flag) {
    network_t *network;
    lsGen     gen;
    node_t    *pi, *fanin, *po;
...
array_t *network_to_array(network_t *network) {
    ...
    for (i = 0; i < array_n(array); i++) {
        ...
        foreach_fanin(node, j, fanin) { node->fanin[j] = fanin->copy; }
 ...
 */
array_t *network_and_node_to_array(network_t *network, node_t *innode, st_table *table) {
    array_t *nodevec, *array;
