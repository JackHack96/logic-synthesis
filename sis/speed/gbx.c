...
node_t *node_my_dup(node) node_t *node;
{
  node_t *new_node;
...
node_t *replace_fanin(node, fanin, replacement_fanin) node_t *node, *fanin,
    *replacement_fanin;
{
  node_t *p, *q, *r;
...
node_t *add_opt_boolean_diff(innode, network, model) node_t *innode;
network_t *network;
delay_model_t model;
{
...
int take_bypass(bypass, network, model, mux_delay,
                actual_mux_delay) bypass_t *bypass;
network_t *network;
delay_model_t model;
...
bypass_t *bypass_copy(bypass) bypass_t *bypass;
{
  bypass_t *result;
...
bypass_t *new_bypass(node, fanout, weight, slack, phase) node_t *fanout, *node;
double weight, slack;
input_phase_t phase;
{
  bypass_t *result;
...
bypass_t *bypass_dup(bypass) bypass_t *bypass;
{
  bypass_t *result;
  SAFE_ALLOC(result, bypass_t, 1);
  *result = *bypass;
...
void free_bypass(bypass) bypass_t *bypass;
{
  lsDestroy(bypass->bypassed_nodes, (void (*)())0);
  FREE(bypass);
}
int bypass_is_extensible(bypass, fanout, node, weight) node_t *fanout, *node;
bypass_t *bypass;
double weight;
{
  node_t *fanin;
...
void bypass_add_node(bypass, fanout, node, weight, phase) node_t *fanout, *node;
bypass_t *bypass;
double weight;
input_phase_t phase;
{
...
void bypass_new_add_node(bypass, fanout, node) node_t *fanout, *node;
bypass_t *bypass;
{
  node_bp_t *record;
  node_t *fanin;
...
void register_bypass(bypass) bypass_t *bypass;
{
  node_t *node;
  lsGen gen;
  node_bp_t *record;
...
void print_node_bp_record(rec, node) node_bp_t *rec;
node_t *node;
{
  int i;
  node_t *fanin;
...
void destroy_bp_record(record) node_bp_t *record;
{
...
node_bp_t *new_node_bp_record(node) node_t *node;
{
  node_bp_t *result;
...
void gbx_init_node_table(network, model) network_t *network;
delay_model_t model;
{
  array_t *nodes;
  node_t *node;
...
lsList bypass_extension_fanouts(bypass, node) node_t *node;
bypass_t *bypass;
{
  lsGen gen;
  node_t *fanout;
...
lsList path_fanouts(node, epsilon) node_t *node;
double epsilon;
{
  int i;
  lsGen gen;
...
node_t *path_fanout(node, epsilon) node_t *node;
double epsilon;
{
  int fanout_found;
  lsGen gen;
...
double retrieve_slack(fanout, node) node_t *fanout, *node;
{
  int i;
  node_bp_t *record;
...
double weight(fanout, node) node_t *fanout, *node;
{
  int i;
  node_bp_t *record;
...
input_phase_t retrieve_phase(fanout, node) node_t *fanout, *node;
{
  int i;
  node_bp_t *record;
...
lsList new_find_bypass_nodes(network, epsilon, mux_delay,
...
void new_trace_bypass(fanout, node, epsilon, mux_delay, last_bypass_ended,
                      model) node_t *fanout,
    *node, *last_bypass_ended;
...
lsList newer_find_bypass_nodes(network, epsilon, mux_delay,
                               model) network_t *network;
double epsilon;
double mux_delay;
delay_model_t model;
...
void newer_trace_bypass(fanout, node, epsilon, mux_delay, last_bypass_ended,
                        model) node_t *fanout,
    *node, *last_bypass_ended;
double epsilon, mux_delay;
delay_model_t model;
...
void extend_bypass(bypass, node, mux_delay) bypass_t *bypass;
node_t *node;
double mux_delay;
{
...
void clean_global_tables(network) network_t *network;
{ st_free_table(bypass_table); }
void gbx_clean_node_table(network) network_t *network;
{
  array_t *nodes;
  int i;
  node_t *node;
...
void print_bypass(bypass) bypass_t *bypass;
{
  lsGen gen;
  node_t *node;
...
int do_gbx_transform(network, epsilon, mux_delay, model, new_proc,
                     actual_mux_delay) network_t *network;
...
int bypass_compare(bypass1, bypass2) char **bypass1, **bypass2;
{ return ((bypass_t *)*bypass2)->gain - ((bypass_t *)*bypass1)->gain; }

int do_all_bypasses(network, epsilon, mux_delay, actual_mux_delay, model,
                    new_proc) network_t *network;
double epsilon, mux_delay, actual_mux_delay;
delay_model_t model;
gbx_trace_t new_proc;
{
...
void print_epsilon_paths(nodelist, node, epsilon) lsList nodelist;
node_t *node;
double epsilon;
{
  lsGen gen;
...
void print_epsilon_network(network, model, epsilon) network_t *network;
delay_model_t model;
double epsilon;
{
  node_t *pi;
...
int do_GBX(network, adaptive_epsilon, epsilon, new_trace_procedure, mux_delay,
           target_delay, target_chosen, model, cutset_mode) network_t **network;
      ...
      if (adaptive_epsilon && !target_chosen && epsilon > 1) {
        ...
        if (gbx_verbose_mode) {
          ...
          case 2:
            ...
            break;
          case 1:
            printf("Cutset found, bypasses taken.\n");
            break;
          }
        }
        (void)delay_trace(*network, gbx_delay_model);
...
int do_bypass_transform(network, epsilon, adaptive_epsilon, mux_delay, model,
                        new_trace_procedure, verbose_mode,
                        cutset_mode) network_t **network;
double epsilon, mux_delay;
delay_model_t model;
gbx_trace_t new_trace_procedure;
