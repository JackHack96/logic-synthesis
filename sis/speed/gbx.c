...
#define foreach_list_item(list, gen, item)                                     \
  for ((gen) = lsStart((list));                                                \
       (lsNext((gen), (lsGeneric *)&(item), LS_NH) == LS_OK                    \
            ? 1                                                                \
            : (lsFinish((gen)), 0));)

node_t *node_my_dup(node_t *node) {
    node_t *new_node;
...
node_t *replace_fanin(node_t *node, node_t *fanin, node_t *replacement_fanin) {
    node_t *p, *q, *r;
    node_t *lit0, *lit1, *tmp0, *tmp1, *tmp2;
    node_t *result;

    if (node_get_fanin_index(node, fanin) == -1)
        return node_dup(node);
...
node_t *add_opt_boolean_diff(node_t *innode, network_t *network, delay_model_t model) {
    ...
    for (i  = 0; i < array_n(nodevec); i++) {
        ...
        foreach_fanin(newnode, j, fanin) {
            if (fanin->type == PRIMARY_INPUT) {
                ...
                if ((orig = name_to_node(innode, node_long_name(fanin))) !=
                    NIL(node_t)) {
...
int take_bypass(bypass_t *bypass, network_t *network, delay_model_t model, double mux_delay, double actual_mux_delay) {
    ...
    */
    ...
    if (node_num_fanin(diff) <= 2) {
        ...
        newdiff = add_opt_boolean_diff(diff, network, model);
    assert(network_check(network));
    ...
    tmp1  = node_literal(newdiff, 1);
    tmp2  = node_and(tmp, tmp1);
    tmp3  = node_literal(bypass->last_node, 1);
    tmp4  = node_literal(newdiff, 0);
    tmp5  = node_and(tmp3, tmp4);
    mux   = node_or(tmp5, tmp2);
    node_free(tmp);
    node_free(tmp1);
    node_free(tmp2);
    node_free(tmp3);
    ...
    if (node_function(p) == NODE_1)
        sub_node = node_constant(1);
    else if (node_function(q) == NODE_1)
        ...
        sub_node = node_constant(1); /* Grrr.  Gotta do something */
...
bypass_t *bypass_copy(bypass_t *bypass) {
    bypass_t *result;
...
bypass_t *new_bypass(node_t *node, node_t *fanout, double weight, double slack, input_phase_t phase) {
    bypass_t     *result;
    node_t       *fanin;
    delay_time_t artime;
    int          i;
...
bypass_t *bypass_dup(bypass_t *bypass) {
    bypass_t *result;
    SAFE_ALLOC(result, bypass_t, 1);
    *result = *bypass;
    result->bypassed_nodes = lsCopy(bypass->bypassed_nodes, (lsGeneric(*)()) 0);
...
void free_bypass(bypass_t *bypass) {
    lsDestroy(bypass->bypassed_nodes, (void (*)()) 0);
    FREE(bypass);
}

int bypass_is_extensible(bypass_t *bypass, node_t *fanout, node_t *node, double weight) {
    node_t       *fanin;
    int          i;
...
void bypass_add_node(bypass_t *bypass, node_t *fanout, node_t *node, double weight, input_phase_t phase) {
    node_bp_t *record;
...
void bypass_new_add_node(bypass_t *bypass, node_t *fanout, node_t *node) {
    node_bp_t     *record;
    node_t        *fanin;
    int           i;
    delay_time_t  artime, cartime;
    ...
    bypass->side_slack = min(bypass->side_slack, control_delay - side_delay);
...
void register_bypass(bypass_t *bypass) {
    node_t    *node;
    lsGen     gen;
    node_bp_t *record;
...
void print_node_bp_record(node_bp_t *rec, node_t *node) {
    int    i;
    node_t *fanin;

    printf("Printing record for node %s.  Slack %f mark %d.  Path fanin %s, "
           "off-path slack %f\n",
    ...
    foreach_fanin(node, i, fanin) {
        printf("fanin %s slack %f weight %f phase %s\n", node_name(fanin),
               ...
               (rec->input_phases[i] == BINATE)
               ? "binate"
               : (rec->input_phases[i] == POS_UNATE)
                 ? "positive"
                 : (rec->input_phases[i] == NEG_UNATE) ? "negative"
                                                       : "unknown");
    }
}

void destroy_bp_record(node_bp_t *record) {
...
node_bp_t *new_node_bp_record(node_t *node) {
    node_bp_t     *result;
    ...
    for (i = 0; i < n; ++i) {
        ...
        switch (phase) {
            case POS_UNATE:rtime.rise -= delay.rise;
                rtime.fall -= delay.fall;
                break;
            case NEG_UNATE:rtime.rise -= delay.fall;
                rtime.fall -= delay.rise;
                break;
            default:rtime.rise = min(rtime.rise, rtime.fall);
                delay.rise     = max(delay.rise, delay.fall);
                rtime.rise -= delay.rise;
                rtime.fall     = rtime.rise;
        }
        result->pin_slacks[i] =
                min(rtime.rise - atime.rise, rtime.fall - atime.fall);
...
void gbx_init_node_table(network_t *network, delay_model_t model) {
    array_t   *nodes;
    node_t    *node;
...
lsList bypass_extension_fanouts(bypass_t *bypass, node_t *node) {
    lsGen  gen;
    node_t *fanout;
    lsList path_fanouts;
...
lsList path_fanouts(node_t *node, double epsilon) {
    int           i;
    lsGen         gen;
    node_t        *fanout;
    lsList        path_fanouts;
    ...
    foreach_fanout(node, gen, fanout) {
        ...
        if (fanout_record->pin_slacks[i] > epsilon)
            continue;
        if (node_function(fanout) == NODE_PO)
            continue;
        ...
        if (phase == PHASE_UNKNOWN)
            continue;
        if (fanout_record->path_fanin != node)
            continue;
        if (fanout_record->path_slack == 0.0)
            continue;
        (void) lsNewBegin(path_fanouts, (lsGeneric) fanout, LS_NH);
    }
    return path_fanouts;
...
node_t *path_fanout(node_t *node, double epsilon) {
    int           fanout_found;
    lsGen         gen;
    ...
    foreach_fanout(node, gen, fanout) {
        ...
        if (fanout_record->path_fanin != node)
            return NIL(node_t);
        if (fanout_record->slack > epsilon)
            continue;
        if (node_function(fanout) == NODE_PO)
            return NIL(node_t);
        if (fanout_found)
            return NIL(node_t); /* two path fanouts */
        if ((phase = node_input_phase(fanout, node)) == BINATE)
            return NIL(node_t);
        if (phase == PHASE_UNKNOWN)
            return NIL(node_t);
...
double retrieve_slack(node_t *fanout, node_t *node) {
    int       i;
    node_bp_t *record;
...
double weight(node_t *fanout, node_t *node) {
    int       i;
    node_bp_t *record;
...
input_phase_t retrieve_phase(node_t *fanout, node_t *node) {
    int       i;
    node_bp_t *record;
...
lsList new_find_bypass_nodes(network_t *networknetwork, double epsilon, double mux_delay, delay_model_t model) {
    ...
    for (i = 0; i < array_n(nodes); ++i) {
        ...
        foreach_fanout(node, gen1, fanout) {
            ...
            if (node_record->path_fanin != node)
                continue;
            if (node_record->input_phases[j] == BINATE)
                continue;
            if (node_record->input_phases[j] == PHASE_UNKNOWN)
                continue;
            if (node_function(fanout) == NODE_PO)
                continue;
        ...
        foreach_list_item(bypass_fanouts, gen, fanout) {
            new_trace_bypass(fanout, node, epsilon, mux_delay, NIL(node_t), model);
        }
    }
    array_free(nodes);
    return bypasses;
...
void new_trace_bypass(node_t *fanout, node_t *node, double epsilon, double mux_delay, node_t *last_bypass_ended,
                      delay_model_t model) {
    ...
    if ((start_node_mode) &&
        ...
        if (slack - slackoffset > 0)
            slack++;
        else
            return;
    } else if (slack - slackoffset < edgeweight)
        return;
    ...
    if (reg_code && (st_lookup(bypass_table, (char *) bypass->last_node,
                               (char **) &old_bypass))) {
...
lsList newer_find_bypass_nodes(network_t *network, double epsilon, double mux_delay, delay_model_t model) {
    array_t   *nodes;
    int       i, j;
    node_t    *node, *fanout;
    node_bp_t *node_record;
    ...
    for (i = 0; i < array_n(nodes); ++i) {
        ...
        /* Now try to extend each bypass */
        foreach_list_item(bypass_fanouts, gen, fanout) {
            newer_trace_bypass(fanout, node, epsilon, mux_delay, NIL(node_t), model);
        }
    }
    array_free(nodes);
    return bypasses;
...
void newer_trace_bypass(node_t *fanout, node_t *node, double epsilon, double mux_delay, node_t *last_bypass_ended,
                        delay_model_t model) {
    double   slack, slackoffset, edgeweight;
    node_t   *fanin;
    int      j;
...
void extend_bypass(bypass_t *bypass, node_t *node, double mux_delay) {
    ...
    while ((bypass->gain <= bypass->side_slack) && (fanouts != LS_NIL)) {
    ...
    if (bypass->gain <= 0) {
        ...
        */
        free_bypass(bypass);
    } else if (st_lookup(bypass_table, (char *) bypass->last_node,
                         (char **) &old_bypass)) {
        if (bypass->gain > old_bypass->gain) {
...
void clean_global_tables(network_t *network) { st_free_table(bypass_table); }

void gbx_clean_node_table(network_t *network) {
    array_t   *nodes;
    int       i;
    node_t    *node;
    node_bp_t *node_record;
...
void print_bypass(bypass_t *bypass) {
    lsGen  gen;
    node_t *node;
...
int do_gbx_transform(network_t *network, double epsilon, double mux_delay, delay_model_t model, gbx_trace_t new_proc,
                     double actual_mux_delay) {
    st_table  *cut_table;
    array_t   *nodes;
    node_t    *node;
    bypass_t  *bypass;
    ...
    switch (new_proc) {
        case GBX_NEW_TRACE:bypasses = new_find_bypass_nodes(network, epsilon, mux_delay, model);
            break;
        case GBX_NEWER_TRACE:bypasses = newer_find_bypass_nodes(network, epsilon, mux_delay, model);
            break;
        default:bypasses = newer_find_bypass_nodes(network, epsilon, mux_delay, model);
    }
    ...
    for (i         = 0; i < array_n(nodes); ++i) {
        ...
        if (st_lookup(bypass_table, (char *) node, (char **) &bypass)) {
            ...
            if (take_bypass(bypass, network, model, mux_delay, actual_mux_delay)) {
                ++ntaken;
                taken = 1;
                /*             com_execute(&network, "verify -m bdd
                   /users/harkness/GBX/bench/5xp1.blif.opt"); com_execute(&network,
                   "write_eqn"); */
            } else
                ++nnottaken;
        } else {
            st_lookup(gbx_node_table, (char *) node, (char **) &node_record);
            if (node_record->slack == 0.0)
                no_improvement = 1;
        }
    }
    if (gbx_verbose_mode) {
...
int bypass_compare(char **bypass1, char **bypass2) {
    return ((bypass_t *) *bypass2)->gain - ((bypass_t *) *bypass1)->gain;
}

int do_all_bypasses(network_t *network, double epsilon, double mux_delay, double actual_mux_delay, delay_model_t model,
...
void print_epsilon_paths(lsList nodelist, node_t *node, double epsilon) {
    lsGen     gen;
    lsGen     gen1;
    node_t    *fanout;
    node_bp_t *record;
    ...
    if (node_function(node) == NODE_PO) {
        foreach_list_item(nodelist, gen1, node) { printf("%s ", node_name(node)); }
        printf("\n");
        return;
    } else {
        foreach_fanout(node, gen, fanout) {
...
void print_epsilon_network(network_t *network, delay_model_t model, double epsilon) {
    node_t    *pi;
    lsGen     gen;
    lsList    nodelist;
    node_bp_t *record;
...
int do_GBX(network_t **network, int adaptive_epsilon, double epsilon, gbx_trace_t new_trace_procedure, double mux_delay,
           double target_delay, char target_chosen, delay_model_t model, int cutset_mode) {
    int       retcode, ret;
    ...
    if (adaptive_epsilon)
        epsilon      = initial_artime - target_delay;
    artime           = initial_artime;

    while (artime > target_delay) {
        lastartime  = artime;
        if (target_chosen && adaptive_epsilon)
            epsilon = artime - target_delay;
        else if (adaptive_epsilon)
            epsilon = min(epsilon, artime - target_delay);
        if (gbx_verbose_mode)
        ...
        if (cutset_mode)
            ...
            retcode = do_all_bypasses(*network, epsilon, mux_delay, actual_mux_delay,
        ...
        switch (retcode) {
            case 0:
                ...
                if ((artime == lastartime) && (mux_delay > 1)) {
                    ...
                    gbx_init_node_table(*network, model);
                    if (cutset_mode)
                        ret = do_gbx_transform(*network, epsilon, mux_delay, model,
                                               new_trace_procedure, actual_mux_delay);
                    else
                        ret = do_all_bypasses(*network, epsilon, mux_delay, actual_mux_delay,
                                              model, new_trace_procedure);
                    if (gbx_verbose_mode) {
                        switch (ret) {
            ...
            case 3:
                ...
                break;
            case 2:(void) delay_latest_output(*network, &lastartime);
                if (gbx_verbose_mode) {
...
int
do_bypass_transform(network_t **network, double epsilon, int adaptive_epsilon, double mux_delay, delay_model_t model,
                    gbx_trace_t new_trace_procedure, int verbose_mode, int cutset_mode) {
    double target_delay;
    char   target_chosen;
    ...
    do_GBX(network, adaptive_epsilon, epsilon, new_trace_procedure, mux_delay,
           target_delay, target_chosen, model, cutset_mode);
