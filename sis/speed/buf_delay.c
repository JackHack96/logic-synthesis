...
void sp_subtract_delay(pin_phase_t phase, delay_time_t block, delay_time_t drive, double load, delay_time_t *req) {
    delay_time_t delay;
                                   ...
                                   int k;
{
    ...
    load = cap_K + sp_get_pin_load(nodeL, model) +
           compute_wire_load(node_network(node), 1);
                                      ...
                                      st_table **load_table, **drive_table;
{
    ...
    *drive_table = st_init_table(st_ptrcmp, st_ptrhash);
    default_inv_index = (buf_param->num_inv > 1 ? buf_param->num_inv - 2
                                                : buf_param->num_inv - 1);
    ...
    if (setting_env && buf_param->interactive) {
        (void) fprintf(
                sisout,
                "WARNING: Unspecified primary output loads assumed to be %6.4f\n",
                load);
    }
    ...
    if (setting_env && buf_param->interactive) {
        (void) fprintf(sisout,
                       "WARNING: Unspecified primary input drive assumed to be  "
                       "(%6.4f:%6.4f)\n",
