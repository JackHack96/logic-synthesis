...
#define SP_DELAY_IMPROVED(fanout_flag, old, new)                               \
  (fanout_flag != CLP ? (new.rise > old.rise + NSP_EPSILON &&new.fall >        \
                         old.fall + NSP_EPSILON)                               \
                      : (new.rise < old.rise - NSP_EPSILON &&new.fall <        \
                         old.fall - NSP_EPSILON))
 ...
 */
/* ARGSUSED */
int new_speed(network_t *network, speed_global_t *speed_param) {
    int      status;
    double   saved_value, slack_diff;
    st_table *table;

    if (network_num_pi(network) == 0)
        return 0; /* No improvement possibe */
    /*
...
static int new_speed_recur(network_t *network, speed_global_t *speed_param, int level) {
    int          i, j;
    sp_clp_t     *rec;
    lsGen        gen, gen1;
    sp_weight_t  *wght;
    ...
    if (level >= speed_param->max_recur) {
        if (speed_param->debug > 1)
            (void) fprintf(sisout, "HIT RECURSION LIMIT\n");
        return success;
    }
    /*
    ...
    mincut = new_speed_select_xform(network, speed_param, clp_table, epsilon);
    ...
    if (speed_param->debug) {
        ...
        for (i = 0; i < array_n(mincut); i++) {
            ...
            (void) fprintf(sisout, "\t%10s :d=(%.2f) a=%6.2f %s\n",
                           node_long_name(node), wght->improvement[j],
                           wght->area_cost[j],
    ...
    for (i         = 0; should_recur && i < array_n(network_array); i++) {
        ...
        if (rec->ti_model->type != CLP) {
            ...
            foreach_primary_output(rec->net, gen, po) {
                ...
                delta          = array_fetch(double, rec->delta, j++);
                delta          = MAX(0.0, delta);
    ...
    if (need_recursion) {
        ...
        for (i = 0; i < array_n(network_array); i++) {
            ...
            if (recur_helped) {
                ...
                opt_netw =
                        (*rec->ti_model->optimize_func)(rec->net, rec->ti_model, rec->glb);
    ...
    for (i = 0; i < array_n(network_array); i++) {
        ...
        if (speed_param->debug > 1) {
            (void) fprintf(sisout, "At %s FINAL %s %s %.2f:%-.2f\n", rec->name,
                           (delay_improved ? "IMPROVED" : "ORIGINAL"),
                           (rec->ti_model->type == FAN
                            ? "REQUIRED"
    ...
    foreach_primary_output(network, gen, fo) {
        ...
        if (node_function(fanin) == NODE_BUF &&
            ...
            foreach_fanout(fanin, gen1, fan) { (void) node_collapse(fan, fanin); }
...
void sp_expand_selection(speed_global_t *speed_param, array_t *mincut, st_table *clp_table, st_table **select_table) {
    int          i, k;
    node_t       *node;
    sp_weight_t  *wght;
    st_generator *stgen;
    ...
    *select_table = st_init_table(st_ptrcmp, st_ptrhash);
    for (i = 0; i < array_n(mincut); i++) {
        node = array_fetch(node_t *, mincut, i);
        (void) st_insert(*select_table, (char *) node, NIL(char));
    }
 ...
 */
void speed_reorder_cutset(speed_global_t *speed_param, st_table *clp_table, array_t **mincut) {
    array_t *new_mincut;
 ...
 */
static void sp_compute_delta(network_t *network, speed_global_t *speed_param, array_t *network_array, int current_pos) {
    int           i;
    double        load;
    array_t       *temp;
    lsGen         gen, gen1;
    ...
    rec->delta = array_alloc(double, 0);
    if (rec->ti_model->type == CLP) {
        ...
        array_insert_last(double, rec->delta, delta);

        /* Record original slacks of the inputs */
        temp = array_alloc(double, 0);
        foreach_primary_input(rec->net, gen, node) {
            ...
            if (orig != NIL(node_t)) {
                ...
                array_insert_last(double, temp, min_slack);
            } else {
        ...
        foreach_primary_input(rec->net, gen, node) {
            ...
            delta     = array_fetch(double, temp, i++) - min_slack;
            array_insert_last(double, rec->delta, delta);
        }
        ...
        delay_wire_required_time(network_get_pi(rec->net, rec->cfi), 0,
                                 speed_param->model, &new);
    ...
    if (speed_param->debug > 1) {
        (void) fprintf(sisout, "At %s INITIAL %.2f to %.2f DELTA %.2f\n", rec->name,
                       rec->old.rise, new.rise, array_fetch(double, rec->delta, 0));
    }
}
 ...
 */
static void sp_set_updated_constraints(sp_clp_t *rec, speed_global_t *speed_param, network_t *network) {
    node_t       *node, *orig;
 ...
 */
network_t *sp_get_network_to_collapse(node_t *f, speed_global_t *speed_param, double *areap) {
    network_t *net;
 ...
 */
static void
nsp_set_initial_load(network_t *orig_netw, network_t *net, speed_global_t *speed_param, st_table *dup_nodes) {
    int         pin;
    lsGen       gen, gen1;
    node_t      *pi, *orig, *fo;
    delay_pin_t *pin_delay;
 ...
 */
static array_t *new_speed_collapse_bfs(node_t *f, speed_global_t *speed_param, st_table *table) {
    int          i, j, k;
                    ...
                    if (should_add) {
                        ...
                        if (speed_param->region_flag == ONLY_TREE) {
                            if (node_num_fanout(fi) == 1) {
                                ...
                                (void) st_insert(table, (char *) fi, (char *) (new_dist));
                                array_insert_last(node_t *, temp_array, fi);
                                /*
                                     (void)fprintf(sisout, "%s -> %s(%d) : fo= %d, fi = %d\n",
                                   temp->name, fi->name, new_dist, node_num_fanout(fi),
                                   node_num_fanin(fi));
                                */
                            } else if (cur_dist >= 0) {
                                /*
                                ...
                                while (node_num_fanin(fi) == 1) {
                                    ...
                                    array_insert_last(node_t *, temp_array, fi);
                                    /*
                                         (void)fprintf(sisout, "%s -> INV %s(%d) : fo= %d, fi =
                                       %d\n", temp->name, fi->name,
                                       new_dist-1,node_num_fanout(fi), node_num_fanin(fi));
                                    */
                                    inv_fi = node_get_fanin(fi, 0);
                                    fi     = inv_fi;
                                }
                                if (fi->type == PRIMARY_INPUT ||
                                    st_is_member(table, (char *) fi) || cur_dist == 0) {
                                    need_to_add = FALSE;
                                }
                                ...
                                if (need_to_add) {
                                    ...
                                    /*
                                         (void)fprintf(sisout, "%s -> %s(%d) : fo= %d, fi = %d\n",
                                       temp->name, fi->name, new_dist-1,node_num_fanout(fi),
                                       node_num_fanin(fi));
                                    */
                                }
                            }
    ...
    if (speed_param->region_flag == COMPROMISE) {
        ...
        if (speed_param->debug > 2) {
            (void) fprintf(sisout, "At %s COMPROMISE added %d nodes in [%.2f %.2f]\n",
                           node_name(f), (array_n(temp_array) - last), min_arr,
                           max_arr);
 ...
 */
static network_t *sp_generate_network_from_array(array_t *node_array, speed_global_t *speed_param) {
    int          i, j;
    char         *name;
    double       load;
    network_t    *net;
        ...
        foreach_fanin(node, j, fi) {
            if (st_lookup(dup_table, (char *) fi, (char **) &dup_fi)) {
                ...
                if (!st_lookup(pi_table, (char *) fi, (char **) &pi_node)) {
                     ...
                     */
                    /*
                                        pin_delay = get_pin_delay(node, j,
                       speed_param->model); delay_set_parameter(pi_node,
                       DELAY_MAX_INPUT_LOAD, pin_delay->load);
                    */
 ...
 */
static delay_time_t nsp_get_input_drive(node_t *node, delay_model_t model) {
    int          i;
    delay_time_t drive;
    delay_pin_t  *pin_delay;
 ...
 */
static void sp_do_initial_resynthesis(sp_clp_t *rec) {
    int          tmp;
    node_t       *np;
    delay_time_t new;
    network_t    *opt_netw;
    ...
    tmp = rec->glb->debug;
    rec->glb->debug = FALSE;
    opt_netw = (*rec->ti_model->optimize_func)(rec->net, rec->ti_model, rec->glb);
    network_free(rec->net);
    rec->net = opt_netw;
    ...
    if (rec->glb->debug) {
        (void) fprintf(sisout,
                       "At %10s \"%s\" new= %.2f:%.2f old= %.2f:%.2f CS=%.2f \n",
 ...
 */
static array_t *speed_filter_cutset(array_t *mincut, speed_global_t *speed_param, st_table *weight_table) {
    lsGen       gen;
    ...
    for (i    = array_n(mincut); i-- > 0;) {
        ...
        if (root_wght->improvement[root_wght->best_technique] < NSP_EPSILON) {
            (void) st_insert(del_table, (char *) root, NIL(char));
            continue;
        }
        /*
        ...
        for (j = array_n(mincut); j-- > 0;) {
            ...
            if (st_is_member(table, (char *) node)) {
                ...
                if (save_node == FALSE) {
                    ...
                    if (speed_param->debug) {
                        (void) fprintf(sisout, "%s (%.2f) in TFI of %s (%.2f)\n", node->name,
                                       SP_IMPROVEMENT(wght), root->name,
                                       SP_IMPROVEMENT(root_wght));
                    }
                }
                array_free(temp_array);
            }
