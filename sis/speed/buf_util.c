 ...
 */
void buffer_alloc(node) node_t *node;
{
 ...
 */
void buffer_free(node) node_t *node;
 ...
 */
void buf_set_buffers(network, use_mapped, buf_param)
 ...
 */
void buf_free_buffers(buf_param) buf_global_t *buf_param;
{
...
static int speed_lib_buffers(lib, limit, buf_param,
                             p_min_req_diff) library_t *lib;
int limit;
buf_global_t *buf_param;
...
void speed_dump_buffer_list(fp, buffer_list) FILE *fp;
lsList buffer_list;
{
  lsGen gen;
  char *dummy;
...
static int sp_compare_buf(item1, item2) lsGeneric item1, item2;
{
 ...
 */
void sp_buf_array_from_list(buf_param) buf_global_t *buf_param;
{
  int i = 0;
  lsGen gen;
  lsGeneric dummy;
 ...
 */
void sp_set_inverter(node, fanin, gate) node_t *node, *fanin;
lib_gate_t *gate;
{
  char *inv_formals[1];
 ...
 */
/* ARGSUSED */
void sp_buf_annotate_gate(node, buf_param) node_t *node;
buf_global_t *buf_param;
{
  int cfi;
  lib_gate_t *gate;
...
lib_gate_t *buf_get_gate_version(gate, version) lib_gate_t *gate;
int version;
{
  lsGen gen;
  char *dummy;
 ...
 */
int buf_failed_slack_test(node, buf_param, gate_version, op_req,
                          op_load) node_t *node;
buf_global_t *buf_param;
int gate_version;
delay_time_t op_req;
 ...
 */
int sp_buf_get_crit_fanin(node, model) node_t *node;
delay_model_t model;
{
  int i, cfi;
  node_t *fanin;
 ...
 */
void set_buf_thresh(network, buf_param) network_t *network;
buf_global_t *buf_param;
{
  delay_time_t time;
 ...
 */
int buf_critical(np, buf_param) node_t *np;
buf_global_t *buf_param;
{
  delay_time_t time;
 ...
 */
void sp_replace_lib_gate(node, gate, pref_gate) node_t *node;
 ...
 */
char *sp_name_of_impl(node) node_t *node;
{
 ...
 */
char *sp_buffer_name(buffer) sp_buffer_t *buffer;
{
  int i;
  static char name[BUFSIZ];
 ...
 */
static void sp_insert_buf_in_list(buffer_list, buf_param, prev,
                                  gate) lsList buffer_list;
buf_global_t *buf_param;
sp_buffer_t *prev;
lib_gate_t *gate;
{
 ...
 */
void speed_free_buffer_list(list) lsList list;
{
  lsGen gen;
  lsGeneric dummy;
 ...
 */
void sp_implement_buffer_chain(network, node) network_t *network;
node_t *node;
{
  lsGen gen;
  int n, i, j;
 ...
 */
void buf_add_implementation(node, gate) node_t *node;
lib_gate_t *gate;
{
  int i, nin;
  char **formals;
...
void buf_init_top_down(network, mode, debug) network_t *network;
int mode, debug;
{
...
void buf_map_interface(network, data) network_t *network;
...
delay_time_t buf_get_required_time_at_input(node) node_t *node;
{ return (BUFFER(node)->req_time); }

void buf_set_required_time_at_input(node, req) node_t *node;
delay_time_t req;
{ BUFFER(node)->req_time = req; }

void buf_set_prev_drive(node, drive) node_t *node;
delay_time_t drive;
{ BUFFER(node)->prev_dr = drive; }

void buf_set_prev_phase(node, phase) node_t *node;
pin_phase_t phase;
{ BUFFER(node)->prev_ph = phase; }
