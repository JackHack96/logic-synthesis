 ...
 */
void buffer_free(node_t *node) { FREE(node->BUFFER_SLOT); }
 ...
 */
void buf_set_buffers(network_t *network, int use_mapped, buf_global_t *buf_param) {
    double    d;
    library_t *lib;
 ...
 */
void buf_free_buffers(buf_global_t *buf_param) {
...
static int speed_lib_buffers(library_t *lib, int limit, buf_global_t *buf_param, double *p_min_req_diff) {
    lsGen       gen;
    lsGeneric   dummy;
    lib_gate_t  *gate;
    lib_class_t *class;
    ...
    *p_min_req_diff = 0.0;
    ...
    if (lib == NIL(library_t)) {
        sp_insert_buf_in_list(buf_param->buffer_list, buf_param, NIL(sp_buffer_t),
                              NIL(lib_gate_t));
        return 1;
    }
...
void speed_dump_buffer_list(FILE *fp, lsList buffer_list) {
    lsGen       gen;
    char        *dummy;
    sp_buffer_t *buffer;
    double      d1, d2;

    (void) fprintf(fp, "type        name       area ipcap  blck_r blck_f drve_r "
                       "drve_f rise   fall\n");
    (void) fprintf(fp, "----------------------------------------------------------"
                       "----------------\n");
    ...
    while (lsNext(gen, &dummy, LS_NH) == LS_OK) {
        ...
        (void) fprintf(
                fp,
                "%s    %-14s %5.1f %-6.3f %-6.3f %-6.3f %-6.3f %-6.3f %-6.3f %-6.3f\n",
                (buffer->phase == PHASE_INVERTING ? "INV" : "BUF"),
                sp_buffer_name(buffer), buffer->area, buffer->ip_load,
                buffer->block.rise, buffer->block.fall, buffer->drive.rise,
                buffer->drive.fall, d1, d2);
    }
    (void) lsFinish(gen);
}

/* Sort the buffers according to size -- hope that the smallest is slowest */
static int sp_compare_buf(lsGeneric item1, lsGeneric item2) {
    ...
    if (buf1->phase == buf2->phase) {
        if (buf2->area < buf1->area)
            return -1;
        else if (buf2->area > buf1->area)
            return 1;
        else
            return 0;
    } else {
 ...
 */
void sp_buf_array_from_list(buf_global_t *buf_param) {
    int         i        = 0;
    lsGen       gen;
    lsGeneric   dummy;
    sp_buffer_t *buffer;
 ...
 */
void sp_set_inverter(node_t *node, node_t *fanin, lib_gate_t *gate) {
    char   *inv_formals[1];
 ...
 */
/* ARGSUSED */
void sp_buf_annotate_gate(node_t *node, buf_global_t *buf_param) {
    int          cfi;
    lib_gate_t   *gate;
    pin_phase_t  phase;
...
lib_gate_t *buf_get_gate_version(lib_gate_t *gate, int version) {
    lsGen      gen;
    char       *dummy;
    int        num_gates;
 ...
 */
int
buf_failed_slack_test(node_t *node, buf_global_t *buf_param, int gate_version, delay_time_t op_req, double op_load) {
    int           i;
    node_t        *fanin;
    lib_gate_t    *root_gate;
    ...
    if (new_fom_best < fom_best) {
        if (buf_param->debug > 10) {
            ...
            (void) fprintf(sisout, "\tNode %s, gate %s, R=%.3f:%.3f, L=%.3f\n",
                           node->name, root_gate->name, op_req.rise, op_req.fall,
                           op_load);
        }
        return 1;
    }
 ...
 */
int sp_buf_get_crit_fanin(node_t *node, delay_model_t model) {
    int          i, cfi;
    node_t       *fanin;
    double       fom, fom_best;
 ...
 */
void set_buf_thresh(network_t *network, buf_global_t *buf_param) {
    delay_time_t time;
 ...
 */
int buf_critical(node_t *np, buf_global_t *buf_param) {
    delay_time_t time;

    time = delay_slack_time(np);
    if (time.rise <= buf_param->crit_slack ||
        time.fall <= buf_param->crit_slack) {
        return TRUE;
    } else {
        return FALSE;
    }
 ...
 */
void sp_replace_lib_gate(node_t *node, lib_gate_t *gate, lib_gate_t *pref_gate) {
    int    i, nin;
    char   **formals;
 ...
 */
char *sp_name_of_impl(node_t *node) {
 ...
 */
char *sp_buffer_name(sp_buffer_t *buffer) {
    int         i;
    static char name[BUFSIZ];

    if (buffer == NIL(sp_buffer_t))
        (void) strcpy(name, "NONE");
    if (buffer->gate[0] == NIL(lib_gate_t)) {
        (void) strcpy(name, "UNIT_FAN");
    } else {
 ...
 */
static void sp_insert_buf_in_list(lsList buffer_list, buf_global_t *buf_param, sp_buffer_t *prev, lib_gate_t *gate) {
    int          i;
    delay_time_t block;
    ...
    if (prev == NIL(sp_buffer_t)) {
        ...
        buffer->block.rise =
                prev->block.fall + prev->drive.fall * load + block.rise;
        buffer->block.fall =
                prev->block.rise + prev->drive.rise * load + block.fall;
        buffer->drive    = ((delay_pin_t *) (gate->delay_info[0]))->drive;
 ...
 */
void speed_free_buffer_list(lsList list) {
    lsGen     gen;
    lsGeneric dummy;
 ...
 */
void sp_implement_buffer_chain(network_t *network, node_t *node) {
    lsGen       gen;
    int         n, i, j;
    ...
    if (BUFFER(node)->type == BUF) {
        ...
        if (buffer->gate[0] != NIL(lib_gate_t)) {
            ...
            if (buffer->depth == 1) {
                ...
                i       = 0;
                foreach_fanout(node, gen, fo) { fanouts[i++] = fo; }
                sp_set_inverter(node, fanin, buffer->gate[0]);
                BUFFER(node)->type      = GATE;
                BUFFER(node)->cfi       = 0;
                BUFFER(node)->impl.gate = buffer->gate[0];
 ...
 */
void buf_add_implementation(node_t *node, lib_gate_t *gate) {
    int    i, nin;
    char   **formals;
    node_t **actuals, *fanin;
...
void buf_init_top_down(network_t *network, int mode, int debug) {
...
void buf_map_interface(network_t *network, buf_alg_input_t *data) {
    int          level = 0;
...
delay_model_t buf_get_model() { return (map_buf_param.model); }

double buf_get_auto_route() { return (map_buf_param.auto_route); }

delay_time_t buf_get_required_time_at_input(node_t *node) { return (BUFFER(node)->req_time); }

void buf_set_required_time_at_input(node_t *node, delay_time_t req) { BUFFER(node)->req_time = req; }

void buf_set_prev_drive(node_t *node, delay_time_t drive) { BUFFER(node)->prev_dr = drive; }

void buf_set_prev_phase(node_t *node, pin_phase_t phase) { BUFFER(node)->prev_ph = phase; }
