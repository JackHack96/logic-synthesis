 ...
 */
int sp_buffer_recur(network, buf_param, fanout_data, req_diff,
                    levelp) network_t *network;
 ...
 * critical
 */
int buf_compare_fanout(ptr1, ptr2) char *ptr1, *ptr2;
{
...
static int buf_cumulative_cap(fanouts, buf_param, num_pos, num_neg, cap_K_posp,
                              cap_K_negp, cap_L_posp,
                              cap_L_negp) sp_fanout_t *fanouts;
buf_global_t *buf_param;
int num_pos, num_neg;
...
void buf_dump_fanout_data(fp, fanouts, n) FILE *fp;
sp_fanout_t *fanouts;
int n;
{
  int i;
...
static void buf_implement_trans3(network, node, node_inv, fanouts,
                                 use_noninv_buf, num_pos, num_neg, best_mn,
                                 best_mp, new_b, new_B) network_t *network;
 ...
 */
static int buf_annotate_trans3(network, buf_param, fanouts, use_noninv_buf,
                               node, node_inv, new_b, new_B, best_g, best_gI,
                               best_b, best_B, margin_pos,
                               margin_neg) network_t *network;
buf_global_t *buf_param;
sp_fanout_t *fanouts;
...
static delay_pin_t *sp_get_versions_of_node(node, buf_param, num_gates,
                                            a_array) node_t *node;
buf_global_t *buf_param;
int *num_gates;
double **a_array;
 ...
 */
int buf_load_constraint_met(node) node_t *node;
{
  int cfi;
  double limit, load;
