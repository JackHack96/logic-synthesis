 ...
 */
int sp_buffer_recur(network_t *network, buf_global_t *buf_param, buf_alg_input_t *fanout_data, delay_time_t req_diff,
                    int *levelp) {
    ...
    if (n == 1 && current_level == 1) {
        if (buf_param->debug > 10) {
            (void) fprintf(
                    sisout, "********Iteration %d -- %d nodes -- aim to save %.3f:%.3f\n",
        ...
        changed = sp_replace_cell_strength(
                network, buf_param, fanout_data, gate_array, num_gates, req_time,
                large_req_time, total_cap_pos, 0.0, &a_saving);
        FREE(gate_array);
        FREE(a_array);
        return changed;
    }
    ...
    if (buf_param->debug > 10) {
        (void) fprintf(sisout,
                       "******Iteration %d -- %d nodes -- aim to save %.3f:%.3f\n",
                       *levelp, n, req_diff.rise, req_diff.fall);
        (void) fprintf(
                sisout, "\tRequired time and Load distribution (max_ip_load = %.3f) \n",
     ...
     */
    do_unbalanced =
            buf_cumulative_cap(fanouts, buf_param, num_pos, num_neg, &cap_K_pos,
                               &cap_K_neg, &cap_L_pos, &cap_L_neg);
    ...
    if (buf_param->debug > 10) {
        (void) fprintf(sisout,
                       "Initial req_time %6.3f:%-6.3f , Target %6.3f:%-6.3f\n",
    ...
    if (current_level == 1 && (buf_param->mode & REPOWER_MASK)) {
        ...
        changed = sp_replace_cell_strength(
                network, buf_param, fanout_data, gate_array, num_gates, pos_min_req,
                neg_min_req, total_cap_pos, total_cap_neg, &a_saving);
                        ...
                        for (B = 0; B < num_buf; B++) { /* choice of buffer B */
                            ...
                            for (g = 0; g < num_gates; g++) { /* Versions of root */
                                ...
                                sp_subtract_delay(pin_delay->phase, pin_delay->block,
                                                  pin_delay->drive, load_op_g, &req_g);
     ...
     */
    if (buf_param->interactive && !met_target && REQ_IMPR(best, orig_req) &&
    ...
    if (met_target || REQ_IMPR(best, orig_req)) {
        if (!met_target) {
            ...
            sp_drive_adjustment(pin_delay->drive,
                                buf_array[best_gI]->ip_load + auto_route,
                                &best_req_gI);
            sp_drive_adjustment(pin_delay->drive,
            ...
            if ((buf_param->mode & BALANCED_MASK) && num_pos == 0 &&
                ...
                changed += buf_evaluate_trans2(network, buf_param, fanout_data,
                                               gate_array, a_array, num_gates,
                                               cap_K_pos, cap_K_neg, work_diff, levelp);
                goto exit_recursion;
            }
        }
        ...
        (void) buf_annotate_trans3(network, buf_param, fanouts, use_noninv_buf, node,
                                   new_inv_node, new_b, new_B, best_g, best_gI,
                                   best_b, best_B, &margin_pos, &margin_neg);
        ...
        if (use_noninv_buf
            ...
            if (use_noninv_buf) {
                ...
                if (new_pos > 0 && (margin_pos.rise > EPS && margin_pos.fall > EPS)) {
                    ...
                    new_fanout_data->max_ip_load =
                            buf_B->ip_load + BUF_MIN_D(margin_pos) / root_node_drive;
                ...
                if (new_neg > 0 && (margin_neg.rise > EPS && margin_neg.fall > EPS)) {
                    ...
                    new_fanout_data->num_neg = 0;
                    new_fanout_data->max_ip_load =
                            buf_B->ip_load + BUF_MIN_D(margin_neg) / root_node_drive;
                    new_fanout_data->node     = new_B;
                    new_fanout_data->inv_node = NIL(node_t);
        ...
        if (excess) {
            ...
            if (use_noninv_buf) {
                ...
                old_fanout_data->fanouts[old_pos + old_neg].req =
                        BUFFER(new_b)->req_time;
                load += old_fanout_data->fanouts[old_pos + old_neg].load;
                MIN_DELAY(req_g, req_g,
                          old_fanout_data->fanouts[old_pos + old_neg].req);
            } else {
                old_fanout_data->fanouts[old_pos + old_neg].fanout = new_B;
                old_fanout_data->fanouts[old_pos + old_neg].load =
        ...
        changed +=
                sp_buffer_recur(network, buf_param, old_fanout_data, margin, levelp);
        ...
        changed += buf_evaluate_trans2(network, buf_param, fanout_data, gate_array,
                                       a_array, num_gates, cap_K_pos, cap_K_neg,
                                       margin, levelp);
 ...
 * critical
 */
int buf_compare_fanout(char *ptr1, char *ptr2) {
...
static int
buf_cumulative_cap(sp_fanout_t *fanouts, buf_global_t *buf_param, int num_pos, int num_neg, double **cap_K_posp,
                   double **cap_K_negp, double **cap_L_posp, double **cap_L_negp) {
    int          i, n, j;
    int          do_unbalanced;
    delay_time_t t;
...
void buf_dump_fanout_data(FILE *fp, sp_fanout_t *fanouts, int n) {
    int i;

    for (i = 0; i < n; i++) {
        (void) fprintf(fp, "%6.3f:%-6.3f @ (%s)%6.3f   ", fanouts[i].req.rise,
                       fanouts[i].req.fall,
...
static void
buf_implement_trans3(network_t *network, node_t *node, node_t *node_inv, sp_fanout_t *fanouts, int use_noninv_buf,
                     int num_pos, int num_neg, int best_mn, int best_mp, node_t **new_b, node_t **new_B) {
    node_t *fo;
 ...
 */
static int
buf_annotate_trans3(network_t *network, buf_global_t *buf_param, sp_fanout_t *fanouts, int use_noninv_buf, node_t *node,
                    node_t *node_inv, node_t *new_b, node_t *new_B, int best_g, int best_gI, int best_b, int best_B,
                    delay_time_t *margin_pos, delay_time_t *margin_neg) {
    int          excess;
    pin_phase_t  prev_ph;
    ...
    if (new_b != NIL(node_t)) {
        ...
        sp_subtract_delay(buf_b->phase, buf_b->block, buf_b->drive, load, &req_b);
     ...
     */
    cap_B = 0;
    req_B = (use_noninv_buf ? large_req_time : req_b);
    if (new_B != NIL(node_t)) {
        buf_B                   = buf_param->buf_array[best_B];
        BUFFER(new_B)->type        = BUF;
        BUFFER(new_B)->impl.buffer = buf_B;
     ...
     */
    cap_gI = 0;
    req_gI = neg_branch_req = large_req_time;
    if (node_inv != NIL(node_t)) {
        ...
        sp_subtract_delay(buf_gI->phase, buf_gI->block, buf_gI->drive, load,
                          &req_gI);
    ...
    if (buf_param->debug > 10) {
        ...
        if (use_noninv_buf) {
            (void) fprintf(sisout,
                           "\tAfter Trans3: margin_pos =  %6.3f:%-6.f margin_neg =  "
                           "%6.3f:%-6.f\n",
...
static delay_pin_t *sp_get_versions_of_node(node_t *node, buf_global_t *buf_param, int *num_gates, double **a_array) {
    char          *dummy;
    int           i, j, cfi;
    lsGen         gate_gen;
 ...
 */
int buf_load_constraint_met(node_t *node) {
    int    cfi;
    double limit, load;
