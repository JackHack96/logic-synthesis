...
int sp_replace_cell_strength(network, buf_param, fanout_data, gate_array,
                             num_gates, crit_pos, crit_neg, total_cap_pos,
                             total_cap_neg, save) network_t *network;
...
delay_time_t *save;                  /* RETURN: saving of transformation */
{
  ...
  if (buf_param->debug > 10) {
    ...
    (void)fprintf(sisout,
                  "\t (+)%5.3f at %6.3f:%-6.3f and (-)%5.3f at %6.3f:%-6.3f\n",
  ...
  if (REQ_IMPR(best, orig_req)) {
    ...
    sp_subtract_delay(prev_ph, prev_bl, prev_dr, total_cap_pos + cap_gI,
                      &req_g);
