...
int
buf_evaluate_trans2(network_t *network, buf_global_t *buf_param, buf_alg_input_t *fanout_data, delay_pin_t *gate_array,
                    double *a_array, int num_gates, double *cap_K_pos, double *cap_K_neg, delay_time_t req_diff,
                    int *levelp) {
    int             changed;
    int             met_target;
    int             k, l, start, end;
    ...
    if (buf_param->debug > 10) {
        (void) fprintf(sisout,
                       "******Trans2-Iteration %d -- aim to save %.3f:%.3f\n",
                ...
                do { /* Partition positive fanouts into "j" groups */
                    for (a = 0; a < num_inv; a++) { /* inv driving pos */
                        ...
                        if (num_pos > 1) {
                            ...
                            req_a  = fanouts[0].req;
                            sp_subtract_delay(buf_a->phase, buf_a->block, buf_a->drive, load,
                                              &req_a);
                        }
    ...
    if (buf_param->debug > 10) {
        (void) fprintf(
                sisout,
                "Without root dupliction balanced trans2 achieves %6.3f:%-6.3f\n",
                best.rise, best.fall);
    }
    ...
    if (REQ_IMPR(best, orig_req)) {
        ...
        */
        buf_annotate_trans2(network, buf_param, fanouts, num_pos, num_neg, node,
                            new_a, new_b, new_gI, best_g, best_gI, best_a, best_b,
                            best_i, best_j);
        ...
        if (!met_target) {
            ...
            */
            changed += buf_evaluate_trans2(network, buf_param, new_fanout_data,
                                           gate_array, a_array, num_gates, new_K_pos,
                                           new_K_neg, margin, levelp);
...
static void
buf_implement_trans2(network_t *network, buf_global_t *buf_param, node_t *node, node_t *inv_node, sp_fanout_t *fanouts,
                     int num_pos, int num_neg, int best_i, int best_j, node_t ***new_ap, node_t **new_bp,
                     node_t ***new_gIp) {
    node_t *new_node;
    ...
    if (num_neg > 0 && best_i > 0) {
        ...
        for (i = best_i; i > 0; i--, start = end) {
            ...
            for (l = start; l < end; l++) {
                assert(
                        node_patch_fanin(fanouts[num_pos + l].fanout, inv_node, new_node));
            }
        }
...
static void
buf_annotate_trans2(network_t *network, buf_global_t *buf_param, sp_fanout_t *fanouts, int num_pos, int num_neg,
                    node_t *node, node_t **new_a, node_t *new_b, node_t **new_gI, int best_g, int best_gI, int best_a,
                    int best_b, int best_i, int best_j) {
    int          start, end;
    int          i, j, l, cfi;
    pin_phase_t  prev_ph;
    ...
    if (new_a != NIL(node_t *)) {
        ...
        sp_subtract_delay(buf_b->phase, buf_b->block, buf_b->drive, cap_a, &req_b);
