...
static char *usage[] = {
        "usage: %s [-n name] [-t thresh] [-cHgf] [-m model] [-t n.n] [-w n.n] [-d "
        "n] [-s method]\n",
        ...
        "\"crit\"(default), \"transitive\", \"compromise\", \"tree\"\n",
        NULL};
...
static void
do_speed_plot(network_t *network, char *plot_name, double thresh, int path_flag, int cutset_flag, int print_gate_name,
              delay_model_t model, int dist, double coeff, int fast_mode, int region_flag) {
    int            i, flag;
    FILE           *gfp;
    lsGen          gen;
    node_t         *np;
    ...
    if (cutset_flag || path_flag) {
        ...
        if (speed_param.new_mode) {
            ...
            new_speed_compute_weight(network, &speed_param, clp_table, &eps);
            if (eps < NSP_EPSILON) {
                mincut = array_alloc(node_t *, 0);
            } else {
                mincut = new_speed_select_xform(network, &speed_param, clp_table, eps);
            }
...
int com__speed_plot(network_t **network, int argc, char **argv) {
    int           c;
    ...
    do_speed_plot(*network, plot_name, thresh, hilite_flag, cutset_flag,
                  print_gate_name, model, dist, coeff, new_method, region_flag);
    return 0;
}
