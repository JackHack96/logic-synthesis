...
int speed_weight(node_t *node, speed_global_t *speed_param, double *t_cost, int *a_cost) {
...
static int
speed_weight_bfs(node_t *node, speed_global_t *speed_param, st_table *table, array_t *a_array, array_t *d_array,
                 double o_time, int *crit, int *n, double *min_a, double *max_a, double *min_d, double *sum_a,
                 double *sum_d, double *area) {
 ...
 */
static void speed_mark_input_cone(node_t *node, st_table *area_table, st_table *table, delay_model_t model) {
    array_t      *array;
...
static void linfit(array_t *xin, array_t *yin, double xmin, double ymin, double *slope) {
...
static void speed_sort2(int n, double ra[], double rb[]) {
    int    i, j, ir, l;
    double rra, rrb;
...
static void speed_rank(int n, double w[]) {
    int    j = 1, ji, jt;
    double rank;
 ...
 */
static void
weight_compute(node_t *node, delay_model_t model, array_t *a_array, array_t *d_array, double o_time, double *min_a,
               double *max_a, double *min_d, double *sum_a, double *sum_d) {
    delay_time_t at, st;
...
/* ARGSUSED */
enum st_retval speed_weight_count_lit(char *key, char *value, char *arg) {
    lib_gate_t *gate;
 ...
 */
double sp_compute_duplicated_area(array_t *array, st_table *table, st_table *area_table, delay_model_t model) {
    lsGen  gen;
    double area;
    node_t *temp, *fo;
    int    i, fanout_flag;
...
delay_time_t *sp_compute_side_req_time(node_t *node, st_table *table, st_table *dup_table, delay_model_t model) {
    int          pin;
    lsGen        gen;
    node_t       *fo;
 ...
 */
static delay_time_t sp_compute_input_req(node_t *fo, int pin, delay_model_t model, delay_time_t fo_req) {
    delay_time_t  delay;
    input_phase_t phase;
    ...
    switch (phase) {
        case POS_UNATE:input_req.rise = fo_req.rise - delay.rise;
            input_req.fall            = fo_req.fall - delay.fall;
            break;
        case NEG_UNATE:input_req.fall = fo_req.rise - delay.rise;
            input_req.rise            = fo_req.fall - delay.fall;
            break;
        case BINATE:input_req.rise = fo_req.rise - MAX(delay.rise, delay.fall);
