 ...
 * cube of the node.
 */
node_t *speed_dec_node_cube(node_t *f, int i) {
    node_cube_t cube;
 ...
 */
void speed_del_critical_cubes(node_t *node, speed_global_t *speed_param, double thresh) {
...
node_t *name_to_node(node_t *node, char *name) {
    node_t *fanin;
    int    i;
...
int speed_print_level(network_t *network, array_t *node_array, double thresh, int crit_only, int flag) {
    lsGen        gen;
    node_t       *p;
    char         *dummy;
    st_table     *node_table;
    ...
    foreach_primary_output(network, gen, p) { (void) assign_level(p); }
    ...
    if (node_array == NIL(array_t)) {
        ...
        for (i     = 0; i < array_n(node_array); i++) {
            ...
            (void) st_insert(node_table, (char *) p, NIL(char));
    ...
    for (i = 0; i < array_n(nodevec); i++) {
        ...
        if (p->type != PRIMARY_OUTPUT && st_is_member(node_table, (char *) p)) {
            ...
            if (crit_only && slack.rise > thresh && slack.fall > thresh)
                continue;
            if (st_lookup(level_table, (char *) p, &dummy)) {
                ...
                if (flag)
                    (void) fprintf(sisout, "Node %s not assigned a level", node_name(p));
            }
        }
    }
    if (flag)
...
int assign_level(node_t *node) {
    ...
    if (node->type == PRIMARY_INPUT || new_level > 0) {
        ...
        level = 0;
        foreach_fanin(node, i, fanin) { level = MAX(level, assign_level(fanin)); }
    }
    level++;
...
st_table *speed_levelize_crit(network_t *network, speed_global_t *speed_param, int *max_levelp) {
    array_t  *nodevec;
    lsGen    gen;
    node_t   *p;
    int      i, level, new_level;
 ...
 */
double sp_get_netw_area(network_t *network) {
    lsGen  gen;
    int    mapped_flag;
    double area;
    node_t *node;
...
void speed_get_stats(network_t *network, library_t *library, delay_model_t model, double *delay, double *area) {
    node_t    *np;
    lsGen     gen;
    network_t *new_net;
    ...
    *area = 0.0;
    ...
    foreach_node(new_net, gen, np) { *area += lib_get_gate_area(np); }
    network_free(new_net);
}

void set_speed_thresh(network_t *network, speed_global_t *speed_param) {
    delay_time_t time;
...
void speed_delete_single_fanin_node(network_t *network, node_t *node) {
    int     i;
    node_t  *fo;
    lsGen   gen;
    array_t *array;
    ...
    if (node_num_fanin(node) <= 1) {
        ...
        foreach_fanout(node, gen, fo) { array_insert_last(node_t *, array, fo); }
        /*
...
void speed_network_delete_node(network_t *network, node_t *node) {
    int     i;
    node_t  *fanin;
    array_t *nodevec;
...
lib_gate_t *sp_lib_get_buffer(library_t *lib) {
    network_t   *network;
    lib_class_t *class;
    lib_gate_t  *gate;
 ...
 */
lib_gate_t *sp_lib_get_inv(library_t *lib) {
    network_t   *network;
    lib_class_t *class;
    lib_gate_t  *gate;
...
lib_gate_t *sp_get_gate(lib_class_t *class, int which) {
    lib_gate_t *gate;
    lsGen      gen;
    char       *dummy;
