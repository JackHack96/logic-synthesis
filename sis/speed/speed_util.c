 ...
 * cube of the node.
 */
node_t *speed_dec_node_cube(f, i) node_t *f;
int i;
{
  node_cube_t cube;
 ...
 */
void speed_del_critical_cubes(node, speed_param, thresh) node_t *node;
...
node_t *name_to_node(node, name) node_t *node;
char *name;
{
  node_t *fanin;
  int i;
...
int speed_print_level(network, node_array, thresh, crit_only,
                      flag) network_t *network;
array_t *node_array;
...
int assign_level(node) node_t *node;
{
  node_t *fanin;
...
int level_cmp(p1, p2) char **p1, **p2;
{
  int level1, level2;
  char *dummy;
...
st_table *speed_levelize_crit(network, speed_param,
                              max_levelp) network_t *network;
speed_global_t *speed_param;
 ...
 */
double sp_get_netw_area(network) network_t *network;
{
  lsGen gen;
  int mapped_flag;
  double area;
...
void speed_get_stats(network, library, model, delay, area) network_t *network;
library_t *library;
delay_model_t model;
double *delay;
double *area;
...
void set_speed_thresh(network, speed_param) network_t *network;
speed_global_t *speed_param;
{
  delay_time_t time;
...
void speed_delete_single_fanin_node(network, node) network_t *network;
node_t *node;
{
  int i;
  node_t *fo;
...
void speed_network_delete_node(network, node) network_t *network;
node_t *node;
{
  int i;
  node_t *fanin;
...
lib_gate_t *sp_lib_get_buffer(lib) library_t *lib;
{
  network_t *network;
  lib_class_t *class;
  lib_gate_t *gate;
 ...
 */
lib_gate_t *sp_lib_get_inv(lib) library_t *lib;
{
  network_t *network;
  lib_class_t *class;
  lib_gate_t *gate;
...
lib_gate_t *sp_get_gate(class, which) lib_class_t *class;
