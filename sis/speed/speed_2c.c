...
int twocube_timeout_occured =
    0; /* Flag describing whether timeout has occured */

int speed_2c_decomp(network, node, speed_param, attempt_no) network_t *network;
node_t *node;
...
static int sp_eval_2c_kernel(ddivisor, node_2c, handlep,
                             state) ddivisor_t *ddivisor;
node_t *node_2c;
lsHandle **handlep;
char *state;
{
  ...
  double k_a_cost, c_a_cost;
  ; /* area cost of kern and co-kern */
  ...
  if (node_function(new_cokernel) != NODE_0 &&
      node_num_fanin(new_cokernel) > 1) {
    node_evaluate(node_2c, new_cokernel, store->globals, &k_t_cost, &k_a_cost);
    node_evaluate(new_cokernel, node_2c, store->globals, &c_t_cost, &c_a_cost);
  } else {
    node_evaluate(node_2c, new_cokernel, store->globals, &k_t_cost, &k_a_cost);
    c_t_cost = POS_LARGE;
    c_a_cost = NEG_LARGE;
