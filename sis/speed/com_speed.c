 ...
 */
void speed_set_default_options(speed_global_t *speed_param) {
...
int speed_fill_options(speed_global_t *speed_param, int argc, char **argv) {
    int    c;
    double coeff;
    ...
    if (argc == 0 || argv == NIL(char *))
        return 0;

    while ((c = util_getopt(argc, argv, "a:d:l:m:s:w:t:C:D:I:ABRTcnfipvr")) !=
           EOF) {
        switch (c) {
            case 'f':speed_param->new_mode = FALSE;
                break;
            case 'p':speed_param->add_inv = TRUE;
                break;
            case 'r':speed_param->area_reclaim = FALSE;
                break;
            case 'c':speed_param->speed_repeat = FALSE;
                speed_param->do_script         = FALSE;
                break;
            case 'i':speed_param->only_init_decomp = TRUE;
                speed_param->do_script             = FALSE;
                break;
            case 'A':speed_param->del_crit_cubes = FALSE;
                break;
            case 'B':speed_param->transform_flag = BEST_BANG_FOR_BUCK;
                break;
            case 'I':speed_param->timeout = atoi(util_optarg);
                (void) fprintf(sisout, "INFO: optimization limit = %d seconds\n",
                               speed_param->timeout);
                break;
            case 'R':speed_param->red_removal = TRUE;
                break;
            case 'n':speed_param->objective = TRANSFORM_BASED;
                break;
            case 'T':speed_param->trace = TRUE;
                break;
            case 'v':speed_param->debug = TRUE;
                break;
            case 'D':speed_param->debug = atoi(util_optarg);
                if (speed_param->debug < 0)
                    speed_param->debug  = FALSE;
                break;
            case 's':
            ...
            case 'w':coeff = atof(util_optarg);
                ...
                break;
            case 't':speed_param->thresh = atof(util_optarg);
                speed_param->do_script   = FALSE;
                break;
            case 'a':num_tries = atoi(util_optarg);
                if (num_tries > 0)
                    speed_param->num_tries = num_tries;
                break;
            case 'C':num_cuts = atoi(util_optarg);
                if (num_cuts > 0)
                    speed_param->max_num_cuts = num_cuts;
                break;
            case 'l':level = atoi(util_optarg);
                if (level > 0)
                    speed_param->max_recur = level;
                break;
            case 'd':dist = atoi(util_optarg);
                ...
                speed_param->do_script = FALSE;
                break;
            case 'm':speed_param->model = delay_get_model_from_name(util_optarg);
                ...
                break;
            default:(void) fprintf(siserr, "ERROR: Unknown option\n");
                return 1;
        }
    }
    return 0;
...
static void speed_print_usage() {
    (void) fprintf(siserr,
                   "usage: speed_up [-cinprvABIRT] [-a n] [-d n] [-w n] [-s "
                   "method] [-t n.n] [-l n] [-D n] [-m model] [node-list]\n");
    (void) fprintf(
            siserr, "The options -s, -a, -l, -C are ignored when -f is specified\n");
    ...
    (void) fprintf(siserr,
                   "    -f\t\tFast mode (heuristic weight computation).\n");
    ...
    (void) fprintf(siserr,
                   "    -r\t\tDo not try to reclaim area (EXPERIMENTAL).\n");
    (void) fprintf(siserr, "    -A\t\tArea saving option: Do not delete critical "
                           "cubes of divisors.\n");
    (void) fprintf(
            siserr,
            "    -R\t\tRun redundancy removal after each successful iteration.\n");
    (void) fprintf(siserr, "    -D\tn\tSet the debugging level.\n");
    (void) fprintf(siserr,
                   "    -m\tmodel\tDelay model (default \"unit-fanout\").\n");
    (void) fprintf(siserr,
                   "    -T\t\tPrint the delays as the speed_up proceeds.\n");
    (void) fprintf(sisout, "********* Options for fast mode, -f  **********\n");
    (void) fprintf(siserr, "    -t\tn.n\tCritical threshold.\n");
    (void) fprintf(siserr,
                   "    -a\tn\tNumber of decomposition attempts (default = 1).\n");
    (void) fprintf(siserr, "    -w\tn\tRelative weight of area (default = 0).\n");
    (void) fprintf(sisout, "*********     Options for new mode   **********\n");
    (void) fprintf(siserr,
                   "    -n\t\tChoose the fewest number of transformations\n");
    (void) fprintf(siserr,
                   "    -B\t\tUse Benefit/cost as goodness of transform.\n");
    (void) fprintf(siserr, "    -I\tn\tTimeout limit for each optimization in "
                           "seconds(default = INFINITY).\n");
    (void) fprintf(
            siserr,
            "    -l\tn\t Set the number of recursion levels (default = 1).\n");
    (void) fprintf(
            siserr,
            "    -s\tmethod\tMethod for selecting the region to transform.\n");
    (void) fprintf(siserr, "\t one of \"crit\"(default), \"transitive\", "
                           "\"compromise\", \"tree\"\n");
    (void) fprintf(
            siserr,
            "    -C\tn\t Set the number of cutsets to generate (default = 50).\n");
}
 ...
 * of Boolean networks.
 */
int com_speed_up(network_t **network, int argc, char **argv) {
    int            i;
    char           *name, *new_name;
    ...
    if (lib_network_is_mapped(*network) &&
        ...
        (void) fprintf(
                sisout,
                "WARNING: Mapped network will be decomposed into 2-input gates\n");
    }
    ...
    if (argc - util_optind != 0) {
        ...
        if (delay_trace(*network, speed_param.model)) {
            SP_GET_PERFORMANCE(&speed_param, *network, np, new_latest, new_name,
                               new_area);
        }
        SP_PRINT(&speed_param, latest, new_latest, area, new_area, name, new_name);
        FREE(name);
        FREE(new_name);
    }
 ...
 */
int com__part_collapse(network_t **network, int argc, char **argv) {
    int            i;
    node_t         *node;
    array_t        *nodevec;
    speed_global_t speed_param;
 ...
 */
int com__print_cutset(network_t **network, int argc, char **argv) {
    array_t        *mincut;
    speed_global_t speed_param;
    st_table       *table;
 ...
 */
int com__print_weight(network_t **network, int argc, char **argv) {
    st_table       *table;
    array_t        *nodevec;
    speed_global_t speed_param;
    ...
    print_weight_usage:
    (void) fprintf(siserr, "usage: print_weight [-m model] [-w n.n] [-d n] [-t "
                           "n.n] list-of-nodes\n");
    (void) fprintf(siserr, "    -d\tn\tDistance for collapsing.\n");
    (void) fprintf(siserr, "    -t\tn.n\tCritical threshold.\n");
    (void) fprintf(siserr, "    -w\tn\tRelative weight of area. \n");
...
int com__speed_delay(network_t **network, int argc, char **argv) {
...
int com__report_delay_data(network_t **network, int argc, char **argv) {
    int           c;
    delay_model_t model;
    ...
    while ((c = util_getopt(argc, argv, "m:")) != EOF) {
        switch (c) {
            case 'm':model = delay_get_model_from_name(util_optarg);
                ...
                break;
            default:(void) fprintf(sisout, "Usage:  _report_delay_data [-m model]\n");
                return 1;
        }
    }
    if (model == DELAY_MODEL_UNKNOWN) {
...
static int com_speedup_alg(network_t **network, int argc, char **argv) {
    int no_options = 0;
    int verbose    = 0;
    ...
    if (verbose || no_options)
        sp_print_local_trans(current_speedup_alg);
    return 0;
    usage:
...
int com_print_level(network_t **network, int argc, char **argv) {
    double        thresh;
    delay_model_t model;
    ...
    while ((c = util_getopt(argc, argv, "m:t:lc")) != EOF) {
        switch (c) {
            ...
            case 't':thresh = atof(util_optarg);
                break;
            case 'l':print_flag = FALSE;
                break;
            case 'c':crit_only = TRUE;
                break;
            default:goto print_level_usage;
        }
    }
    ...
    level =
            speed_print_level(*network, node_array, thresh, crit_only, print_flag);
    ...
    (void) fprintf(
            sisout,
            "\t-m \tmodel\t: model for computing delays - default \"unit_fanout\"\n");
    (void) fprintf(sisout, "\t-t \t#.#\t: Threshold defining critical paths\n");
    (void) fprintf(sisout,
                   "\tnode-list\t: Print nodes in transitive-fanin of list.\n");
    return 1;
}
/* ARGSUSED */
/* Utility routine that reports whether the network is in 2-input AND form */
void com__is_2ip_and(network_t **network, int argc, char **argv) {
    node_t *node;
    lsGen  gen;

    foreach_node(*network, gen, node) {
        if ((node_num_fanin(node) <= 2) &&
            ((node_function(node) != NODE_OR) &&
 ...
 * critical paths
 */
int com__print_fanout(network_t **network, int argc, char **argv) {
    ...
    while ((c = util_getopt(argc, argv, "sn:t:m:")) != EOF) {
        switch (c) {
            case 'm':model = delay_get_model_from_name(util_optarg);
                ...
                break;
            case 's':print_max_only = TRUE;
                break;
            case 't':thresh = atof(util_optarg);
                break;
            case 'n':limit = atoi(util_optarg);
                break;
            default:goto print_fanout_usage;
        }
    }
    ...
    (void) fprintf(sisout,
                   "\t-t \t#.#\t: Threshold of slack defining critical path\n");
 ...
 */
void buf_set_default_options(buf_global_t *buf_param) {
...
int buffer_fill_options(buf_global_t *buf_param, int argc, char **argv) {
    int c;
    ...
    while ((c = util_getopt(argc, argv, "cdLTDf:v:l:")) != EOF) {
        switch (c) {
            case 'd':buf_param->do_decomp = TRUE;
                break;
            case 'c':buf_param->single_pass = TRUE;
                break;
            case 'L':buf_param->only_check_max_load = TRUE;
                break;
            case 'T':buf_param->trace = TRUE;
                break;
            case 'f':buf_param->mode = atoi(util_optarg);
                ...
                break;
            case 'v':buf_param->debug = atoi(util_optarg);
                break;
            case 'l':buf_param->limit = atoi(util_optarg);
                break;
            case 'D':buf_param->debug = TRUE;
                break;
            default:error_append("buffering: Unknown option\n");
                return 1;
        }
    }
    return 0;
...
void buffer_print_usage() {
    (void) fprintf(
            sisout, "Usage: buffer_opt [-l #] [-cdTDL] [-f #] [-v #] [node_list]\n");
    (void) fprintf(sisout, "\t-l \t#\t: Fanout limit (default = 2)\n");
    (void) fprintf(sisout, "\t-c \t\t: Buffer only one node in the pass\n");
    (void) fprintf(sisout,
                   "\t-d \t\t: Docompose complex gates as part of buffering\n");
    ...
    (void) fprintf(sisout,
                   "\t-f \t#\t: Fanout transformations to use (default = 7)\n");
    (void) fprintf(sisout, "\t-v \t#\t: Verbosity level (1-100)\n");
}

int com_buffer_opt(network_t **network, int argc, char **argv) {
    lsGen        gen;
    node_t       *node;
    array_t      *nodevec;
...
void end_speed() { sp_free_local_trans(&current_speedup_alg); }
