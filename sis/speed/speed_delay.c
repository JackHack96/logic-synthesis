 ...
 */
node_t *sp_minimum_slack(network_t *network, double *min_slack) {
    lsGen        gen;
    node_t       *best, *node;
...
int sp_po_req_times_set(network_t *network) {
    int          flag;
    lsGen        gen;
    node_t       *node;
    delay_time_t req;
 ...
 */
void speed_set_delay_data(speed_global_t *speed_param, int use_accl) {
    library_t     *lib;
...
void speed_set_library_accl(speed_global_t *speed_param, int value) { speed_param->library_accl = value; }

int speed_get_library_accl(speed_global_t *speed_param) { return speed_param->library_accl; }

/*
 * Return the delay model of the smallest agte implementing the given fucction
 */
static delay_pin_t *get_pin_delay_of_function(char *s, library_t *lib) {
    network_t   *network;
    lib_class_t *class;
...
int speed_update_arrival_time(node_t *node, speed_global_t *speed_param) {
    delay_time_t time;
...
static int speed_update_arrival_time_recur(node_t *node, speed_global_t *speed_param, delay_time_t *delay) {
    int          i;
    double       temp;
    node_t       *fanin;
    ...
    if (node_function(node) == NODE_PI) {
        ...
        if (speed_param->debug &&
            (fanin_time.rise < 0.0 || fanin_time.fall < 0.0)) {
        ...
        if ((t.fall < 0.0) || (t.rise < 0.0)) {
            ...
            foreach_fanin(node, i, fanin) {
                ...
                switch (node_input_phase(node, fanin)) {
                    case POS_UNATE:delay->rise = MAX(delay->rise, (fanin_time.rise + time.rise));
                        delay->fall            = MAX(delay->fall, (fanin_time.fall + time.fall));
                        break;
                    case NEG_UNATE:delay->rise = MAX(delay->rise, (fanin_time.fall + time.rise));
                        delay->fall            = MAX(delay->fall, (fanin_time.rise + time.fall));
                        break;
                    case BINATE:delay->rise = MAX(delay->rise, (fanin_time.rise + time.rise));
                        delay->rise         = MAX(delay->rise, (fanin_time.fall + time.rise));
                        delay->fall         = MAX(delay->fall, (fanin_time.rise + time.fall));
                        delay->fall         = MAX(delay->fall, (fanin_time.fall + time.fall));
                        break;
                    case PHASE_UNKNOWN:
...
void speed_delay_arrival_time(node_t *node, speed_global_t *speed_param, delay_time_t *time) {
    node_t       *fanin;
...
void speed_set_arrival_time(node_t *node, delay_time_t time) {
...
int speed_delay_trace(network_t *network, speed_global_t *speed_param) {
    lsGen  gen;
    node_t *po;
...
void speed_single_level_update(node_t *node, speed_global_t *speed_param, delay_time_t *delay) {
    int          i;
    node_t       *fanin;
...
void speed_reset_arrival_time(node_t *node) {
    delay_set_parameter(node, DELAY_ARRIVAL_RISE, DELAY_NOT_SET);
    delay_set_parameter(node, DELAY_ARRIVAL_FALL, DELAY_NOT_SET);
}

/* ARGSUSED */
void speed_update_fanout(network_t *network, array_t *nodevec, array_t *fanout_list, speed_global_t *speed_param) {
    lsGen        gen;
    delay_time_t t;
    int          first, last;
...
static delay_time_t speed_delay_node_pin(node_t *node, int i, speed_global_t *speed_param) {
    double        load;
    int           nin, nout;
    delay_model_t model = speed_param->model;
    delay_time_t  delay;
    ...
    if (model == DELAY_MODEL_MAPPED && speed_get_library_accl(speed_param) &&
        ...
        if (nin > 0) {
            ...
            load = compute_wire_load(node_network(node), nout) +
                   nout * speed_param->pin_cap;
