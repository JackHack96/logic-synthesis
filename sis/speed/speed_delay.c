 ...
 */
node_t *sp_minimum_slack(network, min_slack) network_t *network;
double *min_slack;
{
  lsGen gen;
  node_t *best, *node;
...
int sp_po_req_times_set(network) network_t *network;
{
  int flag;
  lsGen gen;
  node_t *node;
 ...
 */
speed_set_delay_data(speed_param, use_accl) speed_global_t *speed_param;
int use_accl;
{
  library_t *lib;
...
void speed_set_library_accl(speed_param, value) speed_global_t *speed_param;
int value;
{ speed_param->library_accl = value; }

int speed_get_library_accl(speed_param) speed_global_t *speed_param;
{ return speed_param->library_accl; }

/*
 * Return the delay model of the smallest agte implementing the given fucction
 */
static delay_pin_t *get_pin_delay_of_function(s, lib) char *s;
library_t *lib;
{
  network_t *network;
  lib_class_t *class;
...
int speed_update_arrival_time(node, speed_param) node_t *node;
...
static int speed_update_arrival_time_recur(node, speed_param,
                                           delay) node_t *node;
speed_global_t *speed_param;
delay_time_t *delay;
{
    ...
    *delay = fanin_time;
    ...
    if ((t.fall < 0.0) || (t.rise < 0.0)) {
      ...
      foreach_fanin(node, i, fanin) {
        ...
        case POS_UNATE:
          delay->rise = MAX(delay->rise, (fanin_time.rise + time.rise));
...
void speed_delay_arrival_time(node, speed_param, time) node_t *node;
speed_global_t *speed_param;
delay_time_t *time;
{
  node_t *fanin;
...
void speed_set_arrival_time(node, time) node_t *node;
delay_time_t time;
{
...
int speed_delay_trace(network, speed_param) network_t *network;
speed_global_t *speed_param;
{
  lsGen gen;
  node_t *po;
...
void speed_single_level_update(node, speed_param, delay) node_t *node;
speed_global_t *speed_param;
delay_time_t *delay;
{
  int i;
...
void speed_reset_arrival_time(node) node_t *node;
{
...
/* ARGSUSED */
void speed_update_fanout(network, nodevec, fanout_list,
                         speed_param) network_t *network;
...
static delay_time_t speed_delay_node_pin(node, i, speed_param) node_t *node;
int i;
speed_global_t *speed_param;
{
  double load;
