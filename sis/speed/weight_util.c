...
st_table *speed_compute_weight(network_t *network, speed_global_t *speed_param, array_t *array) {
        ...
        if (np->type == INTERNAL) {
            if (speed_critical(np, speed_param)) {
                if (speed_weight(np, speed_param, &t_weight, &a_weight)) {
                    ...
                    (void) st_insert(weight_table, (char *) np, (char *) temp_weight);
                    ...
                    (void) st_insert(table, (char *) np, (char *) (MAXWEIGHT * 100));
                }
            }
        }
    }
        ...
        for (i = 0; i < array_n(array); i++) {
            ...
            if (st_lookup(table, (char *) np, &dummy)) {
                ...
                if (st_lookup(weight_table, (char *) np, &dummy1)) {
                    ...
                    (void) fprintf(sisout,
                                   "%-10s Weight( NEEDS TO BE AVOIDED  )   = %-6d\n",
                                   node_name(np), weight);
                }
            }
        }
    ...
    st_foreach_item(weight_table, stgen, &dummy1, &dummy) { FREE(dummy); }
