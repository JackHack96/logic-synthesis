...
/* Exported interface  */
array_t *speed_decomp_interface(f, coeff, model) node_t *f;
double coeff;
delay_model_t model;
{
  array_t *a;
...
array_t *speed_decomp(innode, speed_param, delay_flag) node_t *innode;
speed_global_t *speed_param;
...
void speed_adjust_phase(network) network_t *network;
{
  node_t *node, *fo;
  lsGen gen1;
  int i;
