 ...
 */
int buffer_network(network_t *network, int limit, int trace, double thresh, int single_pass, int do_decomp) {
    int          changed;
 ...
 * cannot improve....
 */
int speed_buffer_network(network_t *network, buf_global_t *buf_param) {
    lsGen         gen;
    ...
    while (loop_again) { /* There is scope for improvement */
        ...
        foreach_node(network, gen, node) { BUFFER(node)->type = NONE; }
                    ...
                    if (speed_buffer_node(network, best, buf_param, required_times_set)) {
                        ...
                        if (buf_param->trace) {
                            (void) fprintf(
                                    ...
                                    (required_times_set ? BUF_MIN_D(prev) : BUF_MAX_D(prev)),
                                    (required_times_set ? BUF_MIN_D(delay) : BUF_MAX_D(delay)),
                                    orig_area, area, orig->name, last->name);
                        }
                        if ((required_times_set && (BUF_MIN_D(delay) < BUF_MIN_D(prev))) ||
                            (!required_times_set && (BUF_MAX_D(delay) > BUF_MAX_D(prev)))) {
                            (void) fprintf(
                                    sisout,
                                    "FAILED CHECK: Performance worsened when buffering %s\n",
                                    best->name);
                        }
                        prev      = delay;
                        orig      = last;
        ...
        if (speed_buffer_recover_area(network, buf_param)) {
            ...
            if (buf_param->trace) {
                (void) fprintf(
                        misout, "\t%s %.2f -> %.2f Area %.0f -> %.0f after Area Recovery\n",
                        (required_times_set ? "Slack" : "Delay"),
                        (required_times_set ? BUF_MIN_D(prev) : BUF_MAX_D(prev)),
                        (required_times_set ? BUF_MIN_D(delay) : BUF_MAX_D(delay)),
                        orig_area, area);
 ...
 */
static buf_alg_input_t *sp_get_fanout_set(node_t *node) {
    int             pin, pin1;
    lsGen           gen, gen1;
    node_t          *fo, *fo1;
    ...
    if (number_inv > 1) {
        ...
        alg_input->fanouts =
                ALLOC(sp_fanout_t, (alg_input->num_pos) + (alg_input->num_neg));
    }
...
static void sp_already_buffered(st_table *table, node_t *node) {
    lsGen  gen;
    node_t *fo;
...
int speed_buffer_array_of_nodes(network_t *network, buf_global_t *buf_param, array_t *nodevec) {
    lsGen  gen;
    node_t *node, *fo;
    int    changed;
 ...
 */
delay_time_t sp_buf_get_target(node_t *node, buf_global_t *buf_param, int constrained, double *max_loadp) {
    lsGen        gen;
    double       diff, load;
 ...
 */
int speed_buffer_node(network_t *network, node_t *node, buf_global_t *buf_param, int constrained) {
    ...
    if (buf_param->debug) {
        (void) fprintf(
                ...
                (constrained ? "CONSTRAINED" : "UNCONSTRAINED"), req_diff.rise,
                req_diff.fall);
    }
 ...
 */
int sp_max_load_violation(node_t *node) {
    int        i;
    double     load, limit;
    lib_gate_t *gate;
 ...
 */
/* ARGSUSED */
int sp_satisfy_max_load(network_t *network, buf_global_t *buf_param) {
    int     i, changed = 0;
    node_t  *node;
 ...
 * downsize the gate
 */
int speed_buffer_recover_area(network_t *network, buf_global_t *buf_param) {
    int     i, changed, false;
    node_t  *node, *temp;
    double  min_slack;
    array_t *nodevec;
...
static int buf_cur_version_is_better(buf_cost_t *cost1, buf_cost_t *cost2, double min_slack) {
    double slack1 = BUF_MIN_D(cost1->slack);
    double slack2 = BUF_MIN_D(cost2->slack);
    double diff   = slack1 - slack2;

    if (BUF_EQUAL_DOUBLE(slack1, min_slack))
        slack1 = min_slack;
    if (BUF_EQUAL_DOUBLE(slack2, min_slack))
        slack2 = min_slack;
    if (BUF_EQUAL_DOUBLE(diff, 0.0))
        diff   = 0.0;
...
static delay_time_t buf_get_reloaded_arrival_time(node_t *node, double load_diff) {
    int          i, ninputs;
    node_t       *fanin;
    lib_gate_t   *gate;
    double       load;
    ...
    if (node->type == PRIMARY_INPUT) {
        ...
        if (!delay_get_pi_arrival_time(node, &arrival)) {
            if ((t.rise = delay_get_default_parameter(
                    net, DELAY_DEFAULT_ARRIVAL_RISE)) != DELAY_NOT_SET &&
                (t.fall = delay_get_default_parameter(
                        net, DELAY_DEFAULT_ARRIVAL_FALL)) != DELAY_NOT_SET) {
                arrival = t;
            }
        }
        ...
        if (!delay_get_pi_drive(node, &drive)) {
            if ((t.rise = delay_get_default_parameter(
                    net, DELAY_DEFAULT_DRIVE_RISE)) != DELAY_NOT_SET &&
                (t.fall = delay_get_default_parameter(
                        net, DELAY_DEFAULT_DRIVE_FALL)) != DELAY_NOT_SET) {
                drive = t;
            }
        }
        ...
        arrival =
                delay_map_simulate(ninputs, arrival_times_p, gate->delay_info, load);
        FREE(arrival_times);
...
delay_time_t buf_get_new_arrival_time(node_t *node, lib_gate_t *new_gate, double min_slack, int *input_worsened) {
    int          i, ninputs;
    node_t       *fanin;
    ...
    *input_worsened = FALSE;
    ...
    arrival =
            delay_map_simulate(ninputs, arrival_times_p, new_gate->delay_info, load);
    FREE(arrival_times);
    FREE(arrival_times_p);
    return arrival;
}
 ...
 */
static int speed_do_area_recover(network_t *network, node_t *node, buf_global_t *buf_param, double min_slack) {
    lsGen        gen;
    ...
    while (lsNext(gen, (char **) &gate, LS_NH) == LS_OK) {
        arrival =
                buf_get_new_arrival_time(node, gate, min_slack, &input_restricted);
        if (input_restricted) {
