 ...
 */
int buffer_network(network, limit, trace, thresh, single_pass,
                   do_decomp) network_t *network;
int limit;
int trace;
double thresh;
int single_pass;
 ...
 * cannot improve....
 */
int speed_buffer_network(network, buf_param) network_t *network;
buf_global_t *buf_param;
{
  lsGen gen;
 ...
 */
static buf_alg_input_t *sp_get_fanout_set(node) node_t *node;
{
  int pin, pin1;
  lsGen gen, gen1;
  node_t *fo, *fo1;
...
static void sp_already_buffered(table, node) st_table *table;
node_t *node;
{
  lsGen gen;
  node_t *fo;
...
int speed_buffer_array_of_nodes(network, buf_param, nodevec) network_t *network;
buf_global_t *buf_param;
array_t *nodevec;
{
  lsGen gen;
 ...
 */
delay_time_t sp_buf_get_target(node, buf_param, constrained,
                               max_loadp) node_t *node;
buf_global_t *buf_param;
int constrained;
double *max_loadp;
{
 ...
 */
int speed_buffer_node(network, node, buf_param, constrained) network_t *network;
node_t *node;
buf_global_t *buf_param;
 ...
 */
int sp_max_load_violation(node) node_t *node;
{
  int i;
  double load, limit;
  lib_gate_t *gate;
 ...
 */
/* ARGSUSED */
int sp_satisfy_max_load(network, buf_param) network_t *network;
buf_global_t *buf_param;
{
  int i, changed = 0;
  node_t *node;
 ...
 * downsize the gate
 */
int speed_buffer_recover_area(network, buf_param) network_t *network;
buf_global_t *buf_param;
{
  int i, changed, false;
  node_t *node, *temp;
...
static int buf_cur_version_is_better(cost1, cost2, min_slack) buf_cost_t *cost1;
buf_cost_t *cost2;
double min_slack;
{
...
static delay_time_t buf_get_reloaded_arrival_time(node, load_diff) node_t *node;
double load_diff;
{
  int i, ninputs;
  node_t *fanin;
...
delay_time_t buf_get_new_arrival_time(node, new_gate, min_slack,
                                      input_worsened) node_t *node;
lib_gate_t *new_gate;
double min_slack;
int *input_worsened;
{
 ...
 */
static int speed_do_area_recover(network, node, buf_param,
                                 min_slack) network_t *network;
node_t *node;
buf_global_t *buf_param;
double min_slack;
{
