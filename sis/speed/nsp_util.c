 ...
 */
void sp_print_delay_data(FILE *fp, network_t *network) {
    lsGen        gen;
    node_t       *node;
    double       area = 0.0;
 ...
 */
void new_speed_adjust_po_arrival(sp_clp_t *rec, delay_model_t model, delay_time_t *arrival_p) {
    delay_time_t t;
    node_t       *node, *po;
 ...
 */
delay_time_t nsp_compute_delay_saving(node_t *po, node_t *node, delay_model_t model) {
...
void sp_print_network(network_t *network, char *comment) {
    int        i;
    lsGen      gen;
    node_t     *node;
    lib_gate_t *gate;
 ...
 */
static void sp_adjust_input_arrival(sp_clp_t *rec, network_t *network) {
    int          i;
    double       load;
    lsGen        gen, gen1;
    delay_pin_t  *pin_delay;
 ...
 */
sp_clp_t *
sp_create_collapse_record(node_t *node, sp_weight_t *wght, speed_global_t *speed_param, sp_xform_t *ti_model) {
...
void nsp_free_collapse_record(sp_clp_t *rec) {
    node_t       *node;
    delay_time_t *tp;
    st_generator *stgen;
    ...
    if (rec->delta != NIL(array_t))
        array_free(rec->delta);
 ...
 */
int new_speed_is_fanout_po(node_t *node) {
    lsGen  gen;
    node_t *fo;
 ...
 */
st_table *speed_store_required_times(network_t *network) {
    lsGen        gen;
    node_t       *po;
    st_table     *table;
    delay_time_t req;
 ...
 */
void speed_restore_required_times(st_table *table) {
    node_t       *po;
    char         *dummy;
    st_generator *stgen;
 ...
 */
void sp_patch_fanouts_of_node(node_t *node, node_t *new_fanin) {
    int    j = 0;
    lsGen  gen;
    node_t **fo_array, *fanout;

    fo_array = ALLOC(node_t *, node_num_fanout(node));
    foreach_fanout(node, gen, fanout) { fo_array[j++] = fanout; }
 ...
 */
array_t *sp_get_local_trans(int n_entries, char **entries) {
    int            i, j, num;
    sp_xform_t     *ti_model;
 ...
 */
void sp_free_local_trans(array_t **p_array_of_methods) {
    int        i;
    sp_xform_t *ti_model;

    if (*p_array_of_methods == NIL(array_t))
        return;
 ...
 */
void sp_print_local_trans(array_t *array_of_methods) {
    int        i;
    sp_xform_t *ti_model;
 ...
 */
int sp_num_active_local_trans(speed_global_t *speed_param) {
    int        i, n;
 ...
 */
int sp_num_local_trans_of_type(speed_global_t *speed_param, int type) {
    int        i, n;
    array_t    *array_of_methods = speed_param->local_trans;
    sp_xform_t *ti_model;
    ...
    if (array_of_methods == NIL(array_t))
        return 0;
    if (type != CLP && type != FAN && type != DUAL)
        return -1;

    for (i = 0, n = 0; i < array_n(array_of_methods); i++) {
        ti_model = array_fetch(sp_xform_t *, array_of_methods, i);
        if (ti_model->on_flag && ti_model->type == type)
            n++;
    }
    return n;
}
 ...
 */
sp_xform_t *sp_local_trans_from_index(speed_global_t *speed_param, int n) {
    array_t    *array_of_methods = speed_param->local_trans;
    sp_xform_t *ti_model;

    if (array_of_methods == NIL(array_t))
        return NIL(sp_xform_t);
    if ((n < 0) || (n >= array_n(array_of_methods)))
        return NIL(sp_xform_t);
...
void nsp_get_orig_edge(network_t *network, char *name, node_t **fo, int *index) {
    char *pos;
...
node_t *nsp_network_find_node(network_t *network, char *name) {
    char   *pos;
    node_t *node;
 ...
 */
void sp_delete_network(speed_global_t *speed_param, network_t *network, node_t *node, network_t *delete_network,
                       st_table *select_table, st_table *clp_table, st_table *equiv_table) {
    int          index;
    char         *name, *pos;
    lsGen        gen, gen1;
    ...
    foreach_primary_input(delete_network, gen, input) {
        ...
        if ((strcmp(input->name, orig_input->name) != 0 &&
            ...
            (void) st_lookup(clp_table, (char *) orig_input, (char **) &wght);
            new_ti_model =
                    sp_local_trans_from_index(speed_param, wght->best_technique);
            ...
            if (strcmp(input->name, orig_input->name) != 0 &&
                ...
                foreach_primary_output(new_net, gen1, po_fi) {
                    corresp_node = nsp_network_find_node(delete_network, po_fi->name);
            ...
            if ((new_po = network_find_node(network, input->name)) != NIL(node_t) &&
                ...
                delay_set_parameter(new_po, DELAY_REQUIRED_RISE, MIN(val, t.rise));
                val = delay_get_parameter(new_po, DELAY_REQUIRED_FALL);
                delay_set_parameter(new_po, DELAY_REQUIRED_FALL, MIN(val, t.rise));
                val = delay_get_parameter(new_po, DELAY_OUTPUT_LOAD);
                delay_set_parameter(new_po, DELAY_OUTPUT_LOAD,
                                    val + delay_load(orig_input));
            } else {
#ifdef SIS
                new_po = network_add_fake_primary_output(network, actual_node);
#else
                ...
                delay_set_parameter(new_po, DELAY_OUTPUT_LOAD, delay_load(orig_input));
            }
...
node_t *nsp_find_equiv_node(network_t *network, st_table *equiv_name_table, char *name) {
    node_t *orig;
    char   *n, *new_n;
 ...
 */
void sp_append_network(speed_global_t *speed_param, network_t *network, network_t *append_network, sp_xform_t *ti_model,
                       st_table *equiv_name_table, array_t *roots) {
        ...
        foreach_fanin(node, j, fanin) {
            if (fanin->type == PRIMARY_INPUT) {
                ...
                if (orig != NIL(node_t)) {
                    ...
                    *pos = '\0';
                    orig = nsp_find_equiv_node(network, equiv_name_table, fanin->name);
                    *pos = NSP_INPUT_SEPARATOR;
                    if (orig == NIL(node_t)) {
...
/* ARGSUSED */
static node_t *sp_find_substitute(network_t *network, node_t *node, delay_model_t model) {
    int       nin;
    lsGen     gen;
    ...
    match = NIL(node_t);
    if ((node->type != INTERNAL) || ((nin = node_num_fanin(node)) < 2))
        return match;
    ...
    foreach_fanout(fanin, gen, fo) {
        ...
        if ((fo == node) || (fo->type == PRIMARY_OUTPUT))
            continue;
        if (node_get_fanin_index(fo, fanin2) < 0)
            continue;
        if (node_num_fanin(fo) != nin)
            continue;
 ...
 * collapsing.
 */
void new_free_weight_info(st_table *clp_table) {
    node_t       *np;
    st_generator *stgen;
    sp_weight_t  *wght;
 ...
 */
double nsp_get_load_limit(node_t *node, int fanin_index, speed_global_t *speed_param) {
    lsGen        gen;
    delay_pin_t  *pin_delay;
    ...
    assert(delay_wire_required_time(node, fanin_index, speed_param->model,
                                    &wire_req));
 ...
 */
network_t *nsp_get_dual_network(node_t *node, speed_global_t *speed_param) {
    (void) fprintf(sisout, "Cannot yet get the dual network\n");
    if (speed_param->model != DELAY_MODEL_MAPPED || node->type == PRIMARY_INPUT ||
        node_num_fanin(node) <= 1) {
...
network_t *buf_get_fanout_network(node_t *node, speed_global_t *speed_param) {
    char         *name;
    int          i, pin;
    lsGen        gen;
    network_t    *new_net;
    ...
    if (inv_node != NIL(node_t)) {
      ...
      network_change_node_name(new_net, dup_inv_node,
                               util_strsav(inv_node->name));
    }
#endif
 ...
 */
int nsp_fanout_compare(char *p1, char *p2) {
...
delay_time_t nsp_min_req_time(network_t *network, delay_model_t model) {
    lsGen        gen;
    node_t       *pi;
 ...
 */
static void nsp_split_fanouts(network_t *network, speed_global_t *speed_param) {
    lsGen        gen;
    int          i, best_index;
    ...
    if (root == NIL(node_t))
        return; /* There is no node to duplicate */
    if (node_num_fanout(root) <= 2)
        return; /* Not enuff fanouts  */
    ...
    if (best_index == (array_n(po_array) - 1))
        best_index = -1;
 ...
 */
/* ARGSUSED */
network_t *sp_noalg_opt(network_t *network, sp_xform_t *ti_model, speed_global_t *speed_param) {
...
/* ARGSUSED */
network_t *sp_divisor_opt(network_t *network, sp_xform_t *ti_model, speed_global_t *speed_param) {
...
/* ARGSUSED */
network_t *sp_2c_kernel_opt(network_t *network, sp_xform_t *ti_model, speed_global_t *speed_param) {
    node_t    *clp_node, *temp;
    network_t *opt_network;
 ...
 */
static void comp_div_timeout_handler() { kernel_timeout_occured = TRUE; }

/* ARGSUSED */
network_t *sp_comp_div_opt(network_t *network, sp_xform_t *ti_model, speed_global_t *speed_param) {
    node_t    *clp_node, *temp;
    network_t *opt_network;
 ...
 * is evaluated correctly....
 */
static void comp_2c_timeout_handler() { twocube_timeout_occured = TRUE; }

/* ARGSUSED */
network_t *sp_comp_2c_opt(network_t *network, sp_xform_t *ti_model, speed_global_t *speed_param) {
    node_t    *clp_node, *temp;
    network_t *opt_network;
 ...
 */
/* ARGSUSED */
network_t *sp_and_or_opt(network_t *network, sp_xform_t *ti_model, speed_global_t *speed_param) {
    node_t    *clp_node;
    network_t *opt_network;
...
static void sp_map_if_required(network_t **network, speed_global_t *speed_param) {
    network_t *map_net;

    if (*network == NIL(network_t) || speed_param->model != DELAY_MODEL_MAPPED)
        return;
...
buf_global_t *nsp_init_buf_param(network_t *network, delay_model_t model) {
    ...
    if (nsp_buffer_param == NIL(buf_global_t)) {
        ...
        use_mapped =
                ((model == DELAY_MODEL_MAPPED || model == DELAY_MODEL_LIBRARY) ? 1 : 0);
 ...
 */
int nsp_downsize_non_crit_gates(network_t *network, delay_model_t model) {
    int success;
...
/* ARGSUSED */
network_t *sp_fanout_opt(network_t *network, sp_xform_t *ti_model, speed_global_t *speed_param) {
    network_t    *opt_network;
 ...
 */
/* ARGSUSED */
network_t *sp_duplicate_opt(network_t *network, sp_xform_t *ti_model, speed_global_t *speed_param) {
    network_t *opt_network;
 ...
 */
/* ARGSUSED */
network_t *sp_dual_opt(network_t *network, sp_xform_t *ti_model, speed_global_t *speed_param) {
    network_t *opt_network;
 ...
 */
/* ARGSUSED */
network_t *sp_cofactor_opt(network_t *network, sp_xform_t *ti_model, speed_global_t *speed_param) {
...
/* ARGSUSED */
network_t *sp_bypass_opt(network_t *network, sp_xform_t *ti_model, speed_global_t *speed_param) {
    network_t *new_net;
 ...
 */
/* ARGSUSED */
delay_time_t new_delay_arrival(node_t *node, speed_global_t *speed_param) {
 ...
 */
/* ARGSUSED */
delay_time_t new_delay_required(node_t *node, speed_global_t *speed_param) {
    delay_time_t req;
 ...
 */
/* ARGSUSED */
delay_time_t new_delay_slack(node_t *node, speed_global_t *speed_param) {
    delay_time_t req;
...
network_t *speed_network_dup(network_t *old) {
    network_t *new;
...
static node_t *speed_node_dup(register node_t *old) {
    register node_t *new;
