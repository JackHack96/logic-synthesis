...
bool speed_critical(node_t *np, speed_global_t *speed_param) {
    delay_time_t time;
...
int speed_is_fanout_po(node_t *node) {
    lsGen  gen;
    node_t *fo;
...
void speed_up_network(network_t *network, speed_global_t *speed_param) {
    int      i;
    st_table *table;
    lsGen    gen, gen1;
 ...
 */
void speed_node_interface(network_t *network, node_t *node, double coeff, delay_model_t model) {
...
void speed_up_node(network_t *network, node_t *np, speed_global_t *speed_param, int delay_flag) {
    array_t      *nodes;
    delay_time_t t;
...
int speed_init_decomp(network_t *network, speed_global_t *speed_param) {
    node_t  *node, *temp;
    array_t *nodevec;
    ...
    if (speed_param->new_mode && speed_param->model == DELAY_MODEL_MAPPED) {
        if (speed_param->interactive) {
            (void) fprintf(sisout,
                           "INFO: For mapped network, 2-input decomp BYPASSED\n");
        }
        return 0;
    }
...
static void speed_replace(network_t *network, node_t *np, array_t *nodes, speed_global_t *speed_param) {
    int          i;
    char         *dummy;
    delay_time_t t;
    st_table     *table;
    ...
    if (array_n(nodes) > 3) {
        ...
        if ((node_num_fanin(np) == 1) && (speed_is_fanout_po(np) == FALSE)) {
 ...
 */
static int speed_resub_alge_node(network_t *network, node_t *f, st_table *table) {
    lsGen   gen;
    node_t  *np, *fo;
    int     i, changed = 0;
    array_t *target, *tl1;
    ...
    for (i = 0; i < array_n(target); i++) {
        ...
        if (node_num_fanout(np) < 2 && node_substitute(f, np, 0)) {
            ...
            if (node_num_fanin(f) <= 1) {
                foreach_fanout(f, gen, fo) { (void) node_collapse(fo, f); }
 ...
 */
array_t *sp_generate_revised_order(array_t *array, speed_global_t *speed_param, st_table *clp_table) {
    array_t *new_array;
...
int sp_compare_cutset_nodes(char **ptr1, char **ptr2) {
    node_t      *f;
 ...
 *  its complement.
 */
static int speed_resub_alge_network(network_t *network) {
    lsGen  gen;
    node_t *np;
    bool   not_done;
