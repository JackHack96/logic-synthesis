...
bool speed_critical(np, speed_param) node_t *np;
...
int speed_is_fanout_po(node) node_t *node;
{
  lsGen gen;
  node_t *fo;
...
void speed_up_network(network, speed_param) network_t *network;
speed_global_t *speed_param;
{
  int i;
  st_table *table;
 ...
 */
void speed_node_interface(network, node, coeff, model) network_t *network;
node_t *node;
double coeff;
delay_model_t model;
{
...
void speed_up_node(network, np, speed_param, delay_flag) network_t *network;
node_t *np;
speed_global_t *speed_param;
...
int speed_init_decomp(network, speed_param) network_t *network;
speed_global_t *speed_param;
{
  node_t *node, *temp;
  array_t *nodevec;
...
static void speed_replace(network, np, nodes, speed_param) network_t *network;
node_t *np;
array_t *nodes;
speed_global_t *speed_param;
{
 ...
 */
static int speed_resub_alge_node(network, f, table) network_t *network;
node_t *f;
st_table *table;
{
  lsGen gen;
 ...
 */
array_t *sp_generate_revised_order(array, speed_param,
                                   clp_table) array_t *array;
speed_global_t *speed_param;
st_table *clp_table;
{
  array_t *new_array;
...
int sp_compare_cutset_nodes(ptr1, ptr2) char **ptr1;
char **ptr2;
{
  node_t *f;
 ...
 *  its complement.
 */
static int speed_resub_alge_network(network) network_t *network;
{
  lsGen gen;
  node_t *np;
  bool not_done;
