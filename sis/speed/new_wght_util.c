        ...
        struct create_flag_struct *flags;
{
    ...
    wght->improvement = ALLOC(
    double, sp_num_models);
    wght->area_cost = ALLOC(
    double, sp_num_models);
            ...
            foreach_fanin(node, j, fanin)
            {
                if (st_lookup(wght_table, (char *) fanin, (char **) &ip_wght)) {
                    ...
                    if (sl < speed_param->crit_slack - NSP_EPSILON) {
                        ...
                        all_fanins_pi = FALSE;
                        array_insert(
                        double, ip_slacks, crit_ip_count, sl);
        ...
        array_t *nodevec;
{
    ...
    for (i = array_n(nodevec); i-- > 0;) {
        ...
        if (temp->type != PRIMARY_OUTPUT) {
            array_insert(
            char *, names_array, index, util_strsav(temp->name));
            index++;
        }
    }
        ...
        int *cost;
{
    ...
    *cost = 0;
    ...
    FREE(phase);
    FREE(parent);
    st_foreach(id_table, free_starray, NIL(
    char));
    st_foreach(order, free_stbddt, NIL(
    char));
        ...
        double target_impr;   /* The improvement that we promised we could deliver */
{
    ...
    do {
        ...
        foreach_primary_output(network_in, gen, node)
        {
            ...
            (void) st_insert(visited, (char *) np, NIL(
            char));
            /*
