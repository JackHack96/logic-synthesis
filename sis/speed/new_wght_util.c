 ...
 * considered....
 */
void new_speed_compute_weight(network_t *network, speed_global_t *speed_param, st_table *wght_table, double *epsilonp) {
    lsGen                     gen;
    int                       pass = 0;
    ...
    *epsilonp = 0;
    ...
    create_flag.create_dual = (sp_num_local_trans_of_type(speed_param, DUAL) > 0);
    create_flag.create_clp  = (sp_num_local_trans_of_type(speed_param, CLP) > 0);
    ...
    do {
        ...
        foreach_node(network, gen, np) {
            if (np->type != PRIMARY_OUTPUT) {
                if (speed_critical(np, speed_param) &&
                    ...
                    nsp_create_relevant_data(network, np, speed_param, wght,
                                             &create_flag);
        ...
        foreach_primary_output(network, gen, node) {
            ...
            if (speed_critical(node, speed_param)) {
                ...
                new_min_slack.rise = MIN(new_min_slack.rise, (slack.rise + impr));
                new_min_slack.fall = MIN(new_min_slack.fall, (slack.fall + impr));
    ...
    if (speed_param->debug > 2) {
        st_foreach_item(wght_table, stgen, (char **) &np, (char **) &wght) {
            (void) fprintf(sisout, "At %s %d D=%.2f A=%.2f (da=%.2f oa=%.2f)\n",
                           np->name, wght->best_technique, SP_IMPROVEMENT(wght),
                           SP_COST(wght), wght->dup_area, wght->orig_area);
        }
    }
    array_free(new_crit_po_array);
    st_free_table(new_crit_table);
 ...
 * traces
 */
static void nsp_create_relevant_data(network_t *network, node_t *np, speed_global_t *speed_param, sp_weight_t *wght,
    ...
    if (flags->create_clp) {
        wght->clp_netw =
                sp_get_network_to_collapse(np, speed_param, &(wght->dup_area));
    }
    ...
    if (flags->create_fanout) {
        wght->fanout_netw = buf_get_fanout_network(np, speed_param);
        wght->cfi         = -1;
        ...
        delay_wire_required_time(np, wght->cfi, speed_param->model, &wght->cfi_req);
    }
 ...
 */
static void nsp_eval_best_transform(node_t *np, sp_weight_t *wght, speed_global_t *speed_param) {
    int        i, j;
    ...
    for (i   = 0; i < sp_num_models; i++) {
        ...
        if (local_trans->on_flag) {
            ...
            if (opt_netw == NIL(network_t)) {
                ...
                sp_compute_improvement(np, opt_netw, wght, speed_param, i, local_trans);
    ...
    if (wght->best_technique >= 0 && SP_IMPROVEMENT(wght) > NSP_EPSILON) {
        local_trans = sp_local_trans_from_index(speed_param, wght->best_technique);
...
static void
sp_compute_improvement(node_t *node, network_t *opt_netw, sp_weight_t *wght, speed_global_t *speed_param, int num,
                       sp_xform_t *local_trans) {
    ...
    auto_load =
            delay_get_default_parameter(node_network(node), DELAY_WIRE_LOAD_SLOPE);
    auto_load = (auto_load == DELAY_NOT_SET ? 0.0 : auto_load);
    ...
    if (local_trans->type == FAN) {
        ...
        if (node->type == PRIMARY_INPUT) {
            ...
            load_diff =
                    delay_compute_fo_load(fanin, speed_param->model) - wght->cfi_load;
        } else {
    ...
    if (local_trans->type == FAN) {
        ...
        wght->area_cost[num] =
                (sp_get_netw_area(opt_netw) + wght->dup_area) - wght->orig_area;
    }
}
 ...
 */
static void nsp_compute_achievable_saving(network_t *network, speed_global_t *speed_param, st_table *wght_table) {
    int          i, j;
    array_t      *nodevec;
    node_t       *node, *fanin;
    ...
    for (i  = 0; i < array_n(nodevec); i++) {
        ...
        if (st_lookup(wght_table, (char *) node, (char **) &wght)) {
            ...
            all_fanins_pi = TRUE;
            if (speed_param->debug > 1) {
                (void) fprintf(sisout, "EPS %s (%s) --", node_name(node),
                               node_long_name(node));
            }
            ...
            for (j     = 0; j < crit_ip_count; j++) {
                ...
                inp_eps = MIN(inp_eps, (ip_wght->epsilon + sl - min_ip_slack));
                if (speed_param->debug > 1) {
                    (void) fprintf(sisout, " (%5.2f+%5.2f)%s", ip_wght->epsilon,
                                   sl - min_ip_slack, fanin->name);
                }
            }
 ...
 * inverters
 */
static int nsp_trivial_mapped_network(network_t *network) {
    lsGen  gen;
    node_t *node;
    int    is_trivial = TRUE;
 ...
 */
static void nsp_print_bdd(bdd_t *f, array_t *nodevec, int num_lit) {
    char      *name;
    int       i, index;
    node_t    *temp;
    network_t *pr_net;
...
int nsp_first_slack_diff(network_t *network, double *slack_diff) {
    lsGen        gen;
    node_t       *node;
 ...
 */
int nsp_critical_edge(node_t *node, int fanin_index, delay_model_t model, double crit_slack) {
    node_t       *fanin;
    delay_time_t req, arr, slack;
...
array_t *nsp_best_selection(bdd_t *f, st_table *node_and_id, int *wtt, int wte, int *cost) {
    ...
    *cost = 0;
    ...
    for (i = num_nodes - 1; i >= 0; i--) {
        ...
        /* Skip if v is a leaf. */
        if (bdd_is_tautology(v, 0) || bdd_is_tautology(v, 1))
            continue;
 ...
 */
static void nsp_dfs_bdd(bdd_t *f, st_table *id_table, st_table *order, int start_num, int *total_num_ptr) {
    int   total;
    bdd_t *v0, *v1;
...
static enum st_retval free_starray(char *key, char *value, char *arg) {
    FREE(value);
    return ST_DELETE;
}

static enum st_retval free_stbddt(char *key, char *value, char *arg) {
    bdd_t *b;
...
static void set_post_visit(st_table *id_table, bdd_t *f, int n) {
...
static int get_post_visit(st_table *id_table, bdd_t *f) {
    int      complement, index, *A;
    bdd_node *F;
    char     *value;
...
static int is_f_visited(st_table *id_table, bdd_t *f) {
    int      complement, index, *A;
    bdd_node *F;
    char     *value;
 ...
 */
array_t *
new_speed_select_xform(network_t *network_in, speed_global_t *speed_param, st_table *clp_table, double target_impr) {
    lsGen         gen;
    ...
    do {
        ...
        foreach_primary_output(network_in, gen, node) {
            ...
            for (i = 0; i < array_n(topo_order); i++) {
                ...
                if (speed_param->debug > 2) {
                    (void) fprintf(sisout, "SAVING %s = %.2f: ", temp->name,
                                   vdata->saving);
                }
        ...
        if (bdd_is_tautology(f, 0)) {
            ...
            sol_array = nsp_best_selection(f, node_and_id, cost_array, 0, &area);
        ...
        while (TRUE) {
            if (!nsp_eval_selection(network_in, speed_param, clp_table, thresh,
                                    sol_array, area, &best_sol, &found_good_set,
                                    &neg_impr) ||
 ...
 */
static int nsp_eval_selection(network_t *network, speed_global_t *speed_param, st_table *clp_table, double thresh,
                              array_t *orig_sol_array, int area, array_t **best_array, int *found_good_set,
                              int *neg_impr) {
    ...
    for (i = 0; i < array_n(sol_array); i++) {
        ...
        sp_delete_network(speed_param, dup_network, temp,
                          NSP_NETWORK(wght, local_trans), select_table, clp_table,
                          NIL(st_table));
    }
    ...
    for (i           = 0; i < array_n(sol_array); i++) {
        ...
        sp_append_network(speed_param, dup_network, wght->best_netw, local_trans,
