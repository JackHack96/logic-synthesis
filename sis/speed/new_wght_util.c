 ...
 * considered....
 */
void new_speed_compute_weight(network, speed_param, wght_table,
                              epsilonp) network_t *network;
 ...
 * traces
 */
static void nsp_create_relevant_data(network, np, speed_param, wght,
                                     flags) network_t *network;
node_t *np;
speed_global_t *speed_param;
sp_weight_t *wght;
 ...
 */
static void nsp_eval_best_transform(np, wght, speed_param) node_t *np;
sp_weight_t *wght;
speed_global_t *speed_param;
{
  int i, j;
  ...
  if (wght->best_technique >= 0 && SP_IMPROVEMENT(wght) > NSP_EPSILON) {
    ...
    if (local_trans->type != CLP)
      sp_network_free(wght->clp_netw);
    if (local_trans->type != FAN)
      sp_network_free(wght->fanout_netw);
    if (local_trans->type != DUAL)
...
static void sp_compute_improvement(
    node, opt_netw, wght, speed_param, num,
    local_trans) node_t *node; /* The node at which the transform is applied */
 ...
 */
static void nsp_compute_achievable_saving(network, speed_param,
                                          wght_table) network_t *network;
speed_global_t *speed_param;
st_table *wght_table;
{
  int i, j;
 ...
 * inverters
 */
static int nsp_trivial_mapped_network(network) network_t *network;
{
  lsGen gen;
  node_t *node;
  int is_trivial = TRUE;
 ...
 */
static void nsp_print_bdd(f, nodevec, num_lit) bdd_t *f;
array_t *nodevec;
{
  char *name;
  int i, index;
...
int nsp_first_slack_diff(network, slack_diff) network_t *network;
double *slack_diff;
{
  lsGen gen;
  node_t *node;
 ...
 */
int nsp_critical_edge(node, fanin_index, model, crit_slack) node_t *node;
int fanin_index;
delay_model_t model;
double crit_slack;
{
...
array_t *nsp_best_selection(f, node_and_id, wtt, wte, cost) bdd_t *f;
 ...
 */
static void nsp_dfs_bdd(f, id_table, order, start_num, total_num_ptr) bdd_t *f;
st_table *id_table, *order;
...
static enum st_retval free_starray(key, value, arg) char *key, *value, *arg;
{
  FREE(value);
  return ST_DELETE;
}

static enum st_retval free_stbddt(key, value, arg) char *key, *value, *arg;
{
  bdd_t *b;
...
static void set_post_visit(id_table, f, n) st_table *id_table;
bdd_t *f;
int n;
{
...
static int get_post_visit(id_table, f) st_table *id_table;
bdd_t *f;
{
  int complement, index, *A;
  bdd_node *F;
...
static int is_f_visited(id_table, f) st_table *id_table;
bdd_t *f;
{
  int complement, index, *A;
  bdd_node *F;
 ...
 */
array_t *new_speed_select_xform(network_in, speed_param, clp_table,
                                target_impr) network_t *network_in;
speed_global_t *speed_param;
...
double target_impr;  /* The improvement that we promised we could deliver */
{
  ...
  for (i = num_lit; i-- > 0; cost_array[i] = POS_LARGE)
    ; /* Initialization */
 ...
 */
static int nsp_eval_selection(network, speed_param, clp_table, thresh,
                              orig_sol_array, area, best_array, found_good_set,
                              neg_impr) network_t *network;
speed_global_t *speed_param;
st_table *clp_table;
