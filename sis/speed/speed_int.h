 ...
 *
 * $Source: /users/pchong/CVS/sis/sis/speed/speed_int.h,v $
 * $Author: pchong $
 * $Revision: 1.1.1.1 $
...
(void) fprintf(sisout,"\t%s  %5.2f -> %5.2f Area %.1f -> %.1f  %s -> %s\n", \
       ((global)->req_times_set ? "Slack" : "Delay"), cur, best, cur_area, \
       best_area, cur_name, best_name); 
...
#define SP_METHOD(flag) \
     (flag == ALONG_CRIT_PATH ? "CRITICAL" :\
      (flag == COMPROMISE ? "COMPROMISE" : \
       (flag == ONLY_TREE ? "TREE" : \
	(flag == TRANSITIVE_FANIN ? "TRANSITIVE" : "UNKNOWN"))))
...
#define NSP_NETWORK(wght,xform) \
      (xform->type == FAN ? (wght->fanout_netw) : \
       (xform->type == DUAL ? (wght->dual_netw): (wght->clp_netw)))

#define sp_network_free(network) \
      {if ((network) != NIL(network_t)) network_free((network)); \
       (network) = NIL(network_t);}

#define SP_IMPROVEMENT(wght) \
      ((wght)->best_technique == -1 ? \
       (NEG_LARGE) : ((wght)->improvement[(wght)->best_technique]))
#define SP_COST(wght)   \
      ((wght)->best_technique == -1 ? \
       (POS_LARGE) : ((wght)->area_cost[(wght)->best_technique]))
