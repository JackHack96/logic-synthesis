...
#define COMPARE(key, nodekey, compare)                                         \
  ((compare == avl_numcmp) ? (int)key - (int)nodekey : (*compare)(key, nodekey))
...
avl_tree *avl_init_table(compar) int (*compar)();
{
  avl_tree *tree;
...
avl_lookup(tree, key, value_p) avl_tree *tree;
register char *key;
char **value_p;
{
...
avl_first(tree, key_p, value_p) avl_tree *tree;
char **key_p;
char **value_p;
{
  register avl_node *node;
...
avl_last(tree, key_p, value_p) avl_tree *tree;
char **key_p;
char **value_p;
{
  register avl_node *node;
...
avl_insert(tree, key, value) avl_tree *tree;
char *key;
char *value;
{
...
avl_find_or_add(tree, key, slot_p) avl_tree *tree;
char *key;
char ***slot_p;
{
  register avl_node **node_p, *node;
...
avl_delete(tree, key_p, value_p) avl_tree *tree;
char **key_p;
char **value_p;
{
...
static void avl_record_gen_forward(node, gen) avl_node *node;
avl_generator *gen;
{
...
static void avl_record_gen_backward(node, gen) avl_node *node;
avl_generator *gen;
{
  if (node != NIL(avl_node)) {
...
avl_generator *avl_init_gen(tree, dir) avl_tree *tree;
int dir;
{
  avl_generator *gen;
...
avl_gen(gen, key_p, value_p) avl_generator *gen;
char **key_p;
char **value_p;
{
  avl_node *node;
...
void avl_free_gen(gen) avl_generator *gen;
{
  FREE(gen->nodelist);
  FREE(gen);
}

static avl_node *find_rightmost(node_p) register avl_node **node_p;
{
  register avl_node *node;
  register int stack_n = 0;
  avl_node **stack_nodep[STACK_SIZE];
...
static void do_rebalance(stack_nodep, stack_n) register avl_node ***stack_nodep;
...
static rotate_left(node_p) register avl_node **node_p;
{
...
static rotate_right(node_p) avl_node **node_p;
{
...
static void avl_walk_forward(node, func) avl_node *node;
void (*func)();
{
  if (node != NIL(avl_node)) {
...
static void avl_walk_backward(node, func) avl_node *node;
void (*func)();
{
  if (node != NIL(avl_node)) {
...
void avl_foreach(tree, func, direction) avl_tree *tree;
void (*func)();
int direction;
{
...
static void free_entry(node, key_free, value_free) avl_node *node;
void (*key_free)();
void (*value_free)();
{
  if (node != NIL(avl_node)) {
...
void avl_free_table(tree, key_free, value_free) avl_tree *tree;
void (*key_free)();
void (*value_free)();
{
...
int avl_count(tree) avl_tree *tree;
{ return tree->num_entries; }

static avl_node *new_node(key, value) char *key;
char *value;
{
...
int avl_numcmp(x, y) char *x, *y;
{ return (int)x - (int)y; }

int avl_check_tree(tree) avl_tree *tree;
{
  int error = 0;
...
static int do_check_tree(node, compar, error) avl_node *node;
int (*compar)();
int *error;
{
  ...
  if (node->left != NIL(avl_node) &&
    ...
    (void)printf("Bad ordering between 0x%08x and 0x%08x", node, node->left);
    ++*error;
  }

  if (node->right != NIL(avl_node) &&
      (*compar)(node->key, node->right->key) > 0) {
    (void)printf("Bad ordering between 0x%08x and 0x%08x", node, node->right);
    ++*error;
  }
