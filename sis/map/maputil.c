...
void map_dup(node_t *old, node_t *new) {
    int i;
...
void map_invalid(node_t *node) {
    if (MAP(node) != 0) {
        MAP(node)->gate = 0;
    }
}

void map_setup_network(network_t *network) {
    lsGen  gen;
    node_t *node;
...
void map_prep(network_t *network, library_t *library) {
...
void map_network_dup(network_t *network) {
    int    i;
    lsGen  gen;
    node_t *node;
...
node_t *map_po_get_fanin(node_t *node) {
...
int network_gate_type(network_t *network) {
    int *type;
...
node_t *network_latch_input(network_t *network) {
    lsGen  gen;
