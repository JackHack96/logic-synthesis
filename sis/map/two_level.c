...
**				Factorized the common code between
*compute_balanced_required_time() *				and
*compute_best_fit_required_time().
**
** 01 10-June-1991 klk		Fixed a bug in the functions
*compute_balanced_required_time() and *
*compute_best_fit_required_time().
**/
...
/* ARGSUSED */
void two_level_init(network_t *network, fanout_alg_t *alg) { alg->optimize = two_level_optimize; }

/* ARGSUSED */
void balanced_init(network_t *network, fanout_alg_t *alg) { alg->optimize = balanced_optimize; }

/* INTERNAL INTERFACE */

static int two_level_optimize(opt_array_t *fanout_info, array_t *tree, fanout_cost_t *cost) {
    generic_fanout_optimizer(fanout_info, tree, cost, optimize_two_level,
                             extract_merge_info, build_selected_tree);
    return 1;
}

static int balanced_optimize(opt_array_t *fanout_info, array_t *tree, fanout_cost_t *cost) {
    generic_fanout_optimizer(fanout_info, tree, cost, optimize_balanced,
                             extract_merge_info, build_selected_tree);
    return 1;
}
...
static multidim_t *optimize_two_level(opt_array_t *fanout_info) {
    two_level_best_fit_fn = compute_best_fit_required_time;
    return optimize_one_source_all_sets_of_sinks(fanout_info,
                                                 0 /* top level only */);
}

static multidim_t *optimize_balanced(opt_array_t *fanout_info) {
...
multidim_t *two_level_optimize_for_lt_trees(opt_array_t *fanout_info) {
    two_level_best_fit_fn = compute_best_fit_required_time;
    return optimize_one_source_all_sets_of_sinks(fanout_info, 1 /* all sets */);
}

static multidim_t *optimize_one_source_all_sets_of_sinks(opt_array_t *fanout_info, int all_sets_of_sinks) {
    multidim_t  *table;
    int         indices[4];
    int         max_sinks;
        ...
        foreach_polarity(source_polarity) {
            ...
            foreach_polarity(sink_polarity) {
                if (fanout_info[sink_polarity].n_elts == 0)
                    continue;
                for (sink_index = 0; sink_index < fanout_info[sink_polarity].n_elts;
                     sink_index++) {
                    if (sink_index > 0 && all_sets_of_sinks == 0)
                        break;
                    entry = INDEX4P(two_level_t, table, source_index, source_polarity,
                                    sink_index, sink_polarity);
                    best_one_source(&fanout_info[sink_polarity], entry, source_index,
                                    sink_index, (source_polarity != sink_polarity));
                }
            }
        }
    }
...
static void
best_one_source(opt_array_t *fanout_info, two_level_t *entry, int source_index, int sink_index, int diff_polarity) {
    int          g, k;
    int          from, to;
    int          n_sinks;
    double       load;
    ...
    load    = fanout_info->total_load - fanout_info->cumul_load[sink_index] +
              map_compute_wire_load(n_sinks);

    for (g = from; g < to; g++) {
        k        = compute_best_number_of_inverters(source_index, g, load, n_sinks);
        required = (two_level_best_fit_fn)(fanout_info, source_index, sink_index, g,
                                           &k, NIL(int));
        if (GETMIN(entry->required) < GETMIN(required)) {
            ...
            entry->area =
                    k * fanout_delay_get_area(g) + fanout_delay_get_area(source_index);
        }
    }
}
...
/* greedy algorithm: */
/* assign the sink to the intermediate node that would provide the largest
 * overall */
/* required time after the sink is inserted */

static delay_time_t
compute_best_fit_required_time(opt_array_t *fanout_info, int source_index, int sink_index, int buffer_index,
                               int *n_buffers, int *sink_to_buffer_map) {
    ...
    int          num_assigned_buffers; /* Number of buffers that are actually assigned
                               sinks */
    ...
    for (sink       = sink_index; sink < n_elts; sink++) {
        ...
        for (buffer = 0; buffer < *n_buffers; buffer++) {
            ...
            local_required = fanout_delay_backward_load_dependent(
                    local_required, buffer_index, local_load);
            if (GETMIN(best_required) < GETMIN(local_required)) {
                best_required = local_required;
                best_fit      = buffer;
            }
        }
        SETMIN(best_fit_info[best_fit].required, best_fit_info[best_fit].required,
               link->required);
        best_fit_info[best_fit].load += link->load;
        best_fit_info[best_fit].n_fanouts++;
        if (sink_to_buffer_map != NIL(int))
            sink_to_buffer_map[sink] = best_fit;
        SETMIN(required_so_far, required_so_far, best_required);
    }
    return from_buffer_assignment_to_required_time(best_fit_info, n_buffers,
                                                   required_so_far, source_index,
                                                   buffer_index, n_sinks);
}

/* balanced algorithm: ignores the required times to perform the allocation */

static delay_time_t
compute_balanced_required_time(opt_array_t *fanout_info, int source_index, int sink_index, int buffer_index,
                               int *n_buffers, int *sink_to_buffer_map) {
    best_fit_t   *best_fit_info;
    delay_time_t required_so_far;
    delay_time_t local_required;
...
static delay_time_t
from_buffer_assignment_to_required_time(best_fit_t *best_fit_info, int *n_buffers, delay_time_t required_so_far,
                                        int source_index, int buffer_index, int n_sinks) {
    int    buffer; /* index on intermediate buffers, from 0 to n_buffers */
    int    num_assigned_buffers; /* number of buffers that are actually assigned
                               sinks */
    int    num_assigned_sinks;   /* number of sinks that are actually assigned to a
                               buffer */
    double load;              /* load at the main buffer */

    /*
     * Make sure all the buffers are assigned some sink.
     * If the number of assigned buffers is not the same as the value of
     * n_buffers, change n_buffers to number of actually assigned buffers.
     */
    ...
    if (num_assigned_buffers != *n_buffers) {
        (void) fprintf(sisout, "balanced: Some buffers in the two-level tree are "
                               "not assigned sinks\n");
        *n_buffers = num_assigned_buffers;
    }

    required_so_far =
            fanout_delay_backward_intrinsic(required_so_far, buffer_index);
    load = fanout_delay_get_buffer_load(buffer_index) * *n_buffers +
           map_compute_wire_load(*n_buffers);
    return fanout_delay_backward_load_dependent(required_so_far, source_index,
                                                load);
}
...
static void extract_merge_info(opt_array_t *fanout_info, multidim_t *table, multidim_t *merge_table) {
    ...
    foreach_gate(n_gates, source_index) {
        foreach_polarity(sink_polarity) {
            foreach_polarity(source_polarity) {
                from = INDEX4P(two_level_t, table, source_index, source_polarity, 0,
                               sink_polarity);
                ...
                if (fanout_info[sink_polarity].n_elts == 0) {
                    ...
                    to->area = from->area;
                           ...
                           source_polarity) {
                    to->load =
                            fanout_delay_get_buffer_load(from->gate_index) * from->n_gates;
...
static void build_selected_tree(opt_array_t *fanout_info, array_t *tree, selected_source_t *source, multidim_t *table) {
    ...
    q               = POLAR_INV(p);
    entry_p =
            INDEX4P(two_level_t, table, source->main_source, source_polarity, 0, p);
    ...
    if (fanout_info[q].n_elts > 0) {
        if (is_source(n_gates, source->buffer)) {
            ...
            entry_q   = INDEX4P(two_level_t, table, source->main_source,
                                source_polarity, 0, q);
            n_fanouts = entry_p->n_gates + entry_q->n_gates;
            fanout_tree_insert_gate(tree, source->main_source, n_fanouts);
            insert_two_level_tree(tree, &fanout_info[p], 0, source->main_source,
                                  entry_p);
            insert_two_level_tree(tree, &fanout_info[q], 0, source->main_source,
                                  entry_q);
        } else {
            ...
            buffer_polarity = (is_inverter(n_gates, source->buffer))
                              ...
                              : source_polarity;
            entry_q =
                    INDEX4P(two_level_t, table, source->buffer, buffer_polarity, 0, q);
...
void
insert_two_level_tree(array_t *tree, opt_array_t *fanout_info, int sink_index, int source_index, two_level_t *entry) {
    int      i, j;
    ...
    (void) (*two_level_best_fit_fn)(fanout_info, source_index, sink_index,
                                    entry->gate_index, &(entry->n_gates),
                                    sink_to_buffer_map);
...
static void insert_one_level_tree(array_t *tree, opt_array_t *fanout_info, st_table *fanout_table) {
    char         *key, *value;
    st_generator *gen;
    st_foreach_item(fanout_table, gen, &key, &value) {
        int         sink_index = (int) key;
        gate_link_t *link      =
                            array_fetch(gate_link_t *, fanout_info->required, sink_index);
