...
/* ARGSUSED */
void two_level_init(network, alg) network_t *network;
fanout_alg_t *alg;
{ alg->optimize = two_level_optimize; }

/* ARGSUSED */
void balanced_init(network, alg) network_t *network;
fanout_alg_t *alg;
...
static multidim_t *optimize_two_level(fanout_info) opt_array_t *fanout_info;
{
...
static multidim_t *optimize_balanced(fanout_info) opt_array_t *fanout_info;
{
...
multidim_t *
    two_level_optimize_for_lt_trees(fanout_info) opt_array_t *fanout_info;
{
  two_level_best_fit_fn = compute_best_fit_required_time;
...
static multidim_t *optimize_one_source_all_sets_of_sinks(
    fanout_info, all_sets_of_sinks) opt_array_t *fanout_info;
...
static void best_one_source(fanout_info, entry, source_index, sink_index,
                            diff_polarity) opt_array_t *fanout_info;
two_level_t *entry;
int source_index;
int sink_index;
...
static delay_time_t
    compute_best_fit_required_time(fanout_info, source_index, sink_index,
                                   buffer_index, n_buffers,
                                   sink_to_buffer_map) opt_array_t *fanout_info;
int source_index;
int sink_index;
int buffer_index;
int *n_buffers;
int *sink_to_buffer_map; /* records mapping sink->buffer; if NIL(int), does
                            nothing */
{
...
static delay_time_t
    compute_balanced_required_time(fanout_info, source_index, sink_index,
                                   buffer_index, n_buffers,
                                   sink_to_buffer_map) opt_array_t *fanout_info;
int source_index;
int sink_index;
...
static delay_time_t from_buffer_assignment_to_required_time(
    best_fit_info, n_buffers, required_so_far, source_index, buffer_index,
    n_sinks) best_fit_t *best_fit_info;
int *n_buffers;
delay_time_t required_so_far;
int source_index;
int buffer_index;
...
static void extract_merge_info(fanout_info, table,
                               merge_table) opt_array_t *fanout_info;
multidim_t *table;
...
void insert_two_level_tree(tree, fanout_info, sink_index, source_index,
                           entry) array_t *tree;
...
static void insert_one_level_tree(tree, fanout_info,
                                  fanout_table) array_t *tree;
opt_array_t *fanout_info;
