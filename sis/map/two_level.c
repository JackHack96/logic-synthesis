        ...
        foreach_polarity(source_polarity) {
            ...
            foreach_polarity(sink_polarity) {
                ...
                for (sink_index = 0; sink_index < fanout_info[sink_polarity].n_elts; sink_index++) {
                    ...
                    best_one_source(&fanout_info[sink_polarity], entry, source_index, sink_index,
        ...
        int diff_polarity;             /* 1 iff source and sinks have different polarity */
{
    ...
    for (g = from; g < to; g++) {
        ...
        required = (two_level_best_fit_fn)(fanout_info, source_index, sink_index, g, &k, NIL(
        int));
...
static delay_time_t compute_best_fit_required_time(fanout_info, source_index, sink_index, buffer_index, n_buffers,
                                                   sink_to_buffer_map)
        opt_array_t *fanout_info;
        int source_index;
        int sink_index;
        ...
        int *sink_to_buffer_map;          /* records mapping sink->buffer; if NIL(int), does nothing */
{
    ...
    return from_buffer_assignment_to_required_time(best_fit_info, n_buffers, required_so_far, source_index,
                                                   buffer_index, n_sinks);
}

/* balanced algorithm: ignores the required times to perform the allocation */

static delay_time_t compute_balanced_required_time(fanout_info, source_index, sink_index, buffer_index, n_buffers,
                                                   sink_to_buffer_map)
        opt_array_t *fanout_info;
        int source_index;
        int sink_index;
...
static delay_time_t from_buffer_assignment_to_required_time(best_fit_info, n_buffers, required_so_far, source_index,
                                                            buffer_index, n_sinks)
        best_fit_t *best_fit_info;
        int *n_buffers;
        delay_time_t required_so_far;
        ...
        two_level_t *entry;
{
    ...
    sink_to_buffer_map = ALLOC(
    int, fanout_info->n_elts);
    fanouts = ALLOC(st_table * , entry->n_gates);
    (void) (*two_level_best_fit_fn)(fanout_info, source_index, sink_index, entry->gate_index, &(entry->n_gates),
                                    sink_to_buffer_map);
    for (i = 0; i < entry->n_gates; i++)
        fanouts[i] = st_init_table(st_numcmp, st_numhash);
    for (j = sink_index; j < fanout_info->n_elts; j++)
        (void) st_insert(fanouts[sink_to_buffer_map[j]], (char *) j, NIL(
    char));
    for (i = 0; i < entry->n_gates; i++) {
