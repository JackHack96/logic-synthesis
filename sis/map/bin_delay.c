...
/* ARGSUSED */
int bin_delay_area_tree_match(network, library, globals) network_t *network;
library_t *library;
...
int bin_delay_tree_match(network, library, globals) network_t *network;
library_t *library;
bin_global_t globals;
{
  global = globals;
...
void bin_for_all_nodes_inputs_first(network, fn) network_t *network;
VoidFn fn;
{
  int i;
  array_t *nodevec;
...
void bin_for_all_nodes_outputs_first(network, fn) network_t *network;
VoidFn fn;
{
  int i;
  array_t *nodevec;
...
static int compute_best_match(network) network_t *network;
{
  lsGen gen;
  prim_t *prim;
...
static int find_best_delay(prim) prim_t *prim;
{
  int i;
  lsGen gen;
  lib_gate_t *gate;
...
static void preserve_best_area(bucket, gate, ninputs,
                               pin_info) area_bucket_t *bucket;
lib_gate_t *gate;
int ninputs;
pin_info_t *pin_info;
{
...
static delay_bucket_t *compute_wire_bucket(gate, input) lib_gate_t *gate;
node_t *input;
{
  delay_bucket_t *bucket;
  int infinity_flag;
...
static delay_bucket_t *compute_constant_gate_bucket(node) node_t *node;
{
  delay_bucket_t *bucket;
...
delay_bucket_t *bin_delay_compute_gate_bucket(gate, ninputs,
                                              pin_info) lib_gate_t *gate;
int ninputs;
pin_info_t *pin_info;
{
  int i;
...
delay_pwl_t bin_delay_compute_gate_pwl(gate, ninputs,
                                       pin_info) lib_gate_t *gate;
int ninputs;
pin_info_t *pin_info;
{
...
delay_time_t bin_delay_compute_pwl_delay(pwl, load) pwl_t *pwl;
double load;
{
...
delay_time_t bin_delay_compute_delay_pwl_delay(pwl, load) delay_pwl_t pwl;
double load;
{
  pwl_point_t *p_rise;
  pwl_point_t *p_fall;
...
delay_pwl_t bin_delay_select_active_pwl_delay(pwl, load) pwl_t *pwl;
double load;
{
  pwl_point_t *point;
  delay_bucket_t *bucket;
...
void bin_delay_update_node_pwl(node, bucket) node_t *node;
delay_bucket_t *bucket;
{
...
static pwl_t *get_bucket_pwl_max(bucket) delay_bucket_t *bucket;
{
  pwl_t *max_pwl;
...
static void delay_bucket_print(fp, bucket) FILE *fp;
delay_bucket_t *bucket;
{
  int i;
...
static void select_best_gate(node) node_t *node;
{
  double load;
  pwl_point_t *point;
  delay_bucket_t *bucket;
...
static void copy_bucket_to_map(node, bucket) node_t *node;
delay_bucket_t *bucket;
{
  int i;
  int n_inputs;
...
static void increment_input_loads(node, bucket) node_t *node;
delay_bucket_t *bucket;
{
  int pin;
  node_t *fanin;
...
static void bin_init_pi(node) node_t *node;
{
  delay_bucket_t *bucket;
...
delay_pwl_t bin_delay_get_pi_delay_pwl(node) node_t *node;
{
  delay_pwl_t result;
...
static void bin_alloc(node) node_t *node;
{
  int i;
  int p;
  node_t *fanin;
...
static void bin_free(node) node_t *node;
{
  int p;
...
static delay_bucket_t *delay_bucket_alloc(type) delay_bucket_type_t type;
{
  delay_bucket_t *bucket;
...
static void print_po_estimated_arrival_times(network) network_t *network;
{
  lsGen gen;
  node_t *node;
...
static node_t *get_input_from_pin_info(pin_info) pin_info_t *pin_info;
{
...
static double get_area_from_pin_info(pin_info) pin_info_t *pin_info;
{
  if (pin_info->leaf == NIL(node_t)) {
