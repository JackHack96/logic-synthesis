...
int map_check_form(network, nand_flag) network_t *network;
int nand_flag;
{
  lsGen gen;
  node_t *p;
  ...
  foreach_node(network, gen, p) {
    if (p->type == INTERNAL) {
      ...
      for (i = node_num_fanin(p) - 1; i >= 0; i--) {
        if (count[2 * i] != 0)
          goto bad_network; /* pos phase */
        if (count[2 * i + 1] != 1)
          goto bad_network; /* neg phase */
      }
      FREE(count);
    }
  }
...
bad_network:
  (void)sprintf(errmsg, "\"%s\": '%s' is not a 1 or 2-input %s gate\n",
                network_name(network), node_name(p),
                nand_flag ? "nand" : "nor");
  error_append(errmsg);
  return 0;
}
