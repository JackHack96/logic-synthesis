...
static node_t *create_inverter(nodelist, fanin) lsList nodelist;
node_t *fanin;
{
  node_t *inv;
...
static int feeds_po(node) node_t *node;
{
  lsGen gen;
  node_t *fanout;
...
static int feeds_latch(node) node_t *node;
{
#ifndef SIS
  return 0;
#else
...
static void add_inverters(nodelist, node, deleted) lsList nodelist;
node_t *node;
st_table *deleted;
{
  int i;
...
void map_add_inverter(network, add_at_pipo) network_t *network;
int add_at_pipo;
{
  lsList nodelist;
  st_table *deleted;
...
void map_remove_inverter(network, report_data) network_t *network;
VoidFn report_data;
{
  int i, j, num_fanout;
