...
static node_t *create_inverter(lsList nodelist, node_t *fanin) {
    node_t *inv;
...
static int feeds_po(node_t *node) {
    lsGen  gen;
    node_t *fanout;
...
static int feeds_latch(node_t *node) {
#ifndef SIS
    return 0;
#else
    lsGen  gen;
...
static void add_inverters(lsList nodelist, node_t *node, st_table *deleted) {
    int     i;
...
void map_add_inverter(network_t *network, int add_at_pipo) {
    lsList       nodelist;
    st_table     *deleted;
    node_t       *node;
    int          need_inv;
    ...
    foreach_node(network, gen, node) {
        if (st_lookup(deleted, (char *) node, NIL(char *)))
            continue;
        switch (node_function(node)) {
            case NODE_PI:need_inv = node_num_fanout(node) > 1 || add_at_pipo;
                break;
            case NODE_PO:
            case NODE_INV:need_inv = 0;
                break;
            case NODE_0:
            case NODE_1:need_inv = 0;
                break;
            case NODE_AND:
            case NODE_OR:
                ...
                break;
            default: fail("map_add_inverter: what is this doing here ?");
                /* NOTREACHED */
        }
        if (need_inv) {
    ...
    st_foreach_item(deleted, del_gen, (char **) &node, NIL(
            char *)) {
...
void map_remove_inverter(network_t *network, VoidFn report_data) {
    int     i, j, num_fanout;
    ...
    for (i = 0; i < array_n(nodes); i++) {
        ...
        if (node_function(inv2) == NODE_INV) {
            ...
            if (node_function(inv1) == NODE_INV) {
                ...
                j          = 0;
                foreach_fanout(inv2, gen, fanout) { fanouts[j++] = fanout; }
            ...
            foreach_fanout(node, gen, fanout) {
                if (node_function(fanout) == NODE_INV) {
                    if (inv1 == NIL(node_t)) {
                        ...
                        fanouts    = ALLOC(node_t *, num_fanout);
                        j          = 0;
                        foreach_fanout(fanout, gen1, fanout1) { fanouts[j++] = fanout1; }
                        for (j = 0; j < num_fanout; j++) {
