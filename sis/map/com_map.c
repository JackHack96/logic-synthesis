...
static int
com_build_map(network, argc, argv)
        network_t **network;
        int argc;
        char **argv;
{
...
static int
com_tree_map(network, argc, argv)
        network_t **network;
        int argc;
        char **argv;
{
...
static int
com_fanout_opt(network, argc, argv)
        network_t **network;
        int argc;
        char **argv;
{
...
/* ARGSUSED */
static int
com_fanout_alg(network, argc, argv)
        network_t **network;
        int argc;
        char **argv;
{
    ...
    if (current_fanout_alg == NIL(array_t)) {
        current_fanout_alg = fanout_opt_get_fanout_alg(0, NIL(
        char *));
    }
    argc--;
    argv++;
    ...
    if (!no_options) current_fanout_alg = fanout_opt_get_fanout_alg(argc, argv);
    if (verbose || no_options) fanout_opt_print_fanout_alg(current_fanout_alg);
    return 0;
    usage:
...
/* ARGSUSED */
static int
com_fanout_param(network, argc, argv)
        network_t **network;
        int argc;
        char **argv;
{
    ...
    fanout_opt_set_fanout_param(current_fanout_alg, argc, argv, &fanout_alg_index);
    if (fanout_alg_index == -1) goto usage;
    if (verbose) fanout_opt_print_fanout_param(current_fanout_alg, fanout_alg_index);
    return 0;
    usage:
...
static bin_global_t DEFAULT_OPTIONS = {
        ...
        0,            /* int fanout_optimize;		// if set, the fanout optimizer is called */
        0,            /* int area_recover;		// if set, area is recovered after fanout opt */
        0,            /* int peephole;		// if set, call fanout peephole optimizer */
        0,            /* array_t *fanout_alg;		// array of fanout algorithm descriptors */
        0,            /* int remove_inverters;	// if set, remove redundant inverters after mapping */
        0,            /* int no_warning;		// if set, does not print any warning message */
        0,            /* int ignore_pipo_data;	// if set, ignore specific values set at PIPO's */
        RAW_NETWORK,        /* network_type_t network_type; // type of the network when mapping is started */
        2,            /* int load_estimation;		// 0-ignore fanout; 1-linear fanout; 2-load of fanout tree */
        1,            /* int ignore_polarity;		// 0-add an inverter delay; 1-same arrival for both pol. */
        0,            /* int cost_function;		// 0-MAX(rise,fall); 1-AVER(rise,fall) */
        0,            /* int n_iterations;		// 0-do it once, 1-repeat once, etc... */
        0,            /* int fanout_iterate;		// 1 if tree covering is called afterwards; 0 otherwise */
        1,            /* int opt_single_fanout;	// 0/1 */
        0,            /* int fanout_log_on;		// 0/1 */
        0,            /* int allow_duplication;       // 0/1 */
        INFINITY,        /* int fanout_limit;		// limit fanout of internal nodes during matching */
        1,                    /* int check_load_limit;        // if set, check load limit during fanout opt */
        1000,            /* int penalty_factor;		// factor by which load is multiplied when exceeds max load limit of a gate */
        0            /* int all_gates_area_recover;  // when set, recover area on all gates, not only buffers */
};

int map_fill_options(options, argc_addr, argv_addr)
        bin_global_t *options;
        int *argc_addr;
        char ***argv_addr;
{
    ...
    *options = DEFAULT_OPTIONS;
    ...
    util_getopt_reset();
    while ((c = util_getopt(argc, argv, "Ab:B:c:dFGf:iI:l:L:m:n:pPrst:v:W")) != EOF) {
        switch (c) {
...
/*
  ...
  if (! options->fanout_optimize && options->new_mode == 0 && options->old_mode == 0.0) {
*/
    ...
    if (options->allow_internal_fanout ==
        -1) { /* determine the default for internal fanout: 11 for min area, 0 otherwise */
        options->allow_internal_fanout = 0;
        if (!options->fanout_optimize && options->new_mode == 0 && options->old_mode == 0.0) {
            options->allow_internal_fanout = 11;
        }
    } else {                      /* if -f option was specified non zero with the fanout optimizer, disallow it */
/* do not disallow it any more: needed for experiments.
    if (options->allow_internal_fanout != 0 && options->fanout_optimize) {
      options->allow_internal_fanout = 0;
      if (! options->no_warning) (void) fprintf(siserr, "WARNING: internal fanout disallowed with fanout optimization\n");
    }
*/
    }
    *argc_addr = argc;
...
static void map_report_usage(command_name)
        char *command_name;
{
    ...
    (void) fprintf(miserr,
                   "\t-B n\tenforce load limit during fanout optimization: 0=no enforcement 1(default)=enforcement\n");
...
/* ARGSUSED */
static int
com_read_library(network, argc, argv)
        network_t **network;
        int argc;
        char **argv;
{
    ...
    /* read a new library */
    if (!lib_read_blif(fp, real_filename,
                       add_inverters, nand_flag, &lib_current_library)) {
        (void) fclose(fp);
        ...
        array_t *node_vec;
{
    ...
    for (i = 0; i < array_n(node_vec); i++) {
        ...
        if (node->type == INTERNAL) {
            ...
            if (gate == NIL(lib_gate_t)) {
                ...
                if (gatename == NIL(char)) gatename = "<none>";
            }
    ...
    avl_foreach_item(gate_count_table, avlgen, AVL_FORWARD, &key, &value)
    {
        ...
        (void) fprintf(sisout, "%-15s : %4d (area=%4.2f)\n",
                       count->name, count->count, count->area);
        total_area += count->count * count->area;
        total_gates += count->count;
    }
    (void) fprintf(sisout,
                   "Total: %d gates, %4.2f area\n", total_gates, total_area);
        ...
        char **argv;
{
    ...
    if (lib_network_is_mapped(*network)) {
        ...
        (void) fprintf(sisout, ">>> removing extra series and parallel inverters <<<\n");
...
/* ARGSUSED */
com_print_map_stats(network, argc, argv
)
network_t **network;
int       argc;
char      **argv;
...
(void)
fprintf(sisout,
"Most Negative Slack\t= %6.2f\n", min_slack);
(void)
fprintf(sisout,
"Sum of Negative Slacks\t= %6.2f\n", total_slack);
(void)
fprintf(sisout,
"Number of Critical PO\t= %d\n", num_out);
return 0;
}
...
com_reduce_depth(network, argc, argv
)
network_t **network;
int       argc;
char      **argv;
...
cluster_size = 8;
options.
depth = -1; /* initialized to unspecified */
options.
dup_ratio = 2.0;
...
while ((
c = util_getopt(argc, argv, "bd:f:rR:s:S:v:g")
) != EOF) {
switch (c) {
case 'b':
options.
type = BEST_RATIO_CONSTRAINT;
options.
relabel = 1;
break;
case 'R':
options.
dup_ratio = atof(util_optarg);
if (options.dup_ratio < 1.0 || options.dup_ratio > INFINITY) goto
usage;
break;
case 'd':
options.
...
options.
depth = atoi(util_optarg);
if (options.depth <= 0) goto
usage;
break;
case 'f':
options.
type = FANIN_CONSTRAINT;
options.
cluster_size = atoi(util_optarg);
if (options.cluster_size <= 1) goto
usage;
break;
case 'g':
options.
...
options.
cluster_size = atoi(util_optarg);
if (options.cluster_size <= 0) goto
usage;
break;
case 'S':
options.
type = SIZE_AS_DEPTH_CONSTRAINT;
options.
cluster_size = atoi(util_optarg);
if (options.cluster_size <= 0) goto
usage;
...
(void)
fprintf(miserr,
"usage: reduce_depth [-S n | -s n | -f n | -d n | -b | -g] [-R n.n] [-r] [-v n]\n");
(void)
fprintf(miserr,
"     -S n=max_cluster_size\tmaximum value of cluster_size; pick up the smallest one yielding the same depth\n");
(void)
fprintf(miserr,
"     -s n=cluster_size\tmaximum number of nodes in a cluster (Default = 8)\n");
(void)
fprintf(miserr,
"     -f n=fanin\tmaximum number of inputs (fanins) for a cluster\n");
(void)
fprintf(miserr,
"     -d n=max_level\tmaximum level of nodes in final network\n");
(void)
fprintf(miserr,
"     -b\tuses the best cluster size for dup ratio less than specified with -R\n");
(void)
fprintf(miserr,
"     -g\tprints out statistics on a cluster size basis; no clustering done\n");
(void)
fprintf(miserr,
"     -R n.n\tspecifies max dup ratio; default 2.0\n");
(void)
fprintf(miserr,
"     -r\trelabeling to reduce logic duplication without increasing # levels\n");
(void)
fprintf(miserr,
"     -v n=verbosity_level\tdebug mode\n");
return 1;
}
    ...
    while ((c = util_getopt(argc, argv, "t:k")) != EOF) {
        switch (c) {
            case 't':timeout = atoi(util_optarg);
                if (timeout <= 0 || timeout > 365 * 24 * 3600) goto usage;
                break;
...
static int
com_check_load_limit(network, argc, argv)
        network_t **network;
        int argc;
        char **argv;
{
    ...
    for (i  = 0; i < array_n(nodevec); i++) {
        ...
        if (node->type == PRIMARY_OUTPUT) {
            ...
            {
                if (fanin->type != PRIMARY_INPUT) {
                    MAP(fanin)->load += delay_get_load(MAP(node)->gate->delay_info[index]);
                }
            }
        }
    }
    for (i = 0; i < array_n(nodevec); i++) {
        ...
        if (node_num_fanin(node) == 0) continue; /* skip constants for now */
        max_load = delay_get_load_limit(MAP(node)->gate->delay_info[0]);
        if (MAP(node)->load > max_load) {
            (void) fprintf(sisout, "Load at node %s for gate %s exceeded: allowed %2.3f actual %2.3f\n",
                           node->name, MAP(node)->gate->name,
                           max_load,
                           MAP(node)->load);
        }
    }
    ...
    foreach_primary_input(*network, gen, node)
    {
        ...
        if (load > max_load) {
            (void) fprintf(sisout, "Load at PI %s exceeded: allowed %2.3f actual %2.3f\n",
...
init_map() {
    ...
    com_add_command("_check_load_limit", com_check_load_limit, /* changes_network */ 0);
 ...
 */
static node_t*
map_copy_clock(new_network, old_latch)
network_t *new_network;
latch_t *old_latch;
{
  ...
  if (old_clock == NIL(node_t)) return NIL(node_t);
  new_clock = network_find_node(new_network, old_clock->name);
  if (new_clock != NIL(node_t)) return new_clock;

  /* make sure that there exists a PI/PO pair in the old network */
  assert(node_type(old_clock) == PRIMARY_OUTPUT && node_num_fanin(old_clock) == 1);
