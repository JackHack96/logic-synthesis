  ...
  *options = DEFAULT_OPTIONS;
  ...
  while ((c = util_getopt(argc, argv, "Ab:B:c:dFGf:iI:l:L:m:n:pPrst:v:W")) != EOF) {
    ...
    case 'A':
      ...
      break;
    case 'G':
      options->all_gates_area_recover = 1;
      options->area_recover = 1;
      options->fanout_optimize = 1;
      break;
    case 'b':
      options->penalty_factor = atoi(util_optarg);
	...
	return 1;
      ...
      break;
    case 'B':
      options->check_load_limit = atoi(util_optarg);
	...
	return 1;
      ...
      break;
    case 'c':
      options->cost_function = atoi(util_optarg);
	...
	return 1;
      ...
      break;
    case 'd':
      options->allow_duplication = 1;
      break;
    case 'F':
      options->fanout_optimize = 1;
      break;
    case 'f':
      options->allow_internal_fanout = atoi(util_optarg);
      break;
    case 'i':
      options->inverter_optz = 0;
      break;
    case 'I':
      options->n_iterations = atoi(util_optarg);
      break;
    case 'l':
      options->load_estimation = atoi(util_optarg);
	...
	return 1;
      ...
      break;
    case 'L':
      options->fanout_limit = atoi(util_optarg);
	...
	return 1;
      ...
      break;
    case 'm':
      options->old_mode = atof(util_optarg);
	...
	options->old_mode = 0.0;
      ...
      break;
    case 'p':
      options->ignore_pipo_data = 1;
      break;
    case 'P':
      options->ignore_polarity = 1 - options->ignore_polarity;
      break;
    case 'r':
      options->raw = 1;
      break;
    case 's':
      options->print_stat = 1;
      break;
    case 't':
      options->old_mode = 2.0;
    ...
    case 'v':
      ...
      break;
    case 'W':
      options->no_warning = 1;
      break;
    default:
      error_append("map_interface: unknown option\n");
      return 1;
    }
  }
...
*/
  ...
  if (options->allow_internal_fanout == -1) { /* determine the default for internal fanout: 11 for min area, 0 otherwise */
...
char **argv;
{
  ...
  while ((c = util_getopt(argc, argv, "bd:f:rR:s:S:v:g")) != EOF) {
    switch(c) {
    case 'b':
      options.type = BEST_RATIO_CONSTRAINT;
      options.relabel = 1;
      break;
    case 'R':
      options.dup_ratio = atof(util_optarg);
      if (options.dup_ratio < 1.0 || options.dup_ratio > INFINITY) goto usage;
      break;
    case 'd':
      options.type = DEPTH_CONSTRAINT;
      options.depth = atoi(util_optarg);
      if (options.depth <= 0) goto usage;
      break;
    case 'f':
      options.type = FANIN_CONSTRAINT;
      options.cluster_size = atoi(util_optarg);
      if (options.cluster_size <= 1) goto usage;
      break;
    case 'g':
      options.type = CLUSTER_STATISTICS;
      break;
    case 'r':
      options.relabel = 1;
      break;
    case 's':
      options.type = SIZE_CONSTRAINT;
      options.cluster_size = atoi(util_optarg);
      if (options.cluster_size <= 0) goto usage;
      break;
    case 'S':
      options.type = SIZE_AS_DEPTH_CONSTRAINT;
      options.cluster_size = atoi(util_optarg);
      if (options.cluster_size <= 0) goto usage;
      break;
    case 'v':
  ...
  if (new_network == NIL(network_t)) return 1;
  *network = new_network;
  return 0;
 usage:
  (void) fprintf(miserr, "usage: reduce_depth [-S n | -s n | -f n | -d n | -b | -g] [-R n.n] [-r] [-v n]\n");
