...
static int com_replace(network, argc, argv) network_t **network;
int argc;
char **argv;
{
  int c;
...
static int com_map(network, argc, argv) network_t **network;
int argc;
char **argv;
{
...
static int com_build_map(network, argc, argv) network_t **network;
int argc;
char **argv;
{
  network_t *new_network;
...
static int com_tree_map(network, argc, argv) network_t **network;
int argc;
char **argv;
{
  network_t *new_network;
...
static int com_fanout_opt(network, argc, argv) network_t **network;
int argc;
char **argv;
{
  network_t *new_network;
...
/* ARGSUSED */
static int com_fanout_alg(network, argc, argv) network_t **network;
int argc;
char **argv;
{
  int no_options = 0;
...
/* ARGSUSED */
static int com_fanout_param(network, argc, argv) network_t **network;
int argc;
char **argv;
{
...
static bin_global_t DEFAULT_OPTIONS = {
    ...
    1, /* int check_load_limit;        // if set, check load limit during fanout
          opt */
    1000, /* int penalty_factor;		// factor by which load is multiplied
             when exceeds max load limit of a gate */
    0 /* int all_gates_area_recover;  // when set, recover area on all gates,
         not only buffers */
};

int map_fill_options(options, argc_addr, argv_addr) bin_global_t *options;
int *argc_addr;
char ***argv_addr;
{
  int c;
...
static void map_report_usage(command_name) char *command_name;
{
...
/* ARGSUSED */
static int com_read_library(network, argc, argv) network_t **network;
int argc;
char **argv;
{
  FILE *fp, *outfp;
...
static void gate_summary(node_vec) array_t *node_vec;
{
  int i, total_gates;
  node_t *node;
  gate_count_t *count;
...
static void print_lib_gate_info(node, print_pins) node_t *node;
int print_pins;
{
  int j;
  node_t *fanin;
...
/* ARGSUSED */
static int com_print_gate(network, argc, argv) network_t **network;
int argc;
char **argv;
{
  node_t *node;
...
/* ARGSUSED */
static int com_treesize(network, argc, argv) network_t **network;
int argc;
char **argv;
{
  if (argc != 1)
...
/* ARGSUSED */
static int com_eat_buffer(network, argc, argv) network_t **network;
int argc;
char **argv;
{
  lsGen gen;
...
/* ARGSUSED */
static int com_premap(network, argc, argv) network_t **network;
int argc;
char **argv;
{
  library_t *library;
...
static int com_map_addinv(network, argc, argv) network_t **network;
int argc;
char **argv;
{
  int at_pipo, c;
...
/* ARGSUSED */
static int com_map_reminv(network, argc, argv) network_t **network;
int argc;
char **argv;
{
  if (argc != 1) {
...
/* ARGSUSED */
static int com_buffer(network, argc, argv) network_t **network;
int argc;
char **argv;
{
  if (argc != 1) {
...
/* ARGSUSED */
static int com_dump_patterns(network, argc, argv) network_t **network;
int argc;
char **argv;
{
  if (lib_get_library() == 0) {
...
/* ARGSUSED */
static int com_print_library(network, argc, argv) network_t **network;
int argc;
char **argv;
{
  lib_class_t *class;
...
/* ARGSUSED */
com_print_map_stats(network, argc, argv) network_t **network;
int argc;
char **argv;
{
  lsGen gen;
...
com_reduce_depth(network, argc, argv) network_t **network;
int argc;
char **argv;
{
  int c;
...
/* ARGSUSED */
static int com_timeout(network, argc, argv) network_t **network;
int argc;
char **argv;
{
  int c;
...
static int com_check_load_limit(network, argc, argv) network_t **network;
int argc;
char **argv;
{
  int i, j;
...
#ifdef SIS
/* Sequential support */
static void map_latch_table_dup(old, new) network_t *old, *new;
{
  lsGen gen;
  node_t *po, *new_po;
  node_t *pi, *new_pi;
 ...
 */
static node_t *map_copy_clock(new_network, old_latch) network_t *new_network;
latch_t *old_latch;
{
