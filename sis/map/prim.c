...
int map_network_to_prim(network, prim_p) network_t *network;
prim_t **prim_p;
{
  prim_t *prim;
...
static prim_t *make_primitive(network, isomorphic_sons) network_t *network;
st_table *isomorphic_sons;
{
  node_t *node, *po;
  prim_t *prim;
  ...
  foreach_node(network, gen, node) {
    ...
    if (node->type == INTERNAL || node->type == PRIMARY_INPUT ||
        (node->type == PRIMARY_OUTPUT &&
         network_gate_type(network) != (int)COMBINATIONAL)) {
#else
    if (node->type == INTERNAL || node->type == PRIMARY_INPUT) {
#endif /* SIS */
...
static int make_edge_list(network, prim) network_t *network;
prim_t *prim;
{
...
static void reorder(node, prev, dir, arc_table, visited) node_t *node, *prev;
edge_dir_t dir;
st_table *arc_table;
st_table *visited;
{
  ...
  if ((node->type != PRIMARY_OUTPUT) ||
      (node->type == PRIMARY_OUTPUT &&
       network_gate_type(node->network) != (int)COMBINATIONAL)) {
#else
  if (node->type != PRIMARY_OUTPUT) {
#endif /* SIS */
...
static int arc_compare(arc1_p, arc2_p) char *arc1_p, *arc2_p;
{
...
static int arc_hash(arc_p, modulus) char *arc_p;
int modulus;
{
...
static int arc_find_or_add(table, node1, node2) st_table *table;
node_t *node1, *node2;
{
  arc_t *arc;
...
static void arc_hash_end(table) st_table *table;
{
  st_generator *gen;
  arc_t *arc;

  st_foreach_item(table, gen, (char **)&arc, NIL(char *)) { FREE(arc); }
  st_free_table(table);
}
...
void prim_free(prim) prim_t *prim;
{
...
void prim_clear_binding(prim) prim_t *prim;
{
...
static void map_prim_dump_node(fp, pn) FILE *fp;
prim_node_t *pn;
{
  (void)fprintf(
      fp, "\tname=%-10s iso=%d nfanin=%2d nfanout=%2d type=%-3s binding=%08x\n",
      pn->name == 0 ? "none" : pn->name, pn->isomorphic_sons, pn->nfanin,
      pn->nfanout,
...
static void map_prim_dump_edge(fp, pe) FILE *fp;
prim_edge_t *pe;
{
  (void)fprintf(fp, "    this_node=%08x connected=%08x dir=%s\n", pe->this_node,
                pe->connected_node, pe->dir == DIR_IN ? "IN" : "OUT");
  if (pe->this_node != 0)
    map_prim_dump_node(fp, pe->this_node);
  if (pe->connected_node != 0)
    map_prim_dump_node(fp, pe->connected_node);
}

void map_prim_dump(fp, prim, detail) FILE *fp;
prim_t *prim;
int detail;
{
  lsGen gen;
  ...
  case (int)ACTIVE_HIGH:
    (void)fprintf(fp, " [type=ACTIVE_HIGH]\n");
    break;
  case (int)ACTIVE_LOW:
    (void)fprintf(fp, " [type=ACTIVE_LOW]\n");
    break;
  case (int)RISING_EDGE:
    (void)fprintf(fp, " [type=RISING_EDGE]\n");
    break;
  case (int)FALLING_EDGE:
    (void)fprintf(fp, " [type=FALLING_EDGE]\n");
    break;
  case (int)COMBINATIONAL:
    (void)fprintf(fp, " [type=COMBINATIONAL]\n");
    break;
  case (int)ASYNCH:
    (void)fprintf(fp, " [type=ASYNCH]\n");
    break;
  default:
    (void)fprintf(fp, " [type=UNKNOWN]\n");
    break;
  }
#else
  (void)fprintf(fp, "\n");
...
node_t *map_prim_get_root(prim) prim_t *prim;
{ return prim->edges->this_node->binding; }

/* check if a node only feeds a real PO */
static int prim_feeds_only_real_po(node, po) node_t *node;
node_t **po;
{
  lsGen gen;
  node_t *fanout;
...
static node_t *prim_first_po(network) network_t *network;
{
  lsGen gen;
  node_t *po;
