...
int map_network_to_prim(network_t *network, prim_t **prim_p) {
    prim_t   *prim;
...
static prim_t *make_primitive(network_t *network, st_table *isomorphic_sons) {
    node_t      *node, *po;
    prim_t      *prim;
...
static int make_edge_list(network_t *network, prim_t *prim) {
...
static void reorder(node_t *node, node_t *prev, edge_dir_t dir, st_table *arc_table, st_table *visited) {
    int         i;
    prim_edge_t *edge;
    node_t      *fanin, *fanout;
    lsGen       gen;
...
static int arc_compare(char *arc1_p, char *arc2_p) {
...
static int arc_hash(char *arc_p, int modulus) {
...
static int arc_find_or_add(st_table *table, node_t *node1, node_t *node2) {
    arc_t *arc;
...
static void arc_hash_end(st_table *table) {
    st_generator *gen;
    arc_t        *arc;
...
void prim_clear_binding(prim_t *prim) {
...
static void map_prim_dump_node(FILE *fp, prim_node_t *pn) {
    (void) fprintf(
...
static void map_prim_dump_edge(FILE *fp, prim_edge_t *pe) {
...
void map_prim_dump(FILE *fp, prim_t *prim, int detail) {
    lsGen       gen;
    lib_gate_t  *gate;
    int         i;
    prim_node_t *pn;
...
node_t *map_prim_get_root(prim_t *prim) { return prim->edges->this_node->binding; }

/* check if a node only feeds a real PO */
static int prim_feeds_only_real_po(node_t *node, node_t **po) {
    lsGen  gen;
    node_t *fanout;
...
static node_t *prim_first_po(network_t *network) {
    lsGen  gen;
    node_t *po;
