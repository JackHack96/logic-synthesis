...
/* ARGSUSED */
void noalg_init(network_t *network, fanout_alg_t *alg) { alg->optimize = noalg_optimize; }

/* INTERNAL INTERFACE */

static int noalg_optimize(opt_array_t *fanout_info, array_t *tree, fanout_cost_t *cost) {
...
static multidim_t *optimize_one_source(opt_array_t *fanout_info) {
    ...
    foreach_gate(n_gates, source_index) {
        foreach_polarity(source_polarity) {
            ...
            compute_best_one_source_one_polarity(&fanout_info[source_polarity], entry,
                                                 source_index);
        }
    }
    return table;
...
static void compute_best_one_source_one_polarity(opt_array_t *fanout_info, noalg_t *entry, int source_index) {
    delay_time_t required;
    double       load;
...
static void build_selected_tree(opt_array_t *fanout_info, array_t *tree, selected_source_t *source, multidim_t *table) {
    ...
    if (fanout_info[q].n_elts > 0) {
        if (is_source(n_gates, source->buffer)) {
            ...
            fanout_tree_insert_gate(tree, source->main_source,
...
void noalg_insert_sinks(array_t *tree, opt_array_t *fanout_info, int from, int to) {
    int sink;
