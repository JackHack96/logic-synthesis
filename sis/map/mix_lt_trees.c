  ...
  for (x_index = fanout_info[POLAR_X].n_elts; x_index >= 0; x_index--) {
	...
	for (n_gaps = 0; n_gaps < max_n_gaps; n_gaps++) {
	  foreach_polarity(source_polarity) {
	    ...
	    best_one_source(table, two_level_table, fanout_info, source_index, source_polarity, x_index, y_index, n_gaps);
	  }
	}
      }
    }
...
static void best_one_source(table, two_level_table, fanout_info, source_index, source_polarity, x_index, y_index, n_gaps)
multidim_t *table;
...
int n_gaps;
{
  ...
  case 0:
    ...
    n_fanouts = get_n_fanouts(fanout_info, table, buffer_index, buffer_polarity, buffer_x, buffer_y, buffer_gaps);
    fanout_tree_insert_gate(tree, buffer_index, n_fanouts);
    insert_mixed_lt_tree(tree, table, fanout_info, buffer_index, buffer_polarity, buffer_x, buffer_y, buffer_gaps);
    break;
  case 1:
