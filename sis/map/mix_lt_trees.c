...
static lt_tree_t LT_TREE_INIT_VALUE = {-1, 0, -1, -1, -1, {-INFINITY, -INFINITY}, 0.0};
...
/* ARGSUSED */
void mixed_lt_trees_init(network, alg)
        network_t *network;
        fanout_alg_t *alg;
{
...
/* ARGSUSED */
void mixed_lt_trees_set_max_n_gaps(alg, property)
        fanout_alg_t *alg;
        alg_property_t *property;
{ max_n_gaps = property->value; }

/* ARGSUSED */
void mixed_lt_trees_set_max_x_index(alg, property)
        fanout_alg_t *alg;
        alg_property_t *property;
{ max_x_index = property->value; }

/* ARGSUSED */
void mixed_lt_trees_set_max_y_index(alg, property)
        fanout_alg_t *alg;
        alg_property_t *property;
        ...
        fanout_cost_t *cost;             /* contains information on the result: required times, area */
{
    ...
    cost->area  = INFINITY;
    if (fanout_info[POLAR_X].n_elts == 0) return 0;
    if (fanout_info[POLAR_Y].n_elts == 0) return 0;
    if (fanout_info[POLAR_X].n_elts > max_x_index && fanout_info[POLAR_Y].n_elts > max_y_index) return 0;
                ...
                for (n_gaps = 0; n_gaps < max_n_gaps; n_gaps++) {
                    foreach_polarity(source_polarity) {
                        if (is_source(n_gates, source_index) &&
                            ...
                            continue;
                        best_one_source(table, two_level_table, fanout_info, source_index, source_polarity, x_index,
                                        y_index, n_gaps);
                    }
                }
            }
...
static void best_one_source(table, two_level_table, fanout_info, source_index, source_polarity, x_index, y_index,
                            n_gaps)
        multidim_t *table;
        ...
        int n_gaps;
{
    ...
    lt_tree_t    *entry = INDEX5P(lt_tree_t, table, source_index, source_polarity, x_index, y_index, n_gaps);

    if (x_index == fanout_info[POLAR_X].n_elts && y_index == fanout_info[POLAR_Y].n_elts) {
        entry->balanced   = 0;
        entry->gate_index = -1;
    ...
    if (x_index == fanout_info[POLAR_X].n_elts) {
        two_level_entry = INDEX4P(two_level_t, two_level_table, source_index, source_polarity, y_index, POLAR_Y);
    } else if (y_index == fanout_info[POLAR_Y].n_elts) {
        two_level_entry = INDEX4P(two_level_t, two_level_table, source_index, source_polarity, x_index, POLAR_X);
    } else {
    ...
    for (sink = p_sink; sink <= fanout_info[p].n_elts; sink++) {
        foreach_buffer(n_gates, buffer_index) {
            ...
            buffer_gaps     = (sink == p_sink && sink < fanout_info[p].n_elts) ? n_gaps - 1 : n_gaps;
            ...
            if (sink == fanout_info[p].n_elts && q_sink == fanout_info[q].n_elts) {
                ...
                buffer_entry   = INDEX5P(lt_tree_t, table, buffer_index, buffer_polarity, next_x, next_y, buffer_gaps);
            ...
            load += fanout_info[p].cumul_load[sink] - fanout_info[p].cumul_load[p_sink];
            load += map_compute_wire_load(n_fanouts);
            if (sink > p_sink) SETMIN(local_required, local_required, fanout_info[p].min_required[p_sink]);
...
static void select_best_source(table, best_source, best_cost)
        multidim_t *table;
        ...
        fanout_cost_t *best_cost;
{
    ...
    foreach_source(n_gates, source_index) {
        ...
        entry           = INDEX5P(lt_tree_t, table, source_index, source_polarity, 0, 0, max_n_gaps - 1);
        ...
        multidim_t *table;
{
    ...
    n_fanouts = get_n_fanouts(fanout_info, table, source->main_source, source_polarity, 0, 0, max_n_gaps - 1);
    fanout_tree_insert_gate(tree, source->main_source, n_fanouts);
    insert_mixed_lt_tree(tree, table, fanout_info, source->main_source, source_polarity, 0, 0, max_n_gaps - 1);
}

static void insert_mixed_lt_tree(tree, table, fanout_info, source_index, source_polarity, x_index, y_index, n_gaps)
        array_t *tree;
        multidim_t *table;
        opt_array_t *fanout_info;
        int source_index;
        ...
        int n_gaps;
{
    ...
    if (x_index == fanout_info[POLAR_X].n_elts && y_index == fanout_info[POLAR_Y].n_elts) return;
    entry  = INDEX5P(lt_tree_t, table, source_index, source_polarity, x_index, y_index, n_gaps);
    p      = source_polarity;
    q      = POLAR_INV(p);
    p_sink = (p == POLAR_X) ? x_index : y_index;
    q_sink = (q == POLAR_X) ? x_index : y_index;
    switch (entry->balanced) {
        case 0:sink = (p == POLAR_X) ? entry->x_index : entry->y_index;
            assert(q_sink == ((q == POLAR_X) ? entry->x_index : entry->y_index));
            noalg_insert_sinks(tree, &fanout_info[p], p_sink, sink);
            if (sink == fanout_info[p].n_elts && q_sink == fanout_info[q].n_elts) return;
            buffer_index    = entry->gate_index;
            buffer_polarity = (is_inverter(n_gates, buffer_index)) ? q : p;
            buffer_gaps     = (sink == p_sink && sink < fanout_info[p].n_elts) ? n_gaps - 1 : n_gaps;
            buffer_x        = (p == POLAR_X) ? sink : q_sink;
            buffer_y        = (p == POLAR_Y) ? sink : q_sink;
            n_fanouts       = get_n_fanouts(fanout_info, table, buffer_index, buffer_polarity, buffer_x, buffer_y,
                                            buffer_gaps);
            fanout_tree_insert_gate(tree, buffer_index, n_fanouts);
            insert_mixed_lt_tree(tree, table, fanout_info, buffer_index, buffer_polarity, buffer_x, buffer_y,
                                 buffer_gaps);
            break;
        case 1:
            if (p_sink == fanout_info[p].n_elts) {
                assert(q_sink < fanout_info[q].n_elts);
                insert_two_level_tree(tree, &fanout_info[q], q_sink, source_index, entry->two_level);
            } else if (q_sink == fanout_info[q].n_elts) {
                assert(p_sink < fanout_info[p].n_elts);
                insert_two_level_tree(tree, &fanout_info[p], p_sink, source_index, entry->two_level);
            } else {
...
static int get_n_fanouts(fanout_info, table, source_index, source_polarity, x_index, y_index, n_gaps)
        opt_array_t *fanout_info;
        multidim_t *table;
        int source_index;
        int source_polarity;
        ...
        int n_gaps;
{
    ...
    if (x_index == fanout_info[POLAR_X].n_elts && y_index == fanout_info[POLAR_Y].n_elts) return 0;
    entry = INDEX5P(lt_tree_t, table, source_index, source_polarity, x_index, y_index, n_gaps);
    ...
    if (sink == fanout_info[p].n_elts && q_sink == fanout_info[q].n_elts) return (sink - p_sink);
