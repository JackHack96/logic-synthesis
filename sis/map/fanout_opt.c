...
**
**  01 02-Nov-90 cyt    Changed #include "fanout_alg.macro.h" to
*"fanout_alg_macro.h"
**                      Added code to solve the fanout limit problem and
*#include delay_int
**  02 27-May-91 klk    New code to handle fanout limit.
**  02 10-Jun-91 klk	Changed primary input max drive to correspond to the
*second largest inverter *			in the library
**/
/*
 * $Log: fanout_opt.c,v $
 ...
 * imported
 *
 * Revision 1.4  1993/08/05  16:14:19  sis
 ...
 * *** empty log message ***
 *
 * Revision 1.1  92/01/08  17:40:39  sis
 * Initial revision
 *
 * Revision 1.5  91/07/03  23:23:00  touati
 * (1): fixes by Cho to use simple lib_gate_type to identify latches.
 * (2): bug fix: -G accepts constant nodes now.
 * (3): bug fix: get the fanin of a PO through map_po_get_fanin
 * (4): bug fix: use a more resilient test to compare floating point numbers:
 *      avoid numerical instabilities.
 *
 * Revision 1.4  91/07/02  10:54:05  touati
 * added map -G option: resize all gates.
 *
 * Revision 1.3  91/06/30  22:28:56  touati
 * added code to perform gate resizing for all gates. Works from outputs to
 * inputs.
 *
 */
...
void
fanout_opt_update_peephole_flag(fanout_alg_t *alg, alg_property_t *property) { alg->peephole_flag = property->value; }

void fanout_opt_update_size_threshold(fanout_alg_t *alg, alg_property_t *property) { alg->min_size = property->value; }

/* called by the command line option */

array_t *fanout_opt_get_fanout_alg(int n_entries, char **entries) {
    int               i, j;
...
void fanout_opt_print_fanout_alg(array_t *array) {
    int          i;
    fanout_alg_t *alg;
...
void fanout_opt_set_fanout_param(array_t *array, int n_entries, char **entries, int *alg_index) {
    int            i;
    int            entry_index;
    fanout_alg_t   *alg;
    alg_property_t *property;
...
void fanout_opt_print_fanout_param(array_t *array, int index) {
    int            i;
    alg_property_t *property;
...
void fanout_optimization(network_t *network, bin_global_t globals) {
    global = globals;
    if (!global.fanout_optimize && !global.area_recover)
        return;
...
static void fanout_opt_init(network_t *network) {
    int            i;
    int            j;
    fanout_alg_t   *alg;
    alg_property_t *property;
...
/* ARGSUSED */
static void fanout_opt_end(network_t *network) { /* nothing */ }

static void do_fanout_opt(node_t *node_x) {
#ifdef SIS
    /* no fanout optimization for latches */
    if (lib_gate_type(lib_gate_of(node_x)) != COMBINATIONAL &&
        lib_gate_type(lib_gate_of(node_x)) != UNKNOWN) {
    ...
    switch (node_function(node_x)) {
        ...
        case NODE_1:
            if (!global.no_warning)
                (void) fprintf(siserr,
                               "WARNING: constants should have been removed earlier\n");
            break;
        case NODE_PO:
            /* already processed */
            break;
        case NODE_BUF: fail("should not be any buffers in underlying network");
            break;
        case NODE_INV:
...
typedef struct {
    ...
    array_t
                  *trees[POLAR_MAX]; /* tree for sources[POLAR_X] and sources[POLAR_Y] */
...
static int fanout_map_optimal(node_t *node_x) {
    int          p, assign_x;
    int          n_fanouts;
    node_t       *node_y;
    node_t       *sources[2];
    ...
    if (!extract_fanout_problem(node_x, sources, fanout_info))
        return 0;
     ...
     * (2) sources[POLAR_X] for sinks of POLAR_X and sources[POLAR_Y] for sinks of
     * POLAR_Y (3) sources[POLAR_X] for sinks of POLAR_Y and sources[POLAR_Y] for
     * sinks of POLAR_X if only one polarity of sources, only (0) or (1) is
     * attempted if only one polarity of sinks, only (0) and (1) are attempted To
     * take tree area into account, we add MAP(source)->gate->area to the cost.
     */

    best_sol = SOURCE_MAX;
    if (sources[POLAR_X] != NIL(node_t)) {
        fanout_single_source_optimal(sources[POLAR_X], fanout_info, POLAR_X,
                                     &fanout_sol[SOURCE_X_ALONE]);
    ...
    if (sources[POLAR_Y] != NIL(node_t)) {
        fanout_single_source_optimal(sources[POLAR_Y], fanout_info, POLAR_Y,
                                     &fanout_sol[SOURCE_Y_ALONE]);
        ...
        } else if (fanout_opt_is_better_cost(&fanout_sol[SOURCE_X_ALONE].cost,
                                             &fanout_sol[SOURCE_Y_ALONE].cost)) {
    ...
    if (sources[POLAR_X] != NIL(node_t) && sources[POLAR_Y] != NIL(node_t) &&
        ...
        foreach_polarity(assign_x) {
            ...
            fanout_single_source_optimal(sources[assign_x], fanout_info_arg, assign_x,
                                         assign_sol);
            fanout_info_arg[POLAR_X] = empty_fanout_info;
            fanout_info_arg[POLAR_Y] = fanout_info[POLAR_Y];
            fanout_single_source_optimal(sources[POLAR_INV(assign_x)],
                                         fanout_info_arg, POLAR_INV(assign_x),
                                         assign_sol);
            assign_sol->cost = fanout_opt_add_cost(&(assign_sol->costs[POLAR_X]),
                                                   &(assign_sol->costs[POLAR_Y]));
            foreach_polarity(p) {
                if (MAP(sources[p])->gate)
                    assign_sol->cost.area += MAP(sources[p])->gate->area;
            }
            if (fanout_opt_is_better_cost(&(fanout_sol[best_sol].cost),
                                          &(assign_sol->cost))) {
                fanout_tree_free(assign_sol->trees[POLAR_X], 0);
                fanout_tree_free(assign_sol->trees[POLAR_Y], 0);
            } else {
                foreach_polarity(p) {
                    if (fanout_sol[best_sol].trees[p])
     ...
     * in that process though. Do not call virtual_network_remove_node
     * recursively. Not clear why it is done here, nor wht it is not done
     * recursively after fanout tree construction. --> Check that later.
     */
    ...
    if (global.fanout_iterate && n_fanouts > 1) {
        ...
        if (node_y != NIL(node_t) && node_num_fanout(node_y) > 1) {
            ...
            MAP(node_y)->fanout_source = (sources[POLAR_X] == NIL(node_t))
                                         ? sources[POLAR_Y]
                                         : sources[POLAR_X];
        } else {
            assert(node_x != NIL(node_t) && node_num_fanout(node_x) > 1);
            MAP(node_x)->fanout_tree   = best_tree;
            MAP(node_x)->fanout_source = (sources[POLAR_X] == NIL(node_t))
                                         ? sources[POLAR_Y]
                                         : sources[POLAR_X];
        }
    }
    ...
    if (fanout_sol[best_sol].trees[POLAR_Y] == NIL(array_t) &&
        MAP(node_x)->node_y != NIL(node_t)) {
...
static void
fanout_single_source_optimal(node_t *source, opt_array_t *fanout_info, int source_polarity, fanout_sol_t *fanout_sol) {
    int           i;
    array_t       **trees;
    fanout_cost_t *costs;
    fanout_alg_t  *alg;
    ...
    for (i = 0; i < n_algs; i++) {
        ...
        if (alg->min_size >
            fanout_info[POLAR_X].n_elts + fanout_info[POLAR_Y].n_elts)
            continue;
        if (alg->on_flag && (*alg->optimize)(fanout_info, trees[i], &costs[i])) {
            ...
            if (alg->peephole_flag)
                fanout_tree_peephole_optimize(trees[i], &costs[i], fanout_info,
                                              global.verbose);
#ifdef DEBUG
    ...
    (void)fprintf(sisout, "source node: %s; Best algorithm: %s. ", source->name,
                  alg->name);
#endif
...
static int extract_fanout_problem(node_t *root, node_t **sources, opt_array_t *fanout_info) {
    int p, q;
    int n_fanouts;
    ...
    foreach_polarity(p) { sources[p] = keep_external_source_only(sources[p]); }
    n_fanouts = fanout_info[POLAR_X].n_elts + fanout_info[POLAR_Y].n_elts;
    if (n_fanouts > 1 || (n_fanouts >= 1 && global.opt_single_fanout))
        return 1;
    foreach_polarity(p) { fanout_info_free(&fanout_info[p], 1); }
    return 0;
}

static st_table *extract_links(node_t *source, node_t *exception) {
    gate_link_t link, *link_ptr;
    st_table    *result = st_init_table(st_ptrcmp, st_ptrhash);

    if (source == NIL(node_t))
        return result;
    if (gate_link_is_empty(source))
        return result;
    gate_link_first(source, &link);
    do {
        if (link.node == exception)
            continue;
...
void fanout_info_preprocess(opt_array_t *fanout_info) {
    int          n_elts;
    int          i            = 0;
    gate_link_t  *key;
    double       acc_load;
...
static node_t *keep_external_source_only(node_t *node) {
    if (node == NIL(node_t))
        return node;
...
static int compare_gate_links(char *obj1, char *obj2) {
...
void fanout_info_free(opt_array_t *fanout_info, int free_link_flag) {
    gate_link_t  *key;
    char         *dummy_value = NIL(char);
    st_generator *gen;
...
int fanout_opt_is_better_cost(fanout_cost_t *cost1, fanout_cost_t *cost2) {
    double slack1 = GETMIN(cost1->slack);
    double slack2 = GETMIN(cost2->slack);
    double diff   = slack1 - slack2;

    if (FP_EQUAL(slack1, 0.0))
        slack1 = 0.0;
    if (FP_EQUAL(slack2, 0.0))
        slack2 = 0.0;
    if (FP_EQUAL(diff, 0.0))
        diff   = 0.0;
...
static fanout_cost_t fanout_opt_add_cost(fanout_cost_t *cost1, fanout_cost_t *cost2) {
...
static void compute_required_times(node_t *node) {
#ifdef SIS
    /* skip latches */
    if (lib_gate_type(lib_gate_of(node)) != COMBINATIONAL &&
        lib_gate_type(lib_gate_of(node)) != UNKNOWN) {
    ...
    switch (node_function(node)) {
        case NODE_UNDEFINED:
            ...
            break;
        case NODE_0:
        case NODE_1: fail("ERROR: constants should have been removed earlier\n");
            break;
        case NODE_PO:
        case NODE_PI:
            /* nothing to do */
...
static void do_global_area_recover(node_t *node) {
    gate_info_t *info;
...
static void resize_node(node_t *node, gate_info_t *info) {
    int           i;
    int           best_index;
...
static delay_time_t get_resized_node_arrival_time(node_t *node, lib_gate_t *new_gate) {
    int          i;
    ...
    for (i          = 0; i < MAP(node)->ninputs; i++) {
        load_diff = delay_get_load(new_gate->delay_info[i]) -
                    delay_get_load(MAP(node)->gate->delay_info[i]);
        arrival_times[i] =
                get_reloaded_node_arrival_time(MAP(node)->save_binding[i], load_diff);
    ...
    arrival    = delay_map_simulate(MAP(node)->ninputs, arrival_times_p,
...
/* the arrival time information is supposed to have been saved in
 * MAP(node)->arrival_info */
/* when flag is set, new load value is kept in node */

static delay_time_t get_reloaded_node_arrival_time(node_t *node, double load_diff) {
    double load, load_limit;
...
/* have to change the loads on the inputs and the gate_links appropriately for
 * load info */
/* do not need to compute the required times: done later */
/* WARNING: may be wrong: what if same gate has several connections to the same
 * node? */

static void replace_node_gate(node_t *node, lib_gate_t *new_gate) {
    int    i;
    node_t *input;
    double load_diff;
...
/* FUNCTION: to be used when the output load has changed to recompute
 * MAP(node)->map_arrival */

static delay_time_t recompute_map_arrival(node_t *node, double load) {
    int          i;
    ...
    if (node->type == PRIMARY_INPUT) {
        ...
        arrival = delay_map_simulate(MAP(node)->ninputs, arrival_times,
                                     MAP(node)->gate->delay_info, load);
        FREE(arrival_times);
    }
    return arrival;
}

/* PROCEDURE: to be used when the arrival times of the inputs have changed to
 * recompute */
/* MAP(node)->arrival_info and MAP(node)->map_arrival */

static void recompute_arrival_times(node_t *node) {
    int          i;
    delay_time_t *arrival_times;
    delay_time_t **arrival_times_p;
    ...
    MAP(node)->map_arrival =
            delay_map_simulate(MAP(node)->ninputs, arrival_times_p,
                               MAP(node)->gate->delay_info, MAP(node)->load);
...
static void virtual_net_for_all_nodes_outputs_first(network_t *network, VoidFn fn) {
    lsGen    gen;
    node_t   *pi;
    st_table *visited;
...
static void outputs_first_rec(node_t *node, VoidFn fn, st_table *visited) {
    gate_link_t link;
...
static void virtual_net_for_all_nodes_inputs_first(network_t *network, VoidFn fn) {
    lsGen    gen;
    node_t   *po;
    node_t   *node;
    st_table *visited;
...
static void inputs_first_rec(node_t *node, VoidFn fn, st_table *visited) {
    int i;
