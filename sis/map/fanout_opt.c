...
void fanout_opt_update_peephole_flag(alg, property) fanout_alg_t *alg;
alg_property_t *property;
{ alg->peephole_flag = property->value; }

void fanout_opt_update_size_threshold(alg, property) fanout_alg_t *alg;
alg_property_t *property;
{ alg->min_size = property->value; }

/* called by the command line option */

array_t *fanout_opt_get_fanout_alg(n_entries, entries) int n_entries;
char **entries;
{
  int i, j;
...
void fanout_opt_print_fanout_alg(array) array_t *array;
{
  int i;
  fanout_alg_t *alg;
...
void fanout_opt_set_fanout_param(array, n_entries, entries,
                                 alg_index) array_t *array;
int n_entries;
char **entries;
int *alg_index;
{
...
void fanout_opt_print_fanout_param(array, index) array_t *array;
int index;
{
  int i;
  alg_property_t *property;
...
void fanout_optimization(network, globals) network_t *network;
...
static void fanout_opt_init(network) network_t *network;
{
  int i;
  int j;
  fanout_alg_t *alg;
...
/* ARGSUSED */
static void fanout_opt_end(network) network_t *network;
{ /* nothing */ }

static void do_fanout_opt(node_x) node_t *node_x;
{
#ifdef SIS
...
static int fanout_map_optimal(node_x) node_t *node_x;
{
  int p, assign_x;
  int n_fanouts;
  node_t *node_y;
...
static void fanout_single_source_optimal(source, fanout_info, source_polarity,
                                         fanout_sol) node_t *source;
...
static int extract_fanout_problem(root, sources, fanout_info) node_t *root;
node_t **sources;
opt_array_t *fanout_info;
{
  int p, q;
...
static st_table *extract_links(source, exception) node_t *source;
node_t *exception;
{
...
void fanout_info_preprocess(fanout_info) opt_array_t *fanout_info;
{
  int n_elts;
  int i = 0;
  gate_link_t *key;
...
static node_t *keep_external_source_only(node) node_t *node;
{
  if (node == NIL(node_t))
    return node;
...
static int compare_gate_links(obj1, obj2) char *obj1, *obj2;
{
...
void fanout_info_free(fanout_info, free_link_flag) opt_array_t *fanout_info;
int free_link_flag;
{
  gate_link_t *key;
  char *dummy_value = NIL(char);
...
int fanout_opt_is_better_cost(cost1, cost2) fanout_cost_t *cost1;
fanout_cost_t *cost2;
{
...
static fanout_cost_t fanout_opt_add_cost(cost1, cost2) fanout_cost_t *cost1;
fanout_cost_t *cost2;
{
...
static void compute_required_times(node) node_t *node;
{
#ifdef SIS
  /* skip latches */
...
static void do_global_area_recover(node) node_t *node;
{
  gate_info_t *info;
...
static void resize_node(node, info) node_t *node;
gate_info_t *info;
{
  int i;
  int best_index;
...
static delay_time_t get_resized_node_arrival_time(node, new_gate) node_t *node;
lib_gate_t *new_gate;
{
  int i;
...
static delay_time_t get_reloaded_node_arrival_time(node,
                                                   load_diff) node_t *node;
double load_diff;
{
  double load, load_limit;
...
static void replace_node_gate(node, new_gate) node_t *node;
lib_gate_t *new_gate;
{
  int i;
  node_t *input;
...
static delay_time_t recompute_map_arrival(node, load) node_t *node;
double load;
{
  int i;
...
static void recompute_arrival_times(node) node_t *node;
{
  int i;
  delay_time_t *arrival_times;
  delay_time_t **arrival_times_p;
...
static void virtual_net_for_all_nodes_outputs_first(network,
                                                    fn) network_t *network;
VoidFn fn;
{
  lsGen gen;
  node_t *pi;
...
static void outputs_first_rec(node, fn, visited) node_t *node;
VoidFn fn;
st_table *visited;
{
  gate_link_t link;
...
static void virtual_net_for_all_nodes_inputs_first(network,
                                                   fn) network_t *network;
VoidFn fn;
{
  lsGen gen;
...
static void inputs_first_rec(node, fn, visited) node_t *node;
VoidFn fn;
st_table *visited;
{
  int i;
