 ...
 * added code to perform gate resizing for all gates. Works from outputs to inputs.
 * 
 */
    ...
    switch (node_function(node_x)) {
        ...
        case NODE_1:
            if (!global.no_warning)
                (void) fprintf(siserr, "WARNING: constants should have been removed earlier\n");
            break;
        case NODE_PO:
            /* already processed */
            break;
 ...
 */
        ...
        foreach_polarity(assign_x) {
            ...
            fanout_single_source_optimal(sources[POLAR_INV(assign_x)], fanout_info_arg, POLAR_INV(assign_x),
                                         assign_sol);
        ...
        node_t *exception;
{
    ...
    do {
        ...
        *link_ptr = link;
        st_insert(result, (char *) link_ptr, NIL(
        char));
        ...
        node_t *node;
{
    ...
    MAP(node)->map_arrival = delay_map_simulate(MAP(node)->ninputs, arrival_times_p, MAP(node)->gate->delay_info,
                                                MAP(node)->load);
        ...
        st_table *visited;
{
    ...
    st_insert(visited, (char *) node, NIL(
    char));
