...
void delay_map_compute_required_times(nin, model, node_required, load,
                                      required) int nin;
char **model;
...
static void map_set_current_network(network) network_t *network;
{
...
void map_alloc_delay_info(network, options) network_t *network;
...
void pipo_set_default_po_required(default_value) delay_time_t default_value;
{
...
delay_time_t pipo_get_pi_arrival(node) node_t *node;
{
  delay_time_t arrival;
...
delay_time_t pipo_get_pi_drive(node) node_t *node;
{
  delay_time_t drive;
...
double pipo_get_pi_load_limit(node) node_t *node;
{
  double load_limit;
...
delay_time_t pipo_get_po_required(node) node_t *node;
{
  delay_time_t required;
...
double pipo_get_po_load(node) node_t *node;
{
  double load;
...
pin_phase_t delay_get_polarity(pin_delay) char *pin_delay;
{ return ((delay_pin_t *)pin_delay)->phase; }

double map_compute_wire_load(n_fanouts) int n_fanouts;
...
double map_compute_fanout_load_correction(n_fanouts) int n_fanouts;
{
  assert(n_fanouts > 0);
...
static double compute_fanout_load_correction(n_fanouts) int n_fanouts;
{
  int buffer_index;
  double buffer_load;
  double total_load;
