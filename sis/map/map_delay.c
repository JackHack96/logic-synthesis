    ...
    for (i          = nin - 1; i >= 0; i--) {
        ...
        switch (pin_delay->phase) {
            ...
            case PHASE_NONINVERTING:required[i].rise = node_required.rise - delay.rise;
                ...
                break;
            case PHASE_NEITHER:required[i].rise = MIN(node_required.rise - delay.rise, node_required.fall - delay.fall);
        ...
        int n_fanouts;
{
    ...
    switch (global.load_estimation) {
        ...
        case 2:total_load = n_fanouts * buffer_load;
            ...
            break;
        default:(void) fprintf(siserr, "unrecognized value of global.load_estimation: %d\n", global.load_estimation);
            fail("");
            break;
    }
    return total_load;
