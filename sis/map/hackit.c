...
static lib_gate_t *choose_largest_gate(library_t *library, char *string) {
    network_t   *network;
    lib_class_t *class;
...
static node_t *create_inverter(lsList nodelist, node_t *fanin, lib_gate_t *inv_gate) {
    node_t *inv;
    char   *formals[1];
    node_t *actuals[1];
...
static void add_inverters(lsList nodelist, node_t *node, lib_gate_t *inv_gate) {
    int    new_inv;
...
void buffer_inputs(network_t *network, library_t *library) {
    lib_gate_t *inv_gate;
    lsList     nodelist;
    lsGen      gen;
    node_t     *pi, *node;
