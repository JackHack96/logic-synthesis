...
void gate_link_init(node) node_t *node;
{
...
double gate_link_compute_load(node) node_t *node;
{
  double load = 0.0;
  char *key, *value;
  st_generator *gen;
...
delay_time_t gate_link_compute_min_required(node) node_t *node;
{
...
int gate_link_n_elts(node) node_t *node;
{
  register st_table *table = MAP(node)->gate_link;
  return st_count(table);
}

int gate_link_is_empty(node) node_t *node;
...
int gate_link_first(node, link) node_t *node;
gate_link_t *link;
{
  register st_table *table = MAP(node)->gate_link;
...
int gate_link_next(node, link) node_t *node;
gate_link_t *link;
{
  char *key, *value;
...
void gate_link_put(node, link) node_t *node;
gate_link_t *link;
{
  gate_link_key_t key;
  gate_link_value_t *value;
...
int gate_link_get(node, link) node_t *node;
gate_link_t *link;
{
  gate_link_key_t key;
  char *value;
...
void gate_link_remove(node, link) node_t *node;
gate_link_t *link;
{
  gate_link_key_t key;
  char *key_ptr;
...
void gate_link_delete_all(node) node_t *node;
{
  gate_link_free(node);
  gate_link_init(node);
}
...
void gate_link_free(node) node_t *node;
{
  char *key, *value;
  st_generator *gen;
  register st_table *table = MAP(node)->gate_link;
...
static int gate_link_key_cmp(key1, key2) char *key1, *key2;
{
  register int i1, i2;
...
static int gate_link_key_hash(key, modulus) char *key;
int modulus;
{
...
void fanout_print_node(node) node_t *node;
{
  if (node == NIL(node_t))
    return;
...
void fanout_print_network(network) network_t *network;
{
  lsGen gen;
  node_t *node;
