...
**
**  01 10-June-91 klk	Changed the function fanout_print_node to global so that
*it can be called from outside *			for debugging purposes.
**/
#include "gate_link.h"
#include "map_delay.h"
#include "map_int.h"
...
void gate_link_init(node_t *node) {
...
double gate_link_compute_load(node_t *node) {
    double            load      = 0.0;
    char              *key, *value;
    st_generator      *gen;
...
delay_time_t gate_link_compute_min_required(node_t *node) {
...
int gate_link_n_elts(node_t *node) {
    register st_table *table = MAP(node)->gate_link;
    return st_count(table);
}

int gate_link_is_empty(node_t *node) { return (gate_link_n_elts(node) == 0); }
...
int gate_link_first(node_t *node, gate_link_t *link) {
    register st_table *table = MAP(node)->gate_link;
    MAP(node)->gen           = st_init_gen(table);
    return gate_link_next(node, link);
}

int gate_link_next(node_t *node, gate_link_t *link) {
    char         *key, *value;
...
void gate_link_put(node_t *node, gate_link_t *link) {
    gate_link_key_t   key;
    gate_link_value_t *value;
    register st_table *table = MAP(node)->gate_link;
...
int gate_link_get(node_t *node, gate_link_t *link) {
    gate_link_key_t   key;
    char              *value;
    register st_table *table = MAP(node)->gate_link;
...
void gate_link_remove(node_t *node, gate_link_t *link) {
    gate_link_key_t   key;
    char              *key_ptr;
    char              *value_ptr;
    register st_table *table = MAP(node)->gate_link;
...
void gate_link_delete_all(node_t *node) {
    gate_link_free(node);
    gate_link_init(node);
}
...
void gate_link_free(node_t *node) {
    char              *key, *value;
    st_generator      *gen;
    register st_table *table = MAP(node)->gate_link;
    st_foreach_item(table, gen, &key, &value) {
...
static int gate_link_key_cmp(char *key1, char *key2) {
    register int             i1, i2;
...
static int gate_link_key_hash(char *key, int modulus) {
...
void fanout_print_node(node_t *node) {
    if (node == NIL(node_t))
        return;
    fprintf(sisout, "node: %s(0x%x)\n", node->name, node);
    fprintf(sisout, "l(%2.2f) ", MAP(node)->load);
    fprintf(sisout, "a(%2.2f,%2.2f) ", MAP(node)->map_arrival.rise,
            MAP(node)->map_arrival.fall);
    fprintf(sisout, "r(%2.2f,%2.2f)\n", MAP(node)->required.rise,
            MAP(node)->required.fall);
    if (node->type == PRIMARY_INPUT) {
        ...
        for (i = 0; i < MAP(node)->ninputs; i++)
            fprintf(sisout, "%s(0x%x) ", MAP(node)->save_binding[i]->name,
...
void fanout_print_network(network_t *network) {
    lsGen  gen;
    node_t *node;

    foreach_node(network, gen, node) { fanout_print_node(node); }
}
