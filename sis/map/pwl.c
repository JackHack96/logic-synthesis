...
pwl_t *pwl_min(pwl_t *f1, pwl_t *f2) { return pwl_compute_min(f1, f2); }

pwl_t *pwl_max(pwl_t *f1, pwl_t *f2) {
    pwl_t *f;
    change_sign(f1);
    change_sign(f2);
...
pwl_t *pwl_sum(pwl_t *f1, pwl_t *f2) { return pwl_compute_sum(f1, f2); }

/* only positive shifts are allowed */

pwl_t *pwl_shift(pwl_t *f, double shift) {
    if (f->n_points == 0 || shift == 0.0)
        return pwl_dup(f);
    assert(shift > 0.0);
...
pwl_t *pwl_create_linear_max(int n_points, pwl_point_t *points) { return create_linear_max(n_points, points); }
...
pwl_t *pwl_create(array_t *points) {
    int         i;
    pwl_point_t *point;
    pwl_t       *result;

    if (points == NIL(array_t))
        return pwl_create_empty();
...
pwl_t *pwl_extract(array_t *points) {
    int         i;
    pwl_point_t *point;
    pwl_t       *result;
...
void pwl_print(FILE *fp, pwl_t *f, VoidFn print_data) {
    int i;
...
void pwl_set_data(pwl_t *pwl, char *data) {
    int i;
...
char *pwl_select(pwl_t *f, double x) {
...
pwl_point_t *pwl_lookup(pwl_t *f, double x) { return pwl_binary_search(f, x); }

double pwl_point_eval(pwl_point_t *point, double x) { return point->y + point->slope * (x - point->x); }

char *pwl_point_data(pwl_point_t *point) { return point->data; }

/* INTERNAL INTERFACE */

static pwl_point_t *pwl_binary_search(pwl_t *f, double value) {
    int    min, max, middle;
    double min_value;
    double max_value;
    double middle_value;
...
static pwl_t *pwl_compute_min(pwl_t *f1, pwl_t *f2) {
    int         intersect_flag;
    pwl_t       *result;
    int         indices[2];
    pwl_t       *fns[2];
    ...
    do {
        ...
        if (point.data == best_point->data && point.slope == best_point->slope)
            continue;
        point.x     = x;
...
static void first_point(pwl_t **fns, int *indices, int *select_p, double *x_p, int *intersect_flag_p) {
    double diff;
...
static int next_point(pwl_t **fns, int *indices, int *select_p, double *x_p, int *intersect_flag_p) {
    int         i;
    double      diff;
    double      min;
    double      mins[2];
    ...
    /* either the end of current interval or the intersection of the two current
     * segments */
    /* if previous one was intersection, can't be intersection again: avoid
     * recomputing */
    min    = INFINITY;
    for (i = 0; i < 2; i++) {
        mins[i] = (fns[i]->n_points - 1 == indices[i])
                  ? INFINITY
                  : fns[i]->points[indices[i] + 1].x;
        min = MIN(min, mins[i]);
    }
    if (*intersect_flag_p) {
        intersect = -INFINITY;
    } else {
        intersect = compute_intersect(&(fns[0]->points[indices[0]]),
                                      &(fns[1]->points[indices[1]]));
    }
    ...
    diff = (seg[0]->y + seg[0]->slope * (*x_p - seg[0]->x)) -
           (seg[1]->y + seg[1]->slope * (*x_p - seg[1]->x));
    if (diff == 0) {
...
static pwl_t *pwl_compute_sum(pwl_t *f1, pwl_t *f2) {
    int         i;
    int         select;
    double      diff;
    int         indices[2];
    ...
    for (;;) {
        if (indices[0] == fns[0]->n_points && indices[1] == fns[1]->n_points)
            break;
        for (i = 0; i < 2; i++) {
        ...
        select     = (diff <= 0) ? 0 : 1;
        if (indices[select] == fns[select]->n_points)
            select = 1 - select;
...
static void pwl_point_print(FILE *fp, pwl_point_t *point, VoidFn print_data) {
    (void) fprintf(fp, "%2.2f %2.2f %2.2f ", point->x, point->y, point->slope);
    (*print_data)(fp, point->data);
    (void) fprintf(fp, "\n");
}

static int pwl_point_cmp(char *obj1, char *obj2) {
    double      diff;
...
static double compute_intersect(pwl_point_t *p1, pwl_point_t *p2) {
    if (p1->slope == p2->slope)
        return (double) -INFINITY;
    return (((p2->y - p2->slope * p2->x) - (p1->y - p1->slope * p1->x)) /
            (p1->slope - p2->slope));
}

/* exact in the interval 0 <= x <= INFINITY */

static pwl_t *create_linear_max(int n_lines, pwl_point_t *origins) {
    double      x, next_x;
    double      max_slope;
    double      intersect;
    int         i;
    ...
    for (;;) {
        ...
        /* if floating point were exact, there would be no need to check for
         * validity */
        /* but the intersection point may be interestimated by the FP hardware */
        /* in that case we need to make sure that we do not select a line already
         * selected */
        ...
        /* if reached the largest slope, nothing will ever intersect it in the
         * future */
        if (point.slope == max_slope)
            break;
...
static double compute_max_slope(int n_lines, pwl_point_t *points) {
    int    i;
...
static int compute_argmax_y(double x, int n_lines, pwl_point_t *points, int *valid) {
    int    i;
    double y, tmp_y;
    int    argmax_y;
    double slope;
...
static void change_sign(pwl_t *f) {
...
static pwl_t *compute_pwl_shift(pwl_t *f, double shift) {
    pwl_t       *result;
    pwl_point_t point;
