...
pwl_t *pwl_min(f1, f2) pwl_t *f1;
pwl_t *f2;
{ return pwl_compute_min(f1, f2); }

pwl_t *pwl_max(f1, f2) pwl_t *f1;
pwl_t *f2;
{
  pwl_t *f;
  change_sign(f1);
...
pwl_t *pwl_create_linear_max(n_points, points) int n_points;
pwl_point_t *points;
...
pwl_t *pwl_create(points) array_t *points;
{
  int i;
  pwl_point_t *point;
  pwl_t *result;
...
pwl_t *pwl_extract(points) array_t *points;
{
  int i;
  pwl_point_t *point;
  pwl_t *result;
...
void pwl_print(fp, f, print_data) FILE *fp;
pwl_t *f;
VoidFn print_data;
{
  int i;
...
void pwl_set_data(pwl, data) pwl_t *pwl;
char *data;
{
  int i;
...
pwl_point_t *pwl_lookup(f, x) pwl_t *f;
double x;
{ return pwl_binary_search(f, x); }

double pwl_point_eval(point, x) pwl_point_t *point;
double x;
{ return point->y + point->slope * (x - point->x); }

char *pwl_point_data(point) pwl_point_t *point;
{ return point->data; }

/* INTERNAL INTERFACE */

static pwl_point_t *pwl_binary_search(f, value) pwl_t *f;
double value;
{
  int min, max, middle;
  double min_value;
...
static pwl_t *pwl_compute_min(f1, f2) pwl_t *f1;
pwl_t *f2;
{
  int intersect_flag;
  pwl_t *result;
...
static void first_point(fns, indices, select_p, x_p,
                        intersect_flag_p) pwl_t **fns;
int *indices;
int *select_p;
double *x_p;
...
static int next_point(fns, indices, select_p, x_p,
                      intersect_flag_p) pwl_t **fns;
int *indices;
int *select_p;
double *x_p;
...
static pwl_t *pwl_compute_sum(f1, f2) pwl_t *f1;
pwl_t *f2;
{
  int i;
  int select;
...
static void pwl_point_print(fp, point, print_data) FILE *fp;
pwl_point_t *point;
VoidFn print_data;
{
...
static int pwl_point_cmp(obj1, obj2) char *obj1;
char *obj2;
{
  double diff;
...
static double compute_intersect(p1, p2) pwl_point_t *p1;
pwl_point_t *p2;
{
...
static pwl_t *create_linear_max(n_lines, origins) int n_lines;
pwl_point_t *origins;
{
  double x, next_x;
  double max_slope;
...
static double compute_max_slope(n_lines, points) int n_lines;
pwl_point_t *points;
{
  int i;
...
static int compute_argmax_y(x, n_lines, points, valid) double x;
int n_lines;
pwl_point_t *points;
int *valid;
{
...
static void change_sign(f) pwl_t *f;
{
...
static pwl_t *compute_pwl_shift(f, shift) pwl_t *f;
double shift;
{
  pwl_t *result;
  pwl_point_t point;
