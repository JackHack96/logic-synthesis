...
static void best_one_source(table, two_level_table, fanout_info, source_index, source_polarity, sink_index,
        ...
        int n_gaps;
{
    ...
    two_level_t  *two_level_entry = INDEX4P(two_level_t, two_level_table, source_index, source_polarity, sink_index,
                                            sink_polarity);
    lt_tree_t    *entry           = INDEX5P(lt_tree_t, table, source_index, source_polarity, sink_index, sink_polarity,
                                            n_gaps);
    ...
    for (sink = sink_index; sink <= fanout_info->n_elts; sink++) {
        ...
        foreach_buffer(n_gates, buffer_index) {
            ...
            if (sink < fanout_info->n_elts) {
                buffer_entry   = INDEX5P(lt_tree_t, table, buffer_index, buffer_polarity, sink, sink_polarity,
                                         buffer_gaps);
        ...
        foreach_polarity(sink_polarity) {
            foreach_polarity(source_polarity) {
                ...
                if (fanout_info[sink_polarity].n_elts == 0) {
                    ...
                    if (from->balanced) {
                        to->load      =
                                fanout_delay_get_buffer_load(from->two_level->gate_index) * from->two_level->n_gates;
        ...
        int n_gaps;
{
    ...
    switch (entry->balanced) {
        case 0:noalg_insert_sinks(tree, fanout_info, sink_index, entry->sink_index);
            ...
            buffer_polarity = (is_inverter(n_gates, buffer_index)) ? POLAR_INV(source_polarity) : source_polarity;
            buffer_gaps     = (entry->sink_index == sink_index && buffer_polarity == sink_polarity) ? n_gaps - 1
                                                                                                    : n_gaps;
            if (entry->sink_index == fanout_info->n_elts) return;
            sink_index = entry->sink_index;
            n_fanouts  = get_n_fanouts(fanout_info, table, buffer_index, buffer_polarity, sink_index, sink_polarity,
                                       buffer_gaps);
            fanout_tree_insert_gate(tree, buffer_index, n_fanouts);
            insert_lt_tree(tree, table, fanout_info, buffer_index, buffer_polarity, sink_index, sink_polarity,
                           buffer_gaps);
            break;
        case 1:insert_two_level_tree(tree, fanout_info, sink_index, source_index, entry->two_level);
            break;
