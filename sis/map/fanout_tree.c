...
enum fanout_tree_enum {
    SINK_NODE, BUFFER_NODE, SOURCE_NODE
};
...
array_t *fanout_tree_alloc() {
    return array_alloc(fanout_node_t, 0);
}

/* if flag free_links_p is 1, should also free the gate_links */

void fanout_tree_free(array, free_links_p)
        array_t *array;
        int free_links_p;
{
    int           i;
...
/* a value of -1 for the fanout_index would indicate something was not computed properly */
/* Always initialized to -1 (default value) to catch that kind of problems */

void fanout_tree_save_links(array)
        array_t *array;
{
    int           i;
    gate_link_t   *link;
    ...
    for (i = 0; i < array_n(array); i++) {
        ...
        if (node->type == SINK_NODE) {
            ...
            *(node->link) = *link;
            if (link->node == NIL(node_t)) continue; /* was a dummy tree to start with. Nothing to do */
...
void fanout_tree_add_edges(array)
        array_t *array;
{
    int           i, j, next_i;
    fanout_node_t *node;
...
void fanout_tree_check(array, cost, fanout_info)
        array_t *array;
        fanout_cost_t *cost;
...
void fanout_tree_peephole_optimize(array, cost, fanout_info, debug)
        array_t *array;
        fanout_cost_t *cost;
        opt_array_t *fanout_info;
        int debug;
...
void fanout_tree_insert_sink(array, link)
        array_t *array;
        gate_link_t *link;
{
    fanout_node_t node;
...
void fanout_tree_insert_gate(array, gate_index, arity)
        array_t *array;
        int gate_index;
        int arity;
{
...
static int add_edges_rec(array, i)
        array_t *array;
        int i;
{
    int           next_i, j;
    ...
    switch (node->type) {
        ...
        case BUFFER_NODE:
            ...
            break;
        default:fail("unexpected node type in do_build_rooted_tree");
            break;
    }
    assert(node->arity == array_n(node->children));
    return next_i;
...
static array_t *fanout_tree_get_roots(array)
        array_t *array;
{
    int           i;
    fanout_node_t *node;
...
static void do_build_tree(trees, array)
        array_t *trees;
        array_t *array;
{
    int           i;
...
static void do_build_tree_rec(source, array)
        fanout_node_t *source;
        array_t *array;
{
    int           i;
    ...
    for (i = 0; i < array_n(source->children); i++) {
        ...
        switch (fanout_node->type) {
            case SINK_NODE:virtual_network_add_to_gate_link(source->node, fanout_node->link);
                break;
            case BUFFER_NODE:fanout_node->node = create_buffer(source->node, fanout_node->gate_index);
...
static node_t *create_buffer(source, gate_index)
        node_t *source;
        int gate_index;
{
...
static void check_polarities(trees, fanout_info)
        array_t *trees;
        opt_array_t *fanout_info;
{
    int           i, p;
...
static void check_polarity_rec(source, source_polarity, sink_table)
        fanout_node_t *source;
        int source_polarity;
        st_table *sink_table;
{
    ...
    for (i = 0; i < array_n(source->children); i++) {
        fanout_node = array_fetch(fanout_node_t * , source->children, i);
        switch (fanout_node->type) {
            case SINK_NODE:assert(st_lookup_int(sink_table, (char *) fanout_node->link, &sink_polarity));
                assert(sink_polarity == source_polarity);
                st_delete(sink_table, (char **) &fanout_node->link, &dummy);
                break;
            case BUFFER_NODE:
                buffer_polarity = (fanout_delay_get_buffer_polarity(fanout_node->gate_index) == POLAR_X)
                                  ? source_polarity : POLAR_INV(source_polarity);
                check_polarity_rec(fanout_node, buffer_polarity, sink_table);
                break;
            default:fail("unexpected node type in check_polarity_rec");
                break;
        }
    }
}

static void check_required(trees, ref_required)
        array_t *trees;
...
delay_time_t
map_compute_fanout_tree_req_time(tree)
        array_t *tree;
{
    fanout_node_t *source;
    delay_time_t  required;
...
static delay_time_t check_required_rec(source)
        fanout_node_t *source;
{
    int           i;
    ...
    for (i    = 0; i < array_n(source->children); i++) {
        fanout_node = array_fetch(fanout_node_t * , source->children, i);
        switch (fanout_node->type) {
            case SINK_NODE:local_required = fanout_node->required = fanout_node->link->required;
                local_load                = fanout_node->link->load;
                break;
            case BUFFER_NODE:fanout_node->required = check_required_rec(fanout_node);
                local_required = fanout_delay_backward_intrinsic(fanout_node->required, fanout_node->gate_index);
...
#endif
    return fanout_delay_backward_load_dependent(required, source->gate_index, load);
}

static void check_area(trees, ref_area)
        array_t *trees;
        double ref_area;
{
    int           i;
...
void print_fanout_tree(trees, array)
        array_t *trees;
        array_t *array;
{
    int           i;
    ...
    for (i = 0; i < array_n(trees); i++) {
        ...
        source_polarity = fanout_delay_get_source_polarity(source->gate_index);
        source->node = fanout_delay_get_source_node(source->gate_index);
        (void) fprintf(sisout, "source(%s,p(%d),", source->node->name, source_polarity);
#ifdef DEBUG
        (void) fprintf(sisout, "r(%2.3f,%2.3f)", source->save_required.rise, source->save_required.fall);
#endif
        (void) fprintf(sisout, "s(%2.3f,%2.3f))\n", source->required.rise, source->required.fall);
        print_tree_rec(source, source_polarity, array, 4);
    }
}

static void print_tree_rec(source, source_polarity, array, n_tabs)
        fanout_node_t *source;
        int source_polarity;
        array_t *array;
        int n_tabs;
{
    ...
    for (i = 0; i < array_n(source->children); i++) {
        fanout_node = array_fetch(fanout_node_t * , source->children, i);
        switch (fanout_node->type) {
            case SINK_NODE:print_tabs(n_tabs);
                if (fanout_node->link->node == NIL(node_t)) {
                    (void) fprintf(sisout, "sink(-%d-,p(%d),", fanout_node->link->pin, source_polarity);
                } else {
                    (void) fprintf(sisout, "sink(%s,p(%d),", fanout_node->link->node->name, source_polarity);
                }
                (void) fprintf(sisout, "l(%2.3f),", fanout_node->link->load);
                (void) fprintf(sisout, "r(%2.3f,%2.3f))\n", fanout_node->required.rise, fanout_node->required.fall);
                break;
            case BUFFER_NODE:print_tabs(n_tabs);
                gate            = fanout_delay_get_gate(fanout_node->gate_index);
                buffer_polarity = (fanout_delay_get_buffer_polarity(fanout_node->gate_index) == POLAR_X)
                                  ? source_polarity : POLAR_INV(source_polarity);
                (void) fprintf(sisout, "buffer(%s,p(%d),", gate->name, buffer_polarity);
                (void) fprintf(sisout, "l(%2.3f),", fanout_delay_get_buffer_load(fanout_node->gate_index));
                required = fanout_delay_backward_intrinsic(fanout_node->required, fanout_node->gate_index);
...
static void free_unused_sources(trees)
        array_t *trees;
{
    int           i, source_index;
    node_t        *node;
    ...
    for (i = 0; i < array_n(trees); i++) {
        source = array_fetch(fanout_node_t * , trees, i);
        source->node = fanout_delay_get_source_node(source->gate_index);
        st_insert(used_sources, (char *) source->node, NIL(
        char));
    }
...
static array_t *get_forest_in_bottom_up_order(trees)
        array_t *trees;
{
    int           i;
    fanout_node_t *source;
...
static void get_forest_in_bottom_up_order_rec(source, nodes)
        fanout_node_t *source;
        array_t *nodes;
{
    int           i;
    ...
    for (i = 0; i < array_n(source->children); i++) {
        fanout_node = array_fetch(fanout_node_t * , source->children, i);
        switch (fanout_node->type) {
            case SINK_NODE:array_insert_last(fanout_node_t * , nodes, fanout_node);
                break;
            case BUFFER_NODE:get_forest_in_bottom_up_order_rec(fanout_node, nodes);
...
struct peep_entry_struct {
    delay_time_t required;              /* before intrinsic, after load dependent (except for sinks) */
    delay_time_t arrival;                  /* after intrinsic, before load dependent (except for sinks) */
    double       load;
    double       area;
    int          buffer_index;
...
static int peephole_optimize(trees, nodes, cost)
        array_t *trees;
        array_t *nodes;
        fanout_cost_t *cost;
{
    ...
    for (i = 0; i < array_n(nodes); i++) {
        node = array_fetch(fanout_node_t * , nodes, i);
        if (node->type == SINK_NODE) continue;
...
static void peephole_optimize_node(node, entry)
        fanout_node_t *node;
        peep_entry_t *entry;
{
    int           i;
    ...
    current_incr = ALLOC(
    int, array_n(node->children));
    for (i = 0; i < array_n(node->children); i++)
        current_incr[i] = 0;

    for (;;) {
        if (!incr_critical_child(node, current_incr, &critical_child)) break;
...
static int incr_critical_child(node, current_incr, critical_child_index)
        fanout_node_t *node;
        int *current_incr;
    ...
    *critical_child_index = -1;
    ...
    for (i   = 0; i < array_n(node->children); i++) {
        ...
        if (child_entry->buffer_index != -1)
            local_required = fanout_delay_backward_intrinsic(local_required, child_entry->buffer_index);
    ...
    critical_child = array_fetch(fanout_node_t * , node->children, *critical_child_index);
    return (current_incr[*critical_child_index] < array_n(critical_child->peephole) - 1);
}
        ...
        fanout_cost_t *cost;         /* where the results are put: area, required */
{
    ...
    cost->slack = fanout_delay_backward_load_dependent(required, source_index, load);
    cost->area  = area + fanout_delay_get_area(source_index);
}

static void select_best_log_entry(node, log, entry)
        fanout_node_t *node;
        array_t *log;
        peep_entry_t *entry;
{
    ...
    for (i = 0; i < array_n(node->children); i++)
        array_insert(
    int, entry->children_incr, i, incr[i]);
...
static void build_peephole_trees(trees)
        array_t *trees;
{
    int           i;
    fanout_node_t *source;
...
static void build_peephole_tree_rec(source, entry)
        fanout_node_t *source;
        peep_entry_t *entry;
{
    int           i;
    ...
    for (i = 0; i < array_n(source->children); i++) {
        ...
        switch (fanout_node->type) {
            case SINK_NODE:break;
            case BUFFER_NODE:
                child_incr = array_fetch(
                int, entry->children_incr, i);
                child_entry = array_fetch_p(peep_entry_t, fanout_node->peephole, child_incr);
...
static int peephole_is_better(trees, cost, peephole_cost)
        array_t *trees;
        fanout_cost_t *cost;
        fanout_cost_t *peephole_cost;
{
    ...
    if (peephole_global_debug == -1 || (peephole_global_debug && !peephole_is_better_p)) {
        (void) fprintf(sisout, "before peephole: a(%2.2f) s(%2.4f %2.4f)\t", cost->area, cost->slack.rise,
                       cost->slack.fall);
        (void) fprintf(sisout, "after peephole: a(%2.2f) s(%2.4f %2.4f)\n", peephole_cost->area,
                       peephole_cost->slack.rise, peephole_cost->slack.fall);
    }
...
static void fanout_tree_alloc_peephole(trees)
        array_t *trees;
{
    int           i;
    fanout_node_t *source;
...
static void alloc_peephole_rec(source)
        fanout_node_t *source;
{
    int           i;
    fanout_node_t *fanout_node;
    ...
    for (i = 0; i < array_n(source->children); i++) {
        fanout_node = array_fetch(fanout_node_t * , source->children, i);
        switch (fanout_node->type) {
            case SINK_NODE:peephole_init_sink(fanout_node);
                break;
            case BUFFER_NODE:peephole_init_buffer(fanout_node);
                alloc_peephole_rec(fanout_node);
                break;
            default:fail("unexpected node type in alloc_peephole_rec");
                break;
        }
    }
}

static void peephole_init_sink(node)
        fanout_node_t *node;
{
    peep_entry_t entry;
    ...
    entry.children_incr = array_alloc(
    int, 0);
    array_insert(peep_entry_t, node->peephole, 0, entry);
}

static void peephole_init_source(node)
        fanout_node_t *node;
{
    peep_entry_t entry;
...
static void peephole_init_buffer(node)
        fanout_node_t *node;
{
    int          i;
    int          from, to;
...
static void fanout_free_peephole(nodes)
        array_t *nodes;
{
    int           i, j;
    fanout_node_t *node;
...
static void fanout_tree_compute_arrival_times(trees)
        array_t *trees;
{
    int           i;
    double        load;
...
static void compute_arrival_times_rec(source, source_load)
        fanout_node_t *source;
        double source_load;
{
    int           i, j;
    ...
    local_arrival = fanout_delay_forward_load_dependent(source->arrival, source->gate_index, source_load);
    for (i        = 0; i < array_n(source->children); i++) {
        fanout_node = array_fetch(fanout_node_t * , source->children, i);
        switch (fanout_node->type) {
            case SINK_NODE:entry = array_fetch_p(peep_entry_t, fanout_node->peephole, 0);
            ...
            case BUFFER_NODE:
                for (j               = 0; j < array_n(fanout_node->peephole); j++) {
                    ...
                    fanout_load    = source_load - fanout_delay_get_buffer_load(fanout_node->gate_index) + entry->load;
                    fanout_arrival = fanout_delay_forward_load_dependent(source->arrival, source->gate_index,
                                                                         fanout_load);
                    fanout_arrival = fanout_delay_forward_intrinsic(fanout_arrival, entry->buffer_index);
                    entry->arrival = fanout_arrival;
                }
                fanout_node->arrival = fanout_delay_forward_intrinsic(local_arrival, fanout_node->gate_index);
...
static double fanout_tree_get_node_load(node)
        fanout_node_t *node;
{
    int           i;
    double        load = 0.0;
    ...
    for (i = 0; i < array_n(node->children); i++) {
        ...
        switch (fanout_node->type) {
            case SINK_NODE:load += fanout_node->link->load;
                break;
            case BUFFER_NODE:load += fanout_delay_get_buffer_load(fanout_node->gate_index);
                break;
            default:fail("unexpected node type in alloc_peephole_rec");
                break;
        }
...
static void fanout_tree_remove_unnecessary_buffers(trees, cost)
        array_t *trees;
        fanout_cost_t *cost;
{
    int           i;
...
static void remove_unnecessary_buffers_rec(source)
        fanout_node_t *source;
{
    int           i;
    fanout_node_t *fanout_node;
    ...
    for (i = 0; i < array_n(source->children); i++) {
        fanout_node = array_fetch(fanout_node_t * , source->children, i);
        switch (fanout_node->type) {
            case SINK_NODE:fanout_node->polarity = source->polarity;
                remove_buffer_process_sink(fanout_node);
                break;
            case BUFFER_NODE:
                fanout_node->polarity = (fanout_delay_get_buffer_polarity(fanout_node->gate_index) == POLAR_X)
                                        ? source->polarity : POLAR_INV(source->polarity);
...
static void remove_buffer_process_sink(node)
        fanout_node_t *node;
{
    int          p;
    ...
    st_insert(entry[p].links, (char *) node, NIL(
    char));
...
static void remove_buffer_process_buffer(node)
        fanout_node_t *node;
{
    int was_flattened;
...
static int continue_flattening(node)
        fanout_node_t *node;
{
    int           i;
    fanout_node_t *child;
    ...
    for (i = 0; i < array_n(node->children); i++) {
        child = array_fetch(fanout_node_t * , node->children, i);
        if (child->remove_opt == NIL(array_t)) return 0;
    }
    return 1;
}
...
static void compute_flatten_info(node)
        fanout_node_t *node;
{
    int           i, p;
    fanout_node_t *child;
...
/* check whether by flattening all nodes under "node" we can obtain a better cost */
...
/* if both polarities are present, one inverter is introduced and optimally sized */

static int can_flatten_subtree(node)
        fanout_node_t *node;
{
    int           best_gate_index;
    delay_time_t  slack;
    ...
    if (already_flat_subtree(node)) return 1;
    compute_no_opt_cost(node, &current_cost);
    compute_best_new_cost(node, &cost, &best_gate_index);
    if (fanout_opt_is_better_cost(&current_cost, &cost)) return 0;
...
static void compute_no_opt_cost(node, cost)
        fanout_node_t *node;
        fanout_cost_t *cost;
{
...
static void compute_best_new_cost(node, cost, best_gate_index)
        fanout_node_t *node;
        fanout_cost_t *cost;
        int *best_gate_index;
{
    ...
    foreach_polarity(p) { entry[p] = array_fetch_p(remove_opt_t, node->remove_opt, p); }
    p = node->polarity;
    q = POLAR_INV(p);
    if (st_count(entry[q]->links) == 0) {
        ...
        required          = fanout_delay_backward_load_dependent(entry[p]->required, node->gate_index, load);
        ...
        *best_gate_index = -1;
        ...
        foreach_inverter(n_gates, gate_index) {
            ...
            local_required = fanout_delay_backward_load_dependent(entry[q]->required, gate_index, local_load);
            local_required = fanout_delay_backward_intrinsic(local_required, gate_index);
            ...
            local_required = fanout_delay_backward_load_dependent(local_required, node->gate_index, local_load);
            SETSUB(local_cost.slack, local_required, node->arrival);
            local_cost.area = fanout_delay_get_area(gate_index) + fanout_delay_get_area(node->gate_index);
...
static void update_children_entries(node, best_gate_index)
        fanout_node_t *node;
        int best_gate_index;
{
    int          p, q;
...
static fanout_node_t *get_first_buffer_child(node)
        fanout_node_t *node;
{
    int i;

    for (i = 0; i < array_n(node->children); i++) {
        fanout_node_t *local_child = array_fetch(fanout_node_t * , node->children, i);
        if (local_child->type == BUFFER_NODE) return local_child;
    }
    return NIL(fanout_node_t);
}

static int already_flat_subtree(node)
        fanout_node_t *node;
{
    int           i;
    ...
    if (n_buffer_children == 0) return 1;
    if (n_buffer_children > 1) return 0;
    node = last_buffer_child;
    if (!is_inverter(n_gates, node->gate_index)) return 0;
    n_buffer_children = 0;
    for (i            = 0; i < array_n(node->children); i++) {
        child = array_fetch(fanout_node_t * , node->children, i);
        if (child->type == BUFFER_NODE)
...
static void cleanup_remove_opt_children_entries(node)
        fanout_node_t *node;
{
    int           i;
    fanout_node_t *child;
...
static void cleanup_remove_opt_node(node)
        fanout_node_t *node;
{
    int p;
...
static void remove_buffer_process_source(node)
        fanout_node_t *node;
{
...
static void compute_buffer_node_cost(node)
        fanout_node_t *node;
{
    int           i;
    double        area = 0.0;
    ...
    for (i   = 0; i < array_n(node->children); i++) {
        ...
        if (child->type == BUFFER_NODE)
            local_required = fanout_delay_backward_intrinsic(local_required, child->gate_index);
        SETMIN(required, required, local_required);
    }
    load += map_compute_wire_load(array_n(node->children));
    node->required = fanout_delay_backward_load_dependent(required, node->gate_index, load);
    node->area     = area + fanout_delay_get_area(node->gate_index);
    node->load     = (is_buffer(n_gates, node->gate_index)) ? fanout_delay_get_buffer_load(node->gate_index) : 0.0;
}
...
static int can_merge_subtrees(node)
        fanout_node_t *node;
{
    int           i;
    ...
    for (current_index = 0; current_index < array_n(node_array); current_index++) {
        current_node = array_fetch_p(merge_t, node_array, current_index);
        if (current_node->ignored) continue;
        for (merge_index = current_index + 1; merge_index < array_n(node_array); merge_index++) {
            merge_node = array_fetch_p(merge_t, node_array, merge_index);
            if (merge_node->ignored) continue;
...
static delay_time_t recompute_arrival_time(node, node_array)
        fanout_node_t *node;
        array_t *node_array;
{
    int     i;
    ...
    for (i = 0; i < array_n(node_array); i++) {
        ...
        if (merge_node->ignored && !merge_node->sink) continue;
        load += merge_node->node->load;
        n_nodes++;
    }
    load += map_compute_wire_load(n_nodes);
    return fanout_delay_forward_load_dependent(node->arrival, node->gate_index, load);
}

static int can_merge_node(current_node, merge_node, arrival)
        fanout_node_t *current_node;
        ...
        delay_time_t arrival;
{
    ...
    if (current_node->polarity != merge_node->polarity) return 0;
    ...
    required = fanout_delay_backward_load_dependent(required, current_node->gate_index, load);
    SETSUB(slack, required, arrival);
    return (GETMIN(slack) >= 0.0);
}

static void add_children_cost(array, required, load)
        array_t *array;
        delay_time_t *required;
        double *load;
{
...
static void merge_nodes(current_node, merge_node)
        fanout_node_t *current_node;
        fanout_node_t *merge_node;
{
    int           i;
...
static void adjust_node_children(node, child_array)
        fanout_node_t *node;
        array_t *child_array;
{
    int     i;
    ...
    for (i = 0; i < array_n(child_array); i++) {
        ...
        if (child_entry->ignored && !child_entry->sink) continue;
...
void fanout_tree_extract_fanout_leaves(fanout, array)
...
static void extract_fanout_leaves_rec(fanout, source, source_load, source_polarity)
        multi_fanout_t *fanout;
        fanout_node_t *source;
        double source_load;
        int source_polarity;
{
    ...
    for (i        = 0; i < array_n(source->children); i++) {
        fanout_node = array_fetch(fanout_node_t * , source->children, i);
        switch (fanout_node->type) {
            case SINK_NODE:link = fanout_node->link;
                fanout_index    = link->pin;    /* overloaded: when tree saved, link->pin becomes fanout_index */
                bucket          = fanout_est_fanout_bucket_alloc(fanout);
                bucket->load                                         = source_load;
                bucket->pwl                                          = fanout_delay_get_delay_pwl(source->gate_index,
                                                                                                  source_load,
                                                                                                  source->arrival);
            ...
            case BUFFER_NODE:
                ...
                fanout_polarity = (gate_polarity == POLAR_X) ? source_polarity : POLAR_INV(source_polarity);
                extract_fanout_leaves_rec(fanout, fanout_node, fanout_load, fanout_polarity);
                break;
            default:fail("unexpected node type in extract_fanout_leaves_rec");
                break;
        }
    }
}
...
double fanout_tree_get_source_load(array)
        array_t *array;
{
    array_t       *trees;
    fanout_node_t *source;
...
array_t *fanout_tree_get_root_node(array)
        array_t *array;
{
