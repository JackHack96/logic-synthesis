...
void fanout_tree_free(array_t *array, int free_links_p) {
    int           i;
    fanout_node_t *node;
...
void fanout_tree_save_links(array_t *array) {
    int           i;
    gate_link_t   *link;
    fanout_node_t *node;
...
void fanout_tree_copy(array_t *to, array_t *from) {
    int           i;
    fanout_node_t *node;
...
void fanout_tree_add_edges(array_t *array) {
    int           i, j, next_i;
    fanout_node_t *node;
...
void fanout_tree_check(array_t *array, fanout_cost_t *cost, opt_array_t *fanout_info) {
...
void fanout_tree_peephole_optimize(array_t *array, fanout_cost_t *cost, opt_array_t *fanout_info, int debug) {
    array_t *nodes;
    array_t *trees = fanout_tree_get_roots(array);
...
void fanout_tree_build(array_t *array) {
    array_t *trees = fanout_tree_get_roots(array);
...
void fanout_tree_insert_sink(array_t *array, gate_link_t *link) {
    fanout_node_t node;
...
void fanout_tree_insert_gate(array_t *array, int gate_index, int arity) {
    fanout_node_t node;
    assert(arity > 0);
    node = DEFAULT_FANOUT_NODE;
...
static int add_edges_rec(array_t *array, int i) {
    int           next_i, j;
    fanout_node_t *node;
...
static array_t *fanout_tree_get_roots(array_t *array) {
    int           i;
    fanout_node_t *node;
...
static void do_build_tree(array_t *trees, array_t *array) {
    int           i;
...
static void do_build_tree_rec(fanout_node_t *source, array_t *array) {
    int           i;
...
static node_t *create_buffer(node_t *source, int gate_index) {
...
static void check_polarities(array_t *trees, opt_array_t *fanout_info) {
    int           i, p;
    gate_link_t   *link;
    fanout_node_t *source;
...
static void check_polarity_rec(fanout_node_t *source, int source_polarity, st_table *sink_table) {
    int           i;
    int           sink_polarity;
    int           buffer_polarity;
    fanout_node_t *fanout_node;
...
static void check_required(array_t *trees, delay_time_t ref_required) {
    int           i;
    fanout_node_t *source;
    delay_time_t  required;
...
delay_time_t map_compute_fanout_tree_req_time(array_t *tree) {
    fanout_node_t *source;
    delay_time_t  required;
...
static delay_time_t check_required_rec(fanout_node_t *source) {
    int           i;
...
static void check_area(array_t *trees, double ref_area) {
    int           i;
    array_t       *nodes;
    fanout_node_t *node;
    double        area = 0.0;
...
void print_fanout_tree(array_t *trees, array_t *array) {
    int           i;
    fanout_node_t *source;
...
static void print_tree_rec(fanout_node_t *source, int source_polarity, array_t *array, int n_tabs) {
    int           i;
    int           buffer_polarity;
    lib_gate_t    *gate;
    fanout_node_t *fanout_node;
...
static void print_tabs(int n_tabs) {
    int i;
...
static void free_unused_sources(array_t *trees) {
    int           i, source_index;
    node_t        *node;
    char          *dummy;
    fanout_node_t *source;
...
static array_t *get_forest_in_bottom_up_order(array_t *trees) {
    int           i;
    fanout_node_t *source;
...
static void get_forest_in_bottom_up_order_rec(fanout_node_t *source, array_t *nodes) {
    int           i;
    fanout_node_t *fanout_node;
...
static int peephole_optimize(array_t *trees, array_t *nodes, fanout_cost_t *cost) {
    int           i, j;
    fanout_node_t *node;
    peep_entry_t  *entry;
...
static void peephole_optimize_node(fanout_node_t *node, peep_entry_t *entry) {
    int           i;
    array_t       *log;
    int           critical_child;
    int           *current_incr;
...
static int incr_critical_child(fanout_node_t *node, int *current_incr, int *critical_child_index) {
    int           i;
    delay_time_t  required, local_required;
...
static void compute_current_cost(fanout_node_t *node, int source_index, int *current_incr, fanout_cost_t *cost) {
    int           i;
    delay_time_t  required, local_required;
    double        load, area;
    fanout_node_t *child;
...
static void select_best_log_entry(fanout_node_t *node, array_t *log, peep_entry_t *entry) {
    int           i;
    int           best_log_index;
...
static void build_peephole_trees(array_t *trees) {
    int           i;
    fanout_node_t *source;
    peep_entry_t  *entry;
...
static void build_peephole_tree_rec(fanout_node_t *source, peep_entry_t *entry) {
    int           i;
    int           child_incr;
    peep_entry_t  *child_entry;
    fanout_node_t *fanout_node;
    ...
    for (i = 0; i < array_n(source->children); i++) {
        ...
        switch (fanout_node->type) {
            case SINK_NODE:break;
            case BUFFER_NODE:child_incr = array_fetch(int, entry->children_incr, i);
                child_entry =
...
static int peephole_is_better(array_t *trees, fanout_cost_t *cost, fanout_cost_t *peephole_cost) {
    int           i;
    fanout_node_t *node;
    peep_entry_t  *entry;
...
static void fanout_tree_alloc_peephole(array_t *trees) {
    int           i;
    fanout_node_t *source;
...
static void alloc_peephole_rec(fanout_node_t *source) {
    int           i;
    fanout_node_t *fanout_node;
...
static void peephole_init_sink(fanout_node_t *node) {
    peep_entry_t entry;
...
static void peephole_init_source(fanout_node_t *node) {
    peep_entry_t entry;
...
static void peephole_init_buffer(fanout_node_t *node) {
    int          i;
    int          from, to;
    peep_entry_t entry;
...
static void fanout_free_peephole(array_t *nodes) {
    int           i, j;
    fanout_node_t *node;
...
static void fanout_tree_compute_arrival_times(array_t *trees) {
    int           i;
    double        load;
    fanout_node_t *source;
    peep_entry_t  *entry;
...
static void compute_arrival_times_rec(fanout_node_t *source, double source_load) {
    int           i, j;
    double        fanout_load;
...
static double fanout_tree_get_node_load(fanout_node_t *node) {
    int           i;
    double        load = 0.0;
    fanout_node_t *fanout_node;

    if (node->type != SOURCE_NODE && node->type != BUFFER_NODE) fail(
            "unexpected node type in fanout_tree_get_node_load");
...
static void fanout_tree_remove_unnecessary_buffers(array_t *trees, fanout_cost_t *cost) {
    int           i;
    fanout_node_t *source;
...
static void remove_unnecessary_buffers_rec(fanout_node_t *source) {
    int           i;
    fanout_node_t *fanout_node;
...
static void remove_buffer_process_sink(fanout_node_t *node) {
    int          p;
...
static void remove_buffer_process_buffer(fanout_node_t *node) {
    int was_flattened;
...
static int continue_flattening(fanout_node_t *node) {
    int           i;
    fanout_node_t *child;
...
static void compute_flatten_info(fanout_node_t *node) {
    int           i, p;
    fanout_node_t *child;
    remove_opt_t  entry[2];
...
static int can_flatten_subtree(fanout_node_t *node) {
    int           best_gate_index;
    delay_time_t  slack;
...
static void compute_no_opt_cost(fanout_node_t *node, fanout_cost_t *cost) {
    SETSUB(cost->slack, node->required, node->arrival);
    cost->area = node->area;
}

static void compute_best_new_cost(fanout_node_t *node, fanout_cost_t *cost, int *best_gate_index) {
    int          p, q;
    remove_opt_t *entry[2];
...
static void update_children_entries(fanout_node_t *node, int best_gate_index) {
    int          p, q;
    char         *key, *value;
    st_generator *gen;
    remove_opt_t *entry[2];
...
static fanout_node_t *get_first_buffer_child(fanout_node_t *node) {
    int i;
...
static int already_flat_subtree(fanout_node_t *node) {
    int           i;
...
static void cleanup_remove_opt_children_entries(fanout_node_t *node) {
    int           i;
    fanout_node_t *child;
...
static void cleanup_remove_opt_node(fanout_node_t *node) {
    int p;
...
static void remove_buffer_process_source(fanout_node_t *node) {
...
static void compute_buffer_node_cost(fanout_node_t *node) {
    int           i;
    double        area = 0.0;
    double        load = 0.0;
    delay_time_t  required, local_required;
...
static int can_merge_subtrees(fanout_node_t *node) {
    int           i;
...
static delay_time_t recompute_arrival_time(fanout_node_t *node, array_t *node_array) {
    int     i;
    merge_t *merge_node;
    double  load    = 0.0;
    int     n_nodes = 0;
...
static int can_merge_node(fanout_node_t *current_node, fanout_node_t *merge_node, delay_time_t arrival) {
    double       load;
...
static void add_children_cost(array_t *array, delay_time_t *required, double *load) {
    double        result_load;
...
static void merge_nodes(fanout_node_t *current_node, fanout_node_t *merge_node) {
    int           i;
    fanout_node_t *child;
...
static void adjust_node_children(fanout_node_t *node, array_t *child_array) {
    int     i;
    merge_t *child_entry;
...
void fanout_tree_extract_fanout_leaves(multi_fanout_t *fanout, array_t *array) {
    int           i;
    array_t       *trees;
    fanout_node_t *source;
    double        source_load;
...
static void
extract_fanout_leaves_rec(multi_fanout_t *fanout, fanout_node_t *source, double source_load, int source_polarity) {
    int             i;
    gate_link_t     *link;
    int             gate_polarity;
    int             fanout_polarity;
...
double fanout_tree_get_source_load(array_t *array) {
    array_t       *trees;
    fanout_node_t *source;
    double        source_load;
...
array_t *fanout_tree_get_root_node(array_t *array) { return (fanout_tree_get_roots(array)); }
