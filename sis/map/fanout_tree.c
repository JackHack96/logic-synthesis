...
st_table *sink_table;
{
  ...
  for (i = 0; i < array_n(source->children); i++) {
    ...
    switch (fanout_node->type) {
    case SINK_NODE:
      assert(st_lookup_int(sink_table, (char *) fanout_node->link, &sink_polarity));
      assert(sink_polarity == source_polarity);
      st_delete(sink_table, (char **) &fanout_node->link, &dummy);
      break;
    case BUFFER_NODE:
      buffer_polarity = (fanout_delay_get_buffer_polarity(fanout_node->gate_index) == POLAR_X) ? source_polarity : POLAR_INV(source_polarity);
...
fanout_cost_t *peephole_cost;
{
  ...
  if (peephole_global_debug == -1 || (peephole_global_debug && ! peephole_is_better_p)) {
    (void) fprintf(sisout, "before peephole: a(%2.2f) s(%2.4f %2.4f)\t", cost->area, cost->slack.rise, cost->slack.fall);
    (void) fprintf(sisout, "after peephole: a(%2.2f) s(%2.4f %2.4f)\n", peephole_cost->area, peephole_cost->slack.rise, peephole_cost->slack.fall);
  }
...
double source_load;
{
  ...
  for (i = 0; i < array_n(source->children); i++) {
	...
	fanout_arrival = fanout_delay_forward_load_dependent(source->arrival, source->gate_index, fanout_load); 
...
fanout_node_t *source;
{
  ...
  for (i = 0; i < array_n(source->children); i++) {
    ...
    case SINK_NODE:
      ...
      break;
    case BUFFER_NODE:
      fanout_node->polarity = (fanout_delay_get_buffer_polarity(fanout_node->gate_index) == POLAR_X) ? source->polarity : POLAR_INV(source->polarity);
...
int source_polarity;
{
  ...
  for (i = 0; i < array_n(source->children); i++) {
    ...
    case SINK_NODE:
      ...
      bucket->pwl = fanout_delay_get_delay_pwl(source->gate_index, source_load, source->arrival);
