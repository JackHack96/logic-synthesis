...
network_t *map_network(network_t *network, library_t *library, double mode, int inv_optz, int allow_internal_fanout) {
 ...
 */
int tree_map(network_t *network, library_t *library, bin_global_t globals) {
    global = globals;
...
int do_tree_premap(network_t *network, library_t *library) {
    lsGen        gen;
    network_t    *libnet;
    node_t       *pi, *node;
    ...
    foreach_node(network, gen, node) {
        if (node->type == INTERNAL) {
            switch (node_num_fanin(node)) {
                case 0:gate = node_function(node) == NODE_0 ? zero : one;
                    assert(lib_set_gate(node, gate, NIL(
                                   char *), NIL(node_t *), 0));
                    break;
                case 1:assert(lib_set_gate(node, inv, inv_formal, node->fanin, 1));
                    break;
                case 2:assert(lib_set_gate(node, nand2, nand2_formal, node->fanin, 2));
                    break;
                default: fail("bad node degree; should have been caught before here");
                    break;
            }
        }
    }
...
static int do_tree_match(network_t *network, library_t *library, bin_global_t *options) {
    int     i, some_output;
    node_t  *node, *fanout;
    prim_t  *prim;
    array_t *nodevec;
 ...
 */
static int find_best_match(prim_t *prim) {
    lsGen      gen;
    lib_gate_t *gate;
...
int
compare_cost(double this_area, double this_arrival, double match_area, double match_arrival, int slack_is_critical) {
...
static void find_match_cost(prim_t *prim, lib_gate_t *gate) {
    int          i, new_best;
...
static void match_cost(node_t *node, prim_t *prim, lib_gate_t *gate, double *area, delay_time_t *arrival) {
    register int          i;
...
network_t *map_build_network(network_t *network) {
...
static node_t *build_network_recur(network_t *new_network, node_t *node) {
    int    i, nin;
...
static void set_leaf_inverter_flag(node_t *node) {
    int    i;
    node_t *fanin;
...
static int inverter_optimize(register node_t *node) {
    int i;
...
void patch_constant_cells(network_t *network) {
    lsGen  gen, gen1;
 ...
 * combinational gates.
 */
int seq_filter(prim_t *prim, int debug) {
...
static char *prim_gate_name(prim_t *prim) {
    lsGen      gen;
    lib_gate_t *gate;
...
static int is_buffer(node_t *node1, node_t *node2) {
    lsGen  gen1, gen2;
...
static int is_transitive_fanin(node_t *node1, register node_t *node2) {
    node_t *fanin;
    int    result, i;
...
/* debug routine */
static void map_print_network(network_t *network) {
    array_t *list;
    node_t  *node;
    int     i;
