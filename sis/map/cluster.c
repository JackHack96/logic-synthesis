...
network_t *cluster_under_constraint(network, options)
        network_t *network;
        clust_options_t *options;
{
    ...
    switch (options->type) {
        case SIZE_CONSTRAINT:new_network = cluster_under_size_constraint(network, options);
            break;
        case DEPTH_CONSTRAINT:new_network = cluster_under_depth_constraint(network, options);
            break;
        case SIZE_AS_DEPTH_CONSTRAINT:new_network = cluster_under_size_as_depth_constraint(network, options);
            break;
        case CLUSTER_STATISTICS:gather_cluster_statistics(network, options);
            break;
        case BEST_RATIO_CONSTRAINT:new_network = cluster_under_best_ratio(network, options);
            break;
        case FANIN_CONSTRAINT:new_network = cluster_under_size_constraint(network, options);
            break;
        default:;
    }
    return new_network;
...
/* if flag "relabel" is set, try to relabel nodes to control duplication of logic */
/* without affecting delay */

static network_t *cluster_under_size_constraint(network, options)
        network_t *network;
        clust_options_t *options;
{
    int      max_label;
    ...
    if (options->verbose >= 2) (void) fprintf(misout, "------ labelling nodes-------\n");
    max_label = label_nodes(table, nodevec, options->cluster_size, options->verbose);
    if (options->verbose >= 1) (void) fprintf(misout, "*** # levels = %d\n", max_label + 1);

    if (options->relabel) {
        relabel_nodes(table, nodevec, max_label, options->verbose);
    }

    if (options->verbose >= 2) (void) fprintf(misout, "------ forming clusters-------\n");
    ...
    if (options->verbose >= 2) (void) fprintf(misout, "------ collapsing clusters-------\n");
...
static network_t *cluster_under_depth_constraint(network, options)
        network_t *network;
        clust_options_t *options;
{
    array_t         *nodevec;
    ...
    new_options.cluster_size = lazy_binary_search(min_cluster_size, max_cluster_size, max_label, nodevec,
                                                  best_labelling);

    if (options->verbose >= 1) (void) fprintf(misout, "*** max cluster size = %d\n", new_options.cluster_size);
    array_free(nodevec);
...
static network_t *cluster_under_size_as_depth_constraint(network, options)
        network_t *network;
        clust_options_t *options;
{
    array_t         *nodevec;
...
static void gather_cluster_statistics(network, options)
        network_t *network;
        clust_options_t *options;
{
    int      i;
 ...
 * for all cluster sizes from 2 to the first cluster size leading to a duplication ratio
 * larger than options->dup_ratio. Record the cluster_size corresponding to the smallest
 * duplication ratio among those leading to the smallest value for max_label.
 * If none, simply take cluster_size = 2.
 * Then do the clustering for that size.
 */

static network_t *cluster_under_best_ratio(network, options)
        network_t *network;
        clust_options_t *options;
{
    int             i;
    ...
    for (i = min_cluster_size; i <= max_cluster_size; i++) {
        ...
        if (dup_ratio >= options->dup_ratio) break;
        if (max_label == 0) break;
        if (max_label < best_level || (max_label == best_level && dup_ratio < best_dup_ratio)) {
    ...
    if (best_cluster_size == -1) best_cluster_size = 2;
    array_free(nodevec);

    new_options = *options;
    new_options.type         = SIZE_CONSTRAINT;
    new_options.cluster_size = best_cluster_size;
    if (options->verbose >= 1) (void) fprintf(misout, "*** cluster size = %d\n", new_options.cluster_size);
...
/* allocate a clust_node_t for each node, and put the map "node --> clust_node" in 'table'. */
...
static clust_node_t *clust_node_alloc(table, node)
        st_table *table;
        node_t *node;
{
    clust_node_t *node_info;
...
static int decide_label_node(table, node, node_info, cluster_size)
        st_table *table;
        node_t *node;
        clust_node_t *node_info;
        int cluster_size;
{
    ...
    /*
     * max_fanin_label == -1 iff node is constant node or node with all fanins PI's
     */
    ...
    total_weight = compute_tfi_weight_for_given_label(table, node, max_fanin_label);
...
static int get_max_fanin_label(table, node)
        st_table *table;
        node_t *node;
{
    int          i;
...
static int compute_tfi_weight_for_given_label(table, node, label)
        st_table *table;
        node_t *node;
        int label;
{
    ...
    compute_tfi_for_given_label_rec(table, node, label, tfi_for_given_label, tfi_fanin);
    ...
    if (global_settings.type == FANIN_CONSTRAINT) {
        ...
        st_generator *gen;
        clust_node_t *node_info;
        st_foreach_item(tfi_for_given_label, gen, (char **) &node, (char **) &node_info)
        {
...
static void compute_tfi_for_given_label_rec(table, node, label, visited, visited_fanins)
        st_table *table;
        node_t *node;
        int label;
        st_table *visited;
        st_table *visited_fanins;
{
    ...
    if (node->type == PRIMARY_INPUT) {
        st_insert(visited_fanins, (char *) node, NIL(
        char));
        return;
    }
    ...
    foreach_fanin(node, i, fanin)
    {
        compute_tfi_for_given_label_rec(table, fanin, label, visited, visited_fanins);
    }
}
...
static void relabel_nodes(table, nodevec, max_label, verbose)
        st_table *table;
        array_t *nodevec;
        int max_label;
        int verbose;
{
    ...
    for (i = n_nodes - 1; i >= 0; i--) {
        ...
        {
            if (fanin->type != INTERNAL) continue;
        ...
        if (verbose) { /* debug */
            (void) fprintf(misout, "label of %s = %d", node_long_name(node), node_info->label);
            if (node_info->label < node_info->max_label) (void) fprintf(misout, " -> %d\n", node_info->max_label);
            (void) fprintf(misout, "\n");
        }

        node_info->label = node_info->max_label;
        new_max_label = MAX(new_max_label, node_info->label); /* for consistency check */
    }
        ...
        array_t *clusters;
{
     ...
     * if yes, create a new cluster; if no, don't. Force cluster creation if fanouts to a PO
     */
...
static void put_tfi_in_cluster_rec(table, cluster, node, label)
        st_table *table;
        cluster_t *cluster;
        node_t *node;
        int label;
{
    ...
    if (st_lookup(cluster->nodes, (char *) node, NIL(char *))) return;
    assert(st_lookup(table, (char *) node, (char **) &node_info));
    if (node_info->label < label) return;
    st_insert(cluster->nodes, (char *) node, NIL(
    char));
...
static double duplication_ratio(network, clusters)
        network_t *network;
        array_t *clusters;
{
    int       i;
        ...
        st_table *topological_order;
{
    ...
    assert(n_nodes > 0);
    st_foreach_item(cluster->nodes, gen, (char **) &net_node, NIL(
    char *)) {
...
static void reinit_cluster_info(table)
        st_table *table;
{
    node_t       *node;
    st_generator *gen;
...
static void cluster_free(cluster)
        cluster_t *cluster;
{
        ...
        cluster_t *cluster;
{
    ...
    st_foreach_item(cluster->nodes, gen, (char **) &node, &dummy)
    {
        ...
        switch (type) {
            case NORMAL_NODE:(void) fprintf(misout, " %s ", node->name);
                break;
            case ROOT_NODE:(void) fprintf(misout, " %s:root ", node->name);
                break;
            default:(void) fprintf(misout, "\nunexpected clust_node type\n");
                break;
        }
    }
    (void) fprintf(misout, "\n\n");
