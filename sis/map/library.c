...
/*
 * 01 10-Jun-90 klk 	Changed the procedure choose_nth_smallest_gate from
 * static to global
 */
#include "map_int.h"
#include "sis.h"
...
int lib_read_blif(FILE *fp, char *filename, int add_inverter, int nand_flag, library_t **library_p) {
    int          new_gates, c;
    library_t    *library;
...
static int process_pattern(library_t *library, network_t *network) {
    lib_class_t *class;
    lib_gate_t  *gate;
    prim_t      *prim;
...
static int process_constant(library_t *library, network_t *network) {
    node_t          *node;
...
static int found_match(prim_t *prim) {
    int i;
...
int lib_pattern_matches(network_t *network, prim_t *prim, int ignore_type) {
    node_t *po;
...
#endif /* SIS */
    match_flag = 0;
    gen_all_matches(po, prim, found_match, /* debug */ 0,
            /* allow_internal_fanout */ 0, /* fanout_limit */ INFINITY);
    return match_flag;
}

static prim_t *find_or_create_pattern(library_t *library, network_t *network) {
    lsGen  gen;
    prim_t *prim;
...
static lib_gate_t *find_gate(library_t *library, network_t *network) {
    lsGen      gen;
    lib_gate_t *gate;
    char       *netname;
...
static lib_gate_t *find_or_create_gate(library_t *library, network_t *network) {
    int        i;
    lib_gate_t *gate;
    char       errmsg[1024];
#ifdef SIS
        ...
        if (strcmp(network_name(network), "**wire**") != 0) {
            for (i = network_num_pi(network) - 1; i >= 0; i--) {
                if (gate->delay_info[i] == 0) {
                    (void) sprintf(errmsg,
                                   "'%s': missing.sh delay information for pin '%s'",
                ...
                if (lib_gate_type(gate) != COMBINATIONAL &&
                    lib_gate_type(gate) != ASYNCH) {
                    if (gate->latch_time_info[i] == 0) {
                        (void) sprintf(errmsg, "'%s': missing latch time for pin '%s'",
                                       gate->name, node_name(network_get_pi(network, i)));
                        read_error(errmsg);
                        return 0;
                    }
...
static int compare_network(network_t *net1, network_t *net2) {
    lsGen  gen;
    node_t *po1, *po2;
...
static lib_class_t *find_class(library_t *library, network_t *network) {
    lsGen       gen;
    lib_class_t *class;
...
static lib_class_t *find_or_create_class(library_t *library, lib_gate_t *gate) {
    lib_class_t *class_p;
...
static void derive_dual_classes(library_t *library) {
    lsGen       gen;
    lib_class_t *class;
    network_t   *dual;
...
static network_t *dual_network(network_t *network) {
    ...
    foreach_primary_input(new_network, gen, pi) {
        ...
        foreach_fanout(pi, gen1, node) { assert(node_patch_fanin(node, pi, inv)); }
...
static double get_gate_area(lib_gate_t *gate) {
...
lib_gate_t *choose_smallest_gate(library_t *library, char *string) {
...
lib_gate_t *choose_nth_smallest_gate(library_t *library, char *string, int order) {
    network_t   *network;
    lib_class_t *class_p;
...
static void lib_reset_default_gates() { default_inverter = NIL(lib_gate_t); }
...
/* map from classes to prim_t structures. Cached for efficiency. Need to be
 * reset for every new library */
...
array_t *lib_get_prims_from_class(lib_class_t *class_p) {
    array_t    **slot;
    library_t  *library;
    lsGen      gen1, gen2;
    lib_gate_t *gate;
    ...
    if (st_find_or_add(lib_prim_table, (char *) class_p, (char ***) &slot))
        return *slot;
 ...
 */
static int lib_reduce_network(network_t *network) {
    node_t  *node;
    latch_t *latch;
    ...
    if (network_gate_type(network) != (int) COMBINATIONAL) {
        ...
        if (latch_input == NIL(node_t)) {
            (void) sprintf(errmsg, "latch network '%s' has no latch input",
        ...
        if (latch == NIL(latch_t)) {
            (void) sprintf(errmsg, "latch network '%s' has no latch",
                           network_name(network));
            read_error(errmsg);
            return 0;
        }
        ...
        if (node != NIL(node_t)) {
            ...
            if (fanin != NIL(node_t))
                network_delete_node(network, fanin);

            /* hack for D-type latches */
            if (network_num_pi(network) == 1)
 ...
 */
static int lib_expand_network(network_t *network) {
 ...
 */
static char **lib_get_pin_delay_info(network_t *network) {
#ifdef SIS
    lsGen  gen;
    node_t *node;
 ...
 */
static int lib_get_latch_pin(network_t *network) {
    lsGen  gen;
    ...
    if (latch_output == NIL(node_t))
        return -1;
    return i;
}

/* return the array of setup/hold times */
static latch_time_t **lib_get_latch_time_info(network_t *network) {
    node_t       *node;
    ...
    switch (network_gate_type(network)) {
        ...
        case (int) ASYNCH:return NIL(latch_time_t *);
        default:latch_time_array = ALLOC(latch_time_t *, network_num_pi(network));
            i                    = 0;
            foreach_primary_input(network, gen, node) {
                if (!IS_LATCH_END(node)) {
                    latch_time_array[i] = ALLOC(latch_time_t, 1);
                    latch_time_array[i]->setup =
                            delay_get_parameter(node, DELAY_ARRIVAL_RISE);
                    latch_time_array[i]->hold  =
 ...
 */
static void lib_save_clock_info(network_t *network, node_t *clock) {
    lib_clock_t *clock_info;
    node_t      *clock_fanin;
    double      n;
    ...
    assert(node_type(clock_fanin) == PRIMARY_INPUT &&
           node_num_fanout(clock_fanin) == 1);
    ...
    if (!st_is_member(clock_table, network_name(network))) {
        ...
        clock_info->delay->block.rise =
                delay_get_parameter(clock_fanin, DELAY_BLOCK_RISE);
        clock_info->delay->block.fall =
                delay_get_parameter(clock_fanin, DELAY_BLOCK_FALL);
        clock_info->delay->drive.rise =
                delay_get_parameter(clock_fanin, DELAY_DRIVE_RISE);
        clock_info->delay->drive.fall =
                delay_get_parameter(clock_fanin, DELAY_DRIVE_FALL);
        clock_info->delay->load       =
                delay_get_parameter(clock_fanin, DELAY_INPUT_LOAD);
        clock_info->delay->max_load   =
                delay_get_parameter(clock_fanin, DELAY_MAX_INPUT_LOAD);
        ...
        assert(st_insert(clock_table, util_strsav(network_name(network)),
                         (char *) clock_info) == 0);
    }
...
static void lib_register_type(network_t *network) {
    lsGen   gen;
    node_t  *node;
    ...
    if (!st_is_member(network_type_table, network_name(network))) {
        ...
        assert(st_insert(network_type_table, util_strsav(network_name(network)),
                         (char *) type) == 0);
    }
}
...
static char *node_real_name(node_t *node) {
    char *name, *n;
    int  len, i;
