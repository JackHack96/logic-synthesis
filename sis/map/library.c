 ...
 *
 * $Source: /users/pchong/CVS/sis/sis/map/library.c,v $
 * $Author: pchong $
 * $Revision: 1.1.1.1 $
 ...
 * imported
 *
 * Revision 1.8  1993/06/02  20:33:53  sis
 * Changed functions like strsav to util_strsav to improve machine indepedence.
 *
 * Revision 1.8  1993/06/02  20:33:53  sis
 * Changed functions like strsav to util_strsav to improve machine indepedence.
 *
 * Revision 1.7  1993/01/05  21:50:58  sis
 * Core leak fix.
 *
 * Revision 1.6  1992/12/11  01:37:05  sis
 * network_delete_latch now frees the latch data structure, so
 * there is no need to free it here.
 *
 * Revision 1.5  1992/06/05  16:24:15  sis
 * bug fix in find_or_create_pattern
 * (this fixed a bug where read_blif coredumps after doing a read_library)
 * bug fix in lib_reduce_network
 * (this fixed the core dump on sun machines upon read_library)
 *
 * Revision 1.6  1992/06/03  20:53:33  cmoon
 * lib_reduce_network sets latch control node to NIL.
 *
 * Revision 1.5  1992/05/30  01:04:47  cmoon
 * find_or_create_pattern returns prim after doing lib_expand_network now.
 *
 * Revision 1.4  1992/05/15  21:01:27  cmoon
 * keep SIS from aborting when library spec with inconsistent
 * name convention is used (compare_network).
 *
 * Revision 1.3  1992/05/06  00:27:18  cmoon
 *  in lib_pattern_matches
 *
 * Revision 1.2  1992/03/03  00:02:51  cmoon
