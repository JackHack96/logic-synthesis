...
int inflag;
{
    ...
    switch(inflag){
    case 0: 
      p = network_get_po(gate->network, pin);
      break;
    case 1:
      p = network_get_pi(gate->network, pin);
      break;
#ifdef SIS
    case 2:
      return (gate->control_name);
#endif
    default:
      p = NIL(node_t);
      break;
...
lib_gate_t *gate;
{
    if (gate == 0) return NIL(lib_class_t);
    return gate->class_p;
}
...
node_t *node;
{
    if (MAP(node) == 0) return NIL(lib_gate_t);
    return MAP(node)->gate;
}


char *
lib_get_pin_delay(node, pin)
node_t *node;
int pin;
{
...
int
lib_set_gate(user_node, gate, formals, actuals, nin)
node_t *user_node;
lib_gate_t *gate;
char **formals;
node_t **actuals;
int nin;
{
  ...
  if (node_type(user_node) != PRIMARY_INPUT) {
	...
	if (strcmp(fanin->name, formals[j]) == 0) {
      ...
      if (new_fanin[i] == 0) {
	(void) sprintf(errmsg, 
		       "lib_set_gate: pin '%s' not in formals list\n", fanin->name);
	error_append(errmsg);
	return 0;
      }
    }
...
#ifdef SIS
lib_gate_t *
lib_choose_smallest_latch(library, string, latch_type)
library_t *library;
char *string;
...
lib_gate_t *gate;
{
  ...
  switch(gate->type) {
  case (int) COMBINATIONAL: (void) fprintf(sisout, "comb"); break;
  case (int) ACTIVE_HIGH: (void)fprintf(sisout, "ah"); break;
  case (int) ACTIVE_LOW: (void)fprintf(sisout, "al"); break;
  case (int) RISING_EDGE: (void)fprintf(sisout, "re"); break;
  case (int) FALLING_EDGE: (void)fprintf(sisout, "fe"); break;
  case (int) ASYNCH: (void)fprintf(sisout, "as"); break;
  default: (void)fprintf(sisout, "unknown"); break;
  }
  ...
  if (gate->latch_time_info != 0) {
	...
	(void)fprintf(sisout, "  pin %d : setup = %4.2f, hold = %4.2f\n", i,
		      gate->latch_time_info[i]->setup, gate->latch_time_info[i]->hold);
      }
    }
  }
  if (gate->clock_delay != 0) {
    (void)fprintf(sisout, "  clock delay = %4.2f %4.2f %4.2f %4.2f %4.2f %4.2f\n", 
