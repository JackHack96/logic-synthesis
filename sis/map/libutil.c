...
#ifdef SIS
lib_gate_t *lib_choose_smallest_latch(library, string,
                                      latch_type) library_t *library;
char *string;
...
void lib_dump_gate(gate) lib_gate_t *gate;
{
  ...
  case (int)COMBINATIONAL:
    (void)fprintf(sisout, "comb");
    break;
  case (int)ACTIVE_HIGH:
    (void)fprintf(sisout, "ah");
    break;
  case (int)ACTIVE_LOW:
    (void)fprintf(sisout, "al");
    break;
  case (int)RISING_EDGE:
    (void)fprintf(sisout, "re");
    break;
  case (int)FALLING_EDGE:
    (void)fprintf(sisout, "fe");
    break;
  case (int)ASYNCH:
    (void)fprintf(sisout, "as");
    break;
  default:
    (void)fprintf(sisout, "unknown");
    break;
  }
