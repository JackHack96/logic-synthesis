...
/* this file keeps a log of all node creations during fanout optimization and
 * undo them */
...
void fanout_log_init(options) bin_global_t *options;
{
  global = *options;
...
void fanout_log_cleanup_network(network) network_t *network;
{
  int i;
  node_t *node;
  lsGen gen;
  ...
  foreach_node(network, gen, node) { unmap_node(node); }
}

void fanout_log_register_node(node) node_t *node;
{
  if (global.fanout_log_on) {
...
static void unmap_node(node) node_t *node;
{
