...
static int compare_count(count_t *x, count_t *y) { return y->nodes - x->nodes; }
...
static void find_tree_size(node_t *node, st_table *leaf_table, st_table *visited, node_t *tree_root) {
    int     i;
    count_t *count;
    node_t  *fanin;
...
void map_print_tree_size(FILE *fp, network_t *network) {
    ...
    for (count = count_list; count != NIL(count_t); count = count_next) {
        ...
        (void) fprintf(fp, "    nodes=%3d   leaf=%3d  freq=%3d  (root node = %s)\n",
                       count->nodes + count->leafs, count->leafs, count->freq,
                       count->root_node->name);
        FREE(count);
    }
}
