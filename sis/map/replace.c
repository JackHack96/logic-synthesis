...
void replace_2or(network, verbose) network_t *network;
int verbose;
{
  int i;
  int has_changed;
...
static int node_skip(node) node_t *node;
{
...
static void extract_node_fn(table, node) st_table *table;
node_t *node;
{
  int i, j;
...
static int get_literal(node, row, literal) node_t *node;
int row;
int literal;
{
...
static unsigned int fnhash(obj, modulus) char *obj;
...
static int fncmp(obj1, obj2) char *obj1;
char *obj2;
{
...
static void replace_node(node_table, visited, node,
                         replaced) st_table *node_table;
st_table *visited;
node_t *node;
node_t *replaced;
{
...
/* if not, swap */
static void smaller_pointer_first(fn) node_fn_t *fn;
{
  node_t *tmp_node;
  int tmp_int;
