...
/* replace one node by the other. Then both of these nodes are no longer of
 * interest */
...
void replace_2or(network_t *network, int verbose) {
    int          i;
    int          has_changed;
    st_generator *gen;
    ...
    do {
        ...
        for (i      = 0; i < array_n(nodevec); i++) {
            ...
            if (node_skip(node))
                continue;
            if (!st_lookup(node_table, (char *) node, (char **) &fn))
                continue;
    ...
    st_foreach_item(node_table, gen, (char **) &node, (char **) &fn) { FREE(fn); }
...
static int node_skip(node_t *node) {
...
static void extract_node_fn(st_table *table, node_t *node) {
    int       i, j;
...
static int get_literal(node_t *node, int row, int literal) {
...
static unsigned int fnhash(char *obj, unsigned int modulus) {
    node_fn_t    *fn = (node_fn_t *) obj;
    unsigned int n;

    n = (unsigned int) ((int) fn->fanin[0] + (int) fn->fanin[1] + fn->phase[0] * 2 +
                        fn->phase[1]);
    return n % modulus;
}

static int fncmp(char *obj1, char *obj2) {
    node_fn_t *fn1 = (node_fn_t *) obj1;
    node_fn_t *fn2 = (node_fn_t *) obj2;

    if (fn1->fanin[0] != fn2->fanin[0])
        return 1;
    if (fn1->fanin[1] != fn2->fanin[1])
        return 1;
    if (fn1->phase[0] != fn2->phase[0])
        return 1;
    if (fn1->phase[1] != fn2->phase[1])
        return 1;
    return 0;
}
...
static void replace_node(st_table *node_table, st_table *visited, node_t *node, node_t *replaced) {
    ...
    if (st_lookup(visited, (char *) old_fn, (char **) &visited_node)) {
        if (fanout == visited_node) {
            was_visited = 1;
            tmp         = old_fn;
            st_delete(visited, (char **) &old_fn, NIL(char *));
            assert(tmp == old_fn);
        }
    }
    ...
    new_fn = old_fn;
    if (was_visited)
        st_insert(visited, (char *) new_fn, (char *) fanout);
    network_sweep_node(fanout);
}
...
/* if not, swap */
static void smaller_pointer_first(node_fn_t *fn) {
    node_t *tmp_node;
    int    tmp_int;

    if ((int) fn->fanin[0] < (int) fn->fanin[1])
        return;
    tmp_node = fn->fanin[1];
