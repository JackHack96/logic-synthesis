...
#define SETSUB(a, b, c)                                                        \
  ((a).rise = (b).rise - (c).rise, (a).fall = (b).fall - (c).fall)
#define SETADD(a, b, c)                                                        \
  ((a).rise = (b).rise + (c).rise, (a).fall = (b).fall + (c).fall)
#define GETMIN(a) MIN((a).rise, (a).fall)
#define GETMAX(a) MAX((a).rise, (a).fall)
#define SETMIN(a, b, c)                                                        \
  ((a).rise = MIN((b).rise, (c).rise), (a).fall = MIN((b).fall, (c).fall))
#define SETMAX(a, b, c)                                                        \
  ((a).rise = MAX((b).rise, (c).rise), (a).fall = MAX((b).fall, (c).fall))
#define SETMAX2(a, b) ((a) = ((a) > (b)) ? (a) : (b))
#define SETMIN2(a, b) ((a) = ((a) < (b)) ? (a) : (b))
#define IS_EQUAL(a, b)                                                         \
  (FP_EQUAL((a).rise, (b).rise) && FP_EQUAL((a).fall, (b).fall))
