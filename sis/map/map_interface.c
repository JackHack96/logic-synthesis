...
network_t *complete_map_interface(network, options) network_t *network;
...
network_t *build_mapped_network_interface(network, options) network_t *network;
bin_global_t *options;
{
  network_type_t type;
...
network_t *tree_map_interface(network, options) network_t *network;
bin_global_t *options;
{
  int ok_status;
  network_type_t type;
...
network_t *fanout_opt_interface(network, options) network_t *network;
bin_global_t *options;
{
  network_type_t type;
...
network_type_t map_get_network_type(network) network_t *network;
{
  int is_mapped = 1;
  int is_annotated = 1;
  int all_constants = 1;
...
network_t *map_premap_network(network, options) network_t *network;
bin_global_t *options;
{
  library_t *library = options->library;
...
static network_t *build_network_from_annotations(network,
...
static void map_remove_unnecessary_annotations(network) network_t *network;
{
  lsGen gen;
...
static void visit_mapped_nodes(node, visited) node_t *node;
st_table *visited;
{
  int i;
...
void map_report_data_mapped(network) network_t *network;
{
  node_t *node;
  lsGen gen;
...
static void map_report_data_annotated(network, options) network_t *network;
bin_global_t *options;
{
  if (!options->print_stat)
    return;
...
static void map_report_data(network) network_t *network;
{
  node_t *node;
  lsGen gen;
  double area = 0.0;
...
static void map_compute_loads(network) network_t *network;
{
  int i, j;
  lsGen gen;
  node_t *fanin, *node;
...
/* ARGSUSED */
static void unmap_network(network, options) network_t *network;
bin_global_t *options;
{ options->network_type = RAW_NETWORK; }

void map_report_node_data(node) node_t *node;
{
  int i;
  node_t *fanin;
...
static int map_check_library(network, options) network_t *network;
bin_global_t *options;
{
  library_t *library = options->library;
  lib_gate_t *gate;
