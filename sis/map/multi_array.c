...
multidim_t *generic_multidim_alloc(int type_size, int n_indices, int *max_index) {
    int        i;
    int        size   = type_size;
...
int multidim_array_abort(int code) {
    fprintf(stderr, "multidim array error: ");
    switch (code) {
        case 0:fprintf(stderr, "inconsistent object size");
            break;
        case 1:fprintf(stderr, "index out of bounds ");
            break;
