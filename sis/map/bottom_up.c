...
/* ARGSUSED */
void bottom_up_init(network, alg) network_t *network;
fanout_alg_t *alg;
...
static void build_tree_bottom_up(fanout_info, source_index, tree,
                                 cost) opt_array_t *fanout_info;
int source_index;
array_t *tree;
fanout_cost_t *cost;
{
...
static bottom_up_t *build_tree_bottom_up_rec(fanout_info, source_index,
...
static void select_best_subgroup(fanout_info, source_index, no_buffer_ok,
                                 best_buffer,
                                 best_sink) opt_array_t *fanout_info;
int source_index;
int no_buffer_ok;
int *best_buffer;
int *best_sink;
...
static int compute_sink_index(fanout_info, k) opt_array_t *fanout_info;
int k;
{
  int i;
  double load;
...
static st_table *fanout_info_extract(fanout_info, from,
                                     to) opt_array_t *fanout_info;
int from;
int to;
{
  int i;
...
static void put_tree_in_fanout_tree_order(tree, root) array_t *tree;
bottom_up_t *root;
{
  int i;
  bottom_up_t *node;
...
static bottom_up_t *create_tree_node(fanout_info, sink_index, source_index,
                                     table) opt_array_t *fanout_info;
int sink_index;
int source_index;
st_table *table;
...
static void create_node_for_sink(link, table) gate_link_t *link;
st_table *table;
{
  bottom_up_t *result = ALLOC(bottom_up_t, 1);
...
static int
    get_polarity_with_latest_req_time(fanout_info) opt_array_t *fanout_info;
{
  int p, q;
  gate_link_t *link;
