...
**
**  01 27-May-91 klk		New code to create a buffer tree that satisfies
*the fanout limit
**                              using the bottom_up algorithm.
**  01 10-Jun-91 klk		Fixed a bug in the function
*build_tree_folim_bottom_up_rec().
**/
/*
 * $Log: bottom_up.c,v $
...
struct bottom_up_struct {
    ...
    gate_link_t
                 *link; /* Pointer to a sink node for a leaf node in the buffer tree */
};
...
/* ARGSUSED */
void bottom_up_init(network_t *network, fanout_alg_t *alg) { alg->optimize = bottom_up_optimize; }

/* INTERNAL INTERFACE */

static int bottom_up_optimize(opt_array_t *fanout_info, array_t *tree, fanout_cost_t *cost) {
    int           source_index;
...
static void build_tree_bottom_up(opt_array_t *fanout_info, int source_index, array_t *tree, fanout_cost_t *cost) {
    int          i, p;
...
static bottom_up_t *build_tree_bottom_up_rec(opt_array_t *fanout_info, int source_index, st_table *table) {
    int         p, q;
    int         source_polarity;
    ...
    select_best_subgroup(&fanout_info[p], source_index, no_buffer_ok,
                         &buffer_index, &sink_index);
    if (buffer_index == -1) {
        assert(p == source_polarity && fanout_info[q].n_elts == 0);
        return create_tree_node(&fanout_info[p], 0, source_index, table);
    }

    /* if still work to do: create new_fanout_info properly, function of buffer
     * polarity */
    node = create_tree_node(&fanout_info[p], sink_index, buffer_index, table);
    if (is_inverter(n_gates, buffer_index)) {
        new_fanout_info[p].links =
                fanout_info_extract(&fanout_info[p], 0, sink_index);
        fanout_info_preprocess(&new_fanout_info[p]);
        new_fanout_info[q].links =
                fanout_info_extract(&fanout_info[q], 0, fanout_info[q].n_elts);
...
static void
select_best_subgroup(opt_array_t *fanout_info, int source_index, int no_buffer_ok, int *best_buffer, int *best_sink) {
    int          buffer_index;
    int          k, best_k;
    int          local_sink;
    ...
    *best_buffer = -1;
    ...
    foreach_buffer(n_gates, buffer_index) {
        if (fanout_info->n_elts == 1 && !is_inverter(n_gates, buffer_index))
            continue;
        k              = compute_best_number_of_inverters(source_index, buffer_index, load,
                                                          fanout_info->n_elts);
...
static int compute_sink_index(opt_array_t *fanout_info, int k) {
    int    i;
    double load;
    double load_threshold = fanout_info->total_load / k;

    assert(fanout_info->n_elts > 0);
    if (fanout_info->n_elts == 1)
        return 0;
...
static st_table *fanout_info_extract(opt_array_t *fanout_info, int from, int to) {
    int         i;
    gate_link_t *link;
...
static void put_tree_in_fanout_tree_order(array_t *tree, bottom_up_t *root) {
    int         i;
    bottom_up_t *node;
...
static bottom_up_t *create_tree_node(opt_array_t *fanout_info, int sink_index, int source_index, st_table *table) {
    int          i;
    double       area;
    double       load;
...
static void create_node_for_sink(gate_link_t *link, st_table *table) {
    bottom_up_t *result = ALLOC(bottom_up_t, 1);
...
static int get_polarity_with_latest_req_time(opt_array_t *fanout_info) {
    int          p, q;
    gate_link_t  *link;
    ...
    foreach_polarity(p) {
        ...
        if (fanout_info[p].n_elts == 0)
            return q;
        link = array_fetch(gate_link_t *, fanout_info[p].required,
                           fanout_info[p].n_elts - 1);
        required[p] = link->required;
    }
    return (GETMIN(required[POLAR_X]) > GETMIN(required[POLAR_Y])) ? POLAR_X
                                                                   : POLAR_Y;
}
