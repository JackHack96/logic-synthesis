...
void virtual_delay_compute_arrival_times(network, globals) network_t *network;
...
void virtual_delay_set_po_negative_required(network) network_t *network;
{
  lsGen gen;
  node_t *output;
...
void virtual_delay_set_po_required(network) network_t *network;
{
  lsGen gen;
  node_t *output;
...
/* this routine also computes the required times on the wires ending up at
 * "node" */
/* those required times take the delay through the gate at "node" into account
 */

void virtual_delay_compute_node_required_time(node) node_t *node;
{
  double load;
  ...
  delay_map_compute_required_times(MAP(node)->ninputs,
                                   MAP(node)->gate->delay_info,
                                   MAP(node)->required, load, link_required);
...
static void update_gate_link_loads_rec(table, node) st_table *table;
node_t *node;
{
  int i;
  node_t *input;
...
void virtual_delay_arrival_times_rec(table, node) st_table *table;
node_t *node;
{
  int pin;
...
static void virtual_delay_compute_node_arrival_time(node) node_t *node;
{
  int i;
  node_t *input;
  ...
  case NODE_1:
    if (!global.no_warning)
      (void)fprintf(siserr,
                    "WARNING: arrival time at constant node is (0.00,0.00) \n");
  ...
  case NODE_COMPLEX:
    if (strcmp(MAP(node)->gate->name, "**wire**") == 0) {
      ...
      arrival =
          delay_map_simulate(MAP(node)->ninputs, arrival_times,
