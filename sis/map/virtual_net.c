...
void virtual_network_setup_gate_links(network_t *network, bin_global_t *options) {
    int          i;
    node_t       *node;
    array_t      *nodevec;
    st_generator *gen;
...
void virtual_network_remove_wires(network_t *network) { bin_for_all_nodes_outputs_first(network, do_remove_wires); }
...
void virtual_network_remove_node(node_t *node, int rec_flag) {
    int         pin;
    gate_link_t link;
...
void virtual_network_add_to_gate_link(node_t *source, gate_link_t *link) {
...
void virtual_network_update_link_required_times(node_t *node, delay_time_t *required) {
    int         i;
    node_t      *input;
    gate_link_t link;
    ...
    switch (node->type) {
        ...
        case PRIMARY_OUTPUT:
            ...
            break;
        case INTERNAL:assert(MAP(node)->gate != NIL(lib_gate_t));
...
static void virtual_network_setup_node_gate_links(node_t *node, st_table *delete_table) {
    int         pin;
    node_t      *input;
    gate_link_t link;
...
static void do_remove_wires(node_t *node) {
    node_t      *input;
    gate_link_t link;
    if (MAP(node)->gate == NIL(lib_gate_t))
        return;
...
static void remove_buffers(node_t *node) {
    int         i, n_fanouts;
    lsGen       gen;
    node_t      *fanin, *fanout;
    gate_link_t link;
...
static void do_flatten(node_t *node) {
    int      p;
    lsGen    gen;
    node_t   *fanout;
    node_t   *sources[2];
        ...
        case NODE_COMPLEX:
            ...
            foreach_fanout(node, gen, fanout) {
                switch (lib_gate_type(lib_gate_of(fanout))) {
                    case COMBINATIONAL:
                    case UNKNOWN:break;
                    default:LS_ASSERT(lsFinish(gen));
                        return;
                }
            }
#endif /* SIS */
            break;
        default: fail("unexpected node function");
            /* NOTREACHED */
    }
...
static void extract_local_virtual_network(st_table **nodes, node_t *source) {
    lsGen  gen;
    node_t *fanout;
...
static void extract_local_virtual_network_rec(st_table **nodes, node_t *node, int polarity) {
    lsGen  gen;
    node_t *fanout;
    int    node_polarity;
...
static void make_sources_external(node_t **sources, st_table **nodes) {
    int    p;
    int    is_empty[2];
    int    is_source[2];
...
static int flatten_is_empty(node_t *node, st_table *table) {
    int         is_empty = 1;
    gate_link_t link;
...
static int flatten_is_source(node_t *source) {
...
static void virtual_setup_inverter(node_t *source, node_t *dest) {
    gate_link_t link;
...
static void concentrate_gate_links_on_source(st_table *nodes, st_table *inv_nodes, node_t *source) {
    node_t       *node;
    char         *value;
    st_generator *gen;
    gate_link_t  link;
...
static void remove_intermediate_nodes(st_table *nodes) {
    node_t       *node;
    char         *dummy;
    st_generator *gen;
...
static void flatten_check_consistency(node_t *node) {
    int    alive;
    int    empty_fanout;
    node_t *fanout;
...
void virtual_network_check(network_t *network) {
    lsGen  gen;
    node_t *node;
...
static void virtual_network_check_node(node_t *node) {
    switch (node->type) {
        case INTERNAL:
            if (MAP(node)->gate != NIL(lib_gate_t)) {
                check_fanin(node);
            ...
            break;
        case PRIMARY_INPUT:check_fanout(node);
            break;
        case PRIMARY_OUTPUT:check_fanin(node);
            break;
...
static void check_fanout(node_t *node) {
    gate_link_t link;
    assert(!gate_link_is_empty(node));
    gate_link_first(node, &link);
    do {
...
static void check_fanin(node_t *node) {
    int         i;
    node_t      *fanin;
    gate_link_t link;
