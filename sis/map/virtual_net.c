        ...
        delay_time_t *required;
{
    ...
    switch (node->type) {
        ...
        case PRIMARY_OUTPUT:if (node_num_fanin(node) == 0) return;
        ...
        st_table *delete_table;
{
    ...
    if (gate_link_n_elts(node) == 0) {
        st_insert(delete_table, (char *) node, NIL(
        char));
        return;
    }
    for (pin = 0; pin < MAP(node)->ninputs; pin++) {
        ...
        node_t *node;
{
    ...
    st_delete(nodes[POLAR_Y], (char **) &sources[POLAR_Y], NIL(
    char *));
        ...
        int polarity;
{
    ...
    if (MAP(node)->gate != NIL(lib_gate_t))
        st_insert(nodes[node_polarity], (char *) node, NIL(
    char));
    foreach_fanout(node, gen, fanout)
    {
