...
void fanout_delay_init(options) bin_global_t *options;
{
  int i;
  library_t *library;
  delay_gate_t *buffer;
...
void fanout_delay_add_source(source_node, source_polarity) node_t *source_node;
int source_polarity;
{
  source_type_t type =
...
void fanout_delay_add_pwl_source(source_node,
                                 source_polarity) node_t *source_node;
int source_polarity;
{
  source_type_t type =
...
static void fanout_delay_add_general_source(source_node, source_polarity,
                                            source_type) node_t *source_node;
int source_polarity;
source_type_t source_type;
{
  delay_gate_t *source;
  if (source_polarity == POLAR_X &&
      n_gates.n_neg_sources > n_gates.n_pos_sources)
...
double fanout_delay_get_buffer_load(gate_index) int gate_index;
{
...
delay_time_t fanout_delay_backward_intrinsic(required,
                                             gate_index) delay_time_t required;
int gate_index;
{
...
delay_time_t fanout_delay_backward_load_dependent(required, gate_index,
                                                  load) delay_time_t required;
int gate_index;
double load;
{
  delay_gate_t *gate = array_fetch(delay_gate_t *, gate_array, gate_index);
...
delay_time_t fanout_delay_forward_intrinsic(arrival,
                                            gate_index) delay_time_t arrival;
int gate_index;
{
  delay_gate_t *gate = array_fetch(delay_gate_t *, gate_array, gate_index);
...
delay_time_t fanout_delay_forward_load_dependent(arrival, gate_index,
                                                 load) delay_time_t arrival;
int gate_index;
double load;
{
  delay_gate_t *gate = array_fetch(delay_gate_t *, gate_array, gate_index);
...
delay_pwl_t fanout_delay_get_delay_pwl(gate_index, load,
                                       arrival) int gate_index;
double load;
delay_time_t arrival;
{
  delay_gate_t *gate = array_fetch(delay_gate_t *, gate_array, gate_index);
...
int compute_best_number_of_inverters(
    source_index, buffer_index, load,
    max_n) int source_index; /* index of the source gate (can be a buffer) */
int buffer_index;            /* index of the buffer gate */
double load; /* total load for the two level tree; incorporates wire loads for
                max_n wires */
...
double fanout_delay_get_area(gate_index) int gate_index;
{
  delay_gate_t *delay_gate =
      array_fetch(delay_gate_t *, gate_array, gate_index);
  return (delay_gate->type == GATE_BUFFER) ? delay_gate->buffer.area : 0.0;
}

node_t *fanout_delay_get_source_node(source_index) int source_index;
{
  delay_gate_t *delay_gate =
      array_fetch(delay_gate_t *, gate_array, source_index);
...
lib_gate_t *fanout_delay_get_gate(gate_index) int gate_index;
{
  delay_gate_t *delay_gate =
      array_fetch(delay_gate_t *, gate_array, gate_index);
...
int fanout_delay_get_source_polarity(source_index) int source_index;
{
  delay_gate_t *delay_gate =
      array_fetch(delay_gate_t *, gate_array, source_index);
  assert(delay_gate->type == GATE_SOURCE);
...
int fanout_delay_get_buffer_polarity(buffer_index) int buffer_index;
{
  delay_gate_t *delay_gate =
      array_fetch(delay_gate_t *, gate_array, buffer_index);
  assert(delay_gate->type == GATE_BUFFER);
...
int fanout_delay_get_buffer_index(buffer) lib_gate_t *buffer;
{
  int i;
  delay_gate_t *delay_gate;
...
int fanout_delay_get_source_index(source) node_t *source;
{
  int i;
  delay_gate_t *delay_gate;
...
static void add_buffers(library, string) library_t *library;
char *string;
{
  lsGen gen;
  lib_gate_t *gate;
...
static void delay_gate_initialize(delay_gate) delay_gate_t *delay_gate;
{
  int i;
  lib_gate_t *gate;
  node_t *node;
...
static void delay_gate_free(delay_gate) delay_gate_t *delay_gate;
{
  switch (delay_gate->type) {
  case GATE_BUFFER:
    break;
...
static delay_time_t delay_load_dependent_buffer(required, buffer,
                                                load) delay_time_t required;
delay_gate_t *buffer;
double load;
{
...
static delay_time_t delay_intrinsic_pos_buffer(required,
                                               buffer) delay_time_t required;
delay_gate_t *buffer;
{
  delay_time_t result;
...
static delay_time_t
    delay_forward_intrinsic_pos_buffer(arrival, buffer) delay_time_t arrival;
delay_gate_t *buffer;
{
  delay_time_t result;
...
static delay_time_t delay_intrinsic_neg_buffer(required,
                                               buffer) delay_time_t required;
delay_gate_t *buffer;
{
  delay_time_t result;
...
static delay_time_t
    delay_forward_intrinsic_neg_buffer(arrival, buffer) delay_time_t arrival;
delay_gate_t *buffer;
{
  delay_time_t result;
...
static delay_time_t delay_load_dependent_pi_source(required, source,
                                                   load) delay_time_t required;
delay_gate_t *source;
double load;
{
...
static delay_time_t
    delay_forward_load_dependent_pi_source(arrival, source,
                                           load) delay_time_t arrival;
delay_gate_t *source;
double load;
{
...
static delay_time_t delay_load_dependent_pwl_source(required, source,
                                                    load) delay_time_t required;
delay_gate_t *source;
double load;
{
...
static delay_time_t
    delay_forward_load_dependent_internal_source(arrival, source,
                                                 load) delay_time_t arrival;
delay_gate_t *source;
double load;
{
...
static delay_time_t
    delay_forward_load_dependent_pwl_source(arrival, source,
                                            load) delay_time_t arrival;
delay_gate_t *source;
double load;
{
...
/* ARGSUSED */
static delay_time_t delay_intrinsic_fatal_error(required,
                                                buffer) delay_time_t required;
delay_gate_t *buffer;
{
...
/* ARGSUSED */
static delay_pwl_t
    delay_get_delay_pwl_internal_source(gate, load, arrival) delay_gate_t *gate;
double load;
delay_time_t arrival;
{
  int i;
...
/* ARGSUSED */
static delay_pwl_t delay_get_delay_pwl_pwl_source(gate, load,
                                                  arrival) delay_gate_t *gate;
double load;
delay_time_t arrival;
{
  delay_pwl_t result;
...
/* ARGSUSED */
static delay_pwl_t delay_get_delay_pwl_pi(gate, load,
                                          arrival) delay_gate_t *gate;
double load;
delay_time_t arrival;
{
...
/* ARGSUSED */
static delay_pwl_t delay_get_delay_pwl_buffer(gate, load,
                                              arrival) delay_gate_t *gate;
double load;
delay_time_t arrival;
{
...
static int linear_search_best_number_of_inverters(
    source_index, buffer_index, load, max_n, from,
    to) int source_index; /* index of the source gate (can be a buffer) */
int buffer_index;         /* index of the buffer gate */
double load; /* total load for the two level tree; incorporates wire loads for
                max_n wires */
int max_n;   /* maximum number of intermediate nodes */
