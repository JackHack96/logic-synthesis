 ...
 *
 * $Source: /users/pchong/CVS/sis/sis/main/sis.c,v $
 * $Author: pchong $
 * $Revision: 1.1.1.1 $
...
#ifdef SIS
    (void) fprintf(miserr,
	"    -c cmd\texecute SIS commands `cmd'\n");
    (void) fprintf(miserr,
	"    -f file\texecute SIS commands from a file\n");
#else
    (void) fprintf(miserr,
...
#ifdef SIS
    (void) fprintf(miserr,
	"    -s\t\tsuppress initial 'source %s/.sisrc'\n", sis_library());
#else
    (void) fprintf(miserr,
...
char **argv;
{
	...
	if (c == 'X') graphics_flag = atoi (util_optarg);
    }
