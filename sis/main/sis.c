...
static void usage(char *prog) {
  ...
  (void)fprintf(miserr,
                "usage: %s [-sx] [-c cmd] [-f script] [-o file] [-t type] [-T "
                "type] [file]\n",
                prog);
#ifdef SIS
...
#else
  (void)fprintf(miserr, "    -c cmd\texecute MIS commands `cmd'\n");
  ...
  (void)fprintf(miserr, "    -s\t\tsuppress initial 'source %s/.sisrc'\n",
                sis_library());
#else
  (void)fprintf(miserr, "    -s\t\tsuppress initial 'source %s/.misrc'\n",
                sis_library());
#endif
  FREE(lib_name);
#ifdef SIS
  (void)fprintf(miserr, "    -t type\tspecify input type (blif, eqn, kiss, "
                        "oct, pla, slif, or none)\n");
  (void)fprintf(miserr, "    -T type\tspecify output type (blif, eqn, kiss, "
                        "oct, pla, slif, or none)\n");
#else
  (void)fprintf(miserr, "    -t type\tspecify input type (bdnet, blif, eqn, "
                        "oct, pla, or none)\n");
  (void)fprintf(miserr, "    -T type\tspecify output type (bdnet, blif, eqn, "
                        "oct, pla, or none)\n");
#endif
