 ...
 */
static void stg_free_g(gGeneric thing) {
    array_t     *x;
    char        *name;
    int         i, j, param;
    stg_clock_t *ptr;
...
static void stg_free_v(gGeneric thing) {
    gGeneric x;
...
static void stg_free_e(gGeneric thing) {
    gGeneric x;
...
void stg_free(graph_t *stg) {
    g_free_static(stg, stg_free_g, stg_free_v, stg_free_e);
    return;
}

static gGeneric stg_copy_v(gGeneric old) {
...
static gGeneric stg_copy_e(gGeneric old) {
...
void stg_copy_names(graph_t *old, graph_t *new) {
    array_t *x, *newx;
    char    *name;
    int     i, j;
 ...
 */
void stg_copy_clock_data(graph_t *old, graph_t *new) {
...
static int equivtrans(char *x, char *y) {
    char c;
...
int stg_check(graph_t *stg) {
    ...
    if (!start) {
        (void) fprintf(siserr, "Fatal error in stg_check: no start state specified");
        return 0;
    }
    ...
    if (!g_get_g_slot_static(stg, CURRENT)) {
        (void) fprintf(siserr, "Warning in stg_check: no current state specified\n");
    ...
    foreach_vertex(stg, gen, vert) {
        ...
        if (state_len != code_length) {
            (void) fprintf(siserr,
                           "Fatal error in stg_check:  state %s has an encoding with "
        ...
        if (vert != start && lsLength(g_get_in_edges(vert)) == 0) {
            if (vert != any_state) {
                (void) fprintf(siserr, "Warning in stg_check: unreachable vertex, %s\n",
                               stg_get_state_name(vert));
            }
        }
        ...
        switch (len) {
            case 0:
                (void) fprintf(siserr,
                               ...
                               stg_get_state_name(vert));
            case 1:break;
            default:input_array = ALLOC(char *, len);
                    ...
                    for (j = i - 1; j >= 0; j--) {
                        if (equivtrans(input_array[i], input_array[j])) {
                            if (strcmp(output_array[i], output_array[j]) ||
                                ...
                                (void) fprintf(siserr,
                                               "Fatal error in stg_check: machine is not "
                                               "deterministic (see state %s)\n",
                                               stg_get_state_name(vert));
                                return_code = 0;
                                goto bad_exit1;
                            }
...
void stg_sim(graph_t *stg, char *vector) {
...
void stg_set_names(graph_t *stg, array_t *name_array, int flag) {
    int param;
...
array_t *stg_get_names(graph_t *stg, int flag) {
    int     param;
    array_t *name_array;
...
void stg_set_start(graph_t *stg, vertex_t *v) {
...
void stg_set_current(graph_t *stg, vertex_t *v) {
    if (g_vertex_graph(v) != stg) {
...
vertex_t *stg_create_state(graph_t *stg, char *name, char *encoding) {
    vertex_t *v;
...
edge_t *stg_create_transition(vertex_t *from, vertex_t *to, char *in, char *out) {
    edge_t  *edge;
    lsGen   gen;
    char    *input;
    graph_t *stg;
...
vertex_t *stg_get_state_by_name(graph_t *stg, char *name) {
    lsGen    gen;
    vertex_t *s;
...
vertex_t *stg_get_state_by_encoding(graph_t *stg, char *encoding) {
    lsGen    gen;
    vertex_t *s;
...
void stg_set_state_name(vertex_t *v, char *n) {
    char *oldname;
...
void stg_set_state_encoding(vertex_t *v, char *n) {
    char *oldname;
