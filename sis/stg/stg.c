 ...
 */
static void stg_free_g(thing) gGeneric thing;
{
  array_t *x;
  char *name;
  int i, j, param;
...
static void stg_free_v(thing) gGeneric thing;
{
  gGeneric x;
...
static void stg_free_e(thing) gGeneric thing;
{
  gGeneric x;
...
void stg_free(stg) graph_t *stg;
{
  g_free_static(stg, stg_free_g, stg_free_v, stg_free_e);
  return;
}

static gGeneric stg_copy_v(old) gGeneric old;
{
...
static gGeneric stg_copy_e(old) gGeneric old;
{
...
void stg_copy_names(old, new) graph_t *old, *new;
{
  array_t *x, *newx;
  char *name;
  int i, j;
 ...
 */
void stg_copy_clock_data(old, new) graph_t *old, *new;
{
...
graph_t *stg_dup(stg) graph_t *stg;
{
  graph_t *new;
...
static int equivtrans(x, y) char *x, *y;
{
  char c;
...
void stg_reset(stg) graph_t *stg;
{
  if (!stg) {
    return;
  }
...
void stg_sim(stg, vector) graph_t *stg;
char *vector;
{
...
void stg_set_names(stg, name_array, flag) graph_t *stg;
...
array_t *stg_get_names(stg, flag) graph_t *stg;
int flag; /* == 1 => inputs, 0 => outputs */
{
  int param;
  array_t *name_array;
...
void stg_set_start(stg, v) graph_t *stg;
vertex_t *v;
{
...
void stg_set_current(stg, v) graph_t *stg;
vertex_t *v;
{
  if (g_vertex_graph(v) != stg) {
...
vertex_t *stg_create_state(stg, name, encoding) graph_t *stg;
char *name, *encoding;
{
  vertex_t *v;
...
edge_t *stg_create_transition(from, to, in, out) vertex_t *from, *to;
char *in, *out;
{
  edge_t *edge;
  lsGen gen;
...
vertex_t *stg_get_state_by_name(stg, name) graph_t *stg;
char *name;
{
  lsGen gen;
  vertex_t *s;
...
vertex_t *stg_get_state_by_encoding(stg, encoding) graph_t *stg;
char *encoding;
{
  lsGen gen;
  vertex_t *s;
...
void stg_set_state_name(v, n) vertex_t *v;
char *n;
{
  char *oldname;
...
void stg_set_state_encoding(v, n) vertex_t *v;
char *n;
{
  char *oldname;
