...
/*ARGSUSED*/
static int com_stg_extract(network, argc, argv) network_t **network;
int argc;
char **argv;
{
...
static int com_stg_state_assign(network, argc, argv) network_t **network;
int argc;
char **argv;
{
  FILE *to, *from;
  ...
  while ((c = util_getopt(argc, argv, "x")) != EOF) {
    switch (c) {
    case 'x':
      (void)fprintf(siserr, "usage: state_assign program_name options\n");
      return 1;
    }
  }
  error_init();
...
static int com_stg_state_minimize(network, argc, argv) network_t **network;
int argc;
char **argv;
{
  FILE *to, *from;
  ...
  while ((c = util_getopt(argc, argv, "x")) != EOF) {
    switch (c) {
    case 'x':
      (void)fprintf(siserr, "usage: state_mininimize program_name options\n");
      return 1;
    }
  }
  error_init();
...
/*ARGSUSED*/
static int com_stg_cover(network, argc, argv) network_t **network;
int argc;
char **argv;
{
...
/*ARGSUSED*/
static int com_one_hot(network, argc, argv) network_t **network;
int argc;
char **argv;
{
  graph_t *stg;
...
static int com_stg_to_network(network, argc, argv) network_t **network;
int argc;
char **argv;
{
  network_t *new;
...
network_t *stg_to_network(stg, option) graph_t *stg;
int option;
{
  FILE *to, *from;
  char command[1024];
 ...
 */
static void stg_network_add_clock(network, stg) network_t *network;
graph_t *stg;
{
  lsGen gen;
...
static void stg_network_connect_clock(network, stg) network_t *network;
graph_t *stg;
{
  lsGen gen;
  latch_t *latch;
...
static int write_encoded_espresso_format(fp, stg) FILE *fp;
graph_t *stg;
{
  vertex_t *start;
  edge_t *e;
...
/*ARGSUSED*/
static int com_stg_dump_graph(network, argc, argv) network_t **network;
int argc;
char **argv;
{
  graph_t *stg;
