...
/*ARGSUSED*/
static int com_stg_extract(network_t **network, int argc, char **argv) {
    ...
    while ((c = util_getopt(argc, argv, "aec")) != EOF) {
        switch (c) {
            case 'c':check_containment = TRUE;
                break;
            case 'a':opt = 1;
                break;
            case 'e':override = 1;
                break;
            default:(void) fprintf(siserr, "Usage: %s [-a] [-e] [-c]\n", argv[0]);
                (void) fprintf(siserr, "\t-a: find for all possible start states\n");
                (void) fprintf(siserr,
                               "\t    (only if there are no more than %d latches)\n",
                               STG_LATCH_LIMIT);
                (void) fprintf(siserr,
...
static int com_stg_state_assign(network_t **network, int argc, char **argv) {
    FILE      *to, *from;
    char      command[1024];
    char      buffer[1024];
    char      *infile;
    ...
    while ((c = util_getopt(argc, argv, "x")) != EOF) {
        switch (c) {
            case 'x':(void) fprintf(siserr, "usage: state_assign program_name options\n");
                return 1;
        }
    }
    error_init();
    ...
    if (status && !help) {
        (void) fprintf(
                siserr, "state assignment program returned non-zero exit status\n%s\n",
                command);
        (void) unlink(infile);
        (void) unlink(outfile);
        FREE(infile);
...
static int com_stg_state_minimize(network_t **network, int argc, char **argv) {
    FILE      *to, *from;
    int       i, c, status;
    int       num_states;
    char      command[1024];
    ...
    while ((c = util_getopt(argc, argv, "x")) != EOF) {
        switch (c) {
            case 'x':(void) fprintf(siserr, "usage: state_mininimize program_name options\n");
                return 1;
        }
    }
    error_init();
...
/*ARGSUSED*/
static int com_stg_cover(network_t **network, int argc, char **argv) {
...
/*ARGSUSED*/
static int com_one_hot(network_t **network, int argc, char **argv) {
    graph_t   *stg;
    int       ns, i;
    char      *code;
    lsGen     gen;
...
#ifdef STG_DEBUG
static int stg_test(network) network_t **network;
{
  graph_t *stg;
...
static int com_stg_to_network(network_t **network, int argc, char **argv) {
    network_t *new;
    int       c;
    int       option;
    ...
    while ((c = util_getopt(argc, argv, "e:")) != EOF) {
        switch (c) {
            case 'e':option = atoi(util_optarg);
...
network_t *stg_to_network(graph_t *stg, int option) {
    FILE      *to, *from;
    char      command[1024];
    char      *infile;
    char      *outfile;
    ...
    switch (option) {
        case 0:(void) sprintf(command, "espresso -o fd < %s > %s", infile, outfile);
            stg_single = 0;
            break;

        case 1:(void) sprintf(command, "espresso -o fd < %s > %s", infile, outfile);
            stg_single = 1;
            break;

        case 2:(void) sprintf(command, "espresso -o fd -Dso < %s > %s", infile, outfile);
            stg_single = 1;
            break;

        default:return NIL(network_t);
    }
    ...
    if (status) {
        (void) fprintf(siserr,
                       "stg to network program returned non-zero exit status\n%s\n",
                       command);
        (void) unlink(infile);
        (void) unlink(outfile);
        FREE(infile);
    ...
    while (latches) {
        ...
        c = *start++;
        switch (c) {
            case '0':latch_set_initial_value(l, 0);
                latch_set_current_value(l, 0);
                break;
            case '1':latch_set_initial_value(l, 1);
                latch_set_current_value(l, 1);
                break;
            default: fail("An encoding bit for the start state has an invalid value");
        }
        latches--;
    }
    (void) lsFinish(genpi);
 ...
 */
static void stg_network_add_clock(network_t *network, graph_t *stg) {
    lsGen        gen;
...
static void stg_network_connect_clock(network_t *network, graph_t *stg) {
    lsGen         gen;
    latch_t       *latch;
    int           edge_index;
    latch_synch_t type;
...
static int write_encoded_espresso_format(FILE *fp, graph_t *stg) {
    vertex_t *start;
    edge_t   *e;
    lsGen    lgen;
    int      state_bits;
...
/*ARGSUSED*/
static int com_stg_dump_graph(network_t **network, int argc, char **argv) {
    graph_t *stg;
