...
void setnptr(node_t *node, ndata *value) { (void) st_insert(node_to_ndata_table, (char *) node, (char *) value); }

graph_t *stg_extract(network_t *network, int ctable) {
        ...
        foreach_latch(copy, gen, l) {
            ...
            switch (init_val) {
                ...
                case 3:init_state[i] = '3';
                    break;
                default:(void) fprintf(siserr, "Unknown latch value %i\n", init_val);
                    goto error_exit;
            }
        }
        i++;
 ...
 */
static int stg_valid_network(network_t *network, node_t **control_node, char **clock_namep, int *edge_indexp) {
    int           i;
    lsGen         gen;
    char          *name;
 ...
 */
static void stg_remove_control_logic(network_t *network) {
    int         i;
    lsGen       gen;
    latch_t     *latch;
    sis_clock_t *clock;
...
static stg_clock_t *stg_get_clock_info(network_t *network) {
    lsGen        gen;
    clock_edge_t edge;
