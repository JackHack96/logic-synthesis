...
int stg_statecmp(char *key1, char *key2) {
    int      equal_parts, i;
 ...
 */
int stg_statehash(char *key, int modulus) {
...
static void stg_realloc_state_hash() {
...
void stg_translate_hashed_code(unsigned *h_state, int *stg_state) {
    int      i, j, k;
...
static int is_state(int cid, ndata **obj, int *level) {
    node_t *node;
    ndata  *n;
    lsGen  gen;
...
static ndata *find_hardest_control(int cid, node_t *node, int *noinv) {
    register int i;
    ndata        *n;
...
static ndata *find_easiest_control(int cid, node_t *node, int *noinv) {
    register int i;
 ...
 */
static ndata *find_pi_assignment(int cid, ndata *obj, int level) {
    ndata  *n;
    int    noinv;
...
static void save_edge(int cid, graph_t *stg) {
    char          *b1, *b2;
    register char *t1, *t2;
    ...
    *t2    = '\0';
    if (!st_lookup(state_table, instring, (char **) &instate)) {
        ...
        g_set_v_slot_static(instate, ENCODING_STRING,
                            (gGeneric) util_strsav(instring));
    ...
    if (!st_lookup(state_table, outstring, (char **) &outstate)) {
        ...
        g_set_v_slot_static(outstate, ENCODING_STRING,
                            (gGeneric) util_strsav(outstring));
...
int barray[16] = {1, 2, 4, 8, 16, 32, 64, 128,
                  256, 512, 1024, 2048, 4096, 8192, 16384, 32768};

void ctable_enum(graph_t *stg) {
        ...
        do {
            ...
            switch (find_state) {
                case TRUE:save_edge(0, stg);
                    ...
                    break;
                case FALSE:wpi = find_pi_assignment(0, obj, level);
                    if (wpi != NIL(ndata)) {
                        pi_stack[pi_num] = wpi;
                        *wpi->jflag |= CHANGED;
                        pi_num++;
...
void enumerate(int elength, int *estate, graph_t *stg) {
    ...
    do {
        ...
        switch (find_state) {
            case TRUE:save_edge(elength, stg);
                for (i = 0; i < nlatch; i++) {
                ...
                break;
            case FALSE:wpi = find_pi_assignment(elength, obj, level);
                if (wpi != NIL(ndata)) {
                    pi_stack[pi_num] = wpi;
