...
double clock_lb;
{
  ...
  for (i = 0; i < 2 * NUM_PHASE(cg); i++) {
    if (array_list[i] != NIL(array_t)) {
      ...
      gen_constraints_from(lg, cg, array_list[i], p1, clock_lb, red, black);
...
add_short_path_constraints(cg, lg) graph_t *cg, *lg;
{
  ...
  foreach_list_edge(lg, list, ledge) {
    ...
    array[from_id][to_id] =
        MIN(array[from_id][to_id],
            min_clock_skew(u) + de(ledge) - get_hold(v) - max_clock_skew(v));
  }
  for (i = 0; i < dim; i++) {
    for (j = 0; j < dim; j++) {
      if (array[i][j] < INFTY) {
