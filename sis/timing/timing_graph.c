...
/* function definition
    ...
    calls: tmg_alloc_graph(),
           delay_trace(),
       tmg_alloc_node, g_add_vertex(), tmg_latch_get_phase(),
   tmg_get_latch_type(), tmg_init_pi_arrival_time(), tmg_build_graph(),
           tmg_update_K_edge(), set_mmd()
*/

graph_t *tmg_network_to_graph(network_t *network, delay_model_t model, algorithm_type_t algorithm) {
    node_t   *pi, *node;
    graph_t  *latch_graph;
    vertex_t *vertex;
    latch_t  *latch;
    ...
    foreach_primary_input(network, gen, pi) {
        if (clock_get_by_name(network, node_long_name(pi)) == NIL(sis_clock_t)) {
            ...
            if (network_is_real_pi(network, pi)) {
                ...
                if ((vertex = MVER(latch)) == NIL(vertex_t)) {
                    ...
                    if ((PHASE(vertex) = tmg_latch_get_phase(latch, CL(latch_graph))) ==
                        NOT_SET) {
                        tmg_free_graph_structure(latch_graph);
                        return NIL(graph_t);
                    }
    ...
    foreach_node(network, gen, node) { tmg_delay_free(node); }
    ...
    foreach_latch(network, gen, latch) { UNDEF(latch) = NIL(char); }
    return latch_graph;
}
...
*/
int
tmg_build_graph(node_t *pi, graph_t *latch_graph, vertex_t *source, delay_model_t model, algorithm_type_t algorithm) {
    array_t      *dfs_array;
    node_t       *node, *fi;
    delay_time_t delay;
        ...
        if (node_function(node) != NODE_PO) {
            foreach_fanin(node, j, fi) {
                if (!DIRTY(fi)) {
                    ...
                    if (pin_delay->phase == PHASE_INVERTING ||
                        ...
                        maxdr(node) = MAX(maxdr(node), maxdf(fi) + delay.rise);
                        maxdf(node) = MAX(maxdf(node), maxdr(fi) + delay.fall);
                        mindr(node) = MIN(mindr(node), mindf(fi) + delay.rise);
                        mindf(node) = MIN(mindf(node), mindr(fi) + delay.fall);
                    }

                    if (pin_delay->phase == PHASE_NONINVERTING ||
                        pin_delay->phase == PHASE_NEITHER) {
                        maxdr(node) = MAX(maxdr(node), maxdr(fi) + delay.rise);
                        maxdf(node) = MAX(maxdf(node), maxdf(fi) + delay.fall);
                        mindr(node) = MIN(mindr(node), mindr(fi) + delay.rise);
                        mindf(node) = MIN(mindf(node), mindf(fi) + delay.fall);
                    }
                }
            }
            DIRTY(node) = FALSE;
            ...
            if (!network_is_real_po(node->network, node)) {
                ...
                if ((sink = MVER(latch)) == NIL(vertex_t)) {
                    ...
                    if ((PHASE(sink) = tmg_latch_get_phase(latch, CL(latch_graph))) ==
                        NOT_SET) {
                        return FALSE;
                    }
                }
...
int tmg_update_K_edge(graph_t *g) {
    lsGen       gen;
    edge_t      *edge;
    vertex_t    *from, *to;
...
*/
void tmg_init_pi_arrival_time(node_t *node, delay_model_t model) {
    latch_t      *latch;
    lib_gate_t   *gate;
...
void tmg_report_bug(vertex_t *source, vertex_t *sink) {
...
/* function definition
    ...
    calls: -
*/
static void tmg_delay_alloc(node_t *node) { UNDEF(node) = (char *) ALLOC(my_delay_t, 1); }

static void tmg_delay_free(node_t *node) {
    my_delay_t *md;
