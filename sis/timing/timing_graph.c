...
graph_t *tmg_network_to_graph(network, model, algorithm) network_t *network;
delay_model_t model;
...
/* function definition
   ...
   calls: network_tfo(), get_pin_delay(),
      ...
      tmg_get_latch_type()
*/
int tmg_build_graph(pi, latch_graph, source, model, algorithm) node_t *pi;
graph_t *latch_graph;
vertex_t *source;
delay_model_t model;
algorithm_type_t algorithm;
...
int tmg_update_K_edge(g) graph_t *g;
{
  lsGen gen;
  edge_t *edge;
  vertex_t *from, *to;
...
*/
tmg_init_pi_arrival_time(node, model) node_t *node;
delay_model_t model;
{
  latch_t *latch;
  lib_gate_t *gate;
...
tmg_report_bug(source, sink) vertex_t *source, *sink;
{
...
/* function definition
    ...
    calls: -
*/
static void tmg_delay_alloc(node) node_t *node;
{ UNDEF(node) = (char *)ALLOC(my_delay_t, 1); }

static void tmg_delay_free(node) node_t *node;
{
  my_delay_t *md;
