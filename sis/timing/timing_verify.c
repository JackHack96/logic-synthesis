...
int tmg_check_clocking_scheme(graph_t *latch_graph, network_t *network) {
    ...
    if (debug_type == VERIFY || debug_type == ALL) {
        ...
        for (i = 0; i < NUM_CLOCK(clock_event); i++) {
            (void) fprintf(sisout, "Phase %d  ^ %.2lf  v %.2lf\n", i,
                           RISE(clock_event)[i], FALL(clock_event)[i]);
        }
    }
    ...
    for (i = 0; i < num_e; i++) {
        ...
        foreach_vertex(latch_graph, gen, u) {
            if (TYPE(u) == NNODE) {
                ...
                phase = PHASE(u) - 1;
                switch (LTYPE(u)) {
                    case FFF:DEP(u) = cycle;
                        dep(u) = cycle;
                        break;
                    case FFR:temp = RISE(clock_event)[phase] + SHIFT(clock_event)[phase];
                        DEP(u) = temp;
                        dep(u) = temp;
                        break;
                    case LSH:temp = RISE(clock_event)[phase] + SHIFT(clock_event)[phase];
                        if (ARR(u) < temp) {
                            DEP(u) = temp;
                        } else {
                            DEP(u) = ARR(u);
        ...
        foreach_vertex(latch_graph, gen, u) {
            ...
            foreach_in_edge(u, gen1, e) {
                ...
                if (TYPE(w) != NNODE) {
                    phase1 = PHASE(w) - 1;
                    shift  = FALL(clock_event)[phase] - FALL(clock_event)[phase1] +
                             Ke(e) * cycle;
                } else {
                    shift = 0.0;
                }
            ...
            if (ARR(u) + EPS < max_arrival) {
                ...
                if (LTYPE(u) == FFF || LTYPE(u) == LSH) {
                    ...
                    if (ARR(u) > RISE(clock_event)[phase] + SHIFT(clock_event)[phase] -
                                 tmg_get_set_up(u) + EPS) {
                        (void) fprintf(sisout, "ERROR: set-up violation at %d\n", ID(u));
                        flag = FALSE;
                    }
                }
...
void trace_erroneous_path(graph_t *latch_graph, clock_event_t *clock_event, double cycle, vertex_t *u) {
    lsGen    gen;
    vertex_t *v;
    double   path;

    foreach_vertex(latch_graph, gen, v) { CV_DIRTY(v) = FALSE; }

    path = 0.0;
    (void) trace_recursive_path(latch_graph, clock_event, cycle, u, ARR(u), &path);
}

int trace_recursive_path(graph_t *latch_graph, clock_event_t *clock_event, double cycle, vertex_t *u, double A,
                         double *path) {
    edge_t   *edge;
    vertex_t *w;
    lsGen    gen;
    ...
    foreach_in_edge(u, gen, edge) {
        ...
        if (TYPE(w) != NNODE) {
            phase1 = PHASE(w) - 1;
            shift  = FALL(clock_event)[phase1] - FALL(clock_event)[phase] -
                     Ke(edge) * cycle;
        } else {
            shift = 0.0;
        }
        new_A = A - De(edge) - shift;
        if (trace_recursive_path(latch_graph, clock_event, cycle, w, new_A, path)) {
            *path += De(edge) + shift;
...
clock_event_t *tmg_get_clock_events(graph_t *latch_graph, network_t *network) {
    clock_event_t *clock_event;
    ...
    for (i = 0; i < NUM_CLOCK(clock_event); i++) {
        ...
        FALL(clock_event)
        [i] = clock_get_parameter(transition, CLOCK_NOMINAL_POSITION);
...
void tmg_free_clock_event(clock_event_t *clock_event) {
