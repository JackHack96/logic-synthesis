...
int timing_network_check(network, model) network_t *network;
delay_model_t model;
{
...
*/
l_graph_t *tmg_alloc_graph(algorithm, network) algorithm_type_t algorithm;
network_t *network;
{
  l_graph_t *g;
...
/* function definition
    ...
    calls:    -
*/
l_node_t *tmg_alloc_node(algorithm) algorithm_type_t algorithm;
{
  l_node_t *node;
  copt_node_t *copt;
  cv_node_t *cv;
...
/* function definition
    ...
    calls:   -
*/
int tmg_get_latch_type(latch) latch_t *latch;
{
...
int g_get_edge(from, to, edge) vertex_t *from;
vertex_t *to;
...
array_t *tmg_determine_clock_order(network) network_t *network;
{
  sis_clock_t *clock;
  lsGen gen;
  int no_sort;
...
/* function definition
    ...
    calls: -
*/
sis_clock_t *tmg_latch_get_clock(latch) latch_t *latch;
{
  node_t *gate, *clock_pi;
  sis_clock_t *clock;
...
/* function definition
    ...
    calls:
*/
static int tmg_clock_compare(k1, k2) char **k1;
char **k2;
{
...
*/
int tmg_latch_get_phase(l, a) latch_t *l;
array_t *a;
{
  int i;
...
tmg_set_set_up(s) double s;
{ Uset_up = s; }

tmg_set_hold(s) double s;
{ Uhold = s; }

double tmg_get_set_up(v) vertex_t *v;
{
  lib_gate_t *gate;
  latch_time_t **lt;
...
double tmg_get_hold(v) vertex_t *v;
{
  lib_gate_t *gate;
  latch_time_t **lt;
...
tmg_set_min_sep(s) double s;
{ Umin_sep = s; }

tmg_set_max_sep(s) double s;
{ Umax_sep = s; }

double tmg_get_min_sep() { return Umin_sep; }

double tmg_get_max_sep() { return Umax_sep; }
...
static void tmg_cg_free_g(c) gGeneric c;
{
  c_graph_t *g;
  g = (c_graph_t *)c;
...
static void tmg_cg_free_v(c) gGeneric c;
{
  c_node_t *n;
...
static void tmg_cg_free_e(c) gGeneric c;
{
  c_edge_t *e;
  st_generator *sgen;
  char *dummy;
...
*/
tmg_constraint_graph_free(g) graph_t *g;
{
  int i;
  phase_t *p;
...
tmg_set_gen_algorithm_flag(value) int value;
{ algorithm_flag = value; }

int tmg_get_gen_algorithm_flag() { return algorithm_flag; }

tmg_set_phase_inv(flag) int flag;
{ phase_inv = flag; }

int tmg_get_phase_inv() { return phase_inv; }
...
tmg_alloc_phase_vertices(cg, lg) graph_t *cg, *lg;
{
  phase_t **list;
  int i;
...
/* function definition
    ...
    calls: -
*/
static void tmg_lg_free_g(c) gGeneric c;
{
  l_graph_t *g;
...
static void tmg_lg_free_v(c) gGeneric c;
{
  l_node_t *n;
  n = (l_node_t *)c;
...
static void tmg_lg_free_e(c) gGeneric c;
{
  l_edge_t *e;
  e = (l_edge_t *)c;
  FREE(e);
...
tmg_free_graph_structure(latch_graph) graph_t *latch_graph;
{ g_free(latch_graph, tmg_lg_free_g, tmg_lg_free_v, tmg_lg_free_e); }

double tmg_max_clock_skew(u) vertex_t *u;
{ return (double)(0); }

double tmg_min_clock_skew(u) vertex_t *u;
{ return (double)(0); }
#endif /* SIS */
