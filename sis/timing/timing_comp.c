 ...
 *
 * $Source: /users/pchong/CVS/sis/sis/timing/timing_comp.c,v $
 * $Author: pchong $
 * $Revision: 1.1.1.1 $
...
double
tmg_compute_optimal_clock(latch_graph, network)
graph_t *latch_graph;
network_t *network;
{
    graph_t *clock_graph;
...
graph_t *g;
{
	...
	if (ABS(clock_ub - clock_lb) < EPS2) break;
...
/* function definition 
    ...
    calls:
*/ 
tmg_guess_clock_bound_recursive(u, b1, b2)
vertex_t *u;
double *b1, *b2;
{
    lsGen gen;
...
char *k1, *k2;
{
    ...
    if (ABS(Wv(v1) - Wv(v2)) < EPS1) return 0;
    if (Wv(v1) < Wv(v2)) return -1;
    return 1;
}
...
graph_t *g;
{
	...
	for (i = 0; i < NUM_POP; i++) {
		...
		pq_insert_fast(tree, (char *)pop_array[i], 
			       (char *)pop_array[i]);
	    }
	}
	FREE(pop_array);
...
graph_t *
tmg_construct_clock_graph(lg, clock_lb)
graph_t *lg;
double clock_lb;
{
    graph_t *cg;
	...
	if (array_list[i] != NIL(array_t)) {
	    ...
	    tmg_gen_constraints_from(lg, cg, array_list[i], p1, clock_lb, 
				 red, black);
...
tmg_gen_constraints_from(lg, cg, latch_array, p1, clock_lb, red, black)
graph_t *lg, *cg;
vertex_t *p1;
array_t *latch_array;
double clock_lb;
...
/* function definition 
    ...
    calls:    g_get_edge()
*/ 
tmg_add_short_path_constraints(cg, lg)
graph_t *cg, *lg;
{
    edge_t *cedge, *ledge;
    double **array;
...
double
tmg_solve_constraints(cg, v0, clock_lb)
graph_t *cg;
vertex_t *v0;
double clock_lb;
{
	...
	if (!tmg_is_feasible(cg, cur_clock, v0)) {
	    ...
	    if (clock_ub - clock_lb < EPS) break;
	    clock_ub = cur_clock;
	}
...
tmg_add_model_constraints(lg, cg, v0)
graph_t *lg, *cg;
...
/* function definition 
    ...
    calls:
*/ 
tmg_add_phase_separation_constraints(lg, cg, v0)
graph_t *lg, *cg;
vertex_t *v0;
{
...
/* function definition 
    ...
    calls:
*/ 
tmg_circuit_clock_update(lg, cg, network, clock)
graph_t *lg, *cg;
network_t *network;
double clock;
{
...
*/ 
double
tmg_solve_gen_constraints(cg, v0, clock_lb)
graph_t *cg;
vertex_t *v0;
double clock_lb;
{
...
/* function definition 
    ...
    return value: int
    calls: tmg_init_matrix(), tmg_update_matrix(), tmg_free_matrix(), tmg_evaluate_matrix()
*/ 
int
tmg_exterior_path_search(cg, clock, L, R)
graph_t *cg;
double *clock, L, R;
{
    int num_v, flag;
