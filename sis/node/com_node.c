...
int com_div(network_t **network, int argc, char **argv) {
    array_t *node_vec;
    node_t  *node1, *node2, *q, *r;

    node_vec = com_get_nodes(*network, argc, argv);
    if (array_n(node_vec) != 2) {
        (void) fprintf(miserr, "usage: _div n1 n2 -- test division\n");
        return 1;
    }
...
int com_sim(network_t **network, int argc, char **argv) {
    array_t         *node_vec;
    node_t          *node;
        ...
        switch (c) {
            case 'm':
                if (strcmp(util_optarg, "simpcomp") == 0) {
                    ...
                    mode = NODE_SIM_EXACT;
                } else if (strcmp(util_optarg, "exact-lits") == 0) {
    ...
    for (i = 0; i < array_n(node_vec); i++) {
        ...
        switch (node_function(node)) {
            case NODE_PI:
            case NODE_PO:break;
            default:(void) node_simplify_replace(node, NIL(node_t), mode);
        }
    }
    array_free(node_vec);
    return 0;
    usage:
    (void) fprintf(miserr, "_sim [node-list] -- test simplification\n");
...
int com_cof(network_t **network, int argc, char **argv) {
    array_t *node_vec;
    node_t  *node1, *node2, *r;

    node_vec = com_get_nodes(*network, argc, argv);
    if (array_n(node_vec) != 2) {
        (void) fprintf(miserr, "usage: _cof n1 n2 -- test cofactor\n");
        return 1;
    }
...
int com_wd(network_t **network, int argc, char **argv) {
    array_t *node_vec;
    node_t  *node1, *node2;
    ...
    node1 = array_fetch(node_t *, node_vec, 0);
    node2 = array_fetch(node_t *, node_vec, 1);
    (void) node_substitute(node1, node2, use_complement);
    array_free(node_vec);
    return 0;

    usage:
    (void) fprintf(miserr, "usage: wd [-c] n1 n2 -- re-express n1 using n2\n");
    (void) fprintf(miserr, "    -c\t\talso use the complement of n2\n");
    return 1;
}

int com_scc(network_t **network, int argc, char **argv) {
    array_t *node_vec;
    node_t  *node;
    int     i;
...
int com_invert(network_t **network, int argc, char **argv) {
    array_t *node_vec;
    node_t  *node;
    int     i;
