...
void node_free(node) node_t *node;
{
  daemon_t *d;
...
node_t *node_dup(old) register node_t *old;
{
  register node_t *new;
  register daemon_t *d;
...
node_t **nodevec_dup(fanin, nin) register node_t **fanin;
int nin;
{
  register int i;
...
void node_invalid(node) register node_t *node;
{
  register daemon_t *d;
...
void node_register_daemon(type, func) node_daemon_type_t type;
void (*func)();
{
  daemon_t *daemon;
  ...
  default:
    fail("node_register_daemon: bad daemon type");
  }
}
...
network_t *node_network(node) node_t *node;
{ return node->network; }
