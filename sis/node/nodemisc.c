...
int node_compare_id(char **p1, char **p2) {
    node_t *q1 = *(node_t **) p1, *q2 = *(node_t **) p2;

    return q1->sis_id - q2->sis_id;
}

static int compare_node_by_name(char **p1, char **p2) {
    node_t *q1 = *(node_t **) p1, *q2 = *(node_t **) p2;

    if (q1->name == 0)
        node_assign_name(q1); /* hack */
    if (q2->name == 0)
        node_assign_name(q2); /* hack */
...
void make_common_base(node_t *f, node_t *g, node_t ***new_fanin, int *new_nin, pset_family *newf, pset_family *newg) {
    node_t *func_list[2];
...
static int merge_fanin_list(node_t **func_list, int nfunc, node_t ***new_list, int (*compare)()) {
...
pset_family node_sf_adjust(node_t *node, register node_t **new_fanin, int new_nin) {
...
static pset_family do_sf_permute(pset_family old_sf, int *permute, int new_nin, int has_dup) {
    pset_family   new_sf;
...
void node_minimum_base(node_t *node) {
    pset   floor;
    pcover F1;
    node_t **new_fanin;
    int    var, new_nin;
...
node_t *node_create(pset_family func, node_t **fanin, int nin) {
    node_t *node;
...
/* intended for internal use only ... */
void node_replace_internal(node_t *f, node_t **fanin, int nin, pset_family F) {
    ...
    if (f->fanin != NIL(node_t *))
        FREE(f->fanin);
    if (f->F != NIL(set_family_t))
        sf_free(f->F);
    f->nin   = nin;
    f->fanin = fanin;
    f->F     = F;
...
void node_replace(node_t *f, node_t *r) {
    if (r->F == 0) {
...
int node_base_contain(node_t *f, node_t *g) {
    register int    i, j;
...
static int has_dup_fanin(node_t *node) {
    int    new_nin;
...
void node_remove_dup_fanin(node_t *node) {
    int         new_nin;
    node_t      *func_list[1], **list;
    pset_family newf;
...
static pset_family node_sf_adjust_by_name(node_t *node, register node_t **new_fanin, int new_nin) {
    register int    i, j, *permute;
    register node_t **old_fanin;
    int             has_dup, old_nin;
    pset_family     new_sf;
...
void make_common_base_by_name(node_t *f, node_t *g, node_t ***new_fanin, int *new_nin, pset_family *newf,
                              pset_family *newg) {
    node_t *func_list[2];
