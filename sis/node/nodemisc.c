...
int node_compare_id(p1, p2) char **p1, **p2;
{
...
static int compare_node_by_name(p1, p2) char **p1, **p2;
{
  node_t *q1 = *(node_t **)p1, *q2 = *(node_t **)p2;
...
void make_common_base(f, g, new_fanin, new_nin, newf, newg) node_t *f, *g;
node_t ***new_fanin;
int *new_nin;
...
static int merge_fanin_list(func_list, nfunc, new_list,
                            compare) node_t **func_list;
int nfunc;
node_t ***new_list;
int (*compare)();
{
...
pset_family node_sf_adjust(node, new_fanin, new_nin) node_t *node;
...
static pset_family do_sf_permute(old_sf, permute, new_nin,
                                 has_dup) pset_family old_sf;
int *permute;
int new_nin;
int has_dup;
{
...
void node_minimum_base(node) node_t *node;
{
  pset floor;
  pcover F1;
  node_t **new_fanin;
...
node_t *node_create(func, fanin, nin) pset_family func;
node_t **fanin;
int nin;
{
  node_t *node;
...
/* intended for internal use only ... */
void node_replace_internal(f, fanin, nin, F) node_t *f;
node_t **fanin;
int nin;
pset_family F;
{
...
void node_replace(f, r) node_t *f, *r;
{
  if (r->F == 0) {
...
int node_base_contain(f, g) node_t *f, *g;
{
  register int i, j;
...
static int has_dup_fanin(node) node_t *node;
{
  int new_nin;
...
void node_remove_dup_fanin(node) node_t *node;
{
  int new_nin;
  node_t *func_list[1], **list;
  pset_family newf;
...
static pset_family node_sf_adjust_by_name(node, new_fanin,
                                          new_nin) node_t *node;
register node_t **new_fanin;
int new_nin;
{
...
void make_common_base_by_name(f, g, new_fanin, new_nin, newf, newg) node_t *f,
    *g;
node_t ***new_fanin;
int *new_nin;
pset_family *newf, *newg;
