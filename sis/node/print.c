...
void node_print_negative(FILE *fp, node_t *node) {
    node_t      *node1, *node2;
    node_type_t type;
...
void node_print_rhs(FILE *fp, node_t *node) {
    node_t *node1;
...
static void node_print_internal(FILE *fp, node_t *node, int phase) {
    int    var, x, first_literal, first_pterm;
    pset   last, p;
    node_t *fanin;

    if (simple_print(fp, node, phase))
        return;
    ...
    foreach_set(node->F, last, p) {
        ...
        foreach_fanin(node, var, fanin) {
            switch (x = GETINPUT(p, var)) {
                ...
                default: fail("node_print: corrupt function");
            }
        }
    }
    if (!phase) {
...
static int simple_print(FILE *fp, node_t *node, int phase) {
    if (node->type == PRIMARY_INPUT) {
        return 1;
        ...
        switch (node_function(node)) {
            case NODE_0:(void) fprintf(fp, "-%s-", phase ? "0" : "1");
                return 1;

            case NODE_1:(void) fprintf(fp, "-%s-", phase ? "1" : "0");
                return 1;

            case NODE_BUF:(void) fprintf(fp, "%s%s", node_name(node->fanin[0]), phase ? "" : "'");
                return 1;

            case NODE_INV:(void) fprintf(fp, "%s%s", node_name(node->fanin[0]), phase ? "'" : "");
                return 1;
