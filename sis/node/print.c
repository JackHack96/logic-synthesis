...
void node_print(fp, node) FILE *fp;
node_t *node;
{
  node_t *node1;
...
void node_print_negative(fp, node) FILE *fp;
node_t *node;
{
  node_t *node1, *node2;
  node_type_t type;
...
void node_print_rhs(fp, node) FILE *fp;
node_t *node;
{
  node_t *node1;
...
static void node_print_internal(fp, node, phase) FILE *fp;
node_t *node;
int phase;
{
...
static int simple_print(fp, node, phase) FILE *fp;
node_t *node;
int phase;
{
  if (node->type == PRIMARY_INPUT) {
    ...
    case NODE_BUF:
      (void)fprintf(fp, "%s%s", node_name(node->fanin[0]), phase ? "" : "'");
      return 1;
