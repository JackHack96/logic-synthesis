        ...
        node_t *f, *g;
{
    ...
    if (f->nin == 0) {
        if (f->F->count == 0) {
            ...
            FREE(r->name);
            r->name = NIL(
            char);    /* FREE does set it to nil, this is */
            /* for safety */
            FREE(r->short_name);
            r->short_name = NIL(
            char);
        ...
        node_t *f;
{
    ...
    func = sf_unlist(sf_sort(newf, fancy_lex_order),
                     newf->count, newf->sf_size);
        ...
        node_t *node;
{
    ...
    if (node->type == PRIMARY_INPUT) {
        ...
        if (node->nin == 0) {
            ...
            if (node->nin * 2 - set_ord(p) != node->nin) goto failure;
            return NODE_AND;
        }
        ...
        node_t *node;
{
    ...
    if (node->nin == 0) {
        return ALLOC(
        int, 1);        /* no literals in any variables */
    }
