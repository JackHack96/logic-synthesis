...
node_t *node_and(node_t *f, node_t *g) {
...
node_t *node_or(node_t *f, node_t *g) {
    pset_family newf, newg, func;
    node_t      **new_fanin, *r;
    int         new_nin;
...
node_t *node_xor(node_t *f, node_t *g) {
...
node_t *node_xnor(node_t *f, node_t *g) {
    node_t *fbar, *gbar, *t0, *t1, *r;
...
node_t *node_literal(node_t *f, int phase) {
    node_t      *r, **fanin;
    pset_family func;
    pset        pdest;
...
node_t *node_constant(int phase) {
    node_t      *r;
    pset_family func;
    ...
    switch (phase) {
        ...
        case 1:func->count++;
            break;
        default: fail("node_constant: phase must be 0 or 1");
    }
...
node_t *node_largest_cube_divisor(node_t *f) {
    pset_family func;
    pset        supercube;
    node_t      *r;
...
int node_contains(node_t *f, node_t *g) {
    pset        last, p, *flist;
...
int node_equal(node_t *f, node_t *g) { return node_contains(f, g) && node_contains(g, f); }

/*
 *  node_equal_by_name -- see if node1 == node2 (boolean sense)
 */

int node_equal_by_name(node_t *f, node_t *g) {
...
node_t *node_sort_for_printing(node_t *f) {
    pset_family newf, newg, func;
...
node_function_t node_function(node_t *node) {
    ...
    if (node->type == PRIMARY_INPUT) {
        ...
        if (node->nin == 0) {
            ...
            switch (GETINPUT(p, 0)) {
                case ONE:return NODE_BUF;
                case ZERO:return NODE_INV;
...
node_type_t node_type(node_t *node) { return (node->type); }

int node_simplify_replace(node_t *F, node_t *D, node_sim_type_t mode) {
    node_t *newF;
...
node_t *node_simplify(node_t *node_F, node_t *node_D, node_sim_type_t mode) {
    int         new_nin, dummy;
    ...
    switch (mode) {
        case NODE_SIM_SCC:newf = sf_contain(newf);
            break;
        case NODE_SIM_SIMPCOMP:temp = simplify(cube1list(newf));
            sf_free(newf);
            newf = temp;
            break;
        case NODE_SIM_ESPRESSO:newr = complement(cube2list(newf, newd));
            newf = espresso(newf, newd, newr);
            sf_free(newr);
            break;
        case NODE_SIM_EXACT:newf = minimize_exact(newf, newd, NIL(set_family_t), /*exact*/ 1);
            break;
        case NODE_SIM_EXACT_LITS:newf = minimize_exact_literals(newf, newd, NIL(set_family_t), /*exact*/ 1);
            break;
        case NODE_SIM_DCSIMP:newf = minimize(newf, newd, DCSIMPLIFY);
            break;
        case NODE_SIM_NOCOMP:newf = minimize(newf, newd, NOCOMP);
            break;
        case NODE_SIM_SNOCOMP:newf = minimize(newf, newd, SNOCOMP);
            break;
        default: fail("node_simplify: bad mode\n");
            break;
    }
...
int *node_literal_count(node_t *node) {
...
void node_complement(node_t *node) {
...
uintptr_t node_error(int code) {
    switch (code) {
        case 0: fail("node_get_cube: node does not have a function");
            /* NOTREACHED */
        case 1: fail("node_get_cube: cube index out of bounds");
            /* NOTREACHED */
        case 2: fail("node_get_literal: bad cube");
            /* NOTREACHED */
        case 4: fail("foreach_fanin: node changed during generation");
            /* NOTREACHED */
        case 5: fail("foreach_fanout: node changed during generation");
            /* NOTREACHED */
        default: fail("error code unused");
    }
    return 0;
}
