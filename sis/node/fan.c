...
static void fanin_remove_single_fanout(node, i) node_t *node;
int i;
{
  char *data;
...
void fanin_remove_fanout(node) node_t *node;
{
  register int i;
  char *data;
...
static void fanin_replace_single_fanout(node, i) node_t *node;
int i;
{
  lsHandle handle;
...
void fanin_add_fanout(node) node_t *node;
{
  register int i;
...
lsGen node_fanout_init_gen(node) node_t *node;
{
  lsGen gen;
...
node_t *node_fanout_gen(gen, pin) lsGen gen;
int *pin;
{
  char *data;
  fanout_t *fanout_rec;
...
node_t *node_get_fanin(node, i) node_t *node;
int i;
{
  if (i < 0 || i >= node->nin) {
...
node_t *node_get_fanout(node, i) node_t *node;
int i;
{
  int cnt;
  fanout_t *fanout_rec;
...
int node_get_fanin_index(node, fanin) register node_t *node, *fanin;
{
  register int i;
...
int node_num_fanin(node) node_t *node;
{ return node->nin; }

int node_num_fanout(node) node_t *node;
{
  if (node->network == 0) {
...
int node_patch_fanin(node, fanin, new_fanin) node_t *node;
node_t *fanin;
node_t *new_fanin;
{
  int i;
 ...
 * "new_fanin"
 */
int node_patch_fanin_index(node, fanin_index, new_fanin) node_t *node;
int fanin_index;
node_t *new_fanin;
{
