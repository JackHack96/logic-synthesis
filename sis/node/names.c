...
char *node_name(node) node_t *node;
{
...
char *node_long_name(node) node_t *node;
{ return node->name; }

void node_assign_name(node) node_t *node;
{
  static char buf[80];
  if (node->name != NIL(char))
    FREE(node->name);
...
void node_assign_short_name(node) node_t *node;
{
  int i, c;
...
static char *hack_name(node, name) node_t *node;
char *name;
{
...
int node_is_madeup_name(name, value) char *name;
int *value;
{
...
void network_reset_long_name(network) network_t *network;
{
  lsGen gen;
  node_t *node;
  int index;
...
void network_reset_short_name(network) network_t *network;
{
  lsGen gen;
  node_t *node;
...
void network_rehash_names(network, long_name, short_name) network_t *network;
register int long_name, short_name;
{
  lsGen gen;
  node_t *p;
  int found;
