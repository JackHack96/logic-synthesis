 ...
 */
sis_clock_t *clock_create(char *name) {
    int         i, j;
    sis_clock_t *clock;
    ...
    for (i = RISE_TRANSITION; i <= FALL_TRANSITION;
         i++) { /* RISE and FALL edge */
 ...
 */
void clock_free(sis_clock_t *clock) {
    int i, j;
 ...
 * definition
 */
int clock_add_to_network(network_t *network, sis_clock_t *clock) {
    lsHandle handle;
...
void clock_set_cycletime(network_t *network, double value) {
    int flag                         = clock_get_current_setting_index(network);
    CLOCK(network)->cycle_time[flag] = value;
}

double clock_get_cycletime(network_t *network) {
    int flag = clock_get_current_setting_index(network);
 ...
 */
int clock_delete_from_network(network_t *network, sis_clock_t *clock) {
    int             i, j;
    lsGen           gen;
    char            *data;
 ...
 */
void network_clock_free(network_t *network) {
    int         i;
    lsGen       gen;
    sis_clock_t *clock;
    array_t     *clock_array;
 ...
 */
void network_clock_dup(network_t *old, network_t *new) {
    int             i, j;
    char            *dummy;
    lsGen           gen, gen2;
    clock_setting_t setting;
 ...
 */
static int clock_is_dependent(clock_edge_t edge1, clock_edge_t edge2) {
    lsGen        gen;
    int          status;
    char         *dummy;
 ...
 */
void network_clock_alloc(network_t *network) {
    network_clock_t *clk;
...
static void clock_add_no_check_dependency(clock_setting_t setting, clock_edge_t edge1, clock_edge_t edge2) {
    int j = (int) setting;
 ...
 */
int clock_add_dependency(clock_edge_t edge1, clock_edge_t edge2) {
    int          j;
    char         *dummy;
    lsGen        gen1, gen2;
 ...
 */
void clock_remove_dependency(clock_edge_t edge1, clock_edge_t edge2) {
    lsGen        gen;
    int          delete;
    char         *dummy;
    clock_edge_t *new_edge;
 ...
 */
lsGen clock_gen_dependency(clock_edge_t edge) {
    int j;
 ...
 */
sis_clock_t *clock_get_by_name(network_t *network, char *clk_name) {
    lsGen       gen;
    char        *dummy;
    sis_clock_t *clock;
 ...
 */
char *clock_name(sis_clock_t *clock) { return clock->name; }
 ...
 */
void clock_set_current_setting(network_t *network, clock_setting_t mode) { CLOCK(network)->flag = mode; }

/*
 * Returns the current value of the clock data flag
 */
clock_setting_t clock_get_current_setting(network_t *network) { return CLOCK(network)->flag; }

/*
 * Returns the current value of the clock data flag that can be used
 * to index the data structure ....
 */
int clock_get_current_setting_index(network_t *network) {
 ...
 */
int network_num_clock(network_t *network) { return lsLength(CLOCK(network)->clock_defn); }

int clock_num_dependent_edges(clock_edge_t edge) {
    int       flag, num;
    network_t *net;
 ...
 */
int clock_set_parameter(clock_edge_t clock_edge, clock_param_t parameter, double value) {
    lsGen        gen;
    network_t    *network;
    int          which_edge, flag;
    clock_edge_t *new_edge;
    ...
    switch (parameter) {
        case CLOCK_NOMINAL_POSITION:clock_edge.clock->value[which_edge][flag].nominal = value;
            /*
            ...
            (void) lsFinish(gen);
            break;
        case CLOCK_LOWER_RANGE:clock_edge.clock->value[which_edge][flag].lower_range = value;
            break;
        case CLOCK_UPPER_RANGE:clock_edge.clock->value[which_edge][flag].upper_range = value;
            break;
        default: fail("bad enum value in clock_set_parameter");
            /* NOTREACHED */
    }
    return 1;
}

double clock_get_parameter(clock_edge_t clock_edge, clock_param_t parameter) {
    network_t *network;
    int       which_edge, flag;
    double    t, cycle;
    ...
    switch (parameter) {
        case CLOCK_NOMINAL_POSITION:return clock_edge.clock->value[which_edge][flag].nominal;
        case CLOCK_LOWER_RANGE:return clock_edge.clock->value[which_edge][flag].lower_range;
        case CLOCK_UPPER_RANGE:return clock_edge.clock->value[which_edge][flag].upper_range;
        ...
        default: fail("bad enum value in clock_get_parameter");
            /* NOTREACHED */
    }
}
