 ...
 */
bdd_t *ntbdd_node_to_local_bdd(node, manager, leaves) node_t *node;
bdd_manager *manager;
st_table *leaves;
{
  int i;
 ...
 */
bdd_t *ntbdd_node_to_bdd(node, manager, leaves) node_t *node;
bdd_manager *manager;
st_table *leaves;
{
  ...
  /*
   * Handle the cases where node already has a BDD or where node is in the
   * leaves table.
   */
  ...
  if (ext_f == NIL(bdd_t)) {
    ...
    if (st_lookup_int(leaves, (char *)node, (int *)&varid)) {
      ...
      if (bdd_equal(ext_f, single_var_f)) {
        ...
        ntbdd_set_at_node(
            node, single_var_f); /* automatically frees existing bdd at node */
        return single_var_f;
      }
    } else {
      /*
 ...
 */
static bdd_t *construct_local_bdd(node, manager, leaves) node_t *node;
bdd_manager *manager;
st_table *leaves;
{
  bdd_variableId varid;
  ...
  if (nliterals == 0) {
     ...
     * If there is a fanin and this is a primary output
     * (e.g. this is one of those special primary output nodes)
     * get the variable associated with the input and return the corresponding
     * BDD. Note that we do not return the function of the fanin, but just the
 ...
 */
static bdd_t *sop(node, manager, leaves, bdd_type) node_t *node;
bdd_manager *manager;
st_table *leaves;
 ...
 */
static bdd_t *product(node, manager, row, leaves, bdd_type) node_t *node;
bdd_manager *manager;
int row;
st_table *leaves;
ntbdd_type_t bdd_type;
{
  ...
  for (literal = 0; literal < nliterals; literal++) {
    ...
    case ZERO:
      ...
      if (bdd_type == LOCAL) {
        /*
         * For the LOCAL case, fanin_bdd is not attached to the fanin node.
         * Thus, we must free it so that we don't leave any bdd_t's dangling. In
         * the GLOBAL case, the fanin_bdd is attached to the fanin node, and we
         * leave it their for future reference.  (Same comment applies to case
         * ONE.)
         */
        bdd_free(fanin_bdd);
      }
    ...
    case ONE:
      ...
      fanin_bdd = get_node_bdd(fanin, manager, leaves, bdd_type);
      temp_bdd = bdd_dup(
          fanin_bdd); /* just duplicate, so we have same flow as case ZERO */
      if (bdd_type == LOCAL) {
        bdd_free(fanin_bdd);
      }
      break;
 ...
 */
static bdd_t *get_node_bdd(node, manager, leaves, bdd_type) node_t *node;
bdd_manager *manager;
st_table *leaves;
ntbdd_type_t bdd_type;
{
