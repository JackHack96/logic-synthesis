 ...
 */
bdd_manager *ntbdd_start_manager(num_vars) int num_vars;
{
  bdd_manager *manager;
  ntbdd_t *ntbdd_data;
 ...
 */
void ntbdd_end_manager(manager) bdd_manager *manager;
{
  st_generator *gen;
  network_t *network;
  st_table *table;
 ...
 */
static void bdd_network_free(manager, network) bdd_manager *manager;
network_t *network;
{
  lsGen gen;
  node_t *node;
  ...
  foreach_node(network, gen, node) {
    ...
    if (fn != NIL(bdd_t)) {
      if (bdd_get_manager(fn) == manager) {
        bdd_free(fn);
        /* do not use ntbdd_set_at_node here: problems with manager's
         * hooks->network.network_table */
