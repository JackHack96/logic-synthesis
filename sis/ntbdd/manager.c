 ...
 */
bdd_manager *ntbdd_start_manager(int num_vars) {
    bdd_manager        *manager;
    ntbdd_t            *ntbdd_data;
 ...
 */
void ntbdd_end_manager(bdd_manager *manager) {
    st_generator       *gen;
    network_t          *network;
    st_table           *table;
    bdd_external_hooks *hooks;
 ...
 */
static void bdd_network_free(bdd_manager *manager, network_t *network) {
    lsGen  gen;
    node_t *node;
    bdd_t  *fn;
