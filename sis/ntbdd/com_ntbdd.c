...
static int com_bdd_stats(network) network_t **network; /* value/return */
{
  int i;
  lsGen gen;
  node_t *node;
 ...
 */
static void convert_bdd_to_ntwk_and_print(fn, po_name, var_names) bdd_t *fn;
char *po_name;
array_t *var_names;
{
  network_t *network;
 ...
 */
static int com_bdd_test(network, argc,
                        argv) network_t **network; /* value/return */
int argc;
char **argv;
{
  lsGen gen;
  ...
  /*
   * Create a manager. The current network must have at least one primary input
  ...
  for (i = 0; i < array_n(var_names); i++) {
    (void)fprintf(misout, " index = %d, name = %s\n", i,
                  array_fetch(char *, var_names, i));
  }

  /*
   * Check that the composition of the operations: network to BDD, and BDD to
   * network, is the identity operation.
   */
  ...
  /*
   * Print out all the cubes of the BDD, found by traversing all the paths from
   * the root to the constant 1.
   */
  ...
  (void)fprintf(misout, "compose(%s, %s, %s)\n", node_long_name(a_po),
                node_long_name(a_pi), node_long_name(a_node));
  ...
  f = bdd_and_smooth(po_fn, node_fn,
                     smoothing_vars); /* smoothing out all the variables */
  ...
  if (!bdd_leq(fns[4], fns[0], 1, 1)) {
    ...
    (void)fprintf(miserr,
                  "ERROR: first fn should have implied the second one\n");
  }
  if (verbose) {
    for (i = 0; i < 8; i++) {
...
static int com_bdd_create(network, argc, argv) network_t **network;
int argc;
char **argv;
{
  bdd_manager *manager;
  ...
  if (locality == LOCAL) {
    ...
    for (i = 0; i < array_n(node_vec); i++) {
      ...
      if (node->type == PRIMARY_INPUT) {
        /*
         * If the node is a PI, and it has not been inserted into the table yet,
         * then insert it.
         */
        if (!st_is_member(leaves, (char *)node)) {
          if (!st_insert(leaves, (char *)node, (char *)count))
            count++;
        }
      } else {
        foreach_fanin(node, j,
                      fanin) { /* st_insert returns 0 if no entry was found */
          if (!st_is_member(leaves, (char *)fanin)) {
            if (!st_insert(leaves, (char *)fanin, (char *)count))
              count++;
          }
        }
      }
...
usage:
  (void)fprintf(miserr, "usage: bdd_create [-l] [-o ordering_style] [-t "
                        "time_limit] node1 node2 ...\n");
  (void)fprintf(
      miserr, "	-l: build the bdd local to the specified nodes; not stored\n");
  (void)fprintf(miserr,
                "	ordering_style is one of the following: dfs, random\n");
  return 1;
}

static int com_bdd_print(network, argc, argv) network_t **network;
int argc;
char **argv;
{
  array_t *node_vec;
  ...
  for (i = 0; i < n; i++) {
    ...
    if ((bdd != NIL(bdd_t)) && (node->type != PRIMARY_OUTPUT)) {
      /* Primary outputs will be effectively caught by the fanins of the primary
       * outputs. */
...
static int com_bdd_size(network, argc, argv) network_t **network;
int argc;
char **argv;
{
  array_t *node_vec;
...
static int com_bdd_implies(network, argc, argv) network_t **network;
int argc;
char **argv;
{
  array_t *node_vec;
  ...
  (void)fprintf(
      misout, "%s set to %d %s ", node_name(n1), p1,
      ((bdd_leq(n1_bdd, n2_bdd, p1, p2) == 1) ? "forces" : "does not force"));
  (void)fprintf(misout, "%s to %d\n", node_name(n2), p2);
  return (0);
}

static int com_bdd_cofactor(network, argc, argv) network_t **network;
int argc;
char **argv;
{
  array_t *node_vec;
...
static int com_bdd_smooth(network, argc, argv) network_t **network;
int argc;
char **argv;
{
  array_t *node_vec;
...
static int com_bdd_compose(network, argc, argv) network_t **network;
int argc;
char **argv;
{
  array_t *node_vec;
  ...
  if (argc != 4 || array_n(node_vec) != 3) {
    ...
    if (n1 == NIL(node_t) || n2 == NIL(node_t) || n3 == NIL(node_t)) {
      ...
      bdd =
          bdd_compose(ntbdd_at_node(n1), ntbdd_at_node(n2), ntbdd_at_node(n3));
      bdd_print(bdd);
      bdd_free(bdd);
      status = 0;
    }
...
static int com_bdd_verify(network, argc, argv) network_t **network;
int argc;
char **argv;
{
  FILE *f1, *f2;
...
static /* boolean */ int readit(file, filename, network) FILE *file;
char *filename;
