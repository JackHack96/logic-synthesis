 ...
 */
int ntbdd_verify_network(network_t *net1, network_t *net2, order_method_t order_method,
    ...
    /*
     * If they don't have the same output sets in the first place, don't even
     * bother.
     */
    ...
    if (verify_method == ONE_AT_A_TIME) {
        state = verify_one_at_a_time(po_list1, pi_list1, leaves1, po_list2,
                                     pi_list2, leaves2, order_method);
    } else if (verify_method == ALL_TOGETHER) {
        state = verify_all_together(po_list1, pi_list1, leaves1, po_list2, pi_list2,
                                    leaves2, order_method);
    } else {
...
/*
 * Does the verification one output at a time, using different orderings each
 * time.
 */
static int
verify_one_at_a_time(array_t *po_list1, array_t *pi_list1, st_table *leaves1, array_t *po_list2, array_t *pi_list2,
                     st_table *leaves2, order_method_t order_method) {
    int     i;
    int     state;
    node_t  *po1, *po2;
    ...
    for (i = 0; i < array_n(po_list1); i++) {
        ...
        /*
         * Note that verify_all_together creates and frees a BDD manager on each
         * call.
         */
        state = verify_all_together(single_po1, pi_list1, leaves1, single_po2,
                                    pi_list2, leaves2, order_method);
        if (state == FALSE) {
...
/*
 * A node from pi_list1, and a node with the same name in pi_list2, must hash to
 * the same variable ID in leaves1 and leaves2, respectively.  This is so that
 * BDD's f1 and f2 are built using exactly the same BDD variables.
 *
 * Note: Only the first difference in outputs is found and reported.  TODO:
 * report all differences?
 */
static int
verify_all_together(array_t *po_list1, array_t *pi_list1, st_table *leaves1, array_t *po_list2, array_t *pi_list2,
                    st_table *leaves2, order_method_t order_method) {
    int         i, j;
    int         state;
    int         index;
     ...
     * to do anything with the BDDs. Order the first and derive the order for the
     * second.
     */
    ...
    /*
     * Based on the variable ordering for nodes in leaves1, assign a variable to
     * the nodes in leaves2.  Any node in pi_list2 which matches by name a node in
     * pi_list1, is assigned the same ordering value.  This is so that the BDD
     * package treats the two nodes as if they were the same. If a match is not
     * found for a node in pi_list2, then assign the node the next index value.
     * Note that even if the supports of network1 and network2 are disjoint, the
     * functions may still be equal (e.g. if they are the same constant function).
     */
    for (i = 0; i < array_n(pi_list2); i++) {
        ...
        /*
         * If no match was found, assign pi2 the next index value.  If a match was
         * found, then reset the match_found flag.
         * TODO: if ability to insert variables into order becomes available, then
         * try to interleave variables in network2 not in network1 into variable
         * ordering.  Any change to the variable ordering here should be reflected
         * in the function report_error.
         */
    ...
    /*
     * Both networks will be built in the same manager.  The important thing to
     * remember is that if 2 leaves have the same name, then they are considered
     * equivalent in the manager, because we assigned them the same variable ID.
     */
 ...
 * have the same names.
 */
static int same_orderp(array_t *list1, array_t **list2p) {
    int      i;
    int      index;
    char     *name;
    node_t   *node;
     ...
     */
    if (array_n(list1) != array_n(list2))
        return FALSE;
...
static void
report_error(array_t *pi_list1, st_table *leaves1, array_t *pi_list2, st_table *leaves2, bdd_t *f1, bdd_t *f2,
             node_t *bad_output) {
    int     i;
    int     index;
    int     value;
    ...
    /*
     * Find the differences.  If there are no differences, then there is a program
     * error.
     */
    ...
    /*
     * First extract the variable value from "cube" for each variables in
     * pi_list1.
     */
    for (i = 0; i < array_n(pi_list1); i++) {
        ...
        value = array_fetch(int, cube, index);
        if (value == 2) {
            value = 0;
        }
    ...
    /*
     * For any node name in pi_list2 that is not in pi_list1, extract the variable
     * value from "cube" for that node.  Since all such nodes have a higher index
     * value than any nodes in pi_list1, we can easily detect such nodes: their
     * index value is higher than the max index value in pi_list1.
     */
    for (i = 0; i < array_n(pi_list2); i++) {
        pi = array_fetch(node_t *, pi_list2, i);
        assert(st_lookup_int(leaves2, (char *) pi, &index));
        /*
         * If index is less than or equal to max_index, then this variable was
         * already processed in pi_list1 above.
         */
        if (index <= max_index) {
            continue;
        }
