...
void bdd_alloc_demon(node_t *node) { BDD_SET(node, NIL(bdd_t)); }
...
bdd_t *ntbdd_at_node(node_t *node) {
    if (node == NIL(node_t)) {
 ...
 */
void ntbdd_set_at_node(node_t *node, bdd_t *new_fn) {
    network_t          *network;
    bdd_manager        *manager;
    bdd_t              *old_fn;
 ...
 */
void ntbdd_free_at_node(node_t *node) {
    bdd_t *fn;
