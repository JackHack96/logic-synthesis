...
void
compute_tdc_params(node, options)
        node_t *node;
        int options;
{
    ...
    while (current_group != NULL) {
        ...
        for (i = 0; i < current_group->group_size; i++) {
            ...
            temp_delay, DELAY(node)->pin_params[j]->load);
#endif
        ...
        char *fname;
{
    ...
    (void) fprintf(sisout, "TDC parameters are %f %f %f\n",
                   tdc_model_params.K_0, tdc_model_params.K_1, tdc_model_params.K_2);
}
...
static int
count_bdd_fct_lines(my_bdd, source_group)
        bdd_t *my_bdd;
...
static int
do_count_bdd_fct_lines(my_bdd_node, first_src, last_src,
                       first_dest, last_dest, visited, source_grp)
        bdd_t *my_bdd_node;
        ...
        boolean source_grp;
{
     ...
     */
    if (first_dest <= (int) my_bdd_node_id
        && last_dest >= (int) my_bdd_node_id
        && source_grp) {
#ifdef TDC_DEBUG
    ...
    in_src = (first_src <= (int) my_bdd_node_id) &&
             (last_src >= (int) my_bdd_node_id);
...
static double
do_delay_to_output(current_group)
        pin_group_t *current_group;
{
    double temp_delay;

    if (current_group == NULL) return (0);

    temp_delay = current_group->group_delay + do_delay_to_output(current_group->next);
        ...
        pin_group_t *group;
{
    ...
    if (group->prev != NULL) {
        ...
        prev_group_output_arrival = prev_last_node_delay->arrival.rise + group->prev->group_delay;
        slack += (group->prev->fct_count) * (last_node_delay->arrival.rise - prev_group_output_arrival);
    }
...
*/
static double
tdc_group_delay(bdd, group, options)
        bdd_t *bdd;
        pin_group_t *group;
        int options;
{
...
*/
    ...
    group->group_delay = delay_eqn((group->group_size + fct_line_count), complexity);
        ...
        pin_group_t *group;
{
    ...
    for (i = first_src; i <= last_src && (last_src > 0); i++) {
        ...
        if (count > girdle_count) girdle_count = count;
    }
...
/* Create a sorted list of input signals */
    ...
    while (current_member != NULL && current_member->next != NULL) current_member = current_member->next;
    i = 0;
    while (current_member != NULL) {
    ...
    */
    ...
    current_group->group_delay = tdc_group_delay(my_bdd, current_group, info->options);
    current_group->arr_est     = (node_delay == NIL(delay_node_t) ?
                                  DELAY_NOT_SET : node_delay->arrival.rise)
    ...
    while (current_member != NULL) {
        ...
        if (node_delay->arrival.rise > current_group->arr_est) {
            ...
            current_group->group_delay = tdc_group_delay(my_bdd, current_group, info->options);
            current_group->arr_est     = node_delay->arrival.rise + current_group->group_delay;
        } else {
...
*/
    ...
    while (current_group != NULL) {
        ...
        for(i=0; i<current_group->group_size; i++) {
            node_delay = DELAY(current_member->source_node);
            (void)fprintf(sisout, " Pin %s arrival time: %f\n", node_name(current_member->source_node), node_delay->arrival.rise);
            current_member = current_member->prev;
    }
        current_group = current_group->next;
    }
 ...
 */
static network_t *
tdc_bdd_to_network(info, network)
        tdc_info_t *info;
        network_t *network;
{
    int          i, j, node_id;
...
#endif
        array_insert(
        char *, node_names, node_id, node_name(node));
    }
...
#endif
                if (node_f != NULL) node_collapse(node_f, node_g);
            }
        }
        current_group = current_group->next;
    }
...
static void
do_tdc_bdd_node_list(info, my_bdd, visited, new_table, parent_id)
        tdc_info_t *info;
        bdd_t *my_bdd;
        st_table *visited;
        st_table *new_table;
