...
enum parm_enum {
    ATIME, RTIME, LOAD, DRIVE, DEFAULT_RTIME, DEFAULT_ATIME,
        ...
        char **argv;
{
    ...
    while ((c = util_getopt(argc, argv, "?a:d:i:l:r:A:D:I:L:R:S:W:")) != EOF) {
        switch (c) {
            ...
            case 'R':
            case 'S':
            case 'W':if ((parm_val = atof(util_optarg)) == HUGE) goto usage;
                if (parm_val < 0.0) parm_val = DELAY_VALUE_NOT_GIVEN;
                switch (c) {
                    case 'A': delay_set_default_parameter(*network, DELAY_DEFAULT_ARRIVAL_RISE, parm_val);
                        delay_set_default_parameter(*network, DELAY_DEFAULT_ARRIVAL_FALL, parm_val);
                        break;
                    case 'D': delay_set_default_parameter(*network, DELAY_DEFAULT_DRIVE_RISE, parm_val);
                        delay_set_default_parameter(*network, DELAY_DEFAULT_DRIVE_FALL, parm_val);
                        break;
                    case 'I': delay_set_default_parameter(*network, DELAY_DEFAULT_MAX_INPUT_LOAD, parm_val);
                        break;
                    case 'L': delay_set_default_parameter(*network, DELAY_DEFAULT_OUTPUT_LOAD, parm_val);
                        break;
                    case 'R': delay_set_default_parameter(*network, DELAY_DEFAULT_REQUIRED_RISE, parm_val);
