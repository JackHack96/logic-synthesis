...
static int delay_sort(char **node1_p, char **node2_p) {
...
#define PRINT_DEFAULT(parm)                                                    \
  value = delay_get_default_parameter(net, parm);                              \
  PRINT_VALUE(value)

static int com_print_delay_constraints(network_t **network, int argc, char **argv) {
    array_t      *node_vec;
    int          i;
    node_t       *node;
...
#endif /* SIS */
            (void) fprintf(sisout, "Settings for output %s:\tload=", node_name(node));
            value = DELAY_NOT_SET;
...
static int com_print_delay(network_t **network, int argc, char **argv) {
    ...
    while ((c = util_getopt(argc, argv, "alm:p:f:rs")) != EOF) {
        switch (c) {
            case 'f':model_filename = util_strsav(util_optarg);
                break;
            case 'a':some_flag      = 1;
                aflag               = 1;
            ...
            case 'l':some_flag      = 1;
                ...
                if (delay_sort_flag == -1)
                    delay_sort_flag = 3;
                break;
            case 'p':print_max = atoi(util_optarg);
                break;
            case 'r':some_flag      = 1;
                rflag               = 1;
                if (delay_sort_flag == -1)
    ...
    switch (model) {
        ...
        case DELAY_MODEL_MAPPED:
            if (lib_network_is_mapped(*network)) {
                (void) fprintf(sisout, "...network mapped, using library delay model\n");
                if (!delay_trace(*network, DELAY_MODEL_LIBRARY)) {
                    (void) fprintf(siserr, "%s", error_string());
                    array_free(node_vec);
                    return 1;
        ...
        case DELAY_MODEL_UNIT_FANOUT:(void) fprintf(sisout, " ... using unit delay model (with fanout)\n");
        ...
        case DELAY_MODEL_UNIT:(void) fprintf(sisout, " ... using unit delay model (levels)\n");
        ...
        case DELAY_MODEL_TDC:(void) fprintf(sisout, " ... using tdc delay model with fanout\n");
    ...
    for (i = 0, j = 0; (j < print_max) && (i < array_n(node_vec)); i++) {
        ...
        if (argc == util_optind && node_function(node) == NODE_PO &&
            ...
            node_type(node_get_fanin(node, 0)) == INTERNAL)
            continue;
        else
            j++;
        ...
        if (!some_flag || aflag) {
            if (arrival.rise < -(INFINITY / 2) || arrival.fall < -(INFINITY / 2)) {
                ...
                (void) fprintf(sisout, "arrival=(%5.2f %5.2f) ", arrival.rise,
                               arrival.fall);
            }
        }
        if (!some_flag || rflag) {
            if (required.rise < -(INFINITY / 2) || required.fall < -(INFINITY / 2)) {
                ...
                (void) fprintf(sisout, "required=(%5.2f %5.2f) ", required.rise,
                               required.fall);
            }
        }
        if (!some_flag || sflag) {
            if (slack.rise > (INFINITY / 2) || slack.fall > (INFINITY / 2)) {
                ...
                (void) fprintf(sisout, "slack=(%5.2f %5.2f) ", slack.rise, slack.fall);
            }
        }
        if (lflag) {
    ...
    (void) fprintf(siserr, "    -m model\tchoose delay model (unit, unit-fanout, "
                           "library, mapped, tdc)\n");
    (void) fprintf(
            siserr,
...
static int com_set_delay(network_t **network, int argc, char **argv) {
    ...
    while ((c = util_getopt(argc, argv, "?a:d:i:l:r:A:D:I:L:R:S:W:")) != EOF) {
        switch (c) {
            case 'a':set_parameter = ATIME;
                ++set;
                found    = 1;
                nodetype = INPUTS;
                break;
            case 'd':set_parameter = DRIVE;
                ++set;
                found    = 1;
                nodetype = INPUTS;
                break;
            case 'i':set_parameter = MAX_INPUT_LOAD;
                ++set;
                found    = 1;
                nodetype = INPUTS;
                break;
            case 'l':set_parameter = LOAD;
                ++set;
                found    = 1;
                nodetype = OUTPUTS;
                break;
            case 'r':set_parameter = RTIME;
                ++set;
                found    = 1;
                nodetype = OUTPUTS;
                break;
            ...
            case 'S':
            case 'W':
                if ((parm_val = atof(util_optarg)) == HUGE)
                    goto usage;
                if (parm_val < 0.0)
                    parm_val  = DELAY_VALUE_NOT_GIVEN;
                switch (c) {
                    case 'A':
                    ...
                    case 'R':
                        ...
                        delay_set_default_parameter(*network, DELAY_DEFAULT_REQUIRED_FALL,
                                                    parm_val);
                        break;
                    case 'S':delay_set_default_parameter(*network, DELAY_WIRE_LOAD_SLOPE, parm_val);
                        break;
                    case 'W':delay_set_default_parameter(*network, DELAY_ADD_WIRE_LOAD, parm_val);
                        break;
                }
                break;
            default:goto usage;
    ...
    if (set) {
        ...
        switch (set_parameter) {
            ...
            case DRIVE:
                ...
                break;
            case MAX_INPUT_LOAD:rise_parm = fall_parm = DELAY_MAX_INPUT_LOAD;
                break;
            case LOAD:
                if (nodetype == INPUTS)
                    goto usage;
    ...
    usage:
    (void) fprintf(siserr, "set_delay [-a|d|i|l|r f]  [-A f] [-D f] [-I f] [-L f] "
                           "[-R f] [-S f] [-W f] [o1 o2 ... | i1 i2 ...]\n");
