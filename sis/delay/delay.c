...
int delay_trace(network_t *network, delay_model_t model) {
    int          i;
    node_t       *node;
    array_t      *nodevec;
    double       latest;
...
delay_model_t delay_get_model_from_name(char *name) {
    delay_model_t model;
...
void delay_print_wire_loads(network_t *network, FILE *fp) {
    int                     i, n;
    double                  load;
...
static void delay_print_blif_slif_wire_loads(network_t *network, FILE *fp, int slif_flag) {
    int             i;
    array_t         *pins;
...
#ifdef SIS
        io_fprintf_break(fp, " %4.2f", array_fetch(
        double, pins, i));
#else
...
void delay_print_blif_wire_loads(network_t *network, FILE *fp) { delay_print_blif_slif_wire_loads(network, fp, 0); }

void delay_print_slif_wire_loads(network_t *network, FILE *fp) { delay_print_blif_slif_wire_loads(network, fp, 1); }

#define ARRIVAL(time, new_time) _arrival(&(time), (new_time))

static void _arrival(double *time, double new_time) {
    if (new_time > *time) {
        *time = new_time;
    }
}

static void set_arrival_time(register node_t *node, delay_model_t model) {
    register int    i;
...
static delay_time_t pi_arrival_time(node_t *node) {
    delay_time_t delay;
    delay_pin_t  *pin_delay;
...
static void _required(double *time, double new_time) {
    if (new_time < *time) {
        *time = new_time;
    }
}

static void set_required_time(node_t *node, double latest, delay_model_t model) {
    int          pin;
    node_t       *fanout;
    delay_pin_t  *pin_delay;
 ...
 */
int delay_wire_required_time(node_t *node, int fanin_index, delay_model_t model, delay_time_t *req) {
    delay_pin_t  *pin_delay;
 ...
 */
int delay_wire_slack_time(node_t *node, int fanin_index, delay_model_t model, delay_time_t *slack) {
...
static delay_time_t po_required_time(node_t *node, double latest) {
    delay_time_t delay;
    delay_pin_t  *pin_delay;
...
void delay_compute_atime(node_t *node, delay_model_t model, double load) {
    DELAY(node)->load             = load;
    DELAY(node)->pin_params_valid = DELAY_MODEL_UNKNOWN;
    set_arrival_time(node, model);
}

static void set_fanout_load(node_t *node, delay_model_t model) {
    int         pin;
    lsGen       gen;
    double      load;
    node_t      *fanout;
 ...
 */
double delay_compute_fo_load(node_t *node, delay_model_t model) {
    int         pin;
    lsGen       gen;
    double      op_load, load;
    node_t      *fanout;
 ...
 */
double compute_wire_load(network_t *network, int pins) {
    int                     n;
    double                  val;
    delay_network_t         *delay;
    delay_wire_load_table_t wire_load_table;
    ...
    if (pins > n) {
        if (wire_load_table.slope > 0.0) {
            ...
            val = array_fetch(
            double, wire_load_table.pins, n - 1);
            if (n == 1) {
                return (val);
            } else {
                return (val * (1 + (pins - n)) -
                        (pins - n) * array_fetch(
                double, wire_load_table.pins, n - 2));
            }
        }
    } else {
        return (array_fetch(
        double, wire_load_table.pins, pins - 1));
    }
}
 ...
 */
delay_pin_t *get_pin_delay(node_t *node, int pin, delay_model_t model) {
    char               errmsg[1024];
    delay_pin_t        *delay;
    input_phase_t      phase;
    ...
    switch (model) {
        case DELAY_MODEL_UNIT:internal_delay_model.block = unit_delay_model.block;
        ...
        case DELAY_MODEL_UNIT_FANOUT:internal_delay_model.block = unit_delay_model_with_fanout.block;
        ...
        case DELAY_MODEL_LIBRARY:delay = (delay_pin_t *) lib_get_pin_delay(node, pin);
        ...
        case DELAY_MODEL_MAPPED:delay = (delay_pin_t *) lib_get_pin_delay(node, pin);
            if (delay == NIL(delay_pin_t)) {
                /*
        ...
        case DELAY_MODEL_TDC:
            ...
            delay = DELAY(node)->pin_params[pin];
            break;
        default:delay_error("get_pin_delay: bad model type");
            /* NOTREACHED */
    }
    return (delay);
}
 ...
 */
static delay_pin_t *io_delay(node_t *node, delay_model_t model) {
    double             val;
    delay_time_t       drive;
    static delay_pin_t pipo_delay_model; /* structure to pass data */

    switch (model) {
        case DELAY_MODEL_UNIT:return (&unit_delay_model);
        case DELAY_MODEL_UNIT_FANOUT:pipo_delay_model.block = unit_delay_model_with_fanout.block;
        ...
        case DELAY_MODEL_TDC:
            ...
            return (&pipo_delay_model);
        default: fail("io_delay: bad model");
            /*NOTREACHED*/
    }
}
 ...
 * backup.
 */
static void modelcpy(delay_pin_t *newpin, delay_pin_t *pin, node_t *node) {
 ...
 */
static void allocate_pin_params(node_t *node) {
 ...
 */
static void compute_mapped_params(node_t *node, double mode) {
    network_t    *network;
    int          i, found = 0;
    ...
    foreach_primary_output(network, gen, out_node)
    { /* only one */
 ...
 */
network_t *delay_generate_decomposition(node_t *node, double mode) {
    network_t    *network;
    int          i, found = 0;
    delay_pin_t  *pin, *pin1;
    lsGen        gen, gen1;
...
static delay_time_t compute_delay(node_t *node, delay_pin_t *pin_delay, delay_model_t model) {
    delay_time_t delay;
    ...
    switch (model) {
        ...
        case DELAY_MODEL_LIBRARY:
        case DELAY_MODEL_MAPPED:
        case DELAY_MODEL_TDC:delay.rise = pin_delay->drive.rise * DELAY(node)->load;
            ...
            break;
        default:delay_error("compute_delay: bad model type");
            /* NOTREACHED */
    }
    return delay;
}

static void delay_error(char *s) {
    if (s != NIL(char)) {
        error_append(s);
    }
...
delay_time_t delay_node_pin(node_t *node, int pin_num, delay_model_t model) {
    delay_pin_t  *pin_delay;
    delay_time_t delay;
...
delay_time_t delay_fanout_contribution(node_t *node, int pin_num, node_t *fanout, delay_model_t model) {
    delay_pin_t  *pin_delay;
...
delay_time_t delay_arrival_time(node_t *node) {
...
delay_time_t delay_required_time(node_t *node) {
    /*  Goes in when I get the name of the fns from RR
...
delay_time_t delay_slack_time(node_t *node) {
    /*  Goes in when I get the name of the fns from RR
...
node_t *delay_latest_output(network_t *network, double *latest_p) {
...
static void delay_force_pininfo(node_t *node) {
    delay_pin_t *pin_delay;
...
void delay_set_default_parameter(network_t *network, delay_param_t param, double value) {
    delay_network_t         *delay;
    delay_wire_load_table_t *wire_load_table;
    ...
    switch (param) {
        case DELAY_ADD_WIRE_LOAD:
            if (value < 0) {
                ...
                if (wire_load_table->pins == NIL(array_t)) {
                    wire_load_table->pins = array_alloc(
                    double, 10);
                }
                array_insert_last(
                double, wire_load_table->pins, value);
                wire_load_table->num_pins_set++;
            }
            return;
        case DELAY_WIRE_LOAD_SLOPE:wire_load_table->slope = value;
            return;
        case DELAY_DEFAULT_DRIVE_RISE:delay->pipo_model.drive.rise = value;
            return;
        case DELAY_DEFAULT_DRIVE_FALL:delay->pipo_model.drive.fall = value;
            return;

        case DELAY_DEFAULT_OUTPUT_LOAD:delay->pipo_model.load = value;
            return;
        case DELAY_DEFAULT_MAX_INPUT_LOAD:delay->pipo_model.max_load = value;
            return;

        case DELAY_DEFAULT_ARRIVAL_RISE:delay->default_arrival.rise = value;
            return;
        case DELAY_DEFAULT_ARRIVAL_FALL:delay->default_arrival.fall = value;
            return;

        case DELAY_DEFAULT_REQUIRED_RISE:delay->default_required.rise = value;
            return;
        case DELAY_DEFAULT_REQUIRED_FALL:delay->default_required.fall = value;
            return;

        default: fail("bad param in delay_set_default_parameter");
    }
}

void delay_set_parameter(node_t *node, delay_param_t param, double value) {
    delay_pin_t *pin_delay;
    ...
    switch (param) {
        case DELAY_BLOCK_RISE:pin_delay->block.rise = value;
            break;
        case DELAY_BLOCK_FALL:pin_delay->block.fall = value;
            break;
        case DELAY_DRIVE_RISE:pin_delay->drive.rise = value;
            break;
        case DELAY_DRIVE_FALL:pin_delay->drive.fall = value;
            break;
        case DELAY_OUTPUT_LOAD:
        case DELAY_INPUT_LOAD:pin_delay->load = value;
            break;
        case DELAY_MAX_INPUT_LOAD:pin_delay->max_load = value;
            break;
        case DELAY_PHASE:
        ...
        case DELAY_REQUIRED_RISE:pin_delay->user_time.rise = value;
            break;

        case DELAY_ARRIVAL_FALL:
        case DELAY_REQUIRED_FALL:pin_delay->user_time.fall = value;
            break;

        default: fail("bad enum value in delay_set_parameter");
            /* NOTREACHED */
    }
}

double delay_get_default_parameter(network_t *network, delay_param_t param) {
    delay_network_t *delay;
    ...
    switch (param) {
        case DELAY_DEFAULT_ARRIVAL_RISE:return delay->default_arrival.rise;
        case DELAY_DEFAULT_ARRIVAL_FALL:return delay->default_arrival.fall;
        case DELAY_DEFAULT_REQUIRED_RISE:return delay->default_required.rise;
        case DELAY_DEFAULT_REQUIRED_FALL:return delay->default_required.fall;
        case DELAY_DEFAULT_OUTPUT_LOAD:return delay->pipo_model.load;
        case DELAY_DEFAULT_MAX_INPUT_LOAD:return delay->pipo_model.max_load;
        case DELAY_DEFAULT_DRIVE_RISE:return delay->pipo_model.drive.rise;
        case DELAY_DEFAULT_DRIVE_FALL:return delay->pipo_model.drive.fall;
        case DELAY_WIRE_LOAD_SLOPE:return delay->wire_load_table.slope;
        default: fail("bad param in delay_get_global_parameter");
    }
    /*NOTREACHED*/
}

double delay_get_parameter(node_t *node, delay_param_t param) {
    pin_phase_t phase;
    delay_pin_t *pin_delay;
    ...
    switch (param) {
        case DELAY_BLOCK_RISE:return (pin_delay->block.rise);
        case DELAY_BLOCK_FALL:return (pin_delay->block.fall);
        case DELAY_DRIVE_RISE:return (pin_delay->drive.rise);
        case DELAY_DRIVE_FALL:return (pin_delay->drive.fall);
        case DELAY_OUTPUT_LOAD:
        case DELAY_INPUT_LOAD:return (pin_delay->load);
        case DELAY_MAX_INPUT_LOAD:return (pin_delay->max_load);
        case DELAY_PHASE:phase = pin_delay->phase;
        ...
        case DELAY_ARRIVAL_RISE:
        case DELAY_REQUIRED_RISE:return (pin_delay->user_time.rise);

        case DELAY_ARRIVAL_FALL:
        case DELAY_REQUIRED_FALL:return (pin_delay->user_time.fall);

        default: fail("bad enum in delay_get_parameter");
            /* NOTREACHED */
    }
}

void delay_alloc(node_t *node) {
    delay_node_t *delay;
...
void delay_dup(node_t *oldnode, node_t *newnode) {
    int          nparms, i;
 ...
 */
void delay_invalid(node_t *node) {
    /* BUG FIX: by KJ -- earlier this routine did nothing */
    DELAY(node)->pin_params_valid = DELAY_MODEL_UNKNOWN;
}

void delay_network_alloc(network_t *network) {
    delay_network_t *delay;
...
void delay_network_free(network_t *network) {
    array_t *pins;
...
void delay_network_dup(network_t *new, network_t *old) {
    array_t *oldpins;
 ...
 *  probably does ...
 */
char **delay_giveaway_pin_delay(network_t *network) {
    lsGen       gen;
    node_t      *node;
    int         i;
    char        **delay_info;
 ...
 */
double delay_get_load(char *pin_delay) { return (((delay_pin_t *) pin_delay)->load); }

delay_time_t delay_get_drive(char *pin_delay) { return (((delay_pin_t *) pin_delay)->drive); }

delay_time_t delay_get_block(char *pin_delay) { return (((delay_pin_t *) pin_delay)->block); }

double delay_get_load_limit(char *pin_delay) { return (((delay_pin_t *) pin_delay)->max_load); }

static network_t *map_node_to_network(node_t *node, double mode) {
    network_t *net1, *net2;
    library_t *library;
 ...
 */
int delay_get_po_load(node_t *node, double *load) {
    double l;
    if (node_type(node) != PRIMARY_OUTPUT) {
        return (0);
    }
...
int delay_get_pi_drive(node_t *node, delay_time_t *drive) {
    delay_time_t t;
    if (node_type(node) != PRIMARY_INPUT) {
        return (0);
    }
...
int delay_get_pi_load_limit(node_t *node, double *load_limit) {
    double l;
    if (node_type(node) != PRIMARY_INPUT) {
        return (0);
    }
...
int delay_get_pi_arrival_time(node_t *node, delay_time_t *arrival) {
    delay_time_t t;
    if (node_type(node) != PRIMARY_INPUT) {
        return (0);
    }
...
int delay_get_po_required_time(node_t *node, delay_time_t *required) {
    delay_time_t t;
    if (node_type(node) != PRIMARY_OUTPUT) {
        return (0);
    }
...
static void print_pin_delays(node_t *node, delay_model_t model) {
    int         i;
    int         pin;
    double      load;
    node_t      *fanout;
 ...
 */
int delay_get_synch_edge(node_t *node, clock_edge_t *edgep, int *flag) {
    char        *name;
    sis_clock_t *clock;
    network_t   *network;
...
void delay_copy_terminal_constraints(node_t *po_node) {
    char      *name;
    node_t    *node;
    double    value;
    network_t *network;
 ...
 */
int delay_set_synch_edge(node_t *node, clock_edge_t edge, int flag) {
