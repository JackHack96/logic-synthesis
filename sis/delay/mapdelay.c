...
delay_time_t delay_map_simulate(int nin, delay_time_t **pin_arrivals, char **model, double load) {
    register int         i;
    ...
    for (i = nin - 1; i >= 0; i--) {
        ...
        switch (pin_delay->phase) {
            case PHASE_INVERTING:ARRIVAL(arrival.rise, pin_arrival->fall + delay.rise);
                ARRIVAL(arrival.fall, pin_arrival->rise + delay.fall);
                break;

            case PHASE_NONINVERTING:ARRIVAL(arrival.rise, pin_arrival->rise + delay.rise);
                ARRIVAL(arrival.fall, pin_arrival->fall + delay.fall);
                break;

            case PHASE_NEITHER:ARRIVAL(arrival.rise, pin_arrival->fall + delay.rise);
                ARRIVAL(arrival.fall, pin_arrival->rise + delay.fall);
