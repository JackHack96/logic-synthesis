 ...
 */
void mf_read_node(graph, node_name, type) mfgptr graph;
char *node_name;
int type; /* source = 1, sink = 2, fictitious = -1,  others = 0 */
{
  ...
  if (!(n = MF_ALLOC(1, mf_node_t)))
    mf_error("Memory allocation failure", "mf_read_node");
...
int mf_remove_node(graph, name) mf_graph_t *graph;
char *name;
{
  int i, j, k;
  char *dummy;
...
mf_change_node_type(graph, node, type) mf_graph_t *graph;
mf_node_t *node;
...
void mf_read_edge(graph, node1, node2, capacity) mfgptr graph;
char *node1, *node2;
int capacity;
{
  mfeptr e;
  ...
  if (capacity < 0)
    mf_error("Negative capacity assigned", "mf_read_edge");

  if ((n1 = mf_get_node(graph, node1)) == NIL(mf_node_t)) {
    mf_error("Node undefined", "mf_read_edge");
  }
  if (n1->nout == n1->max_nout)
    reallocate_out_edge(n1);
  ...
  if (n1 == n2)
    mf_error("Self-Loop is not allowed", "mf_read_edge");
 ...
 */
int mf_reread_edge(graph, node1, node2, capacity) mfgptr graph;
char *node1, *node2;
int capacity;
{
  int i;
  ...
  if (capacity < 0)
    mf_error("Negative capacity assigned", "mf_reread_edge");
  ...
  if (n1 == n2)
    mf_error("Self-Loop is not allowed", "mf_reread_edge");
 ...
 */
void mf_error(msg1, msg2) char *msg1, *msg2;
{
  (void)fprintf(siserr, "\n");
...
void reallocate_graph_node(graph) mfgptr graph;
{
  mfnptr *ntemp;
  int i;
  ...
  if (graph->nlist != NIL(mfnptr))
    free((char *)graph->nlist);
  graph->nlist = ntemp;
} /* end of reallocate_graph_node */

void reallocate_out_edge(node) mfnptr node;
{
  int i;
  mfeptr *etemp;
  ...
  if (node->out_edge != NIL(mfeptr))
    FREE(node->out_edge);
  node->out_edge = etemp;
} /* end of reallocate_out_edge */

char *MF_calloc(a, b) int a, b;
{
  int i, size;
  char *p;
...
void mf_free_cutset(cutset) mfcptr cutset;
{
...
void mf_free_node(node) mfnptr node;
{
  int i;
...
void mf_free_graph(graph) mfgptr graph;
{
  int i;
