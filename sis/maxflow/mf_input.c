 ...
 */
void mf_read_node(mfgptr graph, char *node_name, int type) {
    mfnptr n;
    char   *name;
...
int mf_remove_node(mf_graph_t *graph, char *name) {
    int       i, j, k;
    char      *dummy;
...
void mf_change_node_type(mf_graph_t *graph, mf_node_t *node, int type) {
    if (type == 1) {
...
void mf_read_edge(mfgptr graph, char *node1, char *node2, int capacity) {
    mfeptr e;
    mfnptr n1, n2;
 ...
 */
int mf_reread_edge(mfgptr graph, char *node1, char *node2, int capacity) {
    int    i;
    mfeptr e;
    mfnptr n1, n2;
    void reallocate_out_edge();
 ...
 */
void mf_error(char *msg1, char *msg2) {
    (void) fprintf(siserr, "\n");
    (void) fprintf(siserr, "%s in routine %s\n", msg1, msg2);
    abort();
} /* end of mf_error */

void reallocate_graph_node(mfgptr graph) {
    mfnptr *ntemp;
    int    i;
...
void reallocate_out_edge(mfnptr node) {
    int    i;
    mfeptr *etemp;
...
void mf_free_cutset(mfcptr cutset) {
...
void mf_free_graph(mfgptr graph) {
    int i;
