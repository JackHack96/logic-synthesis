...
void maxflow(graph, verify) mfgptr graph;
int verify;
{
...
void augmentation(graph) mfgptr graph;
{
  mfnptr n;
  int increment;
...
void construct_cutset(graph) mfgptr graph;
{
  int i, j;
...
void mf_display_graph(outfile, graph) FILE *outfile;
mfgptr graph;
{
  int i, j;
  mfnptr n;
...
void mf_display_cutset(outfile, graph) FILE *outfile;
mfgptr graph;
{
  int i, j;
  mf_node_t *node;
...
void mf_display_flow(outfile, graph) FILE *outfile;
mfgptr graph;
{
  int i, j;
...
trace_path(graph, n) mfgptr graph;
mfnptr n;
{
  int i;
  int trace;
...
void min_cutset_check(graph) mfgptr graph;
{
  int i, j;
  void unmark_every_node();
  int trace_path();
...
void unmark_every_node(graph) mfgptr graph;
{
  int i;
...
void get_cutset(cutset) mfcptr cutset;
{
  int i, j, k;
...
mf_cutset_t *mf_get_cutset(graph, from_array, to_array,
                           flow_array) mf_graph_t *graph;
...
void maxflow_init(graph) mfgptr graph;
{
  int i, j;
  mfnptr n;
  mfeptr e;
...
void fix_loop(graph) mfgptr graph;
{
  int i, no_loop = FALSE;
  void break_loop();
...
int check_loop(node) mfnptr node;
{
  int i;
  mfnptr nnode;
...
void break_loop(graph, node1, node2) mfgptr graph;
mfnptr node1, node2;
{
  int index;
...
int mf_sizeof_cutset(graph) mfgptr graph;
{
  int i, j, num_arcs;
...
mf_node_t *mf_get_node(graph, name) mf_graph_t *graph;
char *name;
{
  char *dummy;
