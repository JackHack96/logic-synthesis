    ...
    for (i = 0; i < graph->num_of_node; i++) {
        for (j = 0; j < graph->nlist[i]->nout; j++) {
            ...
            (void) fprintf(outfile, "%-9s %-9s %-9d\n", graph->nlist[i]->name,
                           graph->nlist[i]->out_edge[j]->onode->name, graph->nlist[i]->out_edge[j]->flow);
        }
    }
    ...
    for (i = 0; i < graph->num_of_node; i++) {
        ...
        for (j  = 0; j < graph->nlist[i]->nout; j++) {
            ...
            if ((graph->nlist[i]->out_edge[j]->flag & CUTSET) &&
                (graph->nlist[i]->out_edge[j]->flow != graph->nlist[i]->out_edge[j]->capacity)) {
                (void) fprintf(siserr, "\n");
        ...
        array_t **from_array, **to_array, **flow_array;
{
    ...
    *from_array = array_alloc(
    char *, 0);
    *to_array = array_alloc(
    char *, 0);
    *flow_array = array_alloc(
    int, 0);
    ...
    for (i = 0; i < cut->narcs; i++) {
        array_insert(
        char *, *from_array, i, (cut->from_node)[i]);
        array_insert(
        char *, *to_array, i, (cut->to_node)[i]);
        array_insert(
        int, *flow_array, i, (cut->capacity)[i]);
    }
