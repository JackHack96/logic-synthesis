 ...
 *
 * $Source: /users/pchong/CVS/sis/sis/maxflow/maxflow.c,v $
 * $Author: pchong $
 * $Revision: 1.1.1.1 $
...
int verify;
{
    ...
    while (nscan) {
        for (i = 0; i < nscan->nout; i++) {
            ...
            if (!(e->onode->flag & LABELLED) && (e->flow < e->capacity)) {
                ...
                e->onode->path_edge = e;
                e->onode->increment_flow = MIN((e->capacity - e->flow),nscan->increment_flow);
    ...
    check(graph);
    if (verify) min_cutset_check(graph);
}/* end of maxflow */
...
mfgptr graph;
{
    ...
    while (n != graph->source_node) {
        if (n->direction == 1) n->path_edge->flow += increment;
        else n->path_edge->flow -= increment;
        n = n->path_node;
    }
}/* end of augmentation */
    ...
    mf_foreach_node(graph, i, node){
        mf_foreach_fanout(node, j, edge){
            if (edge->flag & CUTSET) {
                (void) fprintf(outfile,"\n");
                (void) fprintf(outfile,"%-14s %-14s %-14d\n",
		mf_node_name(node), mf_node_name(mf_head_of_edge(edge)),
...
mfgptr graph;
{
    ...
    for (i = 0; i < graph->num_of_node; i++) {
        for (j = 0; j < graph->nlist[i]->nout; j++) {
            (void) fprintf(outfile,"\n");
            (void) fprintf(outfile,"%-9s %-9s %-9d\n",graph->nlist[i]->name, graph->nlist[i]->out_edge[j]->onode->name,graph->nlist[i]->out_edge[j]->flow);
        }
    }
...
mfgptr graph;
{
    ...
    for (i = 0; i < graph->num_of_node; i++) {
        ...
        for (j = 0; j < graph->nlist[i]->nout; j++) {
            ...
            if (graph->nlist[i]->out_edge[j]->flow > graph->nlist[i]->out_edge[j]->capacity) {
                (void) fprintf(siserr,"\n");
                (void) fprintf(siserr,"Internal Error: flow is larger than the capacity!\n");
                (void) fprintf(siserr,"edge: input node = %s; output node = %s\n",graph->nlist[i]->name,graph->nlist[i]->out_edge[j]->onode->name);
                abort();
            }
            if ((graph->nlist[i]->out_edge[j]->flag & CUTSET) && (graph->nlist[i]->out_edge[j]->flow != graph->nlist[i]->out_edge[j]->capacity)) {
                (void) fprintf(siserr,"\n");
        ...
        if ((outflow != inflow) && (graph->nlist[i] != graph->source_node) && (graph->nlist[i] != graph->sink_node)) {
            (void) fprintf(siserr,"\n");
...
mfnptr n;
{
    ...
    if (n == graph->sink_node) return(TRUE);
    else {
        trace = FALSE;
        if (!(n->flag & MARKED)) {
            n->flag |= MARKED;
            for (i = 0; i < n->nout; i++) {
                if (!(n->out_edge[i]->flag & CUTSET)) {
                    if (trace_path(graph, n->out_edge[i]->onode)) trace = TRUE;
                }
            }
        }
    }
...
mfcptr cutset;
{
    ...
    /* allocate memory */
    if (!(cutset->from_node = MF_ALLOC(cutset->narcs,char *)) && 
	    cutset->narcs)
        ...
        for (j = 0; j < cutset->graph->nlist[i]->nout; j++) {
            if (cutset->graph->nlist[i]->out_edge[j]->flag & CUTSET) {
                if (cutset->graph->nlist[i]->flag & FICTITIOUS)
                    cutset->from_node[k] =
                        cutset->graph->nlist[i]->fname;
                else
                    cutset->from_node[k] =
                        cutset->graph->nlist[i]->name;
                if (cutset->graph->nlist[i]->out_edge[j]->onode->flag
                    & FICTITIOUS)
                    cutset->to_node[k] = cutset->graph->nlist[i]->out_edge[j]->onode->fname;
                else
                    cutset->to_node[k] =
                        cutset->graph->nlist[i]->out_edge[j]->onode->name;
                cutset->capacity[k] =
                    cutset->graph->nlist[i]->out_edge[j]->flow;
                k++;
            }
        }
    }
...
mfnptr node;
{
    ...
    for (i = 0; i < node->nout; i++) {
        ...
        if (!(nnode->flag & MARKED)) {
            ...
            if (check_loop(nnode) == TRUE) return(TRUE);
        }
...
mfnptr node1, node2;
{
    ...
    index = 0;
    while (node1->out_edge[index]->onode != node2) index++;
    node1->nfict++;
    node2->nfict++;
	...
	n = (mf_node_t *)dummy;
    } else mf_error("node not found","break_loop");
    n->fname = node1->name;
    ...
    mf_read_edge(graph, nname1, node2->name,
        node1->out_edge[index]->capacity);
...
mf_reverse(s)  /*  reverse string s in place */
char s[];
{
   int c,i,j;
...
mf_itoa(n,s)   /*  convert n to characters in s */
int n;
char s[];
{
   int i,sign;
