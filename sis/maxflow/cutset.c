...
/* ARGSUSED */
enum st_retval speed_add_node_to_graph(node, cap, store) char *node, **cap;
char *store;
{
  mfgptr graph;
...
array_t *cutset(network, node_table) network_t *network;
st_table *node_table;
{
  array_t *cut;
...
array_t *cutset_interface(network, node_table, edge_weight) network_t *network;
st_table *node_table;
int edge_weight;
{
...
mf_graph_t *mf_create_flow_network(network, node_table, edge_weight,
                                   p_name_node_table) network_t *network;
st_table *node_table;
int edge_weight;
...
array_t *mf_build_node_cutset(graph, name_node_table) mf_graph_t *graph;
st_table *name_node_table;
{
  int i;
  char *dummy;
