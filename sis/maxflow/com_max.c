...
/* ARGSUSED */
com_maxflow(network, argc, argv) network_t **network;
int argc;
char **argv;
{ maxflow_debug = 1 - maxflow_debug; }

/* ARGSUSED */
com_test_mf(network, argc, argv) network_t **network;
int argc;
char **argv;
{
  int i, j;
  ...
  for (i = 0; i < array_n(from); i++) {
    (void)fprintf(sisout, " %-10s%-10s  %3d\n",
                  ...
                  array_fetch(char *, from, i), array_fetch(char *, to, i),
  ...
  mf_foreach_node(graph, i, node){mf_foreach_fanin(
      node, j, edge){if (mf_is_edge_on_mincut(edge)){(void)fprintf(
      sisout, "%s -> %s : Cap = %d, flow = %d\n",

      mf_node_name(mf_tail_of_edge(edge)), mf_node_name(mf_head_of_edge(edge)),
      mf_get_edge_capacity(edge), mf_get_edge_flow(edge)
...
/* ARGSUSED */
com__run_maxflow(network, argc, argv) network_t **network;
int argc;
char **argv;
{
  int i, j, cap;
  ...
  for (i = 0; i < array_n(from_array); i++) {
    (void)fprintf(sisout, " %-10s%-10s  %3d\n",
                  ...
                  array_fetch(char *, to_array, i), array_fetch(int, flow, i)
