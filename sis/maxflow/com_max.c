...
(void)
mf_remove_node( graph,
"d");
(void)
mf_reread_edge(graph,
"g", "mf_sink", 3);
mf_change_node_type(graph, mf_get_source_node(graph),
0);
mf_change_node_type(graph, mf_get_node(graph,
"a"), 1);
(void)
fprintf(sisout,
"After modfication \n");
...
mf_read_node(graph, from,
1); /* source */

mf_read_node(graph, to,
2); /* sink */
continue;
} else if ( j == 2){
continue;
...
(void)
fprintf( sisout,
"Following maxflow/mincut found:\n");
(void)
fprintf( sisout,
"    From       to       flow\n");
for (
