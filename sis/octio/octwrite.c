...
static void initialize_net_table() {
  NetTable = st_init_table(strcmp, st_strhash);
  return;
}

static octStatus specialGetByNetName(facet, net, name) octObject *facet, *net;
char *name;
{
  char *ptr;
...
static void specialCreateNet(facet, net, name) octObject *facet, *net;
char *name;
{
  octId *id;
  ...
  if (st_lookup(NetTable, name, NIL(char *))) {
    errRaise(SIS_PKG_NAME, OCT_ERROR,
             "trying to create a net that already exists: %s", name);
  }
  if (ohCreateNet(facet, net, name) != OCT_OK) {
    errRaise(SIS_PKG_NAME, OCT_ERROR, "can not create net %s (%s)", name,
             octErrorString());
  }
  id = ALLOC(octId, 1);
  *id = net->objectId;
...
static void specialGetOrCreateNet(facet, net, name) octObject *facet, *net;
char *name;
{
...
static void attachPolicyTermProperties(term, direction,
                                       termtype) octObject *term;
...
static void attachPolicyProperties(facet) octObject *facet;
{
...
static void createInterface(facet) octObject *facet;
{
  ...
  if (octOpenFacet(&interface) < OCT_OK) {
    errRaise(SIS_PKG_NAME, OCT_ERROR, "can not open the facet %s (%s)",
             ohFormatName(&interface), octErrorString());
  }
  ...
  while (octGenerate(&tgen, &term) == OCT_OK) {
    if (ohGetByTermName(&interface, &iterm, term.contents.term.name) < OCT_OK) {
      errRaise(SIS_PKG_NAME, OCT_ERROR,
               ...
               term.contents.term.name, ohFormatName(&interface),
               octErrorString());
    }
    ...
    OH_ASSERT(ohCreatePropStr(&iterm, &dummy, "DIRECTION",
    ...
    OH_ASSERT(ohCreatePropStr(&iterm, &dummy, "TERMTYPE",
                              prop.contents.prop.value.string));
  }
...
static void createGenericMaster(node, name, view) node_t *node;
char *name, *view;
/*
 ...
 */
{
  ...
  if ((status == OCT_OLD_FACET) || (status == OCT_ALREADY_OPEN)) {
    ...
    for (i = 0; i < node_num_fanin(node); i++) {
      ...
      if (ohGetByTermName(&interface, &term, buffer) < OCT_OK) {
        errRaise(SIS_PKG_NAME, OCT_ERROR,
                 "can not get formal terminal %s on %s (%s)", buffer,
                 ohFormatName(&interface), octErrorString());
      }
      attachPolicyTermProperties(&term, "INPUT", "SIGNAL");
    }
    if (ohGetByTermName(&interface, &term, "out0") < OCT_OK) {
      errRaise(SIS_PKG_NAME, OCT_ERROR,
               "can not get formal terminal %s on %s (%s)", "out0",
               ohFormatName(&interface), octErrorString());
    }
    attachPolicyTermProperties(&term, "OUTPUT", "SIGNAL");
  ...
  errRaise(SIS_PKG_NAME, status, "can not open the generic master %s (%s)",
...
static void unpackname(name, cell, view, defaultView) char *name;
char **cell;
char **view;
char *defaultView;
{
...
static void oct_lib_get_gate_cell_and_view(node, facet, cell,
                                           view) node_t *node;
octObject *facet;
char **cell, **view;
{
  ...
  if ((gateName = lib_get_gate_name(node)) == NIL(char)) {
    ...
    (void)sprintf(path, "%s/generic-%d", facet->contents.facet.cell, pinCount);
...
static void factored_form(network, orig_node, node, fform) network_t *network;
...
static array_t *oct_lib_get_factored_function(node) node_t *node;
{
  network_t *network;
  array_t *nodes;
  int i;
...
static char *oct_lib_get_pin_name(node, index) node_t *node;
int index;
{
...
static char *oct_lib_get_out_pin_name(node, index) node_t *node;
int index;
{
  static char buffer[32];
...
static void attachLogicFunction(term, node) octObject *term;
node_t *node;
{
  ...
  if (IsGeneric) {
    ...
    switch (funct) {
    case NODE_UNDEFINED:
      errRaise(SIS_PKG_NAME, OCT_ERROR, "Function for node %s is undefined.",
               node_oct_name(node));
      return;
...
create_primary_output(facet, node) octObject *facet;
node_t *node;
{
  octObject term, net;
...
create_primary_input(facet, node) octObject *facet;
node_t *node;
{
...
void oct_connect_instance_term(facet, instance, termName,
                               netName) octObject *facet;
octObject *instance;
char *termName;
char *netName;
{
  ...
  if (ohGetByTermName(instance, &term, termName) < OCT_OK) {
    errRaise(SIS_PKG_NAME, OCT_ERROR,
             "can not get the actual terminal %s of %s (%s)", termName,
             ohFormatName(instance), octErrorString());
  }
...
void create_latch(facet, node, latch) octObject *facet;
node_t *node;
latch_t *latch;
{
 ...
 */
static char *octio_clock_name(clock, short_flag) sis_clock_t *clock;
int short_flag;
{
  node_t *node;
...
static void doEdge(edge, clockEventsBag, done_table) clock_edge_t *edge;
...
st_table *done_table;
{
  ...
  if (st_lookup_int(done_table, (char *)(edge->clock), &done)) {
    if (done == DONE_BOTH_TRANSITION)
      return;
    if (done == DONE_RISE_TRANSITION && edge->transition == RISE_TRANSITION)
      return;
    if (done == DONE_FALL_TRANSITION && edge->transition == FALL_TRANSITION)
      return;

    done = DONE_BOTH_TRANSITION;
  } else {
    done = (edge->transition == RISE_TRANSITION) ? DONE_RISE_TRANSITION
                                                 : DONE_FALL_TRANSITION;
  }
...
static void write_gen_event(sisBag, clock, transition, done_table,
                            short_flag) octObject *sisBag;
sis_clock_t *clock;
int transition;
st_table *done_table;
int short_flag;
{
  ...
  OH_ASSERT(
      ohCreatePropReal(&clockEventsBag, &prop, "TIME",
...
write_oct_clocks(network, facet, short_flag) network_t *network;
octObject *facet;
...
void write_oct(network, facet) network_t **network;
octObject *facet;
/*
 ...
 */
{
  ...
  if (octOpenFacet(facet) < OCT_OK) {
    errRaise(SIS_PKG_NAME, OCT_ERROR, "can not open %s (%s)",
             ohFormatName(facet), octErrorString());
  }
  ...
  for (i = 0; i < array_n(nodes); i++) {
    ...
    default:
      ...
      OH_ASSERT(
          ohCreateInst(facet, &instance, nodeName, masterCell, masterView));
...
int external_write_oct(network, argc, argv) network_t **network;
int argc;
char **argv;
/*
