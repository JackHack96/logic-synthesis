 ...
 *
 * $Source: /users/pchong/CVS/sis/sis/octio/octread.c,v $
 * $Author: pchong $
 * $Revision: 1.1.1.1 $
...
static node_t *
build_new_node(node, network, instance)
node_t *node;
network_t *network;
octObject *instance;
{
...
static void 
read_oct_add_clock_event(network, eventBag )
    network_t *network;
    octObject *eventBag;
{
    int first = 1;
    ...
    octObject *facet;
{
	...
	if ( ohGetByPropName(&net,&prop,"NETTYPE") != OCT_OK ) continue;
	if ( !STREQ( prop.contents.prop.value.string, "CLOCK" ) ) continue;
	count++;
...
static lsList
is_a_logic_cell(instance, is_comb_cellp)
octObject *instance;
int *is_comb_cellp;
/*
 ...
 */
{
	...
	(void)st_lookup_int(TypeTable, instance->contents.instance.master, &is_comb_cell);
	*is_comb_cellp = is_comb_cell;
	...
	if (ohGetByPropName(&term, &prop, "LOGICFUNCTION") < OCT_OK) {
	    ...
	    if ((termInfo->logicFunction = read_eqn_string(prop.contents.prop.value.string)) == NIL(network_t)) {
		errRaise(SIS_PKG_NAME, OCT_ERROR,
			 "bad equation %s (%s)",
    ...
    (void) st_insert(MasterTable, instance->contents.instance.master, (char *) outputs);
    (void) st_insert(TypeTable, instance->contents.instance.master, (char *)(*is_comb_cellp));
 ...
 */
static void oct_read_set_latch(network, latch, instance)
network_t *network;
latch_t *latch;
octObject *instance;
{
	...
	if (ohGetByPropName(&master, &prop, "CELLTYPE") == OCT_OK &&
		...
		if (STREQ(prop.contents.prop.value.string, "TRANSPARENT-LATCH")){
		    edge_triggered = 0;
		} else if (STREQ(prop.contents.prop.value.string, "MASTER-SLAVE-LATCH")){
		    edge_triggered = 1;
		}
	    }
	...
	if (edge_triggered >= 0){
		...
		if (ohGetByPropName(&term, &prop, "SYNCHTERM") == OCT_OK){
			...
			if (ohTerminalNet(&term, &net) == OCT_OK){
			    if (clock_used){
				errRaise(SIS_PKG_NAME, OCT_ERROR, 
				    "Multiple clocks used on instance %s", ohFormatName(instance));
			    } else {
				clock_used = TRUE;
				name = net.contents.net.name;
				node = read_find_or_create_node(network, name);
...
int mappedp;
{
	...
	if (library) {
		...
		if ((gate = lib_get_gate(library, gateName))) {
...
#endif /* SIS */
		    for (i = 0; i < size; i++) {
			OH_ASSERT(ohGetByTermName(&instance, &term, lib_gate_pin_name(gate, i, 1)));
			OH_ASSERT(octGenFirstContainer(&term, OCT_NET_MASK, &net));
			...
			OH_ASSERT(ohGetByTermName(&instance, &term, lib_gate_pin_name(gate, i, 0)));
			OH_ASSERT(octGenFirstContainer(&term, OCT_NET_MASK, &net));
			...
			if (node_type(outnode) == PRIMARY_INPUT || outnode->F != 0){
			    errRaise(SIS_PKG_NAME, -1, "gate `%s' redefined", node_name(outnode));
			} else {
	...
	while (lsNext(outputGen, &ptr, NIL(lsHandle)) == LS_OK) {
	    ...
	    if (ohGetByPropName(&term, &prop, "LOGICFUNCTION") == OCT_OK) {
		if ((newnetwork = read_eqn_string(prop.contents.prop.value.string)) == NIL(network_t)) {
		    errRaise(SIS_PKG_NAME, OCT_ERROR,
			     "bad equation %s (%s)",
			     prop.contents.prop.value.string,
			     error_string());
	...
	if ((node = network_find_node(network, net.contents.net.name)) == NIL(node_t)) {
	    continue;
	}
	...
	if (funct == NODE_UNDEFINED) {
	    ...
	    network_change_node_name(network, node, util_strsav(term.contents.term.name));
	} else {
	    node_t *new;
	    char *save;
	    /*
	     ...
	     */
	    network_change_node_name(network, new, util_strsav(term.contents.term.name));
	...
	errRaise(SIS_PKG_NAME, OCT_ERROR, "network check failed (%s)", error_string());
    }
...
int
external_read_oct(network, argc, argv)
network_t **network;
int argc;
char **argv;
/*
