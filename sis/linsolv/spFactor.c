...
static char RCSid[]     =
                    "@(#)$Header: /users/pchong/CVS/sis/sis/linsolv/spFactor.c,v 1.1.1.1 "
...
int spOrderAndFactor(char *eMatrix, RealNumber RHS[], RealNumber RelThreshold, RealNumber AbsThreshold,
    ...
    if (NOT Matrix->NeedsOrdering) {
         ...
         * statements indicate to the rest of the code that a full reodering is
         * required rather than a partial reordering, which occurs during a failure
         * of a fast factorization.
         */
        Step = 1;
...
int spFactor(char *eMatrix) {
    ...
    if (Matrix->NeedsOrdering) {
        return spOrderAndFactor(eMatrix, (RealVector) NULL, 0.0, 0.0,
    ...
    for (Step = 2; Step <= Size; Step++) {
        if (Matrix->DoRealDirect[Step]) { /* Update column using direct addressing
                                         scatter-gather. */
    ...
    MatrixPtr Matrix;
{
  ...
  /* Start factorization. */
  for (Step = 2; Step <= Size; Step++) {
    if (Matrix->DoCmplxDirect[Step]) { /* Update column using direct addressing
                                          scatter-gather. */
      ...
      while (pColumn->Row < Step) {
        ...
        while ((pElement = pElement->NextInCol) !=
               NULL) { /* Cmplx expr: Dest[pElement->Row] -= Mult * pElement */
      ...
      while (pColumn->Row < Step) {
        ...
        while (
            (pElement = pElement->NextInCol) !=
            NULL) { /* Cmplx expr: *pDest[pElement->Row] -= Mult * pElement */
...
static void CountMarkowitz(MatrixPtr Matrix, register RealVector RHS, int Step) {
...
static ElementPtr SearchForSingleton(MatrixPtr Matrix, int Step) {
    ...
    while (Singletons-- > 0) {
        ...
        if ((ChosenPivot = Matrix->Diag[I]) != NULL) {
            ...
            if (PivotMag > Matrix->AbsThreshold AND PivotMag >
                                                    Matrix->RelThreshold *
...
int Step;
{
  ...
  for (;;) /* Endless for loop. */
  {
    ...
    if (*pMarkowitzProduct == 1) {
      /* Case where only one element exists in row and column other than
       * diagonal. */
       ...
       * the off diagonals are placed symmetricly. */
      if (pOtherInRow != NULL AND pOtherInCol != NULL) {
        if (pOtherInRow->Col == pOtherInCol->Row) {
          LargestOffDiagonal =
              MAX(ELEMENT_MAG(pOtherInRow), ELEMENT_MAG(pOtherInCol));
...
int Step;
{
  ...
  {
    while (*(--pMarkowitzProduct) >=
           MinMarkowitzProduct) { /* Just passing through. */
    }
...
static ElementPtr SearchEntireMatrix(MatrixPtr Matrix, int Step) {
    ...
    for (I = Step; I <= Size; I++) {
        ...
        while (pElement != NULL) {
            /* Check to see if element is the largest encountered so far.  If so,
               record its magnitude and address. */
            ...
            if ((Product <= MinMarkowitzProduct)
                AND (Magnitude > Matrix->RelThreshold * LargestInCol)
                AND (Magnitude > Matrix->AbsThreshold)) {
                /* Test to see if element has lowest MarkowitzProduct yet found, or
                   whether it is tied with an element found earlier. */
                if (Product < MinMarkowitzProduct) {
                    /* Notice strict inequality in test. This is a new smallest
                     * MarkowitzProduct. */
...
static void ExchangeRowsAndCols(MatrixPtr Matrix, ElementPtr pPivot, register int Step) {
    ...
    if (Row == Col) {
        ...
        if (Row != Step) {
            ...
            Matrix->MarkowitzProd[Row] =
                    Matrix->MarkowitzRow[Row] * Matrix->MarkowitzCol[Row];
        ...
        if (Col != Step) {
            ...
            Matrix->MarkowitzProd[Col] =
                    Matrix->MarkowitzCol[Col] * Matrix->MarkowitzRow[Col];
            ...
            Matrix->Diag[Col] =
                    spcFindElementInCol(Matrix, Matrix->FirstInCol + Col, Col, Col, NO);
        }
        if (Row != Step) {
            Matrix->Diag[Row] =
                    spcFindElementInCol(Matrix, Matrix->FirstInCol + Row, Row, Row, NO);
        }
        Matrix->Diag[Step]    =
                spcFindElementInCol(Matrix, Matrix->FirstInCol + Step, Step, Step, NO);

        /* Update singleton count. */
        Matrix->MarkowitzProd[Step] =
                Matrix->MarkowitzCol[Step] * Matrix->MarkowitzRow[Step];
...
/* Exchange elements in rows while traveling from left to right. */
    ...
    if (Matrix->InternalVectorsAllocated) SWAP(int, Matrix->MarkowitzRow[Row1], Matrix->MarkowitzRow[Row2]);
...
/* Exchange elements in rows while traveling from top to bottom. */
    ...
    if (Matrix->InternalVectorsAllocated) SWAP(int, Matrix->MarkowitzCol[Col1], Matrix->MarkowitzCol[Col2]);
...
static void
ExchangeColElements(MatrixPtr Matrix, int Row1, register ElementPtr Element1, int Row2, register ElementPtr Element2,
                    int Column) {
...
static void
ExchangeRowElements(MatrixPtr Matrix, int Col1, register ElementPtr Element1, int Col2, register ElementPtr Element2,
                    int Row) {
...
static void RealRowColElimination(MatrixPtr Matrix, register ElementPtr pPivot) {
#if REAL
    ...
    while (pUpper != NULL) {
        ...
        while (pLower != NULL) {
            ...
            /* Find element in row that lines up with current lower triangular
             * element. */
...
static void ComplexRowColElimination(MatrixPtr Matrix, register ElementPtr pPivot) {
#if spCOMPLEX
    register ElementPtr pSub;
    register int Row;
    register ElementPtr pLower, pUpper;
...
static void UpdateMarkowitzNumbers(MatrixPtr Matrix, ElementPtr pPivot) {
    ...
    register int        *MarkoRow = Matrix->MarkowitzRow,
                        *MarkoCol = Matrix->MarkowitzCol;
    double              Product;
    ...
    for (ColPtr = pPivot->NextInCol; ColPtr != NULL; ColPtr = ColPtr->NextInCol) {
        ...
        if ((MarkoRow[Row] > LARGEST_SHORT_INTEGER AND MarkoCol[Row] != 0)
            OR (MarkoCol[Row] > LARGEST_SHORT_INTEGER AND MarkoRow[Row] != 0)) {
    ...
    for (RowPtr = pPivot->NextInRow; RowPtr != NULL; RowPtr = RowPtr->NextInRow) {
        ...
        /* Form Markowitz product while being cautious of overflows. */
        if ((MarkoRow[Col] > LARGEST_SHORT_INTEGER AND MarkoCol[Col] != 0)
            OR (MarkoCol[Col] > LARGEST_SHORT_INTEGER AND MarkoRow[Col] != 0)) {
