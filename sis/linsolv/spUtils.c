...
static char RCSid[] =
    "@(#)$Header: /users/pchong/CVS/sis/sis/linsolv/spUtils.c,v 1.1.1.1 "
    "2004/02/07 10:15:05 pchong Exp $";
#endif
...
ElementPtr *ppTwin1, *ppTwin2;
{
  ...
  while (pTwin1 != NULL) {
    if (ABS(pTwin1->Real) == 1.0) {
      ...
      if ((pTwin2 != NULL)
              AND(ABS(pTwin2->Real) == 1.0)) { /* Found symmetric twins. */
        if (++Twins >= 2)
          return Twins;
...
static SwapCols(Matrix, pTwin1, pTwin2)
...
/* Correct array pointers for ARRAY_OFFSET. */
  ...
  for (I = Matrix->Size; I > 0; I--) {
    ...
    while (pElement !=
           NULL) { /* Cmplx expression : Sum += Element * Vector[Col] */
...
/* Correct array pointers for ARRAY_OFFSET. */
  ...
  for (I = Matrix->Size; I > 0; I--) {
    ...
    while (pElement !=
           NULL) { /* Cmplx expression : Sum += Element * Vector[Row] */
...
void spDeterminant(eMatrix, pExponent, pDeterminant,
                   piDeterminant) RealNumber *piDeterminant;
#else
void spDeterminant(eMatrix, pExponent, pDeterminant)
#endif
...
RealNumber spPseudoCondition(eMatrix)
...
RealNumber spCondition(eMatrix, NormOfMatrix, pError)
...
static RealNumber ComplexCondition(Matrix, NormOfMatrix, pError)
...
int *pError;
{
  ...
  for (I = Size; I >= 1; I--) {
    pElement = Matrix->Diag[I]->NextInRow;
    while (pElement !=
           NULL) { /* Cmplx expr: T[I] -= T[pElement->Col] * *pElement. */
  ...
  for (I = 1; I <= Size; I++) {
    pElement = Matrix->Diag[I]->NextInRow;
    while (pElement !=
           NULL) { /* Cmplx expr: T[pElement->Col] -= T[I] * *pElement. */
  ...
  for (I = Size; I >= 1; I--) {
    ...
    pElement = pPivot->NextInCol;
    while (pElement !=
           NULL) { /* Cmplx expr: T[I] -= T[pElement->Row] * *pElement. */
...
RealNumber spRoundoff(eMatrix, Rho)
