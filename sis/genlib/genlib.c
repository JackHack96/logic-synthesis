...
static int leaf_compare(char **t1, char **t2) {
...
int gl_convert_gate_to_blif(char *name, double area, function_t *function, pin_info_t *pins, int use_nor) {
    ...
    for (i = 0; i < nlist; i++) {
        ...
        (void) fprintf(fpout, "# (%d of %d) %s-FORM of ", i + 1, nlist,
                       use_nor ? "NOR" : "NAND");
        ...
        for (pin = pins; pin != 0; pin = pin->next) {
            if (strcmp(pin->name, "*") == 0) {
                ...
                for (j = 0; j < nleafs; j++) {
                    (void) fprintf(
                            fpout, ".delay %s %s %4.3f %4.3f %4.3f %4.3f %4.3f %4.3f\n",
                            leafs[j]->name, pin->phase, pin->value[0], pin->value[1],
                            pin->value[2], pin->value[3], pin->value[4], pin->value[5]);
                }
            } else {
                for (j = 0; j < nleafs; j++) {
                ...
                (void) fprintf(
                        fpout, ".delay %s %s %4.3f %4.3f %4.3f %4.3f %4.3f %4.3f\n",
                        pin->name, pin->phase, pin->value[0], pin->value[1], pin->value[2],
                        pin->value[3], pin->value[4], pin->value[5]);
            }
        }
...
/* sequential support */
int gl_convert_latch_to_blif(char *name, double area, function_t *function, pin_info_t *pins, int use_nor,
                             latch_info_t *latch, pin_info_t *clock, constraint_info_t *constraints) {
    pin_info_t        *pin, *pinnext;
    tree_node_t       **list, **leafs;
    int               i, j, nlist, nleafs;
    FILE              *fpout = gl_out_file;
    ...
    for (i = 0; i < nlist; i++) {
        ...
        for (pin = pins; pin != 0; pin = pin->next) {
            if (strcmp(pin->name, "*") == 0) {
                ...
                for (j = 0; j < nleafs; j++) {
                    ...
                    if (strcmp(leafs[j]->name, latch->out) == 0)
                        continue;
                    (void) fprintf(
                            fpout, ".delay %s %s %4.3f %4.3f %4.3f %4.3f %4.3f %4.3f\n",
                            leafs[j]->name, pin->phase, pin->value[0], pin->value[1],
                ...
                if (strcmp(pin->name, latch->out) != 0) {
                    (void) fprintf(
                            fpout, ".delay %s %s %4.3f %4.3f %4.3f %4.3f %4.3f %4.3f\n",
                            pin->name, pin->phase, pin->value[0], pin->value[1],
                            pin->value[2], pin->value[3], pin->value[4], pin->value[5]);
                }
            }
        }
        ...
        if (clock != NIL(pin_info_t)) {
            ...
            (void) fprintf(fpout, ".delay %s %s %4.3f %4.3f %4.3f %4.3f %4.3f %4.3f\n",
                           clock->name, clock->phase, clock->value[0], clock->value[1],
                           clock->value[2], clock->value[3], clock->value[4],
                           clock->value[5]);
        } else {
            ...
            for (constr = constraints; constr != 0; constr = constr->next) {
                if (strcmp(constr->name, "*") == 0) {
                    ...
                    for (j = 0; j < nleafs; j++) {
                        ...
                        if (strcmp(leafs[j]->name, latch->out) == 0)
                            continue;
                        (void) fprintf(fpout, ".input_arrival %s %4.3f %4.3f\n",
        ...
        (void) fprintf(fpout, ".latch %s %s %s %s 0\n", latch->in, latch->out,
                       latch->type, clock == NIL(pin_info_t) ? "NIL" : clock->name);
...
static int check_internal_phase(tree_node_t *tree, int level) {
    int i;
...
static int set_functions(tree_node_t *tree, int use_nor) {
    int i;
