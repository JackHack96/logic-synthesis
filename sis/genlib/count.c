...
static int count_it(int *value, int n) {
...
static int hack_adjust(int n, int s, int p) {
    int sum, temp[100];
    ...
    switch (s) {
        ...
        default:(void) fprintf(stderr, "s >= 7 not supported.\n");
            exit(1);
    }
    return sum;
}

int gl_number_of_gates(int n, int s, int p) {
