...
static int s_sort(x, y) char *x, *y;
{
...
static int p_sort(x, y) char *x, *y;
{
  tree_node_t *tree1 = *(tree_node_t **)x, *tree2 = *(tree_node_t **)y;
...
int gl_gen_complex_gates(level, s, p, root_type, forms) int level, s, p;
...
tree_node_t ***forms;
{
  ...
  while (gl_next_partition(part, &value, &k)) {
    ...
    if (root_type == AND_NODE) {
      num1 =
          gl_gen_complex_gates(level - 1, part->value[0], p, OR_NODE, &forms1);
    } else {
      num1 =
          gl_gen_complex_gates(level - 1, s, part->value[0], AND_NODE, &forms1);
    }
...
int gl_generate_complex_gates(level, s, p, forms) int level, s, p;
tree_node_t ***forms;
{
  int num1, num2, i;
