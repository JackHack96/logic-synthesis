...
static int s_sort(char *x, char *y) {
    tree_node_t *tree1 = *(tree_node_t **) x, *tree2 = *(tree_node_t **) y;

    return tree1->s - tree2->s;
}

static int p_sort(char *x, char *y) {
    tree_node_t *tree1 = *(tree_node_t **) x, *tree2 = *(tree_node_t **) y;

    return tree1->p - tree2->p;
}

int gl_gen_complex_gates(int level, int s, int p, tree_node_type_t root_type, tree_node_t ***forms) {
