...
static void set_node_type(tree_node_t *tree, tree_node_type_t type) {
    int i;
...
static void gl_write_blif_tables(register FILE *fp, tree_node_t *tree, int use_nor_gate) {
    register int i;
...
static void gl_write_blif_inputs(FILE *fp, tree_node_t *tree, latch_info_t *latch) {
    int         i, nleafs;
    tree_node_t **leafs;
    ...
    for (i = 0; i < nleafs; i++) {
        ...
        if (latch != NIL(latch_info_t)) {
            if (strcmp(leafs[i]->name, latch->out) == 0)
                continue;
        }
        (void) putc(' ', fp);
...
void gl_write_blif(FILE *fp, tree_node_t *tree, int use_nor_gate, latch_info_t *latch) {
...
static array_t *make_tree_recur(tree_node_t **leafs, int nleafs, int level) {
    int         i, j, k;
...
static void make_tree(char **list_in, int n, char *state_in) {
...
static int gl_nand_gate_forms_recur(tree_node_t *tree, tree_node_t ***list, int use_nor_gate) {
    ...
    if (tree->nsons == 0) {
        ...
        *list = ALLOC(tree_node_t *, 1);
        ...
        for (i = 0; i < tree->nsons; i++) {
            nlist[i] =
                    gl_nand_gate_forms_recur(tree->sons[i], &(xlist[i]), use_nor_gate);
        }
        ...
        while (gl_next_combination(comb, &indices)) {
            ...
            if (all_sons_isomorphic) {
                make_tree((char **) temp_list, (int) tree->nsons, (char *) hash_table);
            } else {
                gl_permute((char **) temp_list, (int) tree->nsons, make_tree,
                           (char *) hash_table);
            }
        }
...
int gl_nand_gate_forms(tree_node_t *tree, tree_node_t ***list, int use_nor_gate) {
