...
void gl_table_by_level(int s_max, int p_max) {
    int n, s, p, num;
...
void gl_table_of_gate_count(int s_max, int p_max) {
    int s, p, level, num;
    ...
    for (s = 1; s <= s_max; s++) {
        ...
        for (p = 1; p <= p_max; p++) {
            level = s + p - 2;
            num =
                    gl_number_of_gates(level, s, p) + gl_number_of_gates(level, p, s) - 1;
            (void) printf(" %9d", num);
        }
        (void) printf("\n");
    }
...
void gl_table_of_nand_forms(int s_max, int p_max) {
    int s, p, level, num;
...
int gl_num_leafs(tree_node_t *tree) {
    int i, sum;
...
void gl_print_all_gates_genlib(int level, int s, int p, int invert_output) {
...
void gl_print_all_gates(int level, int s, int p) {
    tree_node_t **list, *tree;
    int         i, nlist;
    ...
    for (i = 0; i < nlist; i++) {
        tree = list[i];
        (void) printf("level=%d s=%d p=%d  ", tree->level, tree->s, tree->p);
        gl_print_tree(stdout, tree);
        (void) printf("\n");
    }
...
void gl_print_all_nand_forms(int level, int s, int p, int use_nor_gate, int invert_output) {
...
int gl_number_of_nand_forms(int level, int s, int p) {
...
void gl_print_nand_forms(char *string, int use_nor_gate) {
    int         i, nlist;
    tree_node_t **list, *tree;
    char        *output_name;
