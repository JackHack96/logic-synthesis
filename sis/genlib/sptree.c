...
tree_node_t *gl_alloc_leaf(name) char *name;
{
  tree_node_t *node;
...
tree_node_t *gl_alloc_node(nsons) int nsons;
{
  tree_node_t *node;
...
void gl_free_node(node) tree_node_t *node;
{
  if (node->sons != NIL(tree_node_t *))
    FREE(node->sons);
...
tree_node_t *gl_dup_tree(tree) tree_node_t *tree;
{
  int i;
  tree_node_t *node;
...
void gl_free_tree(tree) tree_node_t *tree;
{
  int i;
...
tree_node_type_t gl_reverse_type(type) tree_node_type_t type;
{ return (type == OR_NODE) ? AND_NODE : OR_NODE; }

void gl_dualize_node(tree) tree_node_t *tree;
{ tree->type = gl_reverse_type(tree->type); }

void gl_invert_tree(tree) tree_node_t *tree;
{
  int i;
...
void gl_dualize_tree(tree) tree_node_t *tree;
{
  int i;
...
void gl_make_well_formed(tree) tree_node_t *tree;
{
  int i, j, new_nsons;
...
void gl_compute_level(tree, s, p, level) tree_node_t *tree;
int *s, *p, *level;
{
  int i, p1, s1, level1;
...
void gl_print_tree_recur(fp, tree, level) FILE *fp;
tree_node_t *tree;
int level;
{
  int i;
...
void gl_print_tree(fp, tree) FILE *fp;
tree_node_t *tree;
{
...
void gl_print_tree_recur_algebraic(fp, tree, level) FILE *fp;
tree_node_t *tree;
int level;
{
  int i;
...
void gl_print_tree_algebraic(fp, tree) FILE *fp;
tree_node_t *tree;
{
  gl_assign_leaf_names(tree);
  if (tree->phase || tree->nsons == 0) {
...
int gl_qsort_compare_tree(t1, t2) tree_node_t **t1, **t2;
{ return gl_compare_tree(*t1, *t2); }

int gl_compare_tree(tree1, tree2) tree_node_t *tree1, *tree2;
{
  int result, i;
...
void gl_canonical_tree(tree) tree_node_t *tree;
{
  int i;
  ...
  qsort((char *)tree->sons, (int)tree->nsons, sizeof(tree_node_t *),
        gl_qsort_compare_tree);
}

static tree_node_t **g_forms;
static int g_num;

avl_tree *gl_hash_init() { return avl_init_table(gl_compare_tree); }

/* ARGSUSED */
void gl_hash_save(tree, value) char *tree;
char *value;
{ g_forms[g_num++] = (tree_node_t *)tree; }

int gl_hash_end(hash, forms) avl_tree *hash;
tree_node_t ***forms;
{
  g_num = 0;
...
int gl_hash_find_or_add(hash, tree) avl_tree *hash;
tree_node_t *tree;
{
  int s, p, level;
...
static void gl_assign_node_names_recur(tree, count) tree_node_t *tree;
int *count;
{
  int i;
...
void gl_assign_node_names(tree) tree_node_t *tree;
{
  int count = 0;
...
static void gl_assign_leaf_names_recur(tree, count) tree_node_t *tree;
int *count;
{
  int i;
...
void gl_assign_leaf_names(tree) tree_node_t *tree;
{
  int count = 0;
...
static void gl_get_unique_leaf_pointers_recur(tree,
                                              unique_names) tree_node_t *tree;
...
int gl_get_unique_leaf_pointers(tree, leafs) tree_node_t *tree;
tree_node_t ***leafs;
{
  st_table *unique_names;
  st_generator *gen;
...
gl_tree_dump(tree) tree_node_t *tree;
{
  int i;

  if (tree->nsons == 0) {
    (void)printf("LEAF : %s %s-NODE\n", tree->name,
                 tree->type == OR_NODE ? "OR" : "AND");
  } else {
