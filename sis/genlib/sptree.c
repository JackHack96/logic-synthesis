...
tree_node_t *gl_alloc_leaf(char *name) {
    tree_node_t *node;
...
tree_node_t *gl_alloc_node(int nsons) {
    tree_node_t *node;
...
void gl_free_node(tree_node_t *node) {
...
tree_node_t *gl_dup_tree(tree_node_t *tree) {
    int         i;
    tree_node_t *node;
...
void gl_free_tree(tree_node_t *tree) {
    int i;
...
tree_node_type_t gl_reverse_type(tree_node_type_t type) {
    return (type == OR_NODE) ? AND_NODE : OR_NODE;
}

void gl_dualize_node(tree_node_t *tree) {
    tree->type = gl_reverse_type(tree->type);
}

void gl_invert_tree(tree_node_t *tree) {
    int i;
...
void gl_dualize_tree(tree_node_t *tree) {
    int i;
...
void gl_make_well_formed(tree_node_t *tree) {
    int         i, j, new_nsons;
...
void gl_compute_level(tree_node_t *tree, int *s, int *p, int *level) {
    int i, p1, s1, level1;
...
void gl_print_tree_recur(FILE *fp, tree_node_t *tree, int level) {
    int i;
...
void gl_print_tree(FILE *fp, tree_node_t *tree) {
...
void gl_print_tree_recur_algebraic(FILE *fp, tree_node_t *tree, int level) {
    int i;
...
void gl_print_tree_algebraic(FILE *fp, tree_node_t *tree) {
    gl_assign_leaf_names(tree);
    if (tree->phase || tree->nsons == 0) {
...
int gl_qsort_compare_tree(tree_node_t **t1, tree_node_t **t2) {
    return gl_compare_tree(*t1, *t2);
}

int gl_compare_tree(tree_node_t *tree1, tree_node_t *tree2) {
    int result, i;
...
void gl_canonical_tree(tree_node_t *tree) {
    int i;
...
/* ARGSUSED */
void gl_hash_save(char *tree, char *value) {
    g_forms[g_num++] = (tree_node_t *) tree;
}

int gl_hash_end(avl_tree *hash, tree_node_t ***forms) {
    g_num   = 0;
...
int gl_hash_find_or_add(avl_tree *hash, tree_node_t *tree) {
    int s, p, level;
...
static void gl_assign_node_names_recur(tree_node_t *tree, int *count) {
    int i;
...
void gl_assign_node_names(tree_node_t *tree) {
    int count = 0;

    gl_assign_node_names_recur(tree, &count);
}

static void gl_assign_leaf_names_recur(tree_node_t *tree, int *count) {
    int i;
...
void gl_assign_leaf_names(tree_node_t *tree) {
    int count = 0;

    gl_assign_leaf_names_recur(tree, &count);
}

static void gl_get_unique_leaf_pointers_recur(tree_node_t *tree,
...
int gl_get_unique_leaf_pointers(tree_node_t *tree, tree_node_t ***leafs) {
    st_table     *unique_names;
    st_generator *gen;
    char         *key, *value;
    int          nleafs = 0;
...
void gl_tree_dump(tree_node_t *tree) {
    int i;
