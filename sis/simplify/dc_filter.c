 ...
 */
node_t *simp_dc_filter(f, dc, filter) node_t *f, *dc;
sim_filter_t filter;
{
 ...
 */
static void filter_exact(Mfdc) sm_matrix *Mfdc;
{
  array_t *chosen;
  sm_col *col;
...
static void filter_disj_support(Mfdc) sm_matrix *Mfdc;
{ (void)fdc_sm_bp_2(Mfdc); }

static void filter_size(Mfdc) sm_matrix *Mfdc;
{ (void)fdc_sm_bp_4(Mfdc); }

static void filter_fdist(Mfdc, distance) sm_matrix *Mfdc;
int distance;
{ (void)fdc_sm_bp_6(Mfdc, distance); }

static void filter_sdist(Mfdc, distance) sm_matrix *Mfdc;
int distance;
 ...
 */
node_t *simp_obsdc_filter(dc, node_table, var, nodepi) node_t *dc;
 ...
 */
static void fobsdc_sm_bp_1(M, var) sm_matrix *M;
int var;
{
  sm_row *prow;
  sm_col *pcol;
...
node_t *
    simp_obssatdc_filter(dc, f,
                         var) node_t *dc; /*obs. don't care to be filtered. */
node_t *f;
int var;
{
  sm_matrix *Mfdc;
