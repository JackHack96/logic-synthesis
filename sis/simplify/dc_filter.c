 ...
 */
node_t *simp_dc_filter(node_t *f, node_t *dc, sim_filter_t filter) {
    ...
    switch (filter) {
        case SIM_FILTER_EXACT:(void) filter_exact(Mfdc);
            break;
        case SIM_FILTER_DISJ_SUPPORT:(void) filter_exact(Mfdc);
            (void) filter_disj_support(Mfdc);
            break;
        case SIM_FILTER_SIZE:(void) filter_exact(Mfdc);
            (void) filter_size(Mfdc);
            break;
        case SIM_FILTER_FDIST:(void) filter_exact(Mfdc);
            (void) filter_fdist(Mfdc, DIST_BOUND);
            break;
        case SIM_FILTER_SDIST:(void) filter_exact(Mfdc);
            (void) filter_sdist(Mfdc, DIST_BOUND);
            break;
        case SIM_FILTER_ALL:(void) filter_exact(Mfdc);
            (void) filter_disj_support(Mfdc);
            (void) filter_size(Mfdc);
            break;
        case SIM_FILTER_NONE:break;
        default: fail("Wrong filter type");
            exit(-1);
    }
 ...
 */
static void filter_exact(sm_matrix *Mfdc) {
    array_t *chosen;
    sm_col  *col;
...
static void filter_disj_support(sm_matrix *Mfdc) { (void) fdc_sm_bp_2(Mfdc); }

static void filter_size(sm_matrix *Mfdc) { (void) fdc_sm_bp_4(Mfdc); }

static void filter_fdist(sm_matrix *Mfdc, int distance) { (void) fdc_sm_bp_6(Mfdc, distance); }

static void filter_sdist(sm_matrix *Mfdc, int distance) { (void) fdc_sm_bp_7(Mfdc, distance); }

/* removes the cubes in the observability don't care set that are unlikely
 * to be used for simplification.
 */
node_t *simp_obsdc_filter(node_t *dc, st_table *node_table, int var, node_t *nodepi) {
    ...
    n1 = node_constant(1);
    st_foreach_item(node_table, gen, &dummy, NIL(
            char *)) {
        np = (node_t *) dummy;
 ...
 */
static void fobsdc_sm_bp_1(sm_matrix *M, int var) {
    sm_row     *prow;
    sm_col     *pcol;
    sm_element *p;
    int        i, init_rows;
...
node_t *simp_obssatdc_filter(node_t *dc, node_t *f, int var) {
    sm_matrix *Mfdc;
    node_t    *new_dc;
