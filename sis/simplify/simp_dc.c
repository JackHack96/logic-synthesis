        ...
        node_t *f;
{
    ...
    for (i = 0; i < array_n(nodefanout_vec) - 1; i++) {
        ...
        if (curr.node == next.node) {
            (void) st_insert(table, (char *) curr.fanout, NIL(
            char));
            (void) st_insert(table, (char *) next.fanout, NIL(
            char));
        }
    }
        ...
        int il, ol;
{
    ...
    for (i         = 0; i < array_n(f_fanout); i++) {
        ...
        (void) st_insert(f_fanout_table, (char *) fanout, NIL(
        char));
    }
    (void) st_insert(f_fanout_table, (char *) f, NIL(
    char));
    array_free(f_fanout);
    ...
    for (i   = 0; i < array_n(f_fanin); i++) {
        ...
        (void) st_insert(dc_table, (char *) fanin, NIL(
        char));
        ...
        for (j   = 0; j < array_n(n_fanout); j++) {
            ...
            if (!st_lookup(f_fanout_table, (char *) np, &dummy)) {
                (void) st_insert(dc_table, (char *) np, NIL(
                char));
            }
        }
        array_free(n_fanout);
        ...
        node_t *f;
{
    ...
    for (i  = 0; i < array_n(f_fanin); i++) {
        ...
        if (!st_lookup(f_table, (char *) fanin, &dummy)) {
            ...
            (void) st_insert(f_table, (char *) fanin, NIL(
            char));
        }
        ...
        for (j   = 0; j < array_n(f_fanout); j++) {
            ...
            if (!st_lookup(f_table, (char *) fanout, &dummy)) {
                ...
                (void) st_insert(f_table, (char *) fanout, NIL(
                char));
            }
        }
        array_free(f_fanout);
    ...
    foreach_fanin(f, i, fanin)
            (
    void) st_insert(sup_table, (char *) fanin, NIL(
    char));
    for (i = 0; i < array_n(nlist); i++) {
        ...
        if (same_sup) {
            (void) st_insert(sup_table, (char *) np, NIL(
            char));
