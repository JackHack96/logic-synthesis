...
void simplify_node(node_t *f, sim_method_t method, sim_dctype_t dctype, sim_accept_t accept, sim_filter_t filter,
    ...
    switch (dctype) {
        case SIM_DCTYPE_NONE:DC = node_constant(0);
            break;
        case SIM_DCTYPE_FANIN:DC = simp_tfanin_dc(f, simp_fanin_level, simp_fanin_fanout_level);
            break;
        case SIM_DCTYPE_FANOUT:DC = simp_fanout_dc(f);
            break;
        case SIM_DCTYPE_INOUT:DC = simp_inout_dc(f, simp_fanin_level, simp_fanin_fanout_level);
            break;
        case SIM_DCTYPE_SUB_FANIN:DC = simp_sub_fanin_dc(f);
            break;
        case SIM_DCTYPE_ALL:DC = simp_all_dc(f);
            break;
        case SIM_DCTYPE_LEVEL:DC = simp_level_dc(f, level_table);
            break;
        case SIM_DCTYPE_UNKNOWN:
        default: fail("Error: unknown dctype");
            /* NOTREACHED */
    }

    if (simp_trace) {
        (void) sprintf(buffer, "DCb=(%d,%d,%d)", node_num_fanin(DC),
                       node_num_cube(DC), node_num_literal(DC));
    ...
    if (simp_trace) {
        (void) sprintf(buffer, "DCa=(%d,%d,%d)", node_num_fanin(DC),
                       node_num_cube(DC), node_num_literal(DC));
        (void) fprintf(sisout, "%-24s", buffer);
        (void) fflush(sisout);
    }
    ...
    /* minimize f */
    switch (method) {
        case SIM_METHOD_SIMPCOMP:node_sim_type = NODE_SIM_SIMPCOMP;
            break;
        case SIM_METHOD_ESPRESSO:node_sim_type = NODE_SIM_ESPRESSO;
            break;
        case SIM_METHOD_EXACT:node_sim_type = NODE_SIM_EXACT;
            break;
        case SIM_METHOD_EXACT_LITS:node_sim_type = NODE_SIM_EXACT_LITS;
            break;
        case SIM_METHOD_DCSIMP:node_sim_type = NODE_SIM_DCSIMP;
            break;
        case SIM_METHOD_NOCOMP:node_sim_type = NODE_SIM_NOCOMP;
            break;
        case SIM_METHOD_SNOCOMP:node_sim_type = NODE_SIM_SNOCOMP;
            break;
        case SIM_METHOD_UNKNOWN:
        default: fail("Error: unknown simplication method");
            /* NOTREACHED */
    }
...
static void simp_accept(node_t *f, node_t *newf, sim_accept_t accept) {
    bool do_accept;
    int  old, new;
    ...
    switch (accept) {
        case SIM_ACCEPT_FCT_LITS:old = factor_num_literal(f);
            new = factor_num_literal(newf);
            break;
        case SIM_ACCEPT_SOP_LITS:old = node_num_literal(f);
            new = node_num_literal(newf);
            break;
        case SIM_ACCEPT_CUBES:old = node_num_cube(f);
            new = node_num_cube(newf);
            break;
        case SIM_ACCEPT_ALWAYS:old = factor_num_literal(f);
            new       = factor_num_literal(newf);
            do_accept = TRUE;
            break;
        case SIM_ACCEPT_UNKNOWN:
        default: fail("Error: unknown acceptance criteria.");
            exit(-1);
            /* NOTREACHED */
    }
...
void simplify_cspf_node(node_t *f, sim_method_t method, sim_dctype_t dctype, sim_accept_t accept, sim_filter_t filter,
                        st_table *level_table, bdd_manager *mg, st_table *leaves) {
    node_t          *DC, *newf;
    node_sim_type_t node_sim_type;
    char            buffer[80];
...
static node_t *local_dc(node_t *f, bdd_manager *mg, st_table *leaves, sim_filter_t filter, st_table *level_table) {
