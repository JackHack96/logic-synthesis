...
static void full_simplify_usage(command) char *command;
{
...
static void simp_usage(command) char *command;
{
  (void)fprintf(siserr, "usage: %s [-d]", command);
...
int com_simplify(network, argc, argv) network_t **network;
int argc;
char **argv;
{
  int c, i;
...
int com_simp(network, argc, argv) network_t **network;
int argc;
char **argv;
{
  int c, i;
...
static bool get_cone_levels(name, par1, par2) char *name;
int *par1, *par2;
{
...
st_table *find_node_level(network) network_t *network;
{
...
int com_full_simplify(network, argc, argv) network_t **network;
int argc;
char **argv;
{
  st_table *leaves;
...
static void simp_node_to_bdd(node, manager, leaves) node_t *node;
bdd_manager *manager;
st_table *leaves;
{
