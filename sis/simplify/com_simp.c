...
static void full_simplify_usage(char *command) {
    ...
    (void) fprintf(siserr,
                   "    -l   \trestricted don't care to preserve the level");
...
static void simp_usage(char *command) {
    (void) fprintf(siserr, "usage: %s [-d]", command);
...
int com_simplify(network_t **network, int argc, char **argv) {
    int          c, i;
    array_t      *nodevec;
    node_t       *np;
    sim_accept_t accept;
    ...
    while ((c               = util_getopt(argc, argv, "m:i:f:tdl")) != EOF) {
        switch (c) {
            case 'l':dctype = SIM_DCTYPE_LEVEL;
                break;
            case 'd':dctype = SIM_DCTYPE_NONE;
                break;
            case 'm':
            ...
            case 'f':
                ...
                break;
            case 'i':dctype = SIM_DCTYPE_FANIN;
                ...
                break;
            case 't':simp_trace = TRUE;
                break;
            default:simp_usage(argv[0]);
                return 1;
        }
    }
...
int com_simp(network_t **network, int argc, char **argv) {
    int          c, i;
    array_t      *nodevec;
    node_t       *np;
    sim_method_t method;
    ...
    while ((c               = util_getopt(argc, argv, "a:d:m:i:o:f:Dt")) != EOF) {
        switch (c) {
            ...
            case 'f':
                ...
                break;
            case 'i':simp_fanin_level = atoi(util_optarg);
                switch (dctype) {
                    case SIM_DCTYPE_FANOUT:dctype = SIM_DCTYPE_INOUT;
                        break;
                    case SIM_DCTYPE_INOUT:
                    case SIM_DCTYPE_ALL:break;
                    default:dctype = SIM_DCTYPE_FANIN;
                }
                break;
            case 'o':simp_fanin_fanout_level = atoi(util_optarg);
                switch (dctype) {
                    case SIM_DCTYPE_FANOUT:dctype = SIM_DCTYPE_INOUT;
                        break;
                    case SIM_DCTYPE_INOUT:
...
static bool get_cone_levels(char *name, int *par1, int *par2) {
...
st_table *find_node_level(network_t *network) {
...
int com_full_simplify(network_t **network, int argc, char **argv) {
    st_table     *leaves;
    ...
    while ((c               = util_getopt(argc, argv, "f:o:m:t:vdl")) != EOF) {
        switch (c) {
            case 'o':x_ordering = atoi(util_optarg);
                break;
            case 't':timeout = atoi(util_optarg);
                break;
            case 'l':filter = SIM_FILTER_LEVEL;
                break;
            case 'f':
                ...
                break;
            case 'd':dctype = SIM_DCTYPE_FANIN;
                break;
            case 'm':
                if (strcmp(util_optarg, "nocomp") == 0) {
                    method = SIM_METHOD_NOCOMP;
                ...
                break;
            case 'v':hsimp_debug = TRUE;
                break;
            default:full_simplify_usage(argv[0]);
                return 1;
        }
    }
    ...
    if (dctype == SIM_DCTYPE_FANIN) {
        simplify_without_odc(*network, accept, method, dctype, filter, level_table,
                             manager, leaves);
    } else {
        simplify_with_odc(*network, accept, method, dctype, filter, level_table,
                          manager, leaves);
    }
    foreach_node(*network, gen, node) {
...
static void simp_node_to_bdd(node_t *node, bdd_manager *manager, st_table *leaves) {
