 ...
 */
void simplify_without_odc(network, accept, method, dctype, filter, level_table,
                          mg, leaves) network_t *network;
sim_accept_t accept;
sim_method_t method;
sim_dctype_t dctype;
sim_filter_t filter;
...
void simplify_with_odc(network, accept, method, dctype, filter, level_table, mg,
                       leaves) network_t *network;
sim_accept_t accept;
sim_method_t method;
sim_dctype_t dctype;
sim_filter_t filter;
 ...
 */
static void update_cspf_of_fanins(node, ANDpi, mg, leaves) node_t *node;
node_t *ANDpi;
bdd_manager *mg;
st_table *leaves;
{
 ...
 */
void cspf_alloc(node) node_t *node;
{
 ...
 *
 */
void cspf_free(node) node_t *node;
{ FREE(node->OBS); }

/* compute maximum set of permissible functions for an edge.
 *
 */
static double_node_t *compute_mspf(node, fanin) node_t *node;
node_t *fanin;
{
 ...
 * the cube.
 */
static node_t *cube_to_node(node, setp, index) node_t *node;
pset setp;
int index;
{
  node_t *n1, *n2, *n3;
 ...
 */
static node_t *compute_cspf(DC, pfanin, bool_diff, fis_table, ANDpi,
                            fofis_table) node_t *DC;
node_t *pfanin;
node_t *bool_diff;
 ...
 *
 */
bdd_t *cspf_bdd_dc(f, mg) node_t *f;
bdd_manager *mg;
{
  bdd_t *p;
 ...
 *
 */
static st_table *cspf_inout_table(f, il, ol) node_t *f;
int il, ol;
{
 ...
 *
 */
void odc_alloc(node) node_t *node;
{
 ...
 *
 */
void odc_free(node) node_t *node;
{ FREE(node->OBS); }

void find_odc_level(network) network_t *network;
{
  array_t *nodevec;
  int i, j;
  node_t *np, *fanin;
...
int level_node_cmp1(obj1, obj2) char **obj1;
char **obj2;
{
  node_t *node1, *node2;
...
int level_node_cmp2(obj1, obj2) char **obj1;
char **obj2;
{
  node_t *node1, *node2;
...
int level_node_cmp3(obj1, obj2) char **obj1;
char **obj2;
{
  node_t *node1, *node2;
 ...
 *
 */
int odc_value(nodep) node_t *nodep;
{
  node_t *np;
  int value;
  lsGen gen;
  ...
  st_foreach_item(table, sgen, (char **)&np, NIL(char *)) {
    if (np->type != PRIMARY_OUTPUT) {
