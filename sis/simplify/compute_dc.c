...
/*
 * simplifies each node using the local don't cares and a subset of
 * satisfiability don't cares generated for the nodes that have the same support
 * as node being simplified. No observability don't care is computed, therefore,
 * the local don't care is incomplete.
 */
void simplify_without_odc(network_t *network, sim_accept_t accept, sim_method_t method, sim_dctype_t dctype,
                          sim_filter_t filter, st_table *level_table, bdd_manager *mg, st_table *leaves) {
    ...
    for (i = 0; i < array_size; i++) {
        ...
        if (node->type == INTERNAL) {
            simplify_cspf_node(node, method, dctype, accept, filter, level_table, mg,
                               leaves);
        }
    }
...
/*
 * Computes compatible observability don't cares for each node. This is used
 * later to find the local don't cares (don't cares in terms of fanins of each
 * node). Also the satisfiability don't cares for a subset of nodes that can be
 * substituted in the current node is generated. The don't cares are
 * used to simplify the node using a two-level minimizer.
 */

void simplify_with_odc(network_t *network, sim_accept_t accept, sim_method_t method, sim_dctype_t dctype,
                       sim_filter_t filter, st_table *level_table, bdd_manager *mg, st_table *leaves) {
    array_t       *network_node_list; /*an array of the nodes in the network*/
    ...
    for (i = 0; i < array_size; i++) {
        ...
        for (m       = 0; m < array_n(node_list); m++) {
            ...
            (void) st_insert(fis_table, (char *) nd, NIL(char));
        }
        ...
        for (j = 0; j < array_n(fanout_list); j++) {
            ...
            if ((node_function(fanout) != NODE_PO) || (node_num_fanout(fanout) > 0)) {
                ...
                for (m      = 0; m < array_n(fanin_list); m++) {
                    ...
                    (void) st_insert(fofis_table, (char *) nd, NIL(char));
                }
...
/*
 * Because of the use of subset support filter it is possible to add new fanins
 * to a node. If the ODC for such fanins has been already computed, it is
 * necessary to update that ODC. This updating is done here.
 */
static void update_cspf_of_fanins(node_t *node, node_t *ANDpi, bdd_manager *mg, st_table *leaves) {
    int           numin, k, j, m;
    ...
    for (k = 0; k < numin; k++) {
        ...
        for (m     = 0; m < array_n(fanin_list); m++) {
            ...
            (void) st_insert(fis_table, (char *) tmpnode, NIL(char));
        }
        array_free(fanin_list);
        ...
        for (j    = 0; j < k; j++) {
            ...
            if (node_function(pfanin) != NODE_PI) {
                dcnode = compute_cspf(dcnode, pfanin, mspf_node->neg, fis_table, ANDpi,
                                      fofis_table);
            }
        }
        dcbdd     = bdd_dup(ntbdd_node_to_bdd(dcnode, mg, leaves));
 ...
 *
 */
void cspf_free(node_t *node) { FREE(node->OBS); }

/* compute maximum set of permissible functions for an edge.
 *
 */
static double_node_t *compute_mspf(node_t *node, node_t *fanin) {
 ...
 * the cube.
 */
static node_t *cube_to_node(node_t *node, pset setp, int index) {
    node_t *n1, *n2, *n3;
    node_t *fanin;
    int    k, j;
 ...
 */
static node_t *
compute_cspf(node_t *DC, node_t *pfanin, node_t *bool_diff, st_table *fis_table, node_t *ANDpi, st_table *fofis_table) {
 ...
 *
 */
bdd_t *cspf_bdd_dc(node_t *f, bdd_manager *mg) {
    bdd_t *p;
 ...
 *
 */
static st_table *cspf_inout_table(node_t *f, int il, int ol) {
 ...
 *
 */
void odc_free(node_t *node) { FREE(node->OBS); }

void find_odc_level(network_t *network) {
    array_t *nodevec;
    int     i, j;
    node_t  *np, *fanin;
    int     level, tmp;
...
int level_node_cmp1(char **obj1, char **obj2) {
    node_t *node1, *node2;
...
int level_node_cmp2(char **obj1, char **obj2) {
    node_t *node1, *node2;
...
int level_node_cmp3(char **obj1, char **obj2) {
    node_t *node1, *node2;
...
int odc_value(node_t *nodep) {
    ...
    foreach_fanout(nodep, gen, np) { st_insert(table, (char *) np, NIL(char)); }
