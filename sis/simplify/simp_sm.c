 ...
 */
sm_matrix *simp_node_to_sm(node_t *f, node_t *dc) {
    sm_matrix      *M;
    st_table       *table;
    sm_element     *element;
    sm_row         *row;
    ...
    for (i    = 0; i < f_numrows; i++) {
        ...
        foreach_fanin(f, j, fanin) {
            ...
            switch (literal) {
                case ZERO:
                case ONE:(void) st_insert(table, (char *) fanin, (char *) j);
    ...
    for (i     = 0; i < dc_numrows; i++) {
        ...
        foreach_fanin(dc, j, fanin) {
            ...
            switch (literal) {
                case ZERO:
                case ONE:col_num = get_fcol_sm(table, M, fanin);
...
static int get_fcol_sm(st_table *table, sm_matrix *M, node_t *np) {
    char *dummy;
...
void simp_sm_print(sm_matrix *M) {
    sm_row     *row;
    sm_col     *col;
    sm_element *element;
    int        i, j;
        ...
        for (j = 0; j < M->cols_size; j++) {
            ...
            if (element == NIL(sm_element)) {
                ...
                switch (sm_get(int, element)) {
                    case 0:(void) fprintf(sisout, "  0");
                        break;
                    case 1:(void) fprintf(sisout, "  1");
                        break;
                    default: fail("bad sm_element_data");
                }
            }
        }
        (void) fprintf(sisout, "\n");
 ...
 */
node_t *simp_sm_to_node(sm_matrix *M) {
    ...
    sm_foreach_row(M, prow) {
        ...
        sm_foreach_row_element(prow, p) {
            ...
            val = sm_get(int, p);
            switch (val) {
                case 0:PUTINPUT(setp, i, ZERO);
                    break;
                case 1:PUTINPUT(setp, i, ONE);
                    break;
                default: fail("bad sm_element_data");
            }
        }
