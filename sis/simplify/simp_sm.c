 ...
 */
sm_matrix *simp_node_to_sm(f, dc) node_t *f, *dc;
{
  sm_matrix *M;
  st_table *table;
  sm_element *element;
...
static int get_fcol_sm(table, M, np) st_table *table;
sm_matrix *M;
node_t *np;
{
  char *dummy;
...
void simp_sm_print(M) sm_matrix *M;
{
  sm_row *row;
  sm_col *col;
  sm_element *element;
 ...
 */
node_t *simp_sm_to_node(M) sm_matrix *M;
{
