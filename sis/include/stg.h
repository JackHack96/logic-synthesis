...
#define stg_set_num_inputs(stg, i)                                             \
  (void)g_set_g_slot_static((stg), NUM_INPUTS, (gGeneric)i)

#define stg_get_num_outputs(stg) (int)g_get_g_slot_static((stg), NUM_OUTPUTS)
#define stg_set_num_outputs(stg, i)                                            \
  (void)g_set_g_slot_static((stg), NUM_OUTPUTS, (gGeneric)i)

#define stg_get_edge_index(stg) (int)g_get_g_slot_static((stg), EDGE_INDEX)
#define stg_set_edge_index(stg, i)                                             \
  (void)g_set_g_slot_static((stg), EDGE_INDEX, (gGeneric)i)

#define stg_get_num_products(stg) (int)g_get_g_slot_static((stg), NUM_PRODUCTS)
#define stg_set_num_products(stg, i)                                           \
  (void)g_set_g_slot_static((stg), NUM_PRODUCTS, (gGeneric)i)

#define stg_get_num_states(stg) (int)g_get_g_slot_static((stg), NUM_STATES)
#define stg_set_num_states(stg, i)                                             \
  (void)g_set_g_slot_static((stg), NUM_STATES, (gGeneric)i)
