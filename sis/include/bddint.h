...
#define BDD_MAX_REFS                                                           \
  ((bdd_refcount_type)((1l << (8 * sizeof(bdd_refcount_type))) - 1))
#define BDD_MAX_TEMP_REFS                                                      \
  ((bdd_mark_type)((1l << (8 * sizeof(bdd_mark_type) - 1)) - 1))
...
#define BDD_MAX_INDEXINDEX                                                     \
  ((bdd_indexindex_type)((1l << (8 * sizeof(bdd_indexindex_type))) - 1))
#define BDD_MAX_INDEX                                                          \
  ((bdd_index_type)((1l << (8 * sizeof(bdd_index_type))) - 1))
...
struct bdd_manager_ {
  ...
  void (*transform_fn)(cmu_bdd_manager, INT_PTR, INT_PTR, INT_PTR *, INT_PTR *,
                       pointer);
...
#define FIREWALL1(bddm, cleanupcode)                                           \
  do {                                                                         \
    ...
    while ((retcode = (setjmp((bddm)->abort.context)))) {                      \
      bdd_cleanup(bddm, retcode);                                              \
      cleanupcode(bddm)->nodes_at_start = (bddm)->unique_table.entries;        \
    }                                                                          \
  } while (0)
...
#define BDD_NEW_REC(bddm, size)                                                \
  mem_new_rec((bddm)->rms[(ROUNDUP(size) - MIN_REC_SIZE) / ALLOC_ALIGNMENT])
#define BDD_FREE_REC(bddm, rec, size)                                          \
  mem_free_rec((bddm)->rms[(ROUNDUP(size) - MIN_REC_SIZE) / ALLOC_ALIGNMENT],  \
               (rec))
...
#define bdd_insert_in_cache2(bddm, op, f, g, result)                           \
  bdd_insert_in_cache31((bddm), CACHE_TYPE_TWO, (INT_PTR)(op), (INT_PTR)(f),   \
                        (INT_PTR)(g), (INT_PTR)(result))
#define bdd_lookup_in_cache2(bddm, op, f, g, result)                           \
  bdd_lookup_in_cache31((bddm), CACHE_TYPE_TWO, (INT_PTR)(op), (INT_PTR)(f),   \
                        (INT_PTR)(g), (INT_PTR *)(result))
...
#define bdd_insert_in_cache2d(bddm, op, f, g, result)                          \
  bdd_insert_in_cache31((bddm), CACHE_TYPE_TWODATA, (INT_PTR)(op),             \
                        (INT_PTR)(f), (INT_PTR)(g), (INT_PTR)(result))
#define bdd_lookup_in_cache2d(bddm, op, f, g, result)                          \
  bdd_lookup_in_cache31((bddm), CACHE_TYPE_TWODATA, (INT_PTR)(op),             \
                        (INT_PTR)(f), (INT_PTR)(g), (INT_PTR *)(result))

#define bdd_insert_in_cache1d(bddm, op, f, result1, result2)                   \
  bdd_insert_in_cache22((bddm), CACHE_TYPE_ONEDATA, (INT_PTR)(op),             \
                        (INT_PTR)(f), (INT_PTR)(result1), (INT_PTR)(result2))
#define bdd_lookup_in_cache1d(bddm, op, f, result1, result2)                   \
  bdd_lookup_in_cache22((bddm), CACHE_TYPE_ONEDATA, (INT_PTR)(op),             \
                        (INT_PTR)(f), (INT_PTR *)(result1),                    \
                        (INT_PTR *)(result2))
...
#define cache_reclaim_fn_none                                                  \
  ((int (*)(cmu_bdd_manager, cache_entry, pointer))0)
#else
