...
#define mf_foreach_node(graph, i, p)                                           \
  for (i = 0; i < (graph)->num_of_node && (p = (graph)->nlist[i]); i++)

#define mf_foreach_fanout(node, i, e)                                          \
  for (i = 0; i < (node)->nout && (e = (node)->out_edge[i]); i++)

#define mf_foreach_fanin(node, i, e)                                           \
  for (i = 0; i < (node)->nin && (e = (node)->in_edge[i]); i++)

/*
 * Query macros
 */

#define mf_node_name(node) ((node)->name)

#define mf_num_nodes(graph) ((graph)->num_of_node)
...
#define mf_get_sink_node(graph) (graph)->sink_node

#define mf_get_source_node(graph) (graph)->source_node

#define mf_get_edge_flow(edge) (edge)->flow

#define mf_get_edge_capacity(edge) (edge)->capacity

#define mf_modify_edge_capacity(edge, capacity) ((edge)->capacity = capacity)

#define mf_tail_of_edge(edge) ((edge)->inode)

#define mf_head_of_edge(edge) ((edge)->onode)

#define mf_is_edge_on_mincut(edge) ((edge)->flag & CUTSET)
