...
node_t *network_latch_end(node) node_t *node;
{
  node_type_t type;
  network_t *network;
  latch_t *latch;
 ...
 */
void network_create_latch(network, l, n1, n2) network_t *network;
latch_t **l;
node_t *n1, *n2;
{
  lsGen gen;
  ...
  if (node_type(n1) == PRIMARY_OUTPUT && node_type(n2) == PRIMARY_INPUT) {
    ...
    *l = latch_alloc();
    ...
    fail("network_create_latch: latch cannot be added between those two node "
         "types");
  }
}

void network_delete_latch(network, l) network_t *network;
latch_t *l;
{
  lsGen gen;
  char *data = NIL(char);
...
void network_delete_latch_gen(network, gen) network_t *network;
lsGen gen;
{
  node_t *input, *output;
  latch_t *latch;
...
void network_disconnect(node1, node2, po_ptr, pi_ptr) node_t *node1, *node2,
    **po_ptr, **pi_ptr;
{
  if (node1->network != node2->network) {
    fail("network_disconnect: node1 and node2 do not belong to the same "
         "network");
  }
...
void network_connect(node1, node2) node_t *node1, *node2;
{
  lsGen gen;
  ...
  foreach_fanout(node2, gen, fanout) {
    ...
    for (i = 0; i < array_n(tfo_array); i++) {
      ...
      if (n == fanin) {
        ...
        fail(
            "network_connect: connection would result in combinational cycles");
      }
    }
    (void)array_free(tfo_array);
  }
...
graph_t *network_stg(network) network_t *network;
{
  if (network == NIL(network_t)) {
...
void network_set_stg(network, stg) network_t *network;
graph_t *stg;
{
  if (network == NIL(network_t)) {
...
int network_stg_check(network) network_t *network;
{
  array_t *po_list, *ar;
  ...
  */
  ...
  /*
   * Create a manager with number of variables equal to number of primary
   * inputs.
   */
  manager = ntbdd_start_manager(st_count(leaves));

  /*
   * Create single variable BDD's for each primary input.  We do this
   * explicitly, instead of relying on BDD's to be created for the PI's when the
   * BDD's are recursively built for the PO's, because some PI's may not fanout.
   */
  foreach_primary_input(network, gen, pi) {
    if (network_get_control(network, pi) == NIL(node_t)) {
   ...
   * creating BDD's for every node in the transitive fanin of the PO's; the
   * BDD's for the primary inputs were built above.  Note: return value of
 ...
 */
static int network_is_real_pio(network, node, latchp) network_t *network;
node_t *node;
latch_t **latchp;
{
  st_table *latch_table;
...
int network_is_real_po(network, node) network_t *network;
node_t *node;
{
  latch_t *latch;
...
int network_is_real_pi(network, node) network_t *network;
node_t *node;
{
  latch_t *latch;
...
int network_is_control(network, node) network_t *network;
node_t *node;
{
  latch_t *latch;
...
node_t *network_get_control(network, control) network_t *network;
node_t *control;
{
  lsGen gen;
  node_t *fo;
 ...
 * correctly.
 */
node_t *network_add_fake_primary_output(network, node) network_t *network;
node_t *node;
{
  node_t *out;
 ...
 * other name.
 */
void network_replace_io_fake_names(network) network_t *network;
{
  lsGen gen;
  st_table *name_table;
...
static void snetwork_dfs_recur(node, visited) node_t *node;
st_table *visited;
{
  char *dummy;
  int i;
  ...
  foreach_fanin(node, i, fanin) { snetwork_dfs_recur(fanin, visited); }
  if ((node_type(node) == PRIMARY_INPUT) &&
...
st_table *snetwork_tfi_po(network) network_t *network;
{
  st_table *visited;
  lsGen gen;
  node_t *po;
