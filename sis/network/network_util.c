...
network_num_pi(network) network_t *network;
{ return lsLength(network->pi); }

network_num_po(network) network_t *network;
{ return lsLength(network->po); }

network_num_internal(network) network_t *network;
{
  return lsLength(network->nodes) -
         (lsLength(network->pi) + lsLength(network->po));
}
...
node_t *node;
{
  ...
  if (node_is_madeup_name(node->name, &value)) {
    ...
    while (((dc_net != NIL(network_t) &&
             st_lookup(dc_net->name_table, node->name, NIL(char *)))) ||
           st_lookup(network->name_table, node->name, NIL(char *))) {
      node_assign_name(node);
    }
...
node_t *node2;
{
  ...
  key = node1->name;
  assert(st_delete(network->name_table, &key, NIL(char *)));
  key = node2->name;
  assert(st_delete(network->name_table, &key, NIL(char *)));
  key = node1->short_name;
  assert(st_delete(network->short_name_table, &key, NIL(char *)));
  ...
  assert(
      !st_insert(network->short_name_table, node1->short_name, (char *)node1));
  assert(
...
lsHandle network_add_bdd(network, bdd) network_t *network;
bdd_manager *bdd;
{
  lsHandle h;
