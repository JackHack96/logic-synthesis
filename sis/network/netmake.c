...
network_t *network_from_nodevec(nodevec) array_t *nodevec;
{
  int i, j;
  st_table *table;
  ...
  for (i = 0; i < array_n(nodevec); ++i) {
    ...
    if (node->nin > 0) {
      node_replace_internal(node_copy, new_fanin, node->nin, sf_save(node->F));
    } else {
      FREE(new_fanin);
    }
...
network_t *network_create_from_node(user_node) node_t *user_node;
{
  array_t *nodevec;
  network_t *network;
