...
int net_verify_with_dc(network_t *network1, network_t *network2, int method, int verbose) {
    network_t *dc1, *dc2;
    network_t *net1, *net2;
    ...
    /*
     * Process the eql flag.  Regardless of the value of eql, return 0.  Returning
     * 1 when the networks are not equal would indicate that we don't want to
     * continue. However, we want to continue.
     */
    if (eql) {
        if (verbose) {
...
int network_verify(network_t *network1_t, network_t *network2_t, int method) {
    ...
    /*
     * Make sure that there is a 1-1 name correspondance between the primary
     * outputs of the two networks.  If not, report the differences, and then
     * return.
     */
    eql = 1;
    foreach_primary_output(network1, gen, po1) {
...
int verify_by_collapse(network_t *network1, network_t *network2) {
    ...
    /*
     * Assume calling routine has verified a 1-1 name correspondance between the
     * primary outputs of the two networks.  Quit after finding the first
     * difference.  TODO: add command line option to generate all output
     * differences.
     */
    foreach_primary_output(network1, gen, po1) {
        po2 = network_find_node(network2, po1->name);
