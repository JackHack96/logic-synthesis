...
network_t *pla_to_network(pPLA PLA) {
    network_t     *network;
    ...
    foreachi_set(PLA->F, i, p) {
        ...
        for (var = ninputs - 1; var >= 0; var--) {
            switch (GETINPUT(p, var)) {
                case ONE:set_remove(pnew, 2 * var);
                    break;
                case ZERO:set_remove(pnew, 2 * var + 1);
                    break;
            }
        }
        node_scc(node);
...
network_t *pla_to_network_single(pPLA PLA) {
    network_t     *network;
    node_t        *node, **inputs;
    int           out, ninputs;
...
pPLA espresso_read_pla(FILE *fp) {
    pPLA PLA;
...
network_t *pla_to_dcnetwork_single(pPLA PLA) {
    network_t     *network;
    node_t        *node, **inputs;
    int           out, ninputs;
    register pset last, p, pnew;
