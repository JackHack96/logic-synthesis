...
static void check_error(char *s, node_t *node) {
...
int network_check(network_t *network) {
    ...
    foreach_node(network, gen, p) {
        switch (p->type) {
            case PRIMARY_OUTPUT:foreach_primary_output(network, gen1, p1) {
                    if (p == p1) {
                        LS_ASSERT(lsFinish(gen1));
                        break;
                    }
            ...
            case PRIMARY_INPUT:foreach_primary_input(network, gen1, p1) {
                    if (p == p1) {
                        LS_ASSERT(lsFinish(gen1));
                        break;
                    }
            ...
            case INTERNAL:foreach_fanin(p, i, fanin) {
                    found_fanout = 0;
                    found_pin    = -1;
            ...
            default:check_error("node type is not PI, PO, or INTERNAL", p);
                return 0;
        }
    }
