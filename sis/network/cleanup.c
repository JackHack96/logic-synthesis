...
int network_ccleanup(network)
        network_t *network;
{
    return network_cleanup_util(network, 0, NIL(
    int));
}


int
network_cleanup_util(network, sweep_latch, latch_removed)
        network_t *network;
        int sweep_latch;
        int *latch_removed;
{
          ...
          foreach_latch(network, gen, l) {
            ...
            /* Update the DC network.  If no nodes depend on this latch
                ...
                fo_array = ALLOC(node_t *, node_num_fanout(dc_pi));
                        i = 0;
                        foreach_fanout(dc_pi, gen2, fanout) {
                                fo_array[i++] = fanout;
                        }
