...
int network_sweep_latch(latch_t *latch, st_table *reached_table) {
    ...
    if ((func == NODE_0 && (init == 0 || init == 2)) ||
        ...
        i        = 0;
        foreach_fanout(lout, gen, fanout) { fo_array[i++] = fanout; }
...
int network_sweep_node(node_t *node) {
...
int network_sweep(network_t *network) {
...
int network_csweep(network_t *network) { return network_sweep_util(network, 0, NIL(int)); }

int network_sweep_util(network_t *network, int sweep_latch, int *latch_removed) {
    ...
    while (some_change) {
        ...
        if (sweep_latch) {
            ...
            st_foreach_item(lfanin_table, sgen, (char **) &fanin, NIL(
                    char *)) {
                ...
                for (i = 0; i < array_n(latch_array); i++) {
                    ...
                    for (j = i + 1; j < array_n(latch_array); j++) {
                        ...
                        if (l2 == NIL(latch_t))
                            continue;
                        if (latch_equal(l, l2)) {
                            st_insert(delete_latch, (char *) l2, (char *) latch_get_output(l));
            ...
            st_foreach_item(delete_latch, sgen, (char **) &l, (char **) &output) {
                ...
                i        = 0;
                foreach_fanout(node, gen, fanout) { fo_array[i++] = fanout; }
