  ...
  foreach_node(network, gen, node) {
    ...
    if (node->F == 0 && type == INTERNAL) {
      if (relax_i != 0) {
        ...
        if (print_warning)
          read_error(
              "Warning: network `%s', node \"%s\" is not driven (zero assumed)",
...
lsList po_list;
{
  ...
  lsForeachItem(po_list, gen, node) {
    ...
    if (node->type == PRIMARY_INPUT) {
      ...
      (void)fprintf(
          siserr,
          "Warning: input and output named \"%s\":  renaming input \"%s\"\n",
          po_name, name);
  ...
  foreach_primary_output(network, gen, po) {
    ...
    if (node_function(node) == NODE_BUF &&
      ...
      if ((network_latch_end(fanin) == NIL(node_t))) {
        ...
        i = 0;
        foreach_fanout(node, gen1, fo) { fo_array[i++] = fo; }
        for (; i-- > 0;) {
    ...
    if (control != NIL(node_t)) {
      if (!st_is_member(table, (char *)control)) {
        ...
        if (func == NODE_1 || func == NODE_0) {
          read_error("Warning: network `%s', latch control `%s' is constant "
                     "(disconnecting)",
...
void read_filename_to_netname(network, filename) network_t *network;
char *filename;
{
  char *name, *base;
...
int slif_global;
{
  ...
  if (strcmp(word, "area") == 0 || strcmp(word, "cost") == 0) {
    ...
    if (sscanf(rest, "%s %lf %lf %lf %lf %lf %lf", word, &load, &max_load, &br,
               &dr, &bf, &df) != 7) {
...
lsList latch_order_list;
{
  ...
  if (order == 0 && !read_special_case_for_stg(network)) {
    read_error("model `%s': .latch_order & .code must be given w/ stg", name);
    return (0);
  }
  /*
   * Order the latches
...
enum patch_retval { NO_PATCHED, PATCH_ERROR, PATCH_OK };
...
int (*reader)();
{
  ...
  st_foreach_item(search_files, stgen, &name, NIL(char *)) { FREE(name); }
  ...
  if (network == NIL(network_t) || status != 1) {
    ...
    do {
      ...
      if (sfd.patch_return == NO_PATCHED) {
        ...
        st_foreach_item(models, stgen, &name, NIL(char *)) { read_error(name); }
        break;
      }
  ...
  if (network != NIL(network_t)) {
    ...
    if (stg != NIL(graph_t)) {
      ...
      if (v != NIL(vertex_t)) {
        if ((encode_length != 0) && (stg_get_state_encoding(v) == NIL(char))) {
  ...
  /* Correct the naming in the don't care network. The fake primary outputs are
   * renamed to their corresponding primary output names in the care network.
   */
  if (network != NIL(network_t)) {
    ...
    if (bformals != NIL(char *)) {
      ...
      for (k = array_n(bactuals); --k >= 0;) {
        ...
        if (snode == NIL(node_t) || is_real_pio(snet, snode) == 0) {
          read_error("model `%s', \".subckt %s\": formal argument \"%s\" not "
                     "part of `%s'",
        ...
        if (node_type(snode) == PRIMARY_OUTPUT) {
          if (node_function(bnode) != NODE_UNDEFINED) {
            read_error("model `%s', \".subckt %s\": in \"%s=%s\", \"%s\" is "
                       "output but \"%s\" already has function",
        ...
        if (st_insert(nmap, (char *)snode, (char *)bnode) != 0) {
          read_error("model `%s', \".subckt %s\": formal argument \"%s\" "
                     "specified twice",
                     bname, sname, formal);
          sfd->patch_return = PATCH_ERROR;
        }
     ...
     */
      ...
      if (ins == -1) {               /* # inputs not specified */
        ...
        (void)fprintf(siserr, "Warning: wrong number of %s to `%s' in `%s'\n",
                      ((inputs == ins) ? "outputs" : "inputs"), sname, bname);
      }
      k = 0;
    ...
    foreach_node(snet, netgen, snode) {
      ...
      case PRIMARY_OUTPUT:
        ...
        /*
          ...
          (void) fprintf(siserr,
              "Warning: `%s', \".subckt %s\": formal output \"%s\" does not
        fanout\n", bname, sname, io_node_name(snode));
        }
        */
        break;
      case INTERNAL:
        ...
        /* Run node_scc on each interface node in the hierarchy.  This is
               because the interface nodes could have the function a*a'.  If
           this is true, then when "print" is run which calls
           node_make_common_base, the number of fanins is one, but the function
           has no cubes.  This causes an error.  Running node_scc on the
           interface nodes fixes this.  Note that while reading the networks,
           node_scc is called on
...
int *status;
{
  ...
  foreach_primary_output(network, gen, po) {
    ...
    if (po_cnt == 0) {
      ...
      if (node_function(dcpo) == NODE_PO) {
        ...
        read_error("fatal: %s is not a primary output in the external don't "
                   "care network",
                   node->name);
        *status = 0;
        network_free(network);
  ...
  foreach_primary_output(dc_net, gen, dcpo) {
    if (!(network_find_node(network, dcpo->name))) {
      read_error(
          "fatal: output %s appears only in the external don't care network.\n",
          dcpo->name);
      *status = 0;
      network_free(network);
      return;
  ...
  foreach_primary_input(dc_net, gen, pi) {
    if (network_find_node(network, pi->name))
      continue;
    read_error(
        "fatal: input %s appears only in the external don't care  network.\n",
        pi->name);
    *status = 0;
    network_free(network);
    return;
