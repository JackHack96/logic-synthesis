...
static write_codes();
static write_blif_clocks();
static st_table *write_add_buffers();
#endif /* SIS */

static void write_gen_event();

void write_blif_for_bdsyn(fp, network) FILE *fp;
network_t *network;
{
  io_fput_params("\\\n", 32000);
...
void write_blif(fp, network, short_flag, netlist_flag) FILE *fp;
network_t *network;
int short_flag;
int netlist_flag;
{
...
static void do_write_blif(fp, network, short_flag,
                          netlist_flag) register FILE *fp;
network_t *network;
int short_flag;
int netlist_flag;
{
...
static void write_blif_nodes(fp, network, netlist_flag, short_flag) FILE *fp;
network_t *network;
...
static void write_blif_delay(fp, network, short_flag) FILE *fp;
network_t *network;
int short_flag;
...
static void write_blif_latches(fp, network, short_flag, netlist_flag) FILE *fp;
network_t *network;
...
static write_codes(fp, stg) FILE *fp;
graph_t *stg;
{
  vertex_t *state;
  lsGen gen;
...
static write_blif_clocks(fp, network, short_flag) FILE *fp;
network_t *network;
int short_flag;
{
 ...
 */
static char *io_clock_name(clock, short_flag) sis_clock_t *clock;
int short_flag;
{
  node_t *node;
...
static void write_gen_event(fp, clock, transition, done_table,
                            short_flag) FILE *fp;
sis_clock_t *clock;
int transition;
st_table *done_table;
...
static st_table *write_add_buffers(network) network_t *network;
{
  lsGen gen, gen1;
  latch_t *latch;
  node_t *pi, *po, *node;
