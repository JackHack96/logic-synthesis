...
static int com_read(network_t **network, int argc, char **argv) {
    ...
    while ((c = util_getopt(argc, argv, "acs")) != EOF) {
        switch (c) {
            case 'a':append = 1;
                break;
            case 's':single_output = 1;
                break;
            case 'c':single_output = 0;
                break;
            default:goto usage;
        }
    }
...
static int com_write(network_t **network, int argc, char **argv) {
...
int write_pds_sop(FILE *fp, node_t *node, int short_name) {
    ...
    switch (node_function(node)) {
        case NODE_0:(void) fprintf(fp, "GND\n");
            return;
        case NODE_1:(void) fprintf(fp, "VCC\n");
            return;
        case NODE_PO:
            (void) fprintf(fp, "%s\n",
...
static int com_print(network_t **network, int argc, char **argv) {
...
/* ARGSUSED */
int com_print_stats(network_t **network, int argc, char **argv) {
    network_t *net;
    node_t    *p, *node;
...
static int com_print_io(network_t **network, int argc, char **argv) {
    network_t *net;
    lsGen     gen;
    node_t    *node, *p;
    node_t    *fanin, *pnode;
...
/* ARGSUSED */
static int com_chname(network_t **network, int argc, char **argv) {
    if (argc != 1) {
...
static int com_reset_name(network_t **network, int argc, char **argv) {
...
/* ARGSUSED */
static int com_print_altname(network_t **network, int argc, char **argv) {
    array_t *node_vec;
    node_t  *node;
    int     i;
...
/* ARGSUSED */
int com_bdsyn(network_t **old_network, int argc, char **argv) {
    lsGen     gen;
    int       i, min_node;
    network_t *network;
    node_t    *node;
...
static int com_force_init_state_to_zero(network_t **network, int argc,
                                        char **argv) {
    latch_t *latch;
    lsGen   gen;
    array_t *node_vec;
...
static void free_dc_network(network_t *network) {
...
static int invert_polarity(network_t *network, array_t *node_vec) {
    int    i;
    node_t *inv, *node;
    lsGen  gen;
    node_t *fanout, *fanin;
...
static int com_invert_io(network_t **network, int argc, char **argv) {
    int     i, status;
    node_t  *node;
    array_t *node_vec;
