        ...
        char **argv;
{
    ...
    if (argc - util_optind == 0) {
        fp = com_open_file("-", "w", NIL(
        char *), /* silent */0);
    } else if (argc - util_optind == 1) {
        fp = com_open_file(argv[util_optind], "w", NIL(
        char *), /* silent */0);
    } else {
        goto usage;
    }
...
fprintf(fp,
"%s ",
node_decide_name(pi, short_name
));
i++;
if ( i > 9){
(void)
...
fprintf(fp,
"%s ",
node_decide_name(po, short_name
));
i++;
if ( i > 9){
(void)
...
if (literal == ZERO) (void)
fprintf(fp,
"/");
(void)
fprintf(fp,
"%s",
node_decide_name(fanin, short_name
));
first_literal = 0;
break;
}
...
(void)
fprintf(siserr,
"\t\t1 implies don't minimize each node\n");
return 1;
}
