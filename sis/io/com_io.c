...
static int com_read(network, argc, argv) network_t **network;
int argc;
char **argv;
{
...
static int com_write(network, argc, argv) network_t **network;
int argc;
char **argv;
{
  ...
  if (strcmp(argv[0], "write_blif") == 0) {
    ...
    /*	(void) fprintf(siserr,
    "The following messages reflect inadequacies of SLIF, not any user
    fault.\n\n");*/
    if (delays_flag == 0) {
      (void)fprintf(siserr, "   Delay information is not being printed because "
                            "a standard has not been\n");

      (void)fprintf(siserr, "established for SLIF. Specify -d on the command "
                            "line to force printing of delays.\n\n");
    }
    (void)fprintf(siserr, "   Clock events have no equivalent SLIF structure "
                          "and will not be printed.\n");
    (void)fprintf(siserr, "Synch edges, which depend on clock edges, will also "
                          "not be printed.\n\n");

    (void)fprintf(siserr, "   SLIF supports only rising edge D flip-flops. "
                          "Every latch is printed as a\n");
    (void)fprintf(siserr, "rising edge D-ff.  Also, SLIF does not store the "
                          "reset state for a latch.\n\n");
...
static int com_pds(network, argc, argv) network_t **network;
int argc;
char **argv;
{
...
usage:
  ...
  (void)fprintf(sisout, "-c\t\ttreat the network as a combinational one - do "
                        "not print latch eqns\n");
  (void)fprintf(sisout, "-s\t\tuse network short-name\n");
  return 1;
}

write_palasm(fp, network, short_name, combinational_flag) FILE *fp;
network_t *network;
int short_name, combinational_flag; /* combinational_flag = 1 =>  do not write
                                       out latch equations */
{
  node_t *po, *pi, *node;
  lsGen gen;
...
write_pds_sop(fp, node, short_name) FILE *fp;
node_t *node;
int short_name;
{
  ...
  case NODE_PO:
    (void)fprintf(fp, "%s\n",
                  ...
                  node_decide_name(node_get_fanin(node,
...
int po_fanout_count(n) node_t *n;
{
  node_t *fo;
  lsGen gen;
  int po_count;
...
char *node_decide_name(node, short_name) node_t *node;
int short_name;
{
  extern char *io_name();
...
static int com_print(network, argc, argv) network_t **network;
int argc;
char **argv;
{
...
/* ARGSUSED */
com_print_stats(network, argc, argv) network_t **network;
int argc;
char **argv;
{
  network_t *net;
  ...
  if (argc - util_optind >= 1) {
    node_vec =
        com_get_nodes(net, argc - util_optind + 1, argv + util_optind - 1);
    if (array_n(node_vec) < 1) {
      goto usage;
    }
    for (i = 0; i < array_n(node_vec); i++) {
      ...
      if (node_type(node) == PRIMARY_INPUT) {
        ...
        (void)fprintf(sisout, "%-10s %d terms, %d literals\n", node_name(node),
                      node_num_cube(node), node_num_literal(node));
      }
    }
    array_free(node_vec);
...
#ifdef SIS
    (void)fprintf(
        sisout, "%-14s\tpi=%2d\tpo=%2d\tnodes=%3d\tlatches=%2d\nlits(sop)=%4d",
        network_name(net), pi, po, nodes, latches, lits);
#else
    (void)fprintf(sisout, "%-14s\tpi=%2d\tpo=%2d\tnodes=%3d\tlits(sop)=%4d",
...
static int com_print_io(network, argc, argv) network_t **network;
int argc;
char **argv;
{
  network_t *net;
  ...
  if (argc - util_optind >= 1) {
    ...
    for (i = 0; i < array_n(node_vec); i++) {
      ...
      foreach_fanin(node, j, p) { (void)fprintf(sisout, " %s", node_name(p)); }
      (void)fprintf(sisout, "\n");
...
/* ARGSUSED */
static int com_chname(network, argc, argv) network_t **network;
int argc;
char **argv;
{
  if (argc != 1) {
...
static int com_reset_name(network, argc, argv) network_t **network;
int argc;
char **argv;
{
...
/* ARGSUSED */
static int com_print_altname(network, argc, argv) network_t **network;
int argc;
char **argv;
{
  array_t *node_vec;
...
/* ARGSUSED */
com_bdsyn(old_network, argc, argv) network_t **old_network; /* never used ... */
int argc;
char **argv;
{
  lsGen gen;
...
#ifdef SIS
static int com_force_init_state_to_zero(network, argc,
                                        argv) network_t **network;
int argc;
char **argv;
{
  latch_t *latch;
...
static void free_dc_network(network) network_t *network;
{
...
static int invert_polarity(network, node_vec) network_t *network;
array_t *node_vec;
{
  int i;
  node_t *inv, *node;
  ...
  for (i = 0; i < array_n(node_vec); i++) {
    ...
    if (node->type == PRIMARY_INPUT) {
      ...
      (void)fprintf(
          siserr, "\"com_io.c:invert_polarity()\" called for node != PI or PO");
      return 1;
    }
...
static int com_invert_io(network, argc, argv) network_t **network;
int argc;
char **argv;
{
  int i, status;
...
end_io() { read_register_filename(NIL(char)); }
