...
void write_eqn(FILE *fp, network_t *network, int short_name) {
    lsGen  gen;
    node_t *node;
    ...
    foreach_node(network, gen, node) { write_sop(fp, node, short_name, 0); }
}

void write_sop(FILE *fp, node_t *node, int short_name, int slif_flag) {
    register int   i, j;
    ...
    switch (node_function(node)) {
        case NODE_0:io_fputs_break(fp, "0;\n");
            return;
        case NODE_1:io_fputs_break(fp, "1;\n");
            return;
        case NODE_PO:io_write_name(fp, node_get_fanin(node, 0), short_name);
            io_fputs_break(fp, ";\n");
            return;
    }
