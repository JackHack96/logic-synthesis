...
int write_bdnet(FILE *fp, network_t *network) {
    int        i;
    lsGen      gen;
    ...
    if ((flag = com_get_flag("OCT-TECHNOLOGY")) != 0) {
        (void) fprintf(fp, "    TECHNOLOGY %s;\n", flag);
    }
    if ((flag = com_get_flag("OCT-VIEWTYPE")) != 0) {
        (void) fprintf(fp, "    VIEWTYPE %s;\n", flag);
    }
    if ((flag = com_get_flag("OCT-EDITSTYLE")) != 0) {
        (void) fprintf(fp, "    EDITSTYLE %s;\n", flag);
    }
    (void) fprintf(fp, "\n");

    (void) fprintf(fp, "INPUT");
    ...
    (void) fprintf(fp, "ENDMODEL;\n");
    return 1;
}

static void write_instance_name(FILE *fp, char *path, char *viewname, node_t *node) {
    char *gate, *view;
