...
int read_line(FILE *fp, char **linep, int *lenp) {
    int  count, c;
    char *line;
    int  len;
 ...
 */
static int read_blif_loop(FILE *fp, st_table *models, st_table *search_files, network_t **first_network) {
    ...
    while (read_line(fp, &line, &line_len) != EOF) {
        ...
        if (*word == '.') {
            ...
            if (strcmp(word, "model") == 0 || strcmp(word, "circuit") == 0) {
                ...
                error_status =
                        read_search_file(word, models, search_files, read_blif_loop);
            ...
            } else if (strcmp(word, "clock") == 0 || strcmp(word, "clocks") == 0) {
 ...
 */
int read_blif(FILE *fp, network_t **networkp) {
    int status;
 ...
 * lib_read_blif().
 */
int read_blif_first(FILE *fp, network_t **networkp) {
    int i;
 ...
 */
static int read_node_list(network_t *network, node_t ***list) {
    int    nlist, list_max;
    char   *word;
    node_t *node;
 ...
 */
static int read_cover(FILE *fp, node_t *node, node_t **fanin, int ninput, char **linep, int *lenp) {
    register int  i, c;
    register pset pdest;
    pset_family   f;
    char          *word, *rest;
    ...
    while ((c = read_line(fp, linep, lenp)) != EOF) {
        ...
        if (ninput > 0) {
            ...
            for (i = 0; i < ninput; i++) {
                switch (word[i]) {
                    case '0':set_insert(pdest, 2 * i);
                        break;
                    case '1':set_insert(pdest, 2 * i + 1);
                        break;
                    case '2':
                    case '-':set_insert(pdest, 2 * i);
 ...
 */
static int read_wordlist(char ***wordlist) {
    int  i        = 0;
    int  list_max = 10;
    char *word;
 ...
 */
static int read_blif_subckt(network_t *network, st_table *models) {
    ...
    if (strchr(name, '=') != NIL(char)) {
        (void) fprintf(siserr,
                       "\"%s\", line %d: calling subckt %s  (contains `=')\n",
    ...
    for (i = 0; i < nwords; i++) {
        ...
        if (actual == NIL(char) || (*actual++ = '\0', *actual == '\0')) {
            read_error("bad format for formal=actual in .subckt: %s", word_list[i]);
            FREE(word_list);
            return (0);
        }
 ...
 */
static int read_blif_gate(network_t *network, int is_mlatch) {
    ...
    for (i = 0; i < nwords; i++) {
        ...
        actual = strchr(word, '=');
        if (actual == NIL(char) || (*actual++ = '\0', *actual == '\0')) {
            read_error("bad format for formal=actual in %s: %s", gm[is_mlatch],
                       word_list[i]);
            FREE(word_list);
            return (0);
        }
    ...
    if (lib_set_gate(node, gate, word_list, actual_data, nwords) == 0) {
        read_error("error loading gate '%s' (have the correct library?)", name);
        FREE(word_list);
        FREE(actual_data);
        return (0);
    }
 ...
 */
static int read_blif_latch(network_t *network) {
    int           n_params, value;
    char          **word_list;
    ...
    if (n_params >= 4) {
        ...
        if (strcmp(word_list[2], "fe") == 0)
            type = FALLING_EDGE;
        else if (strcmp(word_list[2], "re") == 0)
            type = RISING_EDGE;
        else if (strcmp(word_list[2], "ah") == 0)
            type = ACTIVE_HIGH;
        else if (strcmp(word_list[2], "al") == 0)
            type = ACTIVE_LOW;
        else if (strcmp(word_list[2], "as") == 0)
            type = ASYNCH;
        else {
...
static int read_blif_clock(network_t *network) {
    int         i, nlist;
    node_t      *node;
    char        **word_list;
    sis_clock_t *clock;
    ...
    for (i = 0; i < nlist; i++) {
        ...
        if (!clock_add_to_network(network, clock)) {
            read_error("clock %s is already part of the network", clock_name(clock));
            clock_free(clock);
            FREE(word_list);
            return (0);
        }
...
static int read_blif_clock_event(network_t *network) {
    ...
    while ((word = gettoken(rest, &rest)) != NIL(char)) {
        if (*word == '(') { /* min and max specified */
            ...
            rest = strchr(rest, ')');
            if (rest == NIL(char)) {
                read_error("warning: missing parentheses in event specifcation for "
                           "clock edge %s",
                           word);
                return (0);
            }
...
static int read_blif_code(network_t *network) {
    int      nlist;
    char     **word_list;
    graph_t  *stg;
    vertex_t *v;
