...
static int read_statement(fp, str, len) FILE *fp;
char *str;
int len;
{
  int c, last, count;
 ...
 */
static int read_slif_loop(fp, models, search_files, first_model) FILE *fp;
...
network_t *read_slif(fp) FILE *fp;
{
  network_t *network;
 ...
 * 1 no error
 */
int read_slif_call(fp, network, models, name) FILE *fp;
network_t *network;
st_table *models;
char *name;
{
...
static int read_slif_nodelist(network, line, actuals, flag) network_t *network;
char *line;
array_t *actuals;
...
static int read_slif_latch(net, line, pos) network_t *net;
char *line, *pos;
{
 ...
 * 1 ok
 */
int slif_add_to_library(network) network_t *network;
{
  FILE *fp;
  char *name, buf[128];
  node_t *node;
