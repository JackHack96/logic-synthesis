...
static int read_statement(FILE *fp, char *str, int len) {
    int  c, last, count;
    char *start = str;
 ...
 */
static int read_slif_loop(FILE *fp, st_table *models, st_table *search_files, network_t **first_model) {
    ...
    while ((i = read_statement(fp, line, sizeof(line))) != EOF ||
           include_depth > 0) {
        if (i == EOF) {
            ...
            if (strcmp(command, "model") == 0) {
                ...
                exit_status =
                        read_search_file(arg1, models, search_files, read_slif_loop);
                ...
                if (!strcmp(command, "attribute") ||
                    !strcmp(command, "global_attribute")) {
                    if (arg1 == NIL(char)) {
                        ...
                        if (node == NIL(node_t)) {
                            ...
                            if (node_function(n2) != NODE_UNDEFINED) {
                                if (have_function == 1) {
                                    ...
                                    read_error(".net: %s & %s already have functions", buf,
                                               node_name(n2));
                                    exit_status = 0;
                                    break;
                                }
 ...
 * 1 no error
 */
int read_slif_call(FILE *fp, network_t *network, st_table *models, char *name) {
...
static int read_slif_nodelist(network_t *network, char *line, array_t *actuals, int flag) {
    node_t *node;
...
static int read_slif_latch(network_t *net, char *line, char *pos) {
 ...
 * 1 ok
 */
int slif_add_to_library(network_t *network) {
    FILE        *fp;
    char        *name, buf[128];
    node_t      *node;
    double      input, area;
    ...
    foreach_primary_input(network, gen, node) {
        ...
        if (input == DELAY_NOT_SET) {
            ...
            (void) fprintf(fp, "%s %g %g %g %g %g %g\n",
                           ...
                           input, delay_get_parameter(node, DELAY_MAX_INPUT_LOAD),
