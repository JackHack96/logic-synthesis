...
int write_slif(fp, network, short_flag, netlist_flag, delay_flag) FILE *fp;
network_t *network;
...
static void write_slif_node(fp, node, netlist_flag, short_flag,
                            latch_in) FILE *fp;
node_t *node;
...
static void write_slif_delay(fp, network) FILE *fp;
network_t *network;
{ write_blif_slif_delay(fp, network, 1); }

#ifdef SIS

static void write_slif_latches(fp, network, short_flag, netlist_flag) FILE *fp;
network_t *network;
int netlist_flag, short_flag;
{
  lsGen gen;
 ...
 */
static int write_slif_ignore_node(node) node_t *node;
{
  node_t *fanin;
  int len1, len2;
