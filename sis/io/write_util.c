...
/*VARARGS2*/
void io_fprintf_break(FILE *fp, char *fmt, ...) {
    va_list args;
    char    buf[MAX_WORD];
    ...
    foreach_primary_input(network, gen, pi) {
        ...
        if (network_is_real_pi(network, pi) != 0) {
            ...
            if (get_delay_param(pi, DELAY_ARRIVAL_RISE, &delay)) {
                io_fprintf_break(fp, ".%sinput_arrival %s %4.2f %4.2f", method,
                                 io_name(pi, short_flag), delay.rise, delay.fall);
#ifdef SIS
                if (slif == 0) {
    ...
    foreach_primary_output(network, gen, po) {
        ...
        if (network_is_real_po(network, po) != 0) {
            ...
            if (get_delay_param(po, DELAY_REQUIRED_RISE, &delay)) {
                io_fprintf_break(fp, ".%soutput_required %s %4.2f %4.2f", method,
                                 io_name(po, short_flag), delay.rise, delay.fall);
#ifdef SIS
                if (slif == 0) {
                                                 ...
                                                 int short_flag;
{
    ...
    if (delay_get_synch_edge(pio, &edge, &flag) != 0) {
        if ((node = network_find_node(node_network(pio), clock_name(edge.clock))) ==
            NIL(node_t)) {
            fail("Clock specified in sychronization edge is not in network");
        }
        io_fprintf_break(fp, " %c %c'%s", ((flag == BEFORE_CLOCK_EDGE) ? 'b' : 'a'),
                                                         ...
                                                         delay_time_t *delay;
{
    switch (param) {
        ...
        case DELAY_DEFAULT_MAX_INPUT_LOAD:
            ...
            delay->fall = 0;
            break;
        case DELAY_WIRE_LOAD_SLOPE:delay->rise = delay_get_default_parameter(network, DELAY_WIRE_LOAD_SLOPE);
            delay->fall                        = 0;
            break;
    }
                                              ...
                                              delay_time_t *delay;
{
    switch (param) {
        case DELAY_ARRIVAL_RISE:delay->rise = delay_get_parameter(node, DELAY_ARRIVAL_RISE);
            ...
            break;
        case DELAY_DRIVE_RISE:delay->rise = delay_get_parameter(node, DELAY_DRIVE_RISE);
            delay->fall                   = delay_get_parameter(node, DELAY_DRIVE_FALL);
            break;
        case DELAY_REQUIRED_RISE:delay->rise = delay_get_parameter(node, DELAY_REQUIRED_RISE);
            delay->fall                      = delay_get_parameter(node, DELAY_REQUIRED_FALL);
            break;
        case DELAY_OUTPUT_LOAD:delay->rise = delay_get_parameter(node, DELAY_OUTPUT_LOAD);
            delay->fall                    = 0;
            break;
        case DELAY_MAX_INPUT_LOAD:delay->rise = delay_get_parameter(node, DELAY_MAX_INPUT_LOAD);
            delay->fall                       = 0;
            break;
    }
