...
void astg_ba_set(ba, n) astg_ba_rec *ba;
int n;
{
  if (n < 0 || n > ba->n_elem)
    fail("bad index");
...
void astg_ba_clr(ba, n) astg_ba_rec *ba;
int n;
{
  if (n < 0 || n > ba->n_elem)
    fail("bad index");
...
astg_bool astg_ba_get(ba, n) astg_ba_rec *ba;
int n;
{
...
int astg_ba_cmp(ba1, ba2) astg_ba_rec *ba1, *ba2;
{
  int result;
  int n_byte;
...
astg_ba_rec *astg_ba_init(b, n) astg_ba_rec *b;
int n;
{
...
astg_ba_rec *astg_ba_new(n) int n;
{ return astg_ba_init(ALLOC(astg_ba_rec, 1), n); }

astg_ba_rec *astg_ba_dup(b) astg_ba_rec *b;
{
  astg_ba_rec *bcopy;
  int n_byte;
...
void astg_ba_dispose(ba) astg_ba_rec *ba;
{
  FREE(ba->bit_array);
  FREE(ba);
}
...
static astg_vertex *astg_new_vertex(stg, vtype, name, userdata) astg_graph *stg;
...
extern astg_vertex_enum astg_v_type(v) astg_vertex *v;
{
  /* Return whether a vertex is a place or transition. */
  return v->vtype;
}

char *astg_v_name(v) astg_vertex *v;
{ return (v->name != NULL) ? v->name : "*unnamed*"; }

void astg_set_useful(v, value) astg_vertex *v;
astg_bool value;
{ v->useful = value; }

astg_bool astg_get_useful(v) astg_vertex *v;
{ return v->useful; }

void astg_delete_vertex(v) astg_vertex *v;
{
...
extern int astg_out_degree(v) astg_vertex *v;
{
...
extern int astg_in_degree(v) astg_vertex *v;
{
...
void astg_get_v_locn(v, xp, yp) astg_vertex *v;
float *xp, *yp;
{
  *xp = v->x;
  *yp = v->y;
...
void astg_set_hilite(v, value) astg_vertex *v;
astg_bool value;
{ v->hilited = value; }

astg_bool astg_get_hilite(v) astg_vertex *v;
{ return v->hilited; }

astg_bool astg_get_selected(v) astg_vertex *v;
{ return v->selected; }

/* ------------------------------- Places ----------------------------------- */

astg_place *astg_new_place(g, name, userdata) astg_graph *g;
char *name;
void *userdata;
{
...
static astg_place *astg_dup_place(g, old_p) astg_graph *g;
astg_place *old_p;
{
...
void astg_delete_place(p) astg_place *p;
{ astg_delete_vertex(p); }

astg_bool astg_boring_place(p) astg_place *p;
{
  return (p->vtype == ASTG_PLACE && !p->type.place.user_named &&
          astg_in_degree(p) == 1 && astg_out_degree(p) == 1);
}

extern char *astg_place_name(p) astg_place *p;
{
...
extern astg_place *astg_find_place(stg, name, create) astg_graph *stg;
char *name;
astg_bool create;
{
...
void astg_make_place_name(stg, p) astg_graph *stg;
astg_place *p;
{
...
static enum st_retval free_marking_info(key, value,
                                        arg) char *key; /* ARGSUSED     */
...
astg_bool astg_reset_state_graph(stg) astg_graph *stg;
{
...
extern astg_state *astg_find_state(stg, real_code, create) astg_graph *stg;
...
astg_bool astg_sim_node(node, f, stg, data) node_t *node;
astg_pi_fcn f;
astg_graph *stg;
void *data;
{
  ...
  default:
    ...
    for (cube_n = node_num_cube(node); cube_n-- && !result;) {
      ...
      foreach_fanin(node, in_n, fanin) {
        ...
        case ONE:
          result = astg_sim_node(fanin, f, stg, data);
          break;
        case ZERO:
          result = !astg_sim_node(fanin, f, stg, data);
          break;
...
astg_bool astg_signal_level(sig_name, stg, data) char *sig_name;
astg_graph *stg;
void *data;
{
...
astg_retval astg_mark(place, marking) astg_vertex *place;
...
void astg_unmark(place, marking) astg_vertex *place;
astg_marking *marking;
{
  assert(place);
...
int astg_disabled_count(trans, marking) astg_trans *trans;
astg_marking *marking;
{
...
void astg_print_state(stg, state) astg_graph *stg;
astg_scode state;
{
  astg_signal *sig_p;
  astg_generator gen;
...
extern void astg_set_marking(stg, marking_p) astg_graph *stg;
...
astg_scode astg_adjusted_code(stg, scode) astg_graph *stg;
astg_scode scode;
{ return (scode ^ stg->flow_info.phase_adj); }

astg_scode astg_fire(marking, t) astg_marking *marking;
astg_trans *t;
{
  astg_generator gen;
  astg_edge *e;
...
astg_scode astg_unfire(marking, t) astg_marking *marking;
astg_trans *t;
{
  astg_edge *e;
...
void astg_set_flow_status(stg, status) astg_graph *stg;
astg_retval status;
{ stg->flow_info.status = status; }

astg_scode astg_get_enabled(marking) astg_marking *marking;
{ return marking->enabled; }

astg_retval astg_get_flow_status(stg) astg_graph *stg;
{ return stg->flow_info.status; }

void astg_delete_marking(marking) astg_marking *marking;
{
  FREE(marking->marked_places);
  FREE(marking);
}

astg_marking *astg_dup_marking(marking) astg_marking *marking;
{
...
extern astg_marking *astg_initial_marking(stg) astg_graph *stg;
{
...
void astg_set_marked(marking, p, value) astg_marking *marking;
astg_place *p;
astg_bool value;
{
  if (value) {
...
extern astg_bool astg_get_marked(marking, p) astg_marking *marking;
astg_place *p;
{
...
static char *astg_sanitize_name(name) char *name;
{
...
astg_trans *astg_new_trans(g, name, userdata) astg_graph *g;
char *name;
void *userdata;
{
...
static astg_trans *astg_dup_trans(g, old_t) astg_graph *g;
astg_trans *old_t;
{
...
void astg_delete_trans(t) astg_place *t;
{
  astg_generator gen;
  astg_trans *tx;
...
static void astg_link_trans(new_t) astg_trans *new_t;
{
  new_t->type.trans.next_t = new_t->type.trans.sig_p->t_list;
  new_t->type.trans.sig_p->t_list = new_t;
}

char *astg_trans_alpha_name(t) astg_trans *t;
{ return t->type.trans.alpha_name; }

astg_trans_enum astg_trans_type(t) astg_trans *t;
{ return t->type.trans.trans_type; }

extern char *astg_trans_name(t) astg_place *t;
{
...
extern astg_trans *astg_find_trans(stg, sig_name, type, copy_n,
                                   create) astg_graph *stg;
char *sig_name;
...
extern astg_trans *astg_noninput_trigger(one_trans) astg_trans *one_trans;
{
...
astg_bool astg_has_constraint(t1, t2) astg_trans *t1, *t2;
{
  astg_bool found = ASTG_FALSE;
...
astg_signal *astg_trans_sig(t) astg_trans *t;
{ return t->type.trans.sig_p; }

/* -------------------------------- Edges ----------------------------------- */

astg_edge *astg_new_edge(tail, head) astg_vertex *tail;
astg_vertex *head;
{
  astg_edge *e = ALLOC(astg_edge, 1);
  astg_graph *stg = head->stg;

  if (head->vtype == tail->vtype)
    fail("");
  if (head->stg != tail->stg)
    fail("");
...
void astg_delete_edge(e) astg_edge *e;
{
...
static astg_edge *astg_dup_edge(old_e) astg_edge *old_e;
{
  astg_edge *e =
...
char *astg_edge_name(e) astg_edge *e;
{
...
astg_vertex *astg_head(e) astg_edge *e;
{ return e->head; }

astg_vertex *astg_tail(e) astg_edge *e;
{ return e->tail; }

static char *astg_make_guard_name(e, buffer, n) astg_edge *e;
char *buffer;
int n;
{
...
astg_graph *astg_new(grName) char *grName;
{
...
int astg_num_vertex(stg) astg_graph *stg;
{ return stg->n_vertex; }

char *astg_name(stg) astg_graph *stg;
{ return stg->name; }

void astg_free_components(component_array) array_t *component_array;
{
  int i;
...
void astg_delete_sig(stg, sig_p) astg_graph *stg;
astg_signal *sig_p;
{
  if (sig_p == NULL)
    return;
...
void astg_delete(stg) astg_graph *stg;
{
  astg_generator gen;
  astg_vertex *v;
...
long astg_change_count(stg) astg_graph *stg;
{ return stg->change_count; }

static lsGeneric astg_copy_comments(data) lsGeneric data;
{
...
astg_graph *astg_duplicate(old_g, name) astg_graph *old_g;
char *name;
{
...
extern astg_scode astg_marking_enabled(marking_p) astg_marking *marking_p;
{
...
extern int astg_state_count(stg) astg_graph *stg;
{
...
extern astg_scode astg_state_code(state_p) astg_state *state_p;
{
...
extern astg_scode astg_state_enabled(state_p) astg_state *state_p;
{
...
int astg_state_n_marking(state_p) astg_state *state_p;
{ return array_n(state_p->states); }

int astg_cmp_marking(m1, m2) astg_marking *m1, *m2;
{
  int result = 0;
...
/* --------------------------------- States stuff --------------------------- */
astg_state *astg_new_state(stg) astg_graph *stg;
{
...
astg_bool astg_is_dummy_marking(marking) astg_marking *marking;
{ return marking->is_dummy; }

void astg_add_marking(state_p, marking) astg_state *state_p;
astg_marking *marking;
{
  astg_generator gen;
  astg_scode enabled = 0;
...
void astg_delete_state(state_p) astg_state *state_p;
{
  astg_generator mgen;
  astg_marking *marking;
...
astg_bool astg_spline_iter(e, gen, xp, yp) astg_edge *e;
astg_generator *gen;
float *xp, *yp;
{
  astg_vertex *v;
...
astg_bool astg_sig_x_iter(sig, gen, type, trans_p) astg_signal *sig;
astg_generator *gen;
astg_trans_enum type;
astg_trans **trans_p;
{
...
astg_bool astg_vertex_iter(stg, gen, vtype, vertex_p) astg_graph *stg;
astg_generator *gen;
astg_vertex_enum vtype;
...
astg_bool astg_path_iter(stg, gen, vertex_p) astg_graph *stg;
astg_generator *gen;
astg_vertex **vertex_p;
{
  astg_vertex *v;
...
astg_bool astg_edge_iter(stg, gen, edge_p) astg_graph *stg;
astg_generator *gen;
astg_edge **edge_p;
{
  astg_vertex *v;
...
astg_bool astg_out_edge_iter(v, gen, edge_p) astg_vertex *v;
astg_generator *gen;
astg_edge **edge_p;
{
...
astg_bool astg_in_edge_iter(v, gen, edge_p) astg_vertex *v;
astg_generator *gen;
astg_edge **edge_p;
{
...
astg_bool astg_in_vertex_iter(v, gen, vertex_p) astg_vertex *v;
astg_generator *gen;
astg_vertex **vertex_p;
{
  astg_edge *e = (gen->first_time) ? v->in_edges : gen->ge;
...
astg_bool astg_out_vertex_iter(v, gen, vertex_p) astg_vertex *v;
astg_generator *gen;
astg_vertex **vertex_p;
{
  astg_edge *e = (gen->first_time) ? v->out_edges : gen->ge;
...
astg_bool astg_sig_iter(stg, gen, sig_p) astg_graph *stg;
astg_generator *gen;
astg_signal **sig_p;
{
...
astg_bool astg_state_iter(stg, gen, state_p) astg_graph *stg;
astg_generator *gen;
astg_state **state_p;
{
  astg_bool ok;
...
astg_bool astg_marking_iter(state, gen, marking_p) astg_state *state;
astg_generator *gen;
astg_marking **marking_p;
{
...
astg_bool astg_is_rel(t1, t2) astg_trans *t1, *t2;
{
  astg_bool found = ASTG_FALSE;
  astg_generator pgen, tgen;
  astg_place *p;
...
void astg_add_constraint(t1, t2, check) astg_trans *t1,
    *t2;         /*u Adding constraint from t1 to t2.	*/
...
void astg_sel_vertex(v, val) astg_vertex *v;
astg_bool val;
{ v->selected = val; }

void astg_sel_edge(e, val) astg_edge *e;
astg_bool val;
{ e->selected = val; }

void astg_sel_clear(stg) astg_graph *stg;
{ stg->sel_name = NULL; }

void astg_sel_new(stg, selname, value) astg_graph *stg;
char *selname;
astg_bool value;
{
  astg_generator gen;
...
void astg_sel_show(stg) astg_graph *stg;
{
...
astg_signal_enum astg_signal_type(sig) astg_signal *sig;
{ return sig->sig_type; }

extern char *astg_signal_name(sig) astg_signal *sig;
{
...
extern astg_scode astg_state_bit(sig) astg_signal *sig;
{
...
void astg_set_level(sig, scode, value) astg_signal *sig;
astg_scode *scode;
astg_bool value;
{
  if (value) {
...
astg_bool astg_get_level(sig, scode) astg_signal *sig;
astg_scode scode;
{ return get_bit(scode, sig->state_bit); }

astg_bool astg_is_noninput(sig) astg_signal *sig;
{
  return (sig->sig_type == ASTG_OUTPUT_SIG ||
          sig->sig_type == ASTG_INTERNAL_SIG);
}

astg_bool astg_is_dummy(sig) astg_signal *sig;
{ return (sig->sig_type == ASTG_DUMMY_SIG); }

astg_bool astg_is_input(sig) astg_signal *sig;
{ return (sig->sig_type == ASTG_INPUT_SIG); }

int astg_signal_bit(sig) astg_signal *sig;
{ return sig->state_bit; }

static astg_signal *sig_new(stg, name) astg_graph *stg;
char *name;
{
...
extern astg_signal *astg_find_named_signal(stg, sig_name) astg_graph *stg;
char *sig_name;
{
...
extern astg_signal *astg_find_signal(stg, name, type, create) astg_graph *stg;
char *name;
...
astg_signal *astg_new_sig(stg, type) astg_graph *stg;
astg_signal_enum type;
{
  char sig_name[40];
...
extern astg_bool astg_is_trigger(sig1, sig2) astg_signal *sig1, *sig2;
{
...
static int add_espresso(stg, fout, in_mask, out_mask, new_state,
                        delta_ns) astg_graph *stg;
FILE *fout;
...
static int cmp_encoding(data1, data2) char *data1, *data2;
{
...
int astg_write_pla(stg, fout) astg_graph *stg;
FILE *fout;
{
  astg_signal *sig_p;
