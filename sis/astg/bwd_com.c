...
static void bwd_alloc_daemon(astg, dummy) astg_graph *astg, *dummy;
{
  bwd_t *bwd;
...
static void bwd_dup_daemon(new_astg, old_astg) astg_graph *new_astg, *old_astg;
{
...
static void bwd_free_daemon(astg, dummy) astg_graph *astg, *dummy;
{
  bwd_t *bwd;
...
static void bwd_invalid_daemon(astg, dummy) astg_graph *astg, *dummy;
{
...
int com_bwd_astg_to_f(network_p, argc, argv) network_t **network_p;
int argc;
char **argv;
{
...
int com_bwd_astg_stg_scr(network_p, argc, argv) network_t **network_p;
int argc;
char **argv;
{
  int c, result;
  ...
  while ((c = util_getopt(argc, argv, "v:")) != EOF) {
    switch (c) {
    case 'v':
      astg_debug_flag = atoi(util_optarg);
      break;
    default:
      result = 1;
      break;
...
int com_bwd_stg_to_astg(network_p, argc, argv) network_t **network_p;
int argc;
char **argv;
{
  int c, result;
...
int com_bwd_astg_to_stg(network_p, argc, argv) network_t **network_p;
int argc;
char **argv;
{
...
int com_bwd_astg_slow_down(network_p, argc, argv) network_t **network_p;
int argc;
char **argv;
{
  ...
  while ((c = util_getopt(argc, argv, "bv:d:f:F:im:st:ulL")) != EOF) {
    ...
    case 'v':
      ...
      break;
    case 'd':
      default_del = atof(util_optarg);
      break;
    case 'i':
      iterate = 1;
      break;
    case 'm':
      min_delay_factor = atof(util_optarg);
      break;
    case 's':
      shortest_path = 0;
      break;
    case 't':
      tol = atof(util_optarg);
      break;
    case 'f':
...
int com_bwd_astg_state_minimize(network_p, argc, argv) network_t **network_p;
int argc;
char **argv;
{
  int c, result, cost;
  ...
  while ((c = util_getopt(argc, argv, "bBgGmMuf:o:v:c:p:")) != EOF) {
    ...
    case 'M':
      ...
      break;
    case 'o':
      mincov_option = atoi(util_optarg);
      break;
    case 'v':
      astg_debug_flag = atoi(util_optarg);
      break;
  ...
  if (sig_cost != NIL(st_table)) {
    st_foreach_item(sig_cost, stgen, (char **)&str, NIL(char *)) { FREE(str); }
...
int com_bwd_astg_add_state(network_p, argc, argv) network_t **network_p;
int argc;
char **argv;
{
  int c, result, cost;
 ...
 */
int com_bwd_astg_encode(network, argc, argv) network_t **network;
int argc;
char **argv;
{
...
int com_bwd_astg_write_sg(network_p, argc, argv) network_t **network_p;
int argc;
char **argv;
{
...
int com_stg_scc(network_p, argc, argv) network_t **network_p;
int argc;
char **argv;
{
  graph_t *stg;
