...
static void bwd_alloc_daemon(astg_graph *astg, astg_graph *dummy) {
    bwd_t *bwd;
...
static void bwd_dup_daemon(astg_graph *new_astg, astg_graph *old_astg) {
    ...
    if (old_hazard_list != NIL(st_table)) {
        st_foreach_item(old_hazard_list, gen, &n, (char **) &old_hazards) {
            if (n != NIL(char) && old_hazards != NIL(array_t)) {
                ...
                st_insert(new_hazard_list, util_strsav(n), (char *) new_hazards);
            }
        }
    }
        ...
        st_foreach_item(old_slowed_amounts, gen, &n, (char **) &old_slowed) {
            if (n != NIL(char) && old_slowed != NIL(double)) {
                ...
                *new_slowed = *old_slowed;
                st_insert(new_slowed_amounts, util_strsav(n), (char *) new_slowed);
            }
        }
    }
    ...
    if (old_pi_names != NIL(array_t)) {
        for (i = 0; i < array_n(old_pi_names); i++) {
            array_insert_last(char *, new_pi_names,
                              util_strsav(array_fetch(
                                      char *, old_pi_names, i)));
        }
    }
}

static void bwd_free_daemon(astg_graph *astg, astg_graph *dummy) {
    bwd_t        *bwd;
...
static void bwd_invalid_daemon(astg_graph *astg, astg_graph *dummy) {
...
int com_bwd_astg_to_f(network_t **network_p, int argc, char **argv) {
    ...
    while ((c = util_getopt(argc, argv, "forv:dlh")) != EOF) {
        switch (c) {
            ...
            case 'd':disjoint = 1;
                break;
            case 'h':find_hazards = 0;
                break;
            case 'v':flag = atoi(util_optarg);
                break;
            default:result = 1;
                break;
        }
    ...
    astg_foreach_signal(astg, sgen, sig_p) {
        ...
        if (astg_debug_flag > 1) {
            fprintf(sisout, "%s (%x) ", node_long_name(node), astg_state_bit(sig_p));
        }
    ...
    astg_foreach_signal(astg, sgen, sig_p) {
        ...
        if (astg_signal_type(sig_p) == ASTG_OUTPUT_SIG ||
            ...
            if (astg_debug_flag > 1) {
                fprintf(sisout, "generating function for signal %s\n", sig_p->name);
            }
            use_s_r =
                    st_is_member(s_r_names, sig_p->name) || st_is_member(s_r_names, "*");
            bwd_astg_to_f(*network_p, astg, sig_p, fanin, state, hazard_list, index,
                          use_old, use_s_r, disjoint, force);
        }
        index++;
    }
...
static char *astg_stg_scr_usage[] = {
        ...
        "    Transforms an STG into an STG with the Single Cube Restriction", NULL};

int com_bwd_astg_stg_scr(network_t **network_p, int argc, char **argv) {
    int        c, result;
    graph_t    *stg;
    lsGen      gen;
    vertex_t   *state;
    ...
    while ((c = util_getopt(argc, argv, "v:")) != EOF) {
        switch (c) {
            case 'v':astg_debug_flag = atoi(util_optarg);
                break;
            default:result = 1;
                break;
        }
...
static char *stg_to_astg_usage[] = {
        "usage: %s [-v debug_level]",
        "    Generates an STG from a synchronous state transition graph.", NULL};

int com_bwd_stg_to_astg(network_t **network_p, int argc, char **argv) {
    int        c, result;
    astg_graph *astg;
...
static char *astg_to_stg_usage[] = {
        ...
        "    -m: perform a pre-minimization of the STG (for state encoding)", NULL};

int com_bwd_astg_to_stg(network_t **network_p, int argc, char **argv) {
    ...
    while ((c = util_getopt(argc, argv, "v:om")) != EOF) {
        switch (c) {
            case 'v':d = atoi(util_optarg);
                break;
            case 'o':use_output = 0;
                break;
            case 'm':pre_minimize = 1;
                break;
            default:result = 1;
                break;
        }
    ...
    if (array_n(components) > 1) {
        fprintf(siserr, "internal error: the state transition graph is not "
                        "strongly connected\n");
        if (astg_debug_flag > 0) {
...
int com_bwd_astg_slow_down(network_t **network_p, int argc, char **argv) {
    ...
    while ((c = util_getopt(argc, argv, "bv:d:f:F:im:st:ulL")) != EOF) {
        switch (c) {
            ...
            case 'v':astg_debug_flag = atoi(util_optarg);
                break;
            case 'd':default_del = atof(util_optarg);
                break;
            case 'i':iterate = 1;
                break;
            case 'm':min_delay_factor = atof(util_optarg);
                break;
            case 's':shortest_path = 0;
                break;
            case 't':tol = atof(util_optarg);
                break;
            case 'f':filename = util_optarg;
                break;
            case 'u':do_slow = 0;
                break;
            case 'F':filename = util_optarg;
        ...
        if (file == NULL) {
            ...
            while (fgets(buf, sizeof(buf), file) != NULL) {
                ...
                if (sscanf(buf, "%s %s %lf %lf %lf %lf\n", pi_buf, po_buf,
                           &delay.rise.rise, &delay.rise.fall, &delay.fall.rise,
                           &delay.fall.fall) != 6) {
                ...
                if (!st_lookup(external_del, pi_buf, (char **) &to_table)) {
                    ...
                    st_insert(external_del, util_strsav(pi_buf), (char *) to_table);
                }
                if (st_lookup(to_table, po_buf, (char **) &delay_p)) {
                    fprintf(siserr, "Warning: duplicate delay table entry: %s -> %s\n",
                            pi_buf, po_buf);
                    continue;
                }
                if (astg_debug_flag > 1) {
    ...
    if (lp) {
        ...
        bwd_lp_slow(astg, *network_p, hazard_list, external_del, pi_index, tol,
                    default_del, min_delay_factor, do_bound, lindo);
    } else {
    ...
    if (update && filename != NIL(char)) {
        ...
        if (file == NULL) {
            ...
            st_foreach_item(external_del, pigen, (char **) &pi_name,
                            (char **) &to_table) {
                st_foreach_item(to_table, pogen, (char **) &po_name, (char **) &delay_p) {
                    fprintf(file, "%s %s %lf %lf %lf %lf\n", pi_name, po_name,
                            delay_p->rise.rise, delay_p->rise.fall, delay_p->fall.rise,
                            delay_p->fall.fall);
                    if (astg_debug_flag > 1) {
...
int com_bwd_astg_state_minimize(network_t **network_p, int argc, char **argv) {
    int          c, result, cost;
    ...
    while ((c = util_getopt(argc, argv, "bBgGmMuf:o:v:c:p:")) != EOF) {
        switch (c) {
            case 'f':sig_file = fopen(util_optarg, "r");
                if (sig_file == NULL) {
                    perror(util_optarg);
                    result = 1;
                }
                break;
            case 'c':strcpy(command, util_optarg);
                break;
            case 'p':premin = 1;
                strcpy(outfile, util_optarg);
                break;
            case 'b':use_mdd = 1;
                break;
            case 'B':use_mdd = 2;
            ...
            case 'M':use_mincov = 2;
                break;
            case 'o':mincov_option = atoi(util_optarg);
                break;
            case 'v':astg_debug_flag = atoi(util_optarg);
                break;
            default:result = 1;
    ...
    if (premin) {
        (void) fprintf(siserr, "please issue\n%s\nand then\nkill -CONT %d\n",
                       command, getpid());
        kill(getpid(), SIGSTOP);
    ...
    red_stg = bwd_state_minimize(astg, equiv_states, use_mincov, greedy, go_down,
                                 mincov_option, use_mdd, sig_cost);

    if (sig_cost != NIL(st_table)) {
        st_foreach_item(sig_cost, stgen, (char **) &str, NIL(
                char *)) { FREE(str); }
...
static char *astg_add_state_usage[] = {
        ...
        "    -m: do not preserve the original marking", NULL};

int com_bwd_astg_add_state(network_t **network_p, int argc, char **argv) {
    int        c, result, cost;
    int        restore_marking;
    graph_t    *stg;
    astg_graph *astg;
 ...
 */
int com_bwd_astg_encode(network_t **network, int argc, char **argv) {
    ...
    while ((c = util_getopt(argc, argv, "ushv:")) != EOF) {
        switch (c) {
            ...
            case 'u':user = 1;
                break;
            case 's':enc_summary = 1;
                break;
            case 'v':g_debug = atoi(util_optarg);
                break;
            default:goto usage;
        }
    }
...
static char *write_sg_usage[] = {"usage: %s [filename]",
                                 "    Writes out a state graph file", NULL};

int com_bwd_astg_write_sg(network_t **network_p, int argc, char **argv) {
...
int com_stg_scc(network_t **network_p, int argc, char **argv) {
    graph_t  *stg;
