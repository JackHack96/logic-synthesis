...
static astg_bool astg_uses_name(stg, name) astg_graph *stg;
char *name;
{
  ...
  astg_foreach_signal(stg, gen, sig_p) {
    if (!strcmp(name, sig_p->name))
      return ASTG_TRUE;
  }
  return ASTG_FALSE;
}

static char *astg_make_fake_name(stg, name, suffix) astg_graph *stg;
char *name;
char *suffix;
{
...
char *latch_type;
{
  ...
  astg_foreach_signal(stg, gen, sig_p) {
    if (astg_is_input(sig_p) || astg_is_dummy(sig_p))
      continue;
  ...
  fprintf(fp, ".model %s\n", stg->name);
  astg_print_inputs(fp, ".inputs", stg, NULL);
  fputs("\n", fp);
  ...
  astg_foreach_signal(stg, gen, sig_p) {
    ...
    if (astg_is_noninput(sig_p)) {
      fprintf(fp, ".latch %s %s %s NIL %d\n", fake_po_names[sig_n],
              fake_pi_names[sig_n], latch_type,
              (sig_p->state_bit & scode) != 0);
      sig_n++;
    }
  }
  ...
  astg_foreach_signal(stg, gen, sig_p) {
    if (astg_is_noninput(sig_p))
      fprintf(fp, " %s", fake_po_names[sig_n++]);
  }
  fputs("\n", fp);
