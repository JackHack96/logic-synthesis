...
astg_bool valid_marking(astg_graph *stg) {
    int            i, j;
...
static lsStatus remove_marked_smc(lsGeneric data, lsGeneric arg) {
...
static astg_bool find_marking(astg_graph *stg) {
    lsList         unmarked_smc;
    array_t        *smc;
    ...
    stg->change_count++;
    astg_foreach_place(stg, pgen, p) { astg_plx(p)->initial_token = ASTG_FALSE; }
...
astg_bool astg_one_sm_token(astg_graph *stg) {
...
astg_retval astg_set_marking_by_code(astg_graph *stg, astg_scode state_code, astg_scode sig_mask) {
    astg_retval    status;
    astg_generator sgen;
    astg_state     *state_p;
...
astg_retval astg_set_marking_by_name(astg_graph *stg, st_table *sig_values) {
    ...
    if (status == ASTG_NOT_USC)
        status = ASTG_OK;
    if (status != ASTG_OK)
        return status;

    st_foreach_item_int(sig_values, sig_gen, &sig_name, &value) {
        ...
        if (sig_p == NULL) {
            ...
            if (value)
                state_code |= astg_signal_bit(sig_p);
        }
    }
