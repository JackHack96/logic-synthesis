...
astg_bool valid_marking(stg) astg_graph *stg;
{
  int i, j;
...
static lsStatus remove_marked_smc(data, arg) lsGeneric data;
...
static astg_bool find_marking(stg) astg_graph *stg;
{
  lsList unmarked_smc;
  array_t *smc;
...
astg_bool astg_one_sm_token(stg) astg_graph *stg;
{
...
extern astg_retval astg_unique_state(stg, state_code) astg_graph *stg;
...
extern astg_retval astg_set_marking_by_name(stg, sig_values) astg_graph *stg;
st_table *sig_values;
{
