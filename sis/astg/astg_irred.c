...
static void pr_path(stg, s) astg_graph *stg;
char *s;
{
  astg_generator gen;
  astg_vertex *v;
  ...
  msg("%s:", s);
  astg_foreach_path_vertex(stg, gen, v) { msg(" %s", astg_v_name(v)); }
  msg("\n");
}
...
static int check_redundant(stg, data) astg_graph *stg;
void *data;
{
...
extern int astg_irred(stg, modify) astg_graph *stg;
astg_bool modify;
{
  ...
  astg_foreach_place(stg, gen, p) { p->subset = ASTG_TRUE; }
  ...
  astg_foreach_place(stg, gen, p) {
    ...
    info.red_place = p;
    if (astg_simple_cycles(stg, info.red_tail, check_redundant, (void *)&info,
                           ASTG_SUBSET)) {
      n_redundant++;
...
static float astg_calc_delay(stg) astg_graph *stg;
{
  float rc = 0.0;
  astg_vertex *v;
  astg_generator gen;
...
static int check_cycles(stg, data) astg_graph *stg;
void *data;
{
  ...
  if (cinfo->find_longest) {
    ...
    if (cinfo->longest_cycle == NULL || cycle_delay > cinfo->its_delay) {
      if (cinfo->longest_cycle != NULL)
        array_free(cinfo->longest_cycle);
    ...
    n_found = 1;
    astg_foreach_path_vertex(stg, gen, v) { astg_sel_vertex(v, ASTG_TRUE); }
  }

  return n_found;
}

float astg_longest_cycle(stg) astg_graph *stg;
{
  cycle_info cinfo_rec;

  cinfo_rec.find_longest = ASTG_TRUE;
  cinfo_rec.longest_cycle = NULL;
  astg_simple_cycles(stg, NIL(astg_vertex), check_cycles, (void *)&cinfo_rec,
                     ASTG_ALL);
  if (cinfo_rec.longest_cycle != NULL)
    array_free(cinfo_rec.longest_cycle);
  return cinfo_rec.its_delay;
}

int astg_select_cycle(stg, thru_trans, cycle_n, find_longest,
                      add_to_set) astg_graph *stg;
...
astg_bool add_to_set;   /*i 1=add to existing set, 0=new set	*/
{
  ...
  n_cycle =
      astg_simple_cycles(stg, v, check_cycles, (void *)&cinfo_rec, ASTG_ALL);
