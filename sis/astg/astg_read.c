...
char *astg_make_name(sig_name, trans_type, copy_n) char *sig_name;
...
astg_trans *find_trans_by_name(source, stg, create) io_t *source;
astg_graph *stg;
astg_bool create;
{
  astg_trans_enum trans_type;
...
void io_open(source, stream, s) io_t *source;
FILE *stream;
char *s;
{
  if (s != NULL) {
...
int io_close(source) io_t *source;
{
...
static int io_dump_string(buffer, offset) char *buffer;
int offset;
{
  char *p = buffer;
  char buff[5], *s;
...
void io_msg(source, warning, message) io_t *source;
astg_bool warning;
char *message;
{
  int offset;
...
void io_warn(source, message) io_t *source;
char *message;
{
  if (source->errflag == 0) {
...
int io_error(source, message) io_t *source;
char *message;
{
  if (source->errflag == 0) {
    source->errflag = -1;
...
int io_get2(source) io_t *source;
{
...
static int io_getc(source) io_t *source;
{
  int c;
  do {
    c = io_get(source);
...
static void io_unget(c, source) int c;
io_t *source;
{
...
static int io_peek(source) io_t *source;
{
...
int io_token(source) io_t *source;
{
...
static astg_bool io_maybe(source, target_char) io_t *source;
int target_char;
{
...
static int io_mustbe(source, token) io_t *source;
char *token;
{
  char msgbuf[80];
...
static int is_place_name(stg, vname) astg_graph *stg;
char *vname;
{
  int is_place, c;
  int n;
...
static int read_point_list(source, e) io_t *source;
astg_edge *e;
{
  float xp;
...
static void read_edge_guard(source, se) io_t *source;
astg_edge *se;
{
  if (io_maybe(source, '?')) {
    io_token(source);
...
static int read_place_fanout(source, stg, startp) io_t *source;
astg_graph *stg;
astg_place *startp;
{
  astg_trans *t;
...
static int read_trans_fanout(source, stg, startt) io_t *source;
astg_graph *stg;
astg_trans *startt;
{
...
void astg_read_marking(source, stg) io_t *source;
astg_graph *stg;
{
  astg_trans *t1, *t2, *t;
...
static void read_delay(source, delay) io_t *source;
float *delay;
{
...
static int read_posn(source, xp, yp) io_t *source;
float *xp, *yp;
{
  if (io_maybe(source, '(')) {
    io_token(source);
...
static void read_vertex(source, stg) io_t *source;
astg_graph *stg;
{
  astg_place *p;
  astg_trans *t;
...
static void read_graph_check(source, stg) io_t *source;
astg_graph *stg;
{
...
static void read_graph(source, stg) io_t *source;
astg_graph *stg;
{
  int c;
...
static void read_signals(source, stg, sig_type) io_t *source;
astg_graph *stg;
...
static void read_note(source, stg) io_t *source;
astg_graph *stg;
{
  int c;
  char *p;
...
static void read_stg(source, stg) io_t *source;
astg_graph *stg;
{
  int c;
...
extern astg_graph *
    astg_read(fin,
              fname) FILE *fin; /*i stream to read STG from		*/
...
void astg_write_marking(stg, fout) astg_graph *stg;
FILE *fout;
{
...
static char *trim_float(value, buffer) double value;
char *buffer;
{
  char *p;
...
int astg_write(stg, hide_places, stream) astg_graph *stg;
