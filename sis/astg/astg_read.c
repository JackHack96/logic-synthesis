...
char *astg_make_name(char *sig_name, astg_trans_enum trans_type, int copy_n) {
    ...
    if (trans_type == ASTG_POS_X)
        ...
        strcat(tname, "*");
    else if (trans_type != ASTG_DUMMY_X) fail("bad trans type");
...
static int parse_name(io_t *source, char *signame, astg_trans_enum *type, int *copy_n) {
    char *t, *s;
...
astg_trans *find_trans_by_name(io_t *source, astg_graph *stg, astg_bool create) {
...
void io_open(io_t *source, FILE *stream, char *s) {
    if (s != NULL) {
...
static int io_dump_string(char *buffer, int offset) {
    char *p       = buffer;
    char buff[5], *s;
    int  c;
...
void io_msg(io_t *source, astg_bool warning, char *message) {
    int offset;
...
void io_warn(io_t *source, char *message) {
    if (source->errflag == 0) {
        io_msg(source, ASTG_TRUE, message);
    }
}

int io_error(io_t *source, char *message) {
    if (source->errflag == 0) {
        source->errflag = -1;
...
static int io_getc(io_t *source) {
    int c;
    do {
        c = io_get(source);
        if (c == EOF)
            io_error(source, "unexpected end-of-file");
    } while (c != EOF && c != '\n' && isspace(c));
    return c;
}

static void io_unget(int c, io_t *source) {
    if (source->save_one != EOF) {
        source->save_one = c;
    }
}

static int io_peek(io_t *source) {
...
static astg_bool io_maybe(io_t *source, int target_char) {
    int next_c = io_getc(source);
    if (next_c != target_char)
        io_unget(next_c, source);
    return (next_c == target_char);
}

static int io_mustbe(io_t *source, char *token) {
    char msgbuf[80];
...
static int is_place_name(astg_graph *stg, char *vname) {
    int  is_place, c;
    int  n;
    char *p;
...
static int read_point_list(io_t *source, astg_edge *e) {
    float xp;
...
static void read_edge_guard(io_t *source, astg_edge *se) {
    if (io_maybe(source, '?')) {
        io_token(source);
...
static int read_place_fanout(io_t *source, astg_graph *stg, astg_place *startp) {
    astg_trans *t;
...
static int read_trans_fanout(io_t *source, astg_graph *stg, astg_trans *startt) {
...
void astg_read_marking(io_t *source, astg_graph *stg) {
    astg_trans     *t1, *t2, *t;
...
static void read_delay(io_t *source, float *delay) {
...
static int read_posn(io_t *source, float *xp, float *yp) {
    if (io_maybe(source, '(')) {
        io_token(source);
        if (sscanf(source->buffer, " %f", xp) != 1) {
...
static void read_vertex(io_t *source, astg_graph *stg) {
    astg_place *p;
    astg_trans *t;
    ...
    if (is_place_name(stg, source->buffer)) {
        if ((p = astg_find_place(stg, source->buffer, ASTG_TRUE)) != NULL) {
            ...
            while (io_status(source) == 0 && !io_maybe(source, '\n')) {
                if (read_place_fanout(source, stg, p))
                    break;
            }
        }
    } else {
        if ((t = find_trans_by_name(source, stg, ASTG_TRUE)) != NULL) {
            ...
            while (io_status(source) == 0 && !io_maybe(source, '\n')) {
                if (read_trans_fanout(source, stg, t))
                    break;
            }
        }
    }
...
static void read_graph_check(io_t *source, astg_graph *stg) {
    ...
    astg_foreach_trans(stg, gen, t) {
        if (astg_trans_type(t) == ASTG_HATCH_X) {
            ...
            new_t =
                    astg_find_trans(stg, astg_signal_name(s), ASTG_DUMMY_X, 1, ASTG_TRUE);
            ...
            new_t =
                    astg_find_trans(stg, astg_signal_name(s), ASTG_DUMMY_X, 2, ASTG_TRUE);
...
static void read_graph(io_t *source, astg_graph *stg) {
    int c;
...
static void read_signals(io_t *source, astg_graph *stg, astg_signal_enum sig_type) {
    while (io_status(source) == 0 && io_token(source) != '\n') {
        if (isalpha(source->buffer[0]) && strchr(source->buffer, '/') == NULL &&
...
static void read_note(io_t *source, astg_graph *stg) {
    int  c;
    char *p;
...
static void read_stg(io_t *source, astg_graph *stg) {
    int c;
...
void astg_write_marking(astg_graph *stg, FILE *fout) {
...
static char *trim_float(double value, char *buffer) {
    char *p;
...
int astg_write(astg_graph *stg, astg_bool hide_places, FILE *stream) {
    ...
    astg_foreach_place(stg, pgen, p) {
        ...
        if (p->y != 0 || p->x != 0) {
            fprintf(stream, " (%s,%s)", trim_float(p->x, fbuf1),
                    trim_float(p->y, fbuf2));
        }
    ...
    astg_foreach_trans(stg, tgen, t) {
        ...
        if (t->y != 0 || t->x != 0) {
            fprintf(stream, " (%s,%s)", trim_float(t->x, fbuf1),
                    trim_float(t->y, fbuf2));
        }
