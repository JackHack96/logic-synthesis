...
void astg_do_daemons(astg_graph *stg1, astg_graph *stg2, astg_daemon_enum type) {
    /*	Execute all daemons of the given type. */
    astg_daemon_t *d;

    dbg(1, printf("do daemons %lX %lX %d\n", (long unsigned int)stg1, (long unsigned int)stg2, type));
...
astg_bool astg_is_place_simple(astg_graph *g) {
...
astg_bool astg_is_pure(astg_graph *g) {
    astg_generator gen1, gen2, gen3;
    astg_vertex    *p, *p2;
    astg_edge      *e1, *e2;
    astg_vertex    *t;
...
static int astg_report_cycle(astg_graph *g) {
    astg_generator gen;
    astg_vertex    *v;
    int            rc;

    astg_foreach_path_vertex(g, gen, v) { v->on_path = ASTG_TRUE; }
    rc = (*g->f)(g, g->f_data);
    astg_foreach_path_vertex(g, gen, v) { v->on_path = ASTG_FALSE; }
    return rc;
}

static int astg_cycles(astg_graph *g, astg_vertex *v) {
    astg_generator gen;
    int            rc = 0;
    astg_edge      *e;
...
int astg_simple_cycles(astg_graph *g, astg_vertex *target_v, int (*f)(), void *f_data, astg_bool subset) {
    int            rc = 0;
    astg_bool      x;
    astg_vertex    *v;
    astg_generator gen;
...
static void astg_ts_dfs(astg_graph *g, astg_vertex *v, int *num_p) {
    astg_generator gen;
    astg_edge      *e;

    if (v->unprocessed) {
        v->unprocessed = ASTG_FALSE;
        astg_foreach_out_edge(v, gen, e) { astg_ts_dfs(g, astg_head(e), num_p); }
        v->alg.ts.index = (*num_p)++;
    }
}

array_t *astg_top_sort(astg_graph *g, astg_bool subset) {
    astg_generator gen;
    astg_vertex    *v;
    int            vertex_n;
    array_t        *varray;
...
static void astg_cc_dfs(astg_vertex *v, array_t *vcomp, int component_n) {
    astg_generator gen;
    astg_edge      *e;
...
int astg_connected_comp(astg_graph *g, int  (*f)(), void *f_data, astg_bool subset) {
    astg_generator gen;
    astg_vertex    *v;
    int            component_n;
    array_t        *vcomp;
    ...
    if (!subset) {
        astg_foreach_vertex(g, gen, v) v->
                                                unprocessed = ASTG_TRUE;
    }
    component_n = 0;
...
static void astg_scc_dfs(astg_vertex *v, array_t *vcomp, int component_n) {
    astg_generator gen;
    astg_edge      *e;
...
int astg_strong_comp(astg_graph *g, int (*f)(), void *f_data, astg_bool subset) {
    array_t     *varray;
    array_t     *vcomp;
    astg_vertex *v;
    int         n, component_n;
...
static int astg_find_cycles(astg_vertex *v) {
    astg_generator gen;
    astg_edge      *e;
    int            rc = 0;
...
int astg_has_cycles(astg_graph *g) {
    astg_generator gen;
    astg_vertex    *v;
    int            n_cycles;
...
static astg_bool astg_cp_dfs(astg_vertex *v, astg_vertex *dest) {
    astg_edge      *e;
    astg_bool
                   path_found = ASTG_FALSE;
    astg_generator gen;
    ...
    if (v->unprocessed) {
        ...
        astg_foreach_out_edge(v, gen, e) {
            if (!path_found)
                path_found = astg_cp_dfs(astg_head(e), dest);
        }
    }
    return path_found;
}

astg_bool astg_choose_path(astg_graph *g, astg_vertex *start, astg_vertex *dest) {
    astg_generator gen;
    astg_vertex    *v;
...
void astg_print_path(astg_graph *stg, FILE *stream) {
    astg_generator gen;
    astg_vertex    *v;
...
void astg_print(astg_graph *g, FILE *stream) {
    astg_generator gen;
    astg_vertex    *v;
    astg_edge      *e;
