...
network_t *astg_min(network_t *network) {
    network_t *new_net;
    pPLA      PLA;
...
static void alloc_pipo_names(network_t *network, pPLA PLA) {
...
static void add_latch(network_t *old, network_t *new) {
    lsGen   gen;
...
static void astg_do_min(pPLA PLA, astg_graph *stg) {
    pcover   C, Fnew;
...
static st_table *link_state_minterm(pPLA PLA, astg_graph *stg) {
    st_table       *table;
    astg_generator gen;
    astg_state     *state_p;
    astg_scode     state;
...
static pcube find_minterm(pPLA PLA, astg_graph *stg, astg_scode state) {
    pcube                last, p, c;
    register int         i;
...
static void do_consensus(pPLA PLA, astg_graph *stg, st_table *state_minterms) {
...
static void astg_do_reduce(pPLA PLA, astg_graph *stg) {
    array_t   *state_array;
...
static int
remove_glitch(int index, pcube c, state_rec *state_info, array_t *state_array, astg_graph *stg, pcover F, pPLA PLA) {
...
static int possible_glitch(pcube c, state_rec *state_info, astg_graph *stg, pcover F, pPLA PLA) {
    astg_signal *signal;
...
static int constant_one(pcube c, state_rec *state_info, astg_graph *stg, pcover F, pPLA PLA) {
    pcube       c1, c2, p1, p2;
    astg_signal *s;
    int         i;
    pcube       last, p;
...
static int perform_reduction(int index, pcube c, state_rec *state_info, array_t *state_array, astg_graph *stg, pcover F,
                             pPLA PLA) {
    pcube       *FD, c1;
    pcover      F_old;
    int         i, j;
...
static array_t *sort_states(astg_graph *stg) {
    array_t        *state_array;
...
static int compare_fanouts(char *s1, char *s2) {
    int       diff;
...
static int fc_marking(astg_state *state_p, array_t *fc_places) {
    astg_generator gen;
 ...
 * this signal 'signal'
 */
static int logic_hazard(pcover F, pPLA PLA, astg_signal *signal, astg_graph *stg) {
    register int   i;
    astg_scode     state, enabled;
    astg_generator gen;
    astg_state     *state_p;
...
static node_t *find_real_po(node_t *node) {
    lsGen  gen;
...
/* --- debug routines --- */
/* print present state */
void print_state(astg_graph *stg, astg_scode state) {
    astg_signal    *s;
    astg_generator agen;
...
void print_enabled(astg_graph *stg, astg_scode state, astg_scode enabled) {
    astg_signal    *s;
    astg_generator agen;
...
void print_state_graph(astg_graph *stg) {
    astg_scode     state, enabled;
    astg_generator gen;
    astg_state     *state_p;
...
static void fprint_cube(pcube c, pPLA PLA) {
    int i;
