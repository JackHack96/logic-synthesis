...
network_t *astg_encode(graph_t *stg, astg_graph *astg, int heuristic) {
    network_t *network;
    int       initial;
    lsGen     gen;
    vertex_t  *state;
    ...
    switch (stg_get_num_states(stg)) {
        case 0: fail("STG with no states?");
            /* NOTREACHED */
            break;
        case 1:break;
        case 2:initial = 1;
            ...
            break;
        default:
            if (!extract_and_solve_cons(stg, heuristic))
                return NIL(network_t);
            break;
    }
...
static void dic_add(int l1, int l2, int r1, int r2, int n, pset dic, dic_family_t *list) {
...
static int extract_and_solve_cons(graph_t *stg, int heuristic) {
    edge_t       *edge;
    ...
    stg_foreach_transition(stg, gen, edge) {
        ...
        if (!st_lookup(input_label_table, input_label,
            ...
            if (!st_lookup(next_state_table, (char *) to_state, (char **) &dic_array)) {

                dic_array = array_alloc(vertex_t *, 0);
                array_insert_last(vertex_t *, dic_array, from_state);
                assert(
                        !st_insert(next_state_table, (char *) to_state, (char *) dic_array));
...
static void encode_stg(graph_t *stg, sm_row *cover, sm_matrix *table, dic_family_t *prime_list) {
    int        i, j, state_bits;
    char       **codes;
    sm_element *elem;
    sm_col     *col;
    ...
    state_bits = 0;
    sm_foreach_row_element(cover, elem) { state_bits++; }
...
static network_t *stg_to_net(graph_t *stg, astg_graph *astg) {
    network_t *network;
    FILE      *fp;
