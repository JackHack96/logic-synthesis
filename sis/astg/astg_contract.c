...
static void print_place_info (p)
astg_place *p;
{
    astg_generator gen;
    astg_edge *e;
...
static astg_bool check_62 (stg,te)
astg_graph  *stg;
astg_vertex *te;
{
    astg_edge *e1, *e2, *e3;
...
static astg_bool check_68a1 (te)
astg_vertex *te;
{
    astg_edge *e;
    astg_vertex *place;
...
static astg_bool check_68a2 (te)
astg_vertex *te;
{
    astg_edge *e;
    astg_vertex *place;
...
static astg_bool check_68b (stg,te)
astg_graph  *stg;
astg_vertex *te;
{
    ...
    if (astg_in_degree  (te) <= 1) return ASTG_TRUE;
    if (astg_out_degree (te) <= 1) return ASTG_TRUE;
...
static astg_bool check_68 (stg,te)
astg_graph  *stg;
astg_vertex *te;
{
...
static astg_bool can_contract_trans (stg,te,keep_fc)
astg_graph  *stg;
astg_vertex *te;
...
static astg_bool can_contract (stg,sig_p,keep_fc)
astg_graph *stg;
astg_signal *sig_p;
astg_bool   keep_fc;
{
...
astg_vertex *te;
{
    ...
    astg_foreach_in_edge (te,gen1,e1) {
        pin = astg_tail (e1);
        if (astg_out_degree(pin) == 1) astg_delete_place (pin);
    }
    astg_foreach_out_edge (te,gen2,e2) {
        pout = astg_head (e2);
        if (astg_in_degree(pout) == 1) astg_delete_place (pout);
    }
    astg_delete_trans (te);
}

static void keep_in_sig (stg,sv,outsig)
