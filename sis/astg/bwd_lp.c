 ...
 */
static void new_value(node_t *node, int new) {
    if ((GET_VALUE(node) & UNDEF) || (new & UNDEF)) {
        SET_VALUE(node, new);
    } else {
        if (new) {
            switch (GET_VALUE(node)) {
                case 1:SET_VALUE(node, 1);
                    break;
                case 0:SET_VALUE(node, UP);
                    break;
                case DOWN:
                    fprintf(siserr, "error: up and down transition for %s\n",
                            node_name(node));
                    break;
                case UP:SET_VALUE(node, UP);
                    break;
                default:break;
            }
        } else {
            switch (GET_VALUE(node)) {
                case 0:SET_VALUE(node, 0);
                    break;
                case 1:SET_VALUE(node, DOWN);
                    break;
                case UP:
                    fprintf(siserr, "error: up and down transition for %s\n",
                            node_name(node));
                    break;
                case DOWN:SET_VALUE(node, DOWN);
                    break;
                default:break;
            }
        }
 ...
 */
static void sim_node(node_t *node) {
    int    i, value;
    ...
    switch (node_function(func)) {
        case NODE_0:new_value(node, 0);
            break;
        case NODE_1:new_value(node, 1);
            break;
        default:break;
    }
    if (astg_debug_flag > 3) {
        switch (GET_VALUE(node)) {
            case 0:fprintf(sisout, "    %s 0\n", node_long_name(node));
                break;
            case 1:fprintf(sisout, "    %s 1\n", node_long_name(node));
                break;
            case UP:fprintf(sisout, "    %s +\n", node_long_name(node));
                break;
            case DOWN:fprintf(sisout, "    %s -\n", node_long_name(node));
                break;
            default:break;
        }
    }
...
static void sim_network(network_t *network, array_t *node_vec, st_table *pi_index, pset cb, int change_index,
                        int change_val) {
    int    i;
    node_t *node, *pi;
    lsGen  gen;
    ...
    foreach_primary_input(network, gen, pi) {
        if (!st_lookup(pi_index, bwd_po_name(node_long_name(pi)), (char **) &i)) {
        ...
        if (change_index == i) {
            ...
            switch (GETINPUT(cb, i)) {
                case ONE:new_value(pi, 1);
                    break;
                case ZERO:new_value(pi, 0);
                    break;
                default:new_value(pi, UNDEF);
                    break;
            }
        }
 ...
 */
static void find_subnet(node_t *node, st_table *subnet) {
    node_t *fanin;
    int    i;
 ...
 */
static double subnet_delay(array_t *node_vec, node_t *po, st_table *subnet1, st_table *subnet2, int upper_bound,
    ...
    for (j = 0; j < array_n(node_vec); j++) {
        ...
        if (node_num_fanin(node) == 0) {
            ...
            cnt = 0;
            foreach_fanin(node, i, fanin) {
                if (!st_lookup(subnet1, (char *) fanin, (char **) &fanin_dir)) {
                    continue;
                }
                cnt++;
                ...
                if (astg_debug_flag > 4) {
                    if (node_dir == UP) {
                        fprintf(sisout, "%s+ from %s %f\n", node_long_name(node),
                                node_long_name(fanin), BWD(fanin)->arrival + delta);
                    } else {
                        fprintf(sisout, "%s- from %s %f\n", node_long_name(node),
                                node_long_name(fanin), BWD(fanin)->arrival + delta);
                    }
        ...
        if (node_dir == UP) {
            if (astg_debug_flag > 3) {
                fprintf(sisout, "%s+ %f\n", node_long_name(node), BWD(node)->arrival);
            }
        } else {
            if (astg_debug_flag > 3) {
                fprintf(sisout, "%s- %f\n", node_long_name(node), BWD(node)->arrival);
 ...
 */
static double
hazard_delay(network_t *network, st_table *pi_index, node_t *po, hazard_t *hazard, double min_delay_factor) {
    node_t   *node;
    array_t  *node_vec;
 ...
 * subset
 */
static int find_astg_subset(astg_vertex *v) {
    astg_edge      *e;
    astg_generator gen;
 ...
 */
static void ts_dfs(astg_graph *g, astg_vertex *v, int *num_p) {
    astg_generator gen;
    astg_edge      *e;

    if (v->unprocessed) {
        v->unprocessed = ASTG_FALSE;
        astg_foreach_out_edge(v, gen, e) { ts_dfs(g, astg_head(e), num_p); }
 ...
 */
static double min_del(astg_trans *from, astg_trans *to, st_table *external_del, double default_del) {
    st_table  *to_table;
    char      *n;
    min_del_t *delay;
    ...
    switch (astg_trans_type(from)) {
        case ASTG_POS_X:
            switch (astg_trans_type(to)) {
                case ASTG_POS_X:return delay->rise.rise;
                case ASTG_NEG_X:return delay->rise.fall;
            }
        case ASTG_NEG_X:
            switch (astg_trans_type(to)) {
                case ASTG_POS_X:return delay->fall.rise;
                case ASTG_NEG_X:return delay->fall.fall;
            }
        default:break;
    }
...
static void print_constr(FILE *file, constraint_t *constr) {
    int i;
    ...
    switch (constr->type) {
        case EQ:fprintf(file, "=");
            break;
        case GE:fprintf(file, ">=");
            break;
        case LE:fprintf(file, "<=");
            break;
        default: fail("illegal LP constraint type");
    }
    fprintf(file, "%f\n", constr->val);
}

/* define an order, and stick to it... "level" does not matter */
static int constr_cmp(constraint_t *c1, constraint_t *c2) {
    int result, i;
...
static void add_constr(array_t *constrs, constraint_t *constr) {
...
static void add_constr_1(array_t *constrs, int coeff, int var, int type, int level, double val) {
    constraint_t *constr;
...
static void
add_constr_3(array_t *constrs, int coeff1, int var1, int coeff2, int var2, int coeff3, int var3, int type, int level,
             double val) {
    constraint_t *constr;
 ...
 */
static int write_constr(astg_trans *t1, astg_trans *t2, array_t *constrs, array_t *order, array_t *names, array_t *min_var,
                        st_table *external_del, st_table *sig_table, int pair, int *level, double delta,
                        double default_del) {
    ...
    for (k      = 0; k < array_n(order); k++) {
        ...
        astg_foreach_input_place(to, pgen,
                                 p) {
            astg_foreach_input_trans(p, tgen, from) {
                 ...
                 */
                if (!from->subset ||
                    !st_lookup(trans_table, (char *) from, (char **) &from_n)) {
                    continue;
                }
                /* create a constraint
                ...
                assert(st_lookup(sig_table, astg_signal_name(astg_trans_sig(to)),
                                 (char **) &sig_n));

                add_constr_3(constrs, 1, to_n, -1, from_n, -1, sig_n, GE, *level, w);
            }
        }
        (*level)++;
    }
...
static double solve_lp(array_t *constrs, array_t *min_var, array_t *names, double *slow, int *eval, int lindo) {
    ...
    if (lindo) {
        ...
        /* analyze the output */
        if (astg_debug_flag > 1) {
            sprintf(buf, "cat %s; tr a-z A-Z < %s | awk -f lindo.awk > %s", out, out,
                    tmp);
        } else {
            sprintf(buf, "tr a-z A-Z < %s | awk -f lindo.awk > %s", out, tmp);
        }
        if (astg_debug_flag > 1) {
            fprintf(sisout, "%s\n", buf);
        }
        ...
        for (i = 0; i < array_n(constrs); i++) {
            ...
            switch (constr->type) {
                ...
                case EQ:m3++;
                    break;
                default: fail("illegal linear constraint type");
            }
        }
        m      = m1 + m2 + m3;
        n      = array_n(names);
        ...
        switch (icase) {
            case 1:fprintf(siserr, "unbounded solution ???\n");
                return -2;
            case -1:
                if (astg_debug_flag > 1) {
...
static void
write_lp_recur(array_t *constrs, array_t *min_var, array_t *names, double *slow, double *best_slow, int n, int do_bound,
               int lindo, int *bound, int *eval, double *best_cost) {
    int          i, cnt;
    constraint_t *constr, *constr1;
    char         *str;
    double       cost;
    ...
    for (i = 0; i < cnt; i++) {
        ...
        write_lp_recur(constrs, min_var, names, slow, best_slow, n + cnt, do_bound,
                       lindo, bound, eval, best_cost);
        constr->type = GE;
    }
}
 ...
 */
void
bwd_lp_slow(astg_graph *astg, network_t *network, st_table *hazard_list, st_table *external_del, st_table *pi_index,
            double tol, double default_del, double min_delay_factor, int do_bound, int lindo) {
    ...
    foreach_primary_output(network, gen, po) {
        ...
        for (i = 0; i < array_n(hazards); i++) {
            ...
            delta = hazard_delay(network, pi_index, po, &hazard, min_delay_factor);

            if (astg_debug_flag > 2) {
                fprintf(sisout, "%s%c -> %s%c for %s d2 - d1 %f\n",
                        astg_signal_name(sig2), hazard.dir2, astg_signal_name(sig1),
                        hazard.dir1, bwd_po_name(node_long_name(po)), delta);
            }
            if (delta < 0) {
                if (astg_debug_flag > 2) {
            ...
            for (j = 0; j < array_n(astg->mg_comp); j++) {
                ...
                astg_foreach_trans(astg, tgen1, t1) {
                    if (!t1->active || astg_trans_sig(t1) != sig1 ||
                        astg_trans_type(t1) != type1) {
                        continue;
                    }
                    astg_foreach_trans(astg, tgen2, t2) {
                        if (!t2->active || astg_trans_sig(t2) != sig2 ||
                        ...
                        if (num) {
                            astg_foreach_vertex(astg, vgen, v) {
                                ...
                                array_insert(astg_vertex *, order, (num - v->alg.ts.index - 1),
                                             v);
                                v->weight1.f = 0;
                            }
                        }
                        ...
                        for (k = 0; k < array_n(order); k++) {
                            ...
                            astg_foreach_input_place(to, pgen, p) {
                                astg_foreach_input_trans(p, tgen, from) {
                                    ...
                                    w = min_del(from, to, external_del, default_del);
                                    to->weight1.f = MAX(to->weight1.f, from->weight1.f + w);
                                }
                            }
                            if (astg_debug_flag > 3) {
                                fprintf(sisout, " %s %f", astg_v_name(to), to->weight1.f);
                            }
                        }
                        if (astg_debug_flag > 3) {
                            fprintf(sisout, "\n");
                        ...
                        if (t1->weight1.f > delta) {
                            if (astg_debug_flag > 2) {
                                fprintf(sisout, "%s -> %s trivially satisfied (%f > %f)\n",
                                        astg_trans_name(t2), astg_trans_name(t1), t1->weight1.f,
                                        delta);
                            }
                            array_free(order);
                            continue;
                        ...
                        pair++;
                        if (astg_debug_flag > 2) {
                            fprintf(sisout, "%s -> %s produces constraint\n",
                                    astg_trans_name(t2), astg_trans_name(t1));
                        }
                        write_constr(t1, t2, constrs, order, names, min_var, external_del,
                                     sig_table, pair, &level, delta, default_del);
    ...
    foreach_node(network, gen, node) { FREE(node->BWD_SLOT); }
    ...
    if (array_n(constrs)) {
        ...
        write_lp_recur(constrs, min_var, names, slow, best_slow, 0, do_bound, lindo,
                       &bound, &eval, &best_cost);
