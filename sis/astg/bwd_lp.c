 ...
 */
static void new_value(node, new) node_t *node;
int new;
{
 ...
 */
static void sim_node(node) node_t *node;
{
  int i, value;
...
static void sim_network(network, node_vec, pi_index, cb, change_index,
                        change_val) network_t *network;
array_t *node_vec;
st_table *pi_index;
pset cb;
 ...
 */
static void find_subnet(node, subnet) node_t *node;
st_table *subnet;
{
  node_t *fanin;
  int i;
 ...
 */
static double subnet_delay(node_vec, po, subnet1, subnet2, upper_bound,
                           min_delay_factor) array_t *node_vec;
node_t *po;
 ...
 */
static double hazard_delay(network, pi_index, po, hazard,
 ...
 * subset
 */
static int find_astg_subset(v) astg_vertex *v;
{
  astg_edge *e;
  astg_generator gen;
 ...
 */
static void ts_dfs(g, v, num_p) astg_graph *g;
astg_vertex *v;
int *num_p;
{
  astg_generator gen;
 ...
 */
static double min_del(from, to, external_del, default_del) astg_trans *from,
    *to;
...
static void print_constr(file, constr) FILE *file;
constraint_t *constr;
{
  int i;
...
static int constr_cmp(c1, c2) constraint_t *c1, *c2;
{
  int result, i;
...
static void add_constr(constrs, constr) array_t *constrs;
constraint_t *constr;
{
...
static void add_constr_1(constrs, coeff, var, type, level,
                         val) array_t *constrs;
...
static void add_constr_3(constrs, coeff1, var1, coeff2, var2, coeff3, var3,
                         type, level, val) array_t *constrs;
 ...
 */
static int write_constr(t1, t2, constrs, order, names, min_var, external_del,
                        sig_table, pair, level, delta,
                        default_del) astg_trans *t1,
    *t2;
...
double delta, default_del;
{
  ...
  for (k = 0; k < array_n(order); k++) {
    ...
    astg_foreach_input_place(to, pgen,
                             p){astg_foreach_input_trans(p, tgen, from){
         ...
         */
        if (!from->subset ||
            !st_lookup(trans_table, (char *)from, (char **)&from_n)){continue;
  }
  /* create a constraint
...
static double solve_lp(constrs, min_var, names, slow, eval,
                       lindo) array_t *constrs,
    *min_var, *names;
double *slow;
int *eval, lindo;
{
...
static void write_lp_recur(constrs, min_var, names, slow, best_slow, n,
                           do_bound, lindo, bound, eval,
                           best_cost) array_t *constrs,
    *min_var, *names;
 ...
 */
void bwd_lp_slow(astg, network, hazard_list, external_del, pi_index, tol,
                 default_del, min_delay_factor, do_bound,
                 lindo) astg_graph *astg;
network_t *network;
