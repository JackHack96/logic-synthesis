 ...
 */
void bwd_external_delay_update(network_t *network, st_table *external_del, double min_delay_factor, int silent) {
    st_table     *to_table;
    node_t       *pi, *po, *node;
    delay_time_t delay;
 ...
 * "is_max").
 */
static delay_time_t delay_simulate(network_t *network, node_t *from, node_t *to, int is_max, double min_delay_factor) {
    lsGen        gen;
    node_t       *pi, *node;
    delay_time_t delay1;
 ...
 */
static void
fill_d3s(network_t *network, double **d3s, int nd3s, st_table *pio, st_table *external_del, int shortest_path,
         double default_del, double min_delay_factor) {
    int          from, to, iter;
    lsGen        gen, gen1;
...
void
bwd_slow_down(network_t *network, st_table *hazard_list, st_table *slowed_amounts, st_table *external_del, double tol,
              double default_del, double min_delay_factor, int shortest_path, int iterate, int do_slow) {
    char         *buf;
    ...
    FREE(d3s);
    st_foreach_item(pio, pigen, &buf, NIL(
            char *)) { FREE(buf); }
    st_free_table(pio);
