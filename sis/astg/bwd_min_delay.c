...
int bwd_min_delay_trace(network_t *network, delay_model_t model) {
    int          i;
    node_t       *node;
    array_t      *nodevec;
    double       latest;
 ...
 */
network_t *min_delay_generate_decomposition(node_t *node, double mode) {
    network_t    *network;
    int          i, found = 0;
...
static delay_time_t compute_delay(node_t *node, delay_pin_t *pin_delay, delay_model_t model) {
    ...
    switch (model) {
        ...
        case DELAY_MODEL_MAPPED:delay.rise = pin_delay->drive.rise * DELAY(node)->load;
...
static void delay_error(char *string) {
    if (string != NIL(char)) {
        error_append(string);
    }
...
node_t *min_delay_latest_output(network_t *network, double *latest_p) {
