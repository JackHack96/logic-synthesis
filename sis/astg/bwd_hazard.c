...
static set_family_t *cb_intersect(pset pi, set_family_t *B) {
...
static void bit_to_cube(astg_graph *stg, astg_scode bits, pset cb) {
    register int        i = 0;
    astg_signal         *sig_p;
...
static int func_value(set_family_t *F, pset vector) {
...
void bwd_dup_hazard(hazard_t *new, hazard_t *old) {
    new->dir1    = old->dir1;
    new->dir2    = old->dir2;
...
void bwd_free_hazard(hazard_t *haz) {
    FREE(haz->s1);
    FREE(haz->s2);
    set_free(haz->on_cb1);
...
static int same_hazard(hazard_t *t1, hazard_t *t2) {
    if (t1->dir1 != t2->dir1 || t1->dir2 != t2->dir2) {
        return 0;
    }
 ...
 */
static void
store_hazards_old(array_t *fanin, array_t *hazards, set_family_t *F, set_family_t *R, pset cb, pset top, pset bot,
                  int onset, char *name) {
    ...
    foreach_set(R_1, last, off_cube) {
        ...
        foreach_set(F_1, last1, on_cube1) {
            if (!TESTP(on_cube1, ONEFROMTOP) || cdist01(on_cube1, off_cube) != 1) {
                continue;
            }
            ...
            foreach_set(F_1, last2, on_cube2) {
                if (on_cube1 == on_cube2 || !TESTP(on_cube2, ONEFROMBOT) ||
                ...
                if (i >= array_n(hazards)) {
                    ...
                    if (astg_debug_flag > 0) {
                        fprintf(sisout, "potential hazard %s%c -> %s%c -> %s (%d)\n",
                                hazard.s2, hazard.dir2, hazard.s1, hazard.dir1, name,
                                hazard.on);
                    }
                }
            }
 ...
 */
static void
store_hazards(array_t *fanin, array_t *hazards, set_family_t *F, set_family_t *R, pset cb, pset top, pset bot,
              int onset, char *name) {
 ...
 */
static void
find_bot(array_t *fanin, array_t *hazards, set_family_t *F, set_family_t *R, astg_graph *stg, astg_scode sig_bit,
         astg_scode state_code, st_table *reached, pset cb, pset top, pset bot, int onset, int use_old, char *name) {
    ...
    astg_foreach_signal(stg, sgen, sig_p) {
        ...
        find_bot(fanin, hazards, F, R, stg, sig_bit, new_state, reached, cb, top,
                 bot, onset, use_old, name);
 ...
 */
static void find_not_top(set_family_t *F, astg_graph *stg, astg_scode sig_bit, astg_scode state_code, st_table *reached,
                         st_table *not_top, pset bot, int onset) {
    astg_scode     enabled, to_fire, new_state;
    astg_signal    *sig_p;
    astg_generator sgen;
    ...
    astg_foreach_signal(stg, sgen, sig_p) {
        ...
        find_not_top(F, stg, sig_bit, new_state, reached, not_top, bot, onset);
    }
}
...
void bwd_find_hazards(astg_graph *stg, astg_signal *sig_p, array_t *fanin, set_family_t *node_F, set_family_t *node_R,
                      st_table *hazard_list, char *name, int use_old) {
    astg_generator gen;
 ...
 */
        find_bot(fanin, hazards, F, R, stg, sig_bit, state_code, reached, cb, top,
                 bot, onset, use_old, sig_p
                         ->name);
