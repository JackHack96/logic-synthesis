...
static set_family_t *cb_intersect(pi, B) pset pi;
set_family_t *B;
{
...
static void bit_to_cube(stg, bits, cb) astg_graph *stg;
astg_scode bits;
pset cb;
{
  register int i = 0;
...
static int func_value(F, vector) set_family_t *F;
pset vector;
{
...
void bwd_dup_hazard(new, old) hazard_t *new, *old;
{
  new->dir1 = old->dir1;
  new->dir2 = old->dir2;
...
void bwd_free_hazard(haz) hazard_t *haz;
{
  FREE(haz->s1);
  FREE(haz->s2);
  set_free(haz->on_cb1);
...
static int same_hazard(t1, t2) hazard_t *t1, *t2;
{
 ...
 */
static void store_hazards_old(fanin, hazards, F, R, cb, top, bot, onset,
                              name) array_t *fanin,
    *hazards;
set_family_t *F, *R;
pset cb, top, bot;
 ...
 */
static void store_hazards(fanin, hazards, F, R, cb, top, bot, onset,
                          name) array_t *fanin,
    *hazards;
set_family_t *F, *R;
pset cb, top, bot;
 ...
 */
static void find_bot(fanin, hazards, F, R, stg, sig_bit, state_code, reached,
                     cb, top, bot, onset, use_old, name) array_t *fanin,
    *hazards;
set_family_t *F, *R;
astg_graph *stg;
 ...
 */
static void find_not_top(F, stg, sig_bit, state_code, reached, not_top, bot,
                         onset) set_family_t *F;
astg_graph *stg;
astg_scode sig_bit, state_code;
...
void bwd_find_hazards(stg, sig_p, fanin, node_F, node_R, hazard_list, name,
                      use_old) astg_graph *stg;
astg_signal *sig_p;
array_t *fanin;
