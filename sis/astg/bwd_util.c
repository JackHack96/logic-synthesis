...
astg_scode bwd_astg_state_bit(sig_p) astg_signal *sig_p;
 ...
 */
edge_t *stg_create_nfa_transition(from, to, in, out) vertex_t *from, *to;
char *in, *out;
{
  edge_t *edge;
  graph_t *stg;
...
int bwd_log2(arg) astg_scode arg;
{
  astg_scode i;
  int value;
...
void bwd_pedge(edge) edge_t *edge;
{
...
/* auxiliary debugging function */
void bwd_pstate(state) vertex_t *state;
{
  lsGen gen;
  edge_t *edge;
...
vertex_t *bwd_get_state_by_name(stg, name) graph_t *stg;
char *name;
{
  vertex_t *state;
  lsGen gen;
...
static void print_sg(astg, stg, edge_trans) astg_graph *astg;
graph_t *stg;
st_table *edge_trans;
{
  vertex_t *state, *to;
 ...
 */
astg_scode bwd_new_enabled_signals(edge, edge_trans,
...
char *bwd_marking_string(marked_places) astg_ba_rec *marked_places;
{
  int i;
...
int duplicate(stg, from, to, in, out) graph_t *stg;
vertex_t *from, *to;
char *in, *out;
{
  lsGen gen;
 ...
 */
static void stg_patch(stg, state_out, edge_trans, marking_state, state_marking,
                      old, new, from_stack, from_sp) graph_t *stg;
 ...
 */
static vertex_t *astg_to_stg_recur(flow_info, astg, stg, state, curr_marking,
                                   in, out, buf, use_output, edge_trans,
                                   state_marking, marking_state, state_out,
                                   curr_trans, curr_i, from_stack,
                                   from_sp) astg_flow_t *flow_info;
astg_graph *astg;
graph_t *stg;
astg_scode state;
...
static void stg_set_prod_states(stg) graph_t *stg;
{
  lsGen gen;
  int n;
  vertex_t *state;
...
graph_t *bwd_reduced_stg(stg, partitions, orig_stg_names, names,
                         nstates) graph_t *stg;
 ...
 */
static void remove_subset_states(state1, partitions, class, saved_names,
                                 edge_trans, state_marking,
                                 pre_minimize) vertex_t *state1;
st_table *partitions;
int class;
array_t *saved_names;
st_table *edge_trans, *state_marking;
 ...
 */
static graph_t *merge_equivalent(stg, pre_minimize, edge_trans, state_marking,
...
int bwd_marking_hash(key, modulus) char *key;
int modulus;
{
  astg_ba_rec *ba;
#ifdef UNSAFE
...
int bwd_marking_ptr_cmp(c1, c2) char *c1, *c2;
{
  char *p1, *p2;
...
int bwd_marking_cmp(c1, c2) char *c1, *c2;
{
  int n;
  astg_marking *m1, *m2;
  astg_ba_rec *ba1, *ba2;
 ...
 */
graph_t *bwd_astg_to_stg(astg, code, use_output, pre_minimize) astg_graph *astg;
astg_scode code;
 ...
 */
char *bwd_po_name(name) char *name;
{
  int len;
  static char buf[256];
 ...
 * The result is a statically allocated area !!.
 */
char *bwd_fake_po_name(name) char *name;
{
  static char buf[256];
 ...
 * The result is a statically allocated area !!.
 */
char *bwd_fake_pi_name(name) char *name;
{
  static char buf[256];
...
void bwd_astg_latch(network, latch_type, keep_red) network_t *network;
 ...
 * is sc.
 */
static int scc_search(v, df, stack_st, components, stack_arr, lev) vertex_t *v;
...
static int covers(in1, in2) char *in1, *in2;
{
  int i;
...
static int compatible(in1, in2) char *in1, *in2;
{
  int i;
...
static vertex_t *search_scr_state(new_stg, states, name, in, out, buf, code,
                                  new) graph_t *new_stg;
array_t *states;
...
static void stg_scr_recur(buf, states, old_from, old_stg, new_stg) char *buf;
array_t *states;
vertex_t *old_from;
...
static int is_burst_subset(loop, in1, in2) char *loop, *in1, *in2;
{
  int i;
...
graph_t *bwd_stg_scr(old_stg) graph_t *old_stg;
{
  graph_t *new_stg;
  lsGen gen, gen1, gen2;
...
astg_place *stg_to_astg_recur(from, stg, astg, finedge, foutedge, finbuf,
                              foutbuf, statebuf, innames, outnames, stnames,
                              dummyname) vertex_t *from;
graph_t *stg;
astg_graph *astg;
...
astg_graph *bwd_stg_to_astg(stg, network) graph_t *stg;
network_t *network;
{
  astg_graph *astg;
  lsGen gen;
...
static void string_to_set(set, str) pset set;
char *str;
{
  int i;
...
static void set_to_string(str, set, n) char *str;
pset set;
int n;
{
  int i;
 ...
 */
void bwd_dc_to_self_loop(stg) graph_t *stg;
{
  set_family_t *F, *R;
  pset p, p1, last;
  int nin, nout;
...
void bwd_write_sg(fp, stg, astg) FILE *fp;
graph_t *stg;
astg_graph *astg;
{
  lsGen gen, gen2;
