...
static void
constrain (str, astg, trans1, trans2)
char *str;
astg_graph *astg;
astg_trans *trans1, *trans2;
{
    ...
    *pairptr = pair;
    ...
    fprintf (siserr, "warning: %s: may need constraint %s -> %s\n",
        str, astg_trans_name (trans1), astg_trans_name (trans2));
}
...
static equiv_t *
equiv_alloc (nstates, nclasses)
int nstates, nclasses;
{
    equiv_t *equiv_states;
    pset p, last;
    ...
    equiv_states->state_index = st_init_table_with_params (st_ptrcmp,
        st_ptrhash, ST_DEFAULT_INIT_TABLE_SIZE, /* density */ 1,
    ...
    foreach_set (equiv_states->classes, last, p) {
        set_clear (p, nstates);
    }
    return equiv_states;
}
...
equiv_t *
bwd_read_equiv_states (file, stg)
FILE *file;
graph_t *stg;
{
    ...
    while (fgets (buf, (sizeof(buf)/sizeof(char)), file) != NULL) {
        ...
        if (n != 2) {
            fprintf (siserr,
                "invalid class information in minimizer output:\n%s\n", buf);
            equiv_free (equiv_states);
            return NIL(equiv_t);
        }
        if (! st_lookup (global_orig_stg_names, (char *) name,
            (char **) &names)) {
            fprintf (siserr,
                "invalid class information in minimizer output:\n%s\n", buf);
            equiv_free (equiv_states);
        ...
        for (j = 0; j < array_n (names); j++) {
            state = bwd_get_state_by_name (global_orig_stg,
                array_fetch (char *, names, j));
            if (! st_lookup (equiv_states->state_index, (char *) state,
                (char **) &i)) {
                st_insert (equiv_states->state_index, (char *) state,
                    (char *) idx);
                array_insert (vertex_t *, equiv_states->index_state, idx,
                    state);
                i = idx++;
            }
    ...
    if (idx != nstates) {
        fprintf (siserr,
    "not enough states in class information in minimizer output (%d vs. %d)\n",
            idx, nstates);
        equiv_free (equiv_states);
        return NIL(equiv_t);
    }
    ...
    if (astg_debug_flag > 1) {
        ...
        foreachi_set (equiv_states->classes, i, p) {
            ...
            for (j = 0; j < array_n (equiv_states->index_state); j++) {
                if (is_in_set (p, j)) {
                    state = array_fetch (vertex_t *,
                        equiv_states->index_state, j);
 ...
 */
static int
distinguishable (edge, subset, edge_trans, state_marking)
edge_t *edge;
astg_scode subset;
 ...
 */
static void
br_dfs_bdd(mg, f, id_table, order, start_num, total_num_ptr)
bdd_manager *mg;
bdd_node *f;
 ...
 */
static st_table *
br_shortestpath(f, wtt_table, wte_table)
bdd_t *f;
st_table *wtt_table, *wte_table;
{
    ...
    /* cleanup */
    FREE(length); FREE(phase); FREE(terminal);  FREE(parent);
...
static astg_scode
mdd_subset (mgr, mdd, sig_mdd_var, wtt_table)
mdd_manager *mgr;
mdd_t *mdd;
st_table *sig_mdd_var, *wtt_table;
{
    ...
    st_foreach_item (sig_mdd_var, stgen, (char **) &sig_p, (char **) &lit) {
        if (! st_lookup (var_phase, (char *) lit->node->id,
            (char **) &phase)) {
            continue;
        }
        if (phase) {
 ...
 */
static astg_scode
find_partitioning_signals (astg, stg, equiv_states, index_sig, visited, mincov_option, use_mdd, edge_trans, state_marking)
astg_graph *astg;
graph_t *stg;
equiv_t *equiv_states;
array_t *index_sig;
...
st_table *edge_trans, *state_marking;
{
    ...
    for (s1 = 0; s1 < nstates; s1++) {
        ...
        if (use_mdd) {
#ifdef MDD
            assert (! st_insert (state_mdd_var, (char *) state1, (char *) array_n (mvar_sizes)));
                    ...
                    for (cl2 = cl1 + 1; cl2 < nclasses; cl2++) {
                        ...
                        if (astg_debug_flag > 3) {
                            fprintf (sisout, " %d: ! ( %s %d ) ! ( %s %d )\n",
                                row, stg_get_state_name (state1), cl1,
    ...
    if (use_mdd) {
        ...
        st_foreach_item (state_mdd_var, stgen, (char **) &state1,
            (char **) &v) {
        ...
        astg_foreach_signal (astg, sgen, sig_p) {
            ...
            st_insert (wtt_table, (char *) lit1->node->id, (char *)
                signal_cost (sig_p, astg));
        }
#endif
    }
    ...
    if (use_mdd) {
        ...
        cover = sm_mat_bin_minimum_cover (M, weight, /* heuristic */ 0,
            mincov_debug, /* ubound */ 0, /* option */ mincov_option,
            /* record_fun */ (int(*)()) 0);
...
static int
is_forbidden (cur_trans_sets, edge_trans, edge)
st_table *cur_trans_sets, *edge_trans;
edge_t *edge;
{
    array_t *trans_arr;
    int i;
    ...
    do {
        ...
        for (j = 0; j < array_n (trans_comb); j++) {
            ...
            if (astg_debug_flag > 2 && st_count (cur_trans_sets)) {
                ...
                st_foreach_item (cur_trans_sets, stgen, (char **) &trans,
                    NIL(char *)) {
             ...
             */
            st_foreach_item (in_to_edges, stgen, (char **) &input,
                (char **) &edges) {
                if (astg_debug_flag > 3) {
                ...
                foreachi_set (equiv_states->classes, cl, class) {
                    ...
                    for (e = 0; e < array_n (edges); e++) {
                        ...
                        assert (st_lookup (equiv_states->state_index, (char *) from, (char **) &s));
                        ...
                        assert (st_lookup (equiv_states->state_index, (char *) to, (char **) &s));
                        part = array_fetch (pset, allowed_partitions, s);
                        set_and (intersection, intersection, part);
                        if (astg_debug_flag > 3) {
                            fprintf (sisout, "%s -> %s part %s ",
                                stg_get_state_name (from),
...
static int *
choose_class (stg, allowed_partitions, equiv_states, orig_stg_names)
graph_t *stg;
array_t *allowed_partitions;
equiv_t *equiv_states;
st_table *orig_stg_names;
{
    ...
    st_foreach_item (orig_stg_names, stgen, (char **) &str,
        (char **) &names) {
        if (astg_debug_flag > 3) {
        ...
        for (s = 0; s < array_n (names); s++) {
            ...
            if (astg_debug_flag > 3) {
                fprintf (sisout, "%s part %s ",
                    stg_get_state_name (state), ps1(part));
                fprintf (sisout, "intersection %s\n", ps1(intersection));
            }
        ...
        for (s = 0; s < array_n (names); s++) {
            ...
            if (astg_debug_flag > 2) {
                if (set_ord (intersection) > 1) {
                    fprintf (sisout, "multiple choice for %s:",
                        stg_get_state_name (state));
            ...
            for (cl = 0; cl < equiv_states->classes->count; cl++) {
                if (is_in_set (intersection, cl)) {
                    ...
                    assert (st_lookup (equiv_states->state_index, (char *) state, (char **) &idx));
                    index_part[idx] = cl;
                    break;
                }
            }
 ...
 */
static int
check_this_partition_recur (state1, cl, equiv_states, subset, visited, allowed_partitions, forward, rm, edge_trans, state_marking)
vertex_t *state1;
int cl;
equiv_t *equiv_states;
astg_scode subset;
...
st_table *edge_trans, *state_marking;
{
    ...
    result = 1;
    for (gen=lsStart(forward?g_get_out_edges(state1):g_get_in_edges(state1));
        lsNext(gen,(lsGeneric*)&edge,LS_NH)==LS_OK||((void)lsFinish(gen),0);) {
        state2 = stg_edge_to_state (edge);
        if (state2 == state1) {
            continue;
        }
         ...
         */
            if (! check_this_partition_recur (state2, cl, equiv_states,
                    subset, visited, allowed_partitions, forward,
                    rm, edge_trans, state_marking) &&
                is_in_set (part1, cl)) {
                if (astg_debug_flag > 3) {
                    fprintf (sisout, "Removing %d from %s due to %s\n",
                        cl, stg_get_state_name (state1),
                        stg_get_state_name (state2));
                }
                set_remove (part1, cl);
                result = 0;
            }
...
static int
check_this_partition (state1, cl, equiv_states, subset, visited, allowed_partitions, rm, edge_trans, state_marking)
vertex_t *state1;
int cl;
equiv_t *equiv_states;
astg_scode subset;
...
st_table *edge_trans, *state_marking;
{
    ...
    result = check_this_partition_recur (state1, cl, equiv_states, subset,
        visited, allowed_partitions, /* forward */ 1, rm, edge_trans,
        state_marking);
    if (rm || ! result) {
        set_clear (visited, array_n (equiv_states->index_state));
        result |= check_this_partition_recur (state1, cl, equiv_states, subset,
 ...
 */
static int *
check_partitions (stg, equiv_states, subset, visited, allowed_partitions, trans_comb, edge_trans, state_marking, orig_stg_names)
graph_t *stg;
equiv_t *equiv_states;
astg_scode subset;
pset visited;
...
st_table *edge_trans, *state_marking, *orig_stg_names;
{
    ...
    do {
        ...
        for (s = 0; s < array_n(equiv_states->index_state); s++) {
            ...
            foreachi_set (equiv_states->classes, cl, class) {
                set_clear (visited, array_n (equiv_states->index_state));
                if (! check_this_partition (state, cl, equiv_states, subset,
                    visited, allowed_partitions, /* rm */ 0,
                    edge_trans, state_marking)) {
                    (void) check_this_partition (state, cl, equiv_states,
                        subset, visited, allowed_partitions, /* rm */ 1,
                        edge_trans, state_marking);
                }
            }
        }
    }
    while (closure (stg, allowed_partitions, equiv_states, trans_comb,
        edge_trans));
...
static astg_scode
partition_states_recur (astg, stg, equiv_states, base_subset, visited, masks, allowed_partitions, index_sig, first_signal, first_mask, trans_comb, edge_trans, state_marking, orig_stg_names)
astg_graph *astg;
graph_t *stg;
equiv_t *equiv_states;
    ...
    for (m = first_mask; m < array_n (masks); m++) {
        ...
        for (i = first_signal; i < array_n (index_sig); i++) {
            ...
            /* at most this will return the current subset */
                subset = partition_states_recur (astg, stg, equiv_states,
                    subset, visited, masks, allowed_partitions,
                    index_sig, i + 1, m, trans_comb, edge_trans, state_marking,
                    orig_stg_names);
                if (subset_cost (subset, astg) <
                    subset_cost (best_subset, astg)) {
                    best_subset = subset;
                }
            }
            ...
            subset = partition_states_recur (astg, stg, equiv_states,
                base_subset, visited, masks, allowed_partitions,
                index_sig, i + 1, m, trans_comb, edge_trans, state_marking,
                orig_stg_names);
...
static void
forced_sets_recur (equiv_states, allowed_partitions, state1, visited, tmp, copy, edge_trans, state_marking)
equiv_t *equiv_states;
array_t *allowed_partitions;
vertex_t *state1;
pset visited, tmp;
...
st_table *edge_trans, *state_marking;
{
    ...
    if (copy) {
        if (! setp_equal (part1, tmp)) {
            ...
            if (astg_debug_flag > 3) {
                fprintf (sisout, "%s updated part %s\n",
                    stg_get_state_name (state1), ps1(part1));
            }
        }
    }
    else {
    ...
    foreach_state_outedge (state1, gen, edge) {
        ...
        forced_sets_recur (equiv_states, allowed_partitions, state2,
            visited, tmp, copy, edge_trans, state_marking);
    }
 ...
 */
static void
reduce_input_dependency (stg, equiv_states, allowed_partitions, index_sig, visited, edge_trans, state_marking)
graph_t *stg;
equiv_t *equiv_states;
...
st_table *edge_trans, *state_marking;
{
    ...
    do {
        ...
        stg_foreach_transition (stg, gen, edge) {
            ...
            new_enabled = bwd_new_enabled_signals (edge, edge_trans,
                state_marking);
            ...
            forced_sets_recur (equiv_states, allowed_partitions,
                state1, visited, tmp1, /* copy */ 0, edge_trans, state_marking);

            set_fill (tmp2, nclasses);
            set_clear (visited, nstates);
            forced_sets_recur (equiv_states, allowed_partitions,
                state2, visited, tmp2, /* copy */ 0, edge_trans, state_marking);
            ...
            if ((setp_equal (part1, tmp1) &&
                setp_equal (part2, tmp2) &&
            ...
            set_clear (visited, nstates);
            forced_sets_recur (equiv_states, allowed_partitions,
                state1, visited, tmp, /* copy */ 1, edge_trans, state_marking);
            set_clear (visited, nstates);
            forced_sets_recur (equiv_states, allowed_partitions,
                state2, visited, tmp, /* copy */ 1, edge_trans, state_marking);
        }
    } while (changed);
 ...
 */
static void
find_marking_pairs_recur (stg, astg, orig_stg_names, state_index, edge_trans, partitions, from, p_from, p_to, to_visit, from_set, to_set, cur_trans_sets)
graph_t *stg;
astg_graph *astg;
...
st_table *cur_trans_sets;
{
    ...
    foreach_state_outedge (from, gen, edge) {
        ...
        find_marking_pairs_recur (stg, astg,
            orig_stg_names, state_index, edge_trans,
            partitions, to, p_from, p_to,
            to_visit, from_set, to_set, cur_trans_sets);
    }
...
static void
check_multiple_exit (astg, stg, from, partitions, merged_head, orig_stg_names, edge_trans, index_sig)
astg_graph *astg;
graph_t *stg;
vertex_t *from;
                ...
                for (k1 = 0; k1 < array_n (trans_arr1); k1++) {
                    ...
                    for (k2 = 0; k2 < array_n (trans_arr2); k2++) {
                        ...
                        if (! is_conflict (trans1, trans2)) {
                            constrain (
                                "the STG may not be live (multiple exit point)",
                                astg, trans1, trans2);
                        }
                    }
                }
 ...
 */
static void
find_maximal_concurrent (astg, stg, state, marking, cur_fc, edge_trans, state_marking, trans_set, trans_sets)
astg_graph *astg;
graph_t *stg;
vertex_t *state;
...
array_t *trans_sets;
{
    ...
    /* split on "place" */
        astg_foreach_output_trans (place, tgen, trans) {
            ...
            find_maximal_concurrent (astg, stg, state, marking, fc,
    ...
    /* everything is concurrent: add a new element to the array */
        if (st_count (trans_set)) {
            if (astg_debug_flag > 1) {
                fprintf (sisout, "forbidden transitions for %s:",
                    stg_get_state_name (state));
                st_foreach_item (trans_set, stgen, (char **) &trans,
                    NIL(char *)) {
                    fprintf (sisout, " %s", astg_trans_name (trans));
                }
                fprintf (sisout, "\n");
            }
            array_insert_last (st_table *, trans_sets,
                st_copy (trans_set));
        }
    }
}
...
static void
create_combinations_recur (trans_comb, all_trans_sets, cur_trans_sets, i)
array_t *trans_comb, *all_trans_sets;
st_table *cur_trans_sets;
int i;
{
    int j;
    ...
    if (i >= array_n (all_trans_sets)) {
        ...
        for (j = 0; j < array_n (trans_sets); j++) {
            trans_set = array_fetch (st_table *,
                trans_sets, j);
            st_foreach_item (trans_set, stgen, (char **) &trans,
                NIL(char *)) {
            ...
            create_combinations_recur (trans_comb, all_trans_sets,
                cur_trans_sets, i + 1);
...
static void
find_to_visit (stg, to_visit, partitions, orig_stg_names, state_index, p_from, p_to)
graph_t *stg;
pset to_visit;
 ...
 */
static int
has_neighbor (astg, marking, markings, forward)
astg_graph *astg;
astg_marking *marking;
array_t *markings;
int forward;
 ...
 */
static st_table *
partition_states (astg, stg, equiv_states, use_mincov, greedy, go_down, mincov_option, use_mdd, edge_trans, state_marking, orig_stg_names)
astg_graph *astg;
graph_t *stg;
equiv_t *equiv_states;
...
st_table *edge_trans, *state_marking, *orig_stg_names;
{
    ...
    if (equiv_states->classes->count < 2) {
        /* too easy... */
        fprintf (siserr,
    "info: the STG has complete state coding (no state coding is necessary)\n");
    ...
    st_foreach_item (orig_stg_names, stgen, (char **) &str, (char **) &names) {
        ...
        for (k = 0; k < array_n (marking_arr); k++) {
            ...
            find_maximal_concurrent (astg, stg, state, marking, 0,
                edge_trans, state_marking, trans_set, trans_sets);
        }
    ...
    if (greedy > 1) {
        reduce_input_dependency (stg, equiv_states,
            allowed_partitions, index_sig, visited, edge_trans, state_marking);
    }
    /* find the initial subset */
    if (use_mincov || use_mdd) {
        assert (go_down);
        base_subset = find_partitioning_signals (astg, stg, equiv_states,
            index_sig, visited, mincov_option, use_mdd,
            edge_trans, state_marking);
    }
    else {
        base_subset = 0;
    ...
    if (use_mincov >= 2 || use_mdd >= 2) {
        index_part = check_partitions (stg, equiv_states, base_subset,
            visited, allowed_partitions, trans_comb, edge_trans,
            state_marking, orig_stg_names);
        best_subset = base_subset;
    }
    else {
        ...
        if (go_down) {
            ...
            if (greedy) {
                ...
                for (m = 0; m < array_n (masks); m++) {
                    ...
                    for (i = 0; i < array_n (index_sig); i++) {
                        ...
                        index_part = check_partitions (stg, equiv_states,
                            subset, visited, allowed_partitions, trans_comb,
                            edge_trans, state_marking, orig_stg_names);
                ...
                best_subset = partition_states_recur (astg, stg, equiv_states,
                    base_subset, visited, masks, allowed_partitions, index_sig,
                    0, 0, trans_comb, edge_trans, state_marking,
                    orig_stg_names);
            }
            if (astg_debug_flag > 1) {
            ...
            array_free (masks);
            index_part = check_partitions (stg, equiv_states, best_subset,
                    visited, allowed_partitions, trans_comb, edge_trans,
                    state_marking, orig_stg_names);
        }
        else {
    ...
    stg_foreach_state (stg, gen, from) {
        check_multiple_exit (astg, stg, from, partitions,
            merged_head, orig_stg_names, edge_trans, index_sig);
    }
        ...
        for (p_to = 0; p_to < npart; p_to++) {
            ...
            for (j = 0; j < array_n (trans_comb); j++) {
                ...
                stg_foreach_state (stg, gen, from) {
                    ...
                    find_marking_pairs_recur (stg, astg,
                        orig_stg_names, equiv_states->state_index, edge_trans,
                        partitions, from, p_from, p_to,
                        to_visit, from_set, to_set, cur_trans_sets);
            ...
            for (i = 0; i < array_n (froms); i++) {
                ...
                if (astg_debug_flag > 1) {
                    fprintf (sisout,
                        "creating marking pair from %s to %s\n from",
                ...
                for (s = 0; s < array_n (equiv_states->index_state); s++) {
                    ...
                    from = array_fetch (vertex_t *,
                        equiv_states->index_state, s);
                    assert (st_lookup (state_marking, (char *) from, (char **) &marking_arr));
                    for (k = 0; k < array_n (marking_arr); k++) {
                        marking = array_fetch (astg_marking *, marking_arr, k);
                        array_insert_last (astg_marking *,
                ...
                for (k = 0; k < array_n (from_markings); k++) {
                    ...
                    array_insert_last (astg_marking *,
                            marking_pair.from_markings, marking);
                }
                ...
                for (k = 0; k < array_n (to_markings); k++) {
                    ...
                    array_insert_last (astg_marking *,
                            marking_pair.to_markings, marking);
                }

                if (array_n (marking_pair.to_markings) > 0 &&
                    array_n (marking_pair.from_markings) > 0) {
                    array_insert_last (marking_pair_t, marking_pairs,
                    marking_pair);
                }
                else {
                    array_free (marking_pair.to_markings);
    ...
    for (i = 0; i < array_n (all_trans_sets); i++) {
        ...
        for (j = 0; j < array_n (trans_sets); j++) {
            trans_set =
                array_fetch (st_table *, trans_sets, j);
            st_free_table (trans_set);
        }
        array_free (trans_sets);
    }
    ...
    if (astg_debug_flag > 1) {
        ...
        for (i = 0; i < equiv_states->classes->count; i++) {
            fprintf (sisout, "%2d ", i);
            len = 3;
            st_foreach_item (partitions, stgen, (char **) &state,
                (char **) &cl) {
                if (cl == i) {
                    str = stg_get_state_name (state);
                    len += strlen (str) + 1;
 ...
 */
graph_t *
bwd_state_minimize (astg, equiv_states, use_mincov, greedy, go_down, mincov_option, use_mdd, sig_cost)
astg_graph *astg;
equiv_t *equiv_states;
int use_mincov, greedy, go_down, mincov_option, use_mdd;
st_table *sig_cost;
{
    ...
    g_sig_cost = sig_cost;
    partitions = partition_states (astg, global_orig_stg, equiv_states,
        use_mincov, greedy, go_down, mincov_option, use_mdd, global_edge_trans,
...
static astg_trans *
find_transition (from_trans, from_to_place, sig_p)
astg_trans *from_trans;
        ...
        astg_foreach_output_trans (place, tgen, sig_trans) {
            ...
            astg_foreach_output_place (sig_trans, pgen1, place1) {
                ...
                astg_foreach_output_trans (place1, tgen1, trans1) {
                    if (result != NIL(astg_trans) ||
                        astg_signal_type (astg_trans_sig (trans1))
                            != ASTG_DUMMY_SIG) {
                        continue;
                    }
...
static astg_place *
add_and_return_constraint (from_trans, to_trans)
astg_trans *from_trans, *to_trans;
{
    astg_generator tgen, pgen;
 ...
 */
static int
is_predecessor (from_trans, to_transitions)
astg_trans *from_trans;
 ...
 */
static void
add_state_vars (astg, initial_marking, added, initial, from_transitions, new_places, from_code, to_code, nbits, idx, state_signals)
astg_graph *astg;
...
astg_signal **state_signals;
{
    ...
    if (dum_sig == NIL(astg_signal)) {
        strcpy (buf, "dummy");
        while ((dum_sig = astg_find_signal (astg, buf,
            ASTG_DUMMY_SIG, ASTG_FALSE)) !=
            NIL(astg_signal)) {
            strcat (buf, "_");
        }
        dum_sig = astg_find_signal (astg, buf,
            ASTG_DUMMY_SIG, ASTG_TRUE);
    }
    dum_t1 = astg_find_trans (astg,
        astg_signal_name (dum_sig),
        ASTG_DUMMY_X, dum_i++, ASTG_TRUE);
    dum_t2 = astg_find_trans (astg,
        astg_signal_name (dum_sig),
        ASTG_DUMMY_X, dum_i++, ASTG_TRUE);
    st_foreach_item (new_places, stgen, (char **) &from_to_place,
        NIL(char *)) {
        ...
        st_foreach_item (from_transitions, stgen1, (char **) &from_trans,
            ...
            astg_delete_edge (astg_find_edge (from_trans,
                from_to_place, ASTG_FALSE));
            if (astg_find_edge (dum_t2, from_to_place, ASTG_FALSE)
                == NIL(astg_edge)) {
            ...
            if (new_p1 == NIL(astg_place)) {
                ...
                if (astg_find_edge (new_p1, dum_t2, ASTG_FALSE)
                    == NIL(astg_edge)) {
            ...
            if (astg_debug_flag > 0) {
                fprintf (sisout,
                    "             %s %s %s\n",
                    astg_trans_name (from_trans),
                    astg_place_name (new_p1),
                    astg_trans_name (dum_t1));
            }
        }
    ...
    for (m = 0; m < array_n (marking_pairs); m++) {
        ...
        if (astg_debug_flag > 1) {
            fprintf (sisout, "marking pair from %s to %s\n",
                marking_pair.from_state, marking_pair.to_state);
            fprintf (sisout, "from");
            for (i = 0; i < array_n (marking_pair.from_markings);
                i++) {
                marking = array_fetch (astg_marking *,
                    marking_pair.from_markings, i);
                fprintf (sisout, " s%s",
                    bwd_marking_string (marking->marked_places));
            }
            fprintf (sisout, "\nto");
            for (i = 0; i < array_n (marking_pair.to_markings);
                i++) {
                marking = array_fetch (astg_marking *,
                    marking_pair.to_markings, i);
                fprintf (sisout, " s%s",
                    bwd_marking_string (marking->marked_places));
            }
            fprintf (sisout, "\n");
        ...
        astg_foreach_place (astg, pgen, place) {
            ...
            for (j = 0; j < array_n (marking_pair.to_markings); j++) {
                to_marking = array_fetch (astg_marking *,
                    marking_pair.to_markings, j);
            ...
            for (j = 0; j < array_n (marking_pair.from_markings); j++) {
                from_marking = array_fetch (astg_marking *,
                    marking_pair.from_markings, j);
