...
static int ptr_pair_cmp(char *key1, char *key2) {
    int      result;
...
static int ptr_pair_hash(char *key, int modulus) {
    int      result;
    ptr_pair *pair;
...
static void constrain(char *str, astg_graph *astg, astg_trans *trans1, astg_trans *trans2) {
    ...
    if (astg != curr_astg) {
        if (printed != NIL(st_table)) {
            st_foreach_item(printed, stgen, (char **) &pairptr, NIL(
                    char *)) {
                FREE(pairptr);
            }
...
static int signal_cost(astg_signal *sig_p, astg_graph *astg) {
    int cost;
...
static int subset_cost(astg_scode subset, astg_graph *astg) {
    int            cost;
    astg_generator gen;
    astg_signal    *sig_p;
...
static void print_subset(astg_scode subset, astg_graph *astg) {
    astg_generator gen;
    astg_signal    *sig_p;
...
static void equiv_free(equiv_t *equiv_states) {
...
static equiv_t *equiv_alloc(int nstates, int nclasses) {
...
equiv_t *bwd_read_equiv_states(FILE *file, graph_t *stg) {
...
static int same_class(pset_family classes, int st1, int st2) {
    pset last, class;
 ...
 */
static int distinguishable(edge_t *edge, astg_scode subset, st_table *edge_trans, st_table *state_marking) {
 ...
 * equivalence class.
 */
static astg_scode find_partitioning_signals(astg_graph *astg, graph_t *stg, equiv_t *equiv_states, array_t *index_sig,
                                            pset visited, int mincov_option, int use_mdd,
                                            st_table *edge_trans,
                                            st_table *state_marking) {
...
static int is_forbidden(st_table *cur_trans_sets, st_table *edge_trans, edge_t *edge) {
    array_t    *trans_arr;
    int        i;
    astg_trans *trans;
...
static int
closure(graph_t *stg, array_t *allowed_partitions, equiv_t *equiv_states, array_t *trans_comb, st_table *edge_trans) {
    lsGen        gen;
    edge_t       *edge;
    vertex_t     *from, *to;
    ...
    do {
        ...
        for (j = 0; j < array_n(trans_comb); j++) {
            ...
            if (astg_debug_flag > 2 && st_count(cur_trans_sets)) {
                ...
                st_foreach_item(cur_trans_sets, stgen, (char **) &trans, NIL(
                        char *)) {
...
static int *choose_class(graph_t *stg, array_t *allowed_partitions, equiv_t *equiv_states, st_table *orig_stg_names) {
    int          *index_part;
    vertex_t     *state;
 ...
 */
static int check_this_partition_recur(vertex_t *state1, int cl, equiv_t *equiv_states, astg_scode subset, pset visited,
                                      array_t *allowed_partitions, int forward, int rm, st_table *edge_trans,
                                      st_table *state_marking) {
    int      st1, st2, result;
    lsGen    gen;
    edge_t   *edge;
 ...
 */
            if (!
                        check_this_partition_recur(state2, cl, equiv_states, subset, visited,
...
static int check_this_partition(vertex_t *state1, int cl, equiv_t *equiv_states, astg_scode subset, pset visited,
                                array_t *allowed_partitions, int rm, st_table *edge_trans, st_table *state_marking) {
    int result;
 ...
 */
static int *
check_partitions(graph_t *stg, equiv_t *equiv_states, astg_scode subset, pset visited, array_t *allowed_partitions,
                 array_t *trans_comb, st_table *edge_trans, st_table *state_marking, st_table *orig_stg_names) {
    vertex_t *state;
    pset     part, class;
    int      s, cl;
...
static astg_scode
partition_states_recur(astg_graph *astg, graph_t *stg, equiv_t *equiv_states, astg_scode base_subset, pset visited,
                       array_t *masks, array_t *allowed_partitions, array_t *index_sig, int first_signal,
                       int first_mask, array_t *trans_comb, st_table *edge_trans, st_table *state_marking,
...
static void
forced_sets_recur(equiv_t *equiv_states, array_t *allowed_partitions, vertex_t *state1, pset visited, pset tmp,
                  int copy, st_table *edge_trans, st_table *state_marking) {
    lsGen      gen;
    edge_t     *edge;
    vertex_t   *state2;
    int        s1, s2;
 ...
 */
static void
reduce_input_dependency(graph_t *stg, equiv_t *equiv_states, array_t *allowed_partitions, array_t *index_sig,
                        pset visited, st_table *edge_trans, st_table *state_marking) {
    lsGen       gen;
    edge_t      *edge;
    vertex_t    *state1, *state2;
...
static int is_conflict(astg_trans *trans1, astg_trans *trans2) {
    int            confl;
 ...
 */
static void find_marking_pairs_recur(graph_t *stg, astg_graph *astg, st_table *orig_stg_names, st_table *state_index,
                                     st_table *edge_trans, st_table *partitions, vertex_t *from, int p_from, int p_to,
                                     pset to_visit, pset from_set, pset to_set, st_table *cur_trans_sets) {
    lsGen    gen;
    edge_t   *edge;
    vertex_t *to, *from1;
    int      p, s_from, s_to;
...
static void free_marking_pair(marking_pair_t marking_pair) {
...
static void
check_multiple_exit(astg_graph *astg, graph_t *stg, vertex_t *from, st_table *partitions, st_table *merged_head,
                    st_table *orig_stg_names, st_table *edge_trans, array_t *index_sig) {
    lsGen      gen1, gen2;
 ...
 */
static void find_maximal_concurrent(astg_graph *astg, graph_t *stg, vertex_t *state, astg_marking *marking, int cur_fc,
                                    st_table *edge_trans, st_table *state_marking, st_table *trans_set,
                                    array_t *trans_sets) {
    int            fc, all_in;
    ...
    if (fc > cur_fc) {
        ...
        if (st_count(trans_set)) {
            if (astg_debug_flag > 1) {
                ...
                st_foreach_item(trans_set, stgen, (char **) &trans, NIL(
                        char *)) {
                    fprintf(sisout, " %s", astg_trans_name(trans));
                }
                fprintf(sisout, "\n");
...
static void create_combinations_recur(array_t *trans_comb, array_t *all_trans_sets, st_table *cur_trans_sets, int i) {
    int          j;
    array_t      *trans_sets;
    st_table     *trans_set;
    st_generator *stgen;
...
static void
find_to_visit(graph_t *stg, pset to_visit, st_table *partitions, st_table *orig_stg_names, st_table *state_index,
              int p_from, int p_to) {
    lsGen        gen;
    edge_t       *edge;
...
static int st_eq(st_table *st1, st_table *st2) {
    st_generator *stgen;
    char         *key;
    ...
    st_foreach_item(st1, stgen, &key, NIL(
            char *)) {
 ...
 * transition
 */
static int has_neighbor(astg_graph *astg, astg_marking *marking, array_t *markings, int forward) {
    int            i;
 ...
 */
static st_table *
partition_states(astg_graph *astg, graph_t *stg, equiv_t *equiv_states, int use_mincov, int greedy, int go_down,
                 int mincov_option, int use_mdd, st_table *edge_trans, st_table *state_marking,
                 st_table *orig_stg_names) {
 ...
 */
graph_t *
bwd_state_minimize(astg_graph *astg, equiv_t *equiv_states, int use_mincov, int greedy, int go_down, int mincov_option,
                   int use_mdd, st_table *sig_cost) {
    graph_t      *red_stg;
    st_table     *partitions;
    st_generator *stgen;
    array_t      *names;
...
static astg_trans *find_transition(astg_trans *from_trans, astg_place *from_to_place, astg_signal *sig_p) {
...
static astg_place *add_and_return_constraint(astg_trans *from_trans, astg_trans *to_trans) {
    astg_generator tgen, pgen;
 ...
 */
static int is_predecessor(astg_trans *from_trans, st_table *to_transitions) {
    int            result, found;
    st_generator   *stgen;
    ...
    result = 1;
    st_foreach_item(to_transitions, stgen, (char **) &to_trans, NIL(
            char *)) {
        if (!result) {
            continue;
        }
 ...
 */
static void add_state_vars(astg_graph *astg, astg_marking *initial_marking, st_table *added, st_table *initial,
                           st_table *from_transitions, st_table *new_places, char *from_code, char *to_code, int nbits,
                           int idx, astg_signal **state_signals) {
    int          i;
    ...
    dum_t2 = astg_find_trans(astg, astg_signal_name(dum_sig), ASTG_DUMMY_X,
                             dum_i++, ASTG_TRUE);
    st_foreach_item(new_places, stgen, (char **) &from_to_place, NIL(
            char *)) {
...
void bwd_add_state(graph_t *stg, astg_graph *astg, int restore_marking) {
    ...
    if (restore_marking) {
        st_foreach_item(initial, stgen, (char **) &place, NIL(
                char *)) {
