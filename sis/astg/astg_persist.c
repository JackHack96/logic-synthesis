...
static int find_per(astg_graph *stg, void *data) {
...
static int find_opp(astg_graph *stg, void *data) {
    ...
    astg_foreach_path_vertex(stg, gen, t) {
        ...
        sig_p->n_found = 0;
        if (sig_p->mark) {
            assert(astg_trx(t)->trans_type !=
                   astg_trx(sig_p->last_trans)->trans_type);
...
int astg_set_opp(astg_graph *stg, astg_bool subset) {
    int            result = 0;
    astg_signal    *sig_p;
    astg_trans     *t;
...
static int make_mgc_persistent(astg_graph *stg, astg_bool modify) {
    astg_vertex    *dest;
    astg_graph     *g      = stg;
    lsList         vertex_stack;
    int            n_added = 0;
    ...
    if (astg_set_opp(stg, ASTG_SUBSET) != 0)
        return -1;
    ...
    while (lsDelBegin(vertex_stack, (lsGeneric *) &t) == LS_OK) {
        ...
        astg_foreach_output_place(t, pgen, p) {
            ...
            if (astg_simple_cycles(g, ASTG_ALL_CYCLES, find_per, (void *) &per_rec,
                                   ASTG_SUBSET) == 0) {
                dbg(1, msg("  %s -> %s non-persistent transition\n", astg_v_name(t),
                           astg_v_name(per_rec.per1)));
...
int make_persistent(astg_graph *stg, astg_bool modify) {
