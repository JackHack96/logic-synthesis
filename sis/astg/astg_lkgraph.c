...
static void print_lock_graph(stg) astg_graph *stg;
{
...
static void print_path(stg) astg_graph *stg;
{
  astg_vertex *v;
  astg_generator gen;
...
static void astg_lg_comp(sig_p, component) astg_signal *sig_p;
array_t *component;
{
  astg_signal *s;
  int n;
...
static void astg_sp_insert(q, sig_p, new_weight, src) lsList q;
astg_signal *sig_p;
int new_weight;
astg_signal *src;
{
...
static void astg_sp_adjust(src, dest, src_pqueue) astg_signal *src, *dest;
lsList src_pqueue;
{
...
static astg_bool sandwich(t1, t, t2) int t1, t, t2;
{
  int t1n, t2n;
...
static astg_bool interleaved(s1pos, s1neg, s2pos, s2neg) int s1pos, s1neg;
int s2pos, s2neg;
{ return sandwich(s1pos, s2pos, s1neg) ^ sandwich(s1pos, s2neg, s1neg); }

static astg_bool lg_all_on_path(sig_p, weightp, weightn) astg_signal *sig_p;
int *weightp, *weightn;
{
  astg_generator gen;
  astg_trans *t;
...
static int find_interlocks(stg, data) astg_graph *stg;
void *data;
{
...
static int save_lock_classes(component, comp_n, data) array_t *component;
int comp_n; /* ARGSUSED */
void *data;
{
...
static void free_lock_classes(lc_info) lc_rec *lc_info;
{
  int n;
...
static int make_lock_graph(stg, lc_info) astg_graph *stg;
lc_rec *lc_info;
{
  astg_signal *sig_p;
  astg_generator sgen;
...
static astg_bool can_lock_trans(t1, t2) astg_trans *t1, *t2;
{
...
static astg_do_lock(t1, t2) astg_trans *t1, *t2;
{
  ...
  astg_add_constraint(astg_trx(t1)->opp_trans,
                      ...
                      ASTG_TRUE);
  astg_add_constraint(astg_trx(t2)->opp_trans, astg_noninput_trigger(t1),
                      ASTG_TRUE);
}

static astg_bool astg_lock_sig(s1, s2, modify) astg_signal *s1, *s2;
astg_bool modify;
{
...
static int astg_lock_sig_cost(stg, sig1, sig2) astg_graph *stg;
astg_signal *sig1;
astg_signal *sig2;
{
  int cost = 0;
...
static astg_bool select_from_lc(stg, lc1, sig1_p, lc2, sig2_p) astg_graph *stg;
lock_class *lc1, *lc2;
...
static astg_bool select_two_signals(lc_info, sig1_p, sig2_p) lc_rec *lc_info;
astg_signal **sig1_p;
astg_signal **sig2_p;
{
...
static void lock_new_sig(stg) astg_graph *stg;
{
  astg_signal *new_sig;
...
astg_bool astg_state_coding(stg, do_lock) astg_graph *stg;
astg_bool do_lock;
{
