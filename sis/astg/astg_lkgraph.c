...
static void print_lock_graph(astg_graph *stg) {
...
static void print_path(astg_graph *stg) {
    astg_vertex    *v;
    astg_generator gen;

    astg_foreach_path_vertex(stg, gen, v) {
        if (v->vtype == ASTG_PLACE)
            continue;
...
static void astg_lg_comp(astg_signal *sig_p, array_t *component) {
    astg_signal *s;
    int         n;
...
int astg_lock_graph_components(astg_graph *stg, int (*f)(), void *fdata) {
    astg_generator gen;
    astg_signal    *sig_p;
    array_t        *component;
    int            n_comp = 0;

    astg_foreach_signal(stg, gen, sig_p) { sig_p->unprocessed = ASTG_TRUE; }
...
static void astg_sp_insert(lsList q, astg_signal *sig_p, int new_weight, astg_signal *src) {
    astg_signal *old_sig;
    lsGen       gen;

    if (sig_p->lg_queue != NULL)
        lsRemoveItem(sig_p->lg_queue, NULL);
    sig_p->lg_weight = new_weight;
    sig_p->lg_from   = src;

    gen = lsStart(q);
    while (lsNext(gen, (lsGeneric *) &old_sig, LS_NH) == LS_OK) {
        if (new_weight <= old_sig->lg_weight)
            break;
    }
...
static void astg_sp_adjust(astg_signal *src, astg_signal *dest, lsList src_pqueue) {
...
void astg_lock_graph_shortest_path(astg_graph *stg, astg_signal *source) {
    astg_generator gen;
    lsList         src_pqueue;
...
static astg_bool sandwich(int t1, int t, int t2) {
    int t1n, t2n;
...
static astg_bool interleaved(int s1pos, int s1neg, int s2pos, int s2neg) {
    return sandwich(s1pos, s2pos, s1neg) ^ sandwich(s1pos, s2neg, s1neg);
}

static astg_bool lg_all_on_path(astg_signal *sig_p, int *weightp, int *weightn) {
    astg_generator gen;
    astg_trans     *t;
...
static int find_interlocks(astg_graph *stg, void *data) {
    ...
    astg_foreach_path_vertex(stg, sgen1, v) { v->weight1.i = n++; }

    astg_foreach_signal(lminfo->stg, sgen1, sig1) {

        if (!lg_all_on_path(sig1, &weight1p, &weight1n))
            continue;

        astg_foreach_signal(lminfo->stg, sgen2, sig2) {

            if (!lg_all_on_path(sig2, &weight2p, &weight2n))
                continue;
...
static int save_lock_classes(array_t *component, int comp_n, void *data) {
...
static void free_lock_classes(lc_rec *lc_info) {
    int n;
...
static int make_lock_graph(astg_graph *stg, lc_rec *lc_info) {
    astg_signal    *sig_p;
    astg_generator sgen;
    astg_signal    *sig1, *sig2;
    array_t        *mapping;
    ...
    lc_info->stg = stg;
    astg_simple_cycles(stg, ASTG_ALL_CYCLES, find_interlocks, (void *) lc_info,
                       ASTG_ALL);

    for (i = 0; i < stg->n_sig; i++) {
        for (j = 0; j < i; j++) {
            assert((sm_find(lc_info->lock_matrix, i, j) == 0) ==
                   (sm_find(lc_info->lock_matrix, j, i) == 0));
    ...
    dbg(1,
        msg("lock graph has %d componen%s\n", n_comp, n_comp == 1 ? "t" : "ts"));
    return n_comp;
}

/* ----------------------- Rectify Unique State Coding ------------------- */

static astg_bool can_lock_trans(astg_trans *t1, astg_trans *t2) {
    ...
    if (astg_is_rel(astg_noninput_trigger(t2), t1) ||
        ...
        astg_is_rel(astg_noninput_trigger(astg_trx(t2)->opp_trans),
                    astg_trx(t1)->opp_trans) ||
        astg_is_rel(astg_noninput_trigger(t1), astg_trx(t2)->opp_trans)) {
        can_lock = ASTG_FALSE;
    }
    dbg(3, msg("can_lock %s %s = %d\n", astg_trans_name(t1), astg_trans_name(t2),
               can_lock));
    return can_lock;
}

static void astg_do_lock(astg_trans *t1, astg_trans *t2) {
    /* Form lock t1 -> t2 -> t1->opp -> t2->opp */
    dbg(1, msg("locking %s to %s\n", t1->name, t2->name));
    astg_add_constraint(t1, astg_noninput_trigger(t2), ASTG_TRUE);
    astg_add_constraint(t2, astg_noninput_trigger(astg_trx(t1)->opp_trans),
                        ASTG_TRUE);
    astg_add_constraint(astg_trx(t1)->opp_trans,
                        astg_noninput_trigger(astg_trx(t2)->opp_trans),
                        ASTG_TRUE);
    astg_add_constraint(astg_trx(t2)->opp_trans,
                        astg_noninput_trigger(t1),
                        ASTG_TRUE);
}

static astg_bool astg_lock_sig(astg_signal *s1, astg_signal *s2, astg_bool modify) {
    ...
    astg_foreach_pos_trans(s1, tgen1, t_s1_plus) {
        astg_foreach_pos_trans(s2, tgen2, t_s2_plus) {
            if (can_lock_trans(t_s1_plus, t_s2_plus)) {
                if (modify)
                    astg_do_lock(t_s1_plus, t_s2_plus);
                return ASTG_TRUE;
            } else if (can_lock_trans(t_s1_plus, astg_trx(t_s2_plus)->opp_trans)) {
                if (modify)
                    astg_do_lock(t_s1_plus, astg_trx(t_s2_plus)->opp_trans);
                return ASTG_TRUE;
            }
        }
    }
...
static int astg_lock_sig_cost(astg_graph *stg, astg_signal *sig1, astg_signal *sig2) {
    int            cost = 0;
    astg_signal    *s1, *s2;
...
static astg_bool
select_from_lc(astg_graph *stg, lock_class *lc1, astg_signal **sig1_p, lock_class *lc2, astg_signal **sig2_p) {
...
static astg_bool select_two_signals(lc_rec *lc_info, astg_signal **sig1_p, astg_signal **sig2_p) {
...
static void lock_new_sig(astg_graph *stg) {
    astg_signal *new_sig;
...
astg_bool astg_state_coding(astg_graph *stg, astg_bool do_lock) {
    ...
    if (astg_set_opp(stg, ASTG_ALL) != 0)
        return 1;
    ...
    astg_foreach_signal(stg, sgen, sig_p) {
        ...
        if (sig_p->n_trans != 2) {
            printf("%s has %d transitions of signal %s.\n", astg_name(stg),
                   sig_p->n_trans, astg_signal_name(sig_p));
