...
static void print_enabled(astg_graph *stg, astg_scode state, astg_scode enabled) {
    astg_signal    *s;
    astg_generator agen;
 ...
 */
static astg_scode astg_marking_enabled_dummy(astg_graph *astg, astg_marking *marking) {
    astg_generator tgen;
    astg_trans     *trans;
    astg_marking   *dup;
    astg_scode     result;
 ...
 */
static void
find_maximal_subset(int cur_fc, int cur_sig, astg_graph *astg, astg_scode sig_bit, astg_scode state, astg_scode value,
                    astg_scode enabled, set_family_t *F, set_family_t *R, set_family_t *dup, array_t *dup_arr,
    ...
    if (fc > cur_fc) {
        ...
        if (fc_enabled == (astg_scode) 0) {
            ...
            find_maximal_subset(cur_fc + 1, cur_sig, astg, sig_bit, state, value,
                                enabled, F, R, dup, dup_arr, dup_marking, cb);
            /*
             * }
             */
        } else {
            ...
            astg_foreach_output_trans(place, tgen, trans) {
                ...
                if (fc_enabled & bit) {
                    ...
                    find_maximal_subset(cur_fc + 1, cur_sig, astg, sig_bit, state, value,
                                        new_enabled, F, R, dup, dup_arr, dup_marking, cb);
                }
            }
        }
    } else {
         ...
         */
        astg_foreach_marking(astg_find_state(astg, new_state, 0), mgen, marking) {
            ...
            if (sig_bit & new_enabled) {
                 ...
                 */
                find_maximal_subset(cur_fc, i + 1, astg, sig_bit, state, value, enabled,
                                    F, R, dup, dup_arr, dup_marking, cb);

                /* now disable the current signal and recur again */
                new_enabled = enabled & ~(bit);
                if (new_enabled) {
                    find_maximal_subset(cur_fc, i + 1, astg, sig_bit, state, value,
                                        new_enabled, F, R, dup, dup_arr, dup_marking, cb);
                }
            } else {
...
static void astg_to_f_recur(astg_graph *astg, astg_scode sig_bit, astg_scode state, st_table *reached, set_family_t *F,
                            set_family_t *R, set_family_t *dup, array_t *dup_arr, pset cb) {
    ...
    astg_foreach_marking(astg_find_state(astg, state, 0), mgen, marking) {
        ...
        find_maximal_subset(0, i, astg, sig_bit, state, value, enabled, F, R, dup,
                            dup_arr, marking, cb);
        ...
        astg_foreach_signal(astg, sgen, sig_p) {
            ...
            if (to_fire & enabled) {
                ...
                astg_to_f_recur(astg, sig_bit, new_state, reached, F, R, dup, dup_arr,
                                cb);
            }
        }
    }
 ...
 */
static set_family_t *add_cubes(set_family_t *S, set_family_t *M, set_family_t *R) {
    set_family_t *SM, *D;
    pset         p, p1, last;
    sm_matrix    *table;
    sm_row       *cover;
    ...
    foreach_set(SM, last, p) { RESET(p, ACTIVE); }
    foreach_set(M, last, p) {
        p1 = GETSET(SM, SIZE(p));
    ...
    added_cubes += M->count;
    if (added_cubes > 0) {
        fprintf(sisout, "warning: added %d cubes to make S and R disjoint\n",
                added_cubes);
    }
    return M;
}
...
void bwd_astg_to_f(network_t *network, astg_graph *astg, astg_signal *sig_p, array_t *fanin, astg_scode state,
                   st_table *hazard_list, int index, int use_old, int sep_s_r, int disjoint, int force) {
    ...
    astg_foreach_trans(astg, tgen, trans) { trans->active = 0; }
        ...
        foreachi_set(dup, i, p) {
            ...
            if (dup_on != NIL(st_table) && dup_off != NIL(st_table)) {
                ...
                astg_foreach_signal(astg, sgen, tsig) {
                    switch (GETINPUT(p, j)) {
                        case ONE:fprintf(siserr, " %s=1", tsig->name);
                            break;
                        case ZERO:fprintf(siserr, " %s=0", tsig->name);
                            break;
                        case TWO:fprintf(siserr, " %s=+/-", tsig->name);
                            break;
                        default:break;
                    }
                    j++;
                ...
                st_foreach_item(dup_on, rgen, &dummy, (char **) &dup_marking) {
                ...
                st_foreach_item(dup_off, rgen, &dummy, (char **) &dup_marking) {
                    dup_enabled = (astg_scode) dummy;
                    fprintf(siserr, "  ");
                    astg_foreach_place(astg, pgen, place) {
                        if (astg_get_marked(dup_marking, place)) {
    ...
    if (sep_s_r) {
        ...
        foreach_set(F, last, p) {
            switch (GETINPUT(p, index)) {
                case ONE:set_or(c, p, mask);
                    sf_addset(M, c);
                    break;
                case TWO:sf_addset(S, p);
                    break;
                default: fail("illegal cube value in bwd_astg_to_f\n");
                    break;
            }
        }
        set_free(c);
        ...
        if (M->count) {
            ...
            sf_free(M);
            node =
                    node_create(sf_save(F), array_data(node_t *, fanin), array_n(fanin));
        }
    } else {
        node = node_create(sf_save(F), array_data(node_t *, fanin), array_n(fanin));
    }
