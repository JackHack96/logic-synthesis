...
static void print_enabled(stg, state, enabled) astg_graph *stg;
astg_scode state;
astg_scode enabled;
{
  astg_signal *s;
 ...
 */
static astg_scode astg_marking_enabled_dummy(astg, marking) astg_graph *astg;
 ...
 */
static void find_maximal_subset(cur_fc, cur_sig, astg, sig_bit, state, value,
                                enabled, F, R, dup, dup_arr, dup_marking,
                                cb) int cur_fc,
    cur_sig;
astg_graph *astg;
...
static void astg_to_f_recur(astg, sig_bit, state, reached, F, R, dup, dup_arr,
                            cb) astg_graph *astg;
 ...
 */
static set_family_t *add_cubes(S, M, R) set_family_t *S, *M, *R;
{
  set_family_t *SM, *D;
  pset p, p1, last;
  sm_matrix *table;
...
void bwd_astg_to_f(network, astg, sig_p, fanin, state, hazard_list, index,
                   use_old, sep_s_r, disjoint, force) network_t *network;
astg_graph *astg;
astg_signal *sig_p;
array_t *fanin;
astg_scode state;
