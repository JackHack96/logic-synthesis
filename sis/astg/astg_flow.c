...
array_t *astg_check_new_state(astg_flow_t *finfo, astg_scode new_state) {
    astg_place     *p;
    astg_trans     *sv;
    astg_state     *state_p;
    ...
    state_p = astg_find_state(
            finfo->stg, astg_adjusted_code(finfo->stg, new_state), ASTG_TRUE);
        ...
        if (n_disabled == 0) {
            if (finfo->force_safe) {
                ...
                astg_foreach_output_place(sv, pgen, p) {
                    if (astg_get_marked(finfo->marking, p))
                        n_disabled++;
                }
            }
            if (n_disabled == 0) {
    ...
    if (astg_debug_flag >= 2)
        astg_print_marking(0, finfo->stg, one_marking);
    return enabled;
}

static int flow_state(astg_flow_t *flow_info, astg_scode new_state) {
...
static astg_scode astg_flow_outmask(astg_graph *stg) {
...
static astg_bool astg_flow_check_csc(astg_graph *stg, astg_bool verbose) {
    ...
    if (astg_get_flow_status(stg) != ASTG_OK)
        return ASTG_FALSE;
    ...
    astg_foreach_state(stg, sgen, state_p) {
        ...
        astg_foreach_marking(state_p, mgen, marking_p) {
            if (astg_is_dummy_marking(marking_p))
                continue;
...
static void astg_flow_check_redundant(astg_graph *stg) {
    astg_generator gen;
    astg_place     *p;
    astg_trans     *t;
...
astg_retval astg_token_flow(astg_graph *stg, astg_bool verbose) {
