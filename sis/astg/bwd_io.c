...
static int is_garbage_node(node) node_t *node;
{
  node_t *fanout, *pi;
  lsGen gen;
...
static char *make_legal_name(name) char *name;
{
  static char buf[512];
  int i, len;
...
static node_t *get_named_node(node) node_t *node;
{
...
static char *get_gate_name(node) node_t *node;
{
  char *name;
...
void bwd_write_burch(file, network, min_delay_factor, write_spec) FILE *file;
network_t *network;
