...
extern char *astg_dup(old_astg) char *old_astg;
{
  /*	Duplicate the network astg slot value. */
  return (char *)astg_duplicate((astg_graph *)old_astg, NULL);
}

extern void astg_free(old_astg) char *old_astg;
{
  /*	Free all structures for the given network astg slot. */
  astg_delete((astg_graph *)old_astg);
}

extern astg_graph *astg_current(
    network) network_t *network; /*i network to find astg info for	*/
{
...
extern void astg_set_current(network_p, stg, reset) network_t **network_p;
astg_graph *stg;
astg_bool reset;
{
...
extern void astg_usage(usage, cmd) char **usage;
char *cmd;
{
...
static int com_astg_flow(network_p, argc, argv) network_t **network_p;
int argc;
char **argv;
{
  int c;
  ...
  while ((c = util_getopt(argc, argv, "xo:l:q")) != EOF) {
    switch (c) {
    case 'x':
      full_checks = ASTG_FALSE;
      break;
    case 'l':
      latch_type = util_optarg;
      break;
    case 'o':
      outfile = util_strsav(util_optarg);
      del_out = ASTG_FALSE;
      break;
    case 'q':
      verbose = ASTG_FALSE;
...
static int com_astg_read(network_p, argc, argv) network_t **network_p;
int argc;
char **argv;
{
  int c;
...
static int com_astg_write(network_p, argc, argv) network_t **network_p;
int argc;
char **argv;
{
  astg_retval result = ASTG_OK;
...
static int astg_count_cycles(stg, data) astg_graph *stg;
void *data;
{
...
static int com_astg_cycle(network_p, argc, argv) network_t **network_p;
int argc;
char **argv;
{
  int c;
  ...
  if ((stg = astg_current(*network_p)) == NULL)
    result = ASTG_BAD_OPTION;
...
static char *lockgraph_usage[] = {
    ...
    "usage: %s [-l]", "    print lock graph for an STG",
    "    -l	try to form one lock class first",

    NULL};

static int print_stg_comp(varray, n, extra) array_t *varray;
int n;
void *extra; /*  ARGSUSED */
{
  int i;
...
static int com_astg_lockgraph(network_p, argc, argv) network_t **network_p;
int argc;
char **argv;
{
  int c;
...
static int com_astg_persist(network_p, argc, argv) network_t **network_p;
int argc;
char **argv;
{
  int c;
...
static char *red_usage[] = {
    ...
    "usage: %s [-p]", "    -p  print redundant edges instead of deleting them.",

    NULL};

static int com_astg_irred(network_p, argc, argv) network_t **network_p;
int argc;
char **argv;
{
  int c;
  ...
  if (result == ASTG_BAD_OPTION || curr_stg == NULL) {
    ...
    if (modify)
      astg_set_current(network_p, curr_stg, ASTG_TRUE);
  }
  return (result != ASTG_OK);
}
...
static char *current_usage[] = {
    ...
    "usage: %s [-d #]", "    -d  set debug output (0=no debug output)",
    "    display information about the current stg",

    NULL};

static int com_astg_current(network_p, argc, argv) network_t **network_p;
int argc;
char **argv;
{
  int c;
  ...
  if (argc > util_optind || result == ASTG_BAD_OPTION) {
    ...
    fputs("\n", stdout);
    printf("\tPure: %c  Place-simple: %c\n", astg_is_pure(curr_stg) ? 'Y' : 'N',
           astg_is_place_simple(curr_stg) ? 'Y' : 'N');
    n_comp1 =
        astg_connected_comp(curr_stg, ASTG_NO_CALLBACK, ASTG_NO_DATA, ASTG_ALL);
    n_comp2 =
        astg_strong_comp(curr_stg, ASTG_NO_CALLBACK, ASTG_NO_DATA, ASTG_ALL);
...
static int com_astg_marking(network_p, argc, argv) network_t **network_p;
int argc;
char **argv;
{
  astg_retval result = ASTG_OK;
  ...
  if (stg == NULL || result != ASTG_OK) {
    ...
    if (by_state_code) {
      ...
      while (util_optind < argc) {
        ...
        if (util_optind < argc &&
            sscanf(argv[util_optind++], "%d", &value) == 1) {
  ...
  if (result == ASTG_BAD_OPTION)
    astg_usage(marking_usage, argv[0]);
  return (result != ASTG_OK);
}

static int astg_print_component_pieces(vertices, n, fdata) array_t *vertices;
int n;
void *fdata; /*  ARGSUSED */
{
...
static void select_comp(stg, comp, comp_name, comp_n) astg_graph *stg;
array_t *comp;
char *comp_name;
int comp_n;
{
  ...
  if (n_scc != 1) {
    printf("warning: this %s component has %d strongly connected components.\n",
           comp_name, n_scc);
    astg_strong_comp(stg, astg_print_component_pieces, ASTG_NO_DATA,
                     ASTG_SUBSET);
  }
}
...
static int com_astg_mgc(network_p, argc, argv) network_t **network_p;
int argc;
char **argv;
{
  astg_retval result = ASTG_OK;
...
static int com_astg_smc(network_p, argc, argv) network_t **network_p;
int argc;
char **argv;
{
  astg_retval result = ASTG_OK;
...
static int com_astg_contract(network_p, argc, argv) network_t **network_p;
int argc;
char **argv;
{
  astg_retval result = ASTG_OK;
  ...
  if (result == ASTG_BAD_OPTION || util_optind != argc - 1 ||
      curr_stg == NULL) {
    result = ASTG_BAD_OPTION;
