...
dic_family_t *dic_family_alloc(int num, int nelem) {
    dic_family_t *A;
    int          setsize;
...
void dic_family_free(dic_family_t *D) {
    (void) dic_free(D->dic);
    (void) FREE(D);
}

dic_family_t *dic_family_copy(dic_family_t *D) {
    dic_family_t *A;
    pset         p, s, last;
...
void dic_family_print(dic_family_t *D) {
    pset p, last;
...
void dic_family_add(dic_family_t *A, pset s) {
    pset p;
...
void dic_family_add_contain(dic_family_t *A, pset s) {
    pset p;
    int  i;
...
void dic_family_add_irred(dic_family_t *A, pset s) {
    pset p;
    int  i;
...
void dic_free(pset dic) {
    FREE(dic);
}

void dic_print(pset dic, int nelem) {
    fprintf(sisout, "%s;", pbv1(lhs_dic(dic), nelem));
    fprintf(sisout, "%s\n", pbv1(rhs_dic(dic), nelem));
}

bool is_prime(pset dic, int nelem) {
    pset d;
    bool flag;
...
bool dicp_equal(pset p1, pset p2) {
    return (setp_equal(lhs_dic(p1), lhs_dic(p2)) &&
            setp_equal(rhs_dic(p1), rhs_dic(p2)));
}

dic_family_t *dic_cross_product(dic_family_t *dlist1, dic_family_t *dlist2) {
    dic_family_t *new;
