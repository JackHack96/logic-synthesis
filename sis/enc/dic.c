...
dic_family_t *dic_family_alloc(num, nelem) int nelem;
{
  dic_family_t *A;
  int setsize;
...
void dic_family_free(D) dic_family_t *D;
{
  (void)dic_free(D->dic);
  (void)FREE(D);
}

dic_family_t *dic_family_copy(D) dic_family_t *D;
{
  dic_family_t *A;
  pset p, s, last;
...
void dic_family_print(D) dic_family_t *D;
{
  pset p, last;
...
void dic_family_add(A, s) dic_family_t *A;
pset s;
{
  pset p;
...
void dic_family_add_contain(A, s) dic_family_t *A;
pset s;
{
  pset p;
  int i;
...
void dic_family_add_irred(A, s) dic_family_t *A;
pset s;
{
  pset p;
  int i;
...
void dic_free(dic) pset dic;
{ FREE(dic); }

void dic_print(dic, nelem) pset dic;
int nelem;
{
...
bool is_prime(dic, nelem) pset dic;
int nelem;
{
  pset d;
  bool flag;
...
bool dicp_implies(p1, p2) pset p1;
pset p2;
{
...
bool dicp_covers(p1, p2) pset p1;
pset p2;
{
  return ((setp_implies(lhs_dic(p1), lhs_dic(p2)) &&
           setp_implies(rhs_dic(p1), rhs_dic(p2))) ||
...
dic_family_t *dic_cross_product(dlist1, dlist2) dic_family_t *dlist1;
dic_family_t *dlist2;
{
  dic_family_t *new;
