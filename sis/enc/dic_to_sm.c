...
sm_matrix *dic_to_sm(dic_family_t *prime_list, dic_family_t *seed_list) {
    sm_matrix *M;
    sm_col    *col;
    ...
    for (p = prime_list->dic, c = 0; p < lastp; p += prime_list->dic_size, c++) {
        insert = FALSE;
        for (s = seed_list->dic, r = 0; s < lasts; s += seed_list->dic_size, r++) {
            if (dicp_covers(s, p)) {
...
void print_min_cover(sm_matrix *M, sm_row *cover, dic_family_t *prime_list) {
    sm_col     *col;
    sm_element *p;
    pset       d;
    int        n;
