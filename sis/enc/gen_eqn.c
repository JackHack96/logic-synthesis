 ...
 * obtain a SOP */
dic_family_t *gen_eqn(dic_family_t *dic_list, int climit) {
    pset_family eqn_cov;
    array_t     *eqn_arr;
    cnf_t       *cnf;
    pset        p1, p2;
...
static pset_family cnf_expand(array_t *cnf_arr, int *count, int vars, int limit) {
...
static void cnf_array_free(array_t *cnf_arr) {
    cnf_t *cnf;
    int   n, i;
...
static int cnf_split(array_t *cnf_arr, int *count, int vars, array_t **lhs_arr, pset *rhs_prod) {
    cnf_t *cnf, *cnf_l;
    int   n, i, max, max_i;
...
static pset_family cnf_merge(int vars, int split_var, pset rhs_prod, pset_family cov_l) {
    pset_family sop;
    pset        last, p, tmp;
    bool        rem_contain;
...
static bool special_cases(int n_terms, int *count, int vars, pset_family *sop) {
    pset tmp;
    int  i, j;
...
static dic_family_t *cnf_to_prime(dic_family_t *dic_list, pcover cov, int n) {
    dic_family_t *new;
