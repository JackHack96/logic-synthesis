...
int filter_cons(file1, file2) char *file1;
char *file2;
{
  FILE *fp, *fpout;
  char *line, *symbols;
  ...
  while (fgets(line, MAXLINE, fp) != NIL(char)) {
    ...
    if (sscanf(line, "input: %s\n", symbols) == 1) {
      if (iter == 1) {
        ...
        (void)fprintf(siserr, "Varying mv-constraint length : aborting\n");
        exit(-1);
      }
...
int read_cons(file, dic_list) char *file;
...
dic_family_t *gen_uniq(list) dic_family_t *list;
{
  dic_family_t *new;
...
dic_family_t *reduce_seeds(list) dic_family_t *list;
{
