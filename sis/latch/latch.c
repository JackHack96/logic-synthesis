...
void latch_set_control(latch, control) latch_t *latch;
node_t *control;
{
  network_t *network;
...
latch_t *latch_from_node(n) node_t *n;
{
  network_t *network;
  latch_t *latch;
...
int latch_equal(l1, l2) latch_t *l1, *l2;
{
  if (latch_get_initial_value(l1) != latch_get_initial_value(l2))
    return 0;
  if (latch_get_type(l1) != latch_get_type(l2))
    return 0;
  if (latch_get_control(l1) != latch_get_control(l2))
    return 0;
  if (latch_get_gate(l1) != latch_get_gate(l2))
    return 0;
  return 1;
}
#endif /* SIS */
