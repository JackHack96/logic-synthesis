...
static int com_print_latch(network_t **network, int argc, char **argv) {
    latch_t       *l;
    ...
    if ((argc == 1) || ((argc == 2) && (summary))) {
        foreach_latch(*network, gen, l) {
            ...
            if (latch_type == ACTIVE_HIGH)
                buf    = "ah";
            if (latch_type == ACTIVE_LOW)
                buf    = "al";
            if (latch_type == RISING_EDGE)
                buf    = "re";
            if (latch_type == FALLING_EDGE)
                buf    = "fe";
            if (latch_type == COMBINATIONAL)
                buf    = "co";
            if (latch_type == ASYNCH)
                buf    = "as";
            if (latch_type == UNKNOWN)
                buf    = "un";
            ...
            (void) fprintf(sisout, " init val: %d cur val: %d",
                           latch_get_initial_value(l), latch_get_current_value(l));
            if (!summary) {
    ...
    for (i   = 0; i < array_n(node_vec); i++) {
        ...
        if ((l = latch_from_node(p)) != NIL(latch_t)) {
            ...
            (void) fprintf(misout, "\tNode %s is not a latch input or output\n",
                           node_name(p));
        }
    }
    array_free(node_vec);
...
init_latch() { com_add_command("print_latch", com_print_latch, 0); }
