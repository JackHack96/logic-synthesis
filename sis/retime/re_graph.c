...
int
retime_graph_interface(re_graph *graph, double area_r, double delay_r, double retime_tol, double d_clk, double *new_c) {
    int flag, can_init;

    flag = retime_graph(NIL(network_t), graph, area_r, delay_r, retime_tol, d_clk,
                        new_c, 1, 0, 0, &can_init);
    return flag;
}
 ...
 */
int retime_graph(network_t *network, re_graph *graph, double area_r, double delay_r, double retime_tol, double d_clk,
                 double *new_c, int lp_flag, int min_reg, int should_init, int *can_initialize) {
    int      i, j, status, n;
    ...
    if (current_c > (attempting_c + EPS)) { /* Floating point comaparison */
        (void) fprintf(sisout,
                       "SERIOUS INTERNAL ERROR: The cycle after retiming %4.1f "
                       "exceeds initial %4.1f\n",
