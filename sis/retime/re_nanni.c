...
static void retime_allocate_delay_data(int n) {
    static_valid_table = ALLOC(bool, n);
    static_delay_table = ALLOC(double, n);
}

static void retime_free_delay_data() {
...
int retime_nanni_routine(re_graph *graph, double d_clk, int *retiming) {
    int     i, j, n;
    re_node *re_no;
    array_t *M;
...
static int retime_set_outputs(re_graph *graph, array_t *M, int *retiming) {
    int      i;
    array_t  *S;
    re_node  *node;
    st_table *table;
 ...
 */
static st_table *retime_get_po_from_array(array_t *M) {
    int      i;
    re_node  *node;
    st_table *table;
 ...
 */
static array_t *retime_get_slow_nodes(re_graph *graph, double clk) {
...
static array_t *retime_get_input_reachable(re_graph *graph) {
    array_t  *S;
    re_edge  *edge;
    re_node  *node;
    st_table *table;
...
static int re_translate_retiming_vector(re_graph *graph, int *r) {
    re_node *re_no;
