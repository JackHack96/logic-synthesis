...
void re_computeWD(graph, N, table,
                  pWD) re_graph *graph; /* Graph under consideration */
...
wd_t ***pWD;     /* Returned array of W and D */
{
  ...
  re_foreach_edge(graph, index, edge) {
    ...
    if (edge->source->type == RE_PRIMARY_INPUT &&
      ...
      WD[i][j].d = MIN(WD[i][j].d, offset = edge->source->scaled_user_time);
    }
...
static int greater(WD1, WD2, WD3) wd_t WD1, WD2, WD3;
{
  if (WD1.w > (WD2.w + WD3.w))
    return 1;

  if (WD1.w == (WD2.w + WD3.w))
    if (WD1.d > (WD2.d + WD3.d))
      return 1;

  return 0;
}

static void sum(pWD1, WD2, WD3) wd_t *pWD1, WD2, WD3;
{
  pWD1->w = WD2.w + WD3.w;
  pWD1->d = WD2.d + WD3.d;
}
