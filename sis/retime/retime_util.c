...
void re_graph_free(graph) re_graph *graph;
{
  int i;
  re_node *node;
  re_edge *edge;
  ...
  if (graph->control_name != NIL(char))
    FREE(graph->control_name);

  FREE(graph);
}

re_graph *re_graph_dup(graph) re_graph *graph;
{
  int i, j;
  re_graph *new_graph;
  ...
  re_foreach_node(graph, i, node) {
    ...
    if (new_node->type == RE_PRIMARY_INPUT)
      (void)array_insert_last(re_node *, new_graph->primary_inputs, new_node);
    else if (new_node->type == RE_PRIMARY_OUTPUT)
      (void)array_insert_last(re_node *, new_graph->primary_outputs, new_node);
  ...
  re_foreach_edge(graph, i, edge) {
    ...
    assert(
        st_lookup(node_ref_table, (char *)edge->sink->id, (char **)&new_node));
    new_edge->sink = new_node;

    assert(st_lookup(node_ref_table, (char *)edge->source->id,
                     (char **)&new_node));
...
re_node *re_get_node(graph, index) re_graph *graph;
int index;
{
...
re_edge *re_get_edge(graph, index) re_graph *graph;
int index;
{
...
re_node *re_get_primary_input(graph, index) re_graph *graph;
int index;
{
...
re_node *re_get_primary_output(graph, index) re_graph *graph;
int index;
{
...
re_edge *re_get_fanin(node, index) re_node *node;
int index;
{
...
re_edge *re_get_fanout(node, index) re_node *node;
int index;
{
...
re_edge *re_create_edge(graph, source, sink, index, weight,
                        breadth) re_graph *graph;
re_node *source, *sink;
...
re_edge *re_edge_dup(edge) re_edge *edge;
{
  int i;
  re_edge *new_edge;
...
re_node *re_node_dup(node) re_node *node;
{
  re_node *new_node;
