...
int com_retime(network, argc, argv) network_t **network;
int argc;
char **argv;
{
  re_graph *graph;
...
/* ARGSUSED */
int com__print_stats(network, argc, argv) network_t **network;
int argc;
char **argv;
{
  int c;
...
static void retime_print_stg_data(stg) graph_t *stg;
{
  if (stg == NIL(graph_t))
    return;
...
int com__check_tr(network, argc, argv) network_t **network;
int argc;
char **argv;
{
  lsGen gen;
