...
int com_retime(network_t **network, int argc, char **argv) {
    re_graph        *graph;
    delay_model_t   model;
    ...
    while ((c = util_getopt(argc, argv, "fimnv:c:d:a:t:")) != EOF) {
        switch (c) {
            case 'n':use_mapped = FALSE;
                model           = DELAY_MODEL_UNIT_FANOUT;
                break;
            case 'i':should_initialize = 0; /* Do not initialize latches */
                break;
            case 'm':minimize_reg = TRUE;
                break;
            case 'f':milp_flag = TRUE;
                break;
            case 't':retime_tol = atof(util_optarg);
                break;
            case 'd':delay_r = atof(util_optarg);
                break;
            case 'a':area_r = atof(util_optarg);
                break;
            case 'c':desired_c = atof(util_optarg);
                break;
            case 'v':retime_debug = atoi(util_optarg);
                break;
            default:goto retime_usage;
        }
    }
    ...
    if (use_mapped) {
        if (!lib_network_is_mapped(*network)) {
            (void) fprintf(
                    siserr,
                    "Network is not mapped (use -n option for unmapped networks)\n");
            return 0;
        }
        if (lib_get_library() == NIL(library_t)) {
            (void) fprintf(
                    siserr,
                    "Need to define a library (use -n option for unmapped networks)\n");
            return 0;
        }
    }
    ...
    if (found_unknown_type && old_type != UNKNOWN) {
        (void) fprintf(
                sisout,
                "Latches with unknown synchronization type assumed %s triggered\n",
    ...
    if (desired_c == RETIME_NOT_SET) {
        if ((spec_c = clock_get_cycletime(*network)) > 0) {
            (void) fprintf(
                    sisout,
                    "Retiming will try to meet the cycle time of the specification\n");
            desired_c = spec_c;
        }
    }
     ...
     */
    flag = retime_graph(*network, graph, area_r, delay_r, retime_tol, desired_c,
                        &new_c, milp_flag, minimize_reg, should_initialize,
                        &can_initialize);
    ...
    if (flag <= 0) {
        ...
        if (should_initialize && !can_initialize) {
            (void) fprintf(sisout, "RETIMING DOES NOT PRESERVE INITIAL STATES --- "
                                   "NETWORK NOT CHANGED\n");
            (void) fprintf(sisout,
                           "(use -i option to suppress initial-state computation)\n");
        } else {
    ...
    if (success) {
        ...
        if (new_network == NIL(network_t)) {
            (void) fprintf(siserr, "Unable to reconstruct network: %s",
                           error_string());
            re_delete_temp_nodes(*network);
            return 1;
        }
    ...
    (void) fprintf(
            sisout, "-d\t#.#\t: Set the delay thru register (with -n option only)\n");
    (void) fprintf(
            sisout, "-a\t#.#\t: Set the area of a register (with -n option only)\n");
    (void) fprintf(sisout, "-v\t#\t: Set the verbosity level (0-100)\n");
    return 1;
}

/* ARGSUSED */
int com__print_stats(network_t **network, int argc, char **argv) {
    int           c;
    re_graph      *graph;
    double        init_c;
    latch_synch_t old_type;
    ...
    while ((c = util_getopt(argc, argv, "d:a:")) != EOF) {
        switch (c) {
            ...
            case 'd':delay_r = atof(util_optarg);
                break;
            default:(void) fprintf(sisout, "Unknown option\n");
                return 1;
                /* NOTREACHED */
                break;
        }
    ...
    if (retime_get_clock_data(*network, use_mapped, &flag, &to_init, &old_type,
                              &delay_r, &area_r)) {
        (void) fprintf(siserr, "%s", error_string());
        return 1;
    }
...
static void retime_print_stg_data(graph_t *stg) {
    if (stg == NIL(graph_t))
        return;
...
int com__check_tr(network_t **network, int argc, char **argv) {
    lsGen         gen;
    latch_t       *latch;
    re_graph      *graph;
    network_t     *newnet;
    ...
    if (retime_get_clock_data(*network, use_mapped, &found_unknown_type, &to_init,
                              &old_type, &delay_r, &area_r)) {
        (void) fprintf(siserr, "%s", error_string());
        return 1;
    }
    if (found_unknown_type && old_type != UNKNOWN) {
