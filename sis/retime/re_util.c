 ...
 */
void re_graph_add_node(graph, node, use_mapped, d_latch,
                       node_to_id_table) re_graph *graph;
node_t *node;
int use_mapped;
lib_gate_t *d_latch;
 ...
 */
static double retime_get_gate_load(node, d_latch) node_t *node;
lib_gate_t *d_latch;
{
  int pin;
  lsGen gen;
 ...
 */
static int retime_node_num_fanout(node) node_t *node;
{
  int count;
  lsGen gen;
  node_t *fo, *end;
...
int retime_get_dff_info(network, area_r, delay_r, type) network_t *network;
...
double retime_simulate_gate(gate, load) lib_gate_t *gate;
double load;
{
  int i, j, n;
  delay_time_t t, **a_t;
 ...
 */
re_edge *re_graph_add_edge(graph, from, to, weight, breadth, index,
                           node_to_id_table) re_graph *graph;
node_t *from, *to;
int weight;
double breadth;
...
array_t *re_graph_dfs(graph) re_graph *graph;
{
  int i;
  re_node *p;
  st_table *visited;
...
array_t *re_graph_dfs_from_input(graph) re_graph *graph;
{
  int i;
  re_node *p;
  st_table *visited;
...
static int re_graph_dfs_recur(node, node_vec, visited, dir, level,
                              weight) re_node *node;
array_t *node_vec;
st_table *visited;
...
int re_retime_node(node, r) re_node *node;
int r;
{
  int i;
  re_edge *edge;
...
int retime_check_graph(graph) re_graph *graph;
{
  int i, j;
  re_node *node;
  re_edge *edge;
 ...
 */
void retime_single_node(node, lag) re_node *node;
int lag;
{
  int i;
  re_edge *edge;
...
void retime_dump_graph(fp, graph) FILE *fp;
re_graph *graph;
{
  int i, j;
  re_node *re_no;
 ...
 */
static double retime_get_user_constraint(node) node_t *node;
{
  int flag;
  delay_time_t t;
  clock_edge_t edge;
 ...
 * control information
 */
int retime_get_clock_data(network, use_mapped, found_unknown_type, should_init,
                          old_type, delay_r, area_r) network_t *network;
int use_mapped;
 ...
 */
int retime_is_network_retimable(network) network_t *network;
{
  int flag;
