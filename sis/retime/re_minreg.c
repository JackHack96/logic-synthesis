...
int retime_min_register(graph, c, retiming) re_graph *graph;
double c;
int *retiming;
{
  wd_t **WD;
 ...
 * bunch of vertices....
 */
static void re_add_register_sharing_node(graph) re_graph *graph;
{
  array_t *array;
  re_edge *re_ed, *edge;
  re_node *re_no, *new_node;
 ...
 */
static void re_setup_lp_tableau(graph, N, WD, c, ap, mp, m1p, m2p, m3p,
                                trans) re_graph *graph;
int N;
wd_t **WD;
int c;
double ***ap;
 ...
 */
static void re_adjust_user_delays(graph) re_graph *graph;
{
  int i, j;
  re_edge *re_ed;
  re_node *re_no;
