...
int *r_vec;             /* solution vector: r_vec[i] corresponds to node =
                           array_fetch(re_node *, graph->nodes, i) */
...
static int make_retiming_negative(graph, retiming)
    re_graph *graph; /* graph representation of network for retiming */
...
static void retime_get_input_order(network, graph, input_order,
                                   use_re_node_flag) network_t *network;
re_graph *graph;
st_table *input_order;
int use_re_node_flag; /* if 1, map re_node to integers; if 0, map node_t to
                         integers */
{
  int i;
  re_node *node;
...
static void set_initial_state(graph, latch_values)
    re_graph *graph; /* graph representation of network for retiming */
...
static void simulate_forward(graph, retiming, input_seq)
    re_graph *graph; /* graph representation of network for retiming */
int *retiming;       /* solution vector (integer version) */
array_t *input_seq;  /* array of char* pointing to input sequences from ancestor
                        to reset */
{
...
static re_stack_t *re_stack_init(graph) re_graph *graph;
{
  int i;
  int n_nodes;
  re_stack_t *stack;
...
static void re_stack_free(stack) re_stack_t *stack;
{
  FREE(stack->bottom);
  FREE(stack->in_stack);
  FREE(stack);
...
static void re_stack_push(stack, node) re_stack_t *stack;
re_node *node;
{
  int n_input_latches;
...
static re_node *re_stack_pop(stack) re_stack_t *stack;
{
  re_node *node;
...
static int re_stack_is_empty(stack) re_stack_t *stack;
{ return (stack->current == stack->bottom); }

/* yes iff currently a latch is present on each fanin edge */

static int min_n_input_latch(node) re_node *node;
{
  int i;
  re_edge *backedge;
...
static void retime_primary_input_forward(
    node, input_index, input_seq) re_node *node; /* node being retimed */
int input_index;
array_t *input_seq;
{
  int i, j;
...
static void realloc_initial_values(edge, size_incr) re_edge *edge;
int size_incr;
{
  int i;
...
static void retime_primary_output_forward(
    node, n_shift) re_node *node; /* node being retimed */
int n_shift;
{
  int i;
  re_edge *edge;
...
static void retime_node_forward(node,
                                n_shift) re_node *node; /* node being retimed */
int n_shift;
{
  int i, j;
  re_edge *edge;
...
static int simulate_node_function(node, in_values) node_t *node;
int *in_values;
{
  int i, j, num_in;
  node_cube_t cube;
 ...
 */
static st_table *retime_get_latch_corresp(net1, net2) network_t *net1, *net2;
{
  lsGen gen;
  node_t *po1, *po2;
  latch_t *l1, *l2;
...
static st_table *retime_swap_latches(ancestor, latch_table) st_table *ancestor,
    *latch_table;
{
  int value;
  st_generator *stgen;
