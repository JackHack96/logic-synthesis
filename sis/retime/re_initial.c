 ...
 * In that case, use "add_reset" before retiming and "remove_reset" after
 * retiming.
 *
...
int retime_update_init_states(network_t *network, re_graph *graph, int *r_vec, int should_init, int *can_init) {
    ...
    st_table     *input_order; /* maps external PI in 'network' to small integers:
                            their order of occurrence in 'graph' */
    st_generator *stgen;
    latch_t      *latch;
    char         *input;
    int          value;
    ...
    *can_init = TRUE;
    ...
    if (seqbdd_extract_input_sequence(dup_network, n_shift, input_order,
                                      input_seq, reset_ancestor)) {
        *can_init = FALSE;
    } else {
        if (retime_debug) {
...
/*
 * Expects that initially the PIPO nodes have a retiming value of 0. This is
 * checked. computes n_shift = MAX_i(retiming[i]) and subtract this value to all
 * entries in 'retiming'.
 */

static int make_retiming_negative(re_graph *graph, int *retiming) {
    int     i;
    ...
    re_foreach_primary_input(graph, i, pi) { assert(retiming[pi->id] == 0); }
    re_foreach_primary_output(graph, i, po) { assert(retiming[po->id] == 0); }
    max_value = 0;
    for (i    = n_nodes; i-- > 0;) {
        if (max_value < retiming[i])
            max_value = retiming[i];
    }
    n_shift   = max_value;
    for (i    = n_nodes; i-- > 0;)
        retiming[i] -= n_shift;
...
static void retime_get_input_order(network_t *network, re_graph *graph, st_table *input_order, int use_re_node_flag) {
    int     i;
    re_node *node;
    node_t  *old_pi, *pi;
...
static void set_initial_state(re_graph *graph, st_table *latch_values) {
    re_edge *edge;
    latch_t *latch;
 ...
 * A node is ready to be retimed if it still needs to be retimed and it has
 * latches on all its inputs. A node still needs to be retimed if
 * retiming[node_id] < 0. We need to be careful not to put the same node twice
 * on the stack, or we will get stack overflow. For that,we use the special
 * array 'on_stack' that checks for this condition. After a node has fired, it
 * may enable its outputs: we check to see if any output needs to be put on the
 * stack. The algorithm completes when the stack is empty. For consistency, we
 * check that the retiming is complete then (all entries of 'retiming' are 0).
 */

static void simulate_forward(re_graph *graph, int *retiming, array_t *input_seq) {
    int        i, input_position, local_shift;
    re_node    *node, *output;
    re_edge    *outedge;
    int        n_nodes;
    re_stack_t *stack;
    st_table   *pi_order; /* correspondance between 'graph' PI and indices in
                         'input_seq' */
    ...
    while (!re_stack_is_empty(stack)) {
        ...
        switch (node->type) {
            case RE_PRIMARY_INPUT:assert(array_n(input_seq) == local_shift);
                assert(st_lookup_int(pi_order, (char *) node, &input_position));
                retime_primary_input_forward(node, input_position, input_seq);
                break;
            case RE_PRIMARY_OUTPUT:retime_primary_output_forward(node, local_shift);
                break;
            case RE_INTERNAL:retime_node_forward(node, local_shift);
                break;
            default: fail("node of type RE_IGNORE encountered");
        }
        ...
        re_foreach_fanout(node, i, outedge) {
            if (re_ignore_edge(outedge))
                continue;
            output = outedge->sink;
            if (retiming[output->id] < 0)
                re_stack_push(stack, output);
        }
    }
...
static re_stack_t *re_stack_init(re_graph *graph) {
    int        i;
    int        n_nodes;
    re_stack_t *stack;
...
static void re_stack_free(re_stack_t *stack) {
    FREE(stack->bottom);
    FREE(stack->in_stack);
    FREE(stack);
}
...
/*
 * Does not always push. Checks whether the node needs to be pushed on the stack
 * or not. Supposes that the node needs retiming. This should be checked by the
 * caller.
 */

static void re_stack_push(re_stack_t *stack, re_node *node) {
    int n_input_latches;

    if (stack->in_stack[node->id])
        return;
    n_input_latches = min_n_input_latch(node);
    if (n_input_latches <= 0)
        return;
...
static re_node *re_stack_pop(re_stack_t *stack) {
    re_node *node;
...
static int re_stack_is_empty(re_stack_t *stack) { return (stack->current == stack->bottom); }

/* yes iff currently a latch is present on each fanin edge */

static int min_n_input_latch(re_node *node) {
    int     i;
    re_edge *backedge;
    int     min_weight = (int) INFINITY;

    re_foreach_fanin(node, i, backedge) {
        if (re_ignore_edge(backedge))
            continue;
...
static void retime_primary_input_forward(re_node *node, int input_index, array_t *input_seq) {
    int     i, j;
    re_edge *edge;
...
static void realloc_initial_values(re_edge *edge, int size_incr) {
    int i;
...
static void retime_primary_output_forward(re_node *node, int n_shift) {
    int     i;
    re_edge *edge;
...
static void retime_node_forward(re_node *node, int n_shift) {
    int     i, j;
    re_edge *edge;
    ...
    for (i = 0; i < n_shift; i++) {
        re_foreach_fanin(node, j, edge) {
            if (re_ignore_edge(edge))
                continue;
            input_values[i][edge->sink_fanin_id] =
                    edge->initial_values[i + edge->weight - n_shift];
        }
    }
    re_foreach_fanin(node, j, edge) {
        if (re_ignore_edge(edge))
...
static int simulate_node_function(node_t *node, int *in_values) {
    int             i, j, num_in;
    node_cube_t     cube;
    ...
    for (i = 0; i < num_cubes; i++) {
        ...
        for (j = 0; j < num_in; j++) {
            ...
            switch (literal) {
                case ZERO:local_out_value = and_table[local_out_value][1 - in_values[j]];
                    break;
                case ONE:local_out_value = and_table[local_out_value][in_values[j]];
                    break;
                case TWO:break;
                default: fail("bad literal");
                    /* NOTREACHED */
            }
            if (local_out_value == 0)
                break;
        }
 ...
 */
static st_table *retime_get_latch_corresp(network_t *net1, network_t *net2) {
    lsGen    gen;
    node_t   *po1, *po2;
    latch_t  *l1, *l2;
...
static st_table *retime_swap_latches(st_table *ancestor, st_table *latch_table) {
    int          value;
    st_generator *stgen;
    latch_t      *l1, *l2;
