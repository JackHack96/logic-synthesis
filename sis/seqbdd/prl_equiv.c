...
static void equiv_nets(network_t *, seq_info_t *, bdd_t *, prl_options_t *);

void Prl_EquivNets(network_t *network, prl_options_t *options) {
    seq_info_t *seq_info;
...
struct net_info_struct {
    ...
    array_t    *equiv_array; /* array of nets equivalent to this one; empty if index
                           > root_index */
    net_info_t *best_member; /* best node to be taken as source among all nodes in
                              equiv_array */
...
static void initialize_net_entry(seq_info_t *, net_info_t *, node_t *, int,
                                 bdd_t *);

static void compute_equivalence_classes(int, net_info_t *);

static void report_equivalence_classes(int, net_info_t *);

static int skip_this_net(net_info_t *);

static void collapse_equiv_nets(net_info_t *, prl_options_t *);

static void equiv_nets(network_t *network, seq_info_t *seq_info, bdd_t *dc, prl_options_t *options) {
    int        i;
    lsGen      gen;
    int        node_count;
    int        n_nets;
    ...
    foreach_node(network, gen, node) {
        if (node->type == PRIMARY_OUTPUT)
            continue;
        initialize_net_entry(seq_info, &(net_info[node_count]), node, node_count,
                             care_set);
        node_count++;
    }
...
static int skip_this_net(net_info_t *net_info) {
    if (net_info->root_index < net_info->index)
        return 1;
    if (array_n(net_info->equiv_array) <= 1)
        return 1;
    return 0;
}

static void initialize_net_entry(seq_info_t *seq_info, net_info_t *net_info, node_t *node, int index, bdd_t *care_set) {
    bdd_t *fn;
...
static void compute_equivalence_classes(int n_nets, net_info_t *net_info) {
    int i, j;

    for (i = 0; i < n_nets; i++) {
        assert(net_info[i].index == i);
        if (net_info[i].root_index < net_info[i].index)
            continue;
        for (j = i + 1; j < n_nets; j++) {
            if (net_info[j].root_index < net_info[j].index)
                continue;
            if (bdd_equal(net_info[i].fn, net_info[j].fn)) {
                array_insert_last(net_info_t *, net_info[i].equiv_array,
                                  &(net_info[j]));
...
static void report_equivalence_classes(int n_nets, net_info_t *net_info) {
    int i;
    int max_size;
    int *stats;
    int n_classes;
 ...
 * First: compute the costs: based on depth, then on amount of logic that can be
 * removed. depth is important: guarantees that we are not creating cycles.
 *
 ...
 * 'best_member' has been computing earlier, as the best choice within the
 * class.
 */

static void compute_node_cost_rec(node_t *, st_table *, st_table *);

static void MoveFanout(node_t *, node_t *);

static void collapse_equiv_nets(net_info_t *net_info, prl_options_t *options) {
    int        i;
    int        is_inverted;
    ...
    for (i = 0; i < array_n(net_info->equiv_array); i++) {
        ...
        assert(st_lookup(cost_table, (char *) net->node,
                         (char **) &(net->cost))); /* long cost */
        assert(st_lookup(depth_table, (char *) net->node,
                         (char **) &(net->depth))); /* long depth */
        if (options->verbose > 0) {
            (void) fprintf(sisout, "source %s depth=%d cost=%d\n", net->node->name,
                           net->depth, net->cost);
        }
        if (net->depth < best_depth ||
            (net->depth == best_depth && net->cost < best_cost)) {
    ...
    for (i = 0; i < array_n(net_info->equiv_array); i++) {
        ...
        if (options->verbose > 0) {
            (void) fprintf(sisout, "move fanout of node %s to node %s ",
                           net->node->name, source_node->name);
...
static void compute_node_cost_rec(node_t *node, st_table *cost_table, st_table *depth_table) {
    int    i;
    long   fanin_cost, fanin_depth;
    long   cost, depth;
    node_t *fanin;

    if (st_lookup(cost_table, (char *) node, NIL(char *)))
        return;
    cost  = (node_num_fanout(node) == 1) ? node_num_literal(node) : 0;
    depth = 0;
    foreach_fanin(node, i, fanin) {
        compute_node_cost_rec(fanin, cost_table, depth_table);
        assert(st_lookup(cost_table, (char *) fanin,
                         (char **) &fanin_cost)); /* long fanin_cost */
        assert(st_lookup(depth_table, (char *) fanin,
                         (char **) &fanin_depth)); /* long fanin_depth */
        cost += fanin_cost;
...
static void MoveFanout(node_t *from, node_t *to) {
    int    i;
    lsGen  gen;
    int    n_fanouts;
    node_t *fanout;
    ...
    foreach_fanout(from, gen, fanout) { fanouts[i++] = fanout; }
