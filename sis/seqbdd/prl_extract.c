...
static int AddEnvFsm(network_t *, network_t *, array_t *);

static bdd_t *MakeFsmInputRelation(seq_info_t *, array_t *);

static void FreeConnectionArray(array_t *);

typedef struct {
    char *input_name; /* a copy of the name of an external PI in fsm */

    node_t *copy_output; /* the node to which 'pi' is attached after performing
                          (env x fsm) */
...
int Prl_ExtractEnvDc(network_t *fsm_network, network_t *env_network, prl_options_t *options) {
    ...
    if (AddEnvFsm(product_network, env_network, controlled_fsm_inputs)) {
        ...
        fsm_reachable_transitions =
                bdd_and_smooth(reachable_states, fsm_input_relation, env_vars);
...
static void CopyLatchPo(network_t *, node_t *, char *, node_t *);
 ...
 *	   (pi,po,copy) where 'pi' belongs to 'to_network', 'po' belongs to
 *'env_network', and 'copy' belongs to the new network. 'copy' is the copy in
...
static int AddEnvFsm(network_t *to_network, network_t *env_network, array_t *connections) {
    int        i;
    char       *pi_name;
    lsGen      gen;
    node_t     *pi;
    ...
    foreach_primary_input(to_network, gen, pi) {
        if (!network_is_real_pi(to_network, pi))
            continue;
        connection.pi          = pi;
    ...
    foreach_primary_output(env_network, gen, env_output) {
        ...
        if (!network_is_real_po(env_network, env_output)) {
            ...
            if (pi == NIL(node_t) || !network_is_real_pi(to_network, pi)) {
                if (CopyRealPo(to_network, copy_output, output_name))
                    return 1;
            } else {
    ...
    /*
     * We need to make sure that inputs of 'to_network' that are fed by outputs of
     * 'env_network' are directly connected to the outputs of 'to_network'. for
     * that, we need to move the fanout of 'pi' onto 'copy_output'.
     */
...
static node_t *AddPrimaryOutput(network_t *, node_t *, char *);
...
static int CopyRealPo(network_t *network, node_t *node, char *name) {
    ...
    if (network_find_node(network, name)) {
        (void) fprintf(
                siserr, "Error: network \"%s\" already has a PO named \"%s\"; abort\n",
...
static void CopyLatchPo(network_t *network, node_t *node, char *name, node_t *original_po) {
    node_t  *product_output;
    node_t  *product_po;
    ...
    network_create_latch(network, &product_latch, product_po,
                         matching_input->copy);
...
static node_t *AddPrimaryOutput(network_t *network, node_t *fanin, char *name) {
    node_t *result;
...
static void ExtractBddVars(seq_info_t *seq_info, network_t *env_network, array_t *env_vars, array_t *fsm_state_vars) {
    int    i;
    lsGen  gen;
    bdd_t  *var;
    node_t *input;

    foreach_primary_input(env_network, gen, input) { input->copy->copy = input; }
    for (i = 0; i < array_n(seq_info->input_nodes); i++) {
        ...
        if (input->copy != NIL(node_t) &&
            node_network(input->copy) == env_network) {
...
static bdd_t *MakeFsmInputRelation(seq_info_t *seq_info, array_t *controlled_fsm_inputs) {
    int        i;
    bdd_t      *result;
    ...
    for (i = 0; i < array_n(controlled_fsm_inputs); i++) {
        connection = array_fetch(Connection, controlled_fsm_inputs, i);
        if (connection.copy_output != NIL(node_t)) {
            fn         = ntbdd_node_to_bdd(connection.copy_output, seq_info->manager,
                                           seq_info->leaves);
            var        = array_fetch(bdd_t *, new_input_vars, i);
            tmp        = bdd_xnor(var, fn);
...
typedef int (*CheckFn)(bdd_t *, bdd_t *, seq_info_t *, prl_options_t *, void *);

static int CheckCurrentStateTautology(bdd_t *, bdd_t *, seq_info_t *,
                                      prl_options_t *, void *);

static int BreadthFirstTraversal(seq_info_t *, prl_options_t *, CheckFn,
                                 void *);

static int MakeProductFsm(network_t *, network_t *, array_t *, array_t *);

static void ReportEnvErrorTrace(network_t *, array_t *, seq_info_t *,
                                prl_options_t *);

static void RemoveUnusedPos(network_t *, array_t *, prl_options_t *);
 ...
 *    but free outputs of 'env_network' do not make any sense, so we report a
 *warning and ignore these outputs (done in 3).
 ...
 *    input minterms that generate the error. By simulation through
 *'fsm_network' and 'env_network', we deduce a sequence of input minterms to
 *'fsm_network' that activates the problem.
 *
 * Results:
 *	Returns 0 iff everything went all right.
 *
 ...
 *	that can be generated by the environment and differentiates the two
 *FSMs.
 *
 *----------------------------------------------------------------------
 */

int Prl_VerifyEnvFsm(network_t *fsm_network, network_t *check_network, network_t *env_network, prl_options_t *options) {
    seq_info_t *seq_info;
    ...
    if (MakeProductFsm(check_network, fsm_network, xnor_outputs, other_outputs) ||
        ...
        status   = BreadthFirstTraversal(seq_info, options,
                                         CheckCurrentStateTautology, NIL(void));
        if (status) {
            ReportEnvErrorTrace(check_network, controlled_fsm_inputs, seq_info,
                                options);
        } else {
...
static int BreadthFirstTraversal(seq_info_t *seq_info, prl_options_t *options, CheckFn checkFn, void *closure) {
    bdd_t *current_set;
    bdd_t *total_set;
    ...
    for (;;) {
        ...
        new_current_set =
                (*options->compute_next_states)(current_set, seq_info, options);
...
static int
CheckCurrentStateTautology(bdd_t *current_set, bdd_t *total_set, seq_info_t *seq_info, prl_options_t *options,
                           void *closure) {
    int   i;
    bdd_t *fn;
...
static int CheckAndRecordStates(bdd_t *, bdd_t *, seq_info_t *, prl_options_t *,
                                void *);

static void PrintEnvErrorTrace(array_t *, network_t *, array_t *, seq_info_t *,
                               prl_options_t *);
...
static void
ReportEnvErrorTrace(network_t *product_network, array_t *connections, seq_info_t *seq_info, prl_options_t *options) {
    Closure closure;
    int     iter_count;
    bdd_t   *tmp;
    bdd_t   *output_fn;
    ...
    assert(BreadthFirstTraversal(seq_info, options, CheckAndRecordStates,
                                 (void *) &closure));
    ...
    output_fn =
            array_fetch(bdd_t *, seq_info->external_output_fns, closure.output_index);
    ...
    for (;;) {
        Prl_GetOneEdge(incorrect_domain, seq_info, &incorrect_state,
                       &incorrect_input);
    ...
    PrintEnvErrorTrace(input_trace, product_network, connections, seq_info,
                       options);
...
static int CheckAndRecordStates(bdd_t *current_set, bdd_t *total_set, seq_info_t *seq_info, prl_options_t *options,
                                void *closure) {
    int     i;
    bdd_t   *fn;
...
typedef struct PrintClosureStruct {
    ...
    array_t
              *fsm_inputs; /* an array of node_t *; copies of 'fsm_network' ext. PIs */
}           PrintClosure;

static PrintClosure *PrintClosureInit(network_t *, array_t *, seq_info_t *);

static array_t *PrintClosureSimulateNetwork(bdd_t *, PrintClosure *);

static void PrintClosureFree(PrintClosure *);
...
static void
PrintEnvErrorTrace(array_t *input_trace, network_t *product_network, array_t *connections, seq_info_t *seq_info,
...
static array_t *ExtractFsmInputs(network_t *, array_t *);
...
static PrintClosure *PrintClosureInit(network_t *product_network, array_t *connections, seq_info_t *seq_info) {
    lsGen        gen;
    node_t       *pi;
    latch_t      *latch;
    int          init_value;
...
static array_t *ExtractFsmInputs(network_t *network, array_t *connections) {
    int        i;
    lsGen      gen;
    node_t     *node;
    array_t    *fsm_inputs;
...
static array_t *PrintClosureSimulateNetwork(bdd_t *input, PrintClosure *closure) {
    int     i;
    int     value;
    lsGen   gen;
    latch_t *latch;
    ...
    foreach_primary_input(closure->network, gen, pi) {
        if ((latch = latch_from_node(pi)) != NIL(latch_t)) {
            ...
            assert((var = array_fetch(bdd_t * , closure->input_vars, i)) !=
                   NIL(bdd_t));
    ...
    output_values = simulate_network(closure->network, closure->node_vec,
...
static void PrintClosureFree(PrintClosure *closure) {
...
static node_t *AddXnorPo(network_t *, node_t *, node_t *);
...
static int
MakeProductFsm(network_t *to_network, network_t *from_network, array_t *xnor_outputs, array_t *other_outputs) {
    lsGen  gen;
    node_t *from_output;
    node_t *from_fanin;
    node_t *to_output;
    ...
    foreach_primary_output(from_network, gen, from_output) {
        ...
        } else if (to_output == NIL(node_t) ||
                   !network_is_real_po(to_network, to_output)) {
            return 1;
        } else {
...
static node_t *AddXnorPo(network_t *network, node_t *po, node_t *node) {
    char   *name;
    node_t *tmp0, *tmp1;
    node_t *xnor_node;
...
static void RemoveUnusedPos(network_t *network, array_t *used_outputs, prl_options_t *options) {
    int     i;
    lsGen   gen;
    node_t  *po;
    array_t *to_be_removed = array_alloc(node_t *, 0);

    foreach_primary_output(network, gen, po) { po->copy = NIL(node_t); }
...
static void FreeConnectionArray(array_t *connections) {
    int        i;
    Connection connection;
