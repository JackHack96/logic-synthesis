        ...
        array_t *controlled_fsm_inputs;
{
    ...
    for (i = 0; i < array_n(controlled_fsm_inputs); i++) {
        ...
        if (connection.copy_output != NIL(node_t)) {
            ...
            array_insert_last(
            char *, seq_info->var_names, util_strsav(pi->name));
...
static int CheckCurrentStateTautology(bdd_t *, bdd_t *, seq_info_t *, prl_options_t *, void *);

static int BreadthFirstTraversal(seq_info_t *, prl_options_t *, CheckFn, void *);

static int MakeProductFsm(network_t *, network_t *, array_t *, array_t *);

static void ReportEnvErrorTrace(network_t *, array_t *, seq_info_t *, prl_options_t *);
        ...
        prl_options_t *options;
{
    ...
    if (MakeProductFsm(check_network, fsm_network, xnor_outputs, other_outputs)
        ...
        status   = BreadthFirstTraversal(seq_info, options, CheckCurrentStateTautology, NIL(
        void));
        if (status) {
...
static int CheckAndRecordStates(bdd_t *, bdd_t *, seq_info_t *, prl_options_t *, void *);

static void PrintEnvErrorTrace(array_t *, network_t *, array_t *, seq_info_t *, prl_options_t *);
        ...
        seq_info_t *seq_info;
{
    ...
    closure->input_values = array_alloc(
    int, network_num_pi(closure->network));
        ...
        PrintClosure *closure;
{
    ...
    foreach_primary_input(closure->network, gen, pi)
    {
        ...
        array_insert(
        int, closure->input_values, i, value);
        i++;
    }
