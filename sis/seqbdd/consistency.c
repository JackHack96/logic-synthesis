...
/* ARGSUSED */
range_data_t *consistency_alloc_range_data(network, options) network_t *network;
...
void consistency_free_range_data(data, options) range_data_t *data;
verif_options_t *options;
{
  output_info_t *info = options->output_info;
...
/* ARGSUSED */
bdd_t *consistency_compute_next_states(current_set, data,
...
/* ARGSUSED */
bdd_t *consistency_compute_reverse_image(next_set, data,
                                         options) bdd_t *next_set;
range_data_t *data;
verif_options_t *options;
{
...
int consistency_check_output(current_set, data, index,
                             options) bdd_t *current_set;
range_data_t *data;
int *index;
verif_options_t *options;
...
void consistency_bdd_sizes(data, fn_size, output_size) range_data_t *data;
int *fn_size;
int *output_size;
{
  if (fn_size) {
...
st_table *extract_input_to_output_table(org_pi, new_pi, po_ordering,
                                        network) array_t *org_pi;
array_t *new_pi;
array_t *po_ordering;
network_t *network;
{
...
array_t *extract_state_input_vars(manager, pi_ordering,
                                  ito_table) bdd_manager *manager;
...
/* the order of appearance in pi_ordering */
array_t *extract_state_output_vars(manager, pi_ordering,
                                   ito_table) bdd_manager *manager;
st_table *pi_ordering;
st_table *ito_table;
{
