...
/* ARGSUSED */
range_data_t *consistency_alloc_range_data(network_t *network, verif_options_t *options) {
    ...
    if (options->does_verification) {
        data->output_fn =
                ntbdd_node_to_bdd(info->output_node, data->manager, info->pi_ordering);
    } else {
    ...
    data->consistency_fn   =
            ntbdd_node_to_bdd(info->main_node, data->manager, info->pi_ordering);
...
void consistency_free_range_data(range_data_t *data, verif_options_t *options) {
    output_info_t *info = options->output_info;
...
/* ARGSUSED */
bdd_t *consistency_compute_next_states(bdd_t *current_set, range_data_t *data, verif_options_t *options) {
    bdd_t *result_as_output;
    bdd_t *new_current_set;

    result_as_output =
            bdd_and_smooth(data->consistency_fn, current_set, data->smoothing_inputs);
    new_current_set  =
...
/* ARGSUSED */
bdd_t *consistency_compute_reverse_image(bdd_t *next_set, range_data_t *data, verif_options_t *options) {
    bdd_t *next_set_as_next_state_vars;
    bdd_t *result;

    next_set_as_next_state_vars =
            bdd_substitute(next_set, data->input_vars, data->output_vars);
    result = bdd_and_smooth(data->consistency_fn, next_set_as_next_state_vars,
                            data->output_vars);
...
int consistency_check_output(bdd_t *current_set, range_data_t *data, int *index, verif_options_t *options) {
    if (bdd_leq(current_set, data->output_fn, 1, 1))
        return 1;
    report_inconsistency(current_set, data->output_fn,
                         options->output_info->pi_ordering);
    return 0;
}

void consistency_bdd_sizes(range_data_t *data, int *fn_size, int *output_size) {
    if (fn_size) {
...
/* returns a table that takes a state input node as key, and returns the new
 * primary input */
...
st_table *extract_input_to_output_table(array_t *org_pi, array_t *new_pi, array_t *po_ordering, network_t *network) {
    int      i, index;
    lsGen    gen;
    latch_t  *l;
    node_t   *input, *output;
    ...
    for (i = 0; i < array_n(org_pi); i++) {
        ...
        if (!st_lookup_int(name_table, ps_node->name, &index))
            continue;
...
array_t *extract_state_input_vars(bdd_manager *manager, st_table *pi_ordering, st_table *ito_table) {
    int          index;
    st_generator *gen;
    ...
    st_foreach_item_int(pi_ordering, gen, (char **) &ps_node, &index) {
        if (!st_lookup(ito_table, (char *) ps_node, (char **) &ns_node))
            ps_node = NIL(node_t);
...
/* the order of appearance in pi_ordering */
array_t *extract_state_output_vars(bdd_manager *manager, st_table *pi_ordering, st_table *ito_table) {
    int          index;
    st_generator *gen;
    node_t       *ps_node, *ns_node;
    array_t      *tmp = array_alloc(node_t *, 0);
