...
int seq_verify_interface(network1, network2, options) network_t *network1;
network_t *network2;
...
static int report_error_trace(range_data, options) range_data_t *range_data;
verif_options_t *options;
{
  int output_index;
  bdd_t *current_set;
...
network_t *range_computation_interface(network, options) network_t *network;
verif_options_t *options;
{
  output_info_t output_info;
  range_data_t *data;
...
int breadth_first_stg_traversal(network, data, options) network_t **network;
range_data_t **data;
verif_options_t *options;
{
  int i = 0;
...
static bdd_t *find_good_constraint(current_set, total_set,
                                   cofactor_fn) bdd_t *current_set;
bdd_t *total_set;
BddFn cofactor_fn;
{
...
void report_inconsistency(current_set, output_fn, leaves) bdd_t *current_set;
bdd_t *output_fn;
st_table *leaves;
{
  bdd_gen *gen;
...
static void use_cofactored_set(current_set, total_set,
                               cofactor_fn) bdd_t **current_set;
bdd_t **total_set;
BddFn cofactor_fn;
{
...
node_t *network_copy_subnetwork(network, node, pi_table,
                                node_table) network_t *network;
node_t *node;
st_table *pi_table;
st_table *node_table;
{
...
void print_node_array(array) array_t *array;
{
  int i;
...
static int node_table_entry_cmp(obj1, obj2) char *obj1;
char *obj2;
{
  int diff;
...
static void node_table_entry_print(entry) node_table_entry_t *entry;
{ (void)fprintf(misout, "%s=>%d ", entry->node->name, entry->value); }

void print_node_table(table) st_table *table;
{
  int i;
  node_t *node;
  st_generator *gen;
...
array_t *bdd_extract_var_array(manager, node_list, leaves) bdd_manager *manager;
array_t *node_list;
st_table *leaves;
{
  int i;
...
array_t *get_po_ordering(network, next_state_po, options) network_t *network;
...
st_table *get_pi_ordering(network, po_ordering, new_pi) network_t *network;
array_t *po_ordering;
array_t *new_pi;
{
  lsGen gen;
...
array_t *get_remaining_po(network, po_array) network_t *network;
array_t *po_array;
{
  int i;
  lsGen gen;
...
void output_info_init(info) output_info_t *info;
{ *info = DEFAULT_OUTPUT_INFO; }

void output_info_free(info) output_info_t *info;
{
  if (info->org_pi)
...
void report_elapsed_time(options, comment) verif_options_t *options;
char *comment;
{
...
st_table *from_array_to_table(array) array_t *array;
{
  int i;
  node_t *node;
...
static void bdd_cube_print(cube, leaves) array_t *cube;
st_table *leaves;
{
  int i;
  int phase;
...
array_t *order_nodes(node_vec, pi_only) array_t *node_vec;
int pi_only;
{
  int index;
  lsGen gen;
...
static void seqbdd_get_one_edge(from, data, state, input, options) bdd_t *from;
range_data_t *data;
bdd_t **state;
bdd_t **input;
verif_options_t *options;
...
/* ARGSUSED */
static void print_error_trace(input_trace, data, options) array_t *input_trace;
range_data_t *data;
verif_options_t *options;
{
  int i;
...
static void print_error_input(fp, input, input_vars) FILE *fp;
bdd_t *input;
array_t *input_vars;
{
  int i;
...
static st_table *get_pi_to_var_table(manager, info, data) bdd_manager *manager;
output_info_t *info;
range_data_t *data;
{
  int index;
