...
int Prl_RemoveDependencies(network_t *network, array_t *nodevec, prl_removedep_t *options) {
    int      i;
    node_t   *pi, *po;
    array_t  *pi_tfo;
    st_table *po_tfi;

    if (CheckInput(network, nodevec))
        return 1; /* wrong input */
    if (array_n(nodevec) == 1)
...
static int CheckInput(network_t *network, array_t *nodevec) {
    int    i;
    node_t *pi, *po;
...
static array_t *ExtractPiTfo(network_t *network, node_t *pi) {
    int      i, j;
    node_t   *node, *fanin;
    ...
    for (i = 0; i < array_n(nodevec); i++) {
        ...
        foreach_fanin(node, j, fanin) {
            if (!st_lookup(roots, (char *) fanin, NIL(char *)))
                continue;
...
static void BlowAwayDependencies(array_t *nodes, st_table *roots) {
    int     i;
    int     all_roots;
    int     none_roots;
    node_t  *node;
...
static array_t *GetRootFanouts(node_t *node, st_table *roots) {
    lsGen   gen;
    node_t  *fanout;
...
static node_t *ProcessInternalNode(node_t *node, array_t *root_fanouts) {
    node_t *copy;

    if (array_n(root_fanouts) == 0)
        return NIL(node_t);
    if (array_n(root_fanouts) == node_num_fanout(node))
        return node;
    copy = node_dup(node);
...
static void ProcessPrimaryInput(node_t *node, array_t *root_fanouts) {
    node_t *copy;
...
static void PatchFanin(array_t *nodevec, node_t *old, node_t *new) {
    int    i;
    node_t *node;
    ...
    for (i = 0; i < array_n(nodevec); i++) {
        ...
        node_scc(node);
        if (prl_dep_options.verbosity) {
            (void) fprintf(sisout, "move input of \"%s\" from \"%s\" to \"%s\"\n",
                           node->name, old->name, new->name);
        }
    }
}
