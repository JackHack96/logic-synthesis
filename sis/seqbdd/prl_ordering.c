...
static int set_key_hash(char *, int);

static int set_key_cmp(char *, char *);

static void set_cache_free(st_table *);

static int do_find_best_set_order(set_info_t *, set_key_t *, st_table *, int,
                                  int);

static void extract_best_order_from_cache(set_info_t *, st_table *, set_key_t *,
                                          array_t *);

static int compute_order_cost(set_info_t *, array_t *);

static void print_int_array(array_t *);

static void print_set_info(set_info_t *);

/*
*----------------------------------------------------------------------
*
* Prl_OrderSetHeuristic -- EXTERNAL ROUTINE
*
* Computes an heuristic solution to the following problem:
*
* INPUT: A1,...,An n subsets of some finite set S
* OUTPUT: an ordering of {1,...,n} such that
* 	if s_i = |\cup{1\leq j\leq i} A_i |
* 	cost = (\sum_{1\leq i \leq n} s_i)  is minimized.
*
* This routine is used for ordering output fns in BDD range computation
heuristics.

* The algorithm is exponential in the worst-case.
* It uses a branch and bound heuristic
* as well as the fact that if A_i is included into A_j
* we can always suppose that A_i appears before A_j.
* In fact if A_i is included in A_j
...
*
* Results:
* 	an array of size n_sets representing the optimal ordering of the sets.
*
* Side Effects:
*	None.
*
...
array_t *Prl_OrderSetHeuristic(set_info_t *info, int verbosity, int level) {
    int       cost;
    set_key_t *key    = ALLOC(set_key_t, 1);
    array_t   *result = array_alloc(int, 0);
    st_table  *cache  = st_init_table(set_key_cmp, set_key_hash);

    global_verbose = verbosity;
    if (verbosity >= 4)
        print_set_info(info);
    key->placed_so_far =
            var_set_new(info->n_sets); /* automatically initialized to 0 */
    cost = do_find_best_set_order(info, key, cache, level, LARGE_NUMBER);
...
static int dominator_cmp(char *, char *);

static void compute_bounds(set_info_t *, var_set_t *, var_set_t *, array_t *);

static array_t *extract_dominators(set_info_t *, var_set_t *, var_set_t *);

static var_set_t *extract_uncovered_variables(set_info_t *, set_key_t *);

static int arg_min_remaining_size(set_info_t *, var_set_t *, var_set_t *,
                                  int *);
...
static int do_find_best_set_order(set_info_t *info, set_key_t *key, st_table *cache, int depth, int allocated) {
    int         i;
    int         local_cost;
    set_key_t   *new_key;
    set_value_t *value;
    ...
    if (depth <= 0) {
        ...
        d.cost  = LARGE_NUMBER;
        d.index =
                arg_min_remaining_size(info, key->placed_so_far, dead_vars, &(d.size));
    ...
    for (i = 0; i < array_n(dominators); i++) {
        ...
        dom->cost = local_cost + do_find_best_set_order(info, new_key, cache,
                                                        depth - 1, allocated);
...
static int dominator_cmp(char *obj1, char *obj2) {
...
static int set_key_hash(char *k, int modulus) {
...
/* returns 0 iff equal */
static int set_key_cmp(char *k1, char *k2) {
...
static void set_cache_free(st_table *cache) {
    st_generator *gen;
    set_key_t    *key;
    set_value_t  *value;
...
static void extract_best_order_from_cache(set_info_t *info, st_table *cache, set_key_t *first_key, array_t *result) {
    int         i, j;
    set_key_t   key;
    set_value_t *value;
    ...
    if (i < info->n_sets) {
        for (j = 0; j < info->n_sets; j++) {
            if (var_set_get_elt(key.placed_so_far, j))
                continue;
...
static void compute_bounds(set_info_t *info, var_set_t *placed_so_far, var_set_t *mask, array_t *dominators) {
    int         i, j;
    dominator_t *dom;
    ...
    for (i = 0; i < array_n(dominators); i++) {
        ...
        for (j = 0; j < info->n_sets; j++) {
            if (j == dom->index)
                continue;
            if (var_set_get_elt(placed_so_far, j))
                continue;
...
static int set_is_less_than(var_set_t *, var_set_t *, var_set_t *, int);
...
static array_t *extract_dominators(set_info_t *info, var_set_t *placed_so_far, var_set_t *mask) {
    int         i, j;
    int         is_dominator;
    dominator_t dom;
    ...
    for (i = 0; i < info->n_sets; i++) {
        if (var_set_get_elt(placed_so_far, i))
            continue;
        is_dominator = 1;
        for (j       = 0; j < info->n_sets; j++) {
            if (j == i)
        ...
        if (is_dominator)
            var_set_set_elt(dominators, i);
    }
    tmp    = var_set_new(info->n_vars);
    for (i = 0; i < info->n_sets; i++) {
        if (!var_set_get_elt(dominators, i))
            continue;
...
static int set_is_less_than(var_set_t *set1, var_set_t *set2, var_set_t *mask, int diff) {
    int       result;
...
static var_set_t *extract_uncovered_variables(set_info_t *info, set_key_t *key) {
    int       i;
    var_set_t *result = var_set_new(info->n_vars);

    for (i = 0; i < info->n_sets; i++) {
        if (!var_set_get_elt(key->placed_so_far, i))
            continue;
...
static int arg_min_remaining_size(set_info_t *info, var_set_t *placed_so_far, var_set_t *mask, int *min_size) {
    int       i, size;
    int       best_index = -1;
...
static int compute_order_cost(set_info_t *info, array_t *order) {
    int       i;
    int       index;
    int       result        = 0;
...
static void check_cache(st_table *cache, set_key_t *key, set_value_t *value) {
    st_generator *gen;
    set_key_t    *k;
    set_value_t  *v;

    if (key != NIL(set_key_t) && value != NIL(set_value_t)) {
        fprintf(siserr, "insert %d, %d (%x) --> ", value->index, value->cost,
                value);
        fprintf(siserr, "(%d) ", var_set_hash(key->placed_so_far));
        var_set_print(siserr, key->placed_so_far);
    }
    st_foreach_item(cache, gen, (char **) &k, (char **) &v) {
        fprintf(siserr, "found %d, %d (%x) --> ", v->index, v->cost, v);
        fprintf(siserr, "(%d) ", var_set_hash(k->placed_so_far));
        var_set_print(siserr, k->placed_so_far);
        if (value != NIL(set_value_t)) {
            st_lookup(cache, (char *) k, (char **) &value);
            assert(v == value);
        }
    }
}

static void print_int_array(array_t *data) {
    int i;
...
static void print_set_info(set_info_t *info) {
    int i, j, value;
    for (i = 0; i < info->n_sets; i++) {
