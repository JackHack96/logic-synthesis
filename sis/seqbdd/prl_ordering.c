...
array_t *Prl_OrderSetHeuristic(info, verbosity, level) set_info_t *info;
int verbosity;
int level;
{
  int cost;
...
static int do_find_best_set_order(info, key, cache, depth,
                                  allocated) set_info_t *info;
set_key_t *key;
st_table *cache;
int depth;
int allocated; /* for bounding the search: if allocated < 0, can kill this
                  subtree */
{
  int i;
  int local_cost;
...
static int dominator_cmp(obj1, obj2) char *obj1;
char *obj2;
{
...
static int set_key_hash(k, modulus) char *k;
int modulus;
{
...
/* returns 0 iff equal */
static int set_key_cmp(k1, k2) char *k1;
char *k2;
{
...
static void set_cache_free(cache) st_table *cache;
{
  st_generator *gen;
  set_key_t *key;
  set_value_t *value;
...
static void extract_best_order_from_cache(info, cache, first_key,
                                          result) set_info_t *info;
st_table *cache;
set_key_t *first_key;
array_t *result;
{
...
static void compute_bounds(info, placed_so_far, mask,
                           dominators) set_info_t *info;
var_set_t *placed_so_far;
var_set_t *mask;
array_t *dominators;
{
...
static array_t *extract_dominators(info, placed_so_far, mask) set_info_t *info;
var_set_t *placed_so_far;
var_set_t *mask;
{
  int i, j;
...
static int set_is_less_than(set1, set2, mask, diff) var_set_t *set1;
var_set_t *set2;
var_set_t *mask;
int diff;
{
...
static var_set_t *extract_uncovered_variables(info, key) set_info_t *info;
set_key_t *key;
{
  int i;
...
static int arg_min_remaining_size(info, placed_so_far, mask,
                                  min_size) set_info_t *info;
var_set_t *placed_so_far;
...
static int compute_order_cost(info, order) set_info_t *info;
array_t *order;
{
  int i;
  int index;
...
static void check_cache(cache, key, value) st_table *cache;
set_key_t *key;
set_value_t *value;
{
  st_generator *gen;
...
static void print_int_array(data) array_t *data;
{
  int i;
...
static void print_set_info(info) set_info_t *info;
{
  int i, j, value;
  for (i = 0; i < info->n_sets; i++) {
