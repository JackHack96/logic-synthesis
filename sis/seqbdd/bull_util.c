...
bdd_t *input_cofactor(array_t *bdd_list, array_t *pi_list, bdd_manager *mg, bdd_t *total_set, st_table *cache,
                      st_table *leaves, verif_options_t *options, var_set_t *andset, int pt) {
    ...
    for (j = pt; j < andset->n_elts; j++) {
        if (var_set_get_elt(andset, j)) {
            ...
            right_bdd  = input_cofactor(right_list, right_pi, mg, rtotal_set, cache,
                                        leaves, options, andset, j + 1);
            left_bdd   = input_cofactor(left_list, left_pi, mg, ltotal_set, cache,
                                        leaves, options, andset, j + 1);
    ...
    return (
            bull_cofactor(bdd_list, pi_list, mg, total_set, cache, leaves, options));
}

array_t *disjoint_support_functions(array_t *list) {
    int       row, column;
    int       i, j;
    int       flag;
    array_t   *tmp;
...
bdd_t *range_2_compute(var_set_t *set, array_t *bdd_list, array_t *pi_list) {
    int   i, j;
    int   k1, k2;
