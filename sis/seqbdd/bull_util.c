...
bdd_t *input_cofactor(bdd_list, pi_list, mg, total_set, cache, leaves, options,
                      andset, pt) array_t *bdd_list;
array_t *pi_list;
bdd_manager *mg;
bdd_t *total_set;
st_table *cache;
...
array_t *disjoint_support_functions(list) array_t *list;
{
  int row, column;
  int i, j;
  int flag;
...
bdd_t *range_2_compute(set, bdd_list, pi_list) var_set_t *set;
array_t *bdd_list;
array_t *pi_list;
{
  int i, j;
