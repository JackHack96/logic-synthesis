...
static st_table *extract_order_info(network_t *network) {
    int      count       = 0;
    node_t   *pi;
    lsGen    gen;
...
static void order_table(st_table *node_table, st_table *name_table) {
    int          i;
    int          index;
    int          count;
    node_t       *node;
    ...
    st_foreach_item(node_table, gen, (char **) &node, NIL(
            char *)) {
        nodes[count++] = node;
    }
    for (i = 0; i < count; i++) {
...
static int get_node_order(char *name, st_table *name_table) {
    int  rank;
    char *new_name;
...
static int extract_trailing_y(char *name, char **new_name) {
    char *result;
