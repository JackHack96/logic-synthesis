...
void get_manual_order(order, options) st_table *order;
...
static st_table *extract_order_info(network) network_t *network;
{
  int count = 0;
  node_t *pi;
  lsGen gen;
...
static void order_table(node_table, name_table) st_table *node_table;
st_table *name_table;
{
  int i;
  int index;
...
static int get_node_order(name, name_table) char *name;
st_table *name_table;
{
  int rank;
  char *new_name;
...
static int extract_trailing_y(name, new_name) char *name;
char **new_name;
{
  char *result;
...
static void replace_character(name, old, new) char *name;
char old;
char new;
{
  char *p;
