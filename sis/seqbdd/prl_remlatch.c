...
void Prl_RemoveLatches(network_t *network, prl_options_t *options) {
    seq_info_t *seq_info;
...
static bdd_t *GetReachableStatesFromDc(seq_info_t *seq_info, prl_options_t *options) {
    bdd_t *reachable_states;
...
static void
FirstFitLatchRemoval(network_t *network, seq_info_t *seq_info, bdd_t *valid_states, prl_options_t *options) {
    int          i;
    bdd_t        *set;
    bdd_t        *var;
    node_t       *pi;
...
static void extract_var_info(extra_vars_t *extra_vars, bdd_t *var, bdd_t *set, var_info_t *var_info) {
...
static st_table *compute_from_var_to_pi_table(network_t *network, seq_info_t *seq_info, array_t *input_names) {
    int      i, var_id;
    bdd_t    *var;
    node_t   *pi;
    char     *input_name;
...
static void RemoveOneLatch(network_t *network, array_t *input_names, st_table *from_var_to_pi, var_info_t *var_info) {
...
static void remove_unused_pis(network_t *network) {
    int    i;
    lsGen  gen;
    int    n_nodes;
    node_t *node;
...
static void replace_latch_by_node(network_t *network, latch_t *latch, node_t *node) {
    lsGen  gen;
    node_t *pi, *po, *fanin;
...
static void
get_good_support_fn(extra_vars_t *extra_vars, bdd_t *fn, bdd_t *dc, int *support_size, bdd_t **new_fn, bdd_t **new_dc) {
    int       i;
    bdd_t     *var;
    bdd_t     *derived_fn;
    bdd_t     *g, *h;
...
static extra_vars_t *extra_vars_alloc(seq_info_t *seq_info) {
    int          i;
    extra_vars_t *result;
...
static void extra_vars_free(extra_vars_t *extra_vars) {
...
static int get_output_distance(st_table *from_var_to_pi, bdd_t *var) {
    node_t   *pi;
    int      distance;
...
static int get_output_distance_rec(st_table *table, node_t *node) {
    node_t  *pi;
    lsGen   gen;
    node_t  *fanout;
    latch_t *latch;
...
static void PerformLocalRetiming(network_t *network, int verbosity) {
    int     done;
    int     i;
    node_t  *node;
    array_t *nodes;
...
static int save_enough_latches(network_t *network, node_t *node) {
    int    i;
    int    n_latches_saved;
    node_t *fanin;
...
static void move_latches_forward(network_t *network, node_t *node, int verbosity) {
    int      i;
    node_t   *po, *pi;
    lsGen    gen;
    node_t   *fanout;
...
static void red_latch_remove_latches(network_t *network, node_t *node, st_table *table) {
    st_generator *gen;
    latch_t      *latch;
    node_t       *input, *output, *fanin;

    st_foreach_item(table, gen, (char **) &latch, NIL(
            char *)) {
...
int Prl_LatchOutput(network_t *network, array_t *node_vec, int verbosity) {
    int    i;
    int    status;
    node_t *node;
...
static int force_latched_output(network_t *network, node_t *po, int verbosity) {
...
static node_t *get_candidate_node(node_t *node) {
    node_t   *candidate;
...
static node_t *get_candidate_node_rec(node_t *node, st_table *visited) {
    int    i;
    node_t *fanin;
    node_t *candidate;
...
static int node_only_fed_by_latches(node_t *node) {
    int       i;
    node_t    *fanin;
    network_t *network;
...
static void prl_print_bdd(bdd_t *fn, array_t *var_names) {
    network_t *network;
...
static void RemoveOneBootLatch(network_t *network, prl_options_t *options) {
    lsGen      gen;
    latch_t    *latch;
    node_t     *po, *fanin;
    ...
    foreach_latch(network, gen, latch) {
        ...
        switch (node_function(fanin)) {
            case NODE_0:input_value = 0;
                break;
            case NODE_1:input_value = 1;
                break;
            default:continue;
        }
...
static int CanFlipInitialBit(network_t *network, seq_info_t *seq_info, latch_t *latch) {
...
static bdd_t *BddFindAlternateInit(bdd_t *fn, bdd_t *literal, seq_info_t *seq_info) {
    bdd_t *tmp1, *tmp2;
...
static bdd_t *ComputeFlippedLiteral(network_t *network, seq_info_t *seq_info, latch_t *latch) {
    int    i;
    node_t *pi;
    bdd_t  *var, *literal;
...
static void ForceNewInitState(network_t *network, seq_info_t *seq_info, bdd_t *new_init_state) {
    int      i;
    latch_t  *latch;
    node_t   *pi, *po;
    bdd_t    *var;
...
static bdd_t *BinaryMergeBdds(bdd_t **bdd_array, int n_elts, BddFn bdd_fn, bdd_t *neutral_elt) {
...
static bdd_t *BddAndPositive(bdd_t *fn1, bdd_t *fn2) { return bdd_and(fn1, fn2, 1, 1); }
