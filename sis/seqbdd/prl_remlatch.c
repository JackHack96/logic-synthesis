 ...
 * 	that do not exist any more after latch removal), we destroy the dc_network
 * 	if there is one.
 *
 *----------------------------------------------------------------------
 */

void Prl_RemoveLatches(network, options)
network_t *network;
prl_options_t *options;
{
 ...
 /* remove logically redundant latches */
  ...
  if (network->dc_network != NIL(network_t) && bdd_is_tautology(reachable_states, 1)) {
...
static bdd_t *GetReachableStatesFromDc(seq_info, options)
seq_info_t *seq_info;
prl_options_t *options;
{
    bdd_t *reachable_states;
...
typedef struct {
  ...
  bdd_t *recoding_fn;		/* the combinational logic fn that can be substituted to the latch */
  bdd_t *recoding_fn_dc;	/* the don't care of that function (unreachable states) */
  bdd_t *new_set;		/* the set of reachable states after this var is removed (if red.) */
...
typedef struct {
    ...
    st_table *new_to_support_map;	/* int-->int; maps new_vars->id --> support_vars->id */
} extra_vars_t;
...
static void FirstFitLatchRemoval(network, seq_info, valid_states, options)
network_t *network;
seq_info_t *seq_info;
bdd_t *valid_states;
prl_options_t *options;
...
static void extract_var_info(extra_vars, var, set, var_info)
extra_vars_t *extra_vars;
bdd_t *var;
bdd_t *set;
var_info_t *var_info;
 ...
 *
 * 'from_var_to_pi' is a st_table mapping bdd_t * vars (corresponding to PIs of 'network')
 * onto the corresponding PIs in 'network'.
 *
 * Results:
 *	None.
 *
...
static void RemoveRedundantLatches(network, seq_info, removed_vars, options)
network_t *network;
seq_info_t *seq_info;
 ...
 *
 * NOTE: 'seq_info->network' != 'network' but the former is a copy of the latter.
 *
 *----------------------------------------------------------------------
 */

static st_table *compute_from_var_to_pi_table(network, seq_info, input_names)
network_t *network;
seq_info_t *seq_info;
array_t *input_names;
{
  ...
  for (i = 0; i < array_n(seq_info->present_state_vars); i++) {
    ...
    if (input_name == NIL(char)) continue;
...
static void RemoveOneLatch(network, input_names, from_var_to_pi, var_info)
network_t *network;
array_t *input_names;
st_table *from_var_to_pi;
var_info_t *var_info;
...
static void remove_unused_pis(network)
network_t *network;
{
    int i;
    lsGen gen;
...
static void replace_latch_by_node(network, latch, node)
network_t *network;
latch_t *latch;
node_t *node;
{
...
static void get_good_support_fn(extra_vars, fn, dc, support_size, new_fn, new_dc)
extra_vars_t *extra_vars;
bdd_t *fn;
bdd_t *dc;
int *support_size;
...
static void extra_vars_free(extra_vars)
extra_vars_t *extra_vars;
{
...
static int get_output_distance(from_var_to_pi, var)
st_table *from_var_to_pi;
bdd_t *var;
{
    node_t *pi;
...
static int get_output_distance_rec(table, node)
st_table *table;
node_t *node;
{
    node_t *pi;
...
static void PerformLocalRetiming(network, verbosity)
network_t *network;
int verbosity;
{
    int done;
...
static int save_enough_latches(network, node)
network_t *network;
node_t *node;
{
    int i;
...
static void move_latches_forward(network, node, verbosity)
network_t *network;
node_t *node;
int verbosity;
{
...
static void red_latch_remove_latches(network, node, table)
network_t *network;
node_t *node;
st_table *table;
{
...
int verbosity;
{
  ...
  for (i = 0; i < array_n(node_vec); i++) {
    ...
    if (status == 1) {
      (void) fprintf(siserr, "Combinational logic dependency detected for node \"%s\":", node->name);
...
static int node_only_fed_by_latches(node)
node_t *node;
{
    int i;
    node_t *fanin;
...
static void prl_print_bdd(fn, var_names)
bdd_t *fn;
array_t *var_names;
{
    network_t *network;
...
static void RemoveOneBootLatch(network, options)
network_t *network;
prl_options_t *options;
{
    lsGen gen;
...
static int CanFlipInitialBit(network, seq_info, latch)
network_t *network;
seq_info_t *seq_info;
latch_t *latch;
{
...
static bdd_t *BddFindAlternateInit(fn, literal, seq_info)
bdd_t *fn;
bdd_t *literal;
seq_info_t *seq_info;
{
  ...
  tmp1 = bdd_and_smooth(fn, seq_info->init_state_fn, seq_info->present_state_vars);
...
static bdd_t *ComputeFlippedLiteral(network, seq_info, latch)
network_t *network;
seq_info_t *seq_info;
latch_t *latch;
{
  ...
  for (i = 0; i < array_n(seq_info->input_nodes); i++) {
    if (pi == array_fetch(node_t *, seq_info->input_nodes, i)) break;
  }
...
static void ForceNewInitState(network, seq_info, new_init_state)
network_t *network;
seq_info_t *seq_info;
bdd_t *new_init_state;
{
...
static bdd_t *BinaryMergeBdds(bdd_array, n_elts, bdd_fn, neutral_elt)
bdd_t **bdd_array;
int n_elts;
BddFn bdd_fn;
bdd_t *neutral_elt;
{
  ...
  if (n_elts == 0) return neutral_elt;
  bdd_free(neutral_elt);
  for (;;) {
    if (n_elts == 1) return bdd_array[0];
    arg1 = &bdd_array[0];
    arg2 = &bdd_array[1];
    result = &bdd_array[0];
...
static bdd_t *BddAndPositive(fn1, fn2)
bdd_t *fn1;
bdd_t *fn2;
{
