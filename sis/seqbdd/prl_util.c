...
void Prl_RemoveDcNetwork(network)
network_t *network;
{
...
void Prl_SetupCopyFields(to_network, from_network)
...
char* Prl_DisambiguateName(network, name, node)
network_t *network;
char* name;
node_t *node;
{
 ...
 * 'node' is a node from some other network whose tfi is to be copied in 'network'.
 * The 'copy' fields of the nodes in 'node'->network are used to keep track
 * of what has been already copied. The 'copy' fields of the PIs of 'node'->network
 * must have been initialized properly already (i.e.: 'CopySubnetwork'
 * should only be called after 'Prl_SetupCopyFields').
 *
 * Results:
...
node_t *Prl_CopySubnetwork(network, node)
network_t *network;
node_t *node;
{
    int i;
...
void Prl_StoreAsSingleOutputDcNetwork(network, dc_network)
network_t *network;
...
void Prl_FreeBddArray(bdd_array)
array_t *bdd_array;
{
  int i;
  bdd_t *fn;
...
void Prl_CleanupDcNetwork(network)
network_t *network;
{
    int i;
    lsGen gen;
...
void Prl_ReportElapsedTime(options, comment)
...
static bdd_t *array_bdd_and(manager, array)
bdd_manager* manager;
array_t *array;
{
    int i;
...
bdd_t     **input;
{
 ...
 /* precompute the vars to be used */
  ...
  if (state != NIL(bdd_t *)) *state = bdd_smooth(minterm, seq_info->external_input_vars);
  if (input != NIL(bdd_t *)) *input = bdd_smooth(minterm, seq_info->present_state_vars);
}
...
st_table *Prl_GetPiToVarTable(seq_info)
seq_info_t *seq_info;
{
    int i;
    node_t *pi;
...
static void StoreVarIdsInTable(vars, table)
array_t *vars;
st_table *table;
{
    int i;
