...
static int fn_info_cmp(char *obj1, char *obj2) {
    int       diff;
...
static void fn_info_print(char *obj) {
...
/* keep everything separate: transition relation and the output fn in case of
 * verif */

/* ARGSUSED */
range_data_t *product_alloc_range_data(network_t *network, verif_options_t *options) {
    int           i;
    node_t        *node;
    bdd_t         *output;
    ...
    data->smoothing_inputs =
            bdd_extract_var_array(data->manager, info->org_pi, info->pi_ordering);

    /* create and save two arrays of bdd variables in order: yi's and their
     * corresponding xi's */
    input_to_output_table = extract_input_to_output_table(
            info->org_pi, info->new_pi, info->po_ordering, network);
    data->input_vars  = extract_state_input_vars(data->manager, info->pi_ordering,
                                                 input_to_output_table);
    data->output_vars = extract_state_output_vars(
            data->manager, info->pi_ordering, input_to_output_table);
...
void product_free_range_data(range_data_t *data, verif_options_t *options) {
    int           i;
    node_t        *node;
    output_info_t *info = options->output_info;
...
bdd_t *product_compute_next_states(bdd_t *current_set, range_data_t *data, verif_options_t *options) {
    bdd_t *result_as_output = bdd_incr_and_smooth(
            data->transition_outputs, current_set, data->output_vars, options);
    bdd_t *new_current_set  =
                  bdd_substitute(result_as_output, data->output_vars, data->input_vars);

    if (options->verbose >= 3) {
        (void) fprintf(misout, "(%d->%d)", bdd_size(result_as_output),
...
bdd_t *product_compute_reverse_image(bdd_t *next_set, range_data_t *data, verif_options_t *options) {
    int     i;
    node_t  *node;
    ...
    result =
            bdd_incr_and_smooth(data->transition_outputs, next_set_as_next_state_vars,
                                ps_and_pi_array, options);
...
bdd_t *
bdd_incr_and_smooth(array_t *transition_outputs, bdd_t *current_set, array_t *keep_vars, verif_options_t *options) {
    int            i;
    int            n_fns;
    bdd_t          *tmp;
    bdd_t          **fns;
...
int product_check_output(bdd_t *current_set, range_data_t *data, int *output_index, verif_options_t *options) {
    int   i;
    bdd_t *output;
    ...
    for (i = 0; i < array_n(data->external_outputs); i++) {
        ...
        if (!bdd_leq(current_set, output, 1, 1)) {
            report_inconsistency(current_set, output,
...
void product_bdd_sizes(range_data_t *data, int *fn_size, int *output_size) {
    int   i;
    bdd_t *output;
...
static support_info_t *support_info_extract(array_t *keep_vars, int n_fns, bdd_t **fns, verif_options_t *options) {
    int            i;
...
static void support_info_free(support_info_t *support_info) {
    int i;
...
static void extract_fn_info(support_info_t *support_info, bdd_t **fns, verif_options_t *options) {
    int       i;
    fn_info_t *info;
...
static void fn_info_free(fn_info_t *info) {
...
static void extract_var_info(support_info_t *support_info, st_table *skip_varids) {
    int        i;
    int        varid;
    var_info_t *info;
...
static void *var_info_free(var_info_t *info) {
    bdd_free(info->var);
...
static void smooth_lonely_variables(support_info_t *support_info, verif_options_t *options) {
    int     varid;
    int     index;
    bdd_t   *tmp;
    int     remaining;
...
static bdd_t *do_fn_merging(support_info_t *support_info, verif_options_t *options) {
    int       i;
    bdd_t     *tmp;
    fn_info_t *fn0, *fn1;
    array_t   *smooth_vars;
...
static array_t *smooth_vars_extract(support_info_t *support_info, fn_info_t *fn0, fn_info_t *fn1) {
    int        i;
    int        remaining;
    array_t    *result;
    int        is_in_index0;
...
static st_table *get_varids_in_table(array_t *bdd_array) {
    int      i;
    int      varid;
...
static void initialize_partition_info(support_info_t *support_info) {
    int      n_partitions;
    int      partition;
    int      i, n, x;
    int      count;
    ...
    n_partitions = 0;
    for (i       = 0; i < support_info->n_fns; i++) {
        if (st_lookup(table, (char *) support_info->fns[i]->partition,
                      (char **) &partition)) {
