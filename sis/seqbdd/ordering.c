...
array_t *find_best_set_order(set_info_t *info, verif_options_t *options) {
    int       cost;
    ...
    if (options->ordering_depth >= 0) {
        cost = do_find_best_set_order(info, key, cache, options->ordering_depth,
                                      LARGE_NUMBER);
...
array_t *find_greedy_set_order(set_info_t *info) {
    int       i;
    int       size;
    int       best_next_index;
    array_t   *result        = array_alloc(int, 0);
