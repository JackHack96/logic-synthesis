...
void extract_network_info(network_t *network, verif_options_t *options) {
    char          *name;
    node_t        *node;
    ...
    switch (options->type) {
        case BULL_METHOD:output_info->po_ordering = get_po_ordering(network, next_state_po, options);
            array_free(next_state_po);
            break;
        case CONSISTENCY_METHOD:output_info->po_ordering = get_po_ordering(network, next_state_po, options);
            array_free(next_state_po);
...
void extract_product_network_info(network_t *network1, network_t *network2, verif_options_t *options) {
    if (options->verbose >= 1)
...
void compute_product_network(network_t *network1, network_t *network2, output_info_t *output_info) {
    lsGen        gen;
    st_generator *sgen;
    char         *name;
    node_t       *output, *n1, *n2;
    ...
    st_foreach_item(output_table, sgen, &name, NIL(
            char *)) { FREE(name); }
...
static void
allocate_and_rename_primary_inputs(network_t *network1, network_t *network2, st_table *input_table, char *postfix) {
    lsGen  gen;
...
static void rename_primary_outputs(network_t *network, st_table *output_table, char *postfix) {
    lsGen  gen;
    node_t *output;
    int    postfix_length = (int) strlen(postfix) + 1;
...
static void rename_internal_nodes(network_t *network, char *postfix) {
    char   *new_name;
    lsGen  gen;
    node_t *node;
    int    postfix_length = (int) strlen(postfix) + 1;
...
static void hack_rename_internal_nodes(network_t *n1, network_t *n2, char *postfix) {
    char   *new_name;
    lsGen  gen;
    node_t *node, *node2;
    int    postfix_length = (int) strlen(postfix) + 1;
...
static void
merge_io_outputs(network_t *network1, network_t *network2, st_table *output_table, output_info_t *output_info) {
    int     i;
    lsGen   gen;
...
array_t *network_extract_next_state_po(network_t *network) {
    lsGen   gen;
    node_t  *n1;
    latch_t *l;
    array_t *result;
...
static array_t *network_extract_extern_pi(network_t *network) {
    lsGen   gen;
    node_t  *input;
    array_t *result = array_alloc(node_t *, 0);
...
node_t *copy_init_state_constraint(network_t *network) {
    lsGen   gen;
    node_t  *init_state;
    node_t  *n1, *n2, *n3;
    latch_t *l;
...
array_t *create_new_pi(network_t *network, array_t *outputs) {
    int     i;
...
node_t *
build_equivalence_node(network_t *network, array_t *nodes1, array_t *nodes2, char *output_name, array_t *xnor_array) {
    int    i;
...
static void extract_product_pi_info(network_t *network, output_info_t *output_info, char *name) {
    int     i;
    node_t  *node;
...
static void remember_primary_output_names(network_t *network, output_info_t *info, int value) {
    lsGen  gen;
    node_t *output;
...
static void extract_local_pipo(output_info_t *info, array_t *local_pi, array_t *local_po, int netid) {
    int    i;
    int    id;
    node_t *pi, *po;
