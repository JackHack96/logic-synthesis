...
/* ARGSUSED */
static void timeout_handle(int x) { longjmp(timeout_env, 1); }
...
static int com_verify_fsm(network_t **network, int argc, char **argv) {
    int             status;
    ...
    if (bdd_range_fill_options(&options, &argc, &argv))
        goto usage;
    if (options.alloc_range_data == 0)
        goto usage;
...
static int com_extract_seq_dc(network_t **network, int argc, char **argv) {
...
static int bdd_range_fill_method(verif_options_t *options, char *method_name) {
...
int bdd_range_fill_options(verif_options_t *options, int *argc_addr, char ***argv_addr) {
    int  c;
    int  argc   = *argc_addr;
    char **argv = *argv_addr;
    char *s;
    ...
    while ((c = util_getopt(argc, argv, "i:m:o:O:t:v:s:V")) != EOF) {
        switch (c) {
            case 'i':options->n_iter = atoi(util_optarg);
                if (options->n_iter<0 || options->n_iter>INFINITY)
                    return 1;
                break;
            case 'm':
                if (bdd_range_fill_method(options, util_optarg) == 0)
                    return 1;
                break;
            case 'o':options->ordering_depth = atoi(util_optarg);
                break;
            case 'O':options->use_manual_order = 1;
                options->order_network_name    = util_strsav(util_optarg);
                break;
            case 't':options->timeout = atoi(util_optarg);
                if (options->timeout < 0 || options->timeout > 3600 * 24 * 365)
                    return 1;
                break;
            case 'v':options->verbose = atoi(util_optarg);
                break;
            case 's':options->sim_file = util_strsav(util_optarg);
                break;
            case 'V':options->stop_if_verify = 1;
                break;
            default:return 1;
        }
    }
    ...
    if (options->use_manual_order) {
        options->order_network =
                read_optional_network("read_eqn", options->order_network_name);
...
static void seqbdd_print_usage(char *name, char *uniq_options) {
    (void) fprintf(siserr, "usage: %s [-o d] [-t s] [-v n] [-V] -m method %s\n",
                   name, uniq_options);
    (void) fprintf(siserr, "    [-o d]: depth of search for good variable "
                           "ordering; d == 0 greedy\n");
    (void) fprintf(siserr, "    [-t s]: time out after s seconds of cpu time\n");
    (void) fprintf(siserr, "    [-v n]: verbosity level\n");
    (void) fprintf(siserr,
                   "    [-V]: returns an error status if verification succeeds\n");
    (void) fprintf(
            siserr,
            "    [-s filename]: save a distinguishing input vector to filename\n");
    (void) fprintf(siserr, "    method is one of: ");
#define use(NAME, STRING, UPPERCASE) (void)fprintf(siserr, "%s ", STRING);
...
static network_t *read_optional_network(char *command, char *filename) {
    int       status;
    char      *buffer;
    network_t *result;
...
static int com_extract_env_seq_dc(network_t **network, int argc, char **argv) {
    int           status;
    prl_options_t options;
    network_t     *fsm_network, *env_network;

    if (Prl_FillOptions(&options, &argc, &argv))
        goto usage;

    fsm_network = *network;
    if (fsm_network == NIL(network_t))
        return 0;
    if (network_num_internal(fsm_network) == 0)
        return 0;
    if (network_num_latch(fsm_network) == 0)
        return 0;
    ...
    if (argc - util_optind == 0) {
        ...
        if (env_network == NIL(network_t)) {
            (void) fprintf(siserr, "Warning: can't read network \"%s\";",
                           argv[util_optind]);
        }
    } else {
        goto usage;
...
static int com_env_verify_fsm(network_t **network, int argc, char **argv) {
    int           status;
    prl_options_t options;
...
static int com_equiv_nets(network_t **network, int argc, char **argv) {
    prl_options_t options;
...
static int com_remove_latches(network_t **network, int argc, char **argv) {
    prl_options_t options;
...
static int com_latch_output(network_t **network, int argc, char **argv) {
    int     c;
    array_t *node_vec;
    int     status    = 0;
    int     verbosity = 0;
    ...
    while ((c = util_getopt(argc, argv, "v:")) != EOF) {
        switch (c) {
            case 'v':verbosity = atoi(util_optarg);
                break;
            default:goto usage;
        }
    }
    node_vec  = com_get_true_nodes(*network, argc - util_optind + 1,
                                   argv + util_optind - 1);
...
static int com_remove_dependencies(network_t **network, int argc, char **argv) {
    int             c;
    array_t         *node_vec;
    ...
    while ((c = util_getopt(argc, argv, "opv:")) != EOF) {
        switch (c) {
            case 'o':options.insert_a_one = 1;
                break;
            case 'p':options.perform_check = 1;
                break;
            case 'v':options.verbosity = atoi(util_optarg);
                break;
            default:goto usage;
        }
    }
    ...
    usage:
    (void) fprintf(siserr,
                   "remove_dep [-o] [-v verbosity_level] <input> <output list>\n");
    /*  (void) fprintf(siserr, "\t[-p] perform dependency check (not
     * implemented)\n"); */
...
static int com_free_dc(network_t **network, int argc, char **argv) {
...
static int Prl_FillOptions(prl_options_t *options, int *argc_addr, char ***argv_addr) {
    int  c;
    int  argc                   = *argc_addr;
    char **argv                 = *argv_addr;
    ...
    while ((c = util_getopt(argc, argv, "o:p:f:t:v:l:riV")) != EOF) {
        switch (c) {
            ...
            case 'l':options->remlatch.max_level = atoi(util_optarg);
                if (options->remlatch.max_level <= 1)
                    return 1;
                break;
            case 'f':options->remlatch.max_cost = atoi(util_optarg);
                if (options->remlatch.max_cost < 1)
                    return 1;
                break;
            case 'r':options->remlatch.local_retiming = 0;
                break;
            case 'i':options->remlatch.remove_boot = 0;
                break;
            case 'V':options->stop_if_verify = 1;
                break;
            default:return 1;
...
static void Prl_PrintUsage(char *name, char *msg) {
    (void) fprintf(
            siserr,
            "usage: %s [-o d] [-t s] [-v n] [-l m] [-f n] [-r] [-b] [-V] %s\n", name,
            msg);
    (void) fprintf(siserr, "---- options for: env_seq_dc, remove_latches, "
                           "equiv_nets, env_verify_fsm ----\n");
    (void) fprintf(siserr,
                   "	[-o d]: depth of search for good variable ordering;");
    (void) fprintf(siserr, " d == 0 means greedy; default is d = 1\n");
    (void) fprintf(siserr, "	[-t s]: time out after s seconds of cpu "
                           "time\n");
    (void) fprintf(siserr, "	[-v n]: verbosity level\n");
    (void) fprintf(siserr, "---- options specific to remove_latches ----\n");
    (void) fprintf(
            siserr,
            "	[-f n]: remove a latch only if fanin of its recoding fn is <= n\n");
    (void) fprintf(
            siserr,
            "	[-l n]: remove a latch only if its recoding fn has a depth <= n\n");
    (void) fprintf(
            siserr, "	[-r]:   do not perform local retiming before remove latches\n");
    (void) fprintf(siserr,
                   "	[-i]:   do not attempt to remove boot latches\n");
    (void) fprintf(siserr, "---- options specific to env_verify_fsm ----\n");
    (void) fprintf(
            siserr, "	[-V]:   returns an error status if verification succeeds\n");
}
