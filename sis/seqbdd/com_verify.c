...
/* ARGSUSED */
static void timeout_handle(x) int x;
...
static int com_verify_fsm(network, argc, argv) network_t **network;
int argc;
char **argv;
{
  int status;
...
static int com_extract_seq_dc(network, argc, argv) network_t **network;
int argc;
char **argv;
{
...
static int bdd_range_fill_method(options, method_name) verif_options_t *options;
char *method_name;
{
...
int bdd_range_fill_options(options, argc_addr,
                           argv_addr) verif_options_t *options;
int *argc_addr;
char ***argv_addr;
{
  int c;
...
static void seqbdd_print_usage(name, uniq_options) char *name;
char *uniq_options;
{
...
#define use(NAME, STRING, UPPERCASE) (void)fprintf(siserr, "%s ", STRING);
  use(consistency, "consistency", CONSISTENCY) use(bull, "bull", BULL)
      use(product, "product", PRODUCT)
#undef use
          (void) fprintf(siserr, "\n");
}

static network_t *read_optional_network(command, filename) char *command;
char *filename;
{
  int status;
  char *buffer;
...
static int com_extract_env_seq_dc(network, argc, argv) network_t **network;
int argc;
char **argv;
{
  int status;
...
static int com_env_verify_fsm(network, argc, argv) network_t **network;
int argc;
char **argv;
{
  int status;
...
static int com_equiv_nets(network, argc, argv) network_t **network;
int argc;
char **argv;
{
  prl_options_t options;
...
static int com_remove_latches(network, argc, argv) network_t **network;
int argc;
char **argv;
{
  prl_options_t options;
...
static int com_latch_output(network, argc, argv) network_t **network;
int argc;
char **argv;
{
  int c;
...
static int com_remove_dependencies(network, argc, argv) network_t **network;
int argc;
char **argv;
{
  int c;
...
static int com_free_dc(network, argc, argv) network_t **network;
int argc;
char **argv;
{
...
static int Prl_FillOptions(options, argc_addr,
                           argv_addr) prl_options_t *options;
int *argc_addr;
char ***argv_addr;
{
  int c;
...
static void Prl_PrintUsage(name, msg) char *name;
char *msg;
{
  (void)fprintf(
      siserr,
