...
void eliminate(network_t *network, int thresh, int limit) {
    array_t  *nodevec;
    int      i, j;
                ...
                if (!st_lookup(dont_elim, (char *) np, &dummy)) {
                    foreach_fanout(np, gen, fanout) {
                        if (node_function(fanout) == NODE_PO) {
                            elimok = FALSE;
                            (void) st_insert(dont_elim, (char *) np, (char *) 1);
                            (void) lsFinish(gen);
                            break;
                        }
                        ...
                        foreach_fanout(np, gen, fanout) {
                            if (node_collapse(fanout, np)) {
                                ...
                                foreach_fanin(fanout, j, fanin) {
                                    (void) st_delete(dont_elim, (char **) &fanin, &dummy);
                                }
                                (void) st_delete(dont_elim, (char **) &fanout, &dummy);
                            }
                        }
                    }
                }
...
static double clp_est(node_t *fanout, node_t *np) {
    double cmp_size;

    switch (node_input_phase(fanout, np)) {
        case PHASE_UNKNOWN:return node_num_cube(fanout);
        case NEG_UNATE:
        case BINATE:cmp_size = cmp_est((ft_t *) np->factored);
            break;
        default:
...
static double cov_est(node_t *fanout, node_t *np, ft_t *root, double cmp_size) {
    switch (root->type) {
        case FACTOR_LEAF:return (cov_leaf_est(fanout, np, root, cmp_size));
        case FACTOR_INV:return (cov_inv_est(fanout, np, root, cmp_size));
        case FACTOR_AND:return (cov_and_est(fanout, np, root, cmp_size));
        case FACTOR_OR:return (cov_or_est(fanout, np, root, cmp_size));
        default:
            /*    shouldn't happen    */
            return 0;
    }
...
static double cmp_est(ft_t *f) {
    double cmp_size;
    ft_t   *p;

    switch (f->type) {
        case FACTOR_1:return 0;
        case FACTOR_0:
        case FACTOR_LEAF:
        case FACTOR_INV:return 1;
        case FACTOR_AND:cmp_size = 0;
            for (p = f->next_level; p != NIL(ft_t); p = p->same_level) {
                cmp_size += cmp_est(p);
            }
            break;
        case FACTOR_OR:cmp_size = 1;
            for (p = f->next_level; p != NIL(ft_t); p = p->same_level) {
                cmp_size *= cmp_est(p);
            }
            break;
...
static double cov_inv_est(node_t *fanout, node_t *np, ft_t *root, double cmp_size) {
...
/*ARGSUSED*/
static double cov_leaf_est(node_t *fanout, node_t *np, ft_t *root, double cmp_size) {
...
static double cov_and_est(node_t *fanout, node_t *np, ft_t *root, double cmp_size) {
    ...
    for (p   = root->next_level; p != NIL(ft_t); p = p->same_level) {
        switch (p->type) {
            case FACTOR_LEAF:sub_cover = cov_leaf_est(fanout, np, p, cmp_size);
                break;
            case FACTOR_INV:sub_cover = cov_inv_est(fanout, np, p, cmp_size);
                break;
            case FACTOR_OR:sub_cover = cov_or_est(fanout, np, p, cmp_size);
                break;
            case FACTOR_AND:sub_cover = cov_and_est(fanout, np, p, cmp_size);
                break;
            default:sub_cover = 0; /*    shouldn't happen */
        }
        cov_size *= sub_cover;
    }
    return cov_size;
...
static double cov_or_est(node_t *fanout, node_t *np, ft_t *root, double cmp_size) {
    double cov_size, sub_cover;
    ft_t   *p;
    ...
    for (p   = root->next_level; p != NIL(ft_t); p = p->same_level) {
        switch (p->type) {
            ...
            case FACTOR_AND:sub_cover = cov_and_est(fanout, np, p, cmp_size);
                break;
            default:sub_cover = 1; /*    shouldn't happen */
        }
        cov_size += sub_cover;
    }
    return cov_size;
