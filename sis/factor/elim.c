...
void eliminate(network, thresh, limit) network_t *network;
int thresh, limit;
{
  array_t *nodevec;
  int i, j;
...
static double clp_est(fanout, np) node_t *fanout, *np;
{
  double cmp_size;
...
static double cov_est(fanout, np, root, cmp_size) node_t *fanout, *np;
ft_t *root;
double cmp_size;
{
  switch (root->type) {
...
static double cmp_est(f) ft_t *f;
{
  double cmp_size;
  ft_t *p;
...
static double cov_inv_est(fanout, np, root, cmp_size) node_t *fanout, *np;
ft_t *root;
double cmp_size;
{
...
/*ARGSUSED*/
static double cov_leaf_est(fanout, np, root, cmp_size) node_t *fanout, *np;
ft_t *root;
double cmp_size;
{
...
static double cov_and_est(fanout, np, root, cmp_size) node_t *fanout, *np;
ft_t *root;
double cmp_size;
{
  ...
  for (p = root->next_level; p != NIL(ft_t); p = p->same_level) {
    switch (p->type) {
    case FACTOR_LEAF:
      sub_cover = cov_leaf_est(fanout, np, p, cmp_size);
      break;
    case FACTOR_INV:
      sub_cover = cov_inv_est(fanout, np, p, cmp_size);
      break;
    case FACTOR_OR:
      sub_cover = cov_or_est(fanout, np, p, cmp_size);
      break;
    case FACTOR_AND:
      sub_cover = cov_and_est(fanout, np, p, cmp_size);
      break;
    default:
...
static double cov_or_est(fanout, np, root, cmp_size) node_t *fanout, *np;
ft_t *root;
double cmp_size;
{
  double cov_size, sub_cover;
