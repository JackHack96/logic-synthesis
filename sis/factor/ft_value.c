 ...
 */
int node_value(nodep) node_t *nodep;
{
...
int factor_num_used(o, i) node_t *o, *i;
{
  stat_t s;
...
/* ARGSUSED */
static int factor_count(r, f, stat) node_t *r;
ft_t *f;
char *stat;
{
  stat_t *s;
...
int factor_num_literal(f) node_t *f;
{
  int sum = 0;
...
/* ARGSUSED */
static int literal_sum(r, np, sum) node_t *r;
ft_t *np;
char *sum;
{
  int *s;
...
void value_print(fp, np) FILE *fp;
node_t *np;
{
  int value;
...
int value_cmp_inc(p1, p2) char **p1, **p2;
{ return node_value((node_t *)*p1) - node_value((node_t *)*p2); }

int value_cmp_dec(p1, p2) char **p1, **p2;
