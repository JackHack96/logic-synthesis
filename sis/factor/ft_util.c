...
void factor_alloc(node_t *f) { f->factored = NIL(char); }

void factor_free(node_t *f) {
...
void factor_invalid(node_t *f) { factor_free(f); }

void factor_dup(node_t *old, node_t *new) {
    factor_free(new);
    new->factored = (char *) ft_dup((ft_t *) old->factored);
}

static void ft_free(ft_t *root) {
    if (root != NIL(ft_t)) {
...
static ft_t *ft_new(factor_type ft_type, int ft_index, int ft_len, ft_t *ft_same, ft_t *ft_next) {
    ft_t *p;
    p = ALLOC(ft_t, 1);
    p->type       = ft_type;
    p->index      = ft_index;
...
void factor_traverse(node_t *f, int (*func)(), char *stat, trav_order order) {
    factor(f);
    (void) ft_traverse_recur(f, (ft_t *) f->factored, func, stat, order);
}

int ft_traverse_recur(node_t *f, ft_t *root, int (*func)(), char *stat, trav_order order) {
 ...
 */
ft_t *factor_nt_to_ft(node_t *f, node_t *nt) {
    switch (node_function(nt)) {
        case NODE_0:return ft_new(FACTOR_0, -1, 0, NIL(ft_t), NIL(ft_t));
        case NODE_1:return ft_new(FACTOR_1, -1, 0, NIL(ft_t), NIL(ft_t));
        default:return node_to_ft(f, nt);
    }
}

static ft_t *node_to_ft(node_t *f, node_t *np) {
    node_cube_t    cube;
    ...
    for (i = node_num_cube(np) - 1; i >= 0; i--) {
        ...
        foreach_fanin(np, j, fanin) {
            ...
            switch (literal) {
                case ZERO:temp = node_to_ft(f, fanin);
                    ...
                    break;
                case ONE:lit = node_to_ft(f, fanin);
                    if (cur_and == NIL(ft_t)) {
                        and = cur_and = lit;
                    } else {
                        cur_and->same_level = lit;
...
void factor_nt_free(node_t *nodep) {
    int    i;
    node_t *np;

    if (nodep->type == UNASSIGNED) {
        foreach_fanin(nodep, i, np) { factor_nt_free(np); }
        node_free(nodep);
    }
}
 ...
 */
node_t *factor_best_literal(node_t *f, node_t *c) {
    ...
    switch (phase) {
        case 0:return node_literal(bp, 0);
        case 1:return node_literal(bp, 1);
        default:(void) fprintf(miserr, "Error: internal error in best_literal\n");
            return NIL(node_t);
    }
}

node_t *factor_quick_kernel(node_t *f) { return ex_find_divisor_quick(f); }

node_t *factor_best_kernel(node_t *f) { return ex_find_divisor(f, 1, 1); }
...
array_t *factor_to_nodes(node_t *f) {
    node_t  *r;
    array_t *fa;
...
static array_t *ft_conv(node_t *nf, ft_t *f, node_t **root) {
    array_t *fa;

    switch (f->type) {
        case FACTOR_0:fa = array_alloc(node_t *, 0);
            *root = node_constant(0);
            array_insert_last(node_t *, fa, *root);
            break;
        case FACTOR_1:fa = array_alloc(node_t *, 0);
            *root = node_constant(1);
            array_insert_last(node_t *, fa, *root);
            break;
        case FACTOR_AND:fa = ft_and_conv(nf, f, root);
            break;
        case FACTOR_OR:fa = ft_or_conv(nf, f, root);
            break;
        case FACTOR_INV:fa = ft_inv_conv(nf, f, root);
            break;
        case FACTOR_LEAF:fa = array_alloc(node_t *, 0);
            *root = node_get_fanin(nf, f->index);
            break;
        default: fail("factor_to_nodes: wrong node type");
            exit(-1);
    }
...
static array_t *ft_or_conv(node_t *nf, ft_t *f, node_t **root) {
    array_t *fa, *tfa;
...
static array_t *ft_and_conv(node_t *nf, ft_t *f, node_t **root) {
    array_t *fa, *tfa;
...
static array_t *ft_inv_conv(node_t *nf, ft_t *f, node_t **root) {
    array_t *fa, *t;
    node_t  *r;
