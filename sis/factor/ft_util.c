...
void factor_alloc(f) node_t *f;
{ f->factored = NIL(char); }

void factor_free(f) node_t *f;
{
...
void factor_invalid(f) node_t *f;
{ factor_free(f); }

void factor_dup(old, new) node_t *old, *new;
{
  factor_free(new);
  new->factored = (char *)ft_dup((ft_t *)old->factored);
}

static void ft_free(root) ft_t *root;
{
  if (root != NIL(ft_t)) {
...
static ft_t *ft_new(ft_type, ft_index, ft_len, ft_same,
                    ft_next) factor_type ft_type;
int ft_index;
int ft_len;
ft_t *ft_same;
ft_t *ft_next;
...
void factor_traverse(f, func, stat, order) node_t *f;
int (*func)();
char *stat;
trav_order order;
{
...
int ft_traverse_recur(f, root, func, stat, order) node_t *f;
ft_t *root;
int (*func)();
char *stat;
trav_order order;
 ...
 */
ft_t *factor_nt_to_ft(f, nt) node_t *f; /* original node */
...
static ft_t *node_to_ft(f, np) node_t *f;
node_t *np;
{
  node_cube_t cube;
...
void factor_nt_free(nodep) node_t *nodep;
{
  int i;
  node_t *np;
 ...
 */
node_t *factor_best_literal(f, c) node_t *f, *c;
{
...
node_t *factor_quick_kernel(f) node_t *f;
{ return ex_find_divisor_quick(f); }

node_t *factor_best_kernel(f) node_t *f;
...
array_t *factor_to_nodes(f) node_t *f;
{
  node_t *r;
  array_t *fa;
...
static array_t *ft_conv(nf, f, root) node_t *nf;
ft_t *f;
node_t **root;
{
  array_t *fa;
...
static array_t *ft_or_conv(nf, f, root) node_t *nf;
ft_t *f;
node_t **root;
{
  array_t *fa, *tfa;
...
static array_t *ft_and_conv(nf, f, root) node_t *nf;
ft_t *f;
node_t **root;
{
  array_t *fa, *tfa;
...
static array_t *ft_inv_conv(nf, f, root) node_t *nf;
ft_t *f;
node_t **root;
{
  array_t *fa, *t;
