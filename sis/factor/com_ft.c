...
int com_factor(network, argc, argv) network_t **network;
int argc;
char **argv;
{
  int c, method;
  ...
  for (i = 0; i < array_n(nodevec); i++) {
    ...
    if (np->type == INTERNAL) {
      ...
      case 1:
        ...
        break;
      case 2:
        (void)fprintf(misout, "boolean factoring is not available\n");
        array_free(nodevec);
        return 1;
      }
    }
...
int com_pf(network, argc, argv) network_t **network;
int argc;
char **argv;
{
  array_t *nodevec;
...
int com_pv(network, argc, argv) network_t **network;
int argc;
char **argv;
{
  array_t *nodevec;
  ...
  while ((c = util_getopt(argc, argv, "p:ad")) != EOF) {
    ...
    case 'd':
      ...
      break;
    default:
      (void)fprintf(misout, "usage: print_value [-pr] [node-list]\n");
  ...
  switch (order) {
  case PV_ASCENDING:
    array_sort(nodevec, value_cmp_inc);
    break;
  case PV_DESCENDING:
    array_sort(nodevec, value_cmp_dec);
    break;
  case PV_ANY:
    break;
  }
...
int com_pft(network, argc, argv) network_t **network;
int argc;
char **argv;
{
  array_t *nodevec;
...
static char *my_node_long_name(node) node_t *node;
{
...
int com_factor_conv(network, argc, argv) network_t **network;
int argc;
char **argv;
{
  array_t *nodevec, *fa;
  ...
  for (i = 0; i < array_n(nodevec); i++) {
    ...
    if (f->type == INTERNAL) {
      ...
      for (j = 0; j < array_n(fa); j++) {
        ...
        case NODE_AND:
          (void)fprintf(misout, "\t%s = AND(", my_node_long_name(np));
          foreach_fanin(np, k, fanin) {
            (void)fprintf(misout, " %s", my_node_long_name(fanin));
          }
          (void)fprintf(misout, ")\n");
          break;
        case NODE_OR:
          (void)fprintf(misout, "\t%s = OR(", my_node_long_name(np));
          foreach_fanin(np, k, fanin) {
            (void)fprintf(misout, " %s", my_node_long_name(fanin));
          }
          (void)fprintf(misout, ")\n");
        ...
        case NODE_INV:
          ...
          break;
        case NODE_0:
          (void)fprintf(misout, "\t%s = 0\n", my_node_long_name(np));
          break;
        case NODE_1:
          (void)fprintf(misout, "\t%s = 1\n", my_node_long_name(np));
          break;
        default:
          (void)fprintf(misout, "illegal node function\n");
          return 1;
        }
        node_free(np);
      }
...
int com_eliminate(network, argc, argv) network_t **network;
int argc;
char **argv;
{
  int limit, value;
...
static int el_valid_val(s) char *s;
{
  int i, first;
...
static node_t *first_fanin(np) node_t *np;
{
  node_t *fanin, *first;
  int i;
