...
pcover minimize(pcover F, pcover D, int type) {
...
pcover nocomp(pcover F, pcover D1) {
    ...
    if (unwrap_onset && (cube.part_size[cube.num_vars - 1] > 1) &&
        ...
        (cost.out < 5000)) EXEC(F = sf_contain(unravel(F, cube.num_vars - 1)), "SETUP      ", F);
    ...
    if (!single_expand) {
        ...
        do {
            ...
            do {
                ...
                EXECUTE(F = ncp_expand(F, Fold, FALSE), EXPAND_TIME, F, cost);
        ...
        /* Save the literal minimum solution */
        if ((best_save_cost.cubes > cost.cubes) ||
            (best_save_cost.cubes == cost.cubes) && (best_save_cost.in > cost.in)) {
            sf_free(Fbest_save);
            Fbest_save = sf_save(F);
        }
...
pcover snocomp(pcover F, pcover D1) {
    pcover E, D, Fold;
    cost_t cost;
    pcube  p, last;
    ...
    if (F->count > 1) EXEC(F = irredundant(F, D), "IRRED_TIME     ", F);
...
pset_family ncp_unate_compl(pset_family A, int max_lit) {
    register pset p, last;

    foreach_set(A, last, p) { PUTSIZE(p, set_ord(p)); }
...
pset_family ncp_unate_complement(pset_family A, int max_lit) {
    pset_family   Abar;
    ...
    if (A->count == 0) {
        ...
        if (min_set_ord == 0) {
            ...
            i = set_ord(restricted);
            if (i > max_lit) {
                Abar = A;
                Abar->count = 0;
            } else {
                Abar = ncp_unate_complement(ncp_abs_covered_many(A, restricted),
                max_lit - i);
                sf_free(A);
                foreachi_set(Abar, i, p) { (void) set_or(p, p, restricted); }
            }

            /* else, recur as usual */
        } else {
            max_i = ncp_abs_select_restricted(A, restricted);
            ...
            foreachi_set(Abar, i, p) { set_insert(p, max_i); }
 ...
 */
static pset_family ncp_abs_covered_many(pset_family A, register pset pick_set) {
 ...
 *  1 / (set_ord(p) - 1).
 */
static int ncp_abs_select_restricted(pset_family A, pset restricted) {
 ...
 *  create a new matrix which is only those rows which are still uncovered
 */
static pset_family ncp_abs_covered(pset_family A, register int pick) {
    register pset        last, p, pdest;
    register pset_family Aprime;
 ...
 * max_lit.
 */
void ncp_compl_special_cases(pcube *T, pcover *Tbar, int max_lit) {
...
pcover nc_super_gasp(pcover F, pcover D, cost_t *cost) {
    pcover G, G1;
...
pcover nc_make_sparse(pcover F, pcover D) {
...
pcover nc_expand_gasp(INOUT pcover F, IN pcover D, IN pcover Foriginal) {
    int    c1index;
    pcover G;
 ...
 */
void nc_expand1_gasp(pcover F, pcover D, pcover Foriginal, int c1index, pcover *G) {
    register int   c2index;
    register pcube p, last, c2under;
    pcube          RAISE     = nc_tmp_cube[28], FREESET = nc_tmp_cube[29],
                   temp      = nc_tmp_cube[30];
    ...
    foreach_set(ROS, last, p) { SET(p, ACTIVE); }
...
pcover nc_last_gasp(pcover F, pcover D, cost_t *cost) {
    pcover G, G1;
 ...
 */
pcover nc_reduce_gasp(pcover F, pcover D) {
 ...
 */
pcover nc_reduce(INOUT pcover F, OUT pcover *Fold, IN pcover D) {
...
pcover nc_random_order(register pcover F, register pcover Fold) {
    pset         temp;
    register int i, k;
#ifdef RANDOM
    long random();
...
pcover nc_permute(pcover F1, pcube *T1, pcover F2) {
 ...
 * in F.
 */
pcover nc_mini_sort(pcover F, pcover *G, int (*compare)()) {
...
pcube nc_reduce_cube(IN pcube *FD, IN pcube p) {
    pcube cunder;
...
pcube nc_sccc(INOUT pcube *T, IN int level) {
    pcube          r, *Tl, *Tr;
...
bool nc_sccc_special_cases(INOUT pcube *T, OUT pcube *result, IN int level) {
 ...
 * expand1
 */
pcover ncp_expand(INOUT pcover F, IN pcover Fold, IN bool nonsparse) {
    register pcube last, p;

    register pcube RAISE             = nc_tmp_cube[0], FREESET = nc_tmp_cube[1],
                   INIT_LOWER        = nc_tmp_cube[2], SUPER_CUBE = nc_tmp_cube[3],
...
void ncp_expand1(pcover BB, pcover CC, pcube RAISE, pcube FREESET, pcube OVEREXPANDED_CUBE, pcube SUPER_CUBE,
...
pcover nc_first_expand(INOUT pcover F, IN bool nonsparse) {
    register pcube last, p;
    ...
    foreachi_set(F, index, p) {
        ...
        if (!TESTP(p, PRIME) && !TESTP(p, COVERED)) {
            ...
            /* expand the cube p, result is RAISE */
            expand1(ROS, F, RAISE, FREESET, OVEREXPANDED_CUBE, SUPER_CUBE, init_lower,
                    &num_covered, p);
 ...
 * unate.
 */
void rem_unnec_r_cubes(pcube *T) {
