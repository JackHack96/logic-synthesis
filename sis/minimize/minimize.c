...
pcover minimize(F, D, type) pcover F;
pcover D;
int type;
...
pcover nocomp(F, D1) pcover F, D1;
...
pcover snocomp(F, D1) pcover F, D1;
...
pset_family ncp_unate_compl(A, max_lit) pset_family A;
int max_lit;
...
pset_family ncp_unate_complement(A, max_lit) pset_family A; /* disposes of A */
int max_lit;
 ...
 */
static pset_family ncp_abs_covered_many(A, pick_set) pset_family A;
 ...
 *  1 / (set_ord(p) - 1).
 */
static int ncp_abs_select_restricted(A, restrict) pset_family A;
pset restrict;
{
 ...
 *  create a new matrix which is only those rows which are still uncovered
 */
static pset_family ncp_abs_covered(A, pick) pset_family A;
register int pick;
{
  register pset last, p, pdest;
  register pset_family Aprime;
 ...
 * max_lit.
 */
void ncp_compl_special_cases(T, Tbar,
                             max_lit) pcube *T; /* will be disposed of */
...
pcover nc_super_gasp(F, D, cost) pcover F, D;
cost_t *cost;
{
  pcover G, G1;
...
pcover nc_all_primes(F) pcover F;
{
...
pcover nc_make_sparse(F, D) pcover F, D;
{
...
pcover nc_expand_gasp(F, D, Foriginal)
 ...
 */
void nc_expand1_gasp(F, D, Foriginal, c1index,
...
pcover nc_last_gasp(F, D, cost) pcover F, D;
cost_t *cost;
{
  pcover G, G1;
 ...
 */
pcover nc_reduce_gasp(F, D) pcover F, D;
{
...
pcover nc_random_order(F, Fold) register pcover F, Fold;
{
  pset temp;
  register int i, k;
#ifdef RANDOM
...
pcover nc_permute(F1, T1, F2) pcover F1;
pcube *T1;
pcover F2;
 ...
 * in F.
 */
pcover nc_mini_sort(F, G, compare) pcover F;
pcover *G;
int (*compare)();
{
...
bool nc_sccc_special_cases(T, result, level)
 ...
 * expand1
 */
pcover ncp_expand(F, Fold, nonsparse)
...
void ncp_expand1(BB, CC, RAISE, FREESET, OVEREXPANDED_CUBE, SUPER_CUBE,
                 INIT_LOWER, num_covered,
                 c) pcover BB; /* Blocking matrix (OFF-set) */
...
pcover nc_first_expand(F, nonsparse)
 ...
 * unate.
 */
void rem_unnec_r_cubes(T) pcube *T;
