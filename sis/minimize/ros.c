...
void get_ROS1(p, overexpanded_cube, pNode) pcube p, overexpanded_cube;
pnc_node pNode;
...
void init_ROS(F, D, maxLevel, nLevel, rosZeros) pcover F;
pcover D;
int maxLevel;
int nLevel;
int rosZeros;
...
bool find_correct_ROS(p) pcube p;
...
void find_overexpanded_cube_dyn(p, overexpanded_cube, T, dist_cube, dist, var,
                                level) pcube p; /* Node to be expanded */
 ...
 */
bool find_oc_dyn_special_cases(p, overexpanded_cube, T, dist_cube, dist, var,
                               level) pcube p; /* Node to be expanded */
pcube overexpanded_cube;                       /* overexpanded cube so far */
pcube T[];                                     /* The current cubelist */
pcube dist_cube; /* Node where distance changed from 0 to 1 */
...
pcover setupBB_dyn(p, overexpanded_cube, T, level) pcube p;
 ...
 * minimize.h
 */
bool BB_dyn_special_cases(BB, p, overexpanded_cube, T, level) pcover *BB;
pcube p;
pcube overexpanded_cube;
pcube *T;
int level;
...
void find_overexpanded_cube(p, overexpanded_cube, pNode, dist_cube, dist, var,
                            level) pcube p; /* Node to be expanded */
pcube overexpanded_cube;                    /* overexpanded cube so far */
 ...
 * Special cases for finding overexpanded cube.
 */
bool find_oc_special_cases(p, overexpanded_cube, pNode, dist_cube, dist, var,
                           level) pcube p; /* Node to be expanded */
pcube overexpanded_cube;                   /* overexpanded cube so far */
pnc_node pNode;                            /* The current node */
pcube dist_cube; /* Node where distance changed from 0 to 1 */
...
pcover setupBB(p, overexpanded_cube, pNode, level) pcube p;
pcube overexpanded_cube;
pnc_node pNode;
int level;
...
bool BB_special_cases(BB, p, overexpanded_cube, pNode, level) pcover *BB;
pcube p;
pcube overexpanded_cube;
pnc_node pNode;
int level;
...
void get_OC(p, overexpanded_cube, old_cube) pcube p,
    overexpanded_cube, old_cube;
 ...
 */
bool partition_ROS(p, overexpanded_cube, level, T, R) pcube p,
    overexpanded_cube;
int level;
pcube *T;
pcover *R;
...
pcover multiply2_sf(A, B) pcover A, B; /* Disposes of A and B */

{
  ...
  foreach_set(A, lastA, pA) foreach_set(B, lastB, pB)
...
int nc_cubelist_partition(T, A, B, comp_debug) pcube *T; /* a list of cubes */
...
void nc_generate_cof_tree(pNode, T, level) pnc_node pNode;
pcube *T;
int level;
 ...
 */
pcover nc_compl_cube(p) register pcube p;
{
...
void nc_free_unate_tree(pNode) pnc_node pNode;
...
void nc_bin_cof(T, Tl, Tr, cleft, cright, var) pcube *T, **Tl, **Tr;
pcube cleft, cright;
int var;
...
bool cdist0v(p, q, var) pcube p, q;
int var;
...
void nc_rem_unnec_cubes(p, T) pcube p;
pcube *T;
...
void nc_or_unnec(c, T, orred_cube) pcube c, *T, orred_cube;
...
void nc_sf_multiply(A, c, var) pset_family A;
pcube c;
int var;
...
void nc_rem_noncov_cubes(p, T) pcube p, *T;
 ...
 * the complement.
 */
void nc_compl_special_cases(
    T, Tbar) pcube *T; /* will be disposed if answer is determined */
 ...
 */
pcube *nc_copy_cubelist(T) pcube *T;
...
void nc_setup_tmp_cubes(num) int num;
...
void nc_free_tmp_cubes(num) int num;
