...
void get_ROS1(pcube p, pcube overexpanded_cube, pnc_node pNode) {
    pcover BB;
    long   t;
    pcube  temp;
...
pcover get_ROS(int i, pcover F, pcover Fold) {
...
void init_ROS(pcover F, pcover D, int maxLevel, int nLevel, int rosZeros) {
...
void
find_overexpanded_cube_dyn(pcube p, pcube overexpanded_cube, pcube *T, pcube dist_cube, int dist, int var, int level) {
    pcube cl, cr;
    pcube *Tl, *Tr;
    int   best;

    if (find_oc_dyn_special_cases(p, overexpanded_cube, T, dist_cube, dist, var,
                                  level)) {
        return;
    } else {
        ...
        if (dist == 0) {
            ...
            if (cdist0v(cl, p, best))
                find_overexpanded_cube_dyn(p, overexpanded_cube, Tl, (pcube) NULL, 0, 0,
                                           level + 1);
            else if (cdist0v(cl, overexpanded_cube, best)) {
                d1_active = TRUE;
                find_overexpanded_cube_dyn(p, overexpanded_cube, Tl, cr, 1, best,
                                           level + 1);
            }

            /* Process the right subtree */
            if (cdist0v(cr, p, best))
                find_overexpanded_cube_dyn(p, overexpanded_cube, Tr, (pcube) NULL, 0, 0,
                                           level + 1);
            else if (cdist0v(cr, overexpanded_cube, best)) {
                d1_active = TRUE;
                find_overexpanded_cube_dyn(p, overexpanded_cube, Tr, cl, 1, best,
                                           level + 1);
            }
            ...
            if (var == best) { /* Split is on the conflicting variable */
                find_overexpanded_cube_dyn(p, overexpanded_cube, Tl, dist_cube, 1, var,
                                           level + 1);
                find_overexpanded_cube_dyn(p, overexpanded_cube, Tr, dist_cube, 1, var,
                                           level + 1);
            } else {
                if (cdist0v(cl, p, best))
                    find_overexpanded_cube_dyn(p, overexpanded_cube, Tl, dist_cube, 1,
                                               var, level + 1);
                if (cdist0v(cr, p, best) && (d1_active))
                    find_overexpanded_cube_dyn(p, overexpanded_cube, Tr, dist_cube, 1,
                                               var, level + 1);
            }
        }
 ...
 */
bool
find_oc_dyn_special_cases(pcube p, pcube overexpanded_cube, pcube T[], pcube dist_cube, int dist, int var, int level) {
    pcube temp;
    int   k, last_word;
    pcube mask;
...
pcover setupBB_dyn(pcube p, pcube overexpanded_cube, pcube *T, int level) {
 ...
 * minimize.h
 */
bool BB_dyn_special_cases(pcover *BB, pcube p, pcube overexpanded_cube, pcube *T, int level) {
...
void find_overexpanded_cube(pcube p, pcube overexpanded_cube, pnc_node pNode, pcube dist_cube, int dist, int var,
                            int level) {
    int k;

    if (find_oc_special_cases(p, overexpanded_cube, pNode, dist_cube, dist, var,
                              level)) {
        return;
    } else {
        ...
        if (dist == 0) {
            for (k = 0; k <= 1; k++) {
                ...
                if (cdist0v(pNode->part_cube[k], p, pNode->var))
                    find_overexpanded_cube(p, overexpanded_cube, pNode->child[k],
                                           (pcube) NULL, 0, 0, level + 1);

                else if (cdist0v(pNode->part_cube[k], overexpanded_cube, pNode->var)) {
                    d1_active = TRUE;
                    find_overexpanded_cube(p, overexpanded_cube, pNode->child[k],
                                           pNode->part_cube[1 - k], 1, pNode->var,
                                           level + 1);
                }
            }
        } else { /* dist == 1 */
            ...
            if (var == pNode->var) { /* Split is on the conflicting variable */
                find_overexpanded_cube(p, overexpanded_cube, pNode->child[LEFT],
                                       dist_cube, 1, var, level + 1);
                find_overexpanded_cube(p, overexpanded_cube, pNode->child[RIGHT],
                                       dist_cube, 1, var, level + 1);
                ...
                if ((cdist0v(pNode->part_cube[RIGHT], p, pNode->var)) && (d1_active))
                    find_overexpanded_cube(p, overexpanded_cube, pNode->child[RIGHT],
                                           dist_cube, 1, var, level + 1);
            }
        }
 ...
 */
bool
find_oc_special_cases(pcube p, pcube overexpanded_cube, pnc_node pNode, pcube dist_cube, int dist, int var, int level) {
    ...
    if (pNode->cubelist != NULL) {
        if (level < Max_level) { /* A unate leaf has been reached */
            ...
            if (dist == 0)
                find_overexpanded_cube_dyn(p, overexpanded_cube, T_d, (pcube) NULL, 0, 0,
                                           level + 1);
            else
                find_overexpanded_cube_dyn(p, overexpanded_cube, T_d, dist_cube, 1, var,
                                           level + 1);
...
pcover setupBB(pcube p, pcube overexpanded_cube, pnc_node pNode, int level) {
    pset_family BBl, BBr;
    pcube       cl, cr;
    int         var;
    ...
    if (BB_special_cases(&BBl, p, overexpanded_cube, pNode, level)) {
        ...
        if (cdist0v(overexpanded_cube, cl, var)) {
            ...
            BBr = setupBB(p, overexpanded_cube, pNode->child[RIGHT], level + 1);
            if ((cdist0v(p, cr, var)) || (BBr->count == 0))
                return (BBr);
...
bool BB_special_cases(pcover *BB, pcube p, pcube overexpanded_cube, pnc_node pNode, int level) {
...
void get_OC(pcube p, pcube overexpanded_cube, pcube old_cube) {
    ...
    if (old_cube == (pcube) NULL) {
        (void) set_copy(overexpanded_cube, cube.fullset);
        find_overexpanded_cube(p, overexpanded_cube, root, (pcube) NULL, 0, 0, 1);
    } else {
 ...
 */
bool partition_ROS(pcube p, pcube overexpanded_cube, int level, pcube *T, pcover *R) {
...
pcover multiply2_sf(pcover A, pcover B) {
    ...
    foreach_set(A, lastA, pA) foreach_set(B, lastB, pB)set_and(GETSET(R, R->count++), pB, pA);
...
int nc_cubelist_partition(pcube *T, pcube **A, pcube **B, unsigned int comp_debug) {
...
void nc_generate_cof_tree(pnc_node pNode, pcube *T, int level) {
 ...
 */
pcover nc_compl_cube(register pcube p) {
...
void nc_free_unate_tree(pnc_node pNode) {
...
void nc_bin_cof(pcube *T, pcube **Tl, pcube **Tr, pcube cleft, pcube cright, int var) {
    ...
    Tc         = T + 2;
    while (((p = *Tc) != NULL) && (GETINPUT(p, var) == 3))
        Tc++;
...
void nc_cof_contain(INOUT pcube *T) {
...
bool cdist0v(pcube p, pcube q, int var) {
...
void nc_rem_unnec_cubes(pcube p, pcube *T) {
...
void nc_or_unnec(pcube c, pcube *T, pcube orred_cube) {
...
bool nc_is_nec(pcube c, pcube *T) {
...
void nc_sf_multiply(pset_family A, pcube c, int var) {
    ...
    foreach_set(A, last, p) for (i = fword; i <= lword; i++) p[i] &= temp[i];
...
void nc_rem_noncov_cubes(pcube p, pcube *T) {
 ...
 * the complement.
 */
void nc_compl_special_cases(pcube *T, pcover *Tbar) {
 ...
 */
pcube *nc_copy_cubelist(pcube *T) {
...
void nc_setup_tmp_cubes(int num) {
...
void nc_free_tmp_cubes(int num) {
