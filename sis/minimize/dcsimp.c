...
int literals_to_raise(p, q) pcube p;
pcube q;
...
void dc_simplify(F, D, LKset) pcube *F;
pcube *D;
pcover *LKset;
{
...
static bool dc_special_cases(F, D, FD) pcube *F;
pcube *D;
pcover *FD;
{
  pcover A, B;
...
bool is_orthagonal(p, A) pcube p;
pcover A;
...
pcover expand_to_largest(F, ros) pcover F;
pcover ros;
