...
pcover dcsimp(pcover F, pcover Old_D) {
...
pcover myexpand(pcover LKset, int lit_dis) {
    pcover T3;
    pcover T;
    pcube  p, mp, por;
    pcube  pp;
    ...
    foreach_set(T3, last1, p) { SET(p, ACTIVE); }
    ...
    foreach_active_set(T3, last1, p) {
            ...
            foreach_active_set(T3, last2, mp) {
                     ...
                     *  if the number of literals is less than limit then do orhagonality
                     * test.
                     */
...
int literals_to_raise(pcube p, pcube q) {
    ...
    for (i = 0; i < (cube.num_binary_vars); i++) {
        if ((lit = GETINPUT(p, i)) != TWO && GETINPUT(q, i) != lit)
            count++;
    }
    return (count);
}

int mydescend(pset *a, pset *b) {
    register pset a1 = *a, b1 = *b;
    if (cube_order(a1) < cube_order(b1))
        return 1;
    else if (cube_order(a1) > cube_order(b1))
        return -1;
    else
        return 0;
}
...
void dc_simplify(pcube *F, pcube *D, pcover *LKset) {
    ...
    if (dc_special_cases(F, D, LKset) == MAYBE) {
        ...
        foreach_set(LKsetl, last, p) {INLINEset_and(p, p, cl); }
        foreach_set(LKsetr, last, p) {INLINEset_and(p, p, cr); }
...
static bool dc_special_cases(pcube *F, pcube *D, pcover *FD) {
    pcover A, B;
...
bool is_orthagonal(pcube p, pcover A) {
...
pcover expand_to_largest(pcover F, pcover ros) {
    ...
    foreachi_set(F, i, p) {
        ...
        foreach_set(ros, last, q) { SET(q, ACTIVE); }
