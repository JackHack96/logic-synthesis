...
network_t *power_symbolic_simulate(network_t *network, st_table *info_table) {
    st_table     *internal = st_init_table(st_ptrcmp, st_ptrhash);
    array_t      *gates, *local_array, *array1, *array3, *array4, *before_switching,
                 *after_switching, *switching_times;
    network_t    *symbolic_network;
    node_t       *actual_gate, *a, *b, *xor, *fanin, *fanin1, **old_to_gate,
                 **new_to_gate, *pi1, *pi2;
    node_info_t  *node_info;
    delay_info_t *delay_info;
    int          i, j, k, l, *array2, next, first, last, *switched, frame, gate_delay, max,
                 min, max_num_tr, transition;
    char         buffer[1000], *key;
    st_generator *sgen;
    ...
    for (i = 0; i < array_n(gates); i++) {
        ...
        if (node_function(actual_gate) == NODE_PI) {
            ...
            switching_times = array_alloc(int, 1);
            array_insert_last(int, switching_times, 0);
        ...
        switching_times  = array_alloc(int, 0);
        ...
        foreach_fanin(actual_gate, j, fanin) {
            ...
            first   = array_fetch(int, array1, 0);
            last    = array_fetch(int, array1, (array_n(array1) - 1));
            if (first < min)
                min = first;
            if (last > max)
                max = last;
        ...
        local_array = array_alloc(int *, 0);
        for (j      = 0; j < (max - min + 1); j++) {
            switched = ALLOC(int, node_num_fanin(actual_gate));
            for (k   = 0; k < node_num_fanin(actual_gate); k++)
                switched[k] = -1;
            array_insert_last(int *, local_array, switched);
        }
        ...
        foreach_fanin(actual_gate, j, fanin) {
            ...
            for (k = 0; k < array_n(array1); k++) {
                first  = array_fetch(int, array1, k) - min;
                array2 = array_fetch(int *, local_array, first);
                array2[j] = 1;
            }
        }
        ...
        foreach_fanin(actual_gate, j, fanin) {
            ...
            first = array_fetch(int, array4, 0);
            if (first == min)
        ...
        for (j = 0; j < (max - min + 1); j++) {
            ...
            transition = 0;
            array2     = array_fetch(int *, local_array, j);
            ...
            if (transition) {
                ...
                array_insert_last(node_t *, before_switching, pi1);
                array_insert_last(node_t *, after_switching, pi2);
                array_insert_last(int, switching_times, j + min + gate_delay);
                ...
                if (next > (max - min)) /* We are done, go out */
                    break;
                array2 = array_fetch(int *, local_array, next);
                for (k = 0; k < node_num_fanin(actual_gate); k++) {
                    if (array2[k] == 1) {
                        ...
                        for (l = 0; l < array_n(array4); l++) {
                            frame = array_fetch(int, array4, l);
                            if (next + min == frame)
                ...
                array2 = array_fetch(int *, local_array, j + 1);
                for (k = 0; k < node_num_fanin(actual_gate); k++) {
                    if (array2[k] == 1) {
                        fanin = node_get_fanin(actual_gate, k);
                        assert(st_lookup(internal, (char *) fanin, (char **) &delay_info));
