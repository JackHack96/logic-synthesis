...
int power_command_line_interface(network_t *network, int argc, char **argv) {
    FILE   *info_file;
    int    i, status, mode, delay, type, psProbOption, sop_or_fact, input_sizing,
           num_samples, sample_gap;
    double total_power;
    ...
    for (i = 1; i != argc;)
        switch (argv[i++][1]) {
            case 'm':
                switch (argv[i][0]) {
                    case 'b':
                    case 'B':mode = BDD_MODE;
                        break;
                    case 's':
                    case 'S':mode = SAMPLE_MODE;
                        break;
                    default:fprintf(siserr, "Invalid mode: %s. Only BDD or SAMPLE.\n", argv[i]);
                        return 1;
                }
                i++;
                continue;
            case 'd':
                switch (argv[i][0]) {
                    ...
                    case 'G':delay = 2;
                        break;
                    default:
                        fprintf(siserr, "Invalid delay: %s. One of ZERO, UNIT or GENERAL.\n",
                                argv[i]);
                        return 1;
                }
                i++;
                ...
                switch (argv[i][0]) {
                    ...
                    case 'S':
                      if (network_num_latch(network))
                        ...
                        fprintf(siserr, "Warning, non sequential network. Combinational "
                    ...
                    case 'D':type = DYNAMIC;
                        break;
                    default:fprintf(siserr, "Invalid type: %s. One of COMBINATIONAL", argv[i]);
#ifdef SIS
            ...
            case 's': /* Only used for sequential type circuits */
              ...
              case 'E':
                if (network_num_latch(network) > 16) {
                  fprintf(siserr, "Too many states! Maximum number of latches for "
                                  "exact method is 16.\n");
                  return 1;
                }
              ...
              /*            case 'l':                 * Not very interesting - remove
                            case 'L':
                              if(network_num_latch(network) > 16){
                                  fprintf(siserr, "Too many states! Maximum number of
                 latches for exact method is 16.\n"); return 1;
                              }
              ...
              */
              ...
              default:
                fprintf(siserr,
                        "Invalid PS prob calculation: %s. One of APPROXIMATION, EXACT "
                        "or UNIFORM.\n",
                        argv[i]);
                return 1;
              }
            ...
            case 'n': /* Only used for sampling mode */
                ...
                i++;
                continue;
            case 'f':info_file = fopen(argv[i], "r");
...
/* Undocumented options: might be useful for someone else, so leave them here */
            ...
            case 'M': /* Until how many inputs we assume transistor resizing */
                ...
                i++;
                continue;
            case 'S':sop_or_fact = SUM_OF_PRODUCTS;
                continue;
            case 'N':sample_gap = atoi(argv[i]);
                i++;
                continue;
            case 'V':power_verbose = atoi(argv[i]);
                i++;
                continue;
    ...
    if (mode == BDD_MODE)
        status = power_main_driver(network, type, delay, psProbOption, info_file,
    ...
    /* Print result */
    switch (type) {
        case COMBINATIONAL:fprintf(sisout, "Combinational power estimation, ");
            break;
#ifdef SIS
        case SEQUENTIAL:
          ...
          case APPROXIMATION_PS:
            if (power_setSize != 1) {
              ...
              if (power_delta != DEFAULT_PS_MAX_ALLOWED_ERROR)
                ...
                fprintf(sisout, "), ");
            } else if (power_delta != DEFAULT_PS_MAX_ALLOWED_ERROR)
        ...
        case PIPELINE:
          ...
          break;
#endif
        case DYNAMIC:fprintf(sisout, "Dynamic power estimation.\n");
    }
    if (type != DYNAMIC)
        switch (delay) {
            case 0:fprintf(sisout, "with Zero delay model.\n");
                break;
            case 1:fprintf(sisout, "with Unit delay model.\n");
                break;
            default:fprintf(sisout, "with General delay model.\n");
        }
    if (mode == SAMPLE_MODE)
    ...
    fprintf(sisout,
            "Network: %s, Power = %.1f uW assuming 20 MHz clock and Vdd = 5V\n",
...
int power_estimate(network_t *network, int type, int delay, double *total_power) {
    int status;
    ...
    status =
            power_main_driver(network, type, delay, APPROXIMATION_PS, NIL(FILE),
                              FACTORED_FORM, DEFAULT_MAX_INPUT_SIZING, total_power);
    return status;
}
...
int power_main_driver(network_t *network, int type, int delay, int psProbOption, FILE *info_file, int sop_or_fact,
    ...
    info_table =
            power_get_node_info(network, info_file, sop_or_fact, input_sizing);
    if (info_file) {
        fclose(info_file);
    }
    ...
    switch (delay) {
        case 0:st_foreach_item(info_table, gen, (char **) &node, (char **) &info) {
                if (info->delay == -1) /* Otherwise already specified */
                    info->delay = 0;
            }
            break;
        case 1:st_foreach_item(info_table, gen, (char **) &node, (char **) &info) {
                if (info->delay == -1) /* Otherwise already specified */
                    info->delay = 1;
            }
            break;
        default:status = delay_trace(network, DELAY_MODEL_MAPPED);
            if (!status) {
    ...
    switch (type) {
        case COMBINATIONAL:
            switch (delay) {
                case 0:status = power_comb_static_zero(network, info_table, total_power);
                    break;
                default:status = power_comb_static_arbit(network, info_table, total_power);
            }
            break;
#ifdef SIS
        case SEQUENTIAL:
          status =
              power_seq_static_arbit(network, info_table, psProbOption, total_power);
          break;
        case PIPELINE:
          status = power_pipe_arbit(network, info_table, total_power);
          break;
#endif
        case DYNAMIC:status = power_dynamic(network, info_table, total_power);
            break;
        default:fprintf(siserr, "Internal error!!! Unknown mode in power_main_driver!\n");
            return 1;
    }
...
st_table *power_get_node_info(network_t *network, FILE *file, int sop_or_fact, int input_sizing) {
    st_table     *info_table;
    node_t       *node;
...
static void power_fill_info_table_from_file(st_table *info_table, network_t *network, FILE *file, int sop_or_fact,
                                            int input_sizing) {
    char         buffer[100];
    power_info_t *power_info;
    node_info_t  *node_info;
 ...
 */
int power_get_mapped_delay(node_t *node) {
    node_t       *fanin;
    delay_time_t delay;
    double       max, act_delay;
    int          i, int_delay;
...
static int load_sum_of_products(network_t *network, node_t *node, int input_sizing) {
    node_t       *fanout, *fanin;
    power_info_t *power_info;
    int          i, total = 0, num_fanin, *literal_count;
    lsGen        gen;

    foreach_fanout(node, gen, fanout) {
        if (node_function(fanout) == NODE_PO) {
            if (network_is_real_po(network, fanout)) {
                if (st_lookup(power_info_table, (char *) fanout, (char **) &power_info))
...
static int load_factored_form(network_t *network, node_t *node, int input_sizing) {
    node_t       *fanout;
    power_info_t *power_info;
    int          total = 0, num_fanin;
    lsGen        gen;
...
static void power_usage() {
...
#ifdef SIS
    fprintf(siserr, "Usage: power_estimate [-m c] [-d c] [-t c] [-s c] [-a n] "
                    "[-e f] [-n n] [-f file]\n");
#else
    fprintf(siserr,
            "Usage: power_estimate [-m c] [-d n] [-t c] [-n n] [-f file]\n");
#endif
    fprintf(siserr,
            "-m c: estimation mode, c either SAMPLING or BDD (default).\n");
    fprintf(siserr, "-d c: delay model, c one of ZERO (default), UNIT or GENERAL "
                    "(from library).\n");
#ifdef SIS
    fprintf(
        siserr,
    ...
    fprintf(siserr, "-s c: PS lines probability calculation method, c one of "
                    "APPROXIMATION (default),\n      EXACT or UNIFORM (0.5 is "
                    "used). Only used for SEQUENTIAL type.\n");
    fprintf(siserr, "-a n: number of PS lines to be correlated (default 1). Only "
                    "used for\n      the APPROXIMATION method.\n");
    fprintf(siserr,
            "-e f: maximum error allowed for PS lines probabilities (default "
            "0.01). Only\n      used for the APPROXIMATION method.\n");
#else
    fprintf(
            siserr,
            "-t c: estimation type, c one of COMBINATIONAL (default) or DYNAMIC.\n");
#endif
    fprintf(siserr,
            "-n n: number of sets of %d input vectors to be simulated (default "
            "100). Only\n      used for SAMPLING mode.\n",
            sizeof(long) * 8);
    fprintf(siserr, "-f file: specify file with input probabilities, node "
