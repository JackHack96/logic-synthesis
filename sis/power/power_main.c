...
int power_command_line_interface(network, argc, argv) network_t *network;
int argc;
char **argv;
{
  FILE *info_file;
  ...
  for (i = 1; i != argc;)
    ...
    case 'm':
      ...
      case 'S':
        ...
        break;
      default:
        fprintf(siserr, "Invalid mode: %s. Only BDD or SAMPLE.\n", argv[i]);
        return 1;
      }
      i++;
      continue;
    ...
    case 't':
      ...
      case 'D':
        ...
        break;
      default:
        fprintf(siserr, "Invalid type: %s. One of COMBINATIONAL", argv[i]);
#ifdef SIS
...
int power_estimate(network, type, delay, total_power) network_t *network;
int type;
int delay;
double *total_power;
{
...
st_table *power_get_node_info(network, file, sop_or_fact,
                              input_sizing) network_t *network;
FILE *file;
int sop_or_fact;
int input_sizing;
{
...
static void power_fill_info_table_from_file(info_table, network, file,
                                            sop_or_fact,
                                            input_sizing) st_table *info_table;
network_t *network;
FILE *file;
int sop_or_fact;
int input_sizing;
 ...
 */
int power_get_mapped_delay(node) node_t *node;
{
  node_t *fanin;
  delay_time_t delay;
...
static int load_sum_of_products(network, node, input_sizing) network_t *network;
node_t *node;
int input_sizing;
{
  node_t *fanout, *fanin;
...
static int load_factored_form(network, node, input_sizing) network_t *network;
node_t *node;
int input_sizing;
{
  node_t *fanout;
