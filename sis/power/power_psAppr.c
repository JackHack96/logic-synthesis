...
double *
power_direct_PS_lines_prob(network_t *network, st_table *info_table, array_t **symbPSOrder, network_t *symbolic) {
    network_t *nsLogic;
...
static double *
calculate_line_prob(network_t *nsLogic, network_t *network, st_table *info_table, array_t *psOrder, array_t *newNSs) {
...
void power_place_PIs_last(array_t **piOrder, array_t *psOrder) {
...
static double *power_calc_func_cofact_prob(bdd_t *f_bdd, double *psProb, int nNSLines, int nPSLines) {
    double       *result;
    int          i;
    pset         sets;
    char         *value, *key;
...
static double *power_count_f_cof_probPS(bdd_t *f, double *psProb, int nNSLines, int nPSLines, long prevInd, pset sets) {
    bdd_t  *child;
...
static double *power_count_f_cof_probPI(bdd_t *f, int nNSLines) {
    bdd_t  *child;
    double *result, *result1;
    int    i, topf;
...
static void multiply_PS_prob(double *result, long index, double *psProb, pset sets, int nNSLines) {
    pset   inclSets;
    double sumProb;
    long   setN;
...
static void
calculate_next_iteration(double *allpsLinesProb, double **psLinesProb, double **Y, double *J, int nNSLines) {
    int    i, j, *indx;
    double sum, *tmp;
...
static void ludcmp(double *a, int n, int *indx) {
    int    i, imax, j, k;
    ...
    for (i = 0; i < n; i++) {
        ...
        for (j = 0; j < n; j++)
            if ((temp = fabs(a[i * n + j])) > big)
                big = temp;
        if (big == 0.0) fail("Singular matrix in routine LUDCMP");
        vv[i] = 1.0 / big;
    }
    for (j = 0; j < n; j++) {
        for (i = 0; i < j; i++) {
...
static double *save_vector(double *vector, int n) {
    double *newVector;
    int    i;
...
static array_t *orderPSLines(network_t *nsLogic) {
    array_t *psOrder;
    node_t  *po;
    lsGen   gen;
...
static array_t *addOutputLogic(network_t *network, array_t *oldPSOrder) {
    array_t *newNSs;
    node_t  **nsSet;
...
static node_t **addNewNSSet(network_t *network, int setSize, int nNewNSs, array_t *oldPSOrder, int i) {
...
static void update_PSLines_prob(network_t *network, st_table *info_table, double *psLineProb, array_t *psOrder) {
    node_t      *ps;
    node_info_t *info;
    int         i;
