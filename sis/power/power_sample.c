...
int power_sample_estimate(network_t *network, int type, int delay, int num_samples, double *total_power) {
    int status;
    ...
    status = power_simulation_estimate(
            network, type, delay, APPROXIMATION_PS, num_samples, num_samples + 1,
            NIL(FILE), FACTORED_FORM, DEFAULT_MAX_INPUT_SIZING, total_power);
    return status;
}
...
int
power_simulation_estimate(network_t *network, int type, int delay, int psProbOption, int num_samples, int sample_gap,
                          FILE *info_file, int sop_or_fact, int input_sizing, double *total_power) {
    st_table     *info_table;
    array_t      *psOrder;
    network_t    *symbolic;
    node_info_t  *info;
    node_t       *node;
    double       *probPI, *psLineProb;
    int          status;
    st_generator *gen;
    ...
    switch (type) {
        case COMBINATIONAL:break; /* Nothing to be done */
#ifdef SIS
        case PIPELINE:
        ...
        case SEQUENTIAL: /* For sampling, only PSlines probability */
            ...
            switch (psProbOption) {
                ...
                case STATELINE_PS:
                    fprintf(siserr,
                            "Warning, this version doesn't support exact state probabilities "
                            "in sample mode.\nThe approximate method will be used.\n");
                case APPROXIMATION_PS: /* Line prob. will be changed inside */
                    if (power_setSize != 1) {
                        fprintf(siserr, "Warning, this version doesn't support correlation "
                                        "between PS lines in sample\nmode. The approximate "
                                        "method will be used with set size 1.\n");
                        power_setSize = 1;
                    }
                    psLineProb =
                    ...
                    array_free(psOrder);
                    break;
                case UNIFORM_PS:break;
                default:
                    fprintf(
                            siserr,
                            "Internal error!!! Unknown option in power_simulation_estimate!\n");
                    return 1;
            }
            break;
#endif /* SIS */
        case DYNAMIC:
            fprintf(siserr, "Error! Sorry, this version does not support dynamic "
                            "circuits in sample mode.\n");
            return 1;
        default:
            fprintf(siserr,
                    "Internal error!!! Unknown type in power_simulation_estimate!\n");
            return 1;
    }
    ...
    *total_power =
            power_sample_do_estimate(symbolic, num_samples, sample_gap, probPI);
...
static double power_sample_do_estimate(network_t *network, int num_iter, int num_gap, double *probPI) {
    double total_power;
    long   mask, *pi_values, *po_values;
    int    i, j, k, n_pi, n_po, seed, *n_ones, num_since_last = 0;
...
static double calculate_power(network_t *network, int *n_ones, int i) {
    node_t       *po;
...
static double *power_get_PIs_prob(network_t *symbolic, st_table *info_table) {
    node_t      *pi;
    node_info_t *info;
    double      *probPI;
    int         i;
...
static void gen_random_pattern(int n, long *pattern, int seed, double *probPI) {
    int i, j;
...
static void simulate_network_word(network_t *network, long *pi_values, long *po_values) {
...
static long simulate_network_word_rec(network_t *network, st_table *table, node_t *node) {
    node_t *input;
    long   result, **inputs;
    int    i, n_inputs = node_num_fanin(node);
    char   *value;
...
static long simulate_node_word(node_t *node, long **inputs) {
    register int i, j;
    node_t       *fanin;
    node_cube_t  cube;
    long         result    = 0, and_result;
    int          num_cubes = node_num_cube(node), literal;
