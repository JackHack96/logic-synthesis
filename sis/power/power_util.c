 ...
 *
 * $Source: /users/pchong/CVS/sis/sis/power/power_util.c,v $
 * $Author: pchong $
 * $Revision: 1.1.1.1 $
...
network_t *network;
{
	...
	fprintf(sisout,
                "Node %-5s\tCap. = %d\tSwitch Prob. = %.2f\tPower = %.1f\n",
		node->name, info->cap_factor, info->switching_prob,
		250.0 * info->cap_factor * info->switching_prob * CAPACITANCE);
	total_power += 250.0 * info->cap_factor * info->switching_prob *
	    CAPACITANCE;
	if(node_function(node) == NODE_PI)
	    PIs_power += 250.0 * info->cap_factor * info->switching_prob *
		CAPACITANCE;
    }
...
pset power_lines_in_set(set, setSize)
pset set;
int setSize;    
{
