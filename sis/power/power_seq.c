 ...
 */
int power_seq_static_arbit(network_t *network, st_table *info_table, int pslOption, double *total_power) {
    ...
    switch (pslOption) {
        case EXACT_PS:power_place_PS_lines_first(&piOrder, stateLineIndex);
            break;
            ...
            case APPROXIMATION_PS:
                if (power_setSize == 1)
                    ...
                    prob_node_one = power_calc_func_prob_w_sets(bdd, PIInfo, psLineProb,
                                                                array_n(psOrder));
                break;
            default:prob_node_one = power_calc_func_prob(bdd, PIInfo);
        }
...
int power_add_fsm_state_logic(network_t *network, network_t *symbolic) {
...
static void power_place_PS_lines_first(array_t **piOrder, st_table *stateLineIndex) {
...
static int power_network_concatenate(network_t *symbolic_net, network_t *ns_logic, st_table *po_link_table,
                                     st_table *ps_link_table, st_table *pi_link_table) {
