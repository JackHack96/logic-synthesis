...
*/
int power_seq_static_arbit(network, info_table, pslOption, total_power)
network_t *network;
st_table *info_table;
int pslOption;
double *total_power;
{
    ...
    *total_power = 0;
    foreach_primary_output(symbolic, gen, po){
        ...
        switch(pslOption){
          case EXACT_PS:
            prob_node_one = power_calc_func_prob_w_stateProb(bdd, PIInfo,
                               stateProb, stateIndex, st_count(stateLineIndex));
            break;
          case APPROXIMATION_PS:
            if(power_setSize == 1)
                prob_node_one = power_calc_func_prob(bdd, PIInfo);
            else
                prob_node_one = power_calc_func_prob_w_sets(bdd, PIInfo,
                                                  psLineProb, array_n(psOrder));
            break;
          default:
            prob_node_one = power_calc_func_prob(bdd, PIInfo);
        }
        orig_node = node->copy;        /* Correspondence to the orig. network */
        assert(st_lookup(power_info_table,(char*)orig_node,(char**)&power_info));
...
int power_add_fsm_state_logic(network, symbolic)
network_t *network;
network_t *symbolic;
{
    st_table *po_table,
...
static void power_place_PS_lines_first(piOrder, stateLineIndex)
array_t **piOrder;
...
static int power_network_concatenate(symbolic_net, ns_logic, po_link_table,
                                     ps_link_table, pi_link_table)
network_t *symbolic_net;     /* Will be changed */
