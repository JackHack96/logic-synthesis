...
void power_PS_lines_from_state(network_t *network, st_table *info_table) {
...
double *
power_exact_state_prob(network_t *network, st_table *info_table, st_table **stateIndex, st_table **stateLineIndex) {
    graph_t *stg;
    double  *stateProb;
...
static double *power_state_prob(network_t *network, graph_t *stg, st_table *info_table, st_table **stateIndex) {
        ...
        foreach_state_inedge(state, genTransition, transition) {
            ...
            for (i          = 0; transitionInput[i] != '\0'; i++)
                switch (transitionInput[i]) {
                    case '0':transitionProb *= (1 - PIProb[i]);
                        continue;
                    case '1':transitionProb *= PIProb[i];
                }
...
static st_table *generatePSLIndexTable(network_t *network) {
    st_table *indexTable;
    latch_t  *latch;
    int      i = 0;
    lsGen    gen;
...
static void update_PSLines_prob(network_t *network, st_table *info_table, double *psLineProb, st_table *psIndex) {
    node_t      *pi;
    node_info_t *info;
    int         index;
    lsGen       gen;
