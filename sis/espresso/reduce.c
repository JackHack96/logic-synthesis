...
pcube sccc(INOUT pcube *T) {
    ...
    if (sccc_special_cases(T, &r) == MAYBE) {
        ...
        r    = sccc_merge(sccc(scofactor(T, cl, best)), sccc(scofactor(T, cr, best)),
                          cl, cr);
        free_cubelist(T);
    }
...
pcube
sccc_merge(INOUT register pcube left, INOUT register pcube right, INOUT register pcube cl, INOUT register pcube cr) {
