...
bool verify(pcover F, pcover Fold, pcover Dold) {
    ...
    foreach_set(F, last, p) if (!cube_is_covered(FD, p)) {
            (void) printf("some minterm in F is not covered by Fold u Dold\n");
            verify_error = TRUE;
            if (verbose_debug)
                (void) printf("%s\n", pc1(p));
            else
    ...
    FD = cube2list(F, Dold);
    foreach_set(Fold, last, p) if (!cube_is_covered(FD, p)) {
            (void) printf("some minterm in Fold is not covered by F u Dold\n");
            verify_error = TRUE;
            if (verbose_debug)
                (void) printf("%s\n", pc1(p));
            else
...
bool PLA_verify(pPLA PLA1, pPLA PLA2) {
     ...
     * match the names
     */
    if (PLA1->label != NULL && PLA1->label[0] != NULL && PLA2->label != NULL &&
        PLA2->label[0] != NULL) {
...
if (PLA1->F != NULL) {
PLA1->
F = sf_permute(PLA1->F, permute, npermute);
}
if (PLA1->R != NULL) {
PLA1->
R = sf_permute(PLA1->R, permute, npermute);
}
if (PLA1->D != NULL) {
PLA1->
D = sf_permute(PLA1->D, permute, npermute);
}
...
bool check_consistency(pPLA PLA) {
    ...
    if (T->count == 0)
        (void) printf("ON-SET and DC-SET are disjoint\n");
    else {
        (void) printf("Some minterm(s) belong to both the ON-SET and DC-SET !\n");
        if (verbose_debug)
            cprint(T);
        verify_error = TRUE;
    }
    ...
    if (T->count == 0)
        (void) printf("ON-SET and OFF-SET are disjoint\n");
    else {
        (void) printf("Some minterm(s) belong to both the ON-SET and OFF-SET !\n");
        if (verbose_debug)
            cprint(T);
        verify_error = TRUE;
    }
    ...
    if (T->count == 0)
        (void) printf("DC-SET and OFF-SET are disjoint\n");
    else {
        (void) printf("Some minterm(s) belong to both the OFF-SET and DC-SET !\n");
        if (verbose_debug)
            cprint(T);
        verify_error = TRUE;
    }
    ...
    if (tautology(cube3list(PLA->F, PLA->D, PLA->R)))
        (void) printf("Union of ON-SET, OFF-SET and DC-SET is the universe\n");
    else {
        T = complement(cube3list(PLA->F, PLA->D, PLA->R));
        (void) printf("There are minterms left unspecified !\n");
        if (verbose_debug)
            cprint(T);
        verify_error = TRUE;
        free_cover(T);
