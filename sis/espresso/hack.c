...
void map_output_symbolic(pPLA PLA) {
    ...
    for (p1 = PLA->symbolic_output; p1 != NIL(symbolic_t); p1 = p1->next) {
        ...
        find_inputs(NIL(set_family_t), PLA,
                    p1
         ...
         *  Not sure what this means
                find_dc_inputs(PLA, p1->symbolic_list,
                                    base, 1 << p1->symbolic_list_length, &newF,
         &newD);
         */
        free_cover(PLA->F);
        PLA->F = newF;
    ...
    symbolic_hack_labels(PLA, PLA->symbolic_output, compress, cube.size, old_size,
                         tot_size);
    set_free(compress);
}
...
void find_inputs(pcover A, pPLA PLA, symbolic_list_t *list, int base, int value,
    ...
    if (list == NIL(symbolic_list_t)) {
        ...
        foreach_set(S, last, p) { set_insert(p, base + value); }
...
void map_symbolic(pPLA PLA) {
    ...
    symbolic_hack_labels(PLA, PLA->symbolic, compress, new_size, cube.size,
                         size_added);
...
pcover map_symbolic_cover(pcover T, symbolic_list_t *list, int base) {
    pset last, p;
    foreach_set(T, last, p) { form_bitvector(p, base, 0, list); }
    return T;
}
...
void form_bitvector(pset p, int base, int value, symbolic_list_t *list) {
    if (list == NIL(symbolic_list_t)) {
        ...
        switch (GETINPUT(p, list->variable)) {
            case ZERO:form_bitvector(p, base, value * 2, list->next);
                break;
            case ONE:form_bitvector(p, base, value * 2 + 1, list->next);
                break;
            case TWO:form_bitvector(p, base, value * 2, list->next);
                form_bitvector(p, base, value * 2 + 1, list->next);
                break;
            default:fatal("bad cube in form_bitvector");
        }
    }
}
...
void disassemble_fsm(pPLA PLA, int verbose_mode) {
    ...
    if (cube.num_vars - cube.num_binary_vars != 2) {
        (void) fprintf(stderr, "use .symbolic and .symbolic-output to specify\n");
        (void) fprintf(stderr,
    ...
    for (i = 0; i < nstates; i++) {
        ...
        if (before > 0) {
            ...
            foreach_set(tF, last, p) { set_insert(p, next_state + i); }
            after = tF->count;
            F     = sf_append(F, tF);
            if (verbose_mode) {
    ...
    go_nowhere =
            unravel_range(go_nowhere, cube.num_binary_vars, cube.num_binary_vars);
    ...
    for (i = 0; i < nstates; i++) {
        for (j = 0; j < nstates; j++) {
                ...
                if (!setp_implies(present_state_mask, p)) {
                    if (is_in_set(p, present_state + i)) {
                        if (is_in_set(p, next_state + j)) {
                            p1 = set_save(p);
                            (void) set_diff(p1, p1, state_mask);
            ...
            if (before > 0) {
                tF = fsm_simplify(tF);
                /* don't allow the next state to disappear ... */
                foreach_set(tF, last, p) { set_insert(p, next_state + j); }
                after = tF->count;
                F     = sf_append(F, tF);
                if (verbose_mode) {
    ...
    foreach_set(PLA->F, last, p) { kiss_print_cube(stdout, PLA, p, "~1"); }
}
