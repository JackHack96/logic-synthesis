...
for (
i = 0;
i < cube.num_binary_vars * 2; i++) {
if (
strncmp(PLA
->label[i], "DONT_CARE", 9) == 0 ||
strncmp(PLA
->label[i], "DONTCARE", 8) == 0 ||
strncmp(PLA
->label[i], "dont_care", 9) == 0 ||
strncmp(PLA
->label[i], "dontcare", 8) == 0) {
...
PLA->
F = sf_delc(PLA->F, 2 * var, 2 * var + 1);
PLA->
D = sf_delc(PLA->D, 2 * var, 2 * var + 1);
}
...
->F);
PLA->
F = complement(cube2list(PLA->D, PLA->R));
}

/* tot_size = width added for all symbolic variables */
tot_size = 0;
for (
p1    = PLA->symbolic_output;
p1!=
NIL(symbolic_t);
   p1 = p1->next
) {
...
PLA->
F = sf_addcol(PLA->F, base, tot_size);
PLA->
D = sf_addcol(PLA->D, base, tot_size);
PLA->
R = sf_addcol(PLA->R, base, tot_size);
...
find_inputs(NIL(set_family_t), PLA, p1

->symbolic_list, base, 0,
&newF, &newD);
/*
...
cube.part_size[cube.output] -= newF->sf_size -
set_ord(compress);
...
PLA->
F = sf_compress(PLA->F, compress);
PLA->
D = sf_compress(PLA->D, compress);
...
find_inputs(S, PLA, list
->next, base, value*2, newF, newD);
free_cover(S);
...
for (
var = cube.num_binary_vars;
var < cube.num_vars-1; var++) {
...
base = cube.first_part[cube.output];
PLA->
F = sf_addcol(PLA->F, base, size_added);
PLA->
D = sf_addcol(PLA->D, base, size_added);
PLA->
R = sf_addcol(PLA->R, base, size_added);
...
PLA->
F = map_symbolic_cover(PLA->F, p1->symbolic_list, base);
PLA->
D = map_symbolic_cover(PLA->D, p1->symbolic_list, base);
PLA->
R = map_symbolic_cover(PLA->R, p1->symbolic_list, base);
base += 1 << p1->
symbolic_list_length;
new_part_size[newvar++] = 1 << p1->
symbolic_list_length;
}
...
for (
i = 0;
i < cube.first_part[cube.output]; i++) {
...
for (
i  = 0;
i < (1 << p1->symbolic_list_length); i++) {
if (p3 ==
NIL(symbolic_label_t)
) {
PLA->label[base+i] =
...
foreach_set(PLA->F, last, p) {
if (
setp_implies(present_state_mask, p
)) { /* from any state ! */
...
F     = sf_append(F, tF);
if (verbose_mode) {
(void) printf("# state EVERY to %d, before=%d after=%d\n",
i, before, after);
}
}
}
...
foreach_set(PLA->F, last, p) {
if (
setp_disjoint(p, next_state_mask
)) { /* no next state !! */
...
if (verbose_mode) {
(void) printf("# state ANY to NOWHERE, before=%d after=%d\n", before, after);
}
...
F     = sf_append(F, tF);
if (verbose_mode) {
(void) printf("# state %d to %d, before=%d after=%d\n",
i, j, before, after);
}
}
}
...
part_size = ALLOC(
int, cube.num_vars);
cube.part_size[cube.num_binary_vars] =
nstates;
cube.part_size[cube.num_binary_vars+1] =
nstates;
cube.part_size[cube.num_binary_vars+2] = nout -
nstates;

cube_setup();

foreach_set(PLA->F, last, p) {
kiss_print_cube(stdout, PLA, p,
"~1");
}
}
