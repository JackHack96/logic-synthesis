    ...
    foreach_set(F, last, p) {
        ...
        if (debug & IRRED1) {
            (void) printf("IRRED1: zr=%d ze=%d to-go=%d time=%s\n", R->count,
                          (*E)->count, F->count - (R->count + (*E)->count),
                          print_time(ptime()));
        }
    }
    free_cubelist(FD);
    ...
    foreach_set(R, last, p) {
        ...
        if (debug & IRRED1) {
            (void) printf("IRRED1: zr=%d zrt=%d to-go=%d time=%s\n", (*Rp)->count,
                          (*Rt)->count, R->count - ((*Rp)->count + (*Rt)->count),
                          print_time(ptime()));
        }
    }
    free_cubelist(ED);
...
sm_matrix *irred_derive_table(pcover D, pcover E, pcover Rp) {
    ...
    foreach_set(D, last, p) { RESET(p, REDUND); }
    foreach_set(E, last, p) { RESET(p, REDUND); }

    /* Mark each cube in Rp as partially redundant */
    foreach_set(Rp, last, p) { SET(p, REDUND); /* belongs to redundant set */ }
    ...
    foreach_set(Rp, last, p) {
        ...
        if (debug & IRRED1) {
            (void) printf("IRRED1: %d of %d to-go=%d, table=%dx%d time=%s\n", i,
                          Rp->count, Rp->count - i, table->nrows, table->ncols,
                          print_time(ptime()));
        }
...
bool cube_is_covered(pcube *T, pcube c) { return tautology(cofactor(T, c)); }
...
bool tautology(pcube *T) {
    ...
    if ((result = taut_special_cases(T)) == MAYBE) {
        ...
        result =
                tautology(scofactor(T, cl, best)) && tautology(scofactor(T, cr, best));
        free_cubelist(T);
        free_cube(cl);
        free_cube(cr);
    }
    ...
    if (debug & TAUT) {
        (void) printf("exit TAUTOLOGY[%d]: %s\n", --taut_level, print_bool(result));
    }
    return result;
}
...
/* Check for a column of all 0's which implies no tautology */
    ...
    if (cdata.vars_unate == cdata.vars_active) {
        ...
        *Tsave++                    = NULL;
        ...
        if (debug & TAUT) {
            (void) printf("UNATE_REDUCTION: %d unate variables, reduced to %d\n",
...
static void fcube_is_covered(pcube *T, pcube c, sm_matrix *table) { ftautology(cofactor(T, c), table); }
