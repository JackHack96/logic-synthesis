    ...
    for (var = 0; var < cube.num_binary_vars; var++)
        switch (io_getc(fp)) {
            ...
            case '\t': var--;
                break;
            case '2':
            case '-': set_insert(cf, var * 2 + 1);
            case '0': set_insert(cf, var * 2);
                break;
...
cube.
num_vars = cube.num_binary_vars + 1;
cube.
part_size = ALLOC(
int, cube.num_vars);
}
...
read_symbolic(fp, PLA, word,
&newlist)) {
if (PLA->symbolic_output ==
NIL(symbolic_t)
) {
PLA->
symbolic_output = newlist;
} else {
for (
...
for (
i = cube.first_part[cube.num_vars - 1];
i <=
last;
i++)
if ((
...
if (trace) {
totals(time, READ_TIME, PLA
->F, &cost);
}

/* Decide how to break PLA into ON-set, OFF-set and DC-set */
time = ptime();
if (pos || PLA->phase != NULL || PLA->symbolic_output !=
NIL(symbolic_t)
) {
needs_offset = TRUE;
}
...
->R);
PLA->
R = complement(cube2list(PLA->F, PLA->D));
...
EXECUTE(PLA
->
R = complement(cube2list(PLA->F, PLA->D)), COMPL_TIME, PLA
->R, cost);
}
}
        ...
        pPLA PLA;
{
    ...
    if (PLA->symbolic_output != NIL(symbolic_t)) {
        for (p1 = PLA->symbolic_output; p1 != NIL(symbolic_t); p1 = p1->next) {
            (void) printf("# output symbolic: ");
            for (p2 = p1->symbolic_list; p2 != NIL(symbolic_list_t); p2 = p2->next) {
                (void) printf(" %d", p2->pos);
            }
            (void) printf("\n");
        }
    }
        ...
        int *ip;
{
    ...
    if (PLA->label == NIL(char *) || PLA->label[0] == NIL(
    char)) {
