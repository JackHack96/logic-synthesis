...
void skip_line(register FILE *fpin, register FILE *fpout, register bool echo) {
    register int ch;
    ...
    for (var = cube.num_binary_vars; var < cube.num_vars - 1; var++)
        ...
        if (cube.part_size[var] < 0) {
            ...
            if (equal(token, "-") || equal(token, "ANY")) {
                ...
                if (i > last) {
                    (void) fprintf(stderr,
                                   "declared size of variable %d (counting from variable "
                                   "0) is too small\n",
                                   var);
                    exit(-1);
                }
            ...
            if (
                ...
                if (cube.fullset != NULL) {
                    ...
                    if (cube.num_vars < cube.num_binary_vars)
                        fatal("num_vars (1st field of .mv) must exceed num_binary_vars (2nd "
                              "field of .mv)");
                    cube.part_size = ALLOC(int, cube.num_vars);
...
int read_pla(IN FILE *fp, IN bool needs_dcset, IN bool needs_offset, IN int pla_type, OUT pPLA *PLA_return) {
    ...
    if (pos) {
        ...
        PLA->R     = onset;
        PLA->phase = new_cube();
        (void) set_diff(PLA->phase, cube.fullset, cube.var_mask[cube.num_vars - 1]);
...
void PLA_summary(pPLA PLA) {
    ...
    if (cube.num_binary_vars == cube.num_vars - 1)
        ...
        (void) printf(" with %d variables (%d binary, mv sizes", cube.num_vars,
                      cube.num_binary_vars);
        for (var = cube.num_binary_vars; var < cube.num_vars; var++)
