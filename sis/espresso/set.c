...
static int intcpy(register unsigned int *d, register unsigned int *s, register long n) {
    register int i;
    for (i = 0; i < n; i++) {
        *d++ = *s++;
    }
...
pset_family sf_save(register pset_family A) { return sf_copy(sf_new(A->count, A->sf_size), A); }
...
void sf_delset(pset_family A, int i) { (void) set_copy(GETSET(A, i), GETSET(A, --A->count)); }
...
pset_family sf_bm_read(FILE *fp) {
    ...
    for (i = 0; i < rows; i++) {
        ...
        for (j = 0; j < cols; j++) {
            switch (getc(fp)) {
                case '0':break;
                case '1':set_insert(pdest, j);
                    break;
                default:fatal("Error reading set family");
            }
        }
        if (getc(fp) != '\n') {
