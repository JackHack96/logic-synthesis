...
static int intcpy(d, s, n) register unsigned int *d, *s;
register long n;
{
  register int i;
  for (i = 0; i < n; i++) {
...
int bit_index(a) register unsigned int a;
{
  register int i;
  if (a == 0)
    return -1;
...
int set_dist(a, b) register pset a, b;
{
  register int i, sum = 0;
  register unsigned int val;
  for (i = LOOP(a); i > 0; i--)
...
pset set_clear(r, size) register pset r;
int size;
{
...
pset set_fill(r, size) register pset r;
register int size;
{
  register int i = LOOPINIT(size);
  *r = i;
...
pset set_copy(r, a) register pset r, a;
{
...
pset set_and(r, a, b) register pset r, a, b;
{
...
pset set_or(r, a, b) register pset r, a, b;
{
  register int i = LOOP(a);
  PUTLOOP(r, i);
  do
...
pset set_diff(r, a, b) register pset r, a, b;
{
  register int i = LOOP(a);
  PUTLOOP(r, i);
  do
...
pset set_xor(r, a, b) register pset r, a, b;
{
  register int i = LOOP(a);
#ifdef IBM_WATC
  PUTLOOP(r, i);
...
pset set_merge(r, a, b, mask) register pset r, a, b, mask;
{
  register int i = LOOP(a);
  PUTLOOP(r, i);
  do
...
bool set_andp(r, a, b) register pset r, a, b;
{
  register int i = LOOP(a);
...
bool set_orp(r, a, b) register pset r, a, b;
{
  register int i = LOOP(a);
  register unsigned int x = 0;
  PUTLOOP(r, i);
...
bool setp_empty(a) register pset a;
{
  register int i = LOOP(a);
  do
    if (a[i])
...
bool setp_full(a, size) register pset a;
register int size;
{
  register int i = LOOP(a);
...
bool setp_equal(a, b) register pset a, b;
{
  register int i = LOOP(a);
  do
    if (a[i] != b[i])
...
bool setp_disjoint(a, b) register pset a, b;
{
  register int i = LOOP(a);
  do
    if (a[i] & b[i])
...
bool setp_implies(a, b) register pset a, b;
{
  register int i = LOOP(a);
  do
    if (a[i] & ~b[i])
...
pset_family sf_active(A) pset_family A;
{
...
pset_family sf_inactive(A) pset_family A;
{
...
pset_family sf_append(A, B) pset_family A, B;
{
  long asize = A->count * A->wsize;
  long bsize = B->count * B->wsize;
...
pset_family sf_read(fp) FILE *fp;
{
  int i, j;
  register pset p, last;
  pset_family A;
...
void set_write(fp, a) register FILE *fp;
register pset a;
{
...
pset_family sf_bm_read(fp) FILE *fp;
{
  int i, j, rows, cols;
  register pset pdest;
  pset_family A;
...
void set_adjcnt(a, count, weight) register pset a;
 ...
 */
int *sf_count_restricted(A, r) pset_family A;
register pset r;
{
  register pset p;
  register int i, base, *count;
 ...
 */
pset_family sf_delc(A, first, last) pset_family A;
int first, last;
 ...
 */
pset_family sf_addcol(A, firstcol, n) pset_family A;
int firstcol, n;
{
  int maxsize;
 ...
 *  slow operation.
 */
pset_family sf_delcol(A, firstcol, n) pset_family A;
 ...
 */
pset_family sf_copy_col(dst, dstcol, src, srccol) pset_family dst, src;
int dstcol, srccol;
{
 ...
 */
pset_family sf_compress(A, c) pset_family A; /* will be freed */
register pset c;
{
  register pset p;
  register int i, bcol;
 ...
 *   are to be retained.
 */
pset_family sf_permute(A, permute, npermute) pset_family A;
