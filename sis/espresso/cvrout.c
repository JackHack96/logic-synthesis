        ...
        int output_type;
{
    ...
    if (PLA->label != NIL(char *) && PLA->label[1] != NIL(
    char)
    ...
    PLA->label[cube.first_part[cube.output]] != NIL(
    char)
    && cube.output != -1) {
        (void) fprintf(fp, ".ob");
        ...
        pPLA PLA;
{
    ...
    for (i = 0; i < cube.part_size[cube.output]; i++) {
        ...
        foreach_set(PLA->F, last, p) if (is_in_set(p, i + cube.first_part[cube.output])) {
                if (firstor)
                    (void) printf("("), col += 1;
                else
                    (void) printf(" | ("), col += 4;
        ...
        int num;
{
    ...
    (void) printf("%s[%d]: ord(T)=%d\n", name, num, T->count);
    if (verbose_debug)
        foreach_set(T, last, p) (void) printf("%4d. %s\n", cnt++, pc1(p));
}
        ...
        pcover T;
{
    ...
    foreach_set(T, last, p) (void) printf("%s\n", pc1(p));
}
...
j++) {
if (
setp_equal(GETSET(A, i), GETSET(A, j))) {
weight[i]++;
