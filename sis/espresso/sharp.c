...
pcover cv_sharp(A, B) pcover A, B;
{
  pcube last, p;
  pcover T;
...
pcover cb_recur_sharp(c, T, first, last, level) pcube c;
pcover T;
int first, last, level;
{
  ...
  if (first == last) {
    ...
    if ((debug & SHARP) && level < 4) {
      (void)printf("# SHARP[%d]: %4d = %4d x %4d, time = %s\n", level,
  ...
  if (cdist0(a, b)) {
    ...
    for (var = 0; var < cube.num_vars; var++) {
      if (!setp_empty(set_and(temp, d, cube.var_mask[var]))) {
        (void)set_diff(temp1, a, cube.var_mask[var]);
        (void)set_or(GETSET(r, r->count++), temp, temp1);
      }
    }
  } else {
    r = sf_addset(r, a);
...
pcover make_disjoint(A) pcover A;
{
  pcover R, new;
...
pcover cv_dsharp(A, B) pcover A, B;
{
  register pcube last, p;
  pcover T;

  T = new_cover(0);
  foreach_set(A, last, p) { T = sf_union(T, cb_dsharp(p, B)); }
  return T;
}

/* cb1_dsharp -- disjoint-sharp product between a cover and a cube */
pcover cb1_dsharp(T, c) pcover T;
pcube c;
{
  pcube last, p;
  pcover R;
  ...
  R = new_cover(T->count);
  foreach_set(T, last, p) { R = sf_union(R, dsharp(p, c)); }
  return R;
}

/* cb_dsharp -- disjoint-sharp product between a cube and a cover */
pcover cb_dsharp(c, T) pcube c;
pcover T;
{
  pcube last, p;
  pcover Y, Y1;
  ...
  if (T->count == 0) {
    ...
    Y = new_cover(T->count);
    (void)set_copy(GETSET(Y, Y->count++), c);
  ...
  if (cdist0(a, b)) {
    ...
    for (var = 0; var < cube.num_vars; var++) {
      ...
      if (!setp_disjoint(diff, cube.var_mask[var])) {
        ...
        (void)set_or(mask, mask, cube.var_mask[var]);
...
pcover cv_intersect(A, B) pcover A, B;
{
