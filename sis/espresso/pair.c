        ...
        for (var2 = var1 + 1; var2 < cube.num_binary_vars; var2++) {
            ...
            switch (strategy) {
                ...
                case 0:
                    /*s = "weak division";*/
                    mask = new_cube();
                    (void) set_or(mask, cube.var_mask[var1], cube.var_mask[var2]);
...
generate_all_pairs(pair, cube
.num_binary_vars, candidate, minimize_pair);
        ...
        ppair pair;
{
    ...
    switch (pair_minim_strategy) {
        case 2: EXEC_S(phase_assignment(global_PLA, 0), "OPO       ", global_PLA->F);
            if (summary)
                (void) printf("# phase is %s\n", pc1(global_PLA->phase));
            break;
        case 1: EXEC_S(global_PLA->F = minimize_exact(global_PLA->F, global_PLA->D,
                                                      global_PLA->R, 1), "EXACT     ", global_PLA->F);
            break;
        case 0: EXEC_S(global_PLA->F = espresso(global_PLA->F, global_PLA->D,
