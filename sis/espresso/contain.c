...
*/
pset_family d1merge(INOUT pset_family A, IN int var) { return dist_merge(A, cube.var_mask[var]); }

/* d1_rm_equal -- distance-1 merge (merge cubes which are equal under a mask) */
int d1_rm_equal(register pset *A1, int (*compare)()) {
...
pset_family sf_unlist(IN pset *A1, IN int totcnt, IN int size) {
    ...
    R->count = totcnt;
    for (pr = R->data, pa = A1; (p = *pa++) != NULL; pr += R->wsize) INLINEset_copy(pr, p);
    FREE(A1);
    return R;
}
