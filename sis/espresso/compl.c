...
static pcover compl_merge(pcube *T1, pcover L, pcover R, register pcube cl, register pcube cr, int var, int lifting) {
    ...
    /* Perform lifting */
    switch (lifting) {
        case USE_COMPL_LIFT_ONSET:T = cubeunlist(T1);
            compl_lift_onset(L1, T, cr, var);
            compl_lift_onset(R1, T, cl, var);
            free_cover(T);
            break;
        case USE_COMPL_LIFT_ONSET_COMPLEX:T = cubeunlist(T1);
            compl_lift_onset_complex(L1, T, var);
            compl_lift_onset_complex(R1, T, var);
            free_cover(T);
            break;
        case USE_COMPL_LIFT:compl_lift(L1, R1, cr, var);
            compl_lift(R1, L1, cl, var);
            break;
        case NO_LIFTING:break;
        default:;
    }
    FREE(L1);
    FREE(R1);
 ...
 */
static void compl_d1merge(register pcube *L1, register pcube *R1) {
...
static pcover compl_cube(register pcube p) {
