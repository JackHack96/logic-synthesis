...
void check_args(int argc, char **argv) {
  ...
  while ((c = getopt(argc, argv, "s:S:d:D:IihHvVnN$")) > 0) {
    switch (toupper(c)) {
    case 'S':
      (void)strncpy(VSS, optarg, MAX_NAME_LENGTH);
      break;
    case 'D':
      (void)strncpy(VDD, optarg, MAX_NAME_LENGTH);
      break;
    case 'V':
      DEBUG = 1;
      break;
...
void print_signals(struct MODELstruct *model) {
  ...
  while (VECTptr->next != NULL) {
    ...
    if (VECTptr->dir)
      if (DEBUG)
        (void)fprintf(stderr, "\t%s [%d..%d]", VECTptr->name, VECTptr->start,
                      VECTptr->end);
      else if (DEBUG)
        (void)fprintf(stderr, "\t%s [%d..%d]", VECTptr->name, VECTptr->end,
                      VECTptr->start);
  }
