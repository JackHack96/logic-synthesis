    ...
    do {
        ...
        switch (next) {
            case sZERO:
                ...
                next = sWAIT;
                break;
            case sWAIT: if (DEBUG) (void) fprintf(stderr, "--sWAIT-- tok <%s>\n", w);
                if (*w == '(') {
                    /* is a vector */
                    next = sNUM;
                } else {
...
void print_signals(struct MODELstruct *model){
    ...
    while (VECTptr->next != NULL) {
        VECTptr = VECTptr->next;
        if (VECTptr->dir)
            if (DEBUG) (void) fprintf(stderr, "\t%s [%d..%d]", VECTptr->name, VECTptr->start, VECTptr->end);
...
void print_vst(struct MODELstruct *model){
    ...
    while (Iptr->next != NULL) {
        ...
        if (ADDPOWER) {
            (void) fprintf(Out, "\t%s => %s,\n\t%s => %s,\n\t%s => %s);\n\n", Fptr->name, Aptr->name, VSS, VSS, VDD,
                           VDD);
        } else {
            (void) fprintf(Out, "\t%s => %s);\n\n", Fptr->name, Aptr->name);
        }
