 ...
 *                         <> temp9 contains the final minimized cover : * (P =
 *union Mi for i=1,...,q)                *
...
symbolic_loop() {
  ...
  if ((fp9 = fopen(temp9, "w")) == NULL) {
    ...
    if (ISYMB) {
      sprintf(mvline, ".mv %d %d %d %d %d\n", 3, 0, inputnum, statenum,
              statenum + outputfield - 1);
      fputs(mvline, fp9);
    } else {
      sprintf(mvline, ".mv %d %d %d %d\n", inputfield + 2, inputfield, statenum,
              statenum + outputfield - 1);
      fputs(mvline, fp9);
    }
  }
    ...
    if (OUT_ALL || (!OUT_ALL && card_gain > 0)) {
      for (onjptr = outsymbol_list; onjptr != (OUTSYMBOL *)0;
        ...
        if (onjptr->nlab != oniptr->nlab) {
          ...
          sprintf(command, "espresso -Dintersect %s %s > %s", temp81, temp82,
                  temp83);
          system(command);
...
/*******************************************************************************
 *   Sorts the sets ONi according to a heuristic criterion - * Heuristic
 *implemented : to sort the sets ONi in descending order of        * cardinality
 *, so that the largest sets will benefit from larger don't      * care sets *
 *
 *******************************************************************************/
...
oncover_inter2(onjptr) OUTSYMBOL *onjptr;

{
  ...
  if (ISYMB) {
      sprintf(string1,".mv %d %d %d %d %d\n", 3, 0 , inputnum , statenum,
  1+outputfield-1); fputs(string1,fp82); } else { sprintf(string1,".mv %d %d %d
...
/*******************************************************************************
 *   Invokes the minimizer to get the output constraints * This version uses the
 *espresso-II version#2.0 multiple-valued minimizer    *
 *                                                                              *
 *   External files used : <> temp6 is the input to espresso (ON1i , OFFi) *
 *                         <> temp7 is the output from espresso (Mi) *
 *                         <> temp81 contains Mi with simplified output part *
...
int out_mini(oniptr) OUTSYMBOL *oniptr;

{
  ...
  for (outptr = outsymbol_list; outptr != (OUTSYMBOL *)0;
       outptr = outptr->next) {
    if (outptr->nlab != oniptr->nlab && not_offi(oniptr, outptr) == 1) {
      if ((fp1 = fopen(temp1, "r")) == NULL) {
        fprintf(stderr, "fopen: can't read temp1\n");
        exit(-1);
      } else {
...
not_offi(oniptr, outptr) OUTSYMBOL *oniptr;
OUTSYMBOL *outptr;

{
  ORDER_PATH *pathptr;

  for (pathptr = path_graph[oniptr->nlab]; pathptr != (ORDER_PATH *)0;
       pathptr = pathptr->next) {
    if (pathptr->dest == outptr->nlab)
      return (0);
  }
...
final_cover6(oniptr) OUTSYMBOL *oniptr;
...
final_cover7(oniptr) OUTSYMBOL *oniptr;
...
int count_prods(name) char *name;
{
