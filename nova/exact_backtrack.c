 ...
 *
 * $Source: /users/pchong/CVS/sis/nova/exact_backtrack.c,v $
 * $Author: pchong $
 * $Revision: 1.2 $
 * $Date: 2004/03/14 08:35:18 $
 *
 */
#include "inc/nova.h"
...
*   product of the linearly ordered sets of the integer dimensions that        *
*   each constraint of cardinality 1 can assume (cardinality of each such      *
*   set = maxdim_face - mindim_face for that constraint)                       *
*******************************************************************************/
...

{
    ...
    if (VERBOSE) printf("bktup_count = %d\n",bktup_count);
...
BOOLEAN backtrack_down(net_num,dim_cube)
int net_num;
int dim_cube;

{
	...
	face_found = assign_face(curptr,dim_cube,net_num);
		...
		if (DEBUG) printf("Backwards to %s\n", 
...
CODORDER_LINK *first_to_code(dim_cube)
int dim_cube;

{
    ...
    if ((ord_link = (CODORDER_LINK *) calloc((unsigned)1,sizeof(CODORDER_LINK))) == 
                                                         ( CODORDER_LINK *) 0) {
    ...
    if (DEBUG) printf("The first seed is : %s\n", 
...
/*******************************************************************************
*                    Selects a new constraint to code                          *
* Try the selection in the following priority order  :                         *
* 1) are there constraints of category 1 not already coded with the same       *
*  curdim_face as constrptr and having intersections in common with it ?       *
*  i.e. among the sons of constrptr having fathers of category 1 with the same *
*  curdim_face choose one and among its fathers choose one not already coded   *
* 2) are there constraints of category 1 not already coded with the same       *
*  curdim_face as constrptr ?                                                  *
* 3) are there constraints not already coded with the same curdim_face as      *
*  constrptr and having intersections in common with it ?                      *
...
* 4) are there constraints not already coded with the same curdim_face as      *
*  constrptr ?                                                                 *
* 5) are there constraints not already coded with a smaller curdim_face ?      *
*******************************************************************************/

CONSTRAINT_E *next_to_code(constrptr,dim_cube)
CONSTRAINT_E *constrptr;
int dim_cube;

{
	...
	for (i = graph_depth-1; i >= 0; i--) {
		    ...
		    free_mem(seed);
                    scanner->face_ptr->seed_valid = TRUE;
	            if (DEBUG) { printf("The seed is : %s\n", scanner->face_ptr->seed_value);
                    printf("next_to_code : branch n. 5\n"); }
	            return(scanner);
                }
            }
	}
...
CODORDER_LINK *insert_in_list(curptr,next_constr)
...

{
    ...
    if ( (cube_code = (char *) calloc((unsigned)dim_cube+1,sizeof(char))) == (char *) 0) {
        fprintf(stderr,"Insufficient memory for cube_code in start_code");
        exit(-1);
    }
    ...
    strcpy(graph_levels[graph_depth-1]->face_ptr->seed_value,cube_code);
    graph_levels[graph_depth-1]->face_ptr->seed_valid   = TRUE;
    strcpy(graph_levels[graph_depth-1]->face_ptr->first_value,cube_code);
    graph_levels[graph_depth-1]->face_ptr->first_valid  = TRUE;
    strcpy(graph_levels[graph_depth-1]->face_ptr->cur_value,cube_code);
    graph_levels[graph_depth-1]->face_ptr->code_valid   = TRUE;
    graph_levels[graph_depth-1]->face_ptr->count_index  =         1;
    graph_levels[graph_depth-1]->face_ptr->comb_index   =         1;
    graph_levels[graph_depth-1]->face_ptr->lexmap_index =         1;
...
char *copy_seed(dim_cube,cur_face)
int dim_cube;
char *cur_face;

{
    ...
    if ( (seed_code = (char *) calloc((unsigned)dim_cube+1,sizeof(char))) == (char *) 0) {
...
char *start_seed(dim_cube,dim_face)
int dim_cube;
int dim_face;
...

{
    ...
    new_code = gen_newcode_cat1(curptr,
		curptr->constraint_e->face_ptr->seed_value,dim_cube);
    ...
    for (ord_temptr = curptr; ord_temptr != (CODORDER_LINK *) 0;
        ...
        if ( mylog2(minpow2(ord_temptr->constraint_e->card)) == level ) {
...
/*******************************************************************************
*              Assigns a face to the current constraint                        *
* Given a constraint, it assigns to it a face compatible with the partial      *
* assignment built up to now; at the same time, it assigns faces to those      *
* constraints of category 2, that are children of the constraint being         *
* currently encoded and have another father already encoded.                   *
* When unable to map a constraint to a face, it informs backtrack_down which   *
* takes actions.                                                               *
* Faces are generated calling the routines gen_newface_catx, and they are      *
* verified with help of the routines fathers_codes_ok and code_verify          *
*******************************************************************************/

BOOLEAN assign_face(curptr,dim_cube,net_num)
CODORDER_LINK *curptr;
int dim_cube;
int net_num;

{
	...
	/* checks if new_code satisfies the inclusion relations with
	   respect to the codes already assigned                      */
	    ...
	    ok_to_code = TRUE;
	    if (DEBUG) printf("code_verify verified the code\n");
	  ...
	  for (level = graph_depth-1; level >= 0; level--) {
	    for (scanner = graph_levels[level]; scanner != (CONSTRAINT_E *) 0;
		    ...
		    /* that have a non-void intersection with the current one */
                    inter_ptr = 
                               exist_son(scanner,curptr->constraint_e,dim_cube);

		    if ( inter_ptr != (CONSTRAINT_E *) 0 ) {

                        if (DEBUG) { printf("\nThere are non-empty intersections\n");
                        printf("%s intersects with ", 
						curptr->constraint_e->relation);
			...
			if ( inter_ptr->face_ptr->code_valid == FALSE ) {
			    /* intersection not already coded */
			       ...
			       break                                          */
                            if (DEBUG) { printf("codes = %s ", 
				     curptr->constraint_e->face_ptr->cur_value);
                            printf("and = %s ", scanner->face_ptr->cur_value); }
                            if ( inter_code != (char *) 0 ) {
				if (DEBUG) printf("intersect in %s (feasible code)\n", 
								    inter_code);
                            } else {
				if (DEBUG) printf("intersect in a non-feasible code\n");
			    }

                            if ( inter_code != (char *) 0 ) {

                                if ( fathers_codes_ok(inter_ptr,inter_code,
				                              dim_cube) == 1 &&
                                  code_verify(inter_ptr,inter_code,net_num,
				                              dim_cube) == 1 ) {
	                            if (DEBUG) printf("code_verify verified the code\n");
			            ok_to_code = TRUE;
				    /* a face is assigned to the intersection */
                                    strcpy(inter_ptr->face_ptr->first_value,inter_code); 
                                    inter_ptr->face_ptr->first_valid = 
								     TRUE;
				    ...
				    set_codfather(curptr->constraint_e,
							       inter_ptr);
			    ...
			    /* intersection already coded */
		            if (DEBUG) printf("Its already assigned code is : %s\n", 
						inter_ptr->face_ptr->cur_value);
			    if ( strcmp(inter_code,
                                        inter_ptr->face_ptr->cur_value) == 0 ) {
				if (DEBUG) printf("Codes are compatible\n");
				ok_to_code = TRUE;
                                free_mem(inter_code);

			    } else {
				if (DEBUG) printf("Codes are incompatible\n");
	                        ok_to_code = FALSE ; 
                                free_mem(inter_code);
				break;
...
* constraint ?                                                                 *
* i.e., among the children of constrptr1 is there any who is also a child of   *
* constrptr2 ?                                                                 *
* if yes and the dimension of the intersection of the fathers' faces is <=     *
*            than the current face dimension of the son                        *
...
*                     b) or there is no intersection constraint)               *
* a pointer to nil is returned .                                               *
* In case b) the reason is that there is more than 1 possible code and we can  *
* generate these codes only via invoking gen_newcode_cat2 at the upper level,  *
* while we can code intersections as a byproduct of other coding operations    *
* only when they can have at most one code ; notice that this was a practical  *
* design decision due to a later understanding of this circumstance            *
*******************************************************************************/

CONSTRAINT_E *exist_son(constrptr1,constrptr2,dim_cube) 
...

{
    ...
    for (son_scanner = constrptr1->down_ptr; son_scanner != (SONS_LINK *) 0;
				           son_scanner = son_scanner->next ) {
        for (ft_scanner = son_scanner->constraint_e->up_ptr; 
                           ft_scanner != (FATHERS_LINK *) 0;
					       ft_scanner = ft_scanner->next) {
...

{
    ...
    for (son_scanner = constrptr1->down_ptr; son_scanner != (SONS_LINK *) 0;
	    ...
	    son_scanner->codfather_ptr = constrptr1;
	    if (DEBUG) printf("The codfather of %s is %s\n", constrptr2->relation , 
...

{
    ...
    if ( (ft_inter = (char *) calloc((unsigned)dim_cube+1,sizeof(char))) == (char *) 0) {
	fprintf(stderr,"Insufficient memory for ft_inter in fathers_codes_ok");
	exit(-1);
    }
    ...
    if ( refptr->up_ptr->next == (FATHERS_LINK *) 0 ) {
	    ...
	    if ( face1_incl_face2(refptr->up_ptr->constraint_e->face_ptr->cur_value,
				                         code,dim_cube) == 1 ) {
		...
		prov_ptr = ft_inter;
		  ...
		  father_scanner->constraint_e->face_ptr->cur_value,0,dim_cube);
		free_mem(prov_ptr);
        ...
        /* 2.1) if the dimension of the intersection of the faces assigned
           ...
           ditto intersection must contain (also not properly) the face
           proposed for the son */
        if (dim_face(ft_inter,dim_cube) > refptr->face_ptr->curdim_face) {
	    if ( face1_incl2_face2(ft_inter,code,dim_cube) == 1 ) {
	        /*printf(
...
char *fathers_codes_inter(refptr,dim_cube)
CONSTRAINT_E *refptr;
int dim_cube;

{
    ...
    if ( refptr->up_ptr->next == (FATHERS_LINK *) 0 ) {
        if ( refptr->up_ptr->constraint_e->face_ptr->code_valid != FALSE ) {
            if (DEBUG) { printf("%s ", refptr->up_ptr->constraint_e->relation);
        ...
        for (father_scanner = refptr->up_ptr; father_scanner != (FATHERS_LINK *) 0;
				       father_scanner = father_scanner->next ) {
            if ( father_scanner->constraint_e->face_ptr->code_valid != FALSE ) {
                if (DEBUG){ printf("%s ",father_scanner->constraint_e->relation);
                printf("with code %s \n", 
 	...
 	if (DEBUG) printf("and intersection = %s \n",ft_inter); 
...
/*******************************************************************************
* The verifications of a new proposed code carried on by this routine are :    *
* 1) an assigned face shouldn't coincide with the new one                      *
* 2) if an assigned face includes properly the new one, the old constraint     *
*    should be a father of the new one                                         *
* 3) if the new face includes properly a face already assigned, the new        *
*    constraint should be a father of the old one                              *
* 4) if an assigned face has a non-empty intersection with the new one (and    *
*    both of them contain properly the intersection), the related constraints  *
*    should have a non_empty intersection                                      *
*    A last check helpful for efficiency, not necessary for correcteness :     *
* 5) if an already coded constraint (not of cardinality 1) has a nonempty      *
...

{
	...
	for (constrptr = graph_levels[i]; constrptr != (CONSTRAINT_E *) 0;
				         constrptr = constrptr->next ) {
            if ( constrptr != refptr && constrptr->face_ptr->code_valid != 
								  FALSE ) {

                /* 1) an assigned face must not coincide with the new one */
                if ( strcmp(constrptr->face_ptr->cur_value,code) == 0 ) {
		    if (DEBUG) printf("The code already exists\n");
		    return(0);
		}

                /* 2) if an assigned face includes properly the new one the old
		      constraint must be a father of the new one              */
		if ( face1_incl_face2(constrptr->face_ptr->cur_value,code,
		                                              dim_cube) == 1 ) {
                    if ( p_incl_s(constrptr->relation,refptr->relation) == 0 ) {
		        if (DEBUG) printf("%s includes properly %s but %s is not father of %s\n",
		...
		if ( face1_incl_face2(code,constrptr->face_ptr->cur_value,
                                                       	      dim_cube) == 1 ) {
                    if ( p_incl_s(refptr->relation,constrptr->relation) == 0 ) {
		        if (DEBUG) printf("%s includes properly %s but %s is not father of %s\n",
			 code, constrptr->face_ptr->cur_value, 
			 refptr->relation, constrptr->relation );
			return(0);
		    }
		} 
		      ...
		      intersection                                            */
		inter_code = a_inter_b(constrptr->face_ptr->cur_value,code,0,
								     dim_cube); 
		if ( inter_code != (char *) 0  &&
		     face1_incl_face2(constrptr->face_ptr->cur_value,inter_code,
							       dim_cube) == 1 &&
		     face1_incl_face2(code,inter_code,dim_cube) == 1 ) {
		    inter_constr = c1_inter_c2(constrptr->relation,
					              refptr->relation,net_num);
                    if ( inter_constr == (char *) 0 ) {
                        if (DEBUG) printf("%s and %s intersect but %s and %s don't\n", 
			              constrptr->face_ptr->cur_value, code, 
			                 constrptr->relation, refptr->relation);
                        free_mem(inter_code);
		        return(0);
		        ...
		        inter_constr = c1_inter_c2(constrptr->relation,
					              refptr->relation,net_num);
                        if ( inter_constr != (char *) 0 ) { 
		          inter_code = a_inter_b(constrptr->face_ptr->cur_value,
                                   code,card_c1(inter_constr,net_num),dim_cube);
                          if ( inter_code == (char *) 0 ) {
                              if (DEBUG) printf("%s & %s intersect but %s and %s don't\n", 
			                 constrptr->relation, refptr->relation,
			                  constrptr->face_ptr->cur_value, code);
                              free_mem(inter_constr);
		              return(0);
	...
	if (DEBUG) printf("\n\noutput verification of state %d (%s)", state_pos1, code);
        for (constrptr = graph_levels[0]; constrptr != (CONSTRAINT_E *) 0;
				             constrptr = constrptr->next ) {
		  ...
		  code of state_pos2 ought to cover the code of state_pos1 */
		   ...
		   if ( codei_covers_codej(constrptr->face_ptr->cur_value,code)
				                                	== 0 ) {
		       if (DEBUG) printf("%d doesn't cover %d",state_pos2,state_pos1);
                       return(0);
		   }
		   if (DEBUG) printf("%d covers %d",state_pos2,state_pos1);
	       }
		  ...
		  code of state_pos1 ought to cover the code of state_pos2 */
		   ...
		   if ( codei_covers_codej(code,constrptr->face_ptr->cur_value)
				                                	== 0 ) {
		       if (DEBUG) printf("%d doesn't cover %d",state_pos1,state_pos2);
                       return(0);
		   }
		   if (DEBUG) printf("%d covers %d",state_pos1,state_pos2);
	       }
	   }
        }
    }
...
/*******************************************************************************
*          updates the counters of the # of codes tried up to now              *
*          and prints the # of codes tried in the last  backtrack_down         *
*                                                                              *
*  "local_work" counts the codes tried in the last backtrack_down              *
*  "cube_work"  counts the codes tried in the current cube                     *
*  "total_work" counts the codes tried in overall by exact_code                *
*******************************************************************************/
...
int locate_state(relation,net_num)
char *relation;
int net_num;
