...
undo_face(backtrackptr) CODORDER_LINK *backtrackptr;
...
code_complete_constr(dim_cube, dim_face) int dim_cube;
int dim_face;

{
  ...
  if ((cube_code = (char *)calloc((unsigned)dim_cube + 1, sizeof(char))) ==
      (char *)0) {
...
assign_face_tofirst(curptr, dim_cube) CODORDER_LINK *curptr;
int dim_cube;
...
set_codfather(constrptr1, constrptr2) CONSTRAINT_E *constrptr1;
CONSTRAINT_E *constrptr2;
...
fathers_codes_ok(refptr, code, dim_cube) CONSTRAINT_E *refptr;
char *code;
int dim_cube;
...
code_verify(refptr, code, net_num, dim_cube) CONSTRAINT_E *refptr;
char *code;
int net_num;
int dim_cube;
  ...
  for (i = graph_depth - 1; i >= 0; i--) {
    for (constrptr = graph_levels[i]; constrptr != (CONSTRAINT_E *)0;
      ...
      if (constrptr != refptr && constrptr->face_ptr->code_valid != FALSE) {
        ...
        if (face1_incl_face2(constrptr->face_ptr->cur_value, code, dim_cube) ==
            1) {
        ...
        if (face1_incl_face2(code, constrptr->face_ptr->cur_value, dim_cube) ==
            1) {
    ...
    for (constrptr = graph_levels[0]; constrptr != (CONSTRAINT_E *)0;
      ...
      if (constrptr != refptr && constrptr->face_ptr->code_valid != FALSE) {
        ...
        if (select_array[state_pos1] == USED &&
          ...
          if (codei_covers_codej(constrptr->face_ptr->cur_value, code) == 0) {
            if (DEBUG)
        ...
        if (select_array[state_pos2] == USED &&
          ...
          if (codei_covers_codej(code, constrptr->face_ptr->cur_value) == 0) {
            if (DEBUG)
