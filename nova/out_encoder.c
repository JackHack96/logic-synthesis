...
out_encoder(bits) int bits;
{
  ...
  for (i = 0; i < n_nodes; i++) {
    ...
    while (try_encoding(graph[node], soln, pow2_num, encoding) == FAIL) {
...
powers(num, num_pow2) int num, *num_pow2;
{
  int i, j;
 ...
 */
read_constraints(graph, dag) pgraph graph[MAXNODE];
t_dag dag[MAXNODE];
{
  FILE *fpin;
  char line[MAXLINE];
...
update_dag(graph, dag, count) pgraph graph[MAXNODE];
t_dag dag[MAXNODE];
int count;
{
  int i;
...
change_graph(graph, n_nodes) pgraph graph[MAXNODE];
int n_nodes;
{
 ...
 */
get_next_node(dag, n_nodes) t_dag dag[MAXNODE];
int n_nodes;
 ...
 */
try_encoding(constraint, soln, num, encoding) pgraph constraint;
int num;
t_solution soln;
char encoding[MAXDIM];
...
t_solution add_soln(soln, node, encoding) t_solution soln;
int node;
char encoding[MAXDIM];
...
print_soln(soln, num) t_solution soln;
int num;
{
