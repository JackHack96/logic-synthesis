...
fprintf(stderr,
"Please increase the memory allocated by: \n");
fprintf(stderr,
"#define MAXDIM     8+1 \n");
fprintf(stderr,
...
while (
try_encoding(graph[node], soln, pow2_num, encoding
)
== FAIL) {
...
if ((
fpin = fopen(constraints, "r")
) == NULL) {
fprintf(stderr,
"Error in opening file constraints\n");
exit(1);
}
count = 0;
while (
fgets(line, MAXLINE, fpin) != NULL) {
i     = 0;
index = getnum(line, &i);
count++;
graph[index] = NULL;
...
if (!found) {
fprintf(stderr,
"Achtung : Reached the unreachable\n");
exit(-1);
}
}
...
if (max_so_far == -1) {
fprintf(stderr,
"No node found : Graph not acyclic");
exit(-1);
} else {
return (max_so_far);
}
