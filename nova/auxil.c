...
int size(void) {
  ...
  if ((fp = fopen(temp4, "r")) == NULL) {
    ...
    if (fgets(line, MAXLINE, fp) != (char *)0 && myindex(line, ".i") != -1) {
      min_inputs = myatoi(line);
    }
    if (VERBOSE)
      printf("\nmin_inputs = %d\n", min_inputs);

    if (fgets(line, MAXLINE, fp) != (char *)0 && myindex(line, ".o") != -1) {
      min_outputs = myatoi(line);
    }
    if (VERBOSE)
      printf("min_outputs = %d\n", min_outputs);

    if (fgets(line, MAXLINE, fp) != (char *)0 && myindex(line, ".p") != -1) {
      min_products = myatoi(line);
    }
    if (VERBOSE)
      printf("min_products = %d\n", min_products);
  }
...
char *a_inter_b(char *a, char *b, int constr_card, int dim) {
  ...
  if ((new_code = (char *)calloc((unsigned)dim + 1, sizeof(char))) ==
      (char *)0) {
