...
sprintf(command,
"rm %s %s %s %s %s %s %s %s", temp6, temp7, temp81, temp82, temp83, temp9, temp10, constraints);
system(command);
}
...
sprintf(command,
"sis -o %s -t pla -T blif %s", dc, temp33);
system(command);
} else {
read_script(esp);
sprintf    (command,
"sis -f %s -o %s -t pla -T blif %s", readscript, dc, esp);
system (command);
sprintf(command,
"rm %s", readscript);
system(command);
...
sprintf(command,
"rm %s %s %s %s %s %s", temp1, temp2, temp3, temp33, temp4, temp5);
system(command);
...
if (!ONEHOT) {
sprintf(command,
"sis -o %s -t pla -T blif %s", blif, esp);
system(command);
} else {
sprintf(command,
"cp %s %s", dc, blif);
...
if ((
readscriptfile = fopen(readscript, "w")
) == NULL) {
fprintf(stderr,
"Error in opening readscript in read_script\n");
exit(-1);
} else {
sprintf(string1,
"read_pla -s %s\n", sfile);
