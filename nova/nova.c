...
main(argc, argv) int argc; /* number of command-lines arguments the program was
                              invoked with */
char *argv[]; /* pointer to an array of character strings that contain the
...

{
  ...
  if (!L_BOUND && !USER && !I_EXACT && !I_HYBRID && !IO_HYBRID && !IO_VARIANT &&
      !I_GREEDY && !RANDOM && !ONEHOT && !I_ANNEAL) {
    I_GREEDY = TRUE;
  }
  ...
  if (IO_HYBRID) {
    ...
    sprintf(command, "rm %s %s %s %s %s %s %s %s", temp6, temp7, temp81, temp82,
            temp83, temp9, temp10, constraints);
    system(command);
  }
  ...
  if (!ONEHOT) {
    ...
    system(command);
    sprintf(command, "rm %s", readscript);
    system(command);
    /* or replace esp by temp3 in the else clause */
  }
  sprintf(command, "rm %s %s %s %s %s %s", temp1, temp2, temp3, temp33, temp4,
          temp5);
  system(command);
  ...
  if (SIS) {
    ...
    if (!ONEHOT) {
      ...
      sprintf(command, "cp %s %s", dc, blif);
      system(command);
    ...
    sprintf(command, "rm %s %s", dc, blif);
    system(command);
    printf("# .end_network\n");
  }
  if (PLA_OUTPUT) {
    sprintf(command, "cat %s", esp);
    system(command);
  }
  sprintf(command, "rm %s", esp);
  system(command);
...
nova_blif() {
  ...
  char line[MAXSTRING], i_line[MAXSTRING], idc_line[MAXSTRING],
      o_line[MAXSTRING], s_line[2 * MAXSTRING], l_line[MAXSTRING],
      w_line[2 * MAXSTRING], lorder_line[2 * MAXSTRING], string[2];
  char *tokptr, *strptr;
  int i, j, input_length, input_max, output_max;
  BOOLEAN found = FALSE;

  if (!INIT_FLAG)
    strcpy(init_state, states[0].name);
  /*printf("init_state = %s\n", init_state);*/
    ...
    while (fgets(line, sizeof(line), blifile)) {
      ...
      if (myindex(line, ".inputs") >= 0) {
        ...
        /* notice: in a previous edition pi's were intentionally
           ...
           (fixed in sept. 1993)                       */
        strcpy(idc_line, i_line);
        strcat(idc_line, " ");
        strcat(idc_line, s_line);
        strcat(idc_line, "\n");
        /*printf("%s\n", s_line);*/
