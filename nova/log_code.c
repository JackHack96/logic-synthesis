...
code_first(source, symblemes, num, code_length) int source;
SYMBLEME *symblemes;
int num;
int code_length;
              ...
              if (hypervertices[k].ass_status == FALSE) {
                ...
                for (l = 0; l < num; l++) {
                  ...
                  if (constrptr->relation[l] == ONE &&
                    ...
                    totdist =
                        totdist + hamm_dist(hypervertices[k].code,
...
int code_propagate(source, symblemes, num, code_length) int source;
SYMBLEME *symblemes;
int num;
int code_length;
...
code_next(source, symblemes, num, code_length) int source;
SYMBLEME *symblemes;
int num;
int code_length;
    ...
    if (hypervertices[k].ass_status == FALSE) {
      ...
      for (l = 0; l < num; l++) {
        ...
        if (symblemes[l].code_status != NOTCODED) {
          ...
          totdist = totdist + hamm_dist(hypervertices[k].code,
                                        symblemes[l].code, code_length);
        }
      }
...
codes_box(code_length) int code_length;
...
freeze(constrptr, symblemes, num, code_length) CONSTRAINT *constrptr;
SYMBLEME *symblemes;
int num;
int code_length;

{
  ...
  for (i = 0; i < (minpow2(constrptr->card) - constrptr->card); i++) {
    ...
    if (mindist <=
        ...
        perimeter(mylog2(minpow2(constrptr
