...
if ((
candidates = (int *) calloc((unsigned) (num + 1), sizeof(int))
) == (int *) 0) {
fprintf(stderr,
"Insufficient memory for candidates in sel_first\n");
exit(-1);
}
...
candidates[i] = candidates[i] + 10 + constrptr2->
weight;
}
}
}
...
for (
j = 0;
j<power(2, code_length); j++) {

if ((hypervertices[j].ass_status == FALSE) &&
(
hamm_dist(symblemes[i]
.code, hypervertices[j].code,
code_length) == 1)) {
...
fprintf(stderr,
"Insufficient memory for candidates in sel_next\n");
exit(-1);
}
