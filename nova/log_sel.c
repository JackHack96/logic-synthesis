...
sel_first(net, symblemes, num) CONSTRAINT *net;
SYMBLEME *symblemes;
int num;

{
  ...
  if ((candidates = (int *)calloc((unsigned)(num + 1), sizeof(int))) ==
      (int *)0) {
  ...
  if ((constrptr1 != (CONSTRAINT *)0) && (i >= 0)) {
    ...
    /*printf("\nSel_first: no constraints - search the first not coded
     * symbleme\n");*/
    for (selected = 0;
...
sel_propagate(net, symblemes, num, code_length) CONSTRAINT *net;
SYMBLEME *symblemes;
int num;
int code_length;
    ...
    if (symblemes[i].code_status == FIXED) {
      ...
      for (j = 0; j < power(2, code_length); j++) {
        ...
        if ((hypervertices[j].ass_status == FALSE) &&
            (hamm_dist(symblemes[i].code, hypervertices[j].code, code_length) ==
             1)) {
            ...
            if ((constrptr->attribute == NOTUSED) &&
              ...
              for (k = 0; k < num; k++) {
                ...
                if ((constrptr->relation[k] == ONE) &&
                  ...
                  for (link = symblemes[i].constraints; link != (CONSTRLINK *)0;
                       link = link->next) {

                    if (link->constraint->attribute == NOTUSED) {

                      score = score + (link->constraint->level + 1) *
...
sel_next(net, symblemes, num) CONSTRAINT *net;
SYMBLEME *symblemes;
int num;

{
  ...
  for (i = num - 1; i >= 0; i--) {
    for (constrptr1 = levelarray[i]; constrptr1 != (CONSTRAINT *)0;
      ...
      if (constrptr1 != (CONSTRAINT *)0 && jump == 1)
        break;
    }
    if (constrptr1 != (CONSTRAINT *)0 && jump == 1)
      break;
  ...
  if ((constrptr1 != (CONSTRAINT *)0) && (i >= 0)) {
    ...
    /*printf("\nSel_next: no constraints - search the first not coded
     * symbleme\n");*/
    for (selected = 0;
         (selected < num) && (symblemes[selected].code_status == FIXED);
         selected++)
