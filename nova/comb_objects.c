...
char *gen_newcode_cat1(curptr, seed_code, dim_cube) CODORDER_LINK *curptr;
char *seed_code;
int dim_cube;
  ...
  if (curptr->constraint_e->face_ptr->lexmap_index == 0) {
    ...
    if (curptr->constraint_e->face_ptr->lexmap_index == lex_max) {
      ...
      if (curptr->constraint_e->face_ptr->count_index < count_max) {
        new_code =
            counting(curptr->constraint_e->face_ptr->cur_value, dim_cube);
        /*printf("gen_newcode_cat1 returned = %s\n", new_code);*/
        ...
        if (curptr->constraint_e->face_ptr->comb_index < comb_max) {
          new_code =
              combinations(curptr->constraint_e->face_ptr->cur_value, dim_cube);
          /*printf("gen_newcode_cat1 returned = %s\n", new_code);*/
          curptr->constraint_e->face_ptr->count_index = 1;
...
char *gen_newcode_cat2(curptr, dim_cube) CODORDER_LINK *curptr;
int dim_cube;
...
char *newcode_cat2(curptr, dim_cube, num_x, dim_subcube,
                   fathers_intercode) CODORDER_LINK *curptr;
int dim_cube;
...

{
  ...
  int count_max, comb_max, lex_max;
  char *new_code, *new_subcode, *seed, *start_seed(), *insert_code(),
      *copy_code(), *combinations(), *counting();
  ...
  if (curptr->constraint_e->face_ptr->lexmap_index == 0) {
    ...
    if (curptr->constraint_e->face_ptr->lexmap_index == lex_max) {
      ...
      if (curptr->constraint_e->face_ptr->count_index < count_max) {
        new_subcode =
            counting(curptr->constraint_e->face_ptr->seed_value, dim_subcube);
        new_code = insert_code(new_subcode, fathers_intercode, dim_cube);
        /*printf("gen_newcode_cat2 returned = %s\n", new_code);*/
        (curptr->constraint_e->face_ptr->count_index)++;
        (curptr->constraint_e->face_ptr->lexmap_index)++;
        ...
        if (curptr->constraint_e->face_ptr->comb_index < comb_max) {
          new_subcode = combinations(curptr->constraint_e->face_ptr->seed_value,
                                     dim_subcube);
          new_code = insert_code(new_subcode, fathers_intercode, dim_cube);
          /*printf("gen_newcode_cat2 returned = %s\n", new_code);*/
          curptr->constraint_e->face_ptr->count_index = 1;
...
char *gen_newcode_cat3(curptr, dim_cube) CODORDER_LINK *curptr;
int dim_cube;

{
  ...
  father_code = curptr->constraint_e->up_ptr->constraint_e->face_ptr->cur_value;
  if (DEBUG)
  ...
  if (curptr->constraint_e->face_ptr->dim_index == 0) {
    curptr->constraint_e->face_ptr->dim_index = num_x;
    i = curptr->constraint_e->face_ptr->dim_index;
    new_code = newcode_cat3(curptr, dim_cube, i, dim_subcube, father_code);
  } else {
    /* a new code is generated with the current dim_index */
    i = curptr->constraint_e->face_ptr->dim_index;
    new_code = newcode_cat3(curptr, dim_cube, i, dim_subcube, father_code);
...
char *newcode_cat3(curptr, dim_cube, num_x, dim_subcube,
                   father_code) CODORDER_LINK *curptr;
int dim_cube;
int num_x;         /* num_x is the current face dimension of curptr */
int dim_subcube;   /* dim_subcube is the face dimension of the
              father of curptr */
...
char *insert_code(new_subcode, father_code, dim_cube) char *new_subcode;
char *father_code;
int dim_cube;

{
  ...
  if ((new_code = (char *)calloc((unsigned)dim_cube + 1, sizeof(char))) ==
      (char *)0) {
...
char *copy_code(code, dim_cube) char *code;
int dim_cube;
...
char *combinations(seed_code, dim_cube) char *seed_code;
int dim_cube;

{
  ...
  if ((output = (char *)calloc((unsigned)dim_cube + 1, sizeof(char))) ==
      (char *)0) {
...
char *counting(seed_code, dim_cube) char *seed_code;
int dim_cube;

{
  ...
  /* counting in base 2 */
  if ((binary = (int *)calloc((unsigned)dim_cube - num_x + 1, sizeof(int))) ==
      (int *)0) {
...
char *min_combinations(seed_code, dim_cube) char *seed_code;
int dim_cube;

{
  ...
  if ((buffer = (int *)calloc((unsigned)dim_cube + 2, sizeof(int))) ==
      (int *)0) {
    fprintf(stderr, "Insufficient memory for object");
    exit(-1);
  }

  if ((stack = (int *)calloc((unsigned)dim_cube + 2, sizeof(int))) ==
      (int *)0) {
