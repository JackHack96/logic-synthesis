 ...
 *
 * $Source: /users/pchong/CVS/sis/nova/log_precode.c,v $
 * $Author: pchong $
 * $Revision: 1.1.1.1 $
...

{
    ...
    for (constrptr = (*net); constrptr != (CONSTRAINT *) 0; 
        ...
        if ( (constrptr->card == 1) || 
             (constrptr->card > (minpow2(strlen((*net)->relation))/2)) ) {
...

{
     ...
     if ((newrelation = (char *) calloc((unsigned)(strlen((*net)->relation)+1),sizeof(char))) == (char *) 0) {
     ...
     do {
         ...
         for (constrptr1 = (*net); constrptr1 != (CONSTRAINT *) 0; 
             ...
             for (constrptr2 = constrptr1->next; constrptr2 != (CONSTRAINT *) 0;
                 ...
                 if ( card == 0 || 
                       card == 1 ||
                        card > (minpow2(strlen((*net)->relation))/2) ) {
                     continue;
                 }
    ...
    for ( temptr = net; temptr != (CONSTRAINT *) 0; temptr = temptr->next ) {
        ...
        for (i=0; i<num; i++) {
            if (temptr->relation[i] == ONE) {
                ...
                if ((newlink = (CONSTRLINK *) calloc((unsigned)1,sizeof(CONSTRLINK))) == ( CONSTRLINK *) 0) {
