...
* hypercube ( up to the closest power of two ) or deletes them                 *
* ( according to whether free positions on the hypercube are still available   *
* or not )                                                                     *
*******************************************************************************/
...
free_places = minpow2(strlen((*net)->relation)) -
              strlen((*net)->relation) - virtuals;
...
if ((
minpow2(temptr
->card) - temptr->card) <= free_places) {
free_places =
free_places - (minpow2(temptr->card) - temptr->card);
temptr->
odd_type = ADMITTED;
} else {
temptr->
...
if ((
newrelation = (char *) calloc((unsigned) (strlen((*net)->relation) + 1), sizeof(char))
) == (char *) 0) {
fprintf(stderr,
"Insufficient memory for newrelation in lattice\n");
exit(-1);
}
newrelation[
strlen((*net)
...
for (
constrptr1 = (*net);
constrptr1 != (CONSTRAINT *) 0;
constrptr1 = constrptr1->next
) {
if ( constrptr1->attribute == PRUNED  ||
constrptr1->level != level ) {
continue;
}
...
/* "constrptr2" points to the second operand of the intersection
   operation */
for (
constrptr2 = constrptr1->next;
constrptr2 != (CONSTRAINT *) 0;
constrptr2 = constrptr2->next
) {
if ( constrptr2->attribute == PRUNED  ||
constrptr2->level != level ) {
continue;
}
...
symblemes[i].
constraints = (CONSTRLINK *) 0;
symblemes[i].
code_status = NOTCODED;
...
if ((
newlink = (CONSTRLINK *) calloc((unsigned) 1, sizeof(CONSTRLINK))
) == ( CONSTRLINK *) 0) {
fprintf(stderr,
"Insufficient memory for newlink in link_constr");
exit(-1);
}
...
temptr->
levelnext = levelarray[temptr->level];
levelarray[temptr->level] =
temptr;
temptr->
cardnext = cardarray[temptr->card];
cardarray[temptr->card] =
temptr;
