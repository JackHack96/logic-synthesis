...
int anneal_code(CONSTRAINT *net, SYMBLEME *simboli, int node_num,
                int codelength) {
  /*
 ...
 */
assign_initial_encoding(SYMBLEME *simboli, int node_num, int codelength) {
  long time();
...
generate_state(simboli, type) SYMBLEME *simboli;
int *type;
{
  /*
...
should_accept(change_in_cost, temp) int change_in_cost;
double temp;
{
...
vert_alloc(codelength) int codelength;

{
  char module;
  int i, j, k;

  if ((vertices = (HYPERVERTEX *)calloc(
           (unsigned)codes_num, sizeof(HYPERVERTEX))) == (HYPERVERTEX *)0) {
    fprintf(stderr, "Insufficient memory for vertices");
    exit(-1);
  }

  for (i = 0; i < codes_num; i++) {
    if ((vertices[i].code = (char *)calloc((unsigned)codelength + 1,
                                           sizeof(char))) == (char *)0) {
