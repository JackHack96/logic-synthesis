...
/*******************************************************************************
 *   Verifies if the exact encoding algorithm satisfied all constraints * (as it
 *should!) - if it didn't , it stops the execution signalling         * the
 *event (it means that there is a bug in the program)                    *
 *******************************************************************************/

int exact_check(net, symblemes, net_num) CONSTRAINT **net;
SYMBLEME *symblemes;
int net_num;

{
  ...
  if ((face = (char *)calloc((unsigned)code_lgt + 1, sizeof(char))) ==
      (char *)0) {
...
excompl_states(null_code) int null_code;
