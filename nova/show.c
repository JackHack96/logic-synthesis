...
shownet(net, net_name, header) CONSTRAINT **net;
char *net_name;
char *header;

{
  ...
  for (scanner = (*net); scanner != (CONSTRAINT *)0; scanner = scanner->next) {
    printf("%s wgt:%d lev:%d nxst:%s ", scanner->relation, scanner->weight,
           scanner->level, scanner->next_states);
...
show_code(symblemes, num) SYMBLEME *symblemes;
int num;
...
show_exactcode(symblemes, num) SYMBLEME *symblemes;
int num;
...
show_bestcode(symblemes, num) SYMBLEME *symblemes;
int num;
...
show_exbestcode(symblemes, num) SYMBLEME *symblemes;
int num;
...
show_fathers() {
  ...
  for (i = graph_depth - 1; i >= 0; i--) {
    for (downconstr = graph_levels[i]; downconstr != (CONSTRAINT_E *)0;
      ...
      for (father_scanner = downconstr->up_ptr;
...
show_implicant() {
  ...
  for (scanner = implicant_list; scanner != (IMPLICANT *)0;
    ...
    case USED:
      ...
      break;
    default:
      printf("illegal: %d", scanner->attribute);
    }
    printf("%s", scanner->row);
  }
}

show_orderarray(net_num) int net_num;
