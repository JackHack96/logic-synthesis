...
exact_code(net_num) int net_num;
...
faces_alloc(net_num) int net_num;

{
  ...
  for (i = graph_depth - 1; i >= 0; i--) {
    for (constrptr_e = graph_levels[i]; constrptr_e != (CONSTRAINT_E *)0;
      ...
      if ((constrptr_e->face_ptr->seed_value = (char *)calloc(
               (unsigned)net_num + 1, sizeof(char))) == (char *)0) {
        fprintf(stderr, "Insufficient memory for seed_value in faces_alloc");
        exit(-1);
      }
      if ((constrptr_e->face_ptr->first_value = (char *)calloc(
               (unsigned)net_num + 1, sizeof(char))) == (char *)0) {
        fprintf(stderr, "Insufficient memory for first_value in faces_alloc");
        exit(-1);
      }
      if ((constrptr_e->face_ptr->cur_value = (char *)calloc(
               (unsigned)net_num + 1, sizeof(char))) == (char *)0) {
 ...
 *      be assigned to the constraints (function of dim_cube) - * it returns the
 *dimension of the hypercube : it may be increased         * with respect to its
 *input value as a consequence of the fact that       * the dimensions of the
 *faces of some constraints may need to be          * increased to be able to
 *contain other constraints at the same level     * (i.e. sons at the same
 *level)                                           *
 *******************************************************************************/

int faces_dim_set(dim_cube) int dim_cube;

{
  ...
  for (i = graph_depth - 2; i >= 0; i--) {
    for (constrptr = graph_levels[i]; constrptr != (CONSTRAINT_E *)0;
      ...
      fathers_num = 0;
      for (ft_scanner = constrptr->up_ptr; ft_scanner != (FATHERS_LINK *)0;
      ...
      if (fathers_num == 1) {
        ...
        if (constrptr->up_ptr->constraint_e == supremum_ptr) {
          ...
          if (constrptr->card == 1) {
            ...
            constrptr->face_ptr->mindim_face = mylog2(minpow2(constrptr->card));
            if (VERBOSE)
              printf("mindim_face = %d ", constrptr->face_ptr->mindim_face);
            constrptr->face_ptr->maxdim_face = constrptr->face_ptr->mindim_face;
            if (VERBOSE)
 ...
 ** the subdag rooted here - * if mindim_face of root_ptr is bigger than the
 *minimum dimension of the       * subgraph rooted here return mindim_face of
 *root_ptr (i.e., it can accomodate * the subgraph rooted here), * otherwise
 *update mindim_face & curdim_face of face_ptr to the minimum        * dimension
 *of the subgraph + 1 and return it (i.e., it needs a dimension      * larger by
 *one to accomodate the subgraph rooted here)                        *
 *******************************************************************************/

int max_tree(root_ptr) CONSTRAINT_E *root_ptr;

{
  ...
  if (root_ptr->down_ptr == (SONS_LINK *)0) {
    ...
    /* if mindim_face of root_ptr is bigger than the minimum dimension of
       ...
       here) */
    if (root_ptr->face_ptr->mindim_face > (local_max)) {
      /*printf("max_tree evaluates to %d\n",
                                       root_ptr->face_ptr->mindim_face);*/
      return (root_ptr->face_ptr->mindim_face);
    } else {
      root_ptr->face_ptr->mindim_face = local_max + 1;
      root_ptr->face_ptr->curdim_face = local_max + 1;
      /*printf("max_tree evaluates to %d\n", local_max +1);*/
      return (local_max + 1);
    }
  }
...
faces_maxdim_set(dim_cube) int dim_cube;
 ...
 *                                                                              *
 *  bktup_calls counts the # of face configurations tried in all * total_work
 *counts the # of codes tried in overall by exact_code            *
 *******************************************************************************/
