...
ihybrid_code(net, symblemes, net_name, net_num, code_length) CONSTRAINT **net;
SYMBLEME *symblemes;
char *net_name;
int net_num;
int code_length;
...
project_code(net, symblemes, net_name, net_num, code_length) CONSTRAINT **net;
SYMBLEME *symblemes;
char *net_name;
int net_num;
int code_length;

{
  ...
  if ((mask_constr = (int *)calloc((unsigned)net_num, sizeof(int))) ==
      (int *)0) {
  ...
  for (i = mindim_cube + 1; i <= code_length; i++) {
    ...
    while (last_constr != (CONSTRAINT *)0) {
      ...
      /*printf("last_constr->relation = %s\n", last_constr->relation);*/
      for (k = 0; k < net_num; k++) {
        if (last_constr->relation[k] == ONE)
          mask_constr[k] = 1;
        if (last_constr->relation[k] == ZERO)
          mask_constr[k] = 0;
        mask_move[k] = 0;
      }
      ...
      while (state_pos != -1) {
        ...
        action = face_action(net, symblemes, net_num, last_constr, mask_constr,
                             state_pos);
...
elong_code(symblemes, net_num) SYMBLEME *symblemes;
int net_num;
...
CONSTRAINT *inspect_net(net, symblemes, net_num) CONSTRAINT **net;
SYMBLEME *symblemes;
int net_num;
...
int state_sel(net, symblemes, net_num, mask) CONSTRAINT **net;
SYMBLEME *symblemes;
int net_num;
int *mask;

{
  ...
  for (i = 0; i < net_num; i++) {
    ...
    if (mask[i] == 1 && symblemes[i].exact_code[length - 1] == ZERO) {
      ...
      for (constrptr = (*net); constrptr != (CONSTRAINT *)0;
           constrptr = constrptr->next) {
        if (constrptr->attribute == NOTUSED && constrptr->relation[i] == ONE) {
          i_freq++;
        }
      }
      if (i_freq > max_freq) {
...
raise_code(symblemes, state_pos) SYMBLEME *symblemes;
int state_pos;
...
lower_code(symblemes, state_pos) SYMBLEME *symblemes;
int state_pos;
...
lower_codes(symblemes, net_num, mask_move) SYMBLEME *symblemes;
int net_num;
int *mask_move;
...
int face_action(net, symblemes, net_num, constrptr, mask,
                state_pos) CONSTRAINT **net;
SYMBLEME *symblemes;
int net_num;
CONSTRAINT *constrptr;
int *mask;
...

{
  ...
  if ((face = (char *)calloc((unsigned)face_length + 1, sizeof(char))) ==
      (char *)0) {
  ...
  for (j = 0; j < net_num; j++) {
    if (constrptr->relation[j] == ZERO) {
      if (inclusion(symblemes[j].exact_code, face, face_length) == 1) {
        ...
        satisfied = 0;
        if (symblemes[j].exact_code[face_length - 1] == ONE)
          new_cube = 1;
        /*printf("UNSATISFIED\n");*/
      }
    }
...
BOOLEAN feasible_move(net, symblemes, net_num, state) CONSTRAINT **net;
SYMBLEME *symblemes;
int net_num;
int state;
...
moreconstr_used(net, symblemes, net_num) CONSTRAINT **net;
SYMBLEME *symblemes;
int net_num;
...
faces_set(dim_cube) int dim_cube;
...
int num_constr(net) CONSTRAINT **net;
...
CONSTRAINT *update_net(net) CONSTRAINT **net;
...
faces_to_codes(symblemes, net_num) SYMBLEME *symblemes;
int net_num;
...
constr_satisfaction(net, symblemes, code_length, label) CONSTRAINT **net;
SYMBLEME *symblemes;
int code_length;
char *label;

{
  ...
  if ((face = (char *)calloc((unsigned)code_length + 1, sizeof(char))) ==
      (char *)0) {
...
random_patch(symblemes, net_num) SYMBLEME *symblemes;
int net_num;
