...
if (VERBOSE) {
printf("\nApprox_work = %d\n", approx_work);
constr_satisfaction(net, symblemes, dim_cube,
"exact");
}
...
if ((
mask_constr = (int *) calloc((unsigned) net_num, sizeof(int))
) == (int *) 0 ) {
fprintf(stderr,
"Insufficient memory for mask_constr in project_code");
}
if ((
mask_move = (int *) calloc((unsigned) net_num, sizeof(int))
) == (int *) 0 ) {
fprintf(stderr,
"Insufficient memory for mask_move in project_code");
}
...
if (VERBOSE) {
shownet(net, net_name,
"AFTER CUBE EXPANSION");
show_exactcode(symblemes, net_num
);
constr_satisfaction(net, symblemes, i,
"exact");
}
}
...
face_length = strlen(symblemes[0].exact_code);
if ((
face = (char *) calloc((unsigned) face_length + 1, sizeof(char))
) == (char *) 0) {
fprintf(stderr,
"Insufficient memory for face in moreconstr_used");
}
...
if ( constrptr->relation[j] == ZERO ) {
if (
inclusion(symblemes[j]
.exact_code, face, face_length) == 1) {
/*printf("the symbleme %s (%d)  is covered by face %s\n",symblemes[j].name, j, face);*/
satisfied = 0;
/*printf("UNSATISFIED\n");*/
break;
...
if ((
face = (char *) calloc((unsigned) code_length + 1, sizeof(char))
) == (char *) 0) {
fprintf(stderr,
"Insufficient memory for face in constr_satisfaction");
exit(-1);
}
