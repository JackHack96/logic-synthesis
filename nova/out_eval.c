...
"exact") == 0) {
if (
codei_covers_codej(states[i]
.exact_code, states[curptr->index].exact_code) == 1) {
ycover_count++;
}
}
if (
...
if (
codei_covers_codej(states[i]
.exbest_code, states[curptr->index].exbest_code) == 1) {
ycover_count++;
}
}
}
...
"exact") == 0) {
if (
codei_covers_codej(states[j]
.exact_code, states[i].exact_code) != 1) {
ROW = FALSE;
}
}
if (
...
if (
codei_covers_codej(states[j]
.exbest_code, states[i].exbest_code) != 1) {
ROW = FALSE;
}
}
}
