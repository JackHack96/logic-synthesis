...
out_performance(label) char *label;
  ...
  for (i = 0; i < statenum; i++) {
    for (curptr = order_graph[i]; curptr != (ORDER_RELATION *)0;
      ...
      if (strcmp(label, "exact") == 0) {
        if (codei_covers_codej(states[i].exact_code,
                               states[curptr->index].exact_code) == 1) {
          ycover_count++;
        }
      }
      if (strcmp(label, "exbest") == 0) {
        if (codei_covers_codej(states[i].exbest_code,
                               states[curptr->index].exbest_code) == 1) {
          ycover_count++;
        }
      }
    }
...
weighted_outperf(net_num, label) int net_num;
char *label;
  ...
  for (i = 0; i < net_num; i++) {
    ...
    for (j = 0; j < net_num; j++) {
      if (order_array[i][j] == ONE) {
        row_count++;
        if (strcmp(label, "exact") == 0) {
          if (codei_covers_codej(states[j].exact_code, states[i].exact_code) !=
              1) {
            ROW = FALSE;
          }
        }
        if (strcmp(label, "exbest") == 0) {
          if (codei_covers_codej(states[j].exbest_code,
                                 states[i].exbest_code) != 1) {
            ROW = FALSE;
          }
        }
      }
