...
/*******************************************************************************
 *             NECESSARY CONDITIONS ON THE HYPERCUBE DIMENSION * (to allow a
 *quick rejection of dimensions unfeasible for a given problem     * instance).
 ** The following necessary conditions that need to be satisfied by a solution *
 * have been found so far : * 1) first counting criterion - the k-face-poset
 *should have at least as many  * faces of a given cardinality as the instance
 *poset has constraints of a      * a given cardinality * 2) second counting
 *criterion - a constraint cannot have more fathers than    * the face of
 *minimum dimension (that can be assigned to it in the current     * hypercube)
 *has minimal including neighbours in the current hypercube         * 3) third
 *counting criterion - the current hypercube must have at least as    * many
 *free positions (#vertices - net_num) as the minimum number of imaginary *
...
least_dimcube(net_num) int net_num;

{
  ...
  if ((level_count = (int *)calloc((unsigned)graph_depth, sizeof(int))) ==
      (int *)0) {
    if (VERBOSE)
  ...
  while (dim_notfound == TRUE) {
    ...
    while ((i < graph_depth - 1) &&
           (comb_num(cube_dim, cube_dim - i) * power(2, cube_dim - i) >=
            level_count[i])) {
      /*printf("i = %d\n",i);
      printf("faces = %d\n",comb_num(cube_dim,cube_dim-i) *
      power(2,cube_dim-i)); printf("constraints = %d\n",level_count[i]);*/
      i++;
    }
    if (i == graph_depth - 1) {
      dim_notfound = FALSE;
  ...
  if (!((cube_dim == graph_depth - 1) || (level_count[graph_depth - 2] == 0))) {
    /* increase cube_dim to accomodate the complete constraint */
    cube_dim++;
  }

  if (VERBOSE)
    printf("cube_dim after first nec. cond. = %d\n", cube_dim);
  ...
  if (VERBOSE)
    printf("Second counting criterion\n");
  ...
  if (VERBOSE)
    printf("cube_dim after second nec. cond. = %d\n", cube_dim);
  ...
  if (VERBOSE)
    printf("Third counting criterion\n");
  ...
  for (i = graph_depth - 2; i >= 0; i--) {
    for (constrptr_e = graph_levels[i]; constrptr_e != (CONSTRAINT_E *)0;
      ...
      if (immag_num > 0) {
        if ((new_int = (INT_LIST *)calloc((unsigned)1, sizeof(INT_LIST))) ==
            (INT_LIST *)0) {
          if (VERBOSE)
  ...
  while (work > 0) {
    work = 0;
    for (int_scanner = immag_list;
         int_scanner != bound_ptr && int_scanner->next != (INT_LIST *)0;
         int_scanner = int_scanner->next) {
  ...
  if (VERBOSE)
    printf("cube_dim after third nec. cond. = %d\n", cube_dim);

  return (cube_dim);
}

BOOLEAN packing_cond(iter_num, net_num) int iter_num;
int net_num;
    ...
    for (constrptr_e = graph_levels[i]; constrptr_e != (CONSTRAINT_E *)0;
      ...
      if (minpow2(constrptr_e->card) - constrptr_e->card != 0) {
        ...
        for (constrptr2_e = constrptr1_e->next;
          ...
          if (diff != 0 && constrptr1_e->card == constrptr2_e->card &&
              iter_num == diff) {
            /*printf("constrptr2_e = %s\n", constrptr2_e->relation);*/
            card_inter = card_c1c2(constrptr1_e->relation,
                                   constrptr2_e->relation, net_num);
            /*printf("card_inter = %d\n", card_inter);*/
