...
if ((
LForder = (int **) calloc((unsigned) 2, sizeof(int *))
) == (int **) 0) {
fprintf(stderr,
"Insufficient memory for 'LForder'");
exit(-1);
}
for (
i = 0;
...
fprintf(stderr,
"Insufficient memory for LForder[i]");
exit(-1);
}
}

if ((
color = (int *) calloc((unsigned) ns, sizeof(int))
) == (int *) 0) {
fprintf(stderr,
"Insufficient memory for 'color'");
exit(-1);
}
...
if (mink != ns + 1) color[LForder[0][vi-1]-1] =
mink;
else { maxk++; color[LForder[0][vi-1]-1] =
maxk; }
}
colornum = maxk;
