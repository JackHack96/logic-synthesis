...
nova_summ() {
  ...
  if (IO_HYBRID || IO_VARIANT) {
    ...
    fprintf(fpcons, "# Cover (bit-wise covering relations among the codes of "
                    "the states)\n");
    fprintf(fpcons, "# Cover(i,j) = 1 iff the i-th state must be covered by "
                    "the j-th state\n");
