...
/*******************************************************************************
 *   Invokes the minimizer to get the input constraints * This version uses the
...
mini() {
  ...
  /* determine number and sizes of multiple-valued variables and write the
   * command ".mv ...parameters..." */
  if (TYPEFR)
    fputs(".type fr\n", fp);
  if (ISYMB) {
    sprintf(string1, ".mv %d %d %d %d %d\n", 3, 0, inputnum, statenum,
            outputfield + statenum - 1);
    fputs(string1, fp);
  } else {
...
mvlab(string, label, size) char string[MAXSTRING];
int label;
int size;
    ...
    while (fgets(line, MAXLINE, fp) != (char *)0) {
      ...
      if ((linelength == 1) || (myindex(line, "#") >= 0) ||
        ...
        if (ISYMB) {
          ...
          /* we found a new meaningful constraint , let's insert it in
           * "inputnet" */
          if (cont > 1 && cont < inputnum) {
            for (scanner = inputnet; scanner != (CONSTRAINT *)0;
                 scanner = scanner->next) {
              if (strcmp(binput, scanner->relation) == 0)
                break;
            }
