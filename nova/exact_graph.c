...
exact_graph(net, net_num) CONSTRAINT **net;
int net_num;
...
inter_graph(net_num) int net_num;

{
  ...
  if ((newrelation = (char *)calloc((unsigned)net_num + 1, sizeof(char))) ==
      (char *)0) {
    ...
    for (constrptr1 = graph_levels[i]; constrptr1 != (CONSTRAINT_E *)0;
      ...
      for (k = i; k >= 0; k--) {
        ...
        for (constrptr2 = graph_levels[k]; constrptr2 != (CONSTRAINT_E *)0;
          ...
          for (temptr = graph_levels[inter_level]; temptr != (CONSTRAINT_E *)0;
               temptr = temptr->next) {
...
/*******************************************************************************
 *   Adds to the graph of the constraints the following trivial ones : * 1) the
 *constraint including all symbols                                    * 2) the
 *constraints including only one symbol at a time                     *
 *******************************************************************************/

complete_graph(net_num) int net_num;
        ...
        for (sonconstr = graph_levels[j]; sonconstr != (CONSTRAINT_E *)0;
          ...
          if (p_incl_s(ftconstr->relation, sonconstr->relation) == 1 &&
            ...
            for (temptr1 = ftconstr->down_ptr; temptr1 != (SONS_LINK *)0;
              ...
              if (p_incl_s(temptr1->constraint_e->relation,
                           sonconstr->relation) == 1 &&
                  strcmp(temptr1->constraint_e->relation,
                         sonconstr->relation) != 0) {
                break;
              }
            }
            ...
            if (temptr1 == (SONS_LINK *)0 && temptr2 == (CONSTRAINT_E *)0) {
              if ((sons_link = (SONS_LINK *)calloc(
                       (unsigned)1, sizeof(SONS_LINK))) == (SONS_LINK *)0) {
                fprintf(stderr,
                        "Insufficient memory for sons_link in sons_graph");
                exit(-1);
              }
        ...
        for (ftconstr = graph_levels[j]; ftconstr != (CONSTRAINT_E *)0;
          ...
          if (p_incl_s(ftconstr->relation, sonconstr->relation) == 1 &&
            ...
            for (temptr1 = sonconstr->up_ptr; temptr1 != (FATHERS_LINK *)0;
              ...
              if (p_incl_s(ftconstr->relation,
                           temptr1->constraint_e->relation) == 1 &&
                  strcmp(ftconstr->relation, temptr1->constraint_e->relation) !=
                      0) {
                break;
              }
            }
            ...
            if (temptr1 == (FATHERS_LINK *)0 && temptr2 == (CONSTRAINT_E *)0) {
              if ((fathers_link = (FATHERS_LINK *)calloc(
                       (unsigned)1, sizeof(FATHERS_LINK))) ==
                  (FATHERS_LINK *)0) {
                fprintf(
                    stderr,
                    "Insufficient memory for fathers_link in fathers_graph");
                exit(-1);
              }
