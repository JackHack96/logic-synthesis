 ...
 *
 * $Source: /users/pchong/CVS/sis/nova/lower_bound.c,v $
 * $Author: pchong $
 * $Revision: 1.1.1.1 $
...

{
    ...
    int i,j,inp_length,st_length,ones,zeroes,i_col,j_col,ones_color,zero_color,
	bound;

    if ( (arg_array = (char **) calloc((unsigned)4,sizeof(char *))) == (char **) 0) {
    ...
    for ( i = 0; i < 2; i++ ) {
        if ( (arg_array[i] = (char *) calloc((unsigned)inp_length+1,sizeof(char))) == (char *) 0) {
    ...
    for ( i = 2; i < 4; i++ ) {
        if ( (arg_array[i] = (char *) calloc((unsigned)st_length+1,sizeof(char))) == (char *) 0) {
            fprintf(stderr,"Insufficient memory for arg_array[i]");
            exit(-1);
        }
    ...
    if (ones >0) {
        if ( (inco1_array = (int **) calloc((unsigned)ones,sizeof(int *))) == (int **) 0) {
            fprintf(stderr,"Insufficient memory for 'inco1_array'");
            exit(-1);
        }
        for ( i = 0; i < ones; i++ ) {
            if ( (inco1_array[i] = (int *) calloc((unsigned)ones,sizeof(int))) == (int *) 0) {
    ...
    if (zeroes >0) {
        if ( (inco0_array = (int **) calloc((unsigned)zeroes,sizeof(int *))) == (int **) 0) {
            fprintf(stderr,"Insufficient memory for 'inco0_array'");
            exit(-1);
        }
        for ( i = 0; i < zeroes; i++ ) {
            if ( (inco0_array[i] = (int *) calloc((unsigned)zeroes,sizeof(int))) == (int *) 0) {
...
int argument1(index,arg_array0,arg_array2)
int index;
char *arg_array0;
char *arg_array2;
...
int argument0(index,arg_array0,arg_array2)
int index;
char *arg_array0;
char *arg_array2;
...

{
	...
	if (cube1[i] != '-'  ||  cube2[i] != '-') {
            if (cube1[i] == cube2[i]  &&  cube1[i] != cube3[i]) return(0);
	}
    }
    return(1);
