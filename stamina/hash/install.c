...
NLIST *install(name, order, ptr, hashtab, hash_size) char *name;
...
NLIST *hashtab[];
{
  ...
  if ((np = lookup(name, hashtab, hash_size)) == NIL(NLIST)) {
    ...
    if ((np->name = str_save(name)) == NIL(char)) {
      (void)fprintf(stderr, "fail to copy a string, died in install.\n");
      exit(1);
    }
...
NLIST
*install_input(name, hashtab, hash_size) char *name;
/* char *ptr;		Pointer to first structure */
/* int order;		order index of entries in hashtab */
NLIST *hashtab[];
{
