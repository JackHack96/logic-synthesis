...
read_fsm(fp_input) FILE *fp_input;
{
  register i;
  char line[MAXSTRLEN];
  ...
  while (fgets(line, MAXSTRLEN, fp_input) != NULL) {
    ...
    if (item[0][0] == '.') {
      ...
      case 's':
        ...
        break;
      case 'r':
        if ((strcmp(item[1], "*") != 0) && (strcmp(item[1], "ANY") != 0)) {
    ...
    if ((strcmp(pstate_name, "*") != 0) && (strcmp(pstate_name, "ANY") != 0)) {
    ...
    if ((strcmp(nstate_name, "*") != 0) && (strcmp(nstate_name, "ANY") != 0)) {
...
void line_parser(line_buf) register char *line_buf;
{
  register char ch;
  int count;
