...
while (
fgets(line, MAXSTRLEN, fp_input ) != NULL) {
fputs(line, fp_temp
);
line_parser(line);
...
num_st = atoi(item[1]);
if ((
state_hash = hash_initial(num_st)
) ==
NIL(NLIST
*))  {
panic("ALLOC hash");
...
"ANY") != 0)) {
if (!(
install_state(item[1], state_hash, num_st
)))
panic("install fail");
}
user.stat.
...
next = NIL(EDGE);
edges[edge_index] =
edge_ptr;   /* save the address of new edge*/
edge_index++;
...
edge_ptr->
p_state = NIL(STATE); /* a null pointer */
...
edge_ptr->
n_state = NIL(STATE); /* a null pointer */
