...
push(state2, state1) {
#ifdef DEBUG
  (void)printf("Push %s %s \n", states[state1]->state_name,
               states[state2]->state_name);
#endif
  if (state2 > state1)
    _push(&xstack[(num_st - state1 - 2) * (num_st - state1 - 1) / 2 - state2 +
                  num_st - 1]);
  else
    _push(&xstack[(num_st - state2 - 2) * (num_st - state2 - 1) / 2 - state1 +
                  num_st - 1]);
}

static _push(operand) STACK *operand;
{
