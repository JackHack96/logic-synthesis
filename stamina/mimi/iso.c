...
iso_find() {
  ...
  for (i = 0; i < num_st; i++) {
    ...
    for (j = 0; j < num_st; j++) {
      if (i != j) {
        if (i > j)
          id = xstack[(num_st - j - 2) * (num_st - j - 1) / 2 - i + num_st - 1]
                   .status;
        else
          id = xstack[(num_st - i - 2) * (num_st - i - 1) / 2 - j + num_st - 1]
                   .status;
        if (id & COMPATIBLE) {
          iso[i].list[yy++] = j;
        }
  ...
  while (uncovered > 0) {
    ...
    for (i = 0, maxi = 0; i < user.stat.base_max; i++) {
      ...
      if (point > maxi) {
        ...
        /*
                                                if (max[max_id]->class.many >
           max[i]->class.many) { max_weight = weight; maxi=point; max_id=i;
                                                        small_num = max[i]->num;
                                                }
                                                else
                                                        if
           (max[max_id]->class.many == max[i]->class.many) if (max[i]->num <
           small_num) { max_weight = weight; maxi=point; max_id = i; small_num =
           max[i]->num;
                                                                }
        */
        if (weight > max_weight) {
          ...
          if (weight == max_weight) {
            if (max[max_id]->class.many > max[i]->class.many) {
              ...
              max_id = i;
              small_num = max[i]->num;
            } else if (max[max_id]->class.many == max[i]->class.many)
              if (max[i]->class.many) {
