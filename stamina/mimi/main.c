...
while ((
c = util_getopt(argc, argv, "tcrhSRCMPs:m:b:v:o:")
) != EOF) {
switch (c) {
case 's':
if ((user.opt.
solution = atoi(util_optarg)
) > 3)
user.opt.
solution = 1;
break;
...
(void)
strcpy(user
.oname, util_optarg/*,strlen(util_optarg)*/);
break;
case 'r':
user.cmd.
merge = 1;
...
(void)
strcpy(FSM_fileName, argv[util_optind]
);
user.
fname = FSM_fileName;
/***
...
(void)
fprintf(stderr,
"   -s n\t\tminimization heuristics(1,2,3)\n");
(void)
fprintf(stderr,
"   -m n\t\tmapping heuristics(1,2,3,4)\n");
(void)
fprintf(stderr,
