      ...
      if (not_closed) {
        ...
        for (k = 0; k < max[i]->class.imply[j]->num; k++) {
          ...
          if (iso[iso_id].uvoid) {
            ...
            /*
                                                            if
               (iso[iso[iso_id].num].uvoid) panic("UVOID");
            */
          } else {
...
select_one_max(maxid, uncovered, base) struct sbase *base;
{
  register i, j;
  int max_id;
  int newid;
  ...
  for (j = 0; j < big; j++) { /* How many in a maximal compatible */
    ...
    max[user.stat.n_max]->state[base[j].where] = (int)iso[base[j].state].list;
...
contained(c_imply, prime_id) PRIME *prime_id;
IMPLIED *c_imply;
{
  register k, l;
  int not_match;
        ...
        while (shrinked) {
                ...
                foreach_prime(i,b_cover) {
                        ...
                        foreach_state(j,prime[b_cover[i]]) {
                                ...
                                if ((all_states[prime[b_cover[i]]->state[j]] ==
1)
...
{ not_closed = 1; for (m=0; m<user.stat.nstates; m++) { if (i == m) continue; if
(contained(prime[b_cover[i]]->class.imply[k], prime[b_cover[m]])) { not_closed =
0; break;
                                                }
                                        }
                                        if (not_closed) {
...
closed\n"); prime[b_cover[i]]->class.imply = save_imply;
                                                prime[b_cover[i]]->class.many =
save_many; prime[b_cover[i]]->state[prime[b_cover[i]]->num++] = save_state;
                                                break;
                                        }
                                }
                                if (!not_closed) {
...
get_implied_by_other_prime(prime_no, x_imp) int *x_imp;
{
  register i, j, k;
  NLIST *link;
  extern NLIST *_head;
                                  ...
                                  if (contained(&flow_imply,prime[prime_no])) {
                                          for (j=0; b_cover[j] != -1; j++) {
                                                  if (b_cover[j] == prime_no)
                                                          continue;
                                                  if
     (contained(&flow_imply,prime[b_cover[j]])) { if (count++ > 1) break;
                                                  }
                                          }
                                          if (count < 2) {
                                                  for (k=0; k<flow_imply.num;
     k++) if (!x_imp[flow_imply.state[k]]) { x_imp[flow_imply.state[k]]++;
                                                                  (void)
     printf("Don't remove\n");
                                                          }
                                          }
      ...
      if (contained(prime[b_cover[i]]->class.imply[j], prime[prime_no])) {
        ...
        foreach_prime(k, b_cover) {
          if (b_cover[k] == prime_no)
            continue;
          if (contained(prime[b_cover[i]]->class.imply[j], prime[b_cover[k]])) {
            essence = 0;
            break;
          }
        }
