...
j = 0;
j<big;
j++) {        /* How many in a maximal compatible */

max[user.stat.n_max]->state[base[j].where]=
(int ) iso[base[j].state].
list;
iso[base[j].state].
list = (int *) 0;
}
...
nstates = new_states;
user.stat.
rstates = user.stat.nstates + user.stat.ostates;
}
...
for (
j = 0;
j<prime[b_cover[i]]->class.
many;
j++) {
int essence;
if (
contained(prime[b_cover[i]]
->class.imply[j], prime[prime_no])) {
essence = 1;
...
for (
k = 0;
k<prime[b_cover[i]]->class.imply[j]->
num;
k++)
