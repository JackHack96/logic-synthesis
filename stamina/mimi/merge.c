    ...
    for (i = num_st - 2; i > -1; i--)
        for (j = num_st - 1; j > i; j--) {
            ...
            switch (input_intersection(states[i], states[j])) {
                case INCOMPATIBLE: close_stack(id, INCOMPATIBLE);
                    break;
                case DEFINIT_COMPATIBLE: close_stack(id, DEFINIT_COMPATIBLE);
                    break;
                case CONDITIONAL_COMPATIBLE: pack_stack(id);
                    break;
...
if (
intersection(edge1
->input, edge2->input, num_pi)) {

if (!
intersection(edge1
->output, edge2->output, num_po)) {
if (self) {
