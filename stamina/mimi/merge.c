      ...
      if (xstack[id].status & COMPATIBLE) {
        ...
        if (user.opt.verbose > 10)
          (void)fprintf(stderr, "Compatible pair %d : %s %s : %x\n", changed++,
                        states[i]->state_name, states[j]->state_name,
                        xstack[id].status);
      }
      id++;
    }
...
input_intersection(state1, state2) STATE *state1, *state2;
{
  EDGE *edge1, *edge2;
  char *input1, *input2;
  int self;
        ...
        if (!intersection(edge1->output, edge2->output, num_po)) {
          ...
          if (self) {
            ...
            /*
               ...
               %s\n",edge1->p_state->state_name); printf("input %s
               %s\n",edge1->input,edge2->input); printf("next_state
...
intersection(cube1, cube2, bit) char cube1[], cube2[];
{
  register i;
  int inter_minterm;
...
xintersection(cube1, cube2, bit, xnode) NODE *xnode;
char cube1[], cube2[];
{
  register i, add;
