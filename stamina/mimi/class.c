...
transitive(xmax, max_index) PRIMES xmax;
{
  register i, l;
  int n_imply;

  sequ = deleted = 0;
  n_imply = column_implied(xmax[max_index]->state, xmax[max_index]->num, xmax,
                           max_index, 1);

  for (i = 0; i < n_imply; i++) {
    n_imply = column_implied(local_imply[i]->state, local_imply[i]->num, xmax,
                             max_index, 0);
  }
  ...
  if (!(xmax[max_index]->class.imply = ALLOC(IMPLIED *, n_imply)))
    panic("ALLOC class implied");
  MEMCPY(xmax[max_index]->class.imply, local_imply,
         n_imply * sizeof(IMPLIED *));
...
column_implied(state_set, number, xmax, max_index, reset) PRIMES xmax;
int *state_set;
{
  register i, l;
  S_EDGE *top;
...
set_nstate(link, max_index, r_out) NLIST *link;
char *r_out;
{
  S_EDGE *top;
  int num_implied;
...
excluded(xmax, maxim, candidate, index) PRIMES xmax;
{
  register k, l, m;
  int not_match;
  int bound, upbound;
