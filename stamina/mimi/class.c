...
for (
i = 0;
i<SMASK(xmax[max_index]->num); i++)
states[xmax[max_index]->state[i]]->
code = (char *) 1;
...
if ((num_implied > 1) && !
excluded(xmax, max_index, num_implied, sequ
)) {
if (!(local_imply[sequ] =
ALLOC(IMPLIED,
1)))
...
deleted;
if (sequ) {
if (!(xmax[max_index]->class.
imply = ALLOC(IMPLIED * , sequ)
))
...
if ((num_implied > 1) && !
excluded(xmax, max_index, num_implied, i_num
)) {
if (!(local_imply[i_num] =
ALLOC(IMPLIED,
1)))
panic("ALLOC in get_implied");
if (!(local_imply[i_num]->
...
for (
i = 0;
i<SMASK(prime[max_index]->num); i++)
states[prime[max_index]->state[i]]->
code        = (char *) 1;
top         = (S_EDGE *) link->ptr;
num_implied = 0;
exist       = 0;
