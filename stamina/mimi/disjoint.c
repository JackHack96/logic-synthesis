...
if (!(
pedge = ALLOC(S_EDGE, 1)
))
panic("Alloc in disjoint");
pedge->
p_state = nedge->p_state;
pedge->
n_state = nedge->n_state;
pedge->
n_star = nedge->n_star;
pedge->
output = nedge->output;
...
if ((xnode->
literal = single_cube_contain(xnode)
)<2) {
xnode->
right = xnode->left = NIL(NODE);
if (xnode->literal == 1) {
if (_head) {
_hend->
h_next = xnode->cubes;
...
right = xnode->left + 1;
xnode->right->
on_off = xnode->left->on_off + num_pi;
xnode->right->
cubes = xnode->left->cubes = NIL(NLIST);
