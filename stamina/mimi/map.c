          ...
          for (i = 0; i < v_next->p_star; i++) {
            ...
            if (user.opt.hmap == 4) {
              ...
              /*
                                                                              ...
                                                                              if
                 (maxi == my_weight) { if ((vtbl_state[num]*htbl_state[num]) >
  ...
  for (i = 0; i < user.stat.nstates; i++) {
    ...
    while (h_next) {
      ...
      while (h_last) {
        ...
        if ((((int *)(h_next->n_state))[0] == ((int *)(h_last->n_state))[0]) &&
  ...
  */
  ...
  for (link = _head; link; link = link->h_next) {
    ...
    while (v_next) {
      ...
      if (v_next->n_star)
        (void)fprintf(fp, "%s S%d * %s\n", v_next->input, pres, v_next->output);
      else
  ...
  for (i = 0; i < user.stat.ostates; i++) {
    ...
    while (eedge) {
      ...
      if (eedge->n_star)
        (void)fprintf(fp, "%s S%d * %s\n", eedge->input, user.stat.nstates + i,
                      eedge->output);
      else
        (void)fprintf(fp, "%s S%d S%d %s\n", eedge->input,
                      user.stat.nstates + i, (int)eedge->n_state,
                      eedge->output);
      eedge = eedge->next;
    }
  }
...
incomp_row_sum(wstate, product) EDGE *product;
{
  int i, w;
  EDGE *h_next;
...
incomp_column_sum(eedge) EDGE *eedge;
{
  int i, j;
  NLIST *link;
    ...
    while (tedge) {
      ...
      if (!tedge->n_star)
        if (w = intersection(eedge->input, tedge->input, num_pi)) {
          ...
          case 3:
          case 4:
            w = literal_intersect(eedge->input, tedge->input, num_pi, '-');
            w += literal_intersect(eedge->output, tedge->output, num_po, '1');
            break;
          }
          if (eedge->p_star > 1) {
            for (j = 0; j < eedge->p_star; j++)
...
cube_weight(cube, factor) char *cube;
{
  register i, weight;
...
column_sum(output, link, flag, column_weight) char *output;
NLIST *link;
{
  register i, j;
  P_EDGE *v_next;
        ...
        if (!eedge->n_star) {
          if (eedge->p_star)
            if (w = intersection(link->name, eedge->input, num_pi)) {
              ...
              case 3:
              case 4:
                w = literal_intersect(output, eedge->output, num_po, '1');
                w += literal_intersect(link->name, eedge->input, num_pi, '-');
                break;
              default:
                break;
...
row_sum(product, wstate) P_EDGE *product;
{
  int i;
  P_EDGE *h_next;
...
conflict(str1, str2) char str1[], str2[];
{
  register i;
...
literal_intersect(str1, str2, width, xchar) char *str1, *str2;
char xchar;
{
  register i;
  int count;
