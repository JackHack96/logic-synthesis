...
pcover map_cover_to_unate(T) pcube *T;
{
  ...
  foreachi_set(A, i, p) { (void)set_clear(p, A->sf_size); }
  ncol = 0;
...
pset_family unate_complement(A) pset_family A; /* disposes of A */
{
  ...
  if (A->count == 0) {
    ...
    if (min_set_ord == 0) {
      ...
      sf_free(A);
      foreachi_set(Abar, i, p) { set_or(p, p, rstrict); }
    ...
    while (n > 1 && (stack[n - 1].level == stack[n - 2].level || p == nlast)) {
      ...
      if (debug & MINCOV && lvl < 10) {
        printf("# EXACT_MINCOV[%d]: %4d = %4d x %4d, time = %s\n", lvl,
               temp->count, stack[n - 1].sf->count, stack[n - 2].sf->count,
 ...
 *  1 / (set_ord(p) - 1).
 */
static int abs_select_restricted(A, rstrict) pset_family A;
pset rstrict;
{
