...
while ((
i = getopt(argc, argv, "D:S:de:o:r:stv:x")
) != EOF) {
switch (i) {
case 'D':        /* -Dcommand invokes a subcommand */
for (
j = 0;
option_table[j].name != 0; j++) {
if (
strcmp(optarg, option_table[j]
.name) == 0) {
option = j;
break;
}
...
for (
j = 0;
esp_opt_table[j].name != 0; j++) {
if (
strcmp(optarg, esp_opt_table[j]
.name) == 0) {
*(esp_opt_table[j].variable) = esp_opt_table[j].
value;
break;
}
}
...
for (
j = 0;
debug_table[j].name != 0; j++) {
if (
strcmp(optarg, debug_table[j]
.name) == 0) {
debug |= debug_table[j].
value;
break;
...
getPLA(optind
++, argc, argv, option, &PLA, out_type);
getPLA(optind
++, argc, argv, option, &PLA1, out_type);
break;
case 1:
...
read_pla(last_fp, TRUE, TRUE, pla_type, &PLA) != EOF);
exit(0);
}
...
case KEY_super_gasp:
PLA->
F = super_gasp(PLA->F, PLA->D, PLA->R, &cost);
break;

case KEY_gasp:
PLA->
F = last_gasp(PLA->F, PLA->D, PLA->R, &cost);
break;

case KEY_make_sparse:       /* make_sparse step of Espresso */
PLA->
F = make_sparse(PLA->F, PLA->D, PLA->R);
break;
...
Fold = sf_save(PLA->F);
PLA->
F = minimize_exact(PLA->F, PLA->D, PLA->R, exact_cover);
EXECUTE(error = verify(PLA->F, Fold, PLA->D), VERIFY_TIME, PLA->F, cost);
if (error) {
print_solution = FALSE;
PLA->
...
PLA->
F = sf_dupl(unravel_range(PLA->F, first, last));
break;
...
if (summary || trace) {
print_trace(PLA
->F, option_table[option].name, ptime()
...
if (
read_pla(fp, needs_dcset, needs_offset, input_type, PLA
) == EOF) {
(void)
fprintf(stderr,
...
option_table[j].name != 0; j++)
if (
strcmp(argv[i + 1], option_table[j]
.name) == 0) {
*
option = j;
