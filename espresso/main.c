...
char *argv[];
{
  ...
  case 2:
    if (optind + 2 < argc)
      fatal("trailing arguments on command line");
  ...
  case KEY_primes: /* generate all prime implicants */
    EXEC(PLA->F = primes_consensus(cube2list(PLA->F, PLA->D)), "PRIMES     ",
         PLA->F);
    break;
  ...
  /* Print total runtime */
  if (summary || trace) {
    print_trace(PLA->F, option_table[option].name,
                ptime()
  ...
  *option = 0;
  for (i = 1; i < (*argc) - 1; i++) {
    if (strcmp(argv[i], "-do") == 0) {
      ...
      if (option_table[j].name == 0) {
        (void)fprintf(stderr, "espresso: bad keyword \"%s\" following -do\n",
                      argv[i + 1]);
        exit(1);
      }
      break;
  ...
  for (i = 1; i < (*argc) - 1; i++) {
    if (strcmp(argv[i], "-out") == 0) {
      ...
      if (pla_types[j].key == 0) {
        (void)fprintf(stderr, "espresso: bad keyword \"%s\" following -out\n",
                      argv[i + 1]);
        exit(1);
      }
      break;
  ...
  if (check_arg(argc, argv, "-fdr"))
    input_type = FDR_type;
  if (check_arg(argc, argv, "-fr"))
    input_type = FR_type;
